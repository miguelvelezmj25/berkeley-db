 ...
 *
 * <p>This class extends {@link OutputStream} and its <code>write()</code> methods allow it to be
 * used as a standard output stream. In addition, it provides <code>writeFast()</code> methods that
 * are not declared to throw <code>IOException</code>. <code>IOException</code> is never thrown by
 * this class.
 *
 * @author Mark Hayes
 */
public class FastOutputStream extends OutputStream {
  ...
  /**
   * The default initial size of the buffer if no initialSize parameter is specified. This constant
   * is 100 bytes.
   */
  public static final int DEFAULT_INIT_SIZE = 100;

  /**
   * The default amount that the buffer is increased when it is full. This constant is zero, which
   * means to double the current buffer size.
   */
   ...
   *
   * @param buffer the initial buffer; will be owned by this object.
   * @param bumpSize the amount to increment the buffer. If zero (the default), the current buffer
   *     size will be doubled when the buffer is full.
   */
  ...
  /**
   * Equivalent to <code>write(int)</code> but does not throw <code>IOException</code>.
   *
  ...
  /**
   * Equivalent to <code>write(byte[])</code> but does not throw <code>IOException</code>.
   *
  ...
  /**
   * Equivalent to <code>write(byte[],int,int)</code> but does not throw <code>IOException</code>.
   *
   * @param fromBuf the buffer to write.
  ...
  /**
   * Returns the length used in the internal buffer, i.e., the offset at which data will be written
   * next.
   *
