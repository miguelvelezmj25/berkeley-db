...
public class KeyRange {
   ...
   *
   * <p>The inclusive parameter should be true for checking a key read from the database; this will
   * require that the key is within range. When inclusive=false the key is allowed to be equal to
   * the beginKey for the range; this is used for checking a new exclusive bound of a sub-range.
   *
   * <p>Note that when inclusive=false and beginInclusive=true our check is not exactly correct
   * because in theory we should allow the key to be "one less" than the existing bound; however,
   * checking for "one less" is impossible so we do the best we can and test the bounds
   * conservatively.
   */
  ...
  /**
   * Returns whether the given key is within range with respect to the end of the range. See
   * checkBegin for details.
   */
  ...
  public static int compareBytes(
      byte[] data1, int offset1, int size1, byte[] data2, int offset2, int size2) {
  ...
  public static boolean equalBytes(
      byte[] data1, int offset1, int size1, byte[] data2, int offset2, int size2) {
    if (size1 != size2) {
      return false;
    }
  ...
  /**
   * Converts the byte array of this thang to space-separated integers, and suffixed by the record
