...
/**
 * A cursor-like interface that enforces a key range. The method signatures are actually those of
 * SecondaryCursor, but the pKey parameter may be null. It was done this way to avoid doubling the
 * number of methods.
 *
 * <p>This is not a fully general implementation of a range cursor and should not be used directly
 * by applications; however, it may evolve into a generally useful range cursor some day.
 *
 * @author Mark Hayes
 */
public class RangeCursor implements Cloneable {
  ...
  /**
   * The cursor and secondary cursor are the same object. The secCursor is null if the database is
   * not a secondary database.
   */
  private Cursor cursor;

  private SecondaryCursor secCursor;

  /** The range is always non-null, but may be unbounded meaning that it is open and not used. */
  private final KeyRange range;

  /**
   * The pkRange may be non-null only if the range is a single-key range and the cursor is a
   * secondary cursor. It further restricts the range of primary keys in a secondary database.
   */
  private final KeyRange pkRange;

  /** If the DB supported sorted duplicates, then calling Cursor.getSearchBothRange is allowed. */
  private final boolean sortedDups;

  /**
   * The privXxx entries are used only when the range is bounded. We read into these private entries
   * to avoid modifying the caller's entry parameters in the case where we read successfully but the
   * key is out of range. In that case we return NOTFOUND and we want to leave the entry parameters
   * unchanged.
   */
  ...
  /**
   * The initialized flag is set to true whenever we successfully position the cursor. It is used to
   * implement the getNext/Prev logic for doing a getFirst/Last when the cursor is not initialized.
   * We can't rely on Cursor to do that for us, since if we position the underlying cursor
   * successfully but the key is out of range, we have no way to set the underlying cursor to
   * uninitialized. A range cursor always starts in the uninitialized state.
   */
  ...
  /**
   * Create a cloned range cursor. The caller must clone the underlying cursor before using this
   * constructor, because cursor open/close is handled specially for CDS cursors outside this class.
   */
  ...
  /**
   * When an unbounded range is used, this method is called to use the callers entry parameters
   * directly, to avoid the extra step of copying between the private entries and the caller's
   * entries.
   */
  ...
  /**
   * Dups the cursor, sets the cursor and secCursor fields to the duped cursor, and returns the old
   * cursor. Always call endOperation in a finally clause after calling beginOperation.
   *
   * <p>If the returned cursor == the cursor field, the cursor is uninitialized and was not duped;
   * this case is handled correctly by endOperation.
   */
  ...
  /**
   * If the operation succeeded, leaves the duped cursor in place and closes the oldCursor. If the
   * operation failed, moves the oldCursor back in place and closes the duped cursor. oldCursor may
   * be null if beginOperation was not called, in cases where we don't need to dup the cursor.
   * Always call endOperation when a successful operation ends, in order to set the initialized
   * field.
   */
  private void endOperation(
      Cursor oldCursor, OpResult result, DatabaseEntry key, DatabaseEntry pKey, DatabaseEntry data)
  ...
  /**
   * Swaps the contents of the two entries. Used to return entry data to the caller when the
  ...
  /**
   * Shares the same byte array, offset and size between two entries. Used when copying the entry
   * data is not necessary because it is known that the underlying operation will not modify the
   * entry, for example, with getSearchKey.
   */
  ...
  public OpResult getFirst(
      DatabaseEntry key, DatabaseEntry pKey, DatabaseEntry data, OpReadOptions options)
      throws DatabaseException {
  ...
  /**
   * This method will restart the operation when a key range is used and an insertion at the end of
   * the key range is performed in another thread. The restarts are needed because a sequence of
   * cursor movements is performed, and serializable isolation cannot be relied on to prevent
   * insertions in other threads. Without the restarts, getLast could return NOTFOUND when keys in
   * the range exist. This may only be an issue for JE since it uses record locking, while DB core
   * uses page locking.
   */
  ...
  /**
   * Performs getLast operation when a main key range is specified but pkRange is null. Does
   * everything but the final checks for key in range, i.e., when SUCCESS is returned the caller
   * should do the final check.
   */
  ...
  /**
   * Performs getLast operation when both a main key range (which must be a single key range) and a
   * pkRange are specified. Does everything but the final checks for key and pKey in range, i.e.,
   * when SUCCESS is returned the caller should do the final two checks.
   */
  ...
  public OpResult getSearchKeyRange(
      DatabaseEntry key, DatabaseEntry pKey, DatabaseEntry data, OpReadOptions options)
      throws DatabaseException {
    ...
    try {
      ...
      result = doGetSearchKeyRange(options);
      if (result.isSuccess()
          && (!range.check(privKey) || (pkRange != null && !pkRange.check(pKey)))) {
        result = OpResult.FAILURE;
      }
    } finally {
      endOperation(oldCursor, result, key, pKey, data);
  ...
  public OpResult getSearchBothRange(
      DatabaseEntry key, DatabaseEntry pKey, DatabaseEntry data, OpReadOptions options)
      throws DatabaseException {
  ...
  public OpResult getSearchRecordNumber(
      DatabaseEntry key, DatabaseEntry pKey, DatabaseEntry data, OpReadOptions options)
      throws DatabaseException {
    ...
    if (secCursor != null && privPKey != null) {
      ...
      if (DbCompat.IS_JE) {
        return OpResult.make(
            secCursor.get(privKey, privPKey, privData, Get.FIRST, options.jeOptions));
      }
      /* <!-- end JE only --> */
  ...
  private OpResult doGetLast(OpReadOptions options) throws DatabaseException {
    ...
    if (secCursor != null && privPKey != null) {
      ...
      if (DbCompat.IS_JE) {
        return OpResult.make(
            secCursor.get(privKey, privPKey, privData, Get.LAST, options.jeOptions));
      }
      /* <!-- end JE only --> */
  ...
  private OpResult doGetNext(OpReadOptions options) throws DatabaseException {
    ...
    if (secCursor != null && privPKey != null) {
      ...
      if (DbCompat.IS_JE) {
        return OpResult.make(
            secCursor.get(privKey, privPKey, privData, Get.NEXT, options.jeOptions));
      }
      /* <!-- end JE only --> */
  ...
  private OpResult doGetNextDup(OpReadOptions options) throws DatabaseException {
    ...
    if (secCursor != null && privPKey != null) {
      ...
      if (DbCompat.IS_JE) {
        return OpResult.make(
            secCursor.get(privKey, privPKey, privData, Get.NEXT_DUP, options.jeOptions));
      }
      /* <!-- end JE only --> */
      return OpResult.make(
          secCursor.getNextDup(privKey, privPKey, privData, options.getLockMode()));
    } else {
      /* <!-- begin JE only --> */
      if (DbCompat.IS_JE) {
  ...
  private OpResult doGetNextNoDup(OpReadOptions options) throws DatabaseException {
    ...
    if (secCursor != null && privPKey != null) {
      ...
      if (DbCompat.IS_JE) {
        return OpResult.make(
            secCursor.get(privKey, privPKey, privData, Get.NEXT_NO_DUP, options.jeOptions));
      }
      /* <!-- end JE only --> */
      return OpResult.make(
          secCursor.getNextNoDup(privKey, privPKey, privData, options.getLockMode()));
    } else {
      /* <!-- begin JE only --> */
      if (DbCompat.IS_JE) {
  ...
  private OpResult doGetPrev(OpReadOptions options) throws DatabaseException {
    ...
    if (secCursor != null && privPKey != null) {
      ...
      if (DbCompat.IS_JE) {
        return OpResult.make(
            secCursor.get(privKey, privPKey, privData, Get.PREV, options.jeOptions));
      }
      /* <!-- end JE only --> */
  ...
  private OpResult doGetPrevDup(OpReadOptions options) throws DatabaseException {
    ...
    if (secCursor != null && privPKey != null) {
      ...
      if (DbCompat.IS_JE) {
        return OpResult.make(
            secCursor.get(privKey, privPKey, privData, Get.PREV_DUP, options.jeOptions));
      }
      /* <!-- end JE only --> */
      return OpResult.make(
          secCursor.getPrevDup(privKey, privPKey, privData, options.getLockMode()));
    } else {
      /* <!-- begin JE only --> */
      if (DbCompat.IS_JE) {
  ...
  private OpResult doGetPrevNoDup(OpReadOptions options) throws DatabaseException {
    ...
    if (secCursor != null && privPKey != null) {
      ...
      if (DbCompat.IS_JE) {
        return OpResult.make(
            secCursor.get(privKey, privPKey, privData, Get.PREV_NO_DUP, options.jeOptions));
      }
      /* <!-- end JE only --> */
      return OpResult.make(
          secCursor.getPrevNoDup(privKey, privPKey, privData, options.getLockMode()));
    } else {
      /* <!-- begin JE only --> */
      if (DbCompat.IS_JE) {
  ...
  private OpResult doGetSearchKey(OpReadOptions options) throws DatabaseException {
    ...
    if (secCursor != null && privPKey != null) {
      ...
      if (DbCompat.IS_JE) {
        return OpResult.make(
            secCursor.get(privKey, privPKey, privData, Get.SEARCH, options.jeOptions));
      }
      /* <!-- end JE only --> */
      return OpResult.make(
          secCursor.getSearchKey(privKey, privPKey, privData, options.getLockMode()));
    } else {
      /* <!-- begin JE only --> */
      if (DbCompat.IS_JE) {
  ...
  private OpResult doGetSearchKeyRange(OpReadOptions options) throws DatabaseException {
    ...
    if (secCursor != null && privPKey != null) {
      ...
      if (DbCompat.IS_JE) {
        return OpResult.make(
            secCursor.get(privKey, privPKey, privData, Get.SEARCH_GTE, options.jeOptions));
      }
      /* <!-- end JE only --> */
      return OpResult.make(
          secCursor.getSearchKeyRange(privKey, privPKey, privData, options.getLockMode()));
    } else {
      /* <!-- begin JE only --> */
      if (DbCompat.IS_JE) {
  ...
  private OpResult doGetSearchBoth(OpReadOptions options) throws DatabaseException {
    ...
    if (secCursor != null && privPKey != null) {
      ...
      if (DbCompat.IS_JE) {
        return OpResult.make(
            secCursor.get(privKey, privPKey, privData, Get.SEARCH_BOTH, options.jeOptions));
      }
      /* <!-- end JE only --> */
      return OpResult.make(
          secCursor.getSearchBoth(privKey, privPKey, privData, options.getLockMode()));
    } else {
      /* <!-- begin JE only --> */
      if (DbCompat.IS_JE) {
  ...
  private OpResult doGetSearchBothRange(OpReadOptions options) throws DatabaseException {
    ...
    if (secCursor != null && privPKey != null) {
      ...
      if (DbCompat.IS_JE) {
        return OpResult.make(
            secCursor.get(privKey, privPKey, privData, Get.SEARCH_BOTH_GTE, options.jeOptions));
      }
      /* <!-- end JE only --> */
      return OpResult.make(
          secCursor.getSearchBothRange(privKey, privPKey, privData, options.getLockMode()));
    } else {
      /* <!-- begin JE only --> */
      if (DbCompat.IS_JE) {
    ...
    if (secCursor != null && privPKey != null) {
      return OpResult.make(
          DbCompat.getSearchRecordNumber(
              secCursor, privKey, privPKey, privData, options.getLockMode()));
    } else {
      return OpResult.make(
          DbCompat.getSearchRecordNumber(cursor, privKey, privData, options.getLockMode()));
    }
  }
  ...
  /**
   * If the database is a RECNO or QUEUE database, we know its keys are record numbers. We treat a
   * non-positive record number as out of bounds, that is, we return NOTFOUND rather than throwing
   * IllegalArgumentException as would happen if we passed a non-positive record number into the DB
   * cursor. This behavior is required by the collections interface.
   */
