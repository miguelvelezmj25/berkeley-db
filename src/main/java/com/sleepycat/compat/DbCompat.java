 ...
 *
 * <p>Two versions of this class, with the same public interface but different implementations, are
 * maintained in parallel in the DB and JE source trees. By the use of the constants and methods in
 * this class, along with a script that moves the source code from JE to DB, the source code in
 * certain packages is kept "portable" and is shared by the two products. The script translates the
 * package names from com.sleepycat.je to com.sleepycat.db, and perform other fix-ups as described
 * further below.
 *
 * <p>The JE directories that contain portable code are:
 *
 * <p>src/com/sleepycat/bind /collections /persist /util test/com/sleepycat/bind /collections
 * /persist /util
 *
 * <p>In DB, these sources are stored in the following locations:
 *
 * <p>Sources: src/java Tests: test/java/compat
 *
 * <p>To keep this source code portable there are additional coding rules, above and beyond the
 * standard rules (such as coding style) for all JE code.
 *
 * <p>+ In general we should try to use the JE/DB public API, since it is usually the same or
 * similar in both products. If we use internal APIs, they will always be different and will require
 * special handling.
 *
 * <p>+ When there are differences between products, the first choice for handling the difference is
 * to use a DbCompat static method or constant. This keeps the source code the same for both
 * products (except in this DbCompat class).
 *
 * <p>+ When JE-only code is needed -- for example, some APIs only exist in JE, and special handling
 * of JE exceptions is sometimes needed -- the following special comment syntax can be used to
 * bracket the JE-only code:
 * <!-- begin JE only -->
 * JE-only code goes here
 * <!-- end JE only -->
 * This syntax must be used inside of a comment: either inside a javadoc section as shown above, or
 * inside a single-line comment (space before last slash is to prevent ending this javadoc comment):
 *
 * <p>/*
 * <!-- begin JE only -->
 * * / JE-only code goes here /*
 ...
 * * /
 *
 * <p>All lines between the
 * <!-- begin JE only -->
 * and
 * <!-- end JE only -->
 * lines, and including these lines, will be removed by the script that transfers code from JE to
 * DB.
 *
 * <p>+ When DB-only code is needed, the code will exist in the JE product but will never be
 * executed. For DB-only APIs, we hide the API from the user with the @hidden javadoc tag.
 * The @hidden tag is ignored on the DB side. We do not have a way to remove DB-only code completely
 * from the JE product, because we do not use a proprocessor for building JE.
 *
 * <p>+ Because DatabaseException (and all subclasses) are checked exceptions in DB but runtime
 * exceptions in JE, we cannot omit the 'throws' declaration. Another difference is that DB normally
 * throws DatabaseException for all errors, while JE has many specific subclasses for specific
 * errors. Therefore, any method that calls a DB API method (for example, Database.get or put) will
 * have a "throws DatabaseException" clause.
 *
 * <p>+ Special consideration is needed for the @throws clauses in javadoc. We do want to javadoc
 * the JE-only exceptions that are thrown, so the @throws for these exceptions should be inside the
 * "begin/end JE only" brackets. We also need to document the fact that DB may throw
 * DatabaseException for almost any method, so we do that with a final @throws clause that looks
 * like this:
 *
 * @throws DatabaseException the base class for all BDB exceptions.
 *     <p>This is a compromise. JE doesn't throw this exception, but we've described it in a way
 *     that still makes some sense for JE, sort of.
 *     <p>+ Other special handling can be implemented in the transfer script, which uses SED. Entire
 *     files can be excluded from the transfer, for example, the JE-only exception classes. Name
 *     changes can also be made using SED, for example: s/LockConflictException/DeadlockException/.
 *     See the db/dist/s_je2db script for details.
 */
public class DbCompat {
  ...
  /**
   * For read-only cursor operations on a replicated node, we must use a transaction to satisfy HA
   * requirements. However, we use a Durability that avoids consistency checks on the Master, and we
   * use ReadCommitted isolation since that gives the same behavior as a non-transactional cursor:
   * locks are released when the cursor is moved or closed.
   */
  ...
  public static OperationStatus getCurrentRecordNumber(
      Cursor cursor, DatabaseEntry key, LockMode lockMode) throws DatabaseException {

    throw new UnsupportedOperationException();
  }

  public static OperationStatus getSearchRecordNumber(
      Cursor cursor, DatabaseEntry key, DatabaseEntry data, LockMode lockMode)
  ...
  public static OperationStatus append(
      Database db, Transaction txn, DatabaseEntry key, DatabaseEntry data) {
    throw new UnsupportedOperationException();
  }
  ...
  public static boolean setImportunate(final Transaction txn, final boolean importunate) {
  ...
  /**
   * Returns null if the database is not found (and AllowCreate is false) or already exists (and
   * ExclusiveCreate is true).
   */
  public static Database openDatabase(
      Environment env, Transaction txn, String fileName, String dbName, DatabaseConfig config) {
    assert fileName == null;
    try {
  ...
  public static boolean truncateDatabase(
      Environment env, Transaction txn, String fileName, String dbName) {
    assert fileName == null;
    try {
  ...
  /** Returns false if the database is not found. */
  public static boolean renameDatabase(
      Environment env,
      Transaction txn,
      String oldFileName,
      String oldDbName,
  ...
  /**
   * Fires an assertion if the database is not found (and AllowCreate is false) or already exists
   * (and ExclusiveCreate is true).
   */
  public static Database testOpenDatabase(
      Environment env, Transaction txn, String file, String name, DatabaseConfig config) {
    try {
   ...
   * (and ExclusiveCreate is true).
   */
  public static SecondaryDatabase testOpenSecondaryDatabase(
      Environment env,
      Transaction txn,
      String file,
      String name,
      ...
      SecondaryConfig config) {
    try {
      return env.openSecondaryDatabase(txn, makeTestDbName(file, name), primary, config);
    } catch (DatabaseNotFoundException e) {
      assert false;
      return null;
    } catch (DatabaseExistsException e) {
  ...
  public static void enableDeadlockDetection(EnvironmentConfig envConfig, boolean isCDB) {
    // do nothing in JE, deadlock detection is always on
  }

  public static Object getErrorHandler(Environment env) throws DatabaseException {

    return null;
  }

  public static void setErrorHandler(Environment env, Object errHandler) throws DatabaseException {}
  ...
  public static class OpReadOptions {
    ...
    public static OpReadOptions make(ReadOptions options) {
      return (options != null) ? new OpReadOptions(options) : EMPTY;
    }
  ...
  public static class OpWriteOptions {
    ...
    public static OpWriteOptions make(WriteOptions options) {
      return (options != null) ? new OpWriteOptions(options) : EMPTY;
    }
  }
}
