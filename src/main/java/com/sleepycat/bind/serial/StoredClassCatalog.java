 ...
 *
 * <p>A single <code>StoredClassCatalog</code> object is normally used along with a set of databases
 * that stored serialized objects.
 *
 * @author Mark Hayes
...
public class StoredClassCatalog implements ClassCatalog {
  ...
  /**
   * Creates a catalog based on a given database. To save resources, only a single catalog object
   * should be used for each unique catalog database.
   *
   * @param database an open database to use as the class catalog. It must be a BTREE database and
   *     must not allow duplicates.
   * @throws DatabaseException if an error occurs accessing the database.
   * @throws IllegalArgumentException if the database is not a BTREE database or if it configured to
   *     allow duplicates.
   */
  public StoredClassCatalog(Database database) throws DatabaseException, IllegalArgumentException {
    ...
    writeLockMode =
        (DbCompat.getInitializeLocking(envConfig) || envConfig.getTransactional())
            ? LockMode.RMW
            : LockMode.DEFAULT;
  ...
  /**
   * Internal function for getting the class format. Allows passing the DatabaseEntry object for the
   * data, so the bytes of the class format can be examined afterwards.
   */
  ...
  /**
   * Get the ClassInfo for a given class name, adding it and its ObjectStreamClass to the database
   * if they are not already present, and caching both of them using the class info and class format
   * maps. When a class is first loaded from the database, the stored ObjectStreamClass is compared
   * to the current ObjectStreamClass loaded by the Java class loader; if they are different, a new
   * class ID is assigned for the current format.
   */
  ...
  /**
   * Assign a new class ID (increment the current ID record), write the ObjectStreamClass record for
   * this new ID, and update the ClassInfo record with the new ID also. The ClassInfo passed as an
   * argument is the one to be updated.
   */
  private ClassInfo putClassInfo(
      ClassInfo classInfo, String className, DatabaseEntry classKey, ObjectStreamClass classFormat)
  ...
  /**
   * Holds the class format key for a class, maintains a reference to the ObjectStreamClass. Other
   * fields can be added when we need to store more information per class.
   */
  ...
  /**
   * Return whether two class formats are equal. This determines whether a new class format is
   * needed for an object being serialized. Formats must be identical in all respects, or a new
   * format is needed.
   */
  private static boolean areClassFormatsEqual(
      ObjectStreamClass format1, byte[] format1Bytes, ObjectStreamClass format2) {
    try {
  ...
  /**
   * For BDB JE, returns the ClassLoader property of the catalog database environment. This ensures
   * that the Environment's ClassLoader property is used for loading all user-supplied classes.
   *
   * <p>For BDB, this method returns null because no Environment ClassLoader property is available.
   * This method may be overridden to return a ClassLoader.
   */
