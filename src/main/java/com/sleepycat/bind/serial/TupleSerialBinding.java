...
/**
 * An abstract <code>EntityBinding</code> that treats an entity's key entry as a tuple and its data
 * entry as a serialized object.
 *
 * <p>This class takes care of serializing and deserializing the data entry, and converting the key
 * entry to/from {@link TupleInput} and {@link TupleOutput} objects. Its three abstract methods must
 * be implemented by a concrete subclass to convert these objects to/from an entity object.
 *
 * <ul>
 *   <li>{@link #entryToObject(TupleInput,Object)}
 *   <li>{@link #objectToKey(Object,TupleOutput)}
...
public abstract class TupleSerialBinding<D, E> extends TupleBase implements EntityBinding<E> {
  ...
  /**
   * Constructs an entity object from {@link TupleInput} key entry and deserialized data entry
   * objects.
   *
   ...
   * @param keyOutput is the {@link TupleOutput} to which the key should be written.
   */
