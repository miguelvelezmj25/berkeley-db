...
/**
 * A abstract key creator that uses a serial key and a serial data entry. This class takes care of
 * serializing and deserializing the key and data entry automatically. The following abstract method
 * must be implemented by a concrete subclass to create the index key using these objects
 *
 * <ul>
 *   <li>{@link #createSecondaryKey(Object,Object)}
 * </ul>
 *
...
public abstract class SerialSerialKeyCreator<PK, D, SK>
  ...
  public SerialSerialKeyCreator(
    ...
    this(
        new SerialBinding<PK>(classCatalog, primaryKeyClass),
   ...
   */
  public SerialSerialKeyCreator(
      SerialBinding<PK> primaryKeyBinding,
  ...
  public boolean createSecondaryKey(
    ...
    PK primaryKeyInput = primaryKeyBinding.entryToObject(primaryKeyEntry);
   ...
   *
   * @param primaryKey is the deserialized source primary key entry, or null if no primary key entry
   *     is used to construct the index key.
   * @param data is the deserialized source data entry, or null if no data entry is used to
   *     construct the index key.
   * @return the destination index key object, or null to indicate that the key is not present.
   */
