...
/**
 * A concrete <code>TupleBinding</code> for a sorted <code>Long</code> primitive wrapper or a sorted
 * <code>long</code> primitive, that stores the value in the smallest number of bytes possible.
 *
 ...
 *       used with any stored collection.
 * </ol>
 *
...
public class SortedPackedLongBinding extends TupleBinding<Long> {
  ...
  /**
   * Converts a simple <code>Long</code> value into an entry buffer, using SortedPackedLong format.
   *
  ...
  /**
   * Returns a tuple output object of the maximum size needed, to avoid wasting space when a single
   * primitive is output.
   */
