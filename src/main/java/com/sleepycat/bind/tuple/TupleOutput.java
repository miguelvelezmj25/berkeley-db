...
/**
 * An <code>OutputStream</code> with <code>DataOutput</code>-like methods for writing tuple fields.
 * It is used by <code>TupleBinding</code>.
 *
 * <p>This class has many methods that have the same signatures as methods in the {@link
 * java.io.DataOutput} interface. The reason this class does not implement {@link
 * java.io.DataOutput} is because it would break the interface contract for those methods because of
...
public class TupleOutput extends FastOutputStream {
  ...
  /**
   * We represent a null string as a single FF UTF character, which cannot occur in a UTF encoded
   * string.
   */
  ...
  /**
   * Creates a tuple output object for writing a byte array of tuple data, using a given buffer. A
   * new buffer will be allocated only if the number of bytes needed is greater than the length of
   * this buffer. A reference to the byte array will be kept by this object and therefore the byte
   ...
   *
   * @param val is the string containing the values to be written. Only characters with values below
   *     0x100 may be written using this method, since the high-order 8 bits of all characters are
   *     discarded.
  ...
  /**
   * Writes the specified characters to the buffer, converting each character to a two byte unsigned
   * value. Writes values that can be read using {@link TupleInput#readChars}.
   *
  ...
  /**
   * Writes the specified characters to the buffer, converting each character to UTF format, and
   * adding a null terminator byte. Writes values that can be read using {@link
  ...
  /**
   * Writes a boolean (one byte) unsigned value to the buffer, writing one if the value is true and
   * zero if it is false. Writes values that can be read using {@link TupleInput#readBoolean}.
   *
   * @param val is the value to write to the buffer.
   * @return this tuple output object.
   * @see <a href="package-summary.html#integerFormats">Integer Formats</a>
  ...
  /**
   * Writes an unsorted double (eight byte) value to the buffer. Writes values that can be read
   * using {@link TupleInput#readDouble}.
   *
   * @param val is the value to write to the buffer.
   * @return this tuple output object.
   * @see <a href="package-summary.html#floatFormats">Floating Point Formats</a>
   ...
   * Writes values that can be read using {@link TupleInput#readBytes}.
   *
   * @param chars is the array of values to be written. Only characters with values below 0x100 may
  ...
  /**
   * Writes the specified characters to the buffer, converting each character to UTF format. Writes
   * values that can be read using {@link TupleInput#readString(int)} or {@link
   * TupleInput#readString(char[])}.
   *
   * @param chars is the array of characters to be written.
   * @return this tuple output object.
   * @throws NullPointerException if the chars parameter is null.
   ...
   * @throws IllegalArgumentException if the byte array representation of val is larger than 0x7fff
   *     bytes.
   * @see <a href="package-summary.html#integerFormats">Integer Formats</a>
   */
  ...
  /**
   * Returns the exact byte length that would would be output for a given {@code BigInteger} value
   * if {@link TupleOutput#writeBigInteger} were called.
   *
