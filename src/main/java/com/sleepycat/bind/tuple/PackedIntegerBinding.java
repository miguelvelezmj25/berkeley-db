...
/**
 * A concrete <code>TupleBinding</code> for an unsorted <code>Integer</code> primitive wrapper or an
 * unsorted <code>int</code> primitive, that stores the value in the smallest number of bytes
 * possible.
 *
...
public class PackedIntegerBinding extends TupleBinding<Integer> {
  ...
  /**
   * Converts a simple <code>int</code> value into an entry buffer, using PackedInteger format.
   *
