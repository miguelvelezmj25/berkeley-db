...
/**
 * A concrete <code>TupleTupleBinding</code> that delegates to the <code>MarshalledTupleEntry</code>
 * and <code>MarshalledTupleKeyEntity</code> interfaces of the entity class.
 *
 * <p>This class calls the methods of the {@link MarshalledTupleEntry} interface to convert between
 * the data entry and entity object. It calls the methods of the {@link MarshalledTupleKeyEntity}
 * interface to convert between the key entry and the entity object. These two interfaces must both
 * be implemented by the entity class.
 *
 * @author Mark Hayes
 */
public class TupleTupleMarshalledBinding<E extends MarshalledTupleEntry & MarshalledTupleKeyEntity>
   ...
   *
   * <p>The given class is used to instantiate entity objects using {@link Class#newInstance}, and
   * therefore must be a public class and have a public no-arguments constructor. It must also
   * implement the {@link MarshalledTupleEntry} and {@link MarshalledTupleKeyEntity} interfaces.
   *
  ...
  public TupleTupleMarshalledBinding(Class<E> cls) {
    ...
    if (!MarshalledTupleKeyEntity.class.isAssignableFrom(cls)) {
      throw new IllegalArgumentException(
          cls + " does not implement MarshalledTupleKeyEntity");
    }
    if (!MarshalledTupleEntry.class.isAssignableFrom(cls)) {
      throw new IllegalArgumentException(
          cls + " does not implement MarshalledTupleEntry");
    }
  }
