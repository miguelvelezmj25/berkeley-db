...
/**
 * A concrete key creator that works in conjunction with a {@link TupleTupleMarshalledBinding}. This
 * key creator works by calling the methods of the {@link MarshalledTupleKeyEntity} interface to
 * create and clear the index key.
 *
 * <p>Note that a marshalled tuple key creator is somewhat less efficient than a non-marshalled key
 * tuple creator because more conversions are needed. A marshalled key creator must convert the
 * entry to an object in order to create the key, while an unmarshalled key creator does not.
 *
 * @author Mark Hayes
 */
public class TupleTupleMarshalledKeyCreator<
        E extends MarshalledTupleEntry & MarshalledTupleKeyEntity>
    extends TupleTupleKeyCreator<E> {

  private final String keyName;
  private final TupleTupleMarshalledBinding<E> binding;
   ...
   *     MarshalledTupleKeyEntity#marshalSecondaryKey} method to identify the index key.
   */
