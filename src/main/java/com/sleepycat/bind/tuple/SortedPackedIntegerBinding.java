...
/**
 * A concrete <code>TupleBinding</code> for a sorted <code>Integer</code> primitive wrapper or a
 * sorted <code>int</code> primitive, that stores the value in the smallest number of bytes
 * possible.
 *
...
public class SortedPackedIntegerBinding extends TupleBinding<Integer> {
  ...
  /**
   * Converts a simple <code>int</code> value into an entry buffer, using SortedPackedInteger
   * format.
   *
