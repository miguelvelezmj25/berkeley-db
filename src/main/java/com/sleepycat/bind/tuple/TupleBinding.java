...
/**
 * An abstract <code>EntryBinding</code> that treats a key or data entry as a tuple; it includes
 * predefined bindings for Java primitive types.
 *
 * <p>This class takes care of converting the entries to/from {@link TupleInput} and {@link
 * TupleOutput} objects. Its two abstract methods must be implemented by a concrete subclass to
 * convert between tuples and key or data objects.
 *
 * <ul>
 *   <li>{@link #entryToObject(TupleInput)}
 *   <li>{@link #objectToEntry(Object,TupleOutput)}
 * </ul>
 *
...
public abstract class TupleBinding<E> extends TupleBase<E> implements EntryBinding<E> {
   ...
   * @param output is the tuple entry to which the key or data should be written.
   */
   ...
   * </ul>
   *
   * <p><em>Note:</em> {@link #getPrimitiveBinding} returns bindings that do not sort negative
   * floating point numbers correctly by default. See {@link SortedFloatBinding} and {@link
   * SortedDoubleBinding} for details.
   *
   * @param <T> the primitive Java class.
   * @param cls the primitive Java class.
   * @return a new binding for the primitive class or null if the cls parameter is not one of the
   *     supported classes.
   */
