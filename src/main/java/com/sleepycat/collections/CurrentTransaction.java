...
/**
 * Provides access to the current transaction for the current thread within the context of a
 * Berkeley DB environment. This class provides explicit transaction control beyond that provided by
 * the {@link TransactionRunner} class. However, both methods of transaction control manage
...
public class CurrentTransaction {
  ...
  private static final WeakHashMap<Environment, CurrentTransaction> envMap =
      new WeakHashMap<Environment, CurrentTransaction>();

  private final LockMode writeLockMode;
  private final boolean cdbMode;
  private final boolean txnMode;
  private final boolean lockingMode;
  private final ThreadLocal localTrans = new ThreadLocal();
   ...
   */
  private final WeakReference<Environment> envRef;

  /**
   * Gets the CurrentTransaction accessor for a specified Berkeley DB environment. This method
   * always returns the same reference when called more than once with the same environment
   * parameter.
   *
   * @param env is an open Berkeley DB environment.
   * @return the CurrentTransaction accessor for the given environment, or null if the environment
  ...
  /**
   * Gets the CurrentTransaction accessor for a specified Berkeley DB environment. Unlike
   * getInstance(), this method never returns null.
   *
   * @param env is an open Berkeley DB environment.
   */
  ...
  /**
   * Return the LockMode.RMW or null, depending on whether locking is enabled. LockMode.RMW will
   * cause an error if passed when locking is not enabled. Locking is enabled if locking or
   * transactions were specified for this environment.
   */
  ...
  /**
   * Returns the transaction associated with the current thread for this environment, or null if no
  ...
  /**
   * Returns whether auto-commit may be performed by the collections API. True is returned if no
   * collections API transaction is currently active, and no XA transaction is currently active.
   */
  ...
  /**
   * Commits the transaction that is active for the current thread for this environment and makes
   * the parent transaction (if any) the current transaction.
   *
   * @return the parent transaction or null if the committed transaction was not nested.
   *     <!-- begin JE only -->
   * @throws com.sleepycat.je.rep.InsufficientReplicasException if the master in a replicated
   *     environment could not contact a quorum of replicas as determined by the {@link
   *     com.sleepycat.je.Durability.ReplicaAckPolicy}. The application must abort the transaction
   *     and can choose to retry it.
   ...
   *     occurs.
   *     <!-- end JE only -->
   * @throws DatabaseException if an error occurs committing the transaction. The transaction will
   *     still be closed and the parent transaction will become the current transaction.
   * @throws IllegalStateException if no transaction is active for the current thread for this
   *     environment.
   */
  ...
  /**
   * Aborts the transaction that is active for the current thread for this environment and makes the
   * parent transaction (if any) the current transaction.
   *
   * @return the parent transaction or null if the aborted transaction was not nested.
   *     <!-- begin JE only -->
   * @throws EnvironmentFailureException if an unexpected, internal or environment-wide failure
   *     occurs.
   *     <!-- end JE only -->
   * @throws DatabaseException if an error occurs aborting the transaction. The transaction will
   *     still be closed and the parent transaction will become the current transaction.
   * @throws IllegalStateException if no transaction is active for the current thread for this
   *     environment.
   */
   ...
   *
   * @param writeCursor true to open a write cursor in a CDB environment, and ignored for other
   *     environments.
  ...
  /**
   * Returns true if a CDB cursor is open and therefore a Database write operation should not be
   * attempted since a self-deadlock may result.
   */
    ...
    if (cdbMode) {
      ...
      if (cdbCursorsMap != null) {
        CdbCursors cdbCursors = (CdbCursors) cdbCursorsMap.get(db);
          ...
          return cdbCursors != null
                  && (cdbCursors.readCursors.size() > 0 || cdbCursors.writeCursors.size() > 0);
      }
    }
    return false;
  }
