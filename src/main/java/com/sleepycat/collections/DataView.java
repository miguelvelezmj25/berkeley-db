 ...
 *
 * <p>This class defines a view and takes care of reading and updating indices, calling bindings,
 * constraining access to a key range, etc.
 *
 * @author Mark Hayes
 */
final class DataView implements Cloneable {
  ...
  /**
   * Creates a view for a given database and bindings. The initial key range of the view will be
   * open.
   */
  DataView(
      Database database,
    ...
    try {
      ...
      dupsAllowed =
          DbCompat.getSortedDuplicates(dbConfig) || DbCompat.getUnsortedDuplicates(dbConfig);
  ...
  /**
   * Return a new key-set view derived from this view by setting the entity and value binding to
   * null.
   *
   ...
   * @throws KeyRangeException if the specified range is not within the current range.
   */
  ...
  DataView configuredView(CursorConfig config) {
    ...
    DataView view = cloneView();
    view.cursorConfig =
        (config != null) ? DbCompat.cloneCursorConfig(config) : CursorConfig.DEFAULT;
    return view;
  }
  ...
  private void setRange(
      Object beginKey, boolean beginInclusive, Object endKey, boolean endInclusive)
      throws DatabaseException, KeyRangeException {
  ...
  /**
   * Returns whether this is a view on a secondary database rather than directly on a primary
   * database.
   */
   ...
   *
   * <p>Auto-commit must be performed by the caller.
   */
  ...
  /**
   * Appends a value and returns the new key. If a key assigner is used it assigns the key,
   * otherwise a QUEUE or RECNO database is required.
   *
   * <p>Auto-commit must be performed by the caller.
   */
  OperationStatus append(Object value, Object[] retPrimaryKey, Object[] retValue)
  ...
  /**
   * Returns the current transaction if the database is transaction, or null if the database is not
   * transactional or there is no current transaction.
   */
   ...
   */
  DataCursor join(DataView[] indexViews, Object[] indexKeys, JoinConfig joinConfig)
      throws DatabaseException {
  ...
  /**
   * Returns a cursor for this view that reads only records having the index key values at the
   * specified cursors.
   */
  ...
  private void returnPrimaryKeyAndValue(
      DatabaseEntry keyThang, DatabaseEntry valueThang, Object[] retPrimaryKey, Object[] retValue) {
    // Requires: if retPrimaryKey, primary key binding (no index).
    // Requires: if retValue, value or entity binding
  ...
  boolean useKey(Object key, Object value, DatabaseEntry keyThang, KeyRange checkRange)
      throws DatabaseException {
    ...
    if (key != null) {
      ...
      if (!dupsView && isSecondary()) {
        ...
        secKeyCreator.createSecondaryKey(
            secDb, primaryKeyThang,
            valueThang, keyThang);
      } else {
  ...
  /**
   * Returns whether data keys can be derived from the value/entity binding of this view, which
   * determines whether a value/entity object alone is sufficient for operations that require keys.
   */
  ...
  /**
   * Populates the value entry and throws an exception if the primary key would be changed via an
   * entity binding.
   */
  ...
  KeyRange subRange(
    ...
    return useRange.subRange(
        beginThang, beginInclusive,
        endThang, endInclusive);
  }

  /**
   * Returns the range to use for sub-ranges. Returns range if this is not a dupsView, or the
   * dupsRange if this is a dupsView, creating dupsRange if necessary.
   */
