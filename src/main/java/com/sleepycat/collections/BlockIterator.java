...
/**
 * An iterator that does not need closing because a cursor is not kept open across method calls. A
 * cursor is opened to read a block of records at a time and then closed before the method returns.
 *
 * @author Mark Hayes
 */
class BlockIterator<E> extends BaseIterator<E> {
  ...
  /**
   * Slots for a block of record keys and values. The priKeys array is only used for secondary
   * databases; otherwise it is set to the keys array.
   */
  private final byte[][] keys;

  private final byte[][] priKeys;
  private final byte[][] values;

  /**
   * The slot index of the record that would be returned by next(). nextIndex is always greater or
   * equal to zero. If the next record is not available, then nextIndex is equal to keys.length or
   * keys[nextIndex] is null.
   *
   * <p>If the block is empty, then either the iterator is uninitialized or the key range is empty.
   * Either way, nextIndex will be the array length and all array values will be null. This is the
   * initial state set by the constructor. If remove() is used to delete all records in the key
   * range, it will restore the iterator to this initial state. The block must never be allowed to
   * become empty when the key range is non-empty, since then the iterator's position would be lost.
   * [#15858]
   */
  private int nextIndex;

  /**
   * The slot index of the record last returned by next() or previous(), or the record inserted by
   * add(). dataIndex is -1 if the data record is not available. If greater or equal to zero, the
   * slot at dataIndex is always non-null.
   */
  private int dataIndex;

  /**
   * The iterator data last returned by next() or previous(). This value is set to null if dataIndex
   * is -1, or if the state of the iterator is such that set() or remove() cannot be called. For
   * example, after add() this field is set to null, even though the dataIndex is still valid.
   */
  ...
  /**
   * Inserts an added record at a given slot position and shifts other slots accordingly. Also
   * adjusts nextIndex and sets dataIndex to -1.
   */
