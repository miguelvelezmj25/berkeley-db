...
/**
 * Configuration properties used with an {@link EntityStore} or {@link RawStore}.
 *
 * <p>{@code StoreConfig} objects are thread-safe. Multiple threads may safely call the methods of a
 * shared {@code StoreConfig} object.
 *
 * <p>See the <a href="package-summary.html#example">package summary example</a> for an example of
 * using a {@code StoreConfig}.
 *
 * @author Mark Hayes
 */
public class StoreConfig implements Cloneable {
  ...
  /**
   * The default store configuration containing properties as if the configuration were constructed
   * and not modified.
   */
   ...
   *
   * <p>If this property is false and the internal store metadata database does not exist, {@link
   * DatabaseException} will be thrown when the store is opened.
   *
   ...
   *
   * <p>If this property is true and the internal store metadata database already exists, {@link
   * DatabaseException} will be thrown when the store is opened.
   *
   ...
   *
   * <p>This property is true to open all store indices for transactional access. True may not be
   * specified if the environment is not also transactional.
   *
   ...
   *
   * <p>This property is true to open all store indices for read-only access, or false to open them
   * for read-write access. False may not be specified if the environment is read-only.
   *
  ...
  /* <!-- begin JE only --> */
  /**
   * Configures a store to be replicated or non-replicated, in a replicated Environment. By default
   * this property is true, meaning that by default a store is replicated in a replicated
   * Environment.
   *
   * <p>In a non-replicated Environment, this property is ignored. All stores are non-replicated in
   * a non-replicated Environment.
   *
   * @param replicated whether the store is replicated.
   * @return 'this'.
   * @see <a href="../je/rep/ReplicatedEnvironment.html#nonRepDbs">Non-replicated Databases in a
   ...
   * is ignored. All stores are non-replicated in a non-replicated Environment.
   *
   ...
   *
   * <p>This property is true to open all store index databases for deferred-write access. True may
   * not be specified if the store is transactional.
   *
   * <p>Deferred write stores avoid disk I/O and are not guaranteed to be persistent until {@link
   * EntityStore#sync} or {@link Environment#sync} is called or the store is closed normally. This
   * mode is particularly geared toward stores that frequently modify and delete data records. See
   ...
   *
   * <p>This property is true to open all store databases as temporary databases. True may not be
   * specified if the store is transactional.
   *
   * <p>Temporary stores avoid disk I/O and are not persistent -- they are deleted when the store is
   * closed or after a crash. This mode is particularly geared toward in-memory stores. See the
   * Getting Started Guide, Database chapter for a full description of the mode.
   *
   ...
   *
   * <p>This property is true to cause the initial creation of secondary indices to be performed as
   * a bulk load. If this property is true and {@link EntityStore#getSecondaryIndex
   * EntityStore.getSecondaryIndex} has never been called for a secondary index, that secondary
   * index will not be created or written as records are written to the primary index. In addition,
   * if that secondary index defines a foreign key constraint, the constraint will not be enforced.
   *
   * <p>The secondary index will be populated later when the {@code getSecondaryIndex} method is
   * called for the first time for that index, or when the store is closed and re-opened with this
   * property set to false and the primary index is obtained. In either case, the secondary index is
   * populated by reading through the entire primary index and adding records to the secondary index
   * as needed. While populating the secondary, foreign key constraints will be enforced and an
   * exception is thrown if a constraint is violated.
   *
   * <p>When loading a primary index along with secondary indexes from a large input data set,
   * configuring a bulk load of the secondary indexes is sometimes more performant than updating the
   * secondary indexes each time the primary index is updated. The absence of foreign key
   ...
   *
   * <p>If null is specified or this method is not called, an {@link AnnotationModel} instance is
   * used by default.
   *
  ...
  /**
   * Configures mutations for performing lazy evolution of stored instances. Existing mutations for
   * this store are not cleared, so the mutations required are only those changes that have been
   * made since the store was last opened. Some new mutations may override existing specifications,
   * and some may be supplemental.
   *
   * <p>If null is specified and the store already exists, the previously specified mutations are
   * used. The mutations are stored persistently in serialized form.
   *
   * <p>Mutations must be available to handle all changes to classes that are incompatible with the
   * class definitions known to this store. See {@link Mutations} and {@link
   * com.sleepycat.persist.evolve Class Evolution} for more information.
   *
   * <p>If an incompatible class change has been made and mutations are not available for handling
   * the change, {@link IncompatibleClassException} will be thrown when creating an {@link
   * EntityStore}.
   *
   ...
   *     <p>By default this property is {@link DatabaseNamer#DEFAULT}.
