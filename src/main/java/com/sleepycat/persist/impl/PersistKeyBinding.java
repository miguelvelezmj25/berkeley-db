...
public class PersistKeyBinding implements EntryBinding {
  ...
  public PersistKeyBinding(Catalog catalogParam, String clsName, boolean rawAccess) {
    ...
    try {
      keyFormat = PersistEntityBinding.getOrCreateFormat(catalog, clsName, rawAccess);
  ...
  /**
   * Creates a key binding dynamically for use by PersistComparator. Formats are created from
   * scratch rather than using a shared catalog.
   */
  PersistKeyBinding(final Catalog catalog, final Class cls, final String[] compositeFieldOrder) {
  ...
  /** Binds bytes to an object for use by PersistComparator as well as entryToObject. */
  static Object readKey(
      Format keyFormat, Catalog catalog, byte[] bytes, int offset, int length, boolean rawAccess)
      throws RefreshException {

    EntityInput input = new RecordInput(catalog, rawAccess, null, 0, bytes, offset, length);
  ...
  private Object entryToObjectInternal(DatabaseEntry entry) throws RefreshException {
    ...
    return bytesToObject(entry.getData(), entry.getOffset(), entry.getSize());
  }
