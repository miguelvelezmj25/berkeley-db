...
/**
 * Implements EntityOutput to write record key-data pairs. Extends TupleOutput to implement the
 * subset of TupleOutput methods that are defined in the EntityOutput interface.
 *
 * @author Mark Hayes
 */
class RecordOutput extends TupleOutput implements EntityOutput {
  ...
  private final Map<Object, Integer> visited;
  ...
  public void writeObject(Object o, Format fieldFormat) throws RefreshException {
    ...
    if (rawAccess) {
       ...
       */
      format = catalog.getFormat(o.getClass(), false /*checkEntitySubclassIndexes*/);
    }
    if (format.getProxiedFormat() != null) {
      throw new IllegalArgumentException(
          "May not store proxy classes directly: " + format.getClassName());
    }
    /* Check for embedded entity classes and subclasses. */
    if (format.getEntityFormat() != null) {
      throw new IllegalArgumentException(
          "References to entities are not allowed: " + o.getClass().getName());
    }
    ...
    visited.put(o, prohibitNestedRefs ? RecordInput.PROHIBIT_REF_OFFSET : visitedOffset);
  ...
  public void writeKeyObject(Object o, Format fieldFormat) throws RefreshException {
  ...
  /**
   * Registers the top level entity before writing it, to allow nested fields to reference their
   * parent entity. [#17525]
   */
