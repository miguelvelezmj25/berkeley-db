 ...
 *
 * <p>Note that the equals(), compareTo() and hashCode() methods only use the name field in this
 * class. Comparing two FieldInfo objects is only done when both are declared in the same class, so
 * comparing the field name is sufficient.
 *
 * @author Mark Hayes
 */
class FieldInfo implements RawField, Serializable, Comparable<FieldInfo> {
  ...
  void collectRelatedFormats(Catalog catalog, Map<String, Format> newFormats) {
    ...
    if (format instanceof NonPersistentFormat
      ...
      if (field != null && field.getGenericType() instanceof ParameterizedType) {
        collectParameterizedTypeFormats(
            catalog, newFormats, (ParameterizedType) field.getGenericType());
      }
    }
  }
   ...
   */
  private void collectParameterizedTypeFormats(
      Catalog catalog, Map<String, Format> newFormats, ParameterizedType parameType) {
    Type[] types = parameType.getActualTypeArguments();
    for (int i = 0; i < types.length; i++) {
      if (types[i] instanceof ParameterizedType) {
        collectParameterizedTypeFormats(catalog, newFormats, (ParameterizedType) types[i]);
