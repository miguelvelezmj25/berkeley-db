 ...
 *
 * <p>Unlike TupleInput, Strings are returned by {@link #readObject} when using this class.
 *
 * @author Mark Hayes
 */
public interface EntityInput {
  ...
  /**
   * Changes raw mode and returns the original mode, which is normally restored later. For
   * temporarily changing the mode during a conversion.
   */
  boolean setRawAccess(boolean rawAccessParam);

  /**
   * Called via Accessor to read all fields with reference types, except for the primary key field
   * and composite key fields (see readKeyObject below).
   */
   ...
   *
   * <p>For such key fields, no formatId is present nor can the object already be present in the
   * visited object set.
   */
  Object readKeyObject(Format format) throws RefreshException;

  /**
   * Called for a String field, that is not a primary key field or a composite key field with a
   * reference type.
   *
   * <p>For the new String format, no formatId is present nor can the object already be present in
   * the visited object set. For the old String format, this method simply calls readObject for
   * compatibility.
   */
  Object readStringObject() throws RefreshException;

  /**
   * Called via Accessor.readSecKeyFields for a primary key field with a reference type. This method
   * must be called before reading any other fields.
   */
  void registerPriKeyObject(Object o);

  /**
   * Called via Accessor.readSecKeyFields for a primary String key field. This method must be called
  ...
  /**
   * Called via PersistKeyCreator to skip fields prior to the secondary key field. Also called
   * during class evolution so skip deleted fields.
   */
