...
import com.sleepycat.persist.evolve.*;
...
/**
 * Format for persistent complex classes that are not composite key classes. This includes entity
...
public class ComplexFormat extends Format {
  ...
  private final ClassMetadata clsMeta;
  private EntityMetadata entityMeta;
  private FieldInfo priKeyField;
  private final List<FieldInfo> secKeyFields;
  private final List<FieldInfo> nonKeyFields;
  ...
  /**
   * This field contains the names of secondary keys that are incorrectly ordered because, in an
   * earlier version, we failed to set the dup comparator. This bug applies only when the primary
   * key has a comparator. The bug was fixed by setting the dup comparator to the primary key
   * comparator, for all new secondary databases. [#17252]
   *
   * <p>A field containing an empty set signifies that no keys are incorrectly ordered, while a null
   * field signifies that all keys are incorrect (when the primary key has a comparator). The field
   * is assigned to an empty set when declared, so that it will be null only when a pre-fix version
   * of the format is deserialized. (With Java serialization, when a field is added to a class and a
   * previously serialized instance is deserialized, the new field will always be null).
   *
   * <p>This field is used to determine when a dup comparator should be set. We cannot set the
   * comparator for secondary databases created prior to the bug fix, since ordering cannot be
   * changed for existing records. See isSecKeyIncorrectlyOrdered and setSecKeyCorrectlyOrdered.
   *
   * <p>This field does not count in comparisons of formats during evolution. When the user wants to
   * correct the ordering for an incorrectly ordered secondary database, she must delete the
   * database but does not need to increment the class version. In other words, this is information
   * about the database order but is not considered class metadata.
   */
  private Set<String> incorrectlyOrderedSecKeys = new HashSet<String>();

  /**
   * In JE 5.0 we changed the format for String fields. Instead of treating the String as an object
   * with a format ID embedded in the serialized bytes, we treat it as a primitive and do not
   * include the format ID. This works well because a field declared to be type String cannot be
   * used to store any other object, and because the String tuple format supports null values.
   *
   * <p>A field containing false signifies that the old String format was used when the entity was
   * written, while a true value signifies that the new String format was used. The field is
   * assigned to true when declared, so that it will be false only when a pre-JE 5.0 version of the
   * format is deserialized. (With Java serialization, when a boolean field is added to a class and
   * a previously serialized instance is deserialized, the new field will always be false).
   */
  private final boolean newStringFormat = true;
  ...
  ComplexFormat(Catalog catalog, Class cls, ClassMetadata clsMeta, EntityMetadata entityMeta) {
    ...
    if (clsMeta.getPrimaryKey() != null) {
      ...
      if (field == null) {
        throw new IllegalArgumentException(
            "Primary key field does not exist: " + getClassName() + '.' + fieldName);
      }
    ...
    if (clsMeta.getSecondaryKeys() != null) {
      for (SecondaryKeyMetadata secKeyMeta : clsMeta.getSecondaryKeys().values()) {
        ...
        if (field == null) {
          throw new IllegalArgumentException(
              "Secondary key field does not exist: " + getClassName() + '.' + fieldName);
        }
        ...
        if (fieldCls.isPrimitive() && secKeyMeta.getDeleteAction() == DeleteAction.NULLIFY) {
          throw new IllegalArgumentException(
              "NULLIFY may not be used with primitive fields: " + getClassName() + '.' + fieldName);
        }
        nonKeyFields.remove(field);
  ...
  /**
   * Returns getSuperFormat cast to ComplexFormat. It is guaranteed that all super formats of a
   * ComplexFormat are a ComplexFormat.
   */
  ...
  /**
   * Returns getLatestVersion cast to ComplexFormat. It is guaranteed that all versions of a
   * ComplexFormat are a ComplexFormat.
   */
  ...
  void initialize(Catalog catalog, EntityModel model, int initVersion) {
    ...
    if (entityFormat != null && entityFormat != this && priKeyField != null) {
      throw new IllegalArgumentException(
      ...
      if (useEnhanced) {
        ...
        objAccessor =
            new ReflectionAccessor(
                catalog, type, superObjAccessor, priKeyField, secKeyFields, nonKeyFields);
      }
    }
    Accessor superRawAccessor = (superFormat != null) ? superFormat.rawAccessor : null;
    rawAccessor = new RawAccessor(this, superRawAccessor, priKeyField, secKeyFields, nonKeyFields);

    /* Initialize secondary key field addresses. */
    EntityMetadata latestEntityMeta = null;
    if (entityFormat != null) {
      latestEntityMeta = entityFormat.getLatestVersion().getEntityMetadata();
    }
    if (latestEntityMeta != null) {
      ...
      nextKeyLoop:
      for (SecondaryKeyMetadata secKeyMeta : latestEntityMeta.getSecondaryKeys().values()) {
        ...
        for (ComplexFormat format = this; format != null; format = format.getComplexSuper()) {
          if (clsName.equals(format.getLatestVersion().getClassName())) {
            ...
            if (format.newToOldFieldMap != null && format.newToOldFieldMap.containsKey(fieldName)) {
              useFieldName = format.newToOldFieldMap.get(fieldName);
            } else {
            ...
            FieldAddress addr =
                new FieldAddress(isSecField, fieldNum, superLevel, format, info.getType());
  ...
  /**
   * Checks that the type of a new secondary key is not a primitive and that the default contructor
   * does not initialize it to a non-null value.
   */
  private void checkNewSecKeyInitializer(SecondaryKeyMetadata secKeyMeta) {
    if (objAccessor != null) {
      ...
      Object val = objAccessor.getField(obj, addr.fieldNum, addr.superLevel, addr.isSecField);
      if (val != null) {
        if (addr.keyFormat.isPrimitive()) {
          throw new IllegalArgumentException(
              "For a new secondary key field the field type must "
                  + "not be a primitive -- class: "
                  + secKeyMeta.getDeclaringClassName()
                  + " field: "
                  + secKeyMeta.getName());
        } else {
          throw new IllegalArgumentException(
  ...
  Object convertRawObject(
    ...
    if (level > 0) {
      throw new IllegalArgumentException(
          "RawObject has too few superclasses: " + rawObject.getType().getClassName());
    }
    assert level == 0;
    ...
    EntityInput in = new RawComplexInput(catalog, rawAccess, converted, fields, objects);
  ...
  public String getOldKeyName(final String keyName) {
    if (newToOldFieldMap != null && newToOldFieldMap.containsKey(keyName)) {
  ...
  boolean nullifySecKey(Catalog catalog, Object entity, String keyName, Object keyElement) {
    ...
    if (addr != null) {
      Object oldVal = rawAccessor.getField(entity, addr.fieldNum, addr.superLevel, addr.isSecField);
      if (oldVal != null) {
        if (keyElement != null) {
              ...
              if (keyElement.equals(a1[i])) {
                ...
                if (isArray) {
                  rawAccessor.setField(
                      ...
                      addr.superLevel,
                      addr.isSecField,
                      new RawObject(container.getType(), a2));
                } else {
          ...
          rawAccessor.setField(entity, addr.fieldNum, addr.superLevel, addr.isSecField, null);
          return true;
        }
      } else {
        return false;
  ...
  private int getNFields() {
    return ((priKeyField != null) ? 1 : 0) + secKeyFields.size() + nonKeyFields.size();
  }
  ...
  boolean evolve(Format newFormatParam, Evolver evolver) {
    ...
    for (ComplexFormat oldSuper = getComplexSuper();
      ...
      Converter converter =
          mutations.getConverter(oldSuper.getClassName(), oldSuper.getVersion(), null);
      if (converter != null) {
        evolver.addMissingMutation(
            this,
            newFormatParam,
            "Converter is required for this subclass when a "
                + "Converter appears on its superclass: "
                + converter);
        return false;
      }
    ...
    if (getSuperFormat() == null) {
      ...
      if (!getSuperFormat()
          .getLatestVersion()
          .getClassName()
          .equals(newFormatCls.getSuperclass().getName())) {
        thisChanged = true;
        hierarchyChanged = true;
      }
    }
    ...
    for (ComplexFormat oldSuper = getComplexSuper();
      ...
      if (foundNewSuper != null) {
        ...
        if (!oldSuper.isDeleted()) {
          ComplexFormat oldSuperLatest = oldSuper.getComplexLatest();
        ...
        if (oldEntityClass != null && isCurrentVersion()) {
          ...
          for (FieldInfo field : oldSuper.secKeyFields) {
            SecondaryKeyMetadata meta =
                getSecondaryKeyMetadataByFieldName(secKeys, field.getName());
            assert meta != null;
    ...
    if (thisChanged
        && !evolver.checkUpdatedVersion(
            "Changes to the fields or superclass were detected", this, newFormat)) {
      return false;
    }
  ...
  boolean evolveMetadata(Format newFormatParam, Converter converter, Evolver evolver) {
    ...
    Map<String, SecondaryKeyMetadata> oldSecondaryKeys = entityMeta.getSecondaryKeys();
    Map<String, SecondaryKeyMetadata> newSecondaryKeys = newFormat.entityMeta.getSecondaryKeys();
    Set<String> insertedKeys = new HashSet<String>(newSecondaryKeys.keySet());
      ...
      if (deletedKeys.contains(keyName)) {
        ...
        if (newMeta == null) {
          evolver.addInvalidMutation(
              ...
              newFormat,
              converter,
              "Existing key not found in new entity metadata: " + keyName);
          return false;
        }
        insertedKeys.remove(keyName);
        String keyLabel = "secondary key: " + keyName;
        if (!checkKeyTypeChange(newFormat, oldMeta, newMeta, keyLabel, evolver)) {
          return false;
        }
        if (!checkSecKeyMetadata(newFormat, oldMeta, newMeta, evolver)) {
          return false;
        }
      }
    }
    ...
    if (!insertedKeys.isEmpty()) {
      evolver.addEvolveError(
          ...
          newFormat,
          "Error",
          "New keys " + insertedKeys + " not allowed when using a Converter with an entity class");
    }
  ...
  private boolean checkSecKeyMetadata(
      Format newFormat,
    ...
    if (oldMeta.getRelationship() != newMeta.getRelationship()) {
      evolver.addEvolveError(
          this,
          newFormat,
          "Change detected in the relate attribute (Relationship) " + "of a secondary key",
          "Old key: "
              + oldMeta.getKeyName()
              + " relate: "
              + oldMeta.getRelationship()
              + " new key: "
              + newMeta.getKeyName()
              + " relate: "
              + newMeta.getRelationship());
      return false;
    }
    return true;
  }
  ...
  /**
   * Checks that the type of a key field did not change, as known from metadata when a class
   * conversion is used.
   */
  private boolean checkKeyTypeChange(
      Format newFormat,
    ...
    if (!oldClass.equals(newClass)) {
      ...
      if (oldType == null
          ...
          || ((oldType.getWrapperFormat() == null
                  || oldType.getWrapperFormat().getId() != newType.getId())
              && (newType.getWrapperFormat() == null
                  || newType.getWrapperFormat().getId() != oldType.getId()))) {
        evolver.addEvolveError(
            this,
            newFormat,
  ...
  /**
   * Special case for creating FieldReaders for a deleted class when it appears in the class
   * hierarchy of its non-deleted subclass.
   */
  private boolean evolveDeletedClass(Evolver evolver) {
    assert isDeleted();
    if (secKeyFieldReader == null || nonKeyFieldReader == null) {
      if (priKeyField != null && getEntityFormat() != null && !getEntityFormat().isDeleted()) {
        evolver.addEvolveError(
            this,
            this,
            "Class containing primary key field was deleted ",
            "Primary key is needed in an entity class hierarchy: " + priKeyField.getName());
        return false;
      } else {
  ...
  /**
   * Creates a FieldReader for secondary key fields and non-key fields if necessary. Checks the
   * primary key field if necessary. Does not evolve superclass format fields.
   */
  private int evolveAllFields(ComplexFormat newFormat, Evolver evolver) {
    ...
    if (priKeyField != null) {
      int result =
          evolver.evolveRequiredKeyField(this, newFormat, priKeyField, newFormat.priKeyField);
      if (result == Evolver.EVOLVE_FAILURE) {
        evolveFailure = true;
    ...
    FieldReader reader =
        evolveFieldList(
            secKeyFields, newFormat.secKeyFields, true, newFormat.nonKeyFields, newFormat, evolver);
    ...
    reader =
        evolveFieldList(
            nonKeyFields,
            newFormat.nonKeyFields,
            false,
            newFormat.secKeyFields,
            newFormat,
            evolver);
    if (reader == FieldReader.EVOLVE_FAILURE) {
      evolveFailure = true;
   ...
   *
   * <p>Instead of adding a DoNothingFieldReader class, we use a MultiFieldReader with an empty
   * field list. We do not add a new FieldReader class to avoid changing the catalog format.
   * [#15524]
   */
  ...
  /**
   * Evolves a list of fields, either secondary key or non-key fields, for a single class format.
   *
   * @return a FieldReader if field evolution is needed, null if no evolution is needed, or
   *     FieldReader.EVOLVE_FAILURE if an evolution error occurs.
   */
  private FieldReader evolveFieldList(
      List<FieldInfo> oldFields,
     ...
     */
    fieldLoop:
    for (int oldFieldIndex = 0; oldFieldIndex < oldFields.size(); oldFieldIndex += 1) {

      FieldInfo oldField = oldFields.get(oldFieldIndex);
      String oldName = oldField.getName();
      SecondaryKeyMetadata oldMeta = null;
      if (isOldSecKeyField) {
        oldMeta = getSecondaryKeyMetadataByFieldName(clsMeta.getSecondaryKeys(), oldName);
        assert oldMeta != null;
      }

      /* Get field mutations. */
      Renamer renamer = mutations.getRenamer(getClassName(), getVersion(), oldName);
      Deleter deleter = mutations.getDeleter(getClassName(), getVersion(), oldName);
      Converter converter = mutations.getConverter(getClassName(), getVersion(), oldName);
      ...
      if (deleter != null) {
        ...
        if (currentReader instanceof SkipFieldReader
            && currentReader.acceptField(oldFieldIndex, newFieldIndex, isNewSecKeyField)) {
        ...
        if (newField == null) {
          evolver.addMissingMutation(
              this, newFormat, "Field is not present or not persistent: " + oldName);
          evolveFailure = true;
          continue fieldLoop;
        }
      }
      ...
      if (isOldSecKeyField && isNewSecKeyField) {
        newMeta = getSecondaryKeyMetadataByFieldName(newFormat.clsMeta.getSecondaryKeys(), newName);
        assert newMeta != null;
      ...
      if (converter != null) {
        if (isOldSecKeyField) {
          evolver.addInvalidMutation(
              ...
              newFormat,
              converter,
              "Field Converter is not allowed for secondary key " + "fields: " + oldName);
          evolveFailure = true;
        } else {
          currentReader =
              new ConvertFieldReader(converter, oldFieldIndex, newFieldIndex, isNewSecKeyField);
          fieldReaders.add(currentReader);
          readerNeeded = true;
          localEvolveNeeded = true;
        }
      ...
      for (Format formatVersion = oldFieldFormat.getLatestVersion();
        ...
        Set<Format> subclassFormats = evolver.getSubclassFormats(formatVersion);
      ...
      if (oldLatestFormat.getClassName().equals(newFieldFormat.getClassName())
        ...
        currentReader =
            new WidenFieldReader(oldLatestFormat, newFieldFormat, newFieldIndex, isNewSecKeyField);
        fieldReaders.add(currentReader);
        readerNeeded = true;
        localEvolveNeeded = true;
        continue fieldLoop;
  ...
  /**
   * Base class for all FieldReader subclasses. A FieldReader reads one or more fields in the old
   * format data, and may call the new format Accessor to set the field values.
   */
  private abstract static class FieldReader implements Serializable {
    ...
    abstract void readFields(Object o, EntityInput input, Accessor accessor, int superLevel)
        throws RefreshException;
  }

  /** Reads a continguous block of fields that have the same format in the old and new formats. */
  private static class PlainFieldReader extends FieldReader {
    ...
    private final boolean secKeyField;
    ...
    final void readFields(Object o, EntityInput input, Accessor accessor, int superLevel)
      ...
      if (secKeyField) {
        accessor.readSecKeyFields(o, input, startField, endField, superLevel);
      } else {
        accessor.readNonKeyFields(o, input, startField, endField, superLevel);
      }
    }
  }

  /** Skips a continguous block of fields that exist in the old format but not in the new format. */
  private static class SkipFieldReader extends FieldReader {

    private static final long serialVersionUID = -3060281692155253098L;

    private final List<Format> fieldFormats;
  ...
  private static class ConvertFieldReader extends FieldReader {
    ...
    ConvertFieldReader(
        Converter converter, int oldFieldIndex, int newFieldIndex, boolean isNewSecKeyField) {
    ...
    void initialize(
      ...
      if (isOldSecKey) {
        oldFormat = oldParentFormat.secKeyFields.get(oldFieldNum).getType();
      } else {
        oldFormat = oldParentFormat.nonKeyFields.get(oldFieldNum).getType();
      }
      if (secKeyField) {
        newFormat = newParentFormat.secKeyFields.get(fieldNum).getType();
      } else {
        newFormat = newParentFormat.nonKeyFields.get(fieldNum).getType();
      }
    }
    ...
    final void readFields(Object o, EntityInput input, Accessor accessor, int superLevel)
      ...
      EntityInput rawInput = new RawSingleInput(catalog, currentRawMode, null, value, newFormat);

      if (secKeyField) {
        accessor.readSecKeyFields(o, rawInput, fieldNum, fieldNum, superLevel);
      } else {
        accessor.readNonKeyFields(o, rawInput, fieldNum, fieldNum, superLevel);
      }
    }
  }
  ...
  private static class WidenFieldReader extends FieldReader {
    ...
    private final int fromFormatId;
    private final int toFormatId;
    private final int fieldNum;
    private final boolean secKeyField;

    WidenFieldReader(
        Format oldFormat, Format newFormat, int newFieldIndex, boolean isNewSecKeyField) {
    ...
    final void readFields(Object o, EntityInput input, Accessor accessor, int superLevel)
      ...
      EntityInput widenerInput = new WidenerInput(input, fromFormatId, toFormatId);

      if (secKeyField) {
        accessor.readSecKeyFields(o, widenerInput, fieldNum, fieldNum, superLevel);
      } else {
        accessor.readNonKeyFields(o, widenerInput, fieldNum, fieldNum, superLevel);
      }
    }
  }

  /**
   * A FieldReader composed of other FieldReaders, and that calls them in sequence. Used when more
   * than one FieldReader is needed for a list of fields.
   */
  private static class MultiFieldReader extends FieldReader {

    private static final long serialVersionUID = -6035976787562441473L;

    private final List<FieldReader> subReaders;
  ...
  /**
   * The Reader for evolving ComplexFormat instances. Reads the old format data one class (one level
   * in the class hierarchy) at a time. If an Accessor is used at a given level, the Accessor is
   * used for the corresponding level in the new class hierarchy (classes may be inserted/deleted
   * during evolution). At each level, a FieldReader is called to evolve the secondary key and
   * non-key lists of fields.
   */
  private static class EvolveReader implements Reader {
    ...
    /**
     * oldHierarchy contains the formats of the old class hierarchy in most to least derived class
     * order.
     */
    private transient ComplexFormat[] oldHierarchy;

    /**
     * newHierarchyLevels contains the corresponding level in the new hierarchy for each format in
     * oldHierarchy. newHierarchyLevels is indexed by the oldHierarchy index.
     */
    private final int[] newHierarchyLevels;
    ...
    public void initializeReader(
        Catalog catalog, EntityModel model, int initVersion, Format oldFormatParam) {
      ...
      int newDepth = 0;
      for (Format format = newFormat; format != null; format = format.getSuperFormat()) {
        newDepth += 1;
      }
      ComplexFormat[] newHierarchy = new ComplexFormat[newDepth];
      int level = 0;
      for (ComplexFormat format = newFormat; format != null; format = format.getComplexSuper()) {
      ...
      for (ComplexFormat oldFormat2 = oldFormat;
        ...
        ComplexFormat newFormat2 = (level2 != DO_NOT_READ_ACCESSOR) ? newHierarchy[level2] : null;
        level += 1;
    ...
    public Object readObject(Object o, EntityInput input, boolean rawAccess)
      ...
      Accessor accessor = rawAccess ? newFormat.rawAccessor : newFormat.objAccessor;
      ...
      for (int i = maxMinusOne; i >= 0; i -= 1) {
        ...
        if (reader != null) {
          ...
          accessor.readSecKeyFields(o, input, 0, Accessor.MAX_FIELD_NUM, newLevel);
        }
      }
      ...
      for (int i = maxMinusOne; i >= 0; i -= 1) {
        ...
        if (reader != null) {
          ...
          accessor.readNonKeyFields(o, input, 0, Accessor.MAX_FIELD_NUM, newLevel);
        }
      }
      return o;
    }
  ...
  /**
   * The secondary key metadata map (ClassMetadata.getSecondaryKeys) is keyed by key name, not field
   * name. Key name can be different than field name when a @SecondaryKey name property is
   * specified. To look up metadata by field name, we must do a linear search. Luckily, the number
   * of keys per class is never very large. [#16819]
   */
  static SecondaryKeyMetadata getSecondaryKeyMetadataByFieldName(
      Map<String, SecondaryKeyMetadata> secKeys, String fieldName) {
  ...
  /**
   * Called when opening an existing secondary database that should have a dup comparator
   * configured. If true is returned, then this secondary index may have been previously opened
   * without a dup comparator set, and therefore no dup comparator should be set on this database.
   * If false is returned, the dup comparator should be set by the caller.
   */
  boolean isSecKeyIncorrectlyOrdered(String keyName) {
    return incorrectlyOrderedSecKeys == null || incorrectlyOrderedSecKeys.contains(keyName);
  }

  /**
   * Called when creating a new secondary database that should have a dup comparator configured. If
   * true is returned, then this secondary index may have been previously opened without a dup
   * comparator set; this method will update this format to indicate that the dup comparator is now
   * allowed, and the caller should flush the catalog. If false is returned, the caller need not
   * flush the catalog.
   */
