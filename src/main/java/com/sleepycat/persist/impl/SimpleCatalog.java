...
/**
 * A static catalog containing simple types only. Once created, this catalog is immutable.
 *
 * <p>For bindings accessed by a PersistComparator during recovery, the SimpleCatalog provides
 * formats for all simple types. To reduce redundant format objects, the SimpleCatalog's formats are
 * copied when creating a regular PersistCatalog.
 *
 * <p>This class also contains utility methods for dealing with primitives.
 *
 * @author Mark Hayes
 */
public class SimpleCatalog implements Catalog {
  ...
  /**
   * Sets a format for which space in the formatList has been preallocated, and makes it the current
