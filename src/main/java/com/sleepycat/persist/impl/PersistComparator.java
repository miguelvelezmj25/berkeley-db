...
/**
 * The btree comparator for persistent key classes. The serialized form of this comparator is stored
 * in the BDB JE database descriptor so that the comparator can be re-created during recovery.
 *
 * @author Mark Hayes
 */
public class PersistComparator
  ...
  private final String[] comositeFieldOrder;
  ...
  public PersistComparator(PersistKeyBinding binding) {
    ...
    final CompositeKeyFormat format = (CompositeKeyFormat) binding.keyFormat;
    keyClassName = format.getClassName();
    comositeFieldOrder =
        CompositeKeyFormat.getFieldNameArray(format.getClassMetadata().getCompositeKeyFields());
    ...
    for (FieldInfo field : format.getFieldInfo()) {
      ...
      if (fieldFormat.isEnum()) {
        ...
        fieldFormatData.put(enumFormat.getClassName(), enumFormat.getFormatData());
      }
    }
  }

  /**
   * In BDB JE this method will be called after the comparator is deserialized, including during
   * recovery. We must construct the binding here, without access to the stored catalog since
  ...
  public void initialize(ClassLoader loader) {
    ...
    if (fieldFormatData == null) {
      ...
      final Map<String, Format> enumFormats = new HashMap<String, Format>();
    ...
    binding = new PersistKeyBinding(catalog, keyClass, comositeFieldOrder);
  }
