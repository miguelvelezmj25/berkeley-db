...
public class PersistCatalog implements Catalog {
  ...
  /**
   * Key to Data record in the catalog database. In the JE 3.0.12 beta version the formatList record
   * is stored under this key and is converted to a Data object when it is read.
   */
  private static final byte[] DATA_KEY = getIntBytes(-1);

  /**
   * Key to a JE 3.0.12 beta version mutations record in the catalog database. This record is no
   * longer used because mutations are stored in the Data record and is deleted when the beta
   * version is detected.
   */
  ...
  /**
   * A list of all formats indexed by formatId. Element zero is unused and null, since IDs start at
   * one; this avoids adjusting the ID to index the list. Some elements are null to account for
   * predefined IDs that are not used.
   *
   * <p>This field, like formatMap, is volatile because it is reassigned when dynamically adding new
   * formats. See {@link #addNewFormat}.
   */
   ...
   *
   * <p>This field, like formatList, is volatile because it is reassigned when dynamically adding
   * new formats. See {@link #addNewFormat}.
   */
  ...
  /**
   * A temporary map of proxied class name to proxy class name. Used during catalog creation, and
   * then set to null. This map is used to force proxy formats to be created prior to proxied
   * formats. [#14665]
   */
  ...
  /**
   * If a Replica is upgraded, local in-memory evolution may take place prior to the Master being
   * upgraded. In that case, the size of the formatList will be greater than nStoredFormats. In this
   * case, the readOnly state field will be set to true. We must be sure not to write the metadata
   * in this state. [#16655]
   */
  ...
  /**
   * Creates a new catalog, opening the database and reading it from a given catalog database if it
   * already exists. All predefined formats and formats for the given model are added. For modified
   * classes, old formats are defined based on the rules for compatible class changes and the given
   * mutations. If any format is changed or added, and the database is not read-only, write the
   * initialized catalog to the database.
   */
  public PersistCatalog(
     ...
     */
    db = DbCompat.openDatabase(env, null /*txn*/, fileAndDbNames[0], fileAndDbNames[1], dbConfig);
    if (db == null) {
      String dbNameMsg = store.getDbNameMessage(fileAndDbNames);
      if (dbConfig.getExclusiveCreate()) {
        throw new StoreExistsException(
            "Catalog DB already exists and ExclusiveCreate=true, " + dbNameMsg);
      } else {
        assert !dbConfig.getAllowCreate();
        throw new StoreNotFoundException(
            "Catalog DB does not exist and AllowCreate=false, " + dbNameMsg);
      }
    }
    openCount = 1;
  ...
  /**
   * Creates a new catalog when a Replica refresh occurs. Uses some information from the old catalog
   * directly in the new catalog, but all formats are created from scratch and class evolution is
   * attempted.
   */
  ...
  private void initAndRetry(
      final String storePrefix, final EntityModel modelParam, final Mutations mutationsParam)
      throws DatabaseException {

    for (int i = 0; ; i += 1) {
      Transaction txn = null;
      if (transactional && DbCompat.getThreadTransaction(env) == null) {
        txn = env.beginTransaction(null, store.getAutoCommitTxnConfig());
      }
      boolean success = false;
      try {
  ...
  private void init(
    ...
    try {
      ...
      if (mutationsParam != null && !mutations.equals(mutationsParam)) {
      ...
      if (formatList == null) {
        ...
        if (SimpleCatalog.addMissingSimpleFormats(envClassLoader, formatList)) {
          needWrite = true;
        }
       ...
       */
      List<String> knownClasses = new ArrayList<String>(model.getKnownClasses());
      ...
      for (Format oldFormat : formatList) {
        ...
        Renamer renamer = mutations.getRenamer(oldName, oldFormat.getVersion(), null);
      ...
      initEvolver =
          new Evolver(
              this, storePrefix, mutations, newFormats, forceEvolution, disallowClassChanges);
      for (Format oldFormat : formatList) {
        if (oldFormat == null || Format.isPredefined(oldFormat)) {
          continue;
        }
      ...
      final boolean formatsChanged = newFormats.size() > 0 || initEvolver.areFormatsChanged();
  ...
  /**
   * Returns a map from format to a set of its superclass formats. The format for simple types,
   * enums and class Object are not included. Only complex types have superclass formats as defined
   * by Format.getSuperFormat.
   */
  Map<Format, Set<Format>> getSubclassMap() {
    Map<Format, Set<Format>> subclassMap = new HashMap<Format, Set<Format>>();
    for (Format format : formatList) {
  ...
  /**
   * Decrements the reference count and closes the catalog DB when it reaches zero. Returns true if
   * the database was closed or false if the reference count is still non-zero and the database was
   * left open.
   */
  ...
  /**
   * Convenience method that gets the class for the given class name and calls createFormat with the
   * class object.
   */
  ...
  /**
   * If the given class format is not already present in the given map and a format for this class
   * name does not already exist, creates an uninitialized format, adds it to the map, and also
   * collects related formats in the map.
   */
  public Format createFormat(Class type, Map<String, Format> newFormats) {
    ...
    if (proxyClassName != null) {
      ...
      if (metadata == null) {
        throw new IllegalArgumentException(
            "Class could not be loaded or is not persistent: " + className);
      }
      if (metadata.getCompositeKeyFields() != null
          && (metadata.getPrimaryKey() != null || metadata.getSecondaryKeys() != null)) {
        throw new IllegalArgumentException(
      ...
      if (metadata.getCompositeKeyFields() != null) {
        format = new CompositeKeyFormat(this, type, metadata, metadata.getCompositeKeyFields());
      } else {
  ...
  /**
   * Installs an existing format when no evolution is needed, i.e, when the new and old formats are
   * identical.
   */
  ...
  /**
   * When a format is intialized, this method is called to get the version of the serialized object
   * to be initialized. See Catalog.
   */
  public int getInitVersion(Format format, boolean forReader) {

    if (initData == null
        || initData.formatList == null
      ...
      if (forReader) {
         ...
         */
        return (initEvolver != null && initEvolver.isFormatChanged(format))
  ...
  public Format getFormat(final int formatId, final boolean expectStored) throws RefreshException {

    if (formatId < 0) {
      throw DbCompat.unexpectedState(
          "Format ID " + formatId + " is negative," + " may indicate data corruption.");
    }

    /**
     * If we're attempting to read a record containing a format ID that is greater than the maximum
     * known stored format, then we refresh the formats from disk, expecting that the stored formats
     * have been updated by the Master node. Note that format IDs greater than nStoredFormats may
     * exist in the formatList, if evolution took place on this Replica in a read-only mode. Such
     * formats are never written (Replicas do not write) and cannot be used for reading an existing
     * record. [#16655]
     *
     * <p>Do not perform this check if we did not get the format ID from a stored record
     * (expectStored is false). For example, this would cause an erroneous RefreshException when
     * this method is called during a convertRawObject operation, which calls this method to get a
     * fresh copy of a format that may not be stored. [#18690]
     */
    ...
    if (format == null) {
      throw DbCompat.unexpectedState(
          "Format ID " + formatId + " has null format," + " may indicate data corruption.");
    }
   ...
   *
   * <p>This method is called for top level entity instances by PersistEntityBinding. When a new
   * entity subclass format is added we call Store.checkEntitySubclassSecondaries to ensure that all
   * secondary databases have been opened, before storing the entity. We do this here while not
   * holding a synchronization mutex, not in addNewFormat, to avoid deadlocks.
   * checkEntitySubclassSecondaries synchronizes on the Store. [#16399]
   *
   * <p>Historical note: At one time we opened / created the secondary databases rather than
   * requiring the user to open them, see [#15247]. Later we found this to be problematic since a
   * user txn may have locked primary records, see [#16399].
   */
      ...
      if (model != null) {
        ...
        if (checkEntitySubclassIndexes && store != null) {
          ...
          if (entityFormat != null && entityFormat != format) {
            try {
              store.checkEntitySubclassSecondaries(entityFormat.getEntityMetadata(), cls.getName());
  ...
  /**
   * Returns the name of an entity class to be used to form the database name. Normally this is the
   * same as the class name, but in replica upgrade mode it may be an earlier version of a renamed
   * class. Returns null if there is no stored version of the class. [#16655]
   */
   ...
   * [#16655]
   */
  public String getDatabaseKeyName(final String className, final String keyName) {
    final Format format = getStoredFormat(className);
    if (format == null) {
      return null;
    }
  ...
  /**
   * Metadata needs refreshing when a Replica with stale metadata is elected master, and then a user
   * write operation is attempted. [#16655]
   */
  ...
  /**
   * Adds a format for a new class. Returns the format added for the given class, or throws an
   * exception if the given class is not persistent.
   *
   * <p>This method uses a copy-on-write technique to add new formats without impacting other
   * threads.
   */
  private synchronized Format addNewFormat(Class cls) throws RefreshException {
    ...
    Map<String, Format> newFormatMap = new HashMap<String, Format>(formatMap);
    Map<String, Format> newLatestFormatMap = new HashMap<String, Format>(latestFormatMap);
    ...
    Catalog newFormatCatalog =
        new ReadOnlyCatalog(ModelInternal.getClassLoader(model), newFormatList, newFormatMap);
    for (Format newFormat : newFormats.values()) {
  ...
  /**
   * Used to write the catalog when a format has been changed, for example, when Store.evolve has
   * updated a Format's EvolveNeeded property. Uses auto-commit.
   */
  ...
  /**
   * Reads catalog Data, converting old versions as necessary. An empty Data object is returned if
   * no catalog data currently exists. Null is never returned.
   */
  ...
  /**
   * Metadata needs refreshing when a Replica with stale metadata is elected master, and then a user
   * write operation is attempted that also requires a metadata update. [#16655]
   */
  boolean isMetadataStale(Transaction txn) throws DatabaseException {

    Data oldData = readData(txn);

    return (oldData.formatList != null && oldData.formatList.size() > nStoredFormats);
  }
...
//    db.put(txn, key, data);
  ...
  public Class resolveClass(String clsName) throws ClassNotFoundException {
    ...
    return SimpleCatalog.resolveClass(clsName, ModelInternal.getClassLoader(model));
  }

  public Class resolveKeyClass(String clsName) {
    return SimpleCatalog.resolveKeyClass(clsName, ModelInternal.getClassLoader(model));
  }
}
