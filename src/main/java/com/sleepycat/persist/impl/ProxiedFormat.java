...
public class ProxiedFormat extends Format {
  ...
  private final transient String proxyClassName;
  ...
  /**
   * Returns the proxy class name. The proxyClassName field is non-null for a constructed object and
   * null for a de-serialized object. Whenever the proxyClassName field is null (for a de-serialized
   * object), the proxyFormat will be non-null.
   */
  ...
  /**
   * In the future if we implement container proxies, which support nested references to the
   * container, then we will return false if this is a container proxy. [#15815]
   */
  @Override
  ...
  Object convertRawObject(
    ...
    PersistentProxy proxy =
        (PersistentProxy) proxyFormat.convertRawObject(catalog, rawAccess, rawObject, converted);
