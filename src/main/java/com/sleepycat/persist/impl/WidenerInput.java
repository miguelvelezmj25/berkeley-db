...
/**
 * Widens a value returned by another input when any readXxx method is called. Used to cause an
 * Accessor to read a widened value.
 *
 * <p>For non-key fields we support all Java primitive widening: - byte to short, int, long, float,
 * double, BigInteger and BigDecimal - short to int, long, float, double, BigInteger and BigDecimal
 * - char to int, long, float, double, BigInteger and BigDecimal - int to long, float, double,
 * BigInteger and BigDecimal - long to float, double, BigInteger and BigDecimal - float to double
 *
 * <p>For non-key fields we also support: - Java reference widening - primitive to primitive wrapper
 * - Java primitive widening to corresponding primitive wrappers - Java widening of primitive
 * wrapper to primitive wrapper
 *
 * <p>For secondary keys fields we ONLY support: - primitive to primitive wrapper
 *
 * <p>But for primary keys and composite key fields we ONLY support: - primitive to primitive
 * wrapper - primitive wrapper to primitive These conversions don't require any converter, since the
 * stored format is not changed. A WidenerInput is not used for these changes.
 *
 * @author Mark Hayes
 */
class WidenerInput extends AbstractInput {
  ...
  /**
   * Returns whether widening is supported by this class. If false is returned by this method, then
   * widening is disallowed and a field converter or deleter is necessary.
   */
  ...
  public Object readKeyObject(Format fromFormat) throws RefreshException {
