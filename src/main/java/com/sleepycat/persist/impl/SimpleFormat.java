...
/**
 * Format for simple types, including primitives. Additional methods are included to optimize the
 * handling of primitives. Other classes such as PrimitiveArrayFormat and ReflectAccessor take
...
public abstract class SimpleFormat extends Format {
  ...
  void copySecMultiKeyPrimitiveArray(int len, RecordInput input, Set results) {
    ...
    for (int i = 0; i < len; i += 1) {
      DatabaseEntry entry =
          new DatabaseEntry(input.getBufferBytes(), input.getBufferOffset(), primLen);
      results.add(entry);
