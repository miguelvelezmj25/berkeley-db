...
/**
 * Base implementation for EntityStore and RawStore. The methods here correspond directly to those
 * in EntityStore; see EntityStore documentation for details.
 *
 * @author Mark Hayes
 */
public class Store {
  ...
  private static final Map<Environment, Map<String, PersistCatalog>> catalogPool =
  ...
  public synchronized <PK, E> PrimaryIndex<PK, E> getPrimaryIndex(
    ...
    if (priIndex == null) {
      ...
      try {
        ...
        /* Create index object. */
        priIndex =
            new InternalPrimaryIndex(db, primaryKeyClass, keyBinding, entityClass, entityBinding);
  ...
  /**
   * Holds state information about opening a primary index and its secondary indexes. Used to undo
   * the state of this object if the transaction opening the primary and secondaries aborts. Also
   * used to close all databases opened during this process for a non-transactional store.
   */
  private class PrimaryOpenState {

    private final String entityClassName;
    private final IdentityHashMap<Database, Object> databases;
    private final Set<String> secNames;
    ...
    /**
     * Reset all state information and closes any databases opened, when this operation fails. This
     * method should be called for both transactional and non-transsactional operation.
     *
     * <p>For transactional operations on JE, we don't strictly need to close the databases since
     * the transaction abort will do that. However, closing them is harmless on JE, and required for
     * DB core.
     */
    void undoState() {
   ...
   */
  private <SK, PK, E1, E2 extends E1> InternalSecondaryIndex<SK, PK, E2> openSecondaryIndex(
      Transaction txn,
      PrimaryIndex<PK, E1> primaryIndex,
      Class<E2> entityClass,
    ...
    SecondaryDatabase db =
        openSecondaryDatabase(
            txn, fileAndDbNames, primaryIndex, secKeyMeta.getKeyName(), config, doNotCreate);
    if (db == null) {
      assert doNotCreate;
      return null;
    }
    ...
    InternalSecondaryIndex<SK, PK, E2> secIndex =
        new InternalSecondaryIndex(db, primaryIndex, keyClass, keyBinding, getKeyCreator(config));
  ...
  /**
   * Open a secondary database, setting AllowCreate, ExclusiveCreate and AllowPopulate
   * appropriately. We either set all three of these params to true or all to false. This ensures
   * that we only populate a database when it is created, never if it just happens to be empty.
   * [#16399]
   *
   * <p>We also handle correction of a bug in duplicate ordering. See
   * ComplexFormat.incorrectlyOrderedSecKeys.
   *
   * @param doNotCreate is true when StoreConfig.getSecondaryBulkLoad is true and we are opening a
   *     secondary as a side effect of opening a primary, i.e., getSecondaryIndex is not being
   *     called. If doNotCreate is true and the database does not exist, we silently ignore the
   *     failure to create the DB and return null. When getSecondaryIndex is subsequently called,
   *     the secondary database will be created and populated from the primary -- a bulk load.
   */
  private SecondaryDatabase openSecondaryDatabase(
    ...
    final ComplexFormat entityFormat =
        (ComplexFormat) ((PersistEntityBinding) priIndex.getEntityBinding()).entityFormat;
    ...
    try {
      ...
      if (config.getAllowCreate()) {
        ...
        final SecondaryDatabase db =
            DbCompat.openSecondaryDatabase(
                env, txn, fileAndDbNames[0], fileAndDbNames[1], priDb, config);
        if (db != null) {
          /* For unit testing. */
  ...
  /**
   * Checks that all secondary indexes defined in the given entity metadata are already open. This
   * method is called when a new entity subclass is encountered when an instance of that class is
   * stored. [#16399]
   *
   * @throws IllegalArgumentException if a secondary is not open.
   */
  synchronized void checkEntitySubclassSecondaries(
      final EntityMetadata entityMeta, final String subclassName) throws DatabaseException {
      ...
      if (sequenceDb == null) {
        ...
        sequenceDb =
            DbCompat.openDatabase(
                env, null /*txn*/, fileAndDbNames[0], fileAndDbNames[1], dbConfig);
  ...
  private SecondaryConfig getSecondaryConfig(
    ...
    if (config == null) {
      ...
      PersistKeyCreator keyCreator =
          new PersistKeyCreator(catalog, entityMeta, keyClassName, secKeyMeta, rawAccess);
  ...
  /**
   * Parses a whole DB name and returns an array of 2 strings where element 0 is the file name
   * (always null for JE, always non-null for DB core) and element 1 is the logical DB name (always
   * non-null for JE, may be null for DB core).
   */
  ...
  /**
   * Attempts to refresh metadata and returns whether a refresh occurred. May be called when we
   * expect that updated metadata may be available on disk, and if so could be used to satisfy the
   * user's request. For example, if an index is requested and not available, we can try a refresh
   * and the check for the index again.
   */
  ...
  /**
   * Called via RefreshException.refresh when handling the RefreshException in the binding methods,
   * when a Replica detects that its in-memory metadata is stale.
   *
   * <p>During refresh, objects that are visible to the user must not be re-created, since the user
   * may have a reference to them. The PersistCatalog is re-created by this method, and the
   * additional objects listed below are refreshed without creating a new instance. The refresh()
   * method of non-indented classes is called, and these methods forward the call to indented
   * classes.
   *
   * <p>PersistCatalog EntityModel PrimaryIndex PersistEntityBinding PersistKeyAssigner
   * SecondaryIndex PersistKeyCreator PersistKeyBinding
   *
   * <p>These objects have volatile catalog and format fields. When a refresh in one thread changes
   * these fields, other threads should notice the changes ASAP. However, it is not necessary that
   * all access to these fields is synchronized. It is Ok for a mix of old and new fields to be used
   * at any point in time. If an old object is used after a refresh, the need for a refresh may be
   * detected, causing another call to this method. In most cases the redundant refresh will be
   * avoided (see check below), but in some cases an extra unnecessary refresh may be performed.
   * This is undesirable, but is not dangerous. Synchronization must be avoided to prevent blocking
   * during read/write operations.
   *
   * <p>[#16655]
   */
  synchronized PersistCatalog refresh(
      final PersistCatalog oldCatalog, final int errorFormatId, final RefreshException cause) {
  ...
  /* <!-- begin JE only --> */
  /**
   * Configures a TransactionConfig for auto-commit operations on a non-replicated transactional
   * database on a replicated node. We use: - default SyncPolicy for the environment -
   * ReplicaAckPolicy.NONE to avoid consistency checks on the Master - Consistency.NONE to avoid
   * consistency checks on the Replica - localWrite=true to allow writing
   */
  private static void configForNonRepDb(TransactionConfig config, Durability envDurability) {
    config.setDurability(
        new Durability(
            envDurability.getLocalSync(),
            envDurability.getReplicaSync(),
