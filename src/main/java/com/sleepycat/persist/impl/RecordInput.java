...
/**
 * Implements EntityInput to read record key-data pairs. Extends TupleInput to implement the subset
 * of TupleInput methods that are defined in the EntityInput interface.
 *
 * @author Mark Hayes
 */
class RecordInput extends TupleInput implements EntityInput {
  ...
  private final DatabaseEntry priKeyEntry;
  ...
  /**
   * Registers the top level entity before reading it, to allow nested fields to reference their
   * parent entity. [#17525]
   */
  ...
  /**
   * Registers the entity format before reading it, so that old-format String fields can be read
   * properly. [#19247]
   */
