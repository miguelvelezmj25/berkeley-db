 ...
 *
 * <p>Unlike TupleOutput, Strings should be passed to {@link #writeObject} when using this class.
 *
 * <p>Note that currently there is only one implementation of EntityOutput: RecordOutput. There is
 * no RawObjectOutput implemention because we currently have no need to convert from persistent
 * objects to RawObject instances. The EntityOutput interface is only for symmetry with EntityInput
 * and in case we need RawObjectOutput in the future.
 *
 * @author Mark Hayes
 */
public interface EntityOutput {
  ...
  /**
   * Called via Accessor to write all fields with reference types, except for the primary key field
   * and composite key fields (see writeKeyObject below).
   */
  ...
  /**
   * Called via Accessor.writeSecKeyFields for a primary key field with a reference type. This
   * method must be called before writing any other fields.
   */
