...
interface Catalog {
  ...
  /**
   * Returns a format for a given ID, or throws an exception. This method is used when reading an
   * object from the byte array format.
   *
   * @param expectStored is true if reading a record from a database, and therefore the format ID is
   *     expected to be stored also. If the format ID is not stored, a RefreshException is thrown.
   * @throws IllegalStateException if the formatId does not correspond to a persistent class. This
   *     is an internal consistency error.
   */
  Format getFormat(int formatId, boolean expectStored) throws RefreshException;

  /**
   * Returns a format for a given class, or throws an exception. This method is used when writing an
   * object that was passed in by the user.
   *
   * @param checkEntitySubclassIndexes is true if we're expecting this format to be an entity
   *     subclass and therefore subclass secondary indexes should be opened.
   * @throws IllegalArgumentException if the class is not persistent. This is a user error.
   */
  Format getFormat(Class cls, boolean checkEntitySubclassIndexes) throws RefreshException;

  /**
   * Returns a format by class name. Unlike {@link #getFormat(Class,boolean)}, the format will not
   * be created if it is not already known.
   */
