...
class ReflectionAccessor implements Accessor {
  ...
  private final Accessor superAccessor;
  ...
  ReflectionAccessor(
    ...
    if (priKeyField != null) {
      priKey = getField(catalog, priKeyField, true /*isRequiredKeyField*/);
    } else {
      priKey = null;
    }
    if (secKeyFields.size() > 0) {
      secKeys = getFields(catalog, secKeyFields, false /*isRequiredKeyField*/);
    } else {
      secKeys = EMPTY_KEYS;
    }
    if (nonKeyFields.size() > 0) {
      nonKeys = getFields(catalog, nonKeyFields, false /*isRequiredKeyField*/);
    } else {
      nonKeys = EMPTY_KEYS;
    }
  }
  ...
  private FieldAccess[] getFields(
      Catalog catalog, List<FieldInfo> fieldInfos, boolean isRequiredKeyField) {
    int index = 0;
  ...
  private FieldAccess getField(Catalog catalog, FieldInfo fieldInfo, boolean isRequiredKeyField) {
    ...
    if (!Modifier.isPublic(type.getModifiers()) || !Modifier.isPublic(field.getModifiers())) {
      setAccessible(field, field.getName());
    }
    Class fieldCls = field.getType();
    if (fieldCls.isPrimitive()) {
      assert SimpleCatalog.isSimpleType(fieldCls);
      return new PrimitiveAccess(field, (SimpleFormat) catalog.getFormat(fieldCls.getName()));
  ...
  public void readSecKeyFields(
      Object o, EntityInput input, int startField, int endField, int superLevel)
    ...
    try {
      ...
      if (superLevel != 0 && superAccessor != null) {
        superAccessor.readSecKeyFields(o, input, startField, endField, superLevel - 1);
      } else {
        if (superLevel > 0) {
      ...
      if (superLevel <= 0) {
        for (int i = startField; i <= endField && i < secKeys.length; i += 1) {
  ...
  public void readNonKeyFields(
      Object o, EntityInput input, int startField, int endField, int superLevel)
      throws RefreshException {

    try {
      if (superLevel != 0 && superAccessor != null) {
        superAccessor.readNonKeyFields(o, input, startField, endField, superLevel - 1);
      } else {
        if (superLevel > 0) {
          throw DbCompat.unexpectedState("Superclass does not exist");
        }
      ...
      if (superLevel <= 0) {
        for (int i = startField; i <= endField && i < nonKeys.length; i += 1) {
  ...
  public Object getField(Object o, int field, int superLevel, boolean isSecField) {
    if (superLevel > 0) {
      return superAccessor.getField(o, field, superLevel - 1, isSecField);
    }
    try {
      Field fld = isSecField ? secKeys[field].field : nonKeys[field].field;
      return fld.get(o);
    } catch (IllegalAccessException e) {
      throw DbCompat.unexpectedException(e);
    }
  ...
  public void setField(Object o, int field, int superLevel, boolean isSecField, Object value) {
    if (superLevel > 0) {
      superAccessor.setField(o, field, superLevel - 1, isSecField, value);
      return;
    }
    try {
      Field fld = isSecField ? secKeys[field].field : nonKeys[field].field;
  ...
  private abstract static class FieldAccess {
    ...
    FieldAccess(Field field) {
      ...
      isString = field.getType().getName().equals(String.class.getName());
    }

    /** Writes a field. */
    abstract void write(Object o, EntityOutput out) throws IllegalAccessException, RefreshException;

    /** Reads a field. */
    abstract void read(Object o, EntityInput in) throws IllegalAccessException, RefreshException;

    /**
     * Returns whether a field is null (for reference types) or zero (for primitive integer types).
     * This implementation handles the reference types.
     */
  ...
  /** Access for primary key fields and composite key fields with object types. */
  ...
  /**
   * Access for String fields, that are not primary key fields or composite key fields with object
   * types.
   */
  ...
  private static class PrimitiveAccess extends FieldAccess {
    ...
    private final SimpleFormat format;
