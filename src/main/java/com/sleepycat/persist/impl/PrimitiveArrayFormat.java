...
/**
 * An array of primitives having one dimension. Multidimensional arrays are handled by {@link
 * ObjectArrayFormat}.
 *
 * @author Mark Hayes
 */
public class PrimitiveArrayFormat extends Format {
  ...
  void initialize(Catalog catalog, EntityModel model, int initVersion) {
     ...
     */
    componentFormat =
        (SimpleFormat) catalog.getFormat(getExistingType().getComponentType().getName());
  }
  ...
  Object convertRawObject(
    ...
    RawArrayInput input =
        new RawArrayInput(catalog, rawAccess, converted, rawObject, componentFormat);
