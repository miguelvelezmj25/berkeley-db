 ...
 *
 * <p>This class is similar to ComplexFormat in that a composite key class and other complex classes
 * have fields, and the Accessor interface is used to access those fields. Composite key classes are
 * different in the following ways:
 *
 * <p>- The superclass must be Object. No inheritance is allowed.
 *
 * <p>- All instance fields must be annotated with @KeyField, which determines their order in the
 * data bytes.
 *
 * <p>- Although fields may be reference types (primitive wrappers or other simple reference types),
 * they are stored as if they were primitives. No object format ID is stored, and the class of the
 * object must be the declared classs of the field; i.e., no polymorphism is allowed for key fields.
 * In other words, a composite key is stored as an ordinary tuple as defined in the
 * com.sleepycat.bind.tuple package. This keeps the key small and gives it a well defined sort
 * order.
 *
 * <p>- If the key class implements Comparable, it is called by the Database btree comparator. It
 * must therefore be available during JE recovery, before the store and catalog have been opened. To
 * support this, this format can be constructed during recovery. A SimpleCatalog singleton instance
 * is used to provide a catalog of simple types that is used by the composite key format.
 *
 * <p>- When interacting with the Accessor, the composite key format treats the Accessor's non-key
 * fields as its key fields. The Accessor's key fields are secondary keys, while the composite
 * format's key fields are the component parts of a single key.
 *
 * @author Mark Hayes
 */
public class CompositeKeyFormat extends Format {
  ...
  CompositeKeyFormat(
      Catalog catalog, Class cls, ClassMetadata metadata, List<FieldMetadata> fieldMeta) {
  ...
  private CompositeKeyFormat(
      Catalog catalog, Class cls, ClassMetadata metadata, String[] fieldNames) {
    super(catalog, cls);
    ...
    if (superCls != Object.class) {
      throw new IllegalArgumentException(
          "Composite key class must be derived from Object: " + cls.getName());
    }
  ...
  boolean evolve(Format newFormatParam, Evolver evolver) {
    ...
    for (int i = 0; i < fields.size(); i += 1) {
      int result =
          evolver.evolveRequiredKeyField(this, newFormat, fields.get(i), newFormat.fields.get(i));
