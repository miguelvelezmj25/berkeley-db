...
class RawAccessor implements Accessor {
  ...
  public Object newInstance() {
    ...
    return new RawObject(parentFormat, new HashMap<String, Object>(), superObject);
  }
  ...
  public void readSecKeyFields(
    ...
    } else if (priKeyField != null && priKeyField.getType().getId() == Format.ID_STRING) {
      input.registerPriStringKeyObject(getValue(o, priKeyField));
    }
    if (superLevel != 0 && superAccessor != null) {
      superAccessor.readSecKeyFields(getSuper(o), input, startField, endField, superLevel - 1);
    } else {
      if (superLevel > 0) {
    ...
    if (superLevel <= 0) {
      for (int i = startField; i <= endField && i < secKeyFields.size(); i += 1) {
  ...
  public void readNonKeyFields(
    ...
    if (superLevel != 0 && superAccessor != null) {
      superAccessor.readNonKeyFields(getSuper(o), input, startField, endField, superLevel - 1);
    } else {
      if (superLevel > 0) {
        throw DbCompat.unexpectedState("Super class does not exist");
      }
    ...
    if (superLevel <= 0) {
      for (int i = startField; i <= endField && i < nonKeyFields.size(); i += 1) {
  ...
  public Object getField(Object o, int field, int superLevel, boolean isSecField) {
    if (superLevel > 0) {
      return superAccessor.getField(getSuper(o), field, superLevel - 1, isSecField);
    }
    FieldInfo fld = isSecField ? secKeyFields.get(field) : nonKeyFields.get(field);
    return getValue(o, fld);
  }

  public void setField(Object o, int field, int superLevel, boolean isSecField, Object value) {
    if (superLevel > 0) {
      superAccessor.setField(getSuper(o), field, superLevel - 1, isSecField, value);
      return;
    }
    FieldInfo fld = isSecField ? secKeyFields.get(field) : nonKeyFields.get(field);
  ...
  private void writeField(Object o, FieldInfo field, EntityOutput output) throws RefreshException {
  ...
  private void readField(Object o, FieldInfo field, EntityInput input) throws RefreshException {
