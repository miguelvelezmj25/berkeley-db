...
/**
 * An array of objects having a specified number of dimensions. All multidimensional arrays are
 * handled by this class, since even a primitive array of more than one dimension is an array of
 * objects, where the component objects may be primitive arrays. The {@link PrimitiveArrayFormat}
 * class handles primitive arrays of one dimension only.
 *
 * <p>In this class, and {@link PrimitiveArrayFormat}, we resort to using reflection to allocate
 * multidimensional arrays. If there is a need for it, reflection could be avoided in the future by
...
public class ObjectArrayFormat extends Format {
  ...
  boolean isAssignableTo(Format format) {
    ...
    if (format instanceof ObjectArrayFormat) {
      ObjectArrayFormat other = (ObjectArrayFormat) format;
        return useComponentFormat.isAssignableTo(other.useComponentFormat);
    }
    return false;
  }
  ...
  Object convertRawObject(
    ...
    RawArrayInput input =
        new RawArrayInput(catalog, rawAccess, converted, rawObject, useComponentFormat);
