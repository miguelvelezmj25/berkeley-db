...
/**
 * Evolves each old format that is still relevant if necessary, using Mutations to configure
 * deleters, renamers, and converters.
 *
 * @author Mark Hayes
 */
class Evolver {
  ...
  private final Map<String, Format> newFormats;
  private final boolean forceEvolution;
  private final boolean disallowClassChanges;
  private boolean nestedFormatsChanged;
  private final Map<Format, Format> changedFormats;
  private final StringBuilder errors;
  private final Set<String> deleteDbs;
  private final Map<String, String> renameDbs;
  private final Map<Format, Format> renameFormats;
  private final Map<Integer, Boolean> evolvedFormats;
  private final List<Format> unprocessedFormats;
  private final Map<Format, Set<Format>> subclassMap;
  ...
  /**
   * Returns whether any formats were changed during evolution, and therefore need to be stored in
   * the catalog.
   */
  ...
  /**
   * Returns the set of formats for a specific superclass format, or null if the superclass is not a
   * complex type or has not subclasses.
   */
  ...
  /**
   * Returns an error string if any mutations are invalid or missing, or returns null otherwise. If
   * non-null is returned, the store may not be opened.
   */
  String getErrors() {
    if (errors.length() > 0) {
      return errors
   ...
   *
   * <p>We do not require deletion of an unreferenced class for two reasons: 1) built-in proxy
   * classes may not be referenced, 2) the user may wish to declare persistent classes that are not
   * yet used.
   */
  ...
  /**
   * Called by PersistCatalog for all entity formats, and by Format.evolve methods for all
   * potentially referenced non-entity formats.
   */
  ...
  /**
   * Tries to evolve a given existing format to the current version of the class and returns false
   * if an invalid mutation is encountered or the configured mutations are not sufficient.
   */
  private boolean evolveFormatInternal(Format oldFormat) {
    ...
    if (oldFormat.isArray()) {
      if (deleter != null || converter != null || renamer != null) {
        Mutation mutation =
            (deleter != null) ? deleter : ((converter != null) ? converter : renamer);
  ...
  /**
   * Use the old format and discard the new format. Called by Format.evolve when the old and new
  ...
  /**
   * Install an evolver Reader in the old format. Called by Format.evolve when the old and new
  ...
  void renameSecondaryDatabase(
      String oldEntityClass, String newEntityClass, String oldKeyName, String newKeyName) {
    renameDbs.put(
  ...
  private boolean checkUpdatedVersion(Mutation mutation, Format oldFormat, Format newFormat) {
    if (newFormat != null
      ...
      addInvalidMutation(
          oldFormat, newFormat, mutation, "A new higher version number must be assigned");
      return false;
    } else {
      return true;
    }
  ...
  boolean checkUpdatedVersion(String scenario, Format oldFormat, Format newFormat) {
    if (newFormat != null
        ...
        && newFormat.getVersion() <= oldFormat.getVersion()) {
      addEvolveError(
          oldFormat, newFormat, scenario, "A new higher version number must be assigned");
      return false;
    } else {
      return true;
    }
  ...
  int evolveRequiredKeyField(
      Format oldParent, Format newParent, FieldInfo oldField, FieldInfo newField) {
    ...
    Deleter deleter =
        mutations.getDeleter(oldParent.getClassName(), oldParent.getVersion(), oldName);
    if (deleter != null) {
      addInvalidMutation(
          oldParent, newParent, deleter, "Deleter is not allowed for a " + FIELD_LABEL);
      return EVOLVE_FAILURE;
    }
    Converter converter =
        mutations.getConverter(oldParent.getClassName(), oldParent.getVersion(), oldName);
    if (converter != null) {
      addInvalidMutation(
          oldParent, newParent, converter, "Converter is not allowed for a " + FIELD_LABEL);
      return EVOLVE_FAILURE;
    }
    Renamer renamer =
        mutations.getRenamer(oldParent.getClassName(), oldParent.getVersion(), oldName);
