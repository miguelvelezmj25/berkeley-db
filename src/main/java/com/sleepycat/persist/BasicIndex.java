...
/**
 * Implements EntityIndex using a ValueAdapter. This class is abstract and does not implement
 * get()/map()/sortedMap() because it doesn't have access to the entity binding.
 *
 * @author Mark Hayes
 */
abstract class BasicIndex<K, E> implements EntityIndex<K, E> {
  ...
  public EntityCursor<K> keys(K fromKey, boolean fromInclusive, K toKey, boolean toInclusive)
      throws DatabaseException {
  ...
  public EntityCursor<E> entities(K fromKey, boolean fromInclusive, K toKey, boolean toInclusive)
      throws DatabaseException {
  ...
  private <V> EntityCursor<V> cursor(
      Transaction txn, KeyRange range, ValueAdapter<V> adapter, CursorConfig config)
      throws DatabaseException {

    Cursor cursor = db.openCursor(txn, config);
    RangeCursor rangeCursor = new RangeCursor(range, null /*pkRange*/, sortedDups, cursor);
