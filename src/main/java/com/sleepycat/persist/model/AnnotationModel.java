...
/**
 * The default annotation-based entity model. An <code>AnnotationModel</code> is based on
 * annotations that are specified for entity classes and their key fields.
 *
 * <p>{@code AnnotationModel} objects are thread-safe. Multiple threads may safely call the methods
 * of a shared {@code AnnotationModel} object.
 *
 * <p>The set of persistent classes in the annotation model is the set of all classes with the
 * {@link Persistent} or {@link Entity} annotation.
 *
 * <p>The annotations used to define persistent classes are: {@link Entity}, {@link Persistent},
 * {@link PrimaryKey}, {@link SecondaryKey} and {@link KeyField}. A good starting point is {@link
 * Entity}.
 *
 * @author Mark Hayes
 */
public class AnnotationModel extends EntityModel {
   ...
   * [#19377]
   */
  private final Set<String> registeredSpecialClasses;
  ...
  public synchronized ClassMetadata getClassMetadata(String className) {
    ...
    if (metadata == null) {
      ...
      if (entity != null && persistent != null) {
        throw new IllegalArgumentException(
            "Both @Entity and @Persistent are not allowed: " + type.getName());
      }
      boolean isEntity;
      int version;
  ...
  /**
   * Fills in the fields array and returns true if the default rules for field persistence were
   * overridden.
   */
  ...
  private PrimaryKeyMetadata getPrimaryKey(Class<?> type, List<Field> fields) {
    ...
    if (foundField != null) {
      return new PrimaryKeyMetadata(
          foundField.getName(), foundField.getType().getName(), type.getName(), sequence);
    } else {
      return null;
    }
  }
  ...
  /**
   * Add newly discovered metadata to our stash of entity info. This info is maintained as it is
   * discovered because it would be expensive to create it on demand -- all class metadata would
