...
/**
 * An ASM ClassVisitor that examines a class, throws NotPersistentException if it is not persistent,
 * or enhances it if it is persistent. A class is persistent if it contains the @Entity
 * or @Persistent annotations. A resulting enhanced class implements the
 * com.sleepycat.persist.impl.Enhanced interface.
 *
 * <p>NotPersistentException is thrown to abort the transformation in order to avoid making two
 * passes over the class file (one to look for the annotations and another to enhance the bytecode)
 * or outputing a class that isn't enhanced. By aborting the transformation as soon as we detect
 * that the annotations are missing, we make only one partial pass for a non-persistent class.
 *
 * @author Mark Hayes
 */
class BytecodeEnhancer extends ClassVisitor {
  ...
  public FieldVisitor visitField(int access, String name, String desc, String sig, Object value) {
    ...
    if ((access & ACC_STATIC) == 0) {
      FieldInfo info = new FieldInfo(ret, name, desc, (access & ACC_TRANSIENT) != 0);
  ...
  public MethodVisitor visitMethod(
    ...
    if ("<clinit>".equals(name)) {
      ...
      return cv.visitMethod(ACC_PRIVATE + ACC_STATIC, staticBlockMethod, "()V", null, null);
    }
   ...
   * output.writeKeyObject(theField, format); // or output.writeInt(theField); // and other simple
   * types // or if no primary key but has superclass: return super.bdbWritePriKeyField(output,
   * format); }
   */
  private void genBdbWritePriKeyField() {
    MethodVisitor mv =
        cv.visitMethod(
            ACC_PUBLIC,
            "bdbWritePriKeyField",
            "(Lcom/sleepycat/persist/impl/EntityOutput;" + "Lcom/sleepycat/persist/impl/Format;)V",
            null,
            null);
    mv.visitCode();
    if (priKeyField != null) {
  ...
  private void genBdbWriteSecKeyFields() {
     ...
     */
    if (priKeyField != null && isRefType(priKeyField.type) && !priKeyField.isString) {
  ...
  /** output.registerPriKeyObject(priKeyField); // or input.registerPriKeyObject(priKeyField); */
  private void genRegisterPrimaryKey(MethodVisitor mv, boolean input) {
    ...
    mv.visitFieldInsn(GETFIELD, className, priKeyField.name, priKeyField.type.getDescriptor());
    mv.visitMethodInsn(
        INVOKEINTERFACE, entityInputOrOutputClass, "registerPriKeyObject", "(Ljava/lang/Object;)V");
  }

  /** input.registerPriStringKeyObject(priKeyField); */
  private void genRegisterPrimaryStringKey(MethodVisitor mv) {
    String entityInputOrOutputClass = "com/sleepycat/persist/impl/EntityInput";
    mv.visitVarInsn(ALOAD, 1);
    mv.visitVarInsn(ALOAD, 0);
    mv.visitFieldInsn(GETFIELD, className, priKeyField.name, priKeyField.type.getDescriptor());
    mv.visitMethodInsn(
        INVOKEINTERFACE,
        entityInputOrOutputClass,
        "registerPriStringKeyObject",
        "(Ljava/lang/Object;)V");
  }

  /**
   * public void bdbWriteNonKeyFields(EntityOutput output) { // like bdbWriteSecKeyFields but does
   * not call registerPriKeyObject }
   */
   ...
   * superLevel) { // like bdbReadSecKeyFields but does not call registerPriKeyObject }
   */
  ...
  /**
   * public void bdbWriteCompositeKeyFields(EntityOutput output, Format[] formats) {
   * output.writeInt(compositeKeyField1); output.writeKeyObject(compositeKeyField2, formats[1]); //
   * etc }
   */
  ...
  /**
   * public void bdbReadCompositeKeyFields(EntityInput input, Format[] formats) { compositeKeyField1
   * = input.readInt(); compositeKeyField2 = input.readKeyObject(formats[1]); }
   */
  ...
  /** output.writeInt(field); // and other primitives // or output.writeObject(field, null); */
  ...
  /**
   * field = input.readInt(); // and other primitives // or field = (FieldClass) input.readObject();
   */
  ...
  /**
   * public Object bdbGetField(Object o, int field, int superLevel, boolean isSecField) { if
   * (superLevel > 0) { // if has superclass: return super.bdbGetField (o, field, superLevel - 1,
   * isSecField); } else if (isSecField) { switch (field) { case 0: return Integer.valueOf(f2); case
   * 1: return f3; case 2: return f4; } } else { switch (field) { case 0: return
   * Integer.valueOf(f5); case 1: return f6; case 2: return f7; } } return null; }
   */
  private void genBdbGetField() {
    MethodVisitor mv =
        cv.visitMethod(
            ACC_PUBLIC, "bdbGetField", "(Ljava/lang/Object;IIZ)Ljava/lang/Object;", null, null);
    mv.visitCode();
    mv.visitVarInsn(ILOAD, 3);
    Label l0 = new Label();
    ...
    if (hasPersistentSuperclass) {
      ...
      mv.visitVarInsn(ILOAD, 4);
      mv.visitMethodInsn(
          INVOKESPECIAL, className, "bdbGetField", "(Ljava/lang/Object;IIZ)Ljava/lang/Object;");
      mv.visitInsn(ARETURN);
    } else {
      mv.visitJumpInsn(GOTO, l1);
    }
  ...
  /**
   * public void bdbSetPriField(Object o, Object value) { if (priKeyField != null) { thisField =
   * (TheFieldClass) value; } else if (super != null) { // if has superclass:
   * super.bdbSetPriField(o, value) } }
   */
  private void genBdbSetPriField() {
    MethodVisitor mv =
        cv.visitMethod(
            ACC_PUBLIC, "bdbSetPriField", "(Ljava/lang/Object;Ljava/lang/Object;)V", null, null);
    mv.visitCode();
    if (priKeyField != null) {
      mv.visitVarInsn(ALOAD, 0);
      mv.visitVarInsn(ALOAD, 2);
    ...
    switch (sort) {
      case Type.BOOLEAN:
        mv.visitMethodInsn(
            ...
            "com/sleepycat/persist/impl/EntityOutput",
            "writeBoolean",
            "(Z)Lcom/sleepycat/bind/tuple/TupleOutput;");
        break;
      case Type.CHAR:
        mv.visitMethodInsn(
            INVOKEINTERFACE,
            "com/sleepycat/persist/impl/EntityOutput",
            "writeChar",
            "(I)Lcom/sleepycat/bind/tuple/TupleOutput;");
        break;
      case Type.BYTE:
        mv.visitMethodInsn(
            INVOKEINTERFACE,
      ...
      case Type.LONG:
        mv.visitMethodInsn(
            ...
            "com/sleepycat/persist/impl/EntityOutput",
            "writeLong",
            "(J)Lcom/sleepycat/bind/tuple/TupleOutput;");
        break;
      case Type.FLOAT:
        mv.visitMethodInsn(
            INVOKEINTERFACE,
  ...
  private void genReadPrimitive(MethodVisitor mv, int sort) {
    switch (sort) {
      case Type.BOOLEAN:
        mv.visitMethodInsn(
            INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityInput", "readBoolean", "()Z");
        break;
      case Type.CHAR:
        mv.visitMethodInsn(
            INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityInput", "readChar", "()C");
        break;
      case Type.BYTE:
        mv.visitMethodInsn(
            INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityInput", "readByte", "()B");
        break;
      case Type.SHORT:
        mv.visitMethodInsn(
            INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityInput", "readShort", "()S");
        break;
      case Type.INT:
        mv.visitMethodInsn(
            INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityInput", "readInt", "()I");
        break;
      case Type.LONG:
        mv.visitMethodInsn(
            INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityInput", "readLong", "()J");
        break;
      case Type.FLOAT:
        mv.visitMethodInsn(
            INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityInput", "readSortedFloat", "()F");
        break;
      case Type.DOUBLE:
        mv.visitMethodInsn(
            INVOKEINTERFACE, "com/sleepycat/persist/impl/EntityInput", "readSortedDouble", "()D");
        break;
      default:
        throw DbCompat.unexpectedState(String.valueOf(sort));
    }
  ...
  /**
   * Returns the type name for a visitTypeInsn operand, which is the internal name for an object
   * type and the descriptor for an array type. Must not be called for a non-reference type.
   */
  ...
  /**
   * Call this method before comparing a non-reference operand to zero as an int, for example, with
   * IFNE, IFEQ, IFLT, etc. If the operand is a long, float or double, this method will compare it
   * to zero and leave the result as an int operand.
   */
  ...
  /**
   * Aborts the enhancement process when we determine that enhancement is unnecessary or not
   * possible.
   */
  ...
  private abstract static class AnnotationInfo extends AnnotationVisitor {
