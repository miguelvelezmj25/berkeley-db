 ...
 *
 * <p>{@code EntityJoin} objects are thread-safe. Multiple threads may safely call the methods of a
 * shared {@code EntityJoin} object.
 *
 * <p>An equality join is a match on all entities in a given primary index that have two or more
 * specific secondary key values. Note that key ranges may not be matched by an equality join, only
 * exact keys are matched.
 *
 * <p>For example:
 *
 * <pre class="code">
...
public class EntityJoin<PK, E> {
  ...
  private final PrimaryIndex<PK, E> primary;
  ...
  /**
   * Adds a secondary key condition to the equality join. Only entities having the given key value
   * in the given secondary index will be returned by the join operation.
   *
  ...
  /**
   * Opens a cursor that returns the entities qualifying for the join. The join operation is
   * performed as the returned cursor is accessed.
   *
  ...
  /**
   * Opens a cursor that returns the primary keys of entities qualifying for the join. The join
   * operation is performed as the returned cursor is accessed.
   *
   * <p>The operations performed with the cursor will not be transaction protected, and {@link
   * CursorConfig#DEFAULT} is used implicitly.
   *
  ...
  private class JoinForwardCursor<V> implements ForwardCursor<V> {
    ...
    public V next(LockMode lockMode) throws DatabaseException {
      ...
      if (doKeys) {
        ...
        OperationStatus status = joinCursor.getNext(key, data, lockMode);
        if (status == OperationStatus.SUCCESS) {
