 ...
 *
 * <p>Mutations are configured when a store is opened via {@link StoreConfig#setMutations
 * StoreConfig.setMutations}. For example:
 *
 * <pre class="code">
 ...
 *
 * <p>Mutations cause data conversion to occur lazily as instances are read from the store. The
 * {@link EntityStore#evolve EntityStore.evolve} method may also be used to perform eager
 * conversion.
 *
 * <p>Not all incompatible class changes can be handled via mutations. For example, complex
 * refactoring may require a transformation that manipulates multiple entity instances at once. Such
 * changes are not possible with mutations but can made by performing a <a
...
public class Mutations implements Serializable {
  ...
  private final Map<Mutation, Renamer> renamers;
  private final Map<Mutation, Deleter> deleters;
  private final Map<Mutation, Converter> converters;
  ...
  /**
   * Returns the renamer mutation for the given class, version and field, or null if none exists. A
   * null field name should be specified to get a class renamer.
   *
  ...
  /**
   * Returns the deleter mutation for the given class, version and field, or null if none exists. A
   * null field name should be specified to get a class deleter.
   *
   * @param className the class name.
   * @param classVersion the class version.
   ...
   * A null field name should be specified to get a class converter.
   *
   * @param className the class name.
   * @param classVersion the class version.
   * @param fieldName the field name.
  ...
  /**
   * Returns true if this collection has the same set of mutations as the given collection and all
   * mutations are equal.
   */
  @Override
