 ...
 *
 * <p>When a Converter is used with an entity class, secondary keys cannot be automatically deleted
 * based on field deletion, because field Deleter objects are not used in conjunction with a
 * Converter mutation. The EntityConverter can be used instead of a plain Converter to specify the
 * key names to be deleted.
 *
 * <p>It is not currently possible to rename or insert secondary keys when using a Converter
 * mutation with an entity class.
 *
 * @see Converter
...
public class EntityConverter extends Converter {
  ...
  /**
   * Creates a mutation for converting all instances of the given entity class version to the
  ...
  public EntityConverter(
      String entityClassName, int classVersion, Conversion conversion, Set<String> deletedKeys) {
  ...
  /**
   * Returns true if the deleted and renamed keys are equal in this object and given object, and if
   * the {@link Converter#equals} superclass method returns true.
   */
  @Override
