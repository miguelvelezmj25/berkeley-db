...
/**
 * The EntityIndex returned by SecondaryIndex.subIndex. A SubIndex, in JE internal terms, is a
 * duplicates btree for a single key in the main btree. From the user's viewpoint, the keys are
 * primary keys. This class implements that viewpoint. In general, getSearchBoth and
 * getSearchBothRange are used where in a normal index getSearchKey and getSearchRange would be
 * used. The main tree key is always implied, not passed as a parameter.
 *
 * @author Mark Hayes
 */
class SubIndex<PK, E> implements EntityIndex<PK, E> {

  private final SecondaryIndex<?, PK, E> secIndex;
  private final SecondaryDatabase db;
  private final boolean transactional;
  private final boolean sortedDups;
  private final boolean locking;
  private final boolean concurrentDB;
  private final DatabaseEntry keyEntry;
  private final Object keyObject;
  private final KeyRange singleKeyRange;
  private final EntryBinding pkeyBinding;
  private final KeyRange emptyPKeyRange;
  private final EntityBinding entityBinding;
  private final ValueAdapter<PK> keyAdapter;
  private final ValueAdapter<E> entityAdapter;
  ...
  <SK> SubIndex(SecondaryIndex<SK, PK, E> secIndex, EntityBinding entityBinding, SK key)
    ...
    locking = DbCompat.getInitializeLocking(db.getEnvironment().getConfig());
    ...
    keyAdapter = new PrimaryKeyValueAdapter<PK>(priIndex.keyClass, priIndex.keyBinding);
  ...
  public boolean contains(Transaction txn, PK key, LockMode lockMode) throws DatabaseException {
    ...
    OperationStatus status = db.getSearchBoth(txn, keyEntry, pkeyEntry, dataEntry, lockMode);
  ...
  public EntityResult<E> get(Transaction txn, PK key, Get getType, ReadOptions options)
    ...
    OperationResult result = db.get(txn, keyEntry, pkeyEntry, dataEntry, Get.SEARCH_BOTH, options);

    if (result != null) {
      return new EntityResult<>((E) entityBinding.entryToObject(pkeyEntry, dataEntry), result);
    } else {
      return null;
    }
  }
  ...
  public long count() throws DatabaseException {
    ...
    CursorConfig cursorConfig = locking ? CursorConfig.READ_UNCOMMITTED : null;
  ...
  /* <!-- begin JE only --> */
  public OperationResult delete(Transaction txn, PK key, WriteOptions options)
      throws DatabaseException {
  ...
  private OpResult deleteInternal(Transaction txn, PK key, OpWriteOptions options)
      throws DatabaseException {
    ...
    if (transactional && txn == null && DbCompat.getThreadTransaction(env) == null) {
      txn = env.beginTransaction(null, secIndex.getAutoCommitTransactionConfig());
      autoCommit = true;
    }
    ...
    try {
      ...
      if (DbCompat.IS_JE) {
        ...
        if (options.jeOptions != null && options.jeOptions.getCacheMode() != null) {
        ...
        OperationResult result =
            cursor.get(keyEntry, pkeyEntry, dataEntry, Get.SEARCH_BOTH, readOptions);
        if (result != null) {
      ...
      OperationStatus status =
          cursor.getSearchBoth(keyEntry, pkeyEntry, dataEntry, locking ? LockMode.RMW : null);
      if (status == OperationStatus.SUCCESS) {
  ...
  public EntityCursor<PK> keys(PK fromKey, boolean fromInclusive, PK toKey, boolean toInclusive)
    ...
    return cursor(null, fromKey, fromInclusive, toKey, toInclusive, keyAdapter, null);
  }
  ...
  public EntityCursor<PK> keys(
    ...
    return cursor(txn, fromKey, fromInclusive, toKey, toInclusive, keyAdapter, config);
  }

  public EntityCursor<E> entities(PK fromKey, boolean fromInclusive, PK toKey, boolean toInclusive)
      throws DatabaseException {

    return cursor(null, fromKey, fromInclusive, toKey, toInclusive, entityAdapter, null);
  }
  ...
  public EntityCursor<E> entities(
    ...
    return cursor(txn, fromKey, fromInclusive, toKey, toInclusive, entityAdapter, config);
  }
  ...
  private <V> EntityCursor<V> cursor(
    ...
    KeyRange pkeyRange = emptyPKeyRange.subRange(fromEntry, fromInclusive, toEntry, toInclusive);
    return cursor(txn, pkeyRange, adapter, config);
  }

  private <V> EntityCursor<V> cursor(
      Transaction txn, KeyRange pkeyRange, ValueAdapter<V> adapter, CursorConfig config)
      throws DatabaseException {

    Cursor cursor = db.openCursor(txn, config);
    RangeCursor rangeCursor = new RangeCursor(singleKeyRange, pkeyRange, sortedDups, cursor);
  ...
  public synchronized SortedMap<PK, E> sortedMap() {
    if (map == null) {
      map =
          (SortedMap)
              ((StoredSortedMap) secIndex.sortedMap()).duplicatesMap(keyObject, pkeyBinding);
    }
    return map;
  }
}
