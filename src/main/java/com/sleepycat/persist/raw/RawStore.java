...
/**
 * Provides access to the raw data in a store for use by general purpose tools. A <code>RawStore
 * </code> provides access to stored entities without using entity classes or key classes. Keys are
 * represented as simple type objects or, for composite keys, as {@link RawObject} instances, and
 * entities are represented as {@link RawObject} instances.
 *
 * <p>{@code RawStore} objects are thread-safe. Multiple threads may safely call the methods of a
 * shared {@code RawStore} object.
 *
 * <p>When using a {@code RawStore}, the current persistent class definitions are not used. Instead,
 * the previously stored metadata and class definitions are used. This has several implications:
 *
 * <ol>
 *   <li>An {@code EntityModel} may not be specified using {@link StoreConfig#setModel}. In other
 *       words, the configured model must be null (the default).
 *   <li>When storing entities, their format will not automatically be evolved to the current class
 *       definition, even if the current class definition has changed.
 * </ol>
 *
 * @author Mark Hayes
 */
...
/* <!-- end JE only --> */
{
   ...
   * @throws StoreNotFoundException when the {@link StoreConfig#setAllowCreate AllowCreate}
