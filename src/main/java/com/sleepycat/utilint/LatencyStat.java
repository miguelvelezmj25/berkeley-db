...
/**
 * A stat that keeps track of latency in milliseconds and presents average, min, max, 95th and 99th
 * percentile values.
 */
public class LatencyStat implements Cloneable {
  ...
  /**
   * Generate the min, max, avg, 95th and 99th percentile for the collected measurements. Do not
   * clear the measurement collection.
   */
  ...
  /**
   * Generate the min, max, avg, 95th and 99th percentile for the collected measurements, then clear
  ...
  /**
   * Calculate may be called on a stat that is concurrently updating, so while it has to be thread
   * safe, it's a bit inaccurate when there's concurrent activity. That tradeoff is made in order to
   * avoid the cost of synchronization during the set() method. See {@link #trackedValues}.
   */
  ...
  /**
   * Record "numRecordedOps" (one or more) operations that took place in a single request of
   * "nanoLatency" nanos.
   */
