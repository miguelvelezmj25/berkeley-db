...
/**
 * Maintain interval and cumulative stats for a given set of operations, as well as a
 * activityCounter that generates thread dumps if operations take too long. The markStart and
...
public class StatsTracker<T> {
  ...
  public StatsTracker(
    ...
    activityCounter =
        new ActivityCounter(
            activeThreadThreshold, threadDumpIntervalMillis, threadDumpMax, stackTraceLogger);
  }
   ...
   *
   * @param startTime should be the value returned by the corresponding call to markStart
   */
  ...
  /**
   * Should be called after each interval latency stat collection, to reset for the next period's
   * collection.
   */
