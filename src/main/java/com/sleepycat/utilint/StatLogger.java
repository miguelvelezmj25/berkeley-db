...
public class StatLogger {
   ...
   * Parameters control the size and number of rotating log files used. For a rotating set of files,
   * as each file reaches a given size limit, it is closed, rotated out, and a new file opened. The
   * name of the log file is filename.fileext. Successively older files are named by adding "0",
   * "1", "2", etc into the file name. The format is filename.[version number].fileext.
   *
   ...
   * @throws IllegalArgumentException if the log directory is not a directory or if the log file is
   *     not a file.
   */
  public StatLogger(File logdir, String filename, String fileext, int filecount, int rowcount)
      throws IOException {
  ...
  /**
   * Sets the log file header. A new log file may be created if the header does not match the header
  ...
  /**
   * logDelta writes the string if the string is different than the last written log record. The
   * first column is ignored when checking for a difference (current implementation has the time the
   * record is logged as the first column.
   *
