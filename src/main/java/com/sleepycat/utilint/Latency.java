...
/**
 * A struct holding the min, max, avg, 95th, and 99th percentile measurements for the collection of
 * values held in a LatencyStat.
 */
public class Latency implements Serializable, Cloneable {
  ...
  public String toString() {
    ...
    return "maxTrackedLatencyMillis="
        ...
        + " totalOps="
        + FORMAT.format(totalOps)
        + " totalReq="
        + FORMAT.format(totalRequests)
        + " reqOverflow="
        + FORMAT.format(opsOverflow)
        + " min="
        + FORMAT.format(min)
        + " max="
        + FORMAT.format(max)
        ...
        + FORMAT.format(avg)
        + " 95%="
        + FORMAT.format(percent95)
        + " 99%="
        + FORMAT.format(percent99);
  }
  ...
  /**
   * Add the measurements from "other" and recalculate the min, max, and average values. The 95th
   * and 99th percentile are not recalculated, because the histogram from LatencyStatis not
   * available, and those values can't be generated.
   */
  public void rollup(Latency other) {
    ...
    avg =
        ((totalRequests * avg) + (other.totalRequests * other.avg))
