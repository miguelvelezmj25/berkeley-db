...
/**
 * Tracks the number of operations begun, as a way of measuring level of activity. Is capable of
 * displaying thread dumps if the activity level reaches a specified ceiling
 */
public class ActivityCounter {
  ...
  public ActivityCounter(
      int activeThreshold, long requiredIntervalMillis, int maxNumDumps, Logger logger) {
  ...
  /**
   * If the activity level is above a threshold, there is no other thread that is dumping now, and a
   * dump hasn't happened for a while, dump thread stack traces.
   */
  ...
  private class GetStackTraces implements Runnable {
    ...
    public void run() {
      ...
      if (!threadDumpInProgress.compareAndSet(false, true)) {
        logger.warning(
            "Unexpected: ActivityCounter stack trace "
