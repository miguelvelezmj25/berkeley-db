...
/**
 * Statistics returned from {@link Database#preload Database.preload} or {@link
 * Environment#preload}.
 */
public class PreloadStats implements Serializable {
  ...
  /**
   * @deprecated returns zero for data written using JE 5.0 and later, but may return non-zero
   *     values when reading older data.
   */
  ...
  /**
   * Returns the count of the number of times that the internal memory budget specified by {@link
   * com.sleepycat.je.PreloadConfig#setInternalMemoryLimit PreloadConfig.setInternalMemoryLimit()}
   * was exceeded.
   */
