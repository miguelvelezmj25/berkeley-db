...
public class JEManagedConnection implements ManagedConnection {
  ...
  private final TransactionConfig savedTransConfig;
  ...
  protected void sendConnectionEvent(ConnectionEvent connEvent) {
    for (int i = listeners.size() - 1; i >= 0; i--) {
      ...
      } else if (connEvent.getId() == ConnectionEvent.CONNECTION_ERROR_OCCURRED) {
        listener.connectionErrorOccurred(connEvent);
      } else if (connEvent.getId() == ConnectionEvent.LOCAL_TRANSACTION_STARTED) {
        listener.localTransactionStarted(connEvent);
      } else if (connEvent.getId() == ConnectionEvent.LOCAL_TRANSACTION_COMMITTED) {
        listener.localTransactionCommitted(connEvent);
      } else if (connEvent.getId() == ConnectionEvent.LOCAL_TRANSACTION_ROLLEDBACK) {
  ...
  Database openDatabase(String dbName, DatabaseConfig config) throws DatabaseException {
    ...
    if (config.getReadOnly()) {
      synchronized (roDatabaseHandleCache) {
        return openDatabaseInternal(roDatabaseHandleCache, dbName, config);
      }
    } else {
      synchronized (rwDatabaseHandleCache) {
        return openDatabaseInternal(rwDatabaseHandleCache, dbName, config);
      }
    }
  }

  SecondaryDatabase openSecondaryDatabase(
      String dbName, Database primaryDatabase, SecondaryConfig config) throws DatabaseException {
  ...
  private Database openDatabaseInternal(
      Map<String, Database> databaseHandleCache, String dbName, DatabaseConfig config)
  ...
  private SecondaryDatabase openSecondaryDatabaseInternal(
      Map<String, Database> databaseHandleCache,
      String dbName,
    ...
    if (config.getExclusiveCreate()) {
      db = env.openSecondaryDatabase(null, dbName, primaryDatabase, config);
      databaseHandleCache.put(dbName, db);
    } else {
  ...
  private void removeDatabaseFromCache(Map<String, Database> cache, String dbName)
      throws DatabaseException {
