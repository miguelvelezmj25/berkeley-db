 ...
 * EnvironmentMutableConfig is a parameter to {@link Environment#setMutableConfig} and is returned
 * by {@link Environment#getMutableConfig}.
 *
 * <p>There are two types of mutable environment properties: per-environment handle properties, and
 * environment wide properties.
 *
 * <h4>Per-Environment Handle Properties</h4>
 *
 * <p>Per-environment handle properties apply only to a single Environment instance. For example, to
 * change the default transaction commit behavior for a single environment handle, do this:
 *
 * <blockquote>
 *
 * <pre>
 ...
 * </blockquote>
 *
 * <p>The per-environment handle properties are listed below. These properties are accessed using
 * the setter and getter methods listed, as shown in the example above.
 *
 * <ul>
 *   <li>{@link #setDurability}, {@link #getDurability}
 *   <li>{@link #setTxnNoSync}, {@link #getTxnNoSync} <em>deprecated</em>
 *   <li>{@link #setTxnWriteNoSync}, {@link #getTxnWriteNoSync} <em>deprecated</em>
 * </ul>
 *
 * <h4>Environment-Wide Mutable Properties</h4>
 *
 * <p>Environment-wide mutable properties are those that can be changed for an environment as a
 * whole, irrespective of which environment instance (for the same physical environment) is used.
 * For example, to stop the cleaner daemon thread, do this:
 *
 * <blockquote>
 *
 * <pre>
 ...
 * </blockquote>
 *
 * <p>The environment-wide mutable properties are documented as such for each EnvironmentConfig
 * String constant.
 *
...
public class EnvironmentMutableConfig implements Cloneable, Serializable {
  ...
  /**
   * Cache size is a category of property that is calculated within the environment. It is only
   * supplied when returning the cache size to the application and never used internally; internal
   * code directly checks with the MemoryBudget class;
   */
  ...
  /**
   * Note that in the implementation we choose not to extend Properties in order to keep the
  ...
  /**
   * Internal boolean that says whether or not to validate params. Setting it to false means that
   * parameter value validatation won't be performed during setVal() calls. Only should be set to
   * false by unit tests using DbInternal.
   */
   ...
   *
   * @param noSync If true, do not write or synchronously flush the log on transaction commit. This
   *     means that transactions exhibit the ACI (Atomicity, Consistency, and Isolation) properties,
   *     but not D (Durability); that is, database integrity is maintained, but if the JVM or
   *     operating system fails, it is possible some number of the most recently committed
   *     transactions may be undone during recovery. The number of transactions at risk is governed
   *     by how many updates fit into a log buffer, how often the operating system flushes dirty
   *     buffers to disk, and how often the database environment is checkpointed.
   *     <p>This attribute is false by default for this class and for the database environment.
  ...
  public void setTxnNoSyncVoid(boolean noSync) {
    TransactionConfig.checkMixedMode(false, noSync, txnWriteNoSync, getDurability());
    txnNoSync = noSync;
  }
   ...
   *
   * @return true if the database environment is configured for asynchronous transactions.
   ...
   *
   * @param writeNoSync If true, write but do not synchronously flush the log on transaction commit.
   *     This means that transactions exhibit the ACI (Atomicity, Consistency, and Isolation)
   *     properties, but not D (Durability); that is, database integrity is maintained, but if the
   *     operating system fails, it is possible some number of the most recently committed
   *     transactions may be undone during recovery. The number of transactions at risk is governed
   *     by how often the operating system flushes dirty buffers to disk, and how often the database
   *     environment is checkpointed.
   *     <p>The motivation for this attribute is to provide a transaction that has more durability
   *     than asynchronous (nosync) transactions, but has higher performance than synchronous
   *     transactions.
   *     <p>This attribute is false by default for this class and for the database environment.
   * @deprecated replaced by {@link #setDurability}
   */
  ...
  public void setTxnWriteNoSyncVoid(boolean writeNoSync) {
    TransactionConfig.checkMixedMode(false, txnNoSync, writeNoSync, getDurability());
    txnWriteNoSync = writeNoSync;
  }

  /**
   * Returns true if the database environment is configured for transactions which write but do not
   * flush the log.
   *
   * @return true if the database environment is configured for transactions which write but do not
   *     flush the log.
   * @deprecated replaced by {@link #getDurability}
   */
  ...
  /** @hidden The void return setter for use by Bean editors. */
  public void setCacheSizeVoid(long totalBytes) throws IllegalArgumentException {

    DbConfigManager.setVal(
        props, EnvironmentParams.MAX_MEMORY, Long.toString(totalBytes), validateParams);
  }

  /**
   * Returns the memory available to the database system, in bytes. A valid value is only available
   * if this EnvironmentConfig object has been returned from Environment.getConfig().
   *
  ...
  /**
   * A convenience method for setting {@link EnvironmentConfig#MAX_MEMORY_PERCENT}.
   *
  ...
  /**
   * A convenience method for getting {@link EnvironmentConfig#MAX_MEMORY_PERCENT}.
   *
  ...
  /** A convenience method for setting {@link EnvironmentConfig#MAX_OFF_HEAP_MEMORY}. */
  ...
  /** A convenience method for getting {@link EnvironmentConfig#MAX_OFF_HEAP_MEMORY}. */
   ...
   *
   * @param totalBytes is an upper limit on the number of bytes used for data storage, or zero if no
   *     limit is desired.
   * @throws IllegalArgumentException if an invalid parameter is specified.
   * @return this
   * @see EnvironmentConfig#MAX_DISK
   */
  public EnvironmentMutableConfig setMaxDisk(long totalBytes) throws IllegalArgumentException {
  ...
  /** @hidden The void return setter for use by Bean editors. */
  public void setMaxDiskVoid(long totalBytes) throws IllegalArgumentException {

    DbConfigManager.setVal(
        props, EnvironmentParams.MAX_DISK, Long.toString(totalBytes), validateParams);
  }

  /**
   * A convenience method for getting {@link EnvironmentConfig#MAX_DISK}.
   *
   * @return the upper limit on the number of bytes used for data storage, or zero if no limit is
   *     set.
   * @see EnvironmentConfig#MAX_DISK
   */
  ...
  /**
   * Sets the exception listener for an Environment. The listener is called when a daemon thread
   * throws an exception, in order to provide a notification mechanism for these otherwise
   * asynchronous exceptions. Daemon thread exceptions are also printed through stderr.
   *
   * <p>Not all daemon exceptions are fatal, and the application bears responsibility for choosing
   * how to respond to the notification. Since exceptions may repeat, the application should also
   * choose how to handle a spate of exceptions. For example, the application may choose to act upon
   * each notification, or it may choose to batch up its responses by implementing the listener so
   * it stores exceptions, and only acts when a certain number have been received.
   *
   * @param exceptionListener the callback to be executed when an exception occurs.
   * @return this
   */
  ...
  /**
   * Sets the default {@code CacheMode} used for operations performed in this environment. The
   * default cache mode may be overridden on a per-database basis using {@link
   * DatabaseConfig#setCacheMode}, and on a per-record or per-operation basis using {@link
   * Cursor#setCacheMode}, {@link ReadOptions#setCacheMode(CacheMode)} or {@link
   * WriteOptions#setCacheMode(CacheMode)}.
   *
   * @param cacheMode is the default {@code CacheMode} used for operations performed in this
   *     environment. If {@code null} is specified, {@link CacheMode#DEFAULT} will be used.
  ...
  /**
   * Returns the default {@code CacheMode} used for operations performed in this environment, or
   * null if {@link CacheMode#DEFAULT} is used.
   *
   * @return the default {@code CacheMode} used for operations performed on this database, or null
   *     if {@link CacheMode#DEFAULT} is used.
   * @see #setCacheMode
   * @since 4.0.97
   */
  ...
  /**
   * Used by Environment to create a copy of the application supplied configuration. Done this way
   * to provide non-public cloning.
   */
  EnvironmentMutableConfig cloneMutableConfig() {
    EnvironmentMutableConfig copy = clone();
