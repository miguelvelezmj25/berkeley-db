...
public class DbPrintLog {
    ...
    try {
      ...
      if (customDumpReaderClass != null) {
        ...
        if (stats) {
          ...
          reader =
              new VLSNDistributionReader(
                  env, readBufferSize, startLsn, endLsn, endOfFileLsn, verbose, forwards);
   ...
   *      debugging files.
   * </pre>
   *     <p>All arguments are optional. The current directory is used if {@code -h} is not
   *     specified.
   */
  ...
  private static void usage() {
    ...
    System.out.println(
        " -k  <binary|text|hex|obfuscate> " + "(format for dumping the key and data)");
  ...
  /**
   * If a custom dump reader class is specified, we'll use that for DbPrintLog instead of the
   * regular DumpFileReader. The custom reader must have DumpFileReader as a superclass ancestor.
   * Its constructor must have this signature:
   *
   * <p>public class FooReader extends DumpFileReader {
   *
   * <p>public FooReader(EnvironmentImpl env, Integer readBufferSize, Long startLsn, Long finishLsn,
   * Long endOfFileLsn, String entryTypes, String txnIds, Boolean verbose, Boolean repEntriesOnly,
   * Boolean forwards) super(env, readBufferSize, startLsn, finishLsn, endOfFileLsn, entryTypes,
   * txnIds, verbose, repEntriesOnly, forwards);
   *
   * <p>See com.sleepycat.je.util.TestDumper, on the test side, for an example.
   */
  private DumpFileReader getDebugReader(
      String customDumpReaderClass,
      EnvironmentImpl env,
      int readBufferSize,
      long startLsn,
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "-c was specified, but couldn't load " + customDumpReaderClass + " ", e);
    }
    ...
    try {
      ...
      throw new IllegalStateException(
          "-c was specified, but couldn't instantiate " + customDumpReaderClass + " ", e);
    }
