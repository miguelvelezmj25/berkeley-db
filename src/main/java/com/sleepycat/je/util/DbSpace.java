...
public class DbSpace {
  ...
  private static final String USAGE =
      "usage: "
          + CmdUtil.getJavaCommand(DbSpace.class)
          + "\n"
          + "       -h <dir> # environment home directory\n"
          + "       [-q]     # quiet, print grand totals only\n"
          + "       [-u]     # sort by average utilization\n"
  ...
  /**
   * Sets the recalculation property, which if true causes a more expensive recalculation of
   * utilization to be performed for debugging purposes. This property is false by default.
   */
  ...
  /**
   * Sets the start file number, which is a lower bound on the range of files for which utilization
   * is reported and (optionally) recalculated. By default there is no lower bound.
   */
  ...
  /**
   * Sets the ending file number, which is an upper bound on the range of files for which
   * utilization is reported and (optionally) recalculated. By default there is no upper bound.
   */
  ...
  public void print(PrintStream out) throws DatabaseException {
    ...
    long finishFile =
        (finishLsn != DbLsn.NULL_LSN) ? DbLsn.getFileNumber(finishLsn) : Long.MAX_VALUE;

    SortedMap<Long, FileSummary> map =
        envImpl.getUtilizationProfile().getFileSummaryMap(true).subMap(startFile, finishFile);

    map.keySet().removeAll(envImpl.getCleaner().getFileSelector().getInProgressFiles());

    Map<Long, FileSummary> recalcMap =
        doRecalcUtil
            ? UtilizationFileReader.calcFileSummaryMap(envImpl, startLsn, finishLsn)
            : null;
    ...
    out.format(
        ...
        reservedSize / 1024, printReservedFiles ? ":" : ".");
