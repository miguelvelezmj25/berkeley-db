...
/**
 * Used to retrieve as much data as possible from a corrupted environment. This utility is meant to
 * be used programmatically, and is the equivalent to the -R or -r options for {@link DbDump}.
 *
 * <p>To scavenge a database:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>The recovered databases will put placed in the outputDirectory with ".dump" file suffixes. The
 * format of the .dump files will be suitable for use with DbLoad.
 */
public class DbScavenger extends DbDump {
   ...
   */
  private final Map<Long, DatabaseImpl> dbIdToImpl;

  /*
   * Map of database id to the .dump file output stream for that database.
   */
  private final Map<Long, PrintStream> dbIdToOutputStream;
   ...
   * @param doAggressiveScavengerRun true if true, then all data records are dumped, regardless of
   *     whether they are the latest version or not.
   * @param verbose true if status output should be written to System.out during scavenging.
   */
  public DbScavenger(
      Environment env,
  ...
  private void scavengeDbTree(long lastUsedLsn, long nextAvailableLsn) throws DatabaseException {
    ...
    final ScavengerFileReader scavengerReader =
        new ScavengerFileReader(
            envImpl, readBufferSize, lastUsedLsn, DbLsn.NULL_LSN, nextAvailableLsn) {
          protected void processEntryCallback(LogEntry entry, LogEntryType entryType)
  ...
  private long reportProgress(long fileNum, long lastLsn) {
    ...
    if (verbose) {
      if (currentFile != fileNum) {
        ...
        System.out.println(
            "processing file "
                + FileManager.getFileName(currentFile, ".jdb  ")
                + (now - lastTime)
                + " ms");
        lastTime = now;
      }
  ...
  private void processDbTreeEntry(LogEntry entry, LogEntryType entryType) throws DatabaseException {
    ...
    if (processThisEntry && (entry instanceof LNLogEntry)) {
      ...
      if (ln instanceof NameLN) {
        ...
        if (dbIdToName.containsKey(dbId) && !dbIdToName.get(dbId).equals(name)) {
          throw EnvironmentFailureException.unexpectedState(
  ...
  private PrintStream getOutputStream(Long dbId) throws DatabaseException {
    ...
    boolean dupSort = db != null && db.getSortedDuplicates();
