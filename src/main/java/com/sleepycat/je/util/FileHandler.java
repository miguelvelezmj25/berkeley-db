...
/**
 * JE instances of java.util.logging.Logger are configured to use this implementation of
 * java.util.logging.FileHandler. By default, the handler's level is {@link Level#INFO} To enable
 * the console output, use the standard java.util.logging.LogManager configuration to set the
 * desired level:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>The default destination for this output is a circular set of files named
 * &lt;environmentHome&gt;/je.info.# The logging file size can be configured with standard
 * java.util.logging.FileHandler configuration.
 *
 * <p>JE augments the java.util.logging API with a JE environment parameter for setting handler
 * levels. This is described in greater detail in {@link <a
 * href="{@docRoot}/../GettingStartedGuide/managelogging.html">Chapter 12.Administering Berkeley DB
 * Java Edition Applications</a>}
 *
 * @see <a href="{@docRoot}/../GettingStartedGuide/managelogging.html">Chapter 12. Logging</a>
...
public class FileHandler extends java.util.logging.FileHandler {
   ...
   */
  public FileHandler(
      String pattern, int limit, int count, Formatter formatter, EnvironmentImpl envImpl)
        ...
        new ErrorManager() {
          public void error(String msg, Exception e, int code) {
            if (STIFLE_DEFAULT_ERROR_MANAGER) {
              System.out.println("FileHandler stifled exception: " + e);
            } else {
    ...
    Level level =
        LoggerUtils.getHandlerLevel(
            envImpl.getConfigManager(),
            EnvironmentParams.JE_FILE_LEVEL,
