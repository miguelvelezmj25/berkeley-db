...
/**
 * Loads a database from a dump file generated by {@link DbDump}. This utility may be used
 ...
 *
 * <p>To load a database to a stream from code:
 *
 * <pre>
...
public class DbLoad {
  ...
  private static final String usageString =
      "usage: "
          + CmdUtil.getJavaCommand(DbLoad.class)
          + "\n"
          + "       -h <dir>             # environment home directory\n"
          + "       [-f <fileName>]      # input file\n"
          + "       [-n]                 # no overwrite mode\n"
   ...
   *     -I - ignore unknown parameters in the config file.<br>
   *     <p>If -f is not specified, the dump is read from System.in.
   *     <p>The -T option allows JE applications to easily load text files into databases.
   *     <p>The -I option allows loading databases that were dumped with the Berkeley DB C product,
   *     when the dump file contains parameters not known to JE.
   *     <p>The input must be paired lines of text, where the first line of the pair is the key
   *     item, and the second line of the pair is its corresponding data item.
   *     <p>A simple escape mechanism, where newline and backslash (\) characters are special, is
   ...
   *     <p>For this reason, any backslash or newline characters that naturally occur in the text
   *     input must be escaped to avoid misinterpretation by db_load.
   *     <p>-c name=value - Specify configuration options ignoring any value they may have based on
   *     the input. The command-line format is name=value. See the Supported Keywords section below
   ...
   *     -v - report progress <br>
   *     <p>Supported Keywords<br>
   *     version=N - specify the version of the input file. Currently only version 3 is supported.
   *     <br>
   *     format - specify the format of the file. Allowable values are "print" and "bytevalue".<br>
   *     dupsort - specify whether the database allows duplicates or not. Allowable values are
   *     "true" and "false".<br>
   *     type - specifies the type of database. Only "btree" is allowed.<br>
   *     database - specifies the name of the database to be loaded.<br>
  ...
  /**
   * If true, enables output of warning messages. Command line behavior is not available via the
   * public API.
   */
  ...
  /**
   * Sets whether to ignore unknown parameters in the config file. This allows loading databases
   * that were dumped with the Berkeley DB C product, when the dump file contains parameters not
   * known to JE.
   *
   * @param ignoreUnknownConfigMode True to ignore unknown parameters in the config file.
   */
  ...
  /**
   * If progressInterval is set, progress status messages are generated to stdout at set percentages
   * of the load.
   *
   * @param progressInterval Specifies the percentage intervals for status messages. If 0, no
  ...
  /**
   * Used for progress status messages. Must be set to greater than 0 if the progressInterval is
   * greater than 0.
   *
  ...
  public boolean load() throws IOException, DatabaseException {
    ...
    LoggerUtils.envLogMsg(
        Level.INFO, DbInternal.getNonNullEnvImpl(env), "DbLoad.load of " + dbName + " starting");
    ...
    LoggerUtils.envLogMsg(
        Level.INFO, DbInternal.getNonNullEnvImpl(env), "DbLoad.load of " + dbName + " ending.");
  ...
  private void loadData(Database db) throws DatabaseException, IOException {
    ...
    while (keyLine != null && !keyLine.equals("DATA=END")) {
      ...
      if (dataLine == null) {
        throw new IllegalArgumentException("No data to match key " + keyLine);
      }
      /* Add one for \n or \r. */
...
//        db.put(null, key, data);
  ...
  private static final byte backSlashValue = ((byte) ('\\')) & 0xff;
