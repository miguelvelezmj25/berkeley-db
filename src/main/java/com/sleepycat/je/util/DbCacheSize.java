 ...
 *
 * <p>To get an estimate of the in-memory footprint for a given database, specify the number of
 * records and database characteristics and DbCacheSize will return an estimate of the cache size
 * required for holding the database in memory. Based on this information a JE main cache size can
 * be chosen and then configured using {@link EnvironmentConfig#setCacheSize} or using the {@link
 * EnvironmentConfig#MAX_MEMORY} property. An off-heap cache may also be optionally configured using
 * {@link EnvironmentConfig#setOffHeapCacheSize} or using the {@link
 * EnvironmentConfig#MAX_OFF_HEAP_MEMORY} property.
 *
 * <h4>Importance of the JE Cache</h4>
 *
 * The JE cache is not an optional cache. It is used to hold the metadata for accessing JE data. In
 * fact the JE cache size is probably the most critical factor to JE performance, since Btree nodes
 * will have to be fetched during a database read or write operation if they are not in cache.
 * During a single read or write operation, at each level of the Btree that a fetch is necessary, an
 * IO may be necessary at a different disk location for each fetch. In addition, if internal nodes
 * (INs) are not in cache, then write operations will cause additional copies of the INs to be
 * written to storage, as modified INs are moved out of the cache to make room for other parts of
 * the Btree during subsequent operations. This additional fetching and writing means that sizing
 * the cache too small to hold the INs will result in lower operation performance.
 *
 * <p>For best performance, all Btree nodes should fit in the JE cache, including leaf nodes (LNs),
 * which hold the record data, and INs, which hold record keys and other metadata. However, because
 * system memory is limited, it is sometimes necessary to size the cache to hold all or at least
 * most INs, but not the LNs. This utility estimates the size necessary to hold only INs, and the
 * size to hold INs and LNs.
 *
 * <p>In addition, a common problem with large caches is that Java GC overhead can become
 * significant. When a Btree node is evicted from the JE main cache based on JE's LRU algorithm,
 * typically the node will have been resident in the JVM heap for an extended period of time, and
 * will be expensive to GC. Therefore, when most or all LNs do <em>not</em> fit in the main cache,
 * using {@link CacheMode#EVICT_LN} can be beneficial to reduce the Java GC cost of collecting the
 * LNs as they are moved out of the main cache. With EVICT_LN, the LNs only reside in the JVM heap
 * for a short period and are cheap to collect. A recommended approach is to size the JE main cache
 * to hold only INs, and size the Java heap to hold that amount plus the amount needed for GC
 * working space and application objects, leaving any additional memory for use by the file system
 * cache or the off-heap cache. Tests show this approach results in lower GC overhead and more
 * predictable latency.
 *
 * <p>Another issue is that 64-bit JVMs store object references using less space when the heap size
 * is slightly less than 32GiB. When the heap size is 32GiB or more, object references are larger
 * and less data can be cached per GiB of memory. This JVM feature is enabled with the <a
 * href="http://download.oracle.com/javase/7/docs/technotes/guides/vm/performance-enhancements-7.html#compressedOop">Compressed
 * Oops</a> (<code>-XX:+UseCompressedOops</code>) option, although in modern JVMs it is on by
 * default. Because of this factor, and because Java GC overhead is usually higher with larger
 * heaps, a maximum heap size slightly less than 32GiB is recommended, along with Compressed Oops
 * option.
 *
 * <p>Of course, the JE main cache size must be less than the heap size since the main cache is
 * stored in the heap. In fact, around 30% of free space should normally be reserved in the heap for
 * use by Java GC, to avoid high GC overheads. For example, if the application uses roughly 2GiB of
 * the heap, then with a 32GiB heap the JE main cache should normally be no more than 20GiB.
 *
 * <p>As of JE 6.4, an optional off-heap cache may be configured in addition to the main JE cache.
 * See {@link EnvironmentConfig#setOffHeapCacheSize} for information about the trade-offs in using
 * an off-heap cache. When the {@code -offheap} argument is specified, this utility displays sizing
 * information for both the main and off-heap caches. The portion of the data set that fits in the
 * main cache, and the off-heap size needed to hold the rest of the data set, will be shown. The
 * main cache size can be specified with the {@code -maincache} argument, or is implied to be the
 * amount needed to hold all internal nodes if this argument is omitted. Omitting this argument is
 * appropriate when {@link CacheMode#EVICT_LN} is used, since only internal nodes will be stored in
 * the main cache.
 *
 * <p>To reduce Java GC overhead, sometimes a small main cache is used along with an off-heap cache.
 * Note that it is important that the size the main cache is at least large enough to hold all the
 * upper INs (the INs at level 2 and above). This is because the off-heap cache does not contain
 * upper INs, it only contains LNs and bottom internal nodes (BINs). When a level 2 IN is evicted
 * from the main cache, its children (BINs and LNs) in the off-heap cache, if any, must also be
 * evicted, which can be undesirable, especially if the off-heap cache is not full. This utility
 * displays the main cache size needed to hold all upper INs, and displays a warning if this is
 * smaller than the main cache size specified.
 *
 * <h4>Estimating the JE Cache Size</h4>
 *
 * Estimating JE in-memory sizes is not straightforward for several reasons. There is some fixed
 * overhead for each Btree internal node, so fanout (maximum number of child entries per parent
 * node) and degree of node sparseness impacts memory consumption. In addition, JE uses various
 * compact in-memory representations that depend on key sizes, data sizes, key prefixing, how many
 * child nodes are resident, etc. The physical proximity of node children also allows compaction of
 * child physical address values.
 *
 * <p>Therefore, when running this utility it is important to specify all {@link EnvironmentConfig}
 * and {@link DatabaseConfig} settings that will be used in a production system. The {@link
 * EnvironmentConfig} settings are specified by command line options for each property, using the
 * same names as the {@link EnvironmentConfig} parameter name values. For example, {@link
 * EnvironmentConfig#LOG_FILE_MAX}, which influences the amount of memory used to store physical
 * record addresses, can be specified on the command line as:
 *
 * <p>{@code -je.log.fileMax LENGTH}
 *
 * <p>To be sure that this utility takes into account all relevant settings, especially as the
 * utility is enhanced in future versions, it is best to specify all {@link EnvironmentConfig}
 * settings used by the application.
 *
 * <p>The {@link DatabaseConfig} settings are specified using command line options defined by this
 * utility.
 *
 * <ul>
 *   <li>{@code -nodemax ENTRIES} corresponds to {@link DatabaseConfig#setNodeMaxEntries}.
 *   <li>{@code -duplicates} corresponds to passing true to {@link
 *       DatabaseConfig#setSortedDuplicates}. Note that duplicates are configured for DPL
 *       MANY_TO_ONE and MANY_TO_MANY secondary indices.
 *   <li>{@code -keyprefix LENGTH} corresponds to passing true {@link
 *       DatabaseConfig#setKeyPrefixing}. Note that key prefixing is always used when duplicates are
 *       configured.
 * </ul>
 *
 * <p>This utility estimates the JE cache size by creating an in-memory Environment and Database. In
 * addition to the size of the Database, the minimum overhead for the Environment is output. The
 * Environment overhead shown is likely to be smaller than actually needed because it doesn't take
 * into account use of memory by JE daemon threads (cleaner, checkpointer, etc) the memory used for
 * locks that are held by application operations and transactions, the memory for HA network
 * connections, etc. An additional amount should be added to account for these factors.
 *
 * <p>This utility estimates the cache size for a single JE Database, or a logical table spread
 * across multiple databases (as in the case of Oracle NoSQL DB, for example). To estimate the size
 * for multiple databases/tables with different configuration parameters or different key and data
 * sizes, run this utility for each database/table and sum the sizes. If you are summing multiple
 * runs for multiple databases/tables that are opened in a single Environment, the overhead size for
 * the Environment should only be added once.
 *
 * <p>In some applications with databases/tables having variable key and data sizes, it may be
 * difficult to determine the key and data size input parameters for this utility. If a
 * representative data set can be created, one approach is to use the {@link DbPrintLog} utility
 * with the {@code -S} option to find the average key and data size for all databases/tables, and
 * use these values as input parameters, as if there were only a single database/tables. With this
 * approach, it is important that the {@code DatabaseConfig} parameters are the same, or at least
 * similar, for all databases/tables.
 *
 * <h4>Key Prefixing and Compaction</h4>
 *
 * Key prefixing deserves special consideration. It can significantly reduce the size of the cache
 * and is generally recommended; however, the benefit can be difficult to predict. Key prefixing, in
 * turn, impacts the benefits of key compaction, and the use of the {@link
 * EnvironmentConfig#TREE_COMPACT_MAX_KEY_LENGTH} parameter.
 *
 * <p>For a given data set, the impact of key prefixing is determined by how many leading bytes are
 * in common for the keys in a single bottom internal node (BIN). For example, if keys are assigned
 * sequentially as long (8 byte) integers, and the {@link DatabaseConfig#setNodeMaxEntries maximum
 * entries per node} is 128 (the default value) then 6 or 7 of the 8 bytes of the key will have a
 * common prefix in each BIN. Of course, when records are deleted, the number of prefixed bytes may
 * be reduced because the range of key values in a BIN will be larger. For this example we will
 * assume that, on average, 5 bytes in each BIN are a common prefix leaving 3 bytes per key that are
 * unprefixed.
 *
 * <p>Key compaction is applied when the number of unprefixed bytes is less than a configured value;
 * see {@link EnvironmentConfig#TREE_COMPACT_MAX_KEY_LENGTH}. In the example, the 3 unprefixed bytes
 * per key is less than the default used for key compaction (16 bytes). This means that each key
 * will use 16 bytes of memory, in addition to the amount used for the prefix for each BIN. The
 * per-key overhead could be reduced by changing the {@code TREE_COMPACT_MAX_KEY_LENGTH} parameter
 * to a smaller value, but care should be taken to ensure the compaction will be effective as keys
 * are inserted and deleted over time.
 *
 * <p>Because key prefixing depends so much on the application key format and the way keys are
 * assigned, the number of expected prefix bytes must be estimated by the user and specified to
 * DbCacheSize using the {@code -keyprefix} argument.
 *
 * <h4>Key Prefixing and Duplicates</h4>
 *
 * When {@link DatabaseConfig#setSortedDuplicates duplicates} are configured for a Database
 * (including DPL MANY_TO_ONE and MANY_TO_MANY secondary indices), key prefixing is always used.
 * This is because the internal key in a duplicates database BIN is formed by concatenating the
 * user-specified key and data. In secondary databases with duplicates configured, the data is the
 * primary key, so the internal key is the concatenation of the secondary key and the primary key.
 *
 * <p>Key prefixing is always used for duplicates databases because prefixing is necessary to store
 * keys efficiently. When the number of duplicates per unique user-specified key is more than the
 * number of entries per BIN, the entire user-specified key will be the common prefix.
 *
 * <p>For example, a database that stores user information may use email address as the primary key
 * and zip code as a secondary key. The secondary index database will be a duplicates database, and
 * the internal key stored in the BINs will be a two part key containing zip code followed by email
 * address. If on average there are more users per zip code than the number of entries in a BIN,
 * then the key prefix will normally be at least as long as the zip code key. If there are less
 * (more than one zip code appears in each BIN), then the prefix will be shorter than the zip code
 * key.
 *
 * <p>It is also possible for the key prefix to be larger than the secondary key. If for one
 * secondary key value (one zip code) there are a large number of primary keys (email addresses),
 * then a single BIN may contain concatenated keys that all have the same secondary key (same zip
 * code) and have primary keys (email addresses) that all have some number of prefix bytes in
 * common. Therefore, when duplicates are specified it is possible to specify a prefix size that is
 * larger than the key size.
 *
 * <h4>Small Data Sizes and Embedded LNs</h4>
 *
 * Another special data representation involves small data sizes. When the data size of a record is
 * less than or equal to {@link EnvironmentConfig#TREE_MAX_EMBEDDED_LN} (16 bytes, by default), the
 * data is stored (embedded) in the BIN, and the LN is not stored in cache at all. This increases
 * the size needed to hold all INs in cache, but it decreases the size needed to hold the complete
 * data set. If the data size specified when running this utility is less than or equal to
 * TREE_MAX_EMBEDDED_LN, the size displayed for holding INs only will be the same as the size
 * displayed for holdings INs and LNs.
 *
 * <p>See {@link EnvironmentConfig#TREE_MAX_EMBEDDED_LN} for information about the trade-offs in
 * using the embedded LNs feature.
 *
 * <h4>Record Versions and Oracle NoSQL Database</h4>
 *
 * This note applies only to when JE is used with Oracle NoSQL DB. In Oracle NoSQL DB, an internal
 * JE environment configuration parameter is always used: {@code -je.rep.preserveRecordVersion
 * true}. This allows using record versions in operations such as "put if version", "delete if
 * version", etc. This feature performs best when the cache is sized large enough to hold the record
 * versions.
 *
 * <p>When using JE with Oracle NoSQL DB, always add {@code -je.rep.preserveRecordVersion true} to
 * the command line. This ensures that the cache sizes calculated are correct, and also outputs an
 * additional line showing how much memory is required to hold the internal nodes and record
 * versions (but not the leaf nodes). This is the minimum recommended size when the "... if version"
 * operations are used.
 *
 ...
 * </pre>
 *
 * <p>You should run DbCacheSize on the same target platform and JVM for which you are sizing the
 * cache, as cache sizes will vary. You may also need to specify -d32 or -d64 depending on your
 * target, if the default JVM mode is not the same as the mode to be used in production.
 *
 ...
 * command. However, if compressed oops is desired then it <em>must</em> be explicitly specified in
 * the Java command when running DbCacheSize or a JE application. If it is not explicitly specified
 * then JE will not aware of it, even if it is the JVM default setting, and will not take it into
 * account when calculating cache memory sizes.
 *
 * <p>For example:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>This indicates that the minimum memory size to hold only the internal nodes of the Database
 * Btree is approximately 24MB. The maximum size to hold the entire database, both internal nodes
 * and data records, is approximately 107MB. To either of these amounts, at least 3MB (plus more for
 * locks and daemons) should be added to account for the environment overhead.
 *
 * <p>The following example adds the use of an off-heap cache, where the main cache size is
 * specified to be 30MB.
 *
 * <pre>
 * $ java -jar je-X.Y.Z.jar DbCacheSize -records 554719 -key 16 -data 100 \
 ...
 *
 * <ul>
 *   <li>The environment overhead is larger because of memory used for the off-heap LRU.
 *   <li>To cache only internal nodes, an off-heap cache is not needed since the internal nodes take
 *       around 24MB, which when added to the 5MB overhead is less than the 30MB main cache
 *       specified. This is why the number of bytes on the second line is zero.
 *   <li>To cache all nodes, the main cache size specified should be used (25MB added to the 5MB
 *       overhead is 30MB), and an off-heap cache of around 71MB should be configured.
 * </ul>
 *
 * <h4>Output Properties</h4>
 *
 * <p>When {@code -outputproperties} is specified, a list of properties in Java properties file
 * format will be written to System.out, instead of the output shown above. The properties and their
 * meanings are listed below.
 *
 * <ul>
 *   <li>The following properties are always output (except allNodes, see below). They describe the
 *       estimated size of the main cache.
 *       <ul>
 *         <li><strong>overhead</strong>: The environment overhead, as shown under Environment Cache
 *             Overhead above.
 *         <li><strong>internalNodes</strong>: The Btree size in the main cache for holding the
 *             internal nodes. This is the "Internal nodes only" line above (followed by "MAIN
 *             cache" when {@code -offheap} is specified).
 *         <li><strong>internalNodesAndVersions</strong>: The Btree size needed to hold the internal
 *             nodes and record versions in the main cache. This value is zero when {@code -offheap}
 *             is specified; currently JE does not cache record versions off-heap unless their
 *             associated LNs are also cached off-heap, so there is no way to calculate this
 *             property.
 *         <li><strong>allNodes</strong>: The Btree size in the main cache needed to hold all nodes.
 *             This is the "Internal nodes and leaf nodes" line above (followed by "MAIN cache" when
 *             {@code -offheap} is specified). This property is not output unless {@code -data} is
 *             specified.
 *       </ul>
 *   <li>The following properties are output only when {@code -offheap} is specified. They describe
 *       the estimated size of the off-heap cache.
 *       <ul>
 *         <li><strong>minMainCache</strong>: The minimum size of the main cache needed to hold all
 *             upper INs. When the {@code -maincache} value specified is less than this minimum, not
 *             all internal nodes can be cached. See the discussion further above.
 *         <li><strong>offHeapInternalNodes</strong>: The size of the off-heap cache needed to hold
 *             the internal nodes. This is the "Internal nodes only: OFF_HEAP cache" line above.
 *         <li><strong>offHeapAllNodes</strong>: The size of the off-heap cache needed to hold all
 *             nodes. This is the "Internal nodes and leaf nodes: OFF_HEAP cache" line above. This
 *             property is not output unless {@code -data} is specified.
 *       </ul>
 *   <li>The following properties are deprecated but are output for compatibility with earlier
 *       releases.
 *       <ul>
 *         <li>minInternalNodes, maxInternalNodes, minAllNodes, and (when {@code -data} is
 *             specified) maxAllNodes
 *       </ul>
 * </ul>
 *
...
public class DbCacheSize {
  ...
  /**
   * Runs DbCacheSize as a command line utility. For command usage, see {@link DbCacheSize class
   * description}.
   */
  ...
  private static void usage(final String msg) {
    ...
    System.out.println(
        "usage:"
            + "\njava "
            + CmdUtil.getJavaCommand(DbCacheSize.class)
            + "\n   -records <count>"
            + "\n      # Total records (key/data pairs); required"
            + "\n   -key <bytes> "
            + "\n      # Average key bytes per record; required"
  ...
  void calculateCacheSizes() {
    ...
    try {
      ...
      if (offHeapCache) {
        ...
        if (mainCacheSize > 0 && mainCacheSize - envOverhead <= 1024 * 1024) {
      ...
      final int density = orderedInsertion ? ORDERED_DENSITY : DEFAULT_DENSITY;
  ...
  /**
   * Calculates main cache sizes as if there were no off-heap cache. During off-heap cache size
   * calculations, these numbers may be revised.
   */
  private void calcMainCacheSizes() {

    final long mainUINs = nUinNodes * uinWithTargets;

    mainNoLNsOrVLSNs = (nBinNodes * binNoLNsOrVLSNs) + mainUINs;

    mainNoLNsWithVLSNs = (nBinNodes * binNoLNsWithVLSNs) + mainUINs;

    mainWithLNsAndVLSNs = (nBinNodes * binWithLNsAndVLSNs) + mainUINs;
  }
  ...
  private void calcOffHeapNoLNsOrVLSNs() {
     ...
     */
    final long mainSpare =
        (mainDataSize > calcLevel2AndAboveSize()) ? (mainDataSize - calcLevel2AndAboveSize()) : 0;
  ...
  private void calcOffHeapWithLNsAndVLSNs() {
     ...
     */
    final long mainWithOffHeapLNIds = mainNoLNsOrVLSNs + (nBinNodes * binOffHeapLNIds);

    if (mainWithOffHeapLNIds <= mainDataSize) {

      final long mainSpare =
          (mainDataSize > mainNoLNsOrVLSNs) ? (mainDataSize - mainNoLNsOrVLSNs) : 0;

      final long nBINsWithMainLNs = mainSpare / (binWithLNsAndVLSNs - binNoLNsOrVLSNs);
    ...
    offHeapWithLNsAndVLSNs = (nOffHeapBins * binOffHeapWithLNIds) + (nBinNodes * binOffHeapLNs);
  ...
  private void calcTreeSizes(final Environment env) {
    ...
    for (int i = 0; i < nodeAvg; i += 1) {
      ...
      final OperationResult result =
          db.put(
              /*null, */keyEntry, dataEntry, duplicates ? Put.DUP_DATA : Put.NO_OVERWRITE, options);
     ...
     */
    final IN in = DbInternal.getDbImpl(db).getTree().getRootINLatchedExclusive(CacheMode.DEFAULT);
    assert bin == in.getTarget(0);

    for (int i = 1; i < nodeAvg; i += 1) {

      final int result =
          in.insertEntry1(bin, bin.getKey(i), null, bin.getLsn(i), false /*blindInsertion*/);
    ...
    if (offHeapCache) {
      ...
      assert bytesFreed == binOffHeapLNs + (binOffHeapWithLNIds - binOffHeapNoLNIds);
  ...
  /**
   * Prints Java properties for information collected by calculateCacheSizes. Min/max sizes are
   * output for compatibility with earlier versions; in the past, min and max were different values.
   */
  ...
  void printCacheSizes(final PrintStream out) {
    ...
    out.println(line(mainNoLNsOrVLSNs, "Internal nodes only" + mainSuffix));

    if (offHeapCache) {
      out.println(line(offHeapNoLNsOrVLSNs, "Internal nodes only" + offHeapSuffix));
    }

    if (dataSize >= 0) {
      if (!offHeapCache && mainNoLNsWithVLSNs != mainNoLNsOrVLSNs) {
        out.println(line(mainNoLNsWithVLSNs, "Internal nodes and record versions" + mainSuffix));
      }

      out.println(line(mainWithLNsAndVLSNs, "Internal nodes and leaf nodes" + mainSuffix));

      if (offHeapCache) {
        out.println(line(offHeapWithLNsAndVLSNs, "Internal nodes and leaf nodes" + offHeapSuffix));
      }
    ...
    if (offHeapCache && mainMinDataSize > mainDataSize) {
      out.println(
          "\nWARNING: The information above applies to a data set of "
              ...
              + INT_FORMAT.format(mainMinDataSize + envOverhead)
              + " bytes.");
    }
    ...
    if (btreeInfo) {
      ...
      if (offHeapCache) {
        ...
        out.println();
        out.println(line(nMainBINsNoLNsOrVLSNs, "Internal nodes only, BINs" + mainSuffix));
        out.println(
            line(nBinNodes - nMainBINsNoLNsOrVLSNs, "Internal nodes only, BINs" + offHeapSuffix));
        out.println(
            line(nMainBINsWithLNsAndVLSNs, "Internal nodes and leaf nodes, BINs" + mainSuffix));
        out.println(
            line(
                nBinNodes - nMainBINsWithLNsAndVLSNs,
                "Internal nodes and leaf nodes, BINs" + offHeapSuffix));
        out.println(
            line(nMainLNsWithLNsAndVLSNs, "Internal nodes and leaf nodes, LNs" + mainSuffix));
        out.println(
            line(
  ...
  /**
   * For testing, insert the specified data set and initialize measuredMainNoLNsWithVLSNs and
   * measuredMainWithLNsAndVLSNs.
   */
  void measure(final PrintStream out) {

    Environment env = openMeasureEnvironment(true /*createNew*/, false /*setMainSize*/);
    try {
      IN.ACCUMULATED_LIMIT = 0;
      ...
      if (offHeapCache) {
        ...
        env.close();
        env = null;
        env = openMeasureEnvironment(false /*createNew*/, false /*setMainSize*/);
        db = openDatabase(env, false);

        readRecords(out, env, db, false /*readData*/);
        evictMainToDataSize(db, mainDataSize);

        measuredMainNoLNsOrVLSNs = getStats(out, env, "After read keys only, evict main to size");

        measuredOffHeapNoLNsOrVLSNs = getOffHeapCacheSize(DbInternal.getNonNullEnvImpl(env));

        readRecords(out, env, db, true /*readData*/);
        evictMainToDataSize(db, mainDataSize);

        measuredMainWithLNsAndVLSNs = getStats(out, env, "After read all, evict main to size");

        measuredOffHeapWithLNsAndVLSNs = getOffHeapCacheSize(DbInternal.getNonNullEnvImpl(env));

      } else {
        measuredMainWithLNsAndVLSNs = getStats(out, env, "After insert");

        trimLNs(db);

        measuredMainNoLNsWithVLSNs = getStats(out, env, "After trimLNs");

        trimVLSNs(db);

        measuredMainNoLNsOrVLSNs = getStats(out, env, "After trimVLSNs");
      }
      ...
      env = openMeasureEnvironment(false /*createNew*/, offHeapCache /*setMainSize*/);
      db = openDatabase(env, false);

      PreloadStatus status = preloadRecords(out, db, false /*loadLNs*/);

      preloadMainNoLNsOrVLSNs =
          getStats(out, env, "Internal nodes only after preload (" + status + ")");
      ...
      if (assumeEvictLN) {
        ...
        preloadMainWithLNsAndVLSNs = getStats(out, env, "All nodes after preload (" + status + ")");
      }

      if (!offHeapCache) {
        trimLNs(db);

        preloadMainNoLNsWithVLSNs =
            getStats(out, env, "Internal nodes plus VLSNs after preload (" + status + ")");
      }
  ...
  private Environment openMeasureEnvironment(final boolean createNew, final boolean setMainSize) {
    ...
    if (setMainSize) {
       ...
       */
      config.setConfigParam(EnvironmentConfig.LOG_TOTAL_BUFFER_BYTES, String.valueOf(3 << 20));
    } else {
  ...
  private Environment openCalcEnvironment(final boolean createNew) {
    ...
    config.setConfigParam(EnvironmentConfig.FREE_DISK, String.valueOf(1L << 20));

    return openEnvironment(config, createNew);
  }

  private Environment openEnvironment(final EnvironmentConfig config, final boolean createNew) {
    mkTempDir();
    ...
    config.setConfigParam(EnvironmentConfig.ENV_RUN_OFFHEAP_EVICTOR, "false");
    config.setConfigParam(EnvironmentConfig.ENV_RUN_VERIFIER, "false");

    /* Evict in small chunks. */
    config.setConfigParam(EnvironmentConfig.EVICTOR_EVICT_BYTES, "1024");

    final Environment newEnv;

    if (replicated) {
      try {
        final Class repEnvClass = Class.forName("com.sleepycat.je.rep.utilint.DbCacheSizeRepEnv");
        final DbCacheSizeRepEnv repEnv = (DbCacheSizeRepEnv) repEnvClass.newInstance();
     ...
     */
    final long fileSize =
        Integer.parseInt(newEnv.getConfig().getConfigParam(EnvironmentConfig.LOG_FILE_MAX));
  ...
  private Database openDatabase(final Environment env, final boolean createNew) {
  ...
  /** Inserts records and ensures that no eviction occurs. LNs (and VLSNs) are left intact. */
  private void insertRecords(final PrintStream out, final Environment env, final Database db) {
    final DatabaseEntry keyEntry = new DatabaseEntry();
    final DatabaseEntry dataEntry = new DatabaseEntry();
    ...
    if (keyPrefix == 0) {
       ...
       */
      final int nodeAvg =
          orderedInsertion ? nodeMaxEntries : ((nodeMaxEntries * DEFAULT_DENSITY) / 100);
      final int prevKey = lastKey - (nodeAvg * 2);
      final byte[] prevKeyBytes = padLeft(BigInteger.valueOf(prevKey).toByteArray(), maxKeyBytes);
      int calcPrefix = 0;
    ...
    try {
      for (int i = 0; i <= lastKey; i += 1) {
        ...
        final byte[] keyBytes = padLeft(BigInteger.valueOf(keyVal).toByteArray(), maxKeyBytes);
        setKeyData(keyBytes, keyOffset, keyEntry, dataEntry);

        final OperationResult result =
            cursor.put(
                keyEntry, dataEntry, duplicates ? Put.DUP_DATA : Put.NO_OVERWRITE, options);
  ...
  private void readRecords(
      final PrintStream out, final Environment env, final Database db, final boolean readData) {
  ...
  private void checkForEviction(Environment env, int recNum) {
    ...
    if (stats.getOffHeapNodesTargeted() > 0) {
      ...
      throw new IllegalStateException(
          "*** Ran out of off-heap cache at record "
              + recNum
    ...
    if (stats.getNNodesTargeted() > 0) {
      ...
      throw new IllegalStateException(
          "*** Ran out of main cache at record "
              + recNum
  ...
  private boolean iterateBINs(final Database db, final BINVisitor visitor) {
    ...
    while (keepGoing
        && c.getNext(key, data, LockMode.READ_UNCOMMITTED) == OperationStatus.SUCCESS) {
  ...
  /** Pads the given array with zeros on the left, and returns an array of the given size. */
  ...
  private PreloadStatus preloadRecords(
      final PrintStream out, final Database db, final boolean loadLNs) {
    Thread thread = null;
    if (out != null) {
      thread =
          new Thread() {
  ...
  private long getStats(final PrintStream out, final Environment env, final String msg) {
    if (out != null) {
      out.println();
      out.println(msg + ':');
    }
    ...
    if (out != null) {
      out.println(
          "MainCache= "
              ...
              + " Data= "
              + INT_FORMAT.format(dataSize)
              + " BINs= "
              + INT_FORMAT.format(stats.getNCachedBINs())
              + " UINs= "
              ...
              + " OhLNs= "
              + INT_FORMAT.format(stats.getOffHeapCachedLNs())
              + " OhBIN= "
              + INT_FORMAT.format(stats.getOffHeapCachedBINs())
              + " OhBINDeltas= "
