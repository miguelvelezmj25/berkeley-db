...
/**
 * Verifies the checksums in a {@link ReadableByteChannel} for a log file in a JE {@link
 * Environment}. This class is similar to the {@link LogVerificationInputStream} class, but permits
 * using NIO channels and direct buffers to provide better copying performance.
 *
 * <p>This {@code ReadableByteChannel} reads input from some other given {@code
 * ReadableByteChannel}, and verifies checksums while reading. Its primary intended use is to verify
 * log files that are being copied as part of a programmatic backup. It is critical that invalid
 * files are not added to a backup set, since then both the live environment and the backup will be
 * invalid.
 *
...
public class LogVerificationReadableByteChannel implements ReadableByteChannel {
   ...
   * @param channel the underlying {@code ReadableByteChannel} for the log to be read
  ...
  public LogVerificationReadableByteChannel(
      final Environment env, final ReadableByteChannel channel, final String fileName) {
   ...
   */
  public LogVerificationReadableByteChannel(
      final EnvironmentImpl envImpl, final ReadableByteChannel channel, final String fileName) {
   ...
   * {@inheritDoc}
   *
   * <p>This method reads the underlying {@code ReadableByteChannel} and verifies the contents of
   * the stream.
   *
