...
/**
 * Transform one or more je.stat.csv statistics files and write the output to stdout. A set of
 * column names is used to specify the order and which columns are written to the output. The
 * utility is used to create an output file that is easier to analyze by projecting and ordering
 * only the data that is required. Each user specified column name will either be a exact match of a
 * column in the file or a prefix match. In order to output the "time" and all "Op" group statistics
 * a column list "time,Op" could be used. Multiple input files are processed in the order specified
 * on the command line. Duplicate column headers are suppressed in the output when processing
...
public class DbFilterStats {
  ...
  private static final String USAGE =
      "usage: "
          + CmdUtil.getJavaCommand(DbFilterStats.class)
          + "\n"
   ...
   *
   * @param argv An array of command line arguments to the DbFilterStats utility.
   *     <pre>
   ...
   *  &lt;stat file&gt; [&lt;stat file&gt;]
   * </pre>
   *     <p>At least one argument must be specified.
   */
  ...
  /**
   * processFile will form the list of columns based on the projection list. The column data to
   * stdout. If the header has not been output or is different that what was previously written, the
   * column header for the projected columns is written to stdout.
   *
  ...
  private void validateParams() {
    ...
    if (projectionFile != null) {
      if (!projectionFile.exists()) {
        printFatal(
            "Specified projection file " + projectionFile.getAbsolutePath() + " does not exist.");
      }
      if (projectionFile.isDirectory()) {
        printFatal(
            "Specified projection file " + projectionFile.getAbsolutePath() + " is not a file.");
      }
