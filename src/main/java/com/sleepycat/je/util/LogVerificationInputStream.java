...
/**
 * Verifies the checksums in an {@code InputStream} for a log file in a JE {@code Environment}.
 *
 * <p>This {@code InputStream} reads input from some other given {@code InputStream}, and verifies
 * checksums while reading. Its primary intended use is to verify log files that are being copied as
 * part of a programmatic backup. It is critical that invalid files are not added to a backup set,
 * since then both the live environment and the backup will be invalid.
 *
 * <p>The following example verifies log files as they are being copied. The {@link DbBackup} class
 * should normally be used to obtain the array of files to be copied.
 ...
 * </pre>
 *
 * <p>It is important to read the entire underlying input stream until the end-of-file is reached to
 * detect incomplete entries at the end of the log file.
 *
 * <p>Note that {@code mark} and {@code reset} are not supported and {@code markSupported} returns
 * false. The default {@link InputStream} implementation of these methods is used.
 *
 * @see DbBackup
 * @see DbVerifyLog
 */
public class LogVerificationInputStream extends InputStream {
   ...
   * @param fileName the file name of the input stream, for reporting in the {@code
   ...
   *     occurs.
   */
  public LogVerificationInputStream(
      final Environment env, final InputStream in, final String fileName) {
    this(DbInternal.getNonNullEnvImpl(env), in, fileName, -1L);
  }

  /** Internal constructor. If fileNum is less than zero, it is derived from fileName. */
   ...
   * {@inheritDoc}
   *
   * <p>This method reads the underlying {@code InputStream} and verifies the contents of the
   * stream.
   *
   ...
   * {@inheritDoc}
   *
   * <p>This method reads the underlying {@code InputStream} in order to skip the required number of
   * bytes and verifies the contents of the stream. A temporary buffer is allocated lazily for
   * reading.
   *
   * @throws LogVerificationException if a checksum cannot be verified or a log entry is determined
   *     to be invalid by examining its contents.
  ...
  public long skip(final long bytesToSkip) throws IOException {
    ...
    while (remaining > 0) {
      final int lenRead = read(skipBuf, 0, (int) Math.min(SKIP_BUF_SIZE, remaining));
      if (lenRead < 0) {
        break;
      }
      remaining -= lenRead;
