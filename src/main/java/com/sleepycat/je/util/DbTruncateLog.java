...
/**
 * DbTruncateLog is a utility that lets the user truncate JE log starting at a specified file and
 * offset to the last log file, inclusive. Generally used in replication systems for handling
 * com.sleepycat.je.rep.RollbackProhibitedException, to permit the application to interject
 * application specific handling. Should be used with caution.
 *
 * <p>The parameters for DbTruncateLog are provided through the RollbackProhibitedException
 * instance, and the exception message. The goal is to truncate the JE log after a specified file
 * number and file offset. DbTruncateLog will automatically delete all log entries after that
 * specified log entry.
 *
 * <p>For example, suppose the JE log consists of these files:
 *
 * <pre>
 *    00000002.jdb
 *    0000000e.jdb
 ...
 * </pre>
 *
 * And the log must be truncated at file 0x1d, offset 0x34567, users should use the following
 * command:
 *
 * <ol>
 *   <li>DbTruncateLog -h &lt;envDir&gt; -f 0x1d -o 0x34567
 * </ol>
 */
public class DbTruncateLog {
  ...
  public void truncateLog(File env, long truncFileNum, long truncOffset) throws IOException {
    ...
    EnvironmentImpl envImpl = CmdUtil.makeUtilityEnvironment(env, false);
  ...
  private static void usage() {
    ...
    System.out.println(
        "JE log will be deleted starting from the "
