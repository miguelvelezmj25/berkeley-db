 ...
 * comparator classes.
 *
 * <p>To verify a database and write the errors to stream:
 *
 * <pre>
...
public class DbVerify {
  ...
  private static final String usageString =
      "usage: "
          + CmdUtil.getJavaCommand(DbVerify.class)
          + "\n"
  ...
  private final VerifyConfig verifyConfig = new VerifyConfig();
   ...
   * </pre>
   *     <p>-V - show the version of the JE library.<br>
   ...
   *     <p>Note that the DbVerify command line cannot be used to verify the integrity of secondary
   *     databases, because this feature requires the secondary databases to have been opened by the
   *     application. To verify secondary database integrity, use {@link Environment#verify} or
   *     {@link com.sleepycat.je.Database#verify} instead, from within the application.
   ...
   * @param quiet true if the verification should not produce errors to the output stream
   * @deprecated as of 7.5, use {@link Environment#verify} or {@link
   *     com.sleepycat.je.Database#verify} instead. These methods allow specifying all {@link
   ...
   * @return true if the verification found no errors. Currently true is always returned when this
   *     method returns normally, i.e., when no exception is thrown.
