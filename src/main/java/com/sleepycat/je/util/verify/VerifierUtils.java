...
public class VerifierUtils {
  ...
  /**
   * Create the restore marker file from Exception origException and return an EFE that can be
   * thrown by the caller. The EFE will invalidate the environment.
   *
   * @param failureType the failure type that should be recorded in the RestoreRequired log entry.
   * @param origException the exception contains the properties that are stored to the marker file.
   */
  public static EnvironmentFailureException createMarkerFileFromException(
    ...
    if (!envImpl.isReadOnly()) {
      ...
      final RestoreMarker restoreMarker =
          new RestoreMarker(envImpl.getFileManager(), envImpl.getLogManager());
    ...
    return new EnvironmentFailureException(
        envImpl,
        reason,
        "Persistent corruption detected: " + origException.toString() + markerFileError,
        origException);
  }
