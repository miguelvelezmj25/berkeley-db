...
public class BtreeVerifier {
  ...
  public void verifyAll() throws DatabaseException {
    ...
    try {
      ...
      verifyOneDb(DbType.ID.getInternalName(), DbTree.ID_DB_ID, out, true /*verifyAll*/);

      verifyOneDb(DbType.NAME.getInternalName(), DbTree.NAME_DB_ID, out, true /*verifyAll*/);
      ...
      class Traversal implements CursorImpl.WithCursor {
        ...
        public boolean withCursor(
          ...
          final NameLN nameLN = (NameLN) cursor.lockAndGetCurrentLN(LOCKTYPE_NOLOCK);

          if (nameLN != null && !nameLN.isDeleted()) {

            final DatabaseId dbId = nameLN.getId();

            final String dbName = StringUtils.fromUTF8(key.getData());
      ...
      CursorImpl.traverseDbWithCursor(
          dbTree.getNameDatabaseImpl(), LOCKTYPE_NOLOCK, true /*allowEviction*/, traversal);
   ...
   *
   * @param verifyAll if true, we won't log INFO messages for every database to avoid cluttering the
   *     trace log.
   */
  private BtreeStats verifyOneDb(
      String dbName, DatabaseId dbId, PrintStream out, boolean verifyAll) {
    ...
    try {
      ...
      final long batchDelay = btreeVerifyConfig.getBatchDelay(TimeUnit.MILLISECONDS);

      /*
       * The accumulated information for this database.
       */
      final VerifierStatsAccumulator statsAcc =
          new VerifierStatsAccumulator(out, btreeVerifyConfig.getShowProgressInterval());
      ...
      while (true) {
        ...
        try {
          ...
          WalkDatabaseTreeResult result =
              walkDatabaseTree(
                  dbImpl, isSecondaryDb, priDb, secDb, statsAcc, lastKey, lastData, batchSize);
  ...
  private void verifyOrderedKeys(IN in) {
    ...
    final String msg1 = label + in.toSafeString(corruptIndex - 1, corruptIndex);

    IN inFromFile = getINFromFile(targetLsns, dbImpl, msg1);

    try {
      final int newCorruptIndex = verifyOrderedKeysInternal(inFromFile, dbImpl);

      if (newCorruptIndex == 0) {
        throw EnvironmentFailureException.unexpectedState(envImpl, transientMsg(msg1));
      } else {
        final String msg2 = label + inFromFile.toSafeString(newCorruptIndex - 1, newCorruptIndex);
  ...
  private void verifyDanglingLSN(int index, IN in, boolean isBin) {
    ...
    DanglingLSNCheckResult result = verifyDanglingLSNInternal(index, in, isBin, dbImpl);
    ...
    final String msg1 = label + result.getReason() + in.toSafeString(result.problematicIndex);
    ...
    try {
      ...
      for (int i = 0; i < inFromFile.getNEntries(); i++) {
        result = verifyDanglingLSNInternal(i, inFromFile, isBin, dbImpl);
        if (result.problematicIndex >= 0) {
          findAgain = true;
          break;
        }
      ...
      if (!findAgain) {
        ...
        final String msg2 =
            label + result.getReason() + inFromFile.toSafeString(result.problematicIndex);
  ...
  private DanglingLSNCheckResult verifyDanglingLSNInternal(
      int index, IN in, boolean isBin, DatabaseImpl databaseImpl) {
  ...
  private static class FileSizeInfo {
    ...
    String getReason() {
      return (sizeFromLastFile
              ? "File size from last file"
              : (sizeFromCache ? "File size previously cached" : "File size added to cache"))
          + ". ";
    }
  }

  /** @return if the FileSizeInfo.size is gte 0, then it means that the file does exist. */
  ...
  private void verifyObsoleteRecords(int index, IN in, boolean isBin) {
    ...
    if (Arrays.binarySearch(offsets, DbLsn.getFileOffset(curLsn)) >= 0) {
      throw new EnvironmentFailureException(
          ...
          "Active lsn is obsolete: " + DbLsn.getNoFormatString(curLsn) + in.toSafeString(index));
    }
  }
  ...
  private IN getINFromFile(
      final Pair<Long, Long> targetLsns, final DatabaseImpl dbImpl, final String msg) {
    ...
    try {
      ...
      if (targetLsns.second() != DbLsn.NULL_LSN) {
        optionalFullBinEntry = logManager.getLogEntryDirectFromFile(targetLsns.second());
      }
      if (entry == null && optionalFullBinEntry == null) {
        throw EnvironmentFailureException.unexpectedState(envImpl, transientMsg(msg));
      }
  ...
  private boolean findFirstRecord(
      DatabaseImpl dbImpl, Cursor cursor, DatabaseEntry lastKey, DatabaseEntry lastData) {
    ...
    if (isDuplicated) {
      result = cursor.get(usedKey, usedData, Get.SEARCH_BOTH_GTE, NOLOCK_UNCHANGED);
      if (result != null) {
        ...
        result = cursor.get(usedKey, usedData, Get.SEARCH_GTE, NOLOCK_UNCHANGED);
        if (result == null) {
          /* No more records. */
          return false;
        }
  ...
  private WalkDatabaseTreeResult walkDatabaseTree(
    ...
    try {
      ...
      boolean verifySecondary = isSecondaryDb && btreeVerifyConfig.getVerifySecondaries();
      DatabaseEntry foundKey = new DatabaseEntry();
      DatabaseEntry foundData = new DatabaseEntry();

      if (!(verifySecondary || (priDb != null && btreeVerifyConfig.getVerifyDataRecords()))) {
      ...
      while (++recordCount <= batchSize) {
        ...
        try {
          ...
          if (!isSecondaryDb || !verifySecondary) {
            ...
            OperationResult result =
                cursor.get(foundKey, foundData, Get.CURRENT, READLOCK_UNCHANGED);
            if (result == null) {
          ...
          try {
            verifyDanglingLSNAndObsoleteRecordsOneSlot(cursorImpl.getIndex(), bin, true);
          } finally {
          ...
          if (verifySecondary) {
            ...
            verifyIndex(dbImpl, secDb, cursor, foundKey, foundData);

            /* For secondary database, check foreign constraint. */
            verifyForeignConstraint(secDb, cursor, foundKey, dbCache);
          }
          ...
          if (cursor.get(foundKey, foundData, Get.NEXT, NOLOCK_UNCHANGED) == null) {
            return WalkDatabaseTreeResult.NO_MORE_RECORDS;
          }
          ...
          try {
            ...
            LoggerUtils.logMsg(
                ...
                "Exception aborted btree verification of db "
                    + dbImpl.getDebugName()
                    + ", verification of all dbs will stop. "
                    + e);
        ...
        } catch (EnvironmentFailureException | BtreeVerificationException e) {
          throw e;
  ...
  private void verifyIndex(
    ...
    try {
      final SecondaryAssociation secAssoc = DbInternal.getSecondaryAssociation(secDb);
      if (secAssoc.isEmpty()) {
        return;
      }
      ...
      LoggerUtils.logMsg(
          ...
          envImpl,
          Level.WARNING,
          "Secondary corruption is detected during btree " + "verification. " + sie);
  ...
  private void verifyForeignConstraint(
    ...
    final Database foreignDb = DbInternal.getPrivateSecondaryConfig(secDb).getForeignKeyDatabase();
    if (foreignDb == null) {
      return;
    }
    ...
    final DatabaseImpl foreignDbImpl = dbTree.getDb(foreignDbId, -1, dbCache);
    ...
    try (final Cursor foreignCursor =
        DbInternal.makeCursor(foreignDbImpl, locker, null, true /*retainNonTxnLocks*/)) {

      final OperationResult result;
      try {
        result = foreignCursor.get(secKey, tmpData, Get.SEARCH, READLOCK_UNCHANGED);
  ...
  private void verifyPrimaryData(
    ...
    try {
      final SecondaryAssociation secAssoc = DbInternal.getSecondaryAssociation(priDb);
      if (secAssoc.isEmpty()) {
        return;
      }
  ...
  private void checkSecondaryKeysExist(
    ...
    final SecondaryConfig secondaryConfig = DbInternal.getPrivateSecondaryConfig(secDb);
    ...
    if (keyCreator != null) {
      ...
      checkOneSecondaryKeyExists(
          secDb, secDbImpl, priKey, secKey, expirationTime, errMsg, priDb, secAssoc);
  ...
  private boolean checkOneSecondaryKeyExists(
    ...
    try (final Cursor checkCursor =
        DbInternal.makeCursor(secDbImpl, locker, null, false /*retainNonTxnLocks*/)) {

      if (checkCursor.get(secKey, priKey, Get.SEARCH_BOTH, NOLOCK_UNCHANGED) == null) {
         ...
         */
        setSecondaryDbCorrupt(secDb, locker, errMsg, secKey, priKey, expirationTime);
    ...
    if (!DbInternal.isCorrupted(secDb)) {
      ...
      final SecondaryIntegrityException sie =
          new SecondaryIntegrityException(
              ...
              locker,
              errMsg,
              DbInternal.getDbDebugName(secDb),
              secKey,
              priKey,
              expirationTime);
  ...
  /**
   * Thrown during btree verification if a persistent btree corruption is detected.
   *
   * <p>This is an internal exception and ideally it should be a checked exception(not a
   * RuntimeException) so that we can confirm statically that it is always handled. But this would
   * require changes to CursorImpl.WithCursor interface, so for now a runtime exception is used.
   */
  private static class BtreeVerificationException extends RuntimeException {
    ...
    public BtreeVerificationException(final String message, final Throwable cause) {
