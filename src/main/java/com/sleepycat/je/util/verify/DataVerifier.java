...
import static com.sleepycat.je.config.EnvironmentParams.*;

/**
 * Periodically perform checksum verification, Btree verification, or both, depending on {@link
 * com.sleepycat.je.EnvironmentConfig#VERIFY_LOG} and {@link
 * com.sleepycat.je.EnvironmentConfig#VERIFY_BTREE}.
 *
 * <p>The first-time start time and the period of the verification is determined by {@link
 * com.sleepycat.je.EnvironmentConfig#VERIFY_SCHEDULE}.
 */
public class DataVerifier {
  ...
  public DataVerifier(EnvironmentImpl envImpl) {
    ...
    this.timer =
        new Timer(
            envImpl.makeDaemonThreadName(Environment.DATA_CORRUPTION_VERIFIER_NAME),
            true /*isDaemon*/);
    ...
    synchronized (this) {
      if (!shutdownRequest) {
        ...
        if (cronSchedule != null) {
           ...
           * a fixed time.
           */
          timer.scheduleAtFixedRate(verifyTask, verifyDelay, verifyInterval);
        }
      }
    }
  }
  ...
  public void shutdown() {
    ...
    final PollCondition cond =
        new PollCondition(2, timeoutMs) {
          @Override
  ...
  private boolean updateConfig(DbConfigManager configMgr) {
    ...
    if (!configMgr.getBoolean(ENV_RUN_VERIFIER)) {
       ...
       */
      String sysPropVerifySchedule = System.getProperty(VERIFIER_SCHEDULE);
      if (sysPropVerifySchedule != null && !configMgr.isSpecified(VERIFY_SCHEDULE)) {
  ...
  class VerifyTask extends TimerTask {
    ...
    private void updateConfig() {
      ...
      dbLogVerifier.setReadDelay(
          configMgr.getDuration(VERIFY_LOG_READ_DELAY), TimeUnit.MILLISECONDS);

      final VerifyConfig btreeVerifyConfig = new VerifyConfig();
      btreeVerifyConfig.setVerifySecondaries(configMgr.getBoolean(VERIFY_SECONDARIES));
      btreeVerifyConfig.setVerifyDataRecords(configMgr.getBoolean(VERIFY_DATA_RECORDS));
      btreeVerifyConfig.setVerifyObsoleteRecords(configMgr.getBoolean(VERIFY_OBSOLETE_RECORDS));
      btreeVerifyConfig.setBatchSize(configMgr.getInt(VERIFY_BTREE_BATCH_SIZE));
      btreeVerifyConfig.setBatchDelay(
          configMgr.getDuration(VERIFY_BTREE_BATCH_DELAY), TimeUnit.MILLISECONDS);
      ...
      try {
        ...
        if (envImpl.isValid()) {
          StoppableThread.handleUncaughtException(
              envImpl.getLogger(), envImpl, Thread.currentThread(), e);
        }
      } finally {
        if (!success) {
          requestShutdown();
