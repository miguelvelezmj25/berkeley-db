...
/**
 * Dump the contents of a database. This utility may be used programmatically or from the command
 * line.
 *
 * <p>When using this utility as a command line program, and the application uses custom key
 * comparators, be sure to add the jars or classes to the classpath that contain the application's
 * comparator classes.
 *
 * <pre>
 ...
 *
 * <p>To dump a database to a stream from code:
 *
 * <pre>
 ...
 * </pre>
 *
 * <p>Because a <code>DATA=END</code> marker is used to terminate the dump of each database,
 * multiple databases can be dumped and loaded using a single stream. The {@link DbDump#dump} method
 * leaves the stream positioned after the last line written and the {@link DbLoad#load} method
 * leaves the stream positioned after the last line read.
 */
public class DbDump {
  ...
  private static final String usageString =
      "usage: "
          + CmdUtil.getJavaCommand(DbDump.class)
          + "\n"
          + "  -h <dir> # environment home directory\n"
          + "  [-f <fileName>]     # output file, for non -rR dumps\n"
          + "  [-l]                # list databases in the environment\n"
   ...
   * @param formatUsingPrintable true if the dump should use printable characters.
   */
  public DbDump(
      Environment env, String dbName, PrintStream outputFile, boolean formatUsingPrintable) {
    init(env, dbName, outputFile, formatUsingPrintable);
  }
   ...
   * </pre>
   *     <dl>
   *       <dt>-f - the file to dump to. If omitted, output is to System.out. Does not apply when -r
   *           or -R is used.
   *       <dt>-l - list the databases in the environment.
   *       <dt>-p - output printable characters.
   *       <dd>If characters in either the key or data items are printing characters (as defined by
   ...
   *       <dt>-V - display the version of the JE library.
   *       <dt>-s database - the database to dump. Does not apply when -r or -R is used.
   *       <dt>-h dbEnvHome - the directory containing the database environment.
   *       <dt>-d directory - the output directory for *.dump files. Applies only when -r or -R is
   *           used.
   *       <dt>-v - print progress information to stdout for -r or -R mode.
   *       <dt>-r - Salvage data from possibly corrupt data files.
   *       <dd>The records for all databases are output. The records for each database are saved
   *           into &lt;databaseName&gt;.dump files in the current directory.
   *           <p>This option recreates the Btree structure in memory, so as large a heap size as
   *           possible should be specified. If -r cannot be used due to insufficient memory, use -R
   *           instead.
   *           <p>When used on uncorrupted data files, this option should return equivalent data to
   *           a normal dump, but most likely in a different order; in other words, it should output
   *           a transactionally correct data set. However, there is one exception where not all
   *           committed records will be output:
   *           <ul>
   *             <li>When a committed transaction spans more than one .jdb file, and the last file
   *                 in this set of files has been deleted by the log cleaner but earlier files have
   *                 not, records for that transaction that appear in the earlier files will not be
   *                 output. This is because the Commit entry in the last file is missing, and
   *                 DbDump believes that the transaction was not committed. Such missing output
   *                 should be relatively rare. Note that records in deleted files will be output,
   *                 because they were migrated forward by the log cleaner and are no longer
   *                 associated with a transaction.
   *           </ul>
   *       <dt>-R - Aggressively salvage data from a possibly corrupt file.
   *       <dd>
   *           <p>The records for all databases are output. The records for each database are saved
   *           into &lt;databaseName&gt;.dump files in the current directory.
   *           <p>Unlike -r, the -R option does not recreate the Btree structure in memory. However,
   *           it does use a bit set to track all committed transactions, so as large a heap size as
   *           possible should be specified.
   *           <p>-R also differs from -r in that -R does not return a transactionally correct data
   *           set. This is because the Btree information is not reconstructed in memory. Therefore,
   *           data dumped in this fashion will almost certainly have to be edited by hand or other
   *           means before or after the data is reloaded. Be aware of the following abnormalities.
   *           <ul>
   *             <li>Deleted records are often output. An application specific technique should
   *                 normally be used to correct for this.
   *             <li>Multiple versions of the same record are sometimes output. When this happens,
   *                 the more recent version of a record is output first. Therefore, the -n option
   *                 should normally be used when running DbLoad.
   *             <li>When a committed transaction spans more than one .jdb file, and the last file
   *                 in this set of files has been deleted by the log cleaner but earlier files have
   *                 not, records for that transaction that appear in the earlier files will not be
   *                 output. This is because the Commit entry in the last file is missing, and
   ...
   *                 should be relatively rare. Note that records in deleted files will be output,
   *                 because they were migrated forward by the log cleaner and are no longer
   *                 associated with a transaction. (This abnormality also occurs with -r.)
   *           </ul>
   *     </dl>
   *
  ...
  protected void openEnv(boolean doRecovery)
    ...
    if (env == null) {
      ...
      envConfiguration.setConfigParam(
          EnvironmentParams.ENV_RECOVERY.getName(), doRecovery ? "true" : "false");
   ...
   */
  public void dump()
      throws EnvironmentNotFoundException, EnvironmentLockedException, DatabaseNotFoundException,
          IOException {

    openEnv(true);

    LoggerUtils.envLogMsg(
        Level.INFO, DbInternal.getNonNullEnvImpl(env), "DbDump.dump of " + dbName + " starting");
    ...
    LoggerUtils.envLogMsg(
        Level.INFO, DbInternal.getNonNullEnvImpl(env), "DbDump.dump of " + dbName + " ending");
  }
