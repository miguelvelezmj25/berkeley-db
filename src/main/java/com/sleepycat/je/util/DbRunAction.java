...
/**
 * @hidden For internal use only. DbRunAction is a debugging aid that can invoke a JE operation or
 *     background activity from the command line.
 *     <p>batchClean calls Environment.cleanLog() in a loop checkpoint calls
 *     Environment.checkpoint() with force=true compress calls Environment.compress evict calls
 *     Environment.preload, then evictMemory removeDb calls Environment.removeDatabase, but doesn't
 *     do any cleaning removeDbAndClean calls removeDatabase, then cleanLog in a loop
 *     activateCleaner wakes up the cleaner, and then the main thread waits until you type "y" to
 *     the console before calling Environment.close(). The control provided by the prompt is
 *     necessary for daemon activities because often threads check and bail out if the environment
 *     is closed. verifyUtilization calls CleanerTestUtils.verifyUtilization() to compare
 *     utilization as calculated by UtilizationProfile to utilization as calculated by
...
public class DbRunAction {
      ...
      switch (doAction) {
        ...
        case VERIFY_UTILIZATION:
          ...
          VerifyUtils.verifyUtilization(
              envImpl2, true, // expectAccurateObsoleteLNCount
  ...
  private static void removeAndClean(Environment env, String name, boolean doCleaning)
      throws Exception {
  ...
  private static void usage() {
    ...
    System.out.println(
        "  -a <batchClean|compress|evict|checkpoint|"
