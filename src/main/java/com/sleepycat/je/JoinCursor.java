 ...
 *
 * <p>A join cursor is returned when calling {@link Database#join Database.join}.
 *
 * <p>To open a join cursor using two secondary cursors:
 *
 * <pre>
 *     Transaction txn = ...
 *     Database primaryDb = ...
 ...
 * </pre>
 *
 * <p>The join algorithm is described here so that its cost can be estimated and compared to other
 * approaches for performing a query. Say that N cursors are provided for the join operation.
 * According to the order they appear in the array the cursors are labeled C(1) through C(n), and
 * the keys at each cursor position are labeled K(1) through K(n).
 *
 * <ol>
 *   <li>Using C(1), the join algorithm iterates sequentially through all records having K(1). This
 *       iteration is equivalent to a {@link Cursor#getNextDup Cursor.getNextDup} operation on the
 *       secondary index. The primary key of a candidate record is determined in this manner. The
 *       primary record itself is not retrieved and the primary database is not accessed.
 *   <li>For each candidate primary key found in step 1, a Btree lookup is performed using C(2)
 *       through C(n), in that order. The Btree lookups are exact searches to determine whether the
 *       candidate record also contains secondary keys K(2) through K(n). The lookups are equivalent
 *       to a {@link Cursor#getSearchBoth Cursor.getSearchBoth} operation on the secondary index.
 *       The primary record itself is not retrieved and the primary database is not accessed.
 *   <li>If any lookup in step 2 fails, the algorithm advances to the next candidate record using
 *       C(1). Lookups are performed in the order of the cursor array, and the algorithm proceeds to
 *       the next C(1) candidate key as soon as a single lookup fails.
 *   <li>If all lookups in step 2 succeed, then the matching key and/or data is returned by the
 *       {@code getNext} method. If the {@link #getNext(DatabaseEntry,DatabaseEntry,LockMode)}
 *       method signature is used, then the primary database is read to obtain the record data, as
 *       if {@link Cursor#getSearchKey Cursor.getSearchKey} were called for the primary database. If
 *       the {@link #getNext(DatabaseEntry,LockMode)} method signature is used, then only the
 *       primary key is returned and the primary database is not accessed.
 *   <li>The algorithm ends when C(1) has no more candidate records with K(1), and the {@code
 *       getNext} method will then return {@link com.sleepycat.je.OperationStatus#NOTFOUND
...
public class JoinCursor implements ForwardCursor, Closeable {
  ...
  private final DatabaseEntry[] cursorScratchEntries;
  private final DatabaseEntry scratchEntry;
  private final DatabaseEntry firstSecKey;
  private final boolean[] cursorFetchedFirst;

  /** Creates a join cursor without parameter checking. */
  JoinCursor(final Database primaryDb, final Cursor[] cursors, final JoinConfig configParam)
      throws DatabaseException {

    priDb = primaryDb;
    config = (configParam != null) ? configParam.clone() : JoinConfig.DEFAULT;
   ...
   *
   * <p>The cursors passed to {@link Database#join Database.join} are not closed by this method, and
  ...
  public OperationResult get(
    ...
    final LockMode lockMode = (options != null) ? options.getLockMode() : null;
  ...
  /**
   * This operation is not allowed on a join cursor. {@link UnsupportedOperationException} will
   * always be thrown by this method.
   */
  @Override
   ...
   *
   * <p>An entry is returned by the join cursor for each primary key/data pair having all secondary
   * key values that were specified using the array of secondary cursors passed to {@link
   ...
   */
  public OperationStatus getNext(final DatabaseEntry key, final LockMode lockMode) {
   ...
   *
   * <p>Since duplicates are always sorted and duplicate-duplicates are not allowed, a natural join
   * can be implemented by simply traversing through the duplicates of the first cursor to find
   * candidate keys, and then looking for each candidate key in the duplicate set of the other
   * cursors, without ever reseting a cursor to the beginning of the duplicate set.
   *
   * <p>This only works when the same duplicate comparison method is used for all cursors. We don't
   * check for that, we just assume the user won't violate that rule.
   *
   * <p>A future optimization would be to add a SearchMode.BOTH_DUPS operation and use it instead of
   * using SearchMode.BOTH. This would be the equivalent of the undocumented DB_GET_BOTHC operation
   * used by DB core's join() implementation.
   */
  private OperationResult retrieveNext(
      final DatabaseEntry keyParam,
      final DatabaseEntry dataParam,
      final LockMode lockMode,
      final CacheMode cacheMode) {
    boolean readUncommitted = secCursors[0].isReadUncommittedMode(lockMode);
    ...
    while (true) {
      ...
      if (!cursorFetchedFirst[0]) {
        ...
        result = secCursor.getCurrentInternal(firstSecKey, candidateKey, lockMode, cacheMode);
        ...
        result =
            secCursor.retrieveNext(
                firstSecKey, candidateKey, lockMode, cacheMode, GetMode.NEXT_DUP);
      }
      if (result == null) {
      ...
      for (int i = 1; i < secCursors.length; i += 1) {
        ...
        if (!cursorFetchedFirst[i]) {
          result = secCursor.getCurrentInternal(secKey, scratchEntry, lockMode, cacheMode);
          if (readUncommitted && result == null) {
            /* Deleted underneath read-uncommitted; skip it. */
            result =
                secCursor.retrieveNext(secKey, scratchEntry, lockMode, cacheMode, GetMode.NEXT_DUP);
            if (result == null) {
        ...
        result =
            secCursor.search(
                scratchEntry, candidateKey, lockMode, cacheMode, SearchMode.BOTH, true);
        if (result == null) {
      ...
      if (dataParam != null) {
        if (!secCursors[0].readPrimaryAfterGet(
            ...
            lockMode,
            readUncommitted,
            false /*lockPrimaryOnly*/,
            false /*verifyPrimary*/,
            secCursors[0].getCursorImpl().getLocker(),
            secCursors[0].getDatabase(),
            null)) {
          /* Deleted underneath read-uncommitted cursor; skip it. */
          continue;
        }
