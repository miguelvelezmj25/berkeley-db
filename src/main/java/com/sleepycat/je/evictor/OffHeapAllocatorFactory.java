...
public class OffHeapAllocatorFactory {
  ...
  private final OffHeapAllocator defaultAllocator;
  ...
  OffHeapAllocatorFactory()
     ...
     */
    final Class<?> cls = Class.forName("com.sleepycat.je.evictor.CHeapAllocator");

    defaultAllocator = (OffHeapAllocator) cls.newInstance();
  }

  /**
   * @return null if the default allocator is not available on this JVM, presumably because the
   *     Unsafe class is not available.
   */
