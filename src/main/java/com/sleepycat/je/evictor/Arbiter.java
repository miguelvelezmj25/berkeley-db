...
class Arbiter {
   ...
   *
   * <p>Note that this method is intentionally not synchronized in order to minimize overhead when
   * checking for critical eviction. This method is called from application threads for every cursor
   * operation.
   */
   ...
   *
   * <p>Note that this method is intentionally not synchronized in order to minimize overhead,
  ...
  /**
   * Return non zero number of bytes if eviction should happen. Caps the number of bytes a single
   * thread will try to evict.
   */
