 ...
 *
 * <p>Uses the sun.misc.Unsafe class to call the native 'malloc' and 'free' functions to allocate
 * memory from the 'C' runtime heap.
 *
 * <p>This class should not be referenced symbolically by any other other class. This is necessary
 * to avoid a linkage error if JE is run on a JVM without the Unsafe class. The {@link
 * OffHeapAllocatorFactory} loads this class by name, using reflection.
 */
class CHeapAllocator implements OffHeapAllocator {
    ...
    if (CHECK_BOUNDS) {
      ...
      if (memOff < 0 || memOff + len > size(memId)) {
        throw new IndexOutOfBoundsException(
            "memOff=" + memOff + " memSize=" + size(memId) + " copyLen=" + len);
      }
      if (bufOff < 0 || bufOff + len > buf.length) {
        throw new IndexOutOfBoundsException(
            "bufOff=" + bufOff + " bufSize=" + buf.length + " copyLen=" + len);
      }
    }

    unsafe.copyMemory(
        null, memId + SIZE_BYTES + memOff, buf, Unsafe.ARRAY_BYTE_BASE_OFFSET + bufOff, len);
  }
  ...
  public void copy(byte[] buf, int bufOff, long memId, int memOff, int len) {
    ...
    unsafe.copyMemory(
        buf, Unsafe.ARRAY_BYTE_BASE_OFFSET + bufOff, null, memId + SIZE_BYTES + memOff, len);
  }
  ...
  public void copy(long fromMemId, int fromMemOff, long toMemId, int toMemOff, int len) {
    ...
    unsafe.copyMemory(
        null, fromMemId + SIZE_BYTES + fromMemOff, null, toMemId + SIZE_BYTES + toMemOff, len);
  }
}
