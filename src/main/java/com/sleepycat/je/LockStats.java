 ...
 *
 * <p>Note that some of the lock statistics may be expensive to obtain because the lock table is
 * unavailable to other operations while the statistics are gathered. These expensive statistics are
 * only provided if {@link com.sleepycat.je.Environment#getLockStats Environment.getLockStats} is
 * called with a StatsConfig parameter that has been configured for "slow" stats.
 *
...
public class LockStats implements Serializable {
  ...
  /**
   * @hidden For internal use only. Create a stat api instance to house lock related information.
   */
   ...
   * com.sleepycat.je.Environment#getLockStats Environment.getLockStats} is called in "slow" mode.
   */
