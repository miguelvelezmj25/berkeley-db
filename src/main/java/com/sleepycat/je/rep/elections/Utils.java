...
public class Utils {
  ...
  /**
   * @hidden Utility to broadcast a request to set of targets.
   * @param targets of the broadcast
   * @param requestMessage to be broadcast
   * @param threadPool used to issue message in parallel
   * @return the CompletionService representing the futures generated by the broadcast
   */
  public static FutureTrackingCompService<MessageExchange> broadcastMessage(
  ...
  /**
   * @hidden A utility wrapper to handle all exceptions from futures in a consistent way. The above
   *     method illustrates its intended usage pattern
   */
  public abstract static class WithFutureExceptionHandler<T extends MessageExchange> {
     ...
     * @param completionTimeout the timeout indicating how long to wait for all the results. If
     *     running the tasks involves I/O, especially network I/O, the timeout should be sufficient
     *     to ensure that it allows for the associated latency. The timeout assumes that all the
     *     tasks are run in parallel, so it represents the max estimated task completion time
     *     associated with the tasks in the set.
    ...
    /**
     * The method represents the result processing code being wrapped upon a success response
    ...
    /**
     * The counterpart to processResponse. It's invoked when there was no response to a message. The
     * exception, if present, details the reason for the failure; some protocols may choose not to
     * require a response, that is, both the response and result.exception could be null. The
     * default method simply logs the event.
     *
     * <p>Note that Timeouts don't come down this path; they result in tasks being cancelled with
     * the timeout being logged.
     *
     ...
     *
     * @return true if the futures should not be processed but should be cancelled instead.
     */
    protected abstract boolean isShutdown();

    /**
     * Processes futures in the order in which they complete, as determined by the completion
     * service, to minimize unnecessary waiting.
     */
    public final void execute() {
      final long limitTimeMs = System.currentTimeMillis() + unit.toMillis(completionTimeout);
      try {
        for (int count = completionService.getFutures().size(); count > 0; count--) {
          final long pollTimeoutMs = limitTimeMs - System.currentTimeMillis();
          if (pollTimeoutMs <= 0) {
            /* Timed out. */
            LoggerUtils.logMsg(
                logger,
                ...
                formatter,
                Level.INFO,
                "Election messages timed out after " + unit.toMillis(completionTimeout) + "ms.");
            return;
          }

          /* Wait for the next task that is ready. */
          final Future<T> f = completionService.poll(pollTimeoutMs, TimeUnit.MILLISECONDS);
          ...
          if (isShutdown()) {
            LoggerUtils.logMsg(
                ...
                formatter,
                Level.INFO,
                "Election messages terminated." + " Environment being shutdown.");
            /* Simply exit. */
            return;
          }
          ...
          final long futureTimeoutMs = limitTimeMs - System.currentTimeMillis();
        ...
        if (envImpl != null) {
          ...
          if ((rn != null) && rn.isShutdown()) {
            ...
            LoggerUtils.logMsg(
                ...
                formatter,
                Level.INFO,
                "Election operation interrupted." + " Environment being shutdown.");
            /* Simply exit. */
            return;
          }
        }
    ...
    /**
     * The method represents the future process code being wrapped. It processes generic
     * future-specific exceptions, where the impact of the fault can be localized to the future.
     *
      ...
      try {
        ...
        if ((cause instanceof ConnectException)
          ...
          LoggerUtils.logMsg(
              ...
              formatter,
              Level.FINE,
              "Election connection failure " + cause.getMessage());
          return;
        }
  ...
  /**
   * @hidden A subclass of ExecutorCompletionService that tracks submitted tasks, so that futures
   *     associated with tasks can be cancelled in a modular way, without the need for maintaining
   *     distinct state about the futures that were created.
   */
