 ...
 *
 * <p>Each proposal number is built as the catenation of the following components:
 *
 * <p>ms time (8 bytes) | machineId (16 bytes) | locally unique Id (4 bytes)
 *
 * <p>The ms time supplies the increasing number and the IP address is a number unique across
 * machines.
 *
 * <p>The machineId is generated as described below.
 *
 * <p>The locally unique Id is used to allow for multiple unique proposal generators in a single
 * process.
 */
public class TimebasedProposalGenerator {
  ...
  /**
   * Creates an instance with an application-specified locally (machine wide) unique id, e.g. a port
   * number, or a combination of a pid and some other number.
   *
  ...
  public Proposal nextProposal() {
    ...
    return new StringProposal(
        String.format("%016x%s%08x", proposalTime, machineId, locallyUniqueId));
  }
  ...
  /**
   * Implements the Proposal interface for a string based proposal. The string is a hex
  ...
  private static class StringProposal implements Proposal {
    ...
    private static final ProposalParser theParser =
        new ProposalParser() {
          @Override
