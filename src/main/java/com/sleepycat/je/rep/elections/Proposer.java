...
import static com.sleepycat.je.rep.elections.ProposerStatDefinition.*;

/**
 * Plays the role of a Proposer in the consensus algorithm.
 *
 * <p>Note that the Proposer also plays the role of a distinguished learner and informs all other
 * listeners about the acceptance of a proposal.
 */
public abstract class Proposer {
  ...
  /**
   * Initializes a proposer with the set of acceptors identified by the sockets on which they accept
   * proposals.
   */
  public Proposer(Elections elections, NameIdPair nameIdPair) {
    ...
    statistics =
        new StatGroup(ProposerStatDefinition.GROUP_NAME, ProposerStatDefinition.GROUP_DESC);
  ...
  /**
   * Predicate to determine whether we have a quorum based upon the quorum policy. Note that the
   * case of two attendees is special-cased when a SIMPLE_MAJORITY is called for, to return success
   * with just one yes vote.
   *
  ...
  private boolean haveQuorum(QuorumPolicy quorumPolicy, int votes) {
    return elections.getElectionQuorum().haveQuorum(quorumPolicy, votes);
  }

  /**
   * Implements phase 1. It sends our Propose requests to all acceptors and tallies up the results.
   *
   * @param proposal the new unique proposal.
   * @return the results from running Phase1, or null if phase 1 did not get a promise from the
  ...
  private Phase1Result phase1(QuorumPolicy quorumPolicy, Proposal proposal)
    ...
    LoggerUtils.logMsg(
        logger, elections.getRepImpl(), formatter, Level.FINE, "Phase 1 proposal: " + proposal);

    /* Broadcast Propose requests. */
    final Set<InetSocketAddress> acceptorSockets = elections.getAcceptorSockets();
  ...
  /**
   * Tally the results from Phase 1. Terminate the tally if we see any rejection messages which
   * indicate our proposal has been obsoleted by a more recent one. It also tracks the highest
   * numbered proposal and the associated value that was accepted by the Acceptors.
   *
   ...
   * @throws HigherNumberedProposal
   */
  private Phase1Result tallyPhase1Results(
      Proposal currentProposal, final FutureTrackingCompService<MessageExchange> compService)
      throws HigherNumberedProposal {
    ...
    new Utils.WithFutureExceptionHandler<MessageExchange>(
        compService,
        2 * elections.getProtocol().getReadTimeout(),
      ...
      protected void processResponse(MessageExchange me) {
        ...
        if (op == elections.getProtocol().REJECT) {
          ...
          if ((result.acceptProposal == null)
              || ((acceptProposal != null)
                  && (acceptProposal.compareTo(result.acceptProposal) > 0))) {
          ...
          ProtocolError protocolError = (ProtocolError) me.getResponseMessage();
          ...
          LoggerUtils.logMsg(
              ...
              formatter,
              Level.WARNING,
              "Proposer phase 1 protocol error: " + protocolError.getMessage());
          return;
        } else {
          LoggerUtils.logMsg(
              logger,
              ...
              formatter,
              Level.WARNING,
              "Proposer phase 1 unknown" + " response: " + op);
          return;
        }
      }
  ...
  private Phase2Result phase2(
    ...
    final Accept accept = elections.getProtocol().new Accept(proposal, phase2Value);
    FutureTrackingCompService<MessageExchange> compService =
        Utils.broadcastMessage(
            promisories, Acceptor.SERVICE_NAME, accept, elections.getThreadPool());
   ...
   * @throws HigherNumberedProposal if a higher numbered proposal was encountered
   */
  private Phase2Result tallyPhase2Results(
      final QuorumPolicy quorumPolicy, final FutureTrackingCompService<MessageExchange> compService)
      throws HigherNumberedProposal {
    ...
    try {
      new Utils.WithFutureExceptionHandler<MessageExchange>(
        ...
        @Override
        protected void processResponse(MessageExchange me) {
          final ResponseMessage responseMessage = me.getResponseMessage();
          ...
          if (op == protocol.REJECT) {
            ...
            final Protocol.ProtocolError errorMessage = (Protocol.ProtocolError) responseMessage;
            LoggerUtils.logMsg(
                logger,
                elections.getRepImpl(),
                formatter,
                Level.WARNING,
                "Proposer phase 2 " + "protocol error:" + errorMessage.getMessage());
          } else {
            LoggerUtils.logMsg(
                logger,
                elections.getRepImpl(),
                formatter,
                Level.WARNING,
                "Proposer Phase 2 " + "unknown response: " + op);
          }
        }
  ...
  /**
   * Selects the value to be used during Phase2, which if it succeeds will become the "chosen
   * value".
   *
   * @param exchanges the message exchanges from Phase 1
   * @return the candidate value for Phase2, or null, if no suitable value was found during phase 1
   */
  ...
  /**
   * Runs an election using the consensus algorithm. An election can be used either to determine an
   * existing consensus value, or to force an election if the current value is not suitable.
   *
   * @param quorumPolicy the policy used to determine whether we have a quorum.
   * @param retryPredicate the interface which determines whether a retry should be attempted.
   * @return the WinningProposal encapsulating the proposal and chosen value
   * @throws MaxRetriesException when a winning proposal could not be established.
  ...
  public WinningProposal issueProposal(QuorumPolicy quorumPolicy, RetryPredicate retryPredicate)
    ...
    while (retryPredicate.retry()) {
      try {
        ...
        final Value phase2Value = choosePhase2Value(result1.promisoryMessages);
        if (phase2Value == null) {
          continue;
        }

        final Phase2Result result2 =
            phase2(quorumPolicy, proposal, phase2Value, result1.promisories);
        if (result2 == null) {
          continue;
        }
        ...
        LoggerUtils.logMsg(
            ...
            formatter,
            Level.INFO,
            "Winning proposal: " + proposal + " Value: " + phase2Value);
  ...
  private static class Phase1Result {
    ...
    Set<MessageExchange> promisoryMessages = new HashSet<>();
  ...
  public static class WinningProposal {
    ...
    public WinningProposal(Proposal proposal, Value value, StatGroup proposerStats) {
  ...
  public interface Proposal extends Comparable<Proposal> {
    ...
    /**
     * The string representation of the proposal. It must not contain the Protocol.SEPARATOR
     * character.
     *
  ...
  public interface ProposalParser {
     ...
     */
    Proposal parse(String wireFormat);
  }

  /**
   * Interface to determine whether the Proposer should be retry with a new Proposal or abandon the
   * effort.
   */
