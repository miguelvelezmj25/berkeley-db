...
/**
 * Defines the request/response messages used in the implementation of elections.
 *
 * <p>From Proposer to Acceptor: Propose -> Promise | Reject Accept -> Accepted | Reject
 *
 * <p>From Proposer initiator to Learners: Result -> none
 *
 * <p>The following exchange is not part of the elections process, but is used by the Monitor to
 * query a Learner for the latest election result it's aware of, when the Monitor first starts up.
 * It is also used by nodes and utilities that are attempting to find the master.
 *
 * <p>From Monitor to Learner MasterQuery -> MasterQueryResponse | None
 */
public class Protocol extends TextProtocol {
   ...
   *
   * <p>Note that the "minor" part of the suggestion ranking is always tagged on to the end of the
   * promise request payload. Older pre 7.1.3 nodes will ignore extra tokens at the end, since they
   * do not know about the minor component of the ranking. This node will use it if it's present and
   * otherwise use a value of zero for the minor (VLSN) component.
   *
   * <p>So when comparing rankings across old and new nodes, we are effectively comparing a
   * Ranking(VLSN, Long.MIN_VALUE) with a Ranking(DTVLSN, VLSN), resulting in suboptimal election
   * results (from a dtvlsn perspective) while an upgrade is in progress, that is, it will tend to
   * favor the older node. But this inaccuracy will vanish once all nodes have been upgraded.
   */
  public class Promise extends ResponseMessage {
    ...
    /**
     * The major and minor components of the Ranking represent the DTVLSN and the latest VLSN
     * respectively.
     */
    ...
    public boolean equals(Object obj) {
      ...
      if (suggestion == null) {
        ...
        return suggestionRanking.equals(other.suggestionRanking);
    }
    ...
    public String wireFormat() {
      String line =
          wireFormatPrefix()
              + SEPARATOR
              + ((highestProposal != null) ? highestProposal.wireFormat() : "")
              + SEPARATOR
  ...
  public class Reject extends ResponseMessage {
    ...
    public boolean equals(Object obj) {
      ...
      if (higherProposal == null) {
          return other.higherProposal == null;
      } else return higherProposal.equals(other.higherProposal);
    }
  ...
  public class Propose extends RequestMessage {
    ...
    public boolean equals(Object obj) {
      ...
      if (proposal == null) {
          return other.proposal == null;
      } else return proposal.equals(other.proposal);
    }
