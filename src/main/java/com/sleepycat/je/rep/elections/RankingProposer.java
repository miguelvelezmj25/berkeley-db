...
public class RankingProposer extends Proposer {
  ...
  /**
   * If non-zero, use the specified log version as the one that supports writing replication data in
   * the previous format, to control whether the skipPromiseDueToVersion method should always return
   * false, for testing.
   */
  ...
  /**
   * Chooses a Value based on the relative ranking of all Promise responses. The one with the
   * highest ranking is chosen. Zero priority responses are never chosen. In the case of a tie,
   * priority is used to resolve it. If priority is insufficient the socket address is used to order
   * the choice so that a consistent result is obtained across the set irrespective of the iteration
   * order over the set.
   */
  @Override
  protected Value choosePhase2Value(Set<MessageExchange> exchanges) {
    Ranking maxRanking = new Ranking(Long.MIN_VALUE, Long.MIN_VALUE);
    ...
    VersionCalculator calculator = new VersionCalculator(elections, exchanges);

    Value acceptorValue = null;
    for (MessageExchange me : exchanges) {
      if (me.getResponseMessage().getOp() != elections.getProtocol().PROMISE) {
        continue;
      }
      ...
      final boolean isArb =
          (suggestion instanceof MasterValue)
      ...
      final int compareTo = p.getSuggestionRanking().compareTo(maxRanking);
      if (compareTo < 0) {
        continue;
      }
      ...
      if (compareTo == 0) {
         ...
         */
        if ((p.getPriority() == maxPriority)
            && ((maxTarget != null) && (me.target.toString().compareTo(maxTarget) <= 0))) {
          continue;
        }
      }
  ...
  /**
   * Returns a proposal number. Note that the proposal numbers must increase over time, even across
   * restarts of the proposer process.
   *
  ...
  private static class VersionCalculator {
    ...
    public VersionCalculator(Elections elections, Set<MessageExchange> exchanges) {
    ...
    private void calculate() {
      ...
      for (MessageExchange me : exchanges) {
        ...
        if (!logFormats.containsKey(p.getLogVersion())) {
          ...
          logFormats.put(p.getLogVersion(), logFormats.get(p.getLogVersion()) + 1);
        }
      }
      ...
      /* Calculate the majority log version. */
      int electableNodeCount = elections.getRepNode().getGroup().getElectableGroupSize();
    ...
    /**
     * For JE 5 and earlier versions, if there are multiple log versions in a replication group,
     * only elect the nodes with the lowest log version or the nodes with the majority log version
     * to be the master. This behavior is required because, in those JE versions, the master can
     * only supply replication data in the current log format, so the master must be chosen from
     * nodes running the earlier version during an upgrade until a majority of the nodes have been
     * upgraded. This restriction no longer applies as of log version 9 in JE 6 -- see [#22336].
     *
     * <p>Returns true if election will ignore the promise because there are multiple log versions
     * in the group, all log versions correspond to JE 5 and earlier versions, and the log version
     * of a replica satisfies one of the following rules:
     *
     * <ol>
     *   <li>If there is no majority log version in the group, and log version of this replica is
     *       not the lowest log version.
     *   <li>If there exists a majority log version, and log version of this replica is not the
     *       lowest log version, nor the majority log version.
     * </ol>
     */
    boolean skipPromiseDueToVersion(int logVersion) {
      ...
      if (logVersionReplicatePrevious == 0) {
        logVersionReplicatePrevious = LogEntryType.LOG_VERSION_REPLICATE_OLDER;
      }
      if (lowestVersion >= logVersionReplicatePrevious - 1) {
        return false;
      }
        return (majorityVersion == Integer.MIN_VALUE && logVersion != lowestVersion)
