...
import static com.sleepycat.je.rep.impl.node.FeederManagerStatDefinition.*;
...
import static com.sleepycat.je.rep.stream.FeederTxnStatDefinition.*;
import static com.sleepycat.je.rep.utilint.BinaryProtocolStatDefinition.*;
import static com.sleepycat.je.utilint.CollectionUtils.emptySortedMap;

/**
 * Statistics for a replicated environment.
 *
 * <p>The statistics are logically grouped into four categories. Viewing the statistics through
 * {@link ReplicatedEnvironmentStats#toString()} displays the values in these categories, as does
 * viewing the stats through the {@link <a
 * href="{@docRoot}/../jconsole/JConsole-plugin.html">RepJEMonitor mbean</a>}. Viewing the stats
 * with {@link ReplicatedEnvironmentStats#toStringVerbose()} will provide more detailed descriptions
 * of the stats and stat categories.
 *
 * <p>The current categories are:
 *
 * <ul>
 *   <li><b>FeederManager</b>: A feed is the {@link <a
 *       href="{@docRoot}/../ReplicationGuide/introduction.html#replicationstreams">replication
 *       stream</a>} between a master and replica. The current number of feeders gives a sense of
 *       the connectivity of the replication group.
 *   <li><b>BinaryProtocol</b>: These statistics center on the network traffic engendered by the
 *       replication stream, and provide a sense of the network bandwidth seen by the replication
 *       group.
 *   <li><b>Replay</b>: The act of receiving and applying the replication stream at the Replica node
 *       is called Replay. These stats give a sense of how much load the replica node is
 *       experiencing when processing the traffic from the replication group.
 *   <li><b>ConsistencyTracker</b>: The tracker is invoked when consistency policies are used at a
 *       replica node. This provides a measure of delays experienced by read requests at a replica,
 *       in order to conform with the consistency specified by the application.
 * </ul>
 *
...
public class ReplicatedEnvironmentStats implements Serializable {
  ...
  /**
   * The "impossible" return value used by stats accessors to indicate the statistic is not
   * available in this instance of ReplicatedEnvironmentStats, because it represents an earlier
  ...
  public Collection<StatGroup> getStatGroups() {
    return (feederTxnStats != null)
        ? Arrays.asList(
            ...
            vlsnIndexStats)
        : Arrays.asList(
            feederManagerStats, replayStats, trackerStats, protocolStats, vlsnIndexStats);
  }
  ...
  private void addMessageRateStats() {
    ...
    numerator =
        (protocolStats.getLongStat(N_MESSAGES_READ) == null)
            ? 0
            : protocolStats.getLongStat(N_MESSAGES_READ).get();
    denominator =
        (protocolStats.getLongStat(N_READ_NANOS) == null)
            ? 0
            : protocolStats.getLongStat(N_READ_NANOS).get();
    @SuppressWarnings("unused")
    IntegralLongAvgStat msgReadRate =
        new IntegralLongAvgStat(
            protocolStats, MESSAGE_READ_RATE, numerator, denominator, 1000000000);

    numerator =
        (protocolStats.getLongStat(N_MESSAGES_WRITTEN) == null)
            ? 0
            : protocolStats.getLongStat(N_MESSAGES_WRITTEN).get();
    denominator =
        (protocolStats.getLongStat(N_WRITE_NANOS) == null)
            ? 0
            : protocolStats.getLongStat(N_WRITE_NANOS).get();
    @SuppressWarnings("unused")
    IntegralLongAvgStat msgWriteRate =
        new IntegralLongAvgStat(
            protocolStats, MESSAGE_WRITE_RATE, numerator, denominator, 1000000000);
  }
  ...
  private void addBytesRateStats() {
    ...
    numerator =
        (protocolStats.getLongStat(N_BYTES_READ) == null)
            ? 0
    ...
    IntegralLongAvgStat bytesReadRate =
        new IntegralLongAvgStat(protocolStats, BYTES_READ_RATE, numerator, denominator, 1000000000);

    numerator =
        (protocolStats.getLongStat(N_BYTES_WRITTEN) == null)
            ? 0
    ...
    IntegralLongAvgStat bytesWriteRate =
        new IntegralLongAvgStat(
            protocolStats, BYTES_WRITE_RATE, numerator, denominator, 1000000000);
  }

  /* Feeder Stats. */

  /**
   * The number of Feeder threads since this node was started. A Master supplies the Replication
   * Stream to a Replica via a Feeder thread. The Feeder thread is created when a Replica connects
   * to the node and is shutdown when the connection is terminated.
   */
  ...
  /**
   * The number of Feeder threads that were shut down, either because this node, or the Replica
  ...
  /**
   * The lag (in VLSNs) associated with the replica that's farthest behind in replaying the
   * replication stream.
   */
  ...
  /**
   * Returns a map from replica node name to the delay, in milliseconds, between when a transaction
   * was committed on the master and when the master learned that the transaction was processed on
   * the replica, if known. Returns an empty map if this node is not the master.
   *
   * @since 6.3.0
   */
  ...
  /**
   * Returns a map from replica node name to the commit timestamp of the last committed transaction
   * that was processed on the replica, if known. Returns an empty map if this node is not the
   * master.
   *
   * @since 6.3.0
   */
  public SortedMap<String, Long> getReplicaLastCommitTimestampMap() {
    final AtomicLongMapStat stat =
        (AtomicLongMapStat) feederManagerStats.getStat(REPLICA_LAST_COMMIT_TIMESTAMP_MAP);
    if (stat == null) {
      return emptySortedMap();
    }
    return stat.getMap();
  ...
  /**
   * Returns a map from replica node name to the VLSN of the last committed transaction that was
   * processed on the replica, if known. Returns an empty map if this node is not the master.
   *
   * @since 6.3.0
   */
  public SortedMap<String, Long> getReplicaLastCommitVLSNMap() {
    final AtomicLongMapStat stat =
        (AtomicLongMapStat) feederManagerStats.getStat(REPLICA_LAST_COMMIT_VLSN_MAP);
    if (stat == null) {
      return emptySortedMap();
    }
    return stat.getMap();
  ...
  /**
   * Returns a map from replica node name to the lag, in VLSNs, between the replication state of the
   * replica and the master, if known. Returns an empty map if this node is not the master.
   *
   * @since 6.3.0
   */
  ...
  /**
   * Returns a map from replica node name to a moving average of the rate, in VLSNs per minute, that
   * the replica is processing replication data, if known. Returns an empty map if this node is not
   * the master.
   *
   * @since 6.3.0
   */
  public SortedMap<String, Long> getReplicaVLSNRateMap() {
    final LongAvgRateMapStat stat =
        (LongAvgRateMapStat) feederManagerStats.getStat(REPLICA_VLSN_RATE_MAP);
    if (stat == null) {
      return emptySortedMap();
    }
    return stat.getMap();
  ...
  /**
   * The number of transactions that were successfully acknowledged based upon the {@link
   * ReplicaAckPolicy} policy associated with the transaction commit.
   */
  ...
  /**
   * The number of transactions that were not acknowledged as required by the {@link
   * ReplicaAckPolicy} policy associated with the transaction commit. These transactions resulted in
  ...
  /**
   * The total time in milliseconds spent in replicated transactions. This represents the time from
   * the start of the transaction until its successful commit and acknowledgment. It includes the
   * time spent waiting for transaction commit acknowledgments, as determined by {@link
   * #getAckWaitMs()}.
   */
  ...
  /**
   * The total time in milliseconds that the master spent waiting for the {@link ReplicaAckPolicy}
   * to be satisfied during successful transaction commits.
   *
   * @see #getTotalTxnMs()
   */
  ...
  /**
   * The VLSN of the last committed transaction on the master, or 0 if not known or this node is not
   * the master.
   *
   * @since 6.3.0
   */
  ...
  /**
   * The commit timestamp of the last committed transaction on the master, or 0 if not known or this
  ...
  /**
   * A moving average of the rate replication data is being generated by the master, in VLSNs per
   * minute, or 0 if not known or this node is not the master.
   *
   * @since 6.3.0
   */
  ...
  /**
   * The number of commit log records that were replayed by this node when it was a Replica. There
   * is one commit record record for each actual commit on the Master.
   */
  ...
  /**
   * The number of commit log records that needed to be acknowledged to the Master by this node when
   * it was a Replica. The rate of change of this statistic, will show a strong correlation with
   * that of <code>NReplayCommits</code> statistic, if the <code>Durability</code> policy used by
   * transactions on the master calls for transaction commit acknowledgments and the Replica is
   * current with respect to the Master.
   */
  ...
  /**
   * The number of commitSync() calls executed when satisfying transaction commit acknowledgment
  ...
  /**
   * The number of commitNoSync() calls executed when satisfying transaction commit acknowledgment
   * requests from the Master.
   */
  ...
  /**
   * The number of data records (creation, update, deletion) which were replayed while the node was
  ...
  /**
   * The number of group commits that were initiated due to the {@link
   * ReplicationConfig#REPLICA_GROUP_COMMIT_INTERVAL group timeout interval} being exceeded.
   *
   * @since 5.0.76
   */
  ...
  /**
   * The number of group commits that were initiated due the {@link
   * ReplicationConfig#REPLICA_MAX_GROUP_COMMIT max group size} being exceeded.
   *
   * @since 5.0.76
   */
  ...
  /**
   * @hidden TODO: Make visible after experimenting with this new stat
   *     <p>The sum of time periods, measured in milliseconds, between when update operations commit
   *     on the master and then subsequently commit on the replica. Divide this value by the total
   *     number of commit operations, available by calling {@link #getNReplayCommits}, to find the
   *     average commit lag for a single operation.
   *     <p>Note that each lag is computed on the replica by comparing the time of the master
   *     commit, as measured by the master, and time on the replica when it commits locally. As a
   *     result, the return value will be affected by any clock skew between the master and the
   *     replica.
   */
  ...
  /**
   * @hidden TODO: Make visible after experimenting with this new stat
   *     <p>The time in milliseconds between when the latest update operation committed on the
   *     master and then subsequently committed on the replica.
   *     <p>Note that the lag is computed on the replica by comparing the time of the master commit,
   *     as measured by the master, and time on the replica when it commits locally. As a result,
   *     the return value will be affected by any clock skew between the master and the replica.
   */
  ...
  /**
   * The number of bytes of Replication Stream read over the network. It does not include the TCP/IP
   * overhead.
   *
   * <p>If the node has served as both a Replica and Master since it was first started, the number
   * represents the sum total of all Feeder related network activity, as well as Replica network
   * activity.
   */
  ...
  /**
   * The number of Replication Stream messages that were written as part of a message batch instead
   * of being written individually.
   *
   * <p>It represents a subset of the messages returned by {@link #getNProtocolMessagesWritten()}
   *
  ...
  /**
   * Incoming replication message throughput, in terms of messages received from the replication
   * network channels per second.
   *
   * <p>If the node has served as both a Replica and Master since it was first started, the number
   * represents the message reading rate over all Feeder related network activity, as well as
  ...
  /**
   * Outgoing message throughput, in terms of message written to the replication network channels
   * per second.
   *
   * <p>If the node has served as both a Replica and Master since it was first started, the number
   * represents the message writing rate over all Feeder related network activity, as well as
   * Replica network activity.
   */
  ...
  /**
   * Bytes read throughput, in terms of bytes received from the replication network channels per
   * second.
   *
   * <p>If the node has served as both a Replica and Master since it was first started, the number
   * represents the bytes reading rate over all Feeder related network activity, as well as Replica
   * network activity.
   */
  ...
  /**
   * Bytes written throughput, in terms of bytes written to the replication network channels per
   * second.
   *
   * <p>If the node has served as both a Replica and Master since it was first started, the number
   * represents the bytes writing rate over all Feeder related network activity, as well as Replica
   * network activity.
   */
  ...
  /**
   * Returns the number of messages containing log entries that were written to the replication
   * stream using the previous log format to support replication to a replica running an earlier
  ...
  /**
   * The number of times a Replica held back a {@link
   * Environment#beginTransaction(Transaction,TransactionConfig)} operation to satisfy the {@link
  ...
  /**
   * The total time (in msec) for which a Replica held back a {@link
   * Environment#beginTransaction(Transaction,TransactionConfig)} operation to satisfy the {@link
   * TimeConsistencyPolicy}.
   */
