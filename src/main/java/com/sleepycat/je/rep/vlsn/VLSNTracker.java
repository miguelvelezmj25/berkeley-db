...
/**
 * See @link{VLSNIndex} for an overview of the mapping system. The VLSNTracker packages the
 * VLSNRange and the cached, in-memory VLSNBuckets.
 *
 * <p>The tracker has a notion of the "currentBucket", which is the one receiving updates. All other
 * cached buckets are finished and are awaiting a write to the database. Those finished buckets will
 * only be updated in special circumstances, such as log cleaning or replication stream truncation,
 * when we can assume that there will no readers viewing the buckets.
 */
class VLSNTracker {
  ...
  VLSNTracker(
    ...
    protectedFileRange =
        fileProtector.protectFileRange(
            FileProtector.VLSN_INDEX_NAME, 0 /*rangeStart*/, true /*protectVlsnIndex*/);
  ...
  /**
   * Return a bucket for reading a mapping for this VLSN. If vlsn is > lastOnDisk, a bucket is
  ...
  synchronized VLSNBucket getGTEBucket(VLSN vlsn) {
    ...
    throw EnvironmentFailureException.unexpectedState(
        ...
        "VLSN "
            + vlsn
            + " should be held within this tracker. "
            + this
            + " prevBucket="
            + prevBucket);
  }
   ...
   * directly preceding it. If this VLSN is >= firstTrackedVLSN, then we should be able to guarantee
   * that a bucket is returned.
   */
  ...
  /**
   * Record a new VLSN->LSN mapping. We guarantee that the first and last VLSNs in the range have a
   * mapping. If a VLSN comes out of order, we will discard it, and will not update the range.
   */
  synchronized void track(VLSN vlsn, long lsn, byte entryTypeNum) {
    ...
    if (bucketCache.size() == 0) {
      ...
      currentBucket =
          new VLSNBucket(DbLsn.getFileNumber(lsn), stride, maxMappings, maxDistance, vlsn);
    ...
    if (!currentBucket.put(vlsn, lsn)) {
      ...
      if (!currentBucket.put(vlsn, lsn)) {
        throw EnvironmentFailureException.unexpectedState(
            envImpl, "Couldn't put VLSN " + vlsn + " into " + currentBucket);
      }
    }
  ...
  /**
   * Flush the tracker cache to disk. Ideally, we'd allow concurrent VLSN put() calls while a flush
   * to database is happening. To do that, we need a more sophisticated synchronization scheme that
   * defines the immutable vs. mutable portion of the tracker cache. See SR [#17689]
   */
  ...
  private void validateBeforeWrite(
      VLSNBucket target, VLSNRange flushRange, VLSN currentLastVLSN, boolean isLastBucket) {

    if (!(currentLastVLSN.equals(NULL_VLSN))
        && (target.getFirst().compareTo(currentLastVLSN) <= 0)) {
      throw EnvironmentFailureException.unexpectedState(
          envImpl,
          "target bucket overlaps previous bucket. "
              + "currentLastVLSN= "
              + currentLastVLSN
              + " target="
              + target);
    ...
    if (isLastBucket) {
      if (target.getLast().compareTo(flushRange.getLast()) != 0) {
        throw EnvironmentFailureException.unexpectedState(
            envImpl,
            "end of last bucket should match end of range. "
                + "range= "
                + flushRange
                + " target="
                + target);
  ...
  /**
   * Initially the protectedFileRange is set to the file of the range start. It is advanced later
   * when the range head is truncated.
   */
  ...
  synchronized ProtectedFileSet protectRangeHead(String lockerName) {
    ...
    return envImpl
        .getFileProtector()
        .protectFileRange(
            lockerName, protectedFileRange.getRangeStart(), true /*protectVlsnIndex*/);
  }
  ...
  /**
   * Try to advance the VLSNIndex ProtectedFileRange and truncate the head of the tracker's range,
   * so that bytesNeeded can be freed by deleting files in this range.
   *
   * @return {deleteEnd, deleteFileNum} pair if the range was changed, or null if no change is
   *     currently needed/possible. -- deleteEnd is the last VLSN to be truncated. -- deleteFileNum
   *     the file having deleteEnd as its last VLSN.
   */
  synchronized Pair<VLSN, Long> tryTruncateFromHead(
      final long bytesNeeded, final LogItemCache logItemCache) {
    ...
    final Pair<VLSN, Long> truncateInfo =
        envImpl.getFileProtector().checkVLSNIndexTruncation(bytesNeeded, new VLSN(preserveVlsn));
     ...
     */
    return truncateFromHead(truncateInfo.first(), truncateInfo.second(), logItemCache)
        ? truncateInfo
        : null;
  }

  /**
   * Truncate the head of the tracker's range, and advance the ProtectedFileRange accordingly.
   *
   * @return true if range is changed, or false if no change is needed.
   */
  synchronized boolean truncateFromHead(
      VLSN deleteEnd, long deleteFileNum, LogItemCache logItemCache) {
  ...
  private void checkForGhostBucket(SortedMap<Long, VLSNBucket> buckets, long deleteFileNum) {
    ...
    if (!firstBucketVLSN.equals(firstRangeVLSN)) {
      ...
      VLSNBucket placeholder =
          new GhostBucket(firstRangeVLSN, DbLsn.makeLsn(nextFile, 0), lastPossibleLsn);
   ...
   * We can assume no new mappings are coming in, but the VLSNIndex may be read during this time;
   * checkpoints continue to happen during rollbacks. Since syncup is always at a sync-able log
   * entry, and since we don't roll back past a commit, we can assume that the lastSync can be
   * changed to deleteStart.getPrev(), and lastTxnEnd is unchanged. The VLSNRange itself is updated
   * before the tracker and on-disk buckets are pruned.
   *
   * <p>The VLSNIndex guarantees that the beginning and end vlsns in the range have mappings so LTE
   * and GTE search operations work. When truncating the mappings in the tracker, check to see if
   * the new end vlsn has a mapping, since that may not happen if the buckets are cut in the middle,
   * or have a gap due to out of order mapping. For example, suppose mapping for vlsn 20 came before
   * the mappings for vlsn 17, 18 and 19. The buckets would have this gap: Bucket A: vlsns 10-16
   * Bucket B: vlsns 20-25 Bucket C: vlsns 26 - 30 If the new end of vlsn range is 11-14, 17, 18,
   * 19, 21 - 24, or 27-29, we have to end the vlsn tracker with a new mapping of (vlsn
   * (deleteStart-1) -> prevLsn) to cap off the buckets.
   *
   * <p>Cases: End of range is in the gap -- either 17, 18, 19 1. on disk buckets exist, but don't
   * encompass the gap: bucket A on disk 2. all buckets are in the tracker cache: bucket A in
   * tracker 3. on disk buckets encompass the gap: bucket A and B on disk.
   */
  synchronized void truncateFromTail(VLSN deleteStart, long prevLsn) {
    ...
    SortedMap<Long, VLSNBucket> newCache = new TreeMap<>(bucketCache.headMap(targetKey));
  ...
  /**
   * Called by the VLSNIndex to see if we have to add a bucket in the tracker after pruning the
   * on-disk storage. The get{LTE,GTE}Bucket methods assume that there are mappings for the first
   * and last VLSN in the range. But since out of order vlsn mappings can cause non-contiguous
   * buckets, the pruning may have lost the end mapping. [#23491]
   */
    ...
    if (lastOnDiskVLSN.compareTo(lastVLSN) < 0) {
      ...
      if (!firstTrackedVLSN.equals(NULL_VLSN)) {
        ...
        if (lastVLSN.compareTo(firstTrackedVLSN) < 0) {
          throw EnvironmentFailureException.unexpectedState(
              envImpl,
              "Expected this tracker to cover vlsn "
                  + lastVLSN
  ...
  private void addEndMapping(VLSN lastVLSN, long lastLsn) {
    ...
    assert (lastVLSN.compareTo(range.getLast()) == 0)
        : "lastVLSN=" + lastVLSN + " lastLsn = " + lastLsn + " range=" + range;

    VLSNBucket addBucket =
        new VLSNBucket(DbLsn.getFileNumber(lastLsn), stride, maxMappings, maxDistance, lastVLSN);
  ...
  /**
   * Attempt to replace the mappings in this vlsnIndex for deleteStart->lastVLSN with those from the
   * recovery mapper. Since we do not supply a prevLsn argument, we may not be able to "cap" the
   * truncated vlsn bucket the same what that we can in truncateFromTail. Because of that, we may
   * need to remove mappings that are < deleteStart.
   *
   * <p>For example, suppose a bucket holds mappings 10 -> 101 15 -> 201 18 -> 301 If deleteStart ==
   * 17, we will have to delete all the way to vlsn 15.
   *
   * <p>The maintenance of VLSNRange.lastSync and lastTxnEnd are simplified because of assumptions
   * we can make because we are about to append mappings found by the recovery tracker that begin at
   * deleteStart. If lastSync and lastTxnEnd are <= deleteStart, we know that they will either stay
   * the same, or be replaced by lastSync and lastTxnEnd from the recoveryTracker. Even we delete
   * mappings that are < deleteStart, we know that we did not roll back past an abort or commit, so
   * that we do not have to updated the lastSync or lastTxnEnd value.
   */
  ...
  synchronized boolean verify(boolean verbose) {
    ...
    if (firstVLSN.size() > 0) {
      if (!firstVLSN.get(0).equals(firstTrackedVLSN)) {
        if (verbose) {
          System.err.println(
              "firstBucketVLSN = " + firstVLSN.get(0) + " should equal " + firstTrackedVLSN);
        }
        return false;
      }
      ...
      if (!lastBucketVLSN.equals(range.getLast())) {
        if (verbose) {
          System.err.println(
              "lastBucketVLSN = " + lastBucketVLSN + " should equal " + range.getLast());
        }
        return false;
      }
    }
