...
public class VLSNRange {
  ...
  /**
   * When the range is written out by the VLSNTracker, we must always be sure to update the
   * tracker's lastVSLNOnDisk field. Return the last VLSN in the range as part of this method, to
   * help ensure that update.
   *
   * @param envImpl
   * @param dbImpl
   * @param txn
   */
  VLSN writeToDatabase(final EnvironmentImpl envImpl, final DatabaseImpl dbImpl, Txn txn) {
    ...
    Cursor c = null;
    try {
      c = DbInternal.makeCursor(dbImpl, txn, CursorConfig.DEFAULT);
  ...
  public boolean contains(final VLSN vlsn) {
    if (first.equals(NULL_VLSN)) {
      ...
      return (first.compareTo(vlsn) <= 0) && (last.compareTo(vlsn) >= 0);
  }

  /** A new VLSN->LSN mapping has been registered in a bucket. Update the range accordingly. */
  VLSNRange getUpdateForNewMapping(final VLSN newValue, final byte entryTypeNum) {
    VLSN newFirst = first;
    VLSN newLast = last;
  ...
  VLSNRange getUpdate(final VLSNRange other) {
    VLSN newFirst = getComparison(first, other.first, other.first.compareTo(first) < 0);
    VLSN newLast = getComparison(last, other.last, other.last.compareTo(last) > 0);
    VLSN newLastSync =
        getComparison(lastSync, other.lastSync, other.lastSync.compareTo(lastSync) > 0);
    VLSN newLastTxnEnd =
        getComparison(lastTxnEnd, other.lastTxnEnd, other.lastTxnEnd.compareTo(lastTxnEnd) > 0);
    return new VLSNRange(newFirst, newLast, newLastSync, newLastTxnEnd);
  }
  ...
  VLSNRange shortenFromEnd(final VLSN deleteStart) {
    ...
    assert newLast.compareTo(lastTxnEnd) >= 0
        : "Can't truncate at " + newLast + " because it overwrites a commit at " + lastTxnEnd;
  ...
  VLSNRange shortenFromHead(final VLSN deleteEnd) {
    ...
    assert (lastSync.equals(NULL_VLSN) || lastSync.compareTo(newFirst) >= 0)
        : "Can't truncate lastSync= " + lastSync + " deleteEnd=" + deleteEnd;

    VLSN newTxnEnd = lastTxnEnd.compareTo(newFirst) > 0 ? lastTxnEnd : NULL_VLSN;

    return new VLSNRange(newFirst, newLast, lastSync, newTxnEnd);
  }

  /**
   * Compare two VLSNs, normalizing for NULL_VLSN. If one of them is NULL_VLSN, return the other
   * one. If neither are NULL_VLSN, use the result of the comparison, expressed as the value of
   * "better" to decide which one to return. If "better" is true, return "otherVLSN".
   */
  private VLSN getComparison(final VLSN thisVLSN, final VLSN otherVLSN, final boolean better) {
  ...
  static class VLSNRangeBinding extends TupleBinding<VLSNRange> {
    ...
    public VLSNRange entryToObject(final TupleInput ti) {
      ...
      if (onDiskVersion != VERSION) {
        throw EnvironmentFailureException.unexpectedState(
            "Don't expect version diff " + "on_disk=" + onDiskVersion + " source=" + VERSION);
      }

      VLSNRange range =
          new VLSNRange(
              new VLSN(ti.readPackedLong()), // first
    ...
    @Override
    public void objectToEntry(final VLSNRange range, final TupleOutput to) {
