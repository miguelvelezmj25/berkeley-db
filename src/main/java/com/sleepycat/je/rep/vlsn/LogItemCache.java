...
/**
 * A no-wait cache used to retain the most recent VLSNs. The Feeders check this cache first for log
 * entries to send out to the Replicas. Feeders that are feeding at the most up to date portion of
 * the replication stream will likely hit in the cache, preventing a lookup in the log buffers or
 * log files.
 *
 * <p>The log item cache is made up of weak references so there is never any guarantee that even the
 * most recent 32 entries are in there.
 */
class LogItemCache {
  ...
  LogItem get(VLSN vlsn) {
    final LogItem item = getArray()[(int) vlsn.getSequence() & sizeMask].get();
  ...
  /**
   * Returns the cache array, creating a new one, if the GC had cleared the reference to the earlier
   * one.
   *
   * <p>Note that there may be a slight inefficiency if getArray is called concurrently, and it had
   * been cleared earlier, since it would be allocated twice and introduce a cache miss. This
   * occurrence is infrequent enough that it's not worth the overhead of a sync mechanism.
   *
   * @return the underlying array, allocating a new one, if the previous one had been GC'd
   */
  ...
  private final AtomicReference<LogItem>[] getArray() {
    ...
    if (array == null) {
      ...
      cacheReference = new SoftReference<AtomicReference<LogItem>[]>(array);
    }
    return array;
  }
}
