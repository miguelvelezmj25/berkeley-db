...
/**
 * Thrown by {@link Environment#beginTransaction} and {@link Transaction#commit} when these
 * operations are initiated at a Master which is not in contact with a quorum of Replicas as
 * determined by the {@link ReplicaAckPolicy} that is in effect for the operation.
 */
public class InsufficientReplicasException extends OperationFailureException {
  ...
  /**
   * Returns the number of nodes (including the master) that were required to be active in order to
  ...
  /**
   * Returns the set of Replicas that were in contact with the master at the time of the commit
   * operation.
   *
  ...
  private static String makeMsg(
      ReplicaAckPolicy commitPolicy, int requiredAckCount, Set<String> availableReplicas) {
