...
/**
 * Communicates the {@link ReplicatedEnvironment.State state} change at a node to the
 * StateChangeListener. There is a distinct instance of this event representing each state change at
 * a node.
 *
 * <p>Each event instance may have zero or more state change related exceptions associated with it.
 * The exceptions are of type {@link StateChangeException}. {@link StateChangeException} has a
 * method called {@link StateChangeException#getEvent()} that can be used to associate an event with
 * an exception.
 *
...
public class StateChangeEvent implements Serializable {
  ...
  /**
   * @hidden For internal use only. Creates a StateChangeEvent identifying the new state and the new
   *     master if there is a master in the new state.
   * @param state the new state
   * @param masterNameId the new master or <code>NULL</code> if there isn't one.
   */
  ...
  /**
   * Returns the time (in nano second units) the event occurred, as reported by {@link
   * System#nanoTime}
   *
   ...
   * @throws IllegalStateException if the node is in the <code>DETACHED</code> or <code>UNKNOWN
   *     </code> state.
   */
