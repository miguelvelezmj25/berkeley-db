...
/**
 * Thrown when an attempt is made to access an environment that was shutdown by the Master as a
 * result of a call to {@link ReplicatedEnvironment#shutdownGroup(long, TimeUnit)}.
 */
public class GroupShutdownException extends EnvironmentFailureException {
  ...
  public GroupShutdownException(Logger logger, RepNode repNode, long shutdownTimeMs) {
    super(
        ...
        String.format(
            "Master:%s, initiated shutdown at %1tc.",
    ...
    LoggerUtils.warning(
        logger, repNode.getRepImpl(), "Explicit shutdown request from Master:" + masterNodeName);
  }
   ...
   * For internal use only.
   *
   * <p>Returns the shutdownVLSN, if it was available, at the time of the exception
   *
   * @hidden
   */
