...
public class ReplayTxn extends Txn {
  ...
  /** Used when creating ReplayTxns for Replay. The ReplayTxn adds itself to the activeTxns map. */
  public ReplayTxn(
  ...
  /**
   * Don't let the ReplayTxn have a timeout of 0. If it did, it could be deadlocked against a reader
   * txn. As long as there is a non zero timeout, any conflicts will be adjugated by the LockManager
   * in its favor.
   */
  @Override
  ...
  /**
   * Utility method to validate DTVLSN. It ensures that the DTVLSN is not null and that DTVLSN(vlsn)
   * <= VLSN.
   */
  private long validateDTVLSN(VLSN txnVLSN, long checkDTVLSN) {
    ...
    if (checkDTVLSN > txnVLSN.getSequence()) {
      throw new IllegalStateException("DTVLSN(vlsn)=" + checkDTVLSN + " > " + "vlsn=" + txnVLSN);
    }
  ...
  public long commit(
    ...
    this.dtvlsn = validateDTVLSN(clientRepContext.getClientVLSN(), commitDTVLSN);
  ...
  public long abort(ReplicationContext clientRepContext, int abortMasterNodeId, long abortDTVLSN)
      throws DatabaseException {

    setRepContext(clientRepContext);
    this.dtvlsn = validateDTVLSN(clientRepContext.getClientVLSN(), abortDTVLSN);
    /*
  ...
  /**
   * Always return true in order to ensure that the VLSN is logged. Normally this method returns
   * false when no LN has been logged by the txn. But when replaying a Master txn, we need to
   * guarantee that the VLSN is logged on the Replica. [#20543]
   */
  @Override
    ...
    try {
      while ((undoLsn != DbLsn.NULL_LSN) && DbLsn.compareTo(undoLsn, matchpointLsn) > 0) {
        ...
        RecoveryManager.rollbackUndo(
            logger, Level.FINER, location, undo.db, undo.logEntry, undoLsn, revertTo);
  ...
  private void countObsoleteInexact(long undoLsn, UndoReader undo) {
    ...
    envImpl
        .getLogManager()
        .countObsoleteNode(
            undoLsn, null, /*type*/ undo.logEntrySize, undo.db, false /*countExact*/);
  }

  /**
   * Returns the elapsed time associated with this transaction. If the transaction is in progress,
   * it returns the running elapsed time.
   *
  ...
  /**
   * Returns the time when this transaction was committed or aborted. Returns zero if the
