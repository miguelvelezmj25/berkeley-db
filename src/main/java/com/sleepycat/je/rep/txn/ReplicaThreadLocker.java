...
/**
 * A ReplicaThreadLocker is used with a user initiated non-transactional operation on a Replica, for
 * a replicated DB.
 *
 * <p>Like ReadonlyTxn, it enforces read-only semantics and implements the ReplicaConsistencyPolicy.
 * Unlike ReadonlyTxn, the environment default ReplicaConsistencyPolicy is enforced rather than the
 * policy specified via TransactionConfig, and this policy is enforced via the openCursorHook rather
...
public class ReplicaThreadLocker extends ThreadLocker {
  ...
  /**
   * This overridden method prevents writes on a replica. This check is redundant because Cursor
   * won't allow writes to a transactional database when no Transaction is specified. But we check
   * here also for safety and for consistency with ReadonlyTxn.
   */
  @Override
  ...
  public void openCursorHook(DatabaseImpl dbImpl) throws ReplicaConsistencyException {
    ...
    ReadonlyTxn.checkConsistency(repImpl, repImpl.getDefaultConsistencyPolicy());
  }
}
