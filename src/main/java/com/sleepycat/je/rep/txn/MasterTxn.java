...
/**
 * A MasterTxn represents: - a user initiated Txn executed on the Master node, when local-write and
 * read-only are not configured, or - an auto-commit Txn on the Master node for a replicated DB.
 *
 * <p>This class uses the hooks defined by Txn to support the durability requirements of a
 * replicated transaction on the Master.
 */
public class MasterTxn extends Txn {
  ...
  /**
   * Flag to keep track of whether this transaction has taken the read lock that protects access to
   * the blocking latch used by Master Transfer.
   */
  private boolean locked;

  /**
   * Flag to prevent any change to the txn's contents. Used in master->replica transition.
   * Intentionally volatile, so it can be interleaved with use of the MasterTxn mutex.
   */
  ...
  private static final MasterTxnFactory DEFAULT_FACTORY =
      new MasterTxnFactory() {
        ...
        @Override
        public MasterTxn create(
            EnvironmentImpl envImpl, TransactionConfig config, NameIdPair nameIdPair) {
          return new MasterTxn(envImpl, config, nameIdPair);
        }

        @Override
        public MasterTxn createNullTxn(
            EnvironmentImpl envImpl, TransactionConfig config, NameIdPair nameIdPair) {
  ...
  public MasterTxn(EnvironmentImpl envImpl, TransactionConfig config, NameIdPair nameIdPair)
  ...
  /**
   * MasterTxns use txn ids from a reserved negative space. So override the default generation of
   * ids.
   */
  @Override
  ...
  protected void preLogCommitHook() throws DatabaseException {
    ...
    requiredAckCount =
        repImpl.getRepNode().getDurabilityQuorum().getCurrentRequiredAckCount(ackPolicy);
  ...
  /**
   * Prevent this MasterTxn from taking locks if the node becomes a replica. The application has a
   * reference to this Txn, and may attempt to use it well after the node has transitioned from
   * master to replica.
   */
  @Override
  public LockResult lockInternal(
      long lsn,
      LockType lockType,
      boolean noWait,
  ...
  private void throwNotMaster(ReplicatedEnvironment.State nodeState) {
    if (nodeState.isReplica()) {
      throw new ReplicaWriteException(this, ((RepImpl) envImpl).getStateChangeEvent());
    }
   ...
   *
   * <p>We acquire the lock during pre-log hook, and release it during post-log hook. Specifically,
   * there are the following cases:
   *
   * <ol>
   *   <li>For a normal commit, we acquire it in {@code preLogCommitHook()} and release it in {@code
   *       postLogCommitHook()}
   *   <li>For a normal abort (invoked by the application on the {@code Txn.abort()} API), we
   *       acquire the lock in {@code preLogAbortHook()} and release it in {@code
   *       postLogAbortHook()}.
   *   <li>When a commit fails in such a way as to call {@code Txn.throwPreCommitException()}, we go
   *       through the abort path as well. In this case:
   *       <ul>
   *         <li>we will of course already have called {@code preLogCommitHook()};
   *         <li>the abort path calls {@code preLogAbortHook()} and {@code postLogAbortHook()} as
   *             always;
   *         <li>finally we call {@code postLogCommitAbortHook()}
   *       </ul>
   *       Fortunately we can avoid the complexity of dealing with a second (recursive) lock
   *       acquisition here, because by the time either post-hook is called we've done any writing
   *       of VLSNs. Thus, when we want to take the lock, we take it if it hasn't already been
   *       taken, and do nothing if it has; when releasing, we release it if we have it, and do
   *       nothing if we don't.
   * </ol>
   *
   * <p>See additional javadoc at {@code RepImpl.blockLatchLock}
   */
  ...
  public void stampRepWriteTime() {
    this.repWriteStartDeltaMs = (int) (System.currentTimeMillis() - startMs);
  }

  /**
   * Returns the amount of time it took to copy the commit record from the log buffer to the rep
   * stream. It's measured as the time interval starting with the time the preCommit hook completed,
   * to the time the message write to the replication stream was initiated.
   */
  ...
  protected boolean propagatePostCommitException(DatabaseException postCommitException) {
    return postCommitException instanceof InsufficientAcksException || super.propagatePostCommitException(postCommitException);
  }
  ...
  public interface MasterTxnFactory {
    ...
    /**
     * Create a special "null" txn that does not result in any changes to the environment. It's sole
     * purpose is to persist and communicate DTVLSN values.
     */
    MasterTxn createNullTxn(
        EnvironmentImpl envImpl, TransactionConfig config, NameIdPair nameIdPair);
  }

  /* The method used to create user Master Txns via the factory. */
  public static MasterTxn create(
      EnvironmentImpl envImpl, TransactionConfig config, NameIdPair nameIdPair) {
    return factory.create(envImpl, config, nameIdPair);
  }

  public static MasterTxn createNullTxn(
      EnvironmentImpl envImpl, TransactionConfig config, NameIdPair nameIdPair) {
   ...
   *
   * <p>Sets the factory to the one supplied. If the argument is null it restores the factory to the
  ...
  /**
   * Convert a MasterTxn that has any write locks into a ReplayTxn, and close the MasterTxn after it
   * is disemboweled. A MasterTxn that only has read locks is unchanged and is still usable by the
   * application. To be clear, the application can never use a MasterTxn to obtain a lock if the
   * node is in Replica mode, but may indeed be able to use a read-lock-only MasterTxn if the node
   * cycles back into Master status.
   *
   * <p>For converted MasterTxns, all write locks are transferred to a replay transaction, read
   * locks are released, and the txn is closed. Used when a node is transitioning from master to
   * replica mode without recovery, which may happen for an explicit master transfer request, or
   * merely for a network partition/election of new master.
   *
   * <p>The newly created replay transaction will need to be in the appropriate state, holding all
   * write locks, so that the node in replica form can execute the proper syncups. Note that the
   * resulting replay txn will only be aborted, and will never be committed, because the txn
   * originated on this node, which is transitioning from master -> replica.
   *
   * <p>We only transfer write locks. We need not transfer read locks, because replays only operate
   * on writes, and are never required to obtain read locks. Read locks are released though, because
   * (a) this txn is now only abortable, and (b) although the Replay can preempt any read locks held
   * by the MasterTxn, such preemption will add delay.
   *
   * @return a ReplayTxn, if there were locks in this transaction, and there's a need to create a
   *     ReplayTxn.
   */
  public ReplayTxn convertToReplayTxnAndClose(Logger logger, Replay replay) {
    ...
    if (!freeze) {
      throw EnvironmentFailureException.unexpectedState(
          envImpl, "Txn " + getId() + " should be frozen when converting to replay txn");
    }
     ...
     */
    LoggerUtils.info(
        logger, envImpl, "Transforming txn " + getId() + " from MasterTxn to ReplayTxn");
    ...
    try {
      synchronized (this) {
        if (isClosed()) {
          LoggerUtils.info(logger, envImpl, "Txn " + getId() + " is closed, no tranform needed");
          needToClose = false;
          return null;
        }
        ...
        if (lockedLSNs.size() == 0) {
          LoggerUtils.info(
              logger,
              envImpl,
              "Txn " + getId() + " had no write locks, didn't create" + " ReplayTxn");
          needToClose = false;
          return null;
        }
        ...
        LoggerUtils.info(
            logger,
            envImpl,
            "Txn " + getId() + " has " + lockedLSNs.size() + " locks to transform");
        ...
        for (Long lsn : sortedLsns) {
          ...
          LoggerUtils.info(logger, envImpl, "Txn " + getId() + " is transferring lock " + lsn);
       ...
       */
      if (needToClose) {
        LoggerUtils.info(logger, envImpl, "About to close txn " + getId() + " state=" + getState());
        close(false /*isCommit */);
        LoggerUtils.info(logger, envImpl, "Closed txn " + getId() + " state=" + getState());
      }
      assert TestHookExecute.doHookIfSet(convertHook, hookCount++);
    }
  ...
  /**
   * Used to hold the transaction stable while it is being cloned as a ReplayTxn, during
   * master->replica transitions. Essentially, there are two parties that now have a reference to
   * this transaction -- the originating application thread, and the RepNode thread that is trying
   * to set up internal state so it can begin to act as a replica.
   *
   * <p>The transaction will throw UnknownMasterException or ReplicaWriteException if the
   * transaction is frozen, so that the application knows that the transaction is no longer viable,
   * but it doesn't attempt to do most of the follow-on cleanup and release of locks that failed
   * aborts and commits normally attempt. One aspect of transaction cleanup can't be skipped though.
   * It is necessary to do the post log hooks to free up the block txn latch lock so that the
   * transaction can be closed by the RepNode thread. For example: - application thread starts
   * transaction - application takes the block txn latch lock and attempts commit or abort, but is
   * stopped because the txn is frozen by master transfer. - the application must release the block
   * txn latch lock.
   *
