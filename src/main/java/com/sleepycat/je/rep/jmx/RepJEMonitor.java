 ...
 *
 * <p>It not only has the same attributes and operations as the standalone JEMonitor, but also has
 * some special replicated related operations.
 */
    ...
    try {
      if (actionName.equals(OP_DUMP_REPSTATS)) {
        ...
        return ((ReplicatedEnvironment) env)
            .getRepStats(getStatsConfig(new Object[] {false, true}))
            .getTips();
