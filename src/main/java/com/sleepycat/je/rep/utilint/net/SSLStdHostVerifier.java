...
/**
 * This is an implementation of HostnameVerifier which verifies that the host to which we are
 * connected is valid using the standard SSL matching rules. That is, the host string that we are
 * using to connect with must have a match to the common name or a subject alternative name.
 */
  ...
  public boolean verify(String targetHost, SSLSession sslSession) {
    ...
    if (peerCerts[0] instanceof X509Certificate) {
      ...
      try {
        ...
        logger.log(
            INFO,
            "Unable to parse peer certificate: "
                + "issuer = "
                + issuerPrinc
                + ", serialNumber = "
                + serNo);
