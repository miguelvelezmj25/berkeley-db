...
import static com.sleepycat.je.rep.utilint.SizeAwaitMapStatDefinition.*;

/**
 * Creates a Map that Threads can conveniently wait on to contain a specific number of entries,
 * where the values optionally match a predicate. The wait functionality is provided by the
 * sizeAwait() method defined by this class. Map values must not be null.
 */
public class SizeAwaitMap<K, V> implements Map<K, V> {
   ...
   * thread.
   */
  private final HashMap<Integer, ExceptionAwareCountDownLatch> thresholdLatches;
   ...
   * @param predicate the predicate for counting matching entries, or {@code null} to match all
   *     entries
   */
  public SizeAwaitMap(final EnvironmentImpl envImpl, final Predicate<V> predicate) {
    this.envImpl = envImpl;
    this.predicate = predicate;
    thresholdLatches = new HashMap<Integer, ExceptionAwareCountDownLatch>();
    stats =
        new StatGroup(SizeAwaitMapStatDefinition.GROUP_NAME, SizeAwaitMapStatDefinition.GROUP_DESC);
  ...
  /**
   * Causes the requesting thread to wait until the map reaches the specified size or the thread is
   * interrupted.
   *
   * @param thresholdSize the size to wait for.
   * @return true if the threshold was reached, false, if the wait timed out.
   * @throws InterruptedException for the usual reasons, or if the map was cleared and the size
   *     threshold was not actually reached.
   */
  public boolean sizeAwait(int thresholdSize, long timeout, TimeUnit unit)
  ...
  /**
   * Notes the addition of a new value and counts down any latches that were assigned to that
   * threshold.
   */
  @Override
  ...
  /**
   * Clears the underlying map and the latch map, after first counting them down, thus permitting
   * any waiting threads to make progress.
   *
   * @cause the value is non-null if the map is being cleared in response to an exception and
   *     results in the exception being thrown in the waiting threads. It's null if the map is being
   *     cleared as part of a normal shutdown, in which case no exception is thrown.
   */
