...
/**
 * Supplies the basic infrastructure for BinaryProtocols used to exchange messages by the
 * replications stream and during network based restore operations.
 *
 * <p>Note that this class and its subclasses are not synchronized. There must be one instance of
 * this class per thread of control.
 *
 * <p>IMPORTANT: Please avoid all uses of ByteBuffer.get/put when serializing message fields of
 * types: long, int and short to avoid byte order issues. Use LogUtils.read/write methods instead,
...
public abstract class BinaryProtocol {
  ...
  protected static final int MESSAGE_HEADER_SIZE =
      2 /* Message op id (short) */ + 4 /* message size (int) */;

  /* Buffer reused to process the header of every message. */
  protected final ByteBuffer header = ByteBuffer.allocate((MESSAGE_HEADER_SIZE));
   ...
   */
  public final MessageOp CLIENT_VERSION = new MessageOp((short) 1001, ClientVersion.class);

  public final MessageOp SERVER_VERSION = new MessageOp((short) 1002, ServerVersion.class);

  public final MessageOp INCOMPATIBLE_VERSION =
      new MessageOp((short) 1003, IncompatibleVersion.class);

  public final MessageOp PROTOCOL_ERROR = new MessageOp((short) 1004, ProtocolError.class);
   ...
   * @param configuredVersion the version of the protocol that must be implemented/simulated by this
   *     protocol when communicating with the recipient.
   */
  protected BinaryProtocol(
      NameIdPair nameIdPair, int codeVersion, int configuredVersion, EnvironmentImpl envImpl) {
    ...
    nEntriesWrittenOldVersion = new LongStat(stats, N_ENTRIES_WRITTEN_OLD_VERSION);

    /* Initialize with the pre-defined protocol messages. */
    for (MessageOp op :
        new MessageOp[] {CLIENT_VERSION, SERVER_VERSION, INCOMPATIBLE_VERSION, PROTOCOL_ERROR}) {
    ...
    if (envImpl != null) {
      ...
      maxMessageSize = (mMSz == 0) ? (envImpl.getMemoryBudget().getMaxMemory() >> 1) : mMSz;
      useStringDefaultEncoding = configManager.getBoolean(RepParams.PROTOCOL_OLD_STRING_ENCODING);
    } else {
  ...
  /**
   * Returns the version associated with this protocol instance. Request message generated by this
   * instance conform to this version and responses are expected to conform to this version as well.
   *
  ...
  /**
   * Responsible for converting a String to an encoded value for all protocols.
   *
   * <p>In JE 5.0.36 and earlier, only the default encoding was supported. In later releases, a
   * config param was added to force UTF-8 to be used. In JE 5.1 and later, the default will be to
   * use UTF-8. [#20967]
   *
  ...
  /**
   * Responsible for converting an encoded value to a String for all protocols.
   *
   * @see #stringToBytes
   */
  ...
  public static class MessageOp {
    ...
    private final Constructor<? extends Message> constructor;
    ...
    public MessageOp(short opId, Class<? extends Message> messageClass) {
      ...
      try {
        constructor =
            messageClass.getConstructor(messageClass.getEnclosingClass(), ByteBuffer.class);
  ...
  /**
   * Returns the Op from a message buffer. It's always the first item in the buffer. Leaves the
   * message buffer positioned after the Op.
   *
  ...
  /**
   * Fills a dedicated empty buffer with bytes read from the channel. It flips the buffer after it
   * has been filled, so it's ready for reading.
   *
   * @param channel the channel to be read
   * @param buffer the buffer to be filled
   * @throws IOException if the errors were encountered while reading from the channel or the buffer
   *     could not be filled with the expected number of bytes
   */
    ...
    while (buffer.position() < buffer.limit()) {
      ...
      if (numRead <= 0) {
        throw new IOException(
            "Expected bytes: " + buffer.limit() + " read bytes: " + buffer.position());
      }
    }
   ...
   *
   * @param channel the channel to read from. Declared as a ReadableByteChannel rather than the more
   *     obvious SocketChannel to facilitate unit testing.
   * @throws IOException
   */
   ...
   * @throws IOException
   */
  private void write(Message message, WritableByteChannel channel, NameIdPair destinationNameIdPair)
      throws IOException {
    ...
    if (logger.isLoggable(Level.FINER)) {
      if (destinationNameIdPair == NameIdPair.NULL) {
        ...
        LoggerUtils.logMsg(
            logger, envImpl, formatter, Level.FINER, "Sent " + message + " to " + channel);
      } else {
        LoggerUtils.logMsg(
            logger,
            envImpl,
            formatter,
            Level.FINER,
            "Sent to " + destinationNameIdPair.getName() + ": " + message);
      }
    }
   ...
   *
   * <p>If buffering the message would result in an overflow, the current contents of the buffer are
   * flushed before the message is added to the buffer. If the size of the message exceeds the size
   * of the buffer, the message is flushed directly to the network.
   *
   * @param channel the channel to which the buffer is flushed on buffer overflows.
   * @param batchWriteBuffer the buffer accumulating the serialized messages It's best for
   *     performance if this is a direct byte buffer, to avoid another copy when the buffer is
   *     finally flushed.
   * @param nMessages the number of messages currently in the buffer, including this message, that
   *     is, nMessages is always > 0 upon entry.
   * @param message the message to be buffered.
   * @return the number of messages currently in the buffer after accounting for potential buffer
   *     flushes.
   */
  public int bufferWrite(
      WritableByteChannel channel, ByteBuffer batchWriteBuffer, int nMessages, Message message)
      throws IOException {
  ...
  public void flushBufferedWrites(
      WritableByteChannel channel, ByteBuffer batchWriteBuffer, int nMessages) throws IOException {
  ...
  private void flushBuffer(WritableByteChannel channel, ByteBuffer bb) throws IOException {
    ...
    /**
     * Even though the underlying channel is a blocking channel, there are circumstances where the
     * channel.write() returns without writing all of the buffer, in seeming contradiction to the
     * javadoc. One such example is when the peer fails in the middle of a write; in this case there
     * is a "successful" partial write and the next write will result in an I/O exception. There may
     * be other such examples as well, hence the defensive write loop below which tries to continue
  ...
  /**
   * Base message class for all messages exchanged in the protocol. Serialized layout of a message:
   * - opType (short) - size of the body of the message (int) - body of the message - message
   * specific fields
   *
   * <p>All non-abstract subclasses must implement a constructor with a ByteBuffer argument. This
   * constructor is used during serialization to recreate the Message instance. It's considered good
   * practice to declare all attributes of a message as final. It's a simple way to ensure that the
   * above constructor has initialized all the attributes of the message.
   */
  public abstract class Message implements WireFormatable {
    ...
    public boolean match(Message other) {
      return Arrays.equals(wireFormat().array().clone(), other.wireFormat().array().clone());
    }
    ...
    public ByteBuffer allocateInitializedBuffer(int size) {
      ByteBuffer messageBuffer = allocateWriteBuffer(MESSAGE_HEADER_SIZE + size);
  ...
  /**
   * Base class for simple messages. Ones where performance is not of the utmost importance and
   * reflection can be used to simplify message serialization and de-serialization.
   */
  protected abstract class SimpleMessage extends Message {

    /**
     * Assembles a sequence of arguments into its byte based wire format. The method does the
     * serialization in two steps. In step 1, it calculates the length of the buffer. In step 2, it
     * assembles the bytes into the allocated buffer. The interpretive approach used here is
     * suitable for the low performance requirements of the handshake protocol, but not for the core
     * data stream itself. It's for this reason that the method is associated with the Handshake
     * message class and not the Message class.
     *
    ...
    /**
     * Put the bytes for the wire format of the specified byte array into the current position in
     * the buffer.
     *
     * @param buffer the buffer
    ...
    protected VLSN getVLSN(ByteBuffer buffer) {
      ...
      return (vlsn == VLSN.NULL_VLSN_SEQUENCE) ? VLSN.NULL_VLSN : new VLSN(vlsn);
    }
