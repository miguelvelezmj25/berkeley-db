...
/**
 * ServiceDispatcher listens on a specific socket for service requests and dispatches control to the
 * service that is being requested. A service request message has the format:
 *
 * <p>Service:<one byte ServiceName.length><ServiceName>
 *
 * <p>The format of the message is binary, with all text being encoded in ascii.
 *
 * <p>Upon receipt of service request message, the new SocketChannel is queued for processing by the
 * service in the Queue associated with the service. The SocketChannel is the responsibility of the
 * service after this point. It can configure the channel to best suit the requirements of the
 * specific service.
 *
 * <p>The dispatcher returns a single byte to indicate success or failure. The byte value encodes a
 * ServiceDispatcher.Response enumerator.
 */
public class ServiceDispatcher extends StoppableThread {
   ...
   *
   * <p>Do not rearrange the order of the enumerators, since their ordinal values are currently used
   * in messages.
   */
  ...
  /**
   * Create a ServiceDispatcher listening on a specific socket for service requests. This service
   * dispatcher has been created on behalf of a replicated environment, and the node will be
   * informed of any unexpected failures seen by the dispatcher.
   *
   * @param socketAddress the socket on which it listens for service requests. This address may be
   *     extended to cover all local addresses, if {@link RepParams#BIND_INADDR_ANY} has been set to
   *     true.
   * @throws IOException if the socket could not be bound.
   */
  public ServiceDispatcher(
      InetSocketAddress socketAddress, RepImpl repImpl, DataChannelFactory channelFactory)
      throws IOException {

    super(
        repImpl,
        "ServiceDispatcher-" + socketAddress.getHostName() + ":" + socketAddress.getPort());
  ...
  private void bindSocket() throws IOException {
    ...
    if (bindException != null) {
      ...
      if (limitMs > 0) {
        ...
        LoggerUtils.logMsg(
            logger, repImpl, formatter, Level.INFO, RepUtils.exec("netstat", "-lntp"));
      }
  ...
  /**
   * Convenience overloading for when the dispatcher is created without a replicated environment,
   * e.g. when used by the Monitor, and in unit test situations.
   *
   * @see #ServiceDispatcher(InetSocketAddress, RepImpl, DataChannelFactory)
   */
  public ServiceDispatcher(InetSocketAddress socketAddress, DataChannelFactory channelFactory)
      throws IOException {
  ...
  /**
   * Shuts down the dispatcher, so that it's no longer listening for service requests. The port is
   * freed up upon return and the thread used to listen on the port is shutdown.
   */
  ...
  /**
   * Used by the client to set up a channel for the service. It performs the initial handshake
   * requesting the service and interprets the response to determine if it was successful.
   *
   ...
   * @throws ServiceConnectFailedException if the connection could not be made.
   */
  public static void doServiceHandshake(DataChannel channel, String serviceName)
  ...
  /**
   * A variation on the method above. It's used by the client to setup a channel for the service. It
   * performs the initial handshake requesting the service and interpreting the response to
   ...
   * @throws ServiceConnectFailedException if the connection could not be made.
   */
  public static void doServiceHandshake(
      DataChannel channel, String serviceName, AuthenticationMethod[] authInfo)
      throws IOException, ServiceConnectFailedException {

    final ClientHandshake handshake =
        new ClientHandshake(serviceName, authInfo, new ByteChannelIOAdapter(channel));
  ...
  /**
   * Returns the next socketChannel created in response to a request for the service. The
   * socketChannel and the associated socket is configured as requested in the arguments.
   *
   ...
   * @return the configured channel or null if there are no more channels, because the service has
   *     been shut down.
   * @throws InterruptedException
   */
  public DataChannel takeChannel(String serviceName, boolean blocking, int soTimeout)
      throws InterruptedException {
  ...
  /**
   * Returns the specific socket address associated with the dispatcher. Unlike
   * getSocketBoundAddress() it can never return the wild card address INADDR_ANY. This is the
   * address used by clients requesting ServiceDispatcher services.
   *
   * <p>If {@link RepParams#BIND_INADDR_ANY} has been set to true, this is one of the addresses that
   * the socket is associated with.
   *
   ...
   * For testing only.
   *
   * <p>Returns the server socket address that was actually used to bind the socket. It's the
   * wildcard address INADDR_ANY if the HA config {@link RepParams#BIND_INADDR_ANY} has been set to
   * true.
   */
  ...
  /**
   * Registers a service queue with the ServiceDispatcher. Requests for a service result in a new
   * SocketChannel being created on which the service can communicate with the requester of the
   * service.
   *
   * @param serviceName the name of the service being requested
   * @param serviceQueue the queue that will be used to hold channels established for the service.
   */
  ...
  public void register(Service service) {
    ...
    LoggerUtils.logMsg(
        logger, repImpl, formatter, Level.FINE, "Service: " + service.name + " registered.");
  ...
  /**
   * Cancels the registration of a service. Subsequent attempts to access the service will be
   * ignored and the channel will be closed and will not be queued.
   *
  ...
  public void cancel(String serviceName) {
    ...
    service.cancel();
    LoggerUtils.logMsg(
        logger, repImpl, formatter, Level.FINE, "Service: " + serviceName + " shut down.");
  }
  ...
  private void setAuthOptions() {
    ...
    if (auth == null) {
      ...
      authOptions = null;
      LoggerUtils.logMsg(logger, repImpl, formatter, Level.INFO, "No server auth method");
    } else {
      ...
      LoggerUtils.logMsg(
          ...
          formatter,
          Level.INFO,
          "Server auth method: " + method.getMechanismName());
    }
  }

  /**
   * Processes an accept event on the server socket. As a result of the processing a new
   * socketChannel is created, and the selector is registered with the new channel so that it can
   * process subsequent read events.
   */
  ...
  /**
   * Processes read events on newly established socket channels. Input on the channel is verified to
   * ensure that it is a service request. The read is accomplished in two parts, a read for the
   * fixed size prefix and the name length byte, followed by a read of the variable length name
   * itself.
   *
   * <p>Errors result in the channel being closed(with the key being canceled as a result) and a
   * null value being returned.
   *
   * <p>If the service request is sane, we may require the connecting entity to authenticate itself.
   *
   * @param initState the InitState object associated with the new channel
   * @return the ServiceName or null if there was insufficient input, or an error was encountered.
   */
  ...
  /**
   * The central run method. It dispatches to the "accept" and "read" event processing methods. Upon
   * a completed read, it verifies the validity of the service name and queues the channel for
   * subsequent consumption by the service.
   */
  @Override
  public void run() {
    LoggerUtils.logMsg(
    ...
    try {
      while (true) {
        try {
          /**
           * To make the dispatcher resilient to IP address change, we periodically check for such
           * change and rebind the socket if that occurs.
           *
           * <p>Speculation and rational: New communications fail sliently which is possibly caused
           * by that each TCP session use both IP and port number as the identifier. Thus it will
           * drop packages after IP address change. Yet no exception is raised in that situation.
           * Therefore, we cannot rely on exception for detecting such a change, but instead use an
           * active approach.
           */
      ...
      while (skIter.hasNext()) {
        ...
        if (initState != null) {
          LoggerUtils.logMsg(
              logger, repImpl, formatter, Level.INFO, "Server closing in-process handshake");
          closeChannel(initState.getChannel());
          key.cancel();
        }
      }
  ...
  /**
   * Performs the guts of the work underlying a service request. It validates the service request
   * and writes an appropriate response to the channel.
   *
   * @param channel
   * @param serviceName
   */
  ...
  private abstract static class Service {
  ...
  /**
   * A service where requests are simply added to the supplied queue. It's the responsibility of the
   * service creator to drain the queue. This service is used when the service carries out a
   * long-running dialog with the service requester. For example, a Feeder service.
   */
  ...
  /**
   * A queuing service that starts the thread that services the requests lazily, upon first request
   * and terminates the thread when the service is unregistered. The thread must be "interrupt
   * aware" and must exit when it receives an interrupt.
   *
   * <p>This type of service is suitable for services that are used infrequently.
   */
  public class LazyQueuingService extends QueuingService {
    ...
    public LazyQueuingService(
        String serviceName, BlockingQueue<DataChannel> queue, Thread serviceThread) {
    ...
    void requestDispatch(DataChannel channel) {
      ...
      switch (serviceThread.getState()) {
        case NEW:
          serviceThread.start();
          LoggerUtils.logMsg(
              logger, repImpl, formatter, Level.FINE, "Thread started for service: " + name);
          break;
        ...
        default:
          RuntimeException e =
              EnvironmentFailureException.unexpectedState(
                  "Thread for service:" + name + "is in state:" + serviceThread.getState());
          LoggerUtils.logMsg(
              logger, repImpl, formatter, Level.WARNING, LoggerUtils.exceptionTypeAndMsg(e));
          throw e;
      }
  ...
  /**
   * A service that is run immediately in a thread allocated to it. Subtypes implement the
   * getRunnable() method which provides the runnable object for the service. This service frees up
   * the caller from managing the the threads associated with the service. The runnable must manage
   * interrupts so that it can be shut down by the underlying thread pool.
   */
  public abstract static class ExecutingService extends Service {
  ...
  public static class ServiceConnectFailedException extends Exception {
    ...
    public String getMessage() {
      switch (response) {
        ...
        default:
          throw EnvironmentFailureException.unexpectedState(
              "Unexpected response:" + response + " for service:" + serviceName);
      }
    }
  }

  public abstract static class ExecutingRunnable implements Runnable {
    ...
    protected abstract ResponseMessage getResponse(RequestMessage request) throws IOException;
