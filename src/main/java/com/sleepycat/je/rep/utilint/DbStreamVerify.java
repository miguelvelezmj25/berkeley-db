...
/**
 * For internal use only. Utility to verify the replication stream and VLSN index. Used file readers
 * to traverse the log and check that VLSNs are sequential and that the VLSN index is consistent.
 */
public class DbStreamVerify {

  private static final String USAGE =
      "usage: "
          + CmdUtil.getJavaCommand(DbStreamVerify.class)
          + "\n"
  ...
  private class VerifyReader extends FileReader {
    ...
    protected boolean processEntry(ByteBuffer entryBuffer) {
      ...
      if (currentVLSN == null) {
        throw new RuntimeException(
            DbLsn.getNoFormatString(currentLSN) + "Should be a replicated entry");
      }

      if (currentVLSN.isNull()) {
        out1.println(
            "unexpected LSN " + DbLsn.getNoFormatString(getLastLsn()) + " has vlsn " + currentVLSN);
      }
      ...
      if ((lastVLSN != null) && lastVLSN.isNull()) {
        ...
        /* First entry seen */
        out1.println(
            "first VLSN = " + currentVLSN + " at lsn " + DbLsn.getNoFormatString(getLastLsn()));
      } else if (!currentEntryHeader.isInvisible() && !currentVLSN.follows(lastVLSN)) {

        /* Note the first entry, check for a gap. */
        out1.println(
            "gap of "
                + (currentVLSN.getSequence() - lastVLSN.getSequence())
                + " Last="
                + lastVLSN
                + " at lsn "
    ...
    void displayLast() {
      out1.println("LastVLSN = " + lastVLSN + " at " + DbLsn.getNoFormatString(lastLSN));
    }
  }

  /**
   * A Decoder reader can dump the LNs from one of the JE internal databases and display the LNs in
   * a deserialized format, for easy debugging. The target internal databases are the VLSNIndex and
   * the RepGroup. We could add FileSummaryLNs later.
   */
    ...
    DecoderReader(Environment env, PrintStream out, long startLsn, long endLsn, String dbName) {
      ...
      for (LogEntryType entryType : LogEntryType.getAllTypes()) {
        if (entryType.isUserLNType()) {
          targetMap.put(entryType.getTypeNum(), (LNLogEntry<?>) entryType.getNewLogEntry());
        }
      }
    ...
    protected void display() {
      ...
      if (ln.isDeleted()) {
        ...
        if (keyVal.equals(RepGroupDB.GROUP_KEY)) {
          final RepGroupImpl group = new GroupBinding().entryToObject(data);
          ...
          if (nodeBinding == null) {
            throw new IllegalStateException("Node entry before group entry");
          }
