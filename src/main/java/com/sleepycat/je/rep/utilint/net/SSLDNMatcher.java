...
/**
 * This is an implementation of SSLAuthenticator which authenticates based on the Distinguished Name
 * (DN) in the SSL peer's certificate. Matching is done using Java regular expressions against the
 * RFC1779-formatted DN. This is typically used to match against the CN portion of the name.
 */
class SSLDNMatcher {
   ...
   *
   * @param params The instantiation params. The classParams must be a pattern to be matched to a
   *     Distinguished Name in an SSL certificate. The match pattern must be a valid Java regular
   *     expression.
   * @throws IllegalArgumentException if the pattern is not a valid regular expression
   */
