...
/**
 * SSLDataChannel provides SSL-based communications on top of a SocketChannel. We attempt to
 * maintain a degree of compatibility with SocketChannel in terms of request completion semantics.
 * In particular, If in blocking mode: read() will return at least one byte if the buffer has room
 * write() will write the entire buffer If in non-blocking mode: read() and write are not guaranteed
 * to consume or produce anything.
 */
public class SSLDataChannel extends AbstractDataChannel {
  ...
  /**
   * Lock object for protection of appRecvBuffer, netRecvBuffer and SSLEngine unwrap() operations
   */
  private final ReentrantLock readLock = new ReentrantLock();

  /** Lock object for protection of netXmitBuffer and SSLEngine wrap() operations */
  ...
  /**
   * The String identifying the target host that we are connecting to, if this channel was created
   * in client context.
   */
  private final String targetHost;

  /**
   * Possibly null authenticator object used for checking whether the peer for the negotiated
   * session should be trusted.
   */
  private final SSLAuthenticator authenticator;

  /**
   * Possibly null host verifier object used for checking whether the peer for the negotiated
   * session is correct based on the connection target.
   */
  private final HostnameVerifier hostVerifier;

  /**
   * Set to true when a handshake completes and a non-null authenticator acknowledges the session as
   * trusted.
   */
   ...
   *
   * @param socketChannel a SocketChannel over which SSL communcation will occur. This should
   *     generally be connected, but that is not absolutely required until the first read/write
   *     operation.
   * @param sslEngine an SSLEngine instance that will control the SSL interaction with the peer.
   */
  public SSLDataChannel(
  ...
  /**
   * Is the channel capable of determining peer trust? In this case, we are capable only if the
   * application has configured an SSL authenticator
   *
  ...
  /**
   * Is the channel peer trusted? A channel is trusted if the peer should be treated as
   * authenticated. The meaning of this is context dependent. The channel will only be trusted if
   * the configured peer authenticator says it should be trusted, so the creator of this
   * SSLDataChannel knows what "trusted" means.
   *
   ...
   *
   * @param toFill the data buffer into which data will be read. This buffer is expected to be ready
   *     for a put. It need not be empty.
   * @return the count of bytes read into toFill.
   */
  @Override
  public int read(ByteBuffer toFill) throws IOException {
    return (int) read(new ByteBuffer[] {toFill}, 0, 1);
  }

  @Override
  public long read(ByteBuffer[] toFill) throws IOException {
  ...
  @Override
  public int write(ByteBuffer toSend) throws IOException {
    return (int) write(new ByteBuffer[] {toSend}, 0, 1);
  }

  @Override
  public long write(ByteBuffer[] toSend) throws IOException {
  ...
  /**
   * Attempt to flush any pending writes to the underlying socket buffer. The caller should ensure
   * that it is the only thread accessing the DataChannel in order that the return value be
   * meaningful.
   *
   * @return flush status
   */
  ...
  /** If any data is queued up to be sent in the network transmit buffer, try to push it out. */
   ...
   * @param dsts the destination array of ByteBuffers, each of which is expected to be ready for a
   *     put.
  ...
  /** Repeatedly perform handshake operations while there is still more work to do. */
    ...
    if (engineResult != null) {
        ...
        if (sslEngine.getUseClientMode()) {
          if (hostVerifier != null) {
            ...
            if (peerTrusted) {
              ...
              logger.log(
                  INFO, "SSL host verifier reports that " + "connection target is NOT valid");
      ...
      switch (engineResult.getStatus()) {
        ...
        case BUFFER_OVERFLOW:
           ...
           */
            return (sslEngine.getHandshakeStatus() == HandshakeStatus.NEED_WRAP) && flushCount > 0;
