...
/**
 * Common base class for mirror comparisons. Supports both authenticator and host verifier
 * implementations.
 */
class SSLMirrorMatcher {
   ...
   * @param clientMode set to true if the matcher will be evaluated as a client that has a server as
   *     a peer, or false if it will be evaluated as a server that has received a connection from a
   *     client.
  ...
  /**
   * Attempt to determine the Principal that we take on when connecting in client or server context
   * based on the ReplicationNetworkConfig. If we are unable to determine that principal, return
   * null.
   */
  private Principal determinePrincipal(InstanceContext context, boolean clientMode)
      throws IllegalArgumentException {
    ...
    if (aliasProp == null || aliasProp.isEmpty()) {
      ...
      try {
        if (keyStore.size() < 1) {
          ...
          logger.log(
              INFO,
              "KeyStore has multiple entries but no "
