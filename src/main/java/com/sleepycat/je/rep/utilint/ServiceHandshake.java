...
/**
 * Provides Dispatcher service handshake logic, including identification of the service to be
 * accessed and authentication for access to the service.
 *
 * <p>The service initialization protocol looks like one of these flows. In the flows, the notation
 * byte(X) indicates that X is a 1-byte network field and 2byte(X) indicates that X is a 2-byte
 * network field.
 *
 * <p>If No authentication is configured: Client Dispatcher -> "Service:" + byte(len) + <service
 * name> <- byte(OK|BUSY|UNKNOWN_SERVICE)
 *
 * <p>If authentication is configured: Client Dispatcher -> "Service:" + byte(len) + <service name>
 * <- byte(AUTHENTICATE) -> "Authenticate:" + byte(<len>) + <mechanism list> <- "Mechanism:" +
 * 2byte(<len>) + <server-selected mech name> + ":" + <auth mech params> -> <Mechanism-specific
 * payload> <- byte(OK|BUSY|UNKNOWN_SERVICE|INVALID)
 *
 * <p>The code here is organized into client-side and server-side functions. Client-side code is
 * presumed to operated in blocking mode and server-side code is presumed to operate in non-blocking
 * mode.
 */
public class ServiceHandshake {
  ...
  public static class ServerHandshake {
    ...
    ServerHandshake(
        DataChannel dataChannel, ServiceDispatcher dispatcher, AuthenticationMethod[] authInfo) {
    ...
    /**
     * Callback for underlying init operations upon successful discovery of the requested service
     * name.
     */
    ...
    /**
     * Process the handshake incrementally, as channel is presumed to be in non-blocking mode.
     *
     * <p>This function returns one of the following: InitResult.READ: handshake is not complete;
     * more data needed InitResult.FAIL: handshake was rejected or otherwise failed the underlying
     * data channel has been closed. InitResult.DONE: handshake is complete, and any authentication
     * has been performed. A Response still needs to be sent to the client. Caller must do this
     * after any additional validation that may be required.
     */
    InitResult process() throws IOException {
      ...
      if (currentOp instanceof RequireAuthenticateOp || authInfo == null) {
        /*
  ...
  public static class ClientHandshake {
    ...
    /**
     * Process the entire handshake sequence and report the final response code received from the
     * service dispatcher.
     */
    Response process() throws IOException {
      ...
      final DoAuthenticateOp authOp = new DoAuthenticateOp(this, authInfo);
  ...
  public abstract static class ServerInitOp {
    ...
    /**
     * Incrementally process the operation. The operation may require multiple passes since we are
     * presumed to be in non-blocking mode.
     *
     * <p>This function returns one of the following: InitResult.READ: operation is not complete;
     * more data needed InitResult.FAIL: operation was rejected or otherwise failed the underlying
     * data channel has been closed. InitResult.DONE: operation has completed
     */
    protected abstract InitResult processOp(DataChannel channel) throws IOException;
    ...
    /**
     * Helper function to write the contents of the buffer. We make the simplifying assumption here
     * that we will be able to write as much as we want (within reason).
     */
      ...
      while (buffer.remaining() > 0) {
        ...
        if (count < 0) {
          ...
          initState.logMsg(
              Level.WARNING,
              true, // noteError
              "Premature EOF on channel: " + channel + " write() returned: " + count);
          closeChannel(channel);
          return InitResult.FAIL;
        } else if (count == 0) {
          tryCount++;
  ...
  /**
   * The base class of elemental service initialization protocol operations for the client side. The
   * operation is assumed to be in blocking I/O mode.
   */
  public abstract static class ClientInitOp {
    ...
    /**
     * Process the operation in support of the handshake. This operation may consist of read and/or
     * write operations.
     *
     * <p>This function returns one of the following: InitResult.FAIL: operation was rejected or
     * otherwise failed. The underlying communication channel needs to be closed by the caller.
     * InitResult.DONE: operation has completed. The most recent Response from the server received
     * during this operation is available for inspection. Note that this may return DONE with the
     * response set to a failure response value (e.g. INVALID)
     */
  ...
  /**
   * Operations to communicate the requested service name. This is the initial portion of the
   * message and if we have a connection from a foreign entity, this is most likely to detect an
   * error.
   */
  ...
  /**
   * Server-side: expect client to provide a service name. Expected data format: Literal: "Service:"
   * in ASCII encoding Length: 1 byte Service Name: <Length> bytes in ASCII encoding
   */
  static class ReceiveNameOp extends ServerInitOp {
    ...
    protected InitResult processOp(DataChannel channel) throws IOException {
      ...
      if (buffer.capacity() == INITIAL_BUFFER_SIZE) {
         ...
         */
        final String prefix = StringUtils.fromASCII(buffer.array(), 0, REQUEST_PREFIX.length());
        ...
        final ByteBuffer newBuffer = ByteBuffer.allocate(INITIAL_BUFFER_SIZE + nameLength);
      ...
      initState.setServiceName(request.substring(REQUEST_PREFIX.length() + 1));
      return InitResult.DONE;
    }
  }

  /** Client-side: Send the initial service request to the server and await a Response. */
  static class SendNameOp extends ClientInitOp {
     ...
     *
     * @param serviceName the service that is being requested. The service name must be less than
     *     SERVICE_NAME_LIMIT in size.
    ...
    private static byte[] serviceRequestMessage(String serviceName) {
      ...
      final int length = REQUEST_PREFIX_BYTES.length + 1 + serviceNameBytes.length;
  ...
  /**
   * Server side operation to communicate the need for authentication to the client, negotiate an
   * authentication mechanism and then complete the authentication.
   */
  static class RequireAuthenticateOp extends ServerInitOp {
    ...
    protected InitResult processOp(DataChannel channel) throws IOException {
      ...
      if (expectRequestOp == null) {
        ...
        InitResult writeResult = sendBuffer(channel, response.byteBuffer());
        if (writeResult != InitResult.DONE) {
          return writeResult;
        }
        expectRequestOp = new ExpectAuthRequestOp(initState, authInfo);
      }

      if (authOp == null) {
        /* Waiting for the authentication initiation request */
        final InitResult readResult = expectRequestOp.processOp(channel);
        if (readResult != InitResult.DONE) {
          return readResult;
        }

        AuthenticationMethod selectedAuth = expectRequestOp.getSelectedAuthentication();
        ...
        final String authResponseStr =
            selectedAuth.getMechanismName() + ":" + selectedAuth.getServerParams();
        final byte[] authResponseBytes = StringUtils.toASCII(authResponseStr);
        final int length = 1 + AUTH_MECH_PREFIX_BYTES.length + 2 + authResponseBytes.length;
  ...
  /**
   * Client side operation to request authentication. This is done in response to a message from the
   * server notifying us that authentication is required. The implementation of processOp()
   * aggregates functionality from SendRequestOp, which tells the server what authentication we can
   * provide, and then when we've agreed with the server on how to communicate, another ClientOp is
   * created that provides the actual authentication implementation.
   */
  ...
  /**
   * Server side Authentication request setup. Expect an authentication request that looks like:
   * Authenticate:<len><mechanism list> where len is a 1-byte length and mechanism list is a
   * comma-separated list of available authentication mechanisms. Upon completion, selectedMechanism
   * is non-null if the client supports an authentication mechanism in common with us.
   */
  static class ExpectAuthRequestOp extends ServerInitOp {
    ...
    /**
     * Return the negotiated authentication mechanism. Returns null if no common authentication
     * mechanism found.
     */
    ...
    protected InitResult processOp(DataChannel channel) throws IOException {
      ...
      if (phase == WAIT_FOR_REQUEST) {
        final String prefix = StringUtils.fromASCII(buffer.array(), 0, AUTH_PREFIX.length());
        if (!prefix.equals(AUTH_PREFIX)) {
          initState.logMsg(
              Level.WARNING,
              true, // noteError
              "Malformed authentication request: " + prefix);
      ...
      if (phase == WAIT_FOR_LIST_SIZE) {
        ...
        if (mechListSize < 0) {
          initState.logMsg(
              Level.WARNING,
              true, // noteError
              "Negative mechanism list size received: " + mechListSize);
          sendBuffer(channel, Response.FORMAT_ERROR.byteBuffer());
          closeChannel(channel);
          return InitResult.FAIL;
        }
       ...
       */
      final String mechListStr = StringUtils.fromASCII(buffer.array(), 0, buffer.capacity());
      ...
      if (selectedAuth == null) {
        ...
        initState.logMsg(
            Level.WARNING,
            true, // noteError
            "No acceptable authentication mechanism in list: " + mechListStr);
  ...
  static class SendRequestOp extends ClientInitOp {
    ...
    protected InitResult processOp(IOAdapter ioAdapter) throws IOException {
       ...
       * to use.
       */
      final byte[] mechPrefix = new byte[AUTH_MECH_PREFIX_BYTES.length + 2];
      result = ioAdapter.read(mechPrefix);
      if (result < 0) {
        throw new IOException("EOF reading service authenticate response: " + result);
      }
      if (!arraysEqual(AUTH_MECH_PREFIX_BYTES, mechPrefix, AUTH_MECH_PREFIX_BYTES.length)) {
        throw new IOException(
            "Unexpected service authenticate response: " + encodeBytes(mechPrefix));
      }
      final int mechLen = getShort(mechPrefix, AUTH_MECH_PREFIX_BYTES.length);
      if (mechLen < 0) {
      ...
      if (result < 0) {
        throw new IOException("EOF reading service authenticate mechanism: " + result);
      }
       ...
       */
      final String mechStr = StringUtils.fromASCII(mechBytes, 0, mechBytes.length);
      ...
      AuthenticationMethod selectedAuth = findMatch(new String[] {mechList[0]}, authInfo);
      if (selectedAuth == null) {
       ...
       * mechanism object.
       */
      authOp = selectedAuth.getClientOp(initState, mechList.length > 1 ? mechList[1] : "");
    ...
    /**
     * Builds an authentication request suitable for sending over to a ServiceDispatcher. Looks
     * like: Authenticate:<len><mech list>
     *
     * @return the byte encoding of the authentication request message
     */
    private byte[] serviceAuthenticateMessage() {
      final byte[] mechListBytes = StringUtils.toASCII(mechanisms(authInfo));
      final int length = AUTH_PREFIX_BYTES.length + 1 + mechListBytes.length;
      final ByteBuffer buffer = ByteBuffer.allocate(length);
  ...
  /** Returns a comma-delimited list of authentication mechanism names from the authList argument */
  ...
  /**
   * Find the first Authentication instance whose mechanism name matches one of the entries in
   * mechList.
   */
  ...
  /**
   * NoMatchAuthenticationOp communicates that none of the proposed authentication mechanisms
   * available on the client are available on the dispatcher.
   */
   ...
   * @param len the number of bytes to compare - must be less than or equal to the length of the
   *     shorter of array1, array2
  ...
  private static short getShort(byte[] buf, int off) {
    return (short) (((buf[off] & 0xFF) << 8) + ((buf[off + 1] & 0xFF) << 0));
  }
}
