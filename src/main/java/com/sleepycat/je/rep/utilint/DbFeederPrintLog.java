...
/**
 * @hidden Dumps the log using VLSNReaders. These are special FileReaders which use the vlsnIndex to
 *     optimize their traversal of the log. Most common use for this utility is as a debugging
 *     device for the VLSNReaders, because it mimics the way feeders and syncup search the log.
 */
public class DbFeederPrintLog {
  ...
  private void dump(
      ...
      throws IOException,
          DatabaseException, InterruptedException, ChecksumException {
    ...
    try {
      ...
      if (forward) {
        feederReader = new FeederReader(envImpl, vlsnIndex, startLsn, readBufferSize);
        feederReader.initScan(scanVLSN);
      } else {
        backwardsReader =
            new FeederSyncupReader(
                envImpl, vlsnIndex, startLsn, readBufferSize, scanVLSN, DbLsn.NULL_LSN);
      }
