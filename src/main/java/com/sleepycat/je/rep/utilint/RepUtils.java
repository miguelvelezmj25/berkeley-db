...
public class RepUtils {
  ...
  /** Maps from uppercase ReplicaConsistencyPolicy name to the policy's format. */
  private static final Map<String, ConsistencyPolicyFormat<?>> consistencyPolicyFormats =
  ...
  static {
    ...
    addConsistencyPolicyFormat(
        NoConsistencyRequiredPolicy.NAME, new NoConsistencyRequiredPolicyFormat());
  }
  ...
  /**
   * If not null, called by openSocketChannel with the connect options before opening the socket --
   * for unit testing.
   */
  public static volatile TestHook<ConnectOptions> openSocketChannelHook;

  /**
   * Define a new ConsistencyPolicyFormat. Should only be called outside of this class to add
   * support custom policies for testing. Must be called when the system is quiescent, since the map
   * is unsynchronized.
   *
   * @param name must be the first part of the policy string with a non-letter delimiter following
   *     it, or must be the entire policy string.
   * @param format to register.
   */
  public static void addConsistencyPolicyFormat(
      final String name, final ConsistencyPolicyFormat<?> format) {
    consistencyPolicyFormats.put(name.toUpperCase(java.util.Locale.ENGLISH), format);
  }

  /**
   * ReplicaConsistencyPolicy must be stored as a String for use with ReplicationConfig and
   * je.properties. ConsistencyPolicyFormat is an internal handler that formats and parses the
   * string representation of the policy. Only a fixed number of string-representable policies are
   * supported. Other policies that are not string-representable can only be used in
   * TransactionConfig, not ReplicationConfig. For testing only, we allow defining new custom
   * policies.
   */
  ...
  private static class TimeConsistencyPolicyFormat
    ...
    public TimeConsistencyPolicy stringToPolicy(final String string) {
      /* Format: (<lag>, <timeout>) */
      String args = string.substring(TimeConsistencyPolicy.NAME.length());
  ...
  private static class NoConsistencyRequiredPolicyFormat
    ...
    @Override
    public String policyToString(final NoConsistencyRequiredPolicy policy) {
      return NoConsistencyRequiredPolicy.NAME;
    }

    @Override
    public NoConsistencyRequiredPolicy stringToPolicy(final String string) {
      return NoConsistencyRequiredPolicy.NO_CONSISTENCY;
    }
  }

  /**
   * Converts a policy into a string suitable for use as a property value in a je.properties file or
   * elsewhere.
   *
   * @param policy the policy being converted.
   * @return the formatted string representing the policy.
   * @throws IllegalArgumentException if the specific policy does not have a property value format,
   *     via ReplicationConfig(Properties) ctor and setter.
   ...
   * @throws IllegalArgumentException via ReplicationConfig(Properties) ctor and setter.
   */
  public static ReplicaConsistencyPolicy getReplicaConsistencyPolicy(String propertyValue)
      throws IllegalArgumentException {

    final String upperCasePropertyValue = propertyValue.toUpperCase(java.util.Locale.ENGLISH);
    for (final Map.Entry<String, ConsistencyPolicyFormat<?>> entry :
      ...
      if (upperCasePropertyValue.equals(name)
          || (upperCasePropertyValue.startsWith(name)
              ...
              && !Character.isLetter(upperCasePropertyValue.charAt(name.length())))) {
  ...
  /**
   * Like CountDownLatch, but makes provision in the await for the await, or more specifically the
   * new awaitOrException method to be exited via an exception.
   */
  public static class ExceptionAwareCountDownLatch extends CountDownLatch {
    ...
    /**
     * The method used to free an await, ensuring that it throws an exception at the
     * awaitOrException.
     *
     * @param e the exception to be wrapped in a DatabaseException and thrown.
     */
    public void releaseAwait(Exception e) {
      terminatingException.compareAndSet(null, prepareTerminatingException(e, envImpl));
    ...
    /**
     * Blocks, waiting for the latch to count down to zero, or until an {@code Exception} is
     * provided. The exception is thrown in every thread that is waiting in this method.
     *
     * @see #releaseAwait
     */
    ...
    /**
     * DO NOT use this method. Use awaitOrException instead, so that any outstanding exceptions are
     * thrown.
     */
    @Override
    @Deprecated
  ...
  /**
   * Like {@code LinkedBlockingQueue}, but provides a {@code pollOrException()} method that should
   * be used instead of {@code poll()}, so that callers don't have to treat exception cases
   * specially.
   *
   * @see ExceptionAwareCountDownLatch
   */
  @SuppressWarnings("serial")
  public static class ExceptionAwareBlockingQueue<T> extends LinkedBlockingQueue<T> {
  ...
  /**
   * The terminating exception is wrapped in an EFE if it is not already a DatabaseException (which
   * is unexpected). Also text is added to the message indicating it was thrown by an HA thread,
   * since it will often be re-thrown in an app thread and the stack trace may be confusing.
   */
  private static DatabaseException prepareTerminatingException(
      final Exception e, final EnvironmentImpl envImpl) {
  ...
  /**
   * Ideally we should wrap the exception before rethrowing in a different thread, but this confuses
   * exception handlers that call getCause. So instead we add the per-thread local stack trace to
   * the message.
   */
   ...
   */
  public static SocketChannel openSocketChannel(InetSocketAddress addr, ConnectOptions connectOpts)
      throws IOException {
   ...
   * @return the connected channel
   */
  public static DataChannel openBlockingChannel(
      InetSocketAddress addr, DataChannelFactory factory, ConnectOptions connectOpts)
      throws IOException {
  ...
  public static String writeTimesString(StatGroup stats) {
    ...
    long avgWriteNanos = (nMessagesWritten <= 0) ? 0 : (nWriteNanos / nMessagesWritten);

    return String.format(
        " write time: %, dms Avg write time: %,dus", nWriteNanos / 1000000, avgWriteNanos / 1000);
  }
   ...
   * @param accessPropsFile an abstract File naming a file containing property settings.
   * @return the input properties object, updated with the property settings found in the file.
   * @throws IllegalArgumentException if the accessPropsFile contains invalid settings.
   */
  public static Properties populateNetProps(Properties props, File accessPropsFile) {

    Properties rawProps = new Properties();
    DbConfigManager.applyFileConfig(accessPropsFile, rawProps, true); // forReplication
  ...
  /**
   * A simple debugging utility used to obtain information about the execution environment that's
   * only available through some system utility, like netstat, or jps, etc. It's up to the caller to
   * ensure the availability of the utility and ensure that it's on the search path.
   *
   * @args the arguments to a ProcessBuilder with args[0] being the command and args[1-...] being
   *     its args
   * @return a string denoting the output from the command. Or a string prefixed by the word
   *     EXCEPTION, if an exception was encountered, followed by the exception class name and
   *     exception message.
   */
