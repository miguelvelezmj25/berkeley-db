...
import com.sleepycat.je.rep.net.*;
...
public class SSLChannelFactory implements DataChannelFactory {
  ...
  /**
   * A system property to allow users to specify the correct X509 certificate algorithm name based
   * on the JVMs they are using.
   */
  ...
  /**
   * An SSLContext that will hold all the interesting connection parameter information for session
   * creation in server mode.
   */
  private final SSLContext serverSSLContext;

  /**
   * An SSLContext that will hold all the interesting connection parameter information for session
   * creation in client mode.
   */
  ...
  /** An authenticator object for validating SSL session peers when acting in server mode */
  private final SSLAuthenticator sslAuthenticator;

  /** A host verifier object for validating SSL session peers when acting in client mode */
  ...
  public SSLChannelFactory(InstanceParams params) {
    ...
    baseSSLParameters = filterSSLParameters(constructSSLParameters(params), serverSSLContext);
  ...
  public SSLChannelFactory(
    ...
    this.baseSSLParameters = filterSSLParameters(baseSSLParameters, serverSSLContext);
  ...
  public DataChannel acceptChannel(SocketChannel socketChannel) {
    ...
    final SocketAddress socketAddress = socketChannel.socket().getRemoteSocketAddress();
    String host = null;
    ...
    final SSLEngine engine =
        serverSSLContext.createSSLEngine(host, socketChannel.socket().getPort());
    ...
    return new SSLDataChannel(socketChannel, engine, null, null, sslAuthenticator, logger);
  }

  /**
   * Construct a DataChannel wrapping a new connection to the specified address using the associated
   * connection options.
   */
  @Override
  public DataChannel connect(InetSocketAddress addr, ConnectOptions connectOptions)
    ...
    final SSLEngine engine = clientSSLContext.createSSLEngine(host, addr.getPort());
    engine.setSSLParameters(baseSSLParameters);
    engine.setUseClientMode(true);

    return new SSLDataChannel(socketChannel, engine, host, sslHostVerifier, null, logger);
  }
  ...
  public static boolean isValidHostVerifier(String hvSpec) {
    ...
    if (hvSpec.equals("") || hvSpec.equals("mirror") || hvSpec.equals("hostname")) {
      return true;
    }
   ...
   * @param clientMode set to true if the SSLContext is being created for the client side of an SSL
  ...
  private static SSLContext constructSSLContext(InstanceParams params, boolean clientMode) {
    ...
    if (ksInfo != null) {
      try {
         ...
         */
        String ksAliasProp =
            clientMode ? config.getSSLClientKeyAlias() : config.getSSLServerKeyAlias();
   ...
   *
   * @param ksInfo a KeyStoreInfo referencing the Keystore for which the the key manager list is to
   *     be built.
   * @param ksAlias an optional KeyStore alias. If set, the key manager for X509 certs will always
   *     select the certificate with the specified alias.
   * @param clientMode set to true if this is for the client side of an SSL connection and fals
   *     otherwise.
   */
  private static KeyManager[] buildKeyManagerList(
      KeyStoreInfo ksInfo, String ksAlias, boolean clientMode) {
    ...
    try {
      ...
      throw new IllegalStateException(
          "Unable to find appropriate algorithm for " + "keystore file " + ksInfo.ksFile, nsae);
    } catch (UnrecoverableKeyException uke) {
      throw new IllegalStateException(
          "Unable to recover key from keystore file " + ksInfo.ksFile, uke);
    }
  ...
  private static char[] getKeyStorePassword(InstanceContext context) {
    ...
    if (ksPwSource == null) {
      ksPwSource = constructKSPasswordSource(new InstanceParams(context, null));
    }
  ...
  /** Builds a TrustManager list for the input Truststore for use in creating an SSLContext. */
  ...
  private static KeyStoreInfo readTrustStoreInfo(InstanceContext context) {
    ...
    if (tsProp != null) {
      final KeyStore ts = loadStore(tsProp, null, "truststore", tsTypeProp);
  ...
  /**
   * Filter SSLParameter configuration to respect the supported configuration capabilities of the
   * context.
   */
  private static SSLParameters filterSSLParameters(
      SSLParameters configParams, SSLContext filterContext) throws IllegalArgumentException {
    ...
    if (configCipherSuites != null) {
      ...
      configCipherSuites = filterConfig(configCipherSuites, suppCipherSuites);
      if (configCipherSuites.length == 0) {
        throw new IllegalArgumentException(
            "None of the configured SSL cipher suites are supported " + "by the environment.");
      }
    }
    ...
    if (configProtocols != null) {
      ...
      if (configProtocols.length == 0) {
        throw new IllegalArgumentException(
            "None of the configured SSL protocols are supported " + "by the environment.");
      }
    }

    final SSLParameters newParams = new SSLParameters(configCipherSuites, configProtocols);
  ...
  /**
   * Build an SSLAuthenticator or HostnameVerifier based on property configuration. This method
   * looks up a class of the specified name, then finds a constructor that has a single argument of
   * type InstanceParams and constructs an instance with that constructor, then validates that the
   * instance extends or implements the mustImplement class specified.
   *
   * @param params the parameters for constructing this factory.
   * @param checkerClassName the name of the class to instantiate
   * @param checkerClassParams the value of the configured String params argument
   * @param mustImplement a class denoting a required base class or required implemented interface
   *     of the class whose name is specified by checkerClassName.
  ...
  private static Object constructSSLChecker(
    ...
    return DataChannelFactoryBuilder.constructObject(
        ...
        new CtorArgSpec(new Class<?>[] {InstanceParams.class}, new Object[] {objParams}));
  }

  /** Builds an SSLAuthenticator based on the input configuration referenced by params. */
  private static SSLAuthenticator constructSSLAuthenticator(InstanceParams params)
    ...
    return (SSLAuthenticator)
        constructSSLChecker(
            params, authClassName, authParams, SSLAuthenticator.class, "authenticator");
  }

  /** Builds an SSLAuthenticator of a known type. */
  private static SSLAuthenticator constructStdAuthenticator(InstanceParams params, String authSpec)
      throws IllegalArgumentException {

    authSpec = authSpec.trim();
    if (authSpec.startsWith("dnmatch(") && authSpec.endsWith(")")) {
      /* a DN matching authenticator */
      final String match = authSpec.substring("dnmatch(".length(), authSpec.length() - 1);
  ...
  /** Builds an HostnameVerifier based on the configuration referenced in params. */
  private static HostnameVerifier constructSSLHostVerifier(InstanceParams params)
      throws IllegalArgumentException {
    ...
    return (HostnameVerifier)
        constructSSLChecker(
            params, hvClassName, hvParams, HostnameVerifier.class, "hostname verifier");
  }

  /** Builds a HostnameVerifier of a known type. */
  private static HostnameVerifier constructStdHostVerifier(InstanceParams params, String hvSpec)
      throws IllegalArgumentException {

    hvSpec = hvSpec.trim();
    if (hvSpec.startsWith("dnmatch(") && hvSpec.endsWith(")")) {
      /* a DN matching host verifier */
      final String match = hvSpec.substring("dnmatch(".length(), hvSpec.length() - 1);
   ...
   * @param pwSrcParams a possibly null String that has been configured as an argument to the
   *     class's constructor.
  ...
  private static PasswordSource constructPasswordSource(
    ...
    final InstanceParams objParams = new InstanceParams(params.getContext(), pwSrcParams);

    return (PasswordSource)
        DataChannelFactoryBuilder.constructObject(
            pwdSrcClassName,
            PasswordSource.class,
            "password source",
            /* class(InstanceParams) */
            new CtorArgSpec(new Class<?>[] {InstanceParams.class}, new Object[] {objParams}));
  }

  /** Build a PasswordSource for the keystore based on the configuration referenced by params. */
  private static PasswordSource constructKSPasswordSource(InstanceParams params) {

    final ReplicationSSLConfig config =
        (ReplicationSSLConfig) params.getContext().getRepNetConfig();

    final String pwSrcClassName = config.getSSLKeyStorePasswordClass();

    if (pwSrcClassName == null || pwSrcClassName.equals("")) {
      return null;
    }

    final String pwSrcParams = config.getSSLKeyStorePasswordParams();
   ...
   * @throws IllegalArgumentException if the specified parameters do now allow a store to be
   *     successfully loaded
   */
  private static KeyStore loadStore(
      String storeName, char[] storePassword, String storeFlavor, String storeType)
      throws IllegalArgumentException {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Unable to find a " + storeFlavor + " instance of type " + storeType, kse);
    }
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Unable to locate specified " + storeFlavor + " " + storeName, fnfe);
    }
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Error reading from " + storeFlavor + " file " + storeName, ioe);
    } catch (NoSuchAlgorithmException nsae) {
      throw new IllegalArgumentException(
          "Unable to check " + storeFlavor + " integrity: " + storeName, nsae);
  ...
  /**
   * Gets a proper algorithm name for the X.509 certificate key manager. If users already specify it
   * via setting the system property of "je.ssl.x509AlgoName", use it directly. Otherwise, for IBM
   * J9 VM, the name is "IbmX509". For Hotspot and other JVMs, the name of "SunX509" will be used.
   *
