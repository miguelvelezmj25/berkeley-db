...
/**
 * DbSync is a utility for ensuring that a group of replication nodes have fully caught up on the
 * replication stream. The target use case is testing. If a replication group has crashed abruptly,
 * nodes may have closed without finishing the full replay of the replication stream and the
 * environments might not have the same contents. This makes it impossible to compare the contents
 * of the environments for correctness.
 *
 * <p>DbSync assumes that all nodes are down. The utility is invoked for each node in the group. The
 * node will come up and rejoin the group, causing the whole group to reach the same point in the
 * replication stream. If the node becomes the master, it will issue a shutdown request. Otherwise,
 * a node is a replica, and will wait for the shutdown message to come, and will then close.
 */
public class DbSync {
  ...
  private static final String usageString =
      "usage: "
          + CmdUtil.getJavaCommand(DbSync.class)
          + "\n"
   ...
   * @param timeout The permitted time period, in milliseconds, for the replica to catch up with
   *     master.
   */
  public DbSync(
      String envHome,
    ...
    try {
      repEnv =
          new ReplicatedEnvironment(
              new File(envHome), repConfig, envConfig, null, QuorumPolicy.ALL);
      repEnv.setStateChangeListener(listener);
    } catch (Exception e) {
      System.err.println(
          "Can't successfully initialize " + repConfig.getNodeName() + " because of " + e);
      System.exit(-1);
    }
