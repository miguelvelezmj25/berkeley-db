...
/**
 * NamedChannelWithTimeout permits association of timeouts with a DataChannel. This mechanism is
 * necessary, since the standard mechanism for associating timeouts with sockets using
 * Socket.setSoTimeout is not supported by nio SocketChannels.
 */
public class NamedChannelWithTimeout extends NamedChannel {
  ...
  public NamedChannelWithTimeout(
      RepImpl repImpl,
      Logger logger,
  ...
  /**
   * Method invoked by the time thread to check on the channel on a periodic basis. Note that the
   * time that is passed in is a "pseudo" time that is only meaningful for calculating time
   * differences.
   *
   * @param timeMs the pseudo time
   * @return true if the channel is active, false if it isn't and has been closed
   */
  public boolean isActive(long timeMs) {
    ...
    if (timeElapsed > 10000) {
      LoggerUtils.info(
          logger,
          envImpl,
          "Time to close inactive channel " + getNameIdPair() + ": " + timeElapsed + "ms");
    }
    return false;
  }
}
