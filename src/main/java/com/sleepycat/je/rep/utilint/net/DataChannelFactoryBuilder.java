...
public class DataChannelFactoryBuilder {
  ...
  /**
   * Construct the "default" DataChannelFactory that arises from an empty DataChannelFactory
   * configuration.
   */
  ...
  /**
   * Construct a DataChannelFactory from the specified network configuration. The choice of
   * DataChannelFactory type is determined by the setting of {@link
   * ReplicationNetworkConfig#CHANNEL_TYPE je.rep.channelType}.
   *
   * <p>If set to <code>ssl</code>then the internal SSL implementation is is used. If set to <code>
   * custom</code> then a custom channel factory is constructed based on the setting of {@link
   * ReplicationNetworkConfig#CHANNEL_FACTORY_CLASS je.rep.dataChannelFactoryClass}
   *
   * <p>If set to <code>basic</code> or not set, SimpleChannelFactory is instantiated.
   *
   * @param repNetConfig The configuration to control factory building
   * @return a DataChannelFactory
   * @throws IllegalArgumentException if an invalid configuration property value or combination of
   *     values was specified.
   */
  public static DataChannelFactory construct(ReplicationNetworkConfig repNetConfig)
  ...
  /**
   * Construct a DataChannelFactory from the specified access configuration. The choice of
   * DataChannelFactory type is determined by the setting of {@link
   * ReplicationNetworkConfig#CHANNEL_TYPE je.rep.channelType}.
   *
   * <p>If set to <code>ssl</code>then the internal SSL implementation is is used. If set to <code>
   ...
   *
   * @param repNetConfig The configuration to control factory building
   * @param logContext A null-allowable String that contributes to the logging identifier for the
   *     factory.
   * @return a DataChannelFactory
   * @throws IllegalArgumentException if an invalid configuration property value or combination of
   *     values was specified.
  ...
  public static DataChannelFactory construct(
      ReplicationNetworkConfig repNetConfig, String logContext) throws IllegalArgumentException {
   ...
   *
   * @param repNetConfig The configuration to control factory building
   * @param loggerFactory A null-allowable LoggerFactory for use in channel factory construction
   * @return a DataChannelFactory
   * @throws IllegalArgumentException if an invalid configuration property value or combination of
   *     values was specified.
   */
  public static DataChannelFactory construct(
      ReplicationNetworkConfig repNetConfig, LoggerFactory loggerFactory)
      throws IllegalArgumentException {
    ...
    final InstanceContext context = new InstanceContext(repNetConfig, loggerFactory);
    ...
    final InstanceParams factoryParams = new InstanceParams(context, classParams);
   ...
   *
   * @param factoryClassName the name of the class to instantiate, which must implement
   *     DataChannelFactory
   * @param factoryParams the context and factory arguments
   * @return a newly constructed instance
   * @throws IllegalArgumentException if the arguments are invalid
   */
  private static DataChannelFactory construct(String factoryClassName, InstanceParams factoryParams)
      throws IllegalArgumentException {

    return (DataChannelFactory)
        constructObject(
            factoryClassName,
            DataChannelFactory.class,
            "data channel factory",
            new CtorArgSpec(new Class<?>[] {InstanceParams.class}, new Object[] {factoryParams}));
  }

  /**
   * Instantiates a class based on a configuration specification. This method looks up a class of
   * the specified name, then finds a constructor with an argument list that matches the caller's
   * specification, and constructs an instance using that constructor and validates that the
   ...
   *     of the class whose name is specified by instClassName.
   * @param miDesc a descriptive term for the mustImplement class
   * @param ctorArgSpec specifies the required constructor signature and the values to be passed
   * @return an instance of the specified class
   * @throws IllegalArgumentException if any of the input arguments are invalid
   */
  static Object constructObject(
      String instClassName, Class<?> mustImplement, String miDesc, CtorArgSpec ctorArgSpec)
      throws IllegalArgumentException {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Error resolving " + miDesc + " class " + instClassName, cnfe);
    }
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Unable to find an appropriate constructor for " + miDesc + " class " + instClassName);
    }
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Error instantiating " + miDesc + " class " + instClassName + ".  Not accessible?", iae);
    } catch (IllegalArgumentException iae) {
      /* Wrong arguments - should not be possible here */
      throw new IllegalArgumentException(
          "Error instantiating " + miDesc + " class " + instClassName, iae);
    } catch (InstantiationException ie) {
      /* Class is abstract */
      throw new IllegalArgumentException(
          "Error instantiating " + miDesc + " class " + instClassName + ". Class is abstract?", ie);
  ...
  /**
   * A simple class that captures the proposed formal and actual argument lists to match against
  ...
  /**
   * A simple implementation of LoggerFactory that encapsulates the necessary information to do JE
   * environment-friendly logging without needing to know JE HA internal logging.
   */
  static class ChannelLoggerFactory implements LoggerFactory {
    ...
    /**
     * Creates a LoggerFactory for use in construction of channel objects. The caller should supply
     * either an EnvironmentImpl or a Formatter object.
     *
  ...
  /**
   * A simple implementation of InstanceLogger that encapuslates the necessary information to do JE
   * environment-friendly logging without needing to know JE logging rules.
   */
  static class ChannelInstanceLogger implements InstanceLogger {
    ...
    /**
     * Creates a ChannelInstanceLogger for use in construction of channel objects. The caller should
     * supply either an EnvironmentImpl or a Formatter object.
     *
     * @param envImpl a possibly-null EnvironmentImpl
     * @param formatter a possible null formatter
