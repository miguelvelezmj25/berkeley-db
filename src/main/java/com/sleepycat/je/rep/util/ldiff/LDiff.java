...
/**
 * LDiff provides a mechanism for efficiently comparing two quiescent databases, typically residing
 * on different machines connected by a network. The comparison is done at the logical level rather
 * than the physical level, so that we can compare the contents of replicated databases where the
 * logical contents may be identical, but the physical logs may be very different. If the databases
 * are found to be different, it provides information that would help identify the specific nature
 * of the differences.
 *
 * <p>This class provides the external API used to initiate a comparison.
 *
 * <p>For details, please review the document at:
 *
 * @see <a href="https://sleepycat.oracle.com/trac/wiki/JEDatabaseLogicalDiff">LDiff </a>
 */
public class LDiff {
   ...
   * </pre>
   *     <p>-a - generate an analysis of the differences<br>
   *     -b blockSize - the number of records to compare at one time<br>
   *     -h dbEnvHome - the directory or directories containing environment(s) in which to perform
   *     the ldiff<br>
   *     -m maxErrors - the maximum number of errors to detect before declaring the databases
   *     different and ending the operation.<br>
   *     -s database1,database2 - the databases to ldiff.<br>
   *     -q - be quiet, do not write to stdout
   *     <p>If ldiff-ing a specific database, two database names must be specified. If no database
   *     names are given, two environments must be specified. If two database names and two
   *     environments are specified, the first database is opened in the first environment and the
   *     second database is opened in the second environment.
   */
  ...
  /**
   * A mechanism for efficiently comparing two quiescent environments, one local and one on a remote
   * machine. This method assumes that only basic, unauthenticated communication is in use.
   *
  ...
  public boolean diff(Environment env, InetSocketAddress addr, DataChannelFactory dcFactory)
    ...
    final Protocol protocol =
        new Protocol(new NameIdPair("Ldiff", -1), DbInternal.getNonNullEnvImpl(env));
  ...
  /**
   * A mechanism for efficiently comparing two quiescent databases, one of which resides on a remote
   * machine.
   *
  ...
  private boolean diff(Database db, DataChannel channel)
    ...
    final Protocol protocol =
        new Protocol(
            new NameIdPair("Ldiff", -1), DbInternal.getNonNullEnvImpl(db.getEnvironment()));
  ...
  /**
   * A mechanism for efficiently comparing two quiescent databases, typically residing on different
  ...
  /**
   * Find the block in the bag whose rolling checksum and md5 match the given window. The md5 for
   * the window is computed lazily, since it's more expensive to compute. There is a slight chance
   * that multiple blocks in the bag will match the checksum and the md5 hash, return the first such
   ...
   * @return A block which matches the window's checksum and the window's md5 hash, or null if no
   *     block matches.
   */
  ...
  /**
   * Create a bag of blocks from the records in a given database, using the configuration parameters
   * specified when the LDiff object was created.
   *
  ...
  /**
   * Connect to addr and perform a service handshake. Retry as specified by the config object.
   *
   ...
   *
   * <p>TODO: we start simple, by just using it as a boolean indicator and perhaps a block id for
   * unit test purposes? As as the local processing gains in sophistication will provide block and
   * key (insert, update, delete) granularity identification of differences.
   */
