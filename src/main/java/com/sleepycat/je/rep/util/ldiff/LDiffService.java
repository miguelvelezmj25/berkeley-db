...
/**
 * Implementation of the LDiff service that process requests from the LDiff client. It's the network
 * level interface to the remote processing done as part of the ldiff implementation.
 *
 * <p>Note that the service only processes one request at a time, so as not to overload the node.
 */
public class LDiffService extends ExecutingService {
  ...
  class LDiffServiceRunnable implements Runnable {
    ...
    private void sendDiffArea(Cursor cursor, RemoteDiffRequest request, Protocol protocol)
        throws IOException {
