...
/**
 * This class provides the utility to request the current state of a replica in a JE replication
 * group, see more details in {@link com.sleepycat.je.rep.NodeState}.
 */
public class DbPing {
  ...
  private static final String usageString =
      "Usage: "
          + CmdUtil.getJavaCommand(DbPing.class)
          + "\n"
  ...
  private void parseArgs(String[] argv) {
    ...
    if (nodeName == null || groupName == null || socketAddress == null) {
      printUsage(
          "Node name, group name and the node host port are "
   ...
   *
   * @param repNode a class that implements {@link com.sleepycat.je.rep.ReplicationNode}
   * @param groupName name of the group which the node joins
   * @param socketTimeout timeout value for creating a socket connection with the node
   */
  /*
   * SSL deferred
   ...
   * @param groupName name of the group which the node joins
   * @param socketTimeout timeout value for creating a socket connection with the node
   * @param netPropsFile a File containing replication net property settings. Null is allowed.
   * @throws FileNotFoundException if the netPropsFile does not exist
   * @throws IllegalArgumentException if the netProps file contains invalid settings.
   */
   ...
   * @param groupName name of the group which the node joins
   * @param socketTimeout timeout value for creating a socket connection with the node
   * @param netConfig a replication-net configuration object property settings. Null is allowed.
   * @throws IllegalArgumentException if the netProps contains invalid settings.
   */
  public DbPing(
      ReplicationNode repNode,
      String groupName,
      int socketTimeout,
