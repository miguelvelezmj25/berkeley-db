...
/**
 * A utility to reset the members of a replication group, replacing the group with a new group
 * consisting of a single new member as described by the arguments supplied to the utility.
 *
 * <p>This utility is useful when a copy of an existing replicated environment needs to be used at a
 * different site, with the same data, but with a different initial node that can be used to grow
 * the replication group as usual. The utility can also be used to change the group name associated
 * with the environment.
 *
 * <p>The reset environment has a different identity from the environment before the reset operation
 * although it contains the same application data. To avoid confusion, the reset environment is
 * assigned a new internal unique id. The unique id is checked whenever nodes attempt to communicate
 * with each other and ensure that all nodes in a group are dealing with the same data.
 *
 * <p>The reset process is typically accomplished using the steps outlined below. It's good practice
 * to back up your environment before running any utilities that modify an environment.
 *
 * <ol>
 *   <li>Use {@code DbResetRepGroup} to reset an existing environment. {@code DbResetRepGroup} can
 *       be used as a command line utility, and must be executed locally on the host specified in
 *       the -nodeHostPort argument. The host must also contain the environment directory.
 *       Alternatively, {@code DbResetRepGroup} may be used programmatically through the provided
 *       APIs.
 *   <li>Once reset, the environment can be opened with a {@code ReplicatedEnvironment}, using the
 *       same node configuration as the one that was passed in to the utility. No helper host
 *       configuration is needed. Since the group consists of a single node, it will assume the role
 *       of a Master, so long as it is created as an electable node.
 *   <li>Additional nodes may now be created and can join the group as newly created replicas, as
 *       described in {@code ReplicatedEnvironment}. Since these new nodes are empty, they should be
 *       configured to use the new master as their helper node, and will go through the {@link <a
 *       href="{@docRoot}/../ReplicationGuide/lifecycle.html#lifecycle-nodestartup">replication node
 *       lifecycle</a>} to populate their environment directories. In this case, there will be data
 *       in the converted master that can only be transferred to the replica through a file copy
 *       executed with the help of a {@link com.sleepycat.je.rep.NetworkRestore}
 * </ol>
 *
 * <p>For example:
 *
...
public class DbResetRepGroup {
   ...
   *
   * @param envHome The node's replicated environment directory. The directory must be accessible on
   *     this host.
   * @param groupName The name of the new replication group
   * @param nodeName The node's name
   * @param nodeHostPort The host and port for this node. The utility must be executed on this host.
   */
  ...
  /**
   * Replaces the existing group with the new group having a single new node as described by the
