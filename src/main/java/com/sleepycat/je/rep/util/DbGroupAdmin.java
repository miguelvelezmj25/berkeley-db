...
/**
 * DbGroupAdmin supplies the functionality of the administrative class {@link ReplicationGroupAdmin}
 * in a convenient command line utility. For example, it can be used to display replication group
 * information, or to remove a node from the replication group.
 *
 * <p>Note: This utility does not handle security and authorization. It is left to the user to
 * ensure that the utility is invoked with proper authorization.
 *
 * <p>See {@link DbGroupAdmin#main} for a full description of the command line arguments.
 */
/*
 * SSL deferred
...
public class DbGroupAdmin {
  ...
  private static final String usageString =
      "Usage: "
          + CmdUtil.getJavaCommand(DbGroupAdmin.class)
          + "\n"
  ...
  private void parseArgs(String[] argv) {
      ...
      if (thisArg.equals("-groupName")) {
        ...
        if (argc < nArgs) {
          ...
          if (argc < nArgs) {
            ...
            newPort = Integer.parseInt(hostPort.substring(index + 1));
          } else {
    ...
    if (netPropsName != null) {
      try {
        ...
        printUsage(
            "The net properties file " + netPropsName + " does not exist: " + fnfe.getMessage());
      } catch (IllegalArgumentException iae) {
        printUsage(
            "The net properties file " + netPropsName + " is not valid: " + iae.getMessage());
      }
    }
   ...
   * @param helperSockets set of host and port pairs for group members which can be queried to
   ...
   * @param helperSockets set of host and port pairs for group members which can be queried to
   *     obtain group information.
   * @param netPropsFile a File containing replication net property settings. This parameter is
   *     ignored if null.
   * @throws FileNotFoundException if the netPropsFile does not exist
   * @throws IllegalArgumentException if the netPropsFile contains invalid settings.
   */
   ...
   * @param helperSockets set of host and port pairs for group members which can be queried to
   *     obtain group information.
   * @param netConfig replication net configuration - null allowable This parameter is ignored if
   *     null.
   * @throws IllegalArgumentException if the netProps contains invalid settings.
   */
  public DbGroupAdmin(
      String groupName, Set<InetSocketAddress> helperSockets, ReplicationNetworkConfig netConfig) {
  ...
  /**
   * Display group information. Lists all members and the group master. Can be used when reviewing
   * the <a
   * href="http://www.oracle.com/technetwork/database/berkeleydb/je-faq-096044.html#HAChecklist">group
   * configuration. </a>
   */
  ...
  /**
   * Remove a node from the replication group. Once removed, a node cannot be added again to the
   ...
   *     <p>Deletes a node from the replication group, which allows the node to be added to the
   *     group again under the same name.
  ...
  /**
   * Update the network address for a specified node. When updating the address of a node, the node
   * cannot be alive. See {@link ReplicationGroupAdmin#updateAddress} for more information.
   *
  ...
  /**
   * Transfers the master role from the current master to one of the electable replicas specified in
   * the argument list.
   *
   * @param nodeList comma-separated list of nodes
   * @param timeout in <a href="../../EnvironmentConfig.html#timeDuration">same form</a> as accepted
   *     by duration config params
   * @see ReplicatedEnvironment#transferMaster
   */
  @SuppressWarnings("hiding")
  public void transferMaster(String nodeList, String timeout) {
    String result =
        groupAdmin.transferMaster(
            parseNodes(nodeList),
