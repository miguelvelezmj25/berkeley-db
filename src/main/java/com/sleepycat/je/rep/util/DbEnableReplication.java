...
/**
 * A utility to convert an existing, non replicated JE environment for replication. This is useful
 * when the user wants to initially prototype and develop a standalone transactional application,
 * and then add replication as a second stage.
 *
 * <p>JE HA environment log files contain types of log records and metadata used only by
 * replication. Non replicated environments are lacking that information and must undergo a one time
 * conversion process to add that metadata and enable replication. The conversion process is one
 * way. Once an environment directory is converted, the rules that govern {@link
 * ReplicatedEnvironment} apply; namely, the directory cannot be opened by a read/write standalone
 * {@link com.sleepycat.je.Environment}. Only a minimum amount of replication metadata is added, and
 * the conversion process is not dependent on the size of the existing directory.
 *
 * <p>The conversion process takes these steps:
 *
 * <ol>
 *   <li>Use {@code DbEnableReplication} to convert an existing environment directory. {@code
 *       DbEnableReplication} can be used as a command line utility, and must be executed locally on
 *       the host which houses the environment directory. Alternatively, {@code DbEnableReplication}
 *       may be used programmatically through the provided APIs.
 *   <li>Once converted, the environment directory may be treated as an existing master node, and
 *       can be opened with a {@code ReplicatedEnvironment}. No helper host configuration is needed.
 *   <li>Additional nodes may be created and can join the group as newly created replicas, as
 *       described in {@code ReplicatedEnvironment}. Since these new nodes are empty, they should be
 *       configured to use the converted master as their helper node, and will go through the {@link
 *       <a href="{@docRoot}/../ReplicationGuide/lifecycle.html#lifecycle-nodestartup">replication
 *       node lifecycle</a>} to populate their environment directories. In this case, there will be
 *       data in the converted master that can only be transferred to the replica through a file
 *       copy executed with the help of a {@link com.sleepycat.je.rep.NetworkRestore}
 * </ol>
 *
 * <p>For example:
 *
...
public class DbEnableReplication {
  ...
  /**
   * Modify the log files in the environment directory to add a modicum of replication required
   * metadata.
   */
  public void convert() {
