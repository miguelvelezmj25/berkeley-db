...
public class Window {
  ...
  /**
   * Roll a window forward by one key. The rolling checksum is adjusted to account for this move
   * past one key/value pair. Note that the md5 hash associated with the window is computed on
   * demand and is not updated here.
   */
  ...
  public void nextWindow() {
    ...
    while ((i < windowSize)
        && (cursor.getNext(key, data, LockMode.DEFAULT) == OperationStatus.SUCCESS)) {
      if (i == 0) {
  ...
  /**
   * Compute the MD5 hash for the window. This is an expensive operation and should be used
   * sparingly.
   *
  ...
  /**
   * The number of records in the window. The size of the window will match the value set during
   * instantiation, until the end of the database is reached.
   *
   ...
   *
   * <p>1. Each byte (Xi in the tech report) is replaced by a 32 bit Adler checksum of the bytes
   * representing the concatenation of the key/value pair.
   *
   * <p>2. The value for M is 64 instead of 32 to reduce the chances of false collisions on the
   * rolling checksum, given our adaptation of the original algorithm to logically use 32 bit bytes.
   */
