...
/**
 * A bag of Blocks used during the LDiff process. Blocks are accessed by their checksum; when
 * checksums collide, blocks are returned in insertion order.
 */
public class BlockBag implements Iterable<Block> {
   ...
   * @return A List of blocks with the given checksum, in insertion order, or null if no matching
   *     blocks were found.
   */
  public List<Block> get(long chksum) {
    ...
    for (Integer indx : indices) {
      int i = indx.intValue();
      if (i >= blockIndex) ret.add(blocks.get(i));
    }
   ...
   *
   * @return A list of all blocks removed, or null if the bag is already empty.
   */
