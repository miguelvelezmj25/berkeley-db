...
/**
 * Object to represent the thread created by Subscription to process messages received from feeder.
 */
class SubscriptionProcessMessageThread extends StoppableThread {
   ...
   * @param logger Logger
   */
  SubscriptionProcessMessageThread(
      RepImpl impl,
  ...
  /**
   * Implement a soft shutdown. The thread will exist after all messages in the queue are consumed
   * and processed.
   *
  ...
  /**
   * Implement thread run() method. Dequeue message from the queue and process it via the callback.
   */
  @Override
  public void run() {
    ...
    try {
      while (true) {
        if (exitRequest == ExitType.IMMEDIATE) {
          ...
          final Object message =
              queue.poll(SubscriptionConfig.QUEUE_POLL_INTERVAL_MS, TimeUnit.MILLISECONDS);
          ...
          if (message == null) {
            ...
            if (message instanceof GroupShutdownException) {
              ...
              logger.info(
                  "Received shutdown message from "
      ...
      logger.warning(
          "input thread receives exception "
              + e.getMessage()
