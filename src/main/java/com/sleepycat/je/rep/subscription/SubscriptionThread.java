...
class SubscriptionThread extends StoppableThread {
  ...
  SubscriptionThread(
    ...
    inputQueue = new ArrayBlockingQueue<>(config.getInputMessageQueueSize());
    outputQueue = new ArrayBlockingQueue<>(config.getOutputMessageQueueSize());
    ...
    try {
      ...
      while (!isShutdown()) {
        try {
          ...
          if (!auxThreadCreated) {
            LoggerUtils.fine(
                logger, envImpl, "Create auxiliary msg processing " + "and output threads");
      ...
      LoggerUtils.warning(
          logger,
          envImpl,
          "Subscription exited due to security check " + "failure: " + ure.getMessage());
  ...
  /** shutdown the subscriber and all auxiliary threads, close channel to the Feeder. */
  void shutdown() {
    ...
    if (messageProcThread != null) {
      try {
        ...
        LoggerUtils.info(logger, envImpl, "message processing thread has shut down.");
      } catch (Exception e) {
    ...
    if (outputThread != null) {
      try {
        ...
        LoggerUtils.info(logger, envImpl, "output thread has shut down.");
    ...
    LoggerUtils.info(
        logger,
        envImpl,
        "queues cleared and channel closed, subscription " + "thread has completely shut down");
  }
   ...
   */
  void offer(Object message) throws InterruptedException, GroupShutdownException {

    RepImpl repImpl = (RepImpl) envImpl;

    while (!isShutdown()
        && /* stop enqueue msg if thread is shutdown */ !inputQueue.offer(
            message, SubscriptionConfig.QUEUE_POLL_INTERVAL_MS, TimeUnit.MILLISECONDS)) {
      /* Offer timed out. */
      if (!messageProcThread.isAlive()) {
        LoggerUtils.info(
            logger, repImpl, "Thread consuming input queue is gone, start" + " shutdown process");
        throw new GroupShutdownException(
            logger, repImpl, config.getFeederHost(), stats.getHighVLSN(), 0);
      } else {
   ...
   *
   * @throws InternalException if unable to connect to source node due to protocol error
   * @throws EnvironmentFailureException if fail to handshake with source, or source does not have
   *     enough log to start streaming
  ...
  private void initializeConnection()
      throws InternalException, EnvironmentFailureException, ConnectionException,
    ...
    try {
      ...
      ReplicaFeederHandshake handshake =
          new ReplicaFeederHandshake(new SubFeederHandshakeConfig(config.getNodeType()));
      ...
      if (protocol.getVersion() < minReqVersion) {
        throw new BinaryProtocol.ProtocolException(
            "HA protocol version ("
                + protocol.getVersion()
                + ") is "
                + "lower than minimal required version ("
                + minReqVersion
                + ")");
      }
      ...
      LoggerUtils.fine(
          logger,
          envImpl,
          "subscription " + config.getSubNodeName() + " sync-up with feeder at vlsn: " + reqVLSN);
      SubscriberFeederSyncup syncup =
          new SubscriberFeederSyncup(
              namedChannel,
              protocol,
      ...
      LoggerUtils.fine(logger, envImpl, "sync-up with feeder done, start vlsn: " + startVLSN);
      ...
      if (!startVLSN.equals(VLSN.NULL_VLSN)) {
        ...
        protocol.read(namedChannel.getChannel(), Protocol.Heartbeat.class);
      ...
      throw new ConnectionException(
          "Unable to connect due to " + e.getMessage() + ",  will retry later.",
          config.getSleepBeforeRetryMs(),
          e);
    } catch (EnvironmentFailureException e) {
      logger.warning("Fail to handshake with feeder: " + e.getMessage());
      throw e;
    } catch (BinaryProtocol.ProtocolException e) {
      final String msg =
          ("Unable to connect to feeder "
              + config.getFeederHost()
  ...
  private boolean createAuxThread() {
    ...
    if (cachedOutputThread != null) {
      ...
      LoggerUtils.fine(
          logger, envImpl, "output thread created for subscription " + config.getSubNodeName());
      /* start thread to consume data in input queue */
      messageProcThread =
          new SubscriptionProcessMessageThread(repImpl, inputQueue, config, stats, logger);
      messageProcThread.start();
      LoggerUtils.fine(
          logger,
          envImpl,
          "message processing thread created for subscription " + config.getSubNodeName());
      return true;
    } else {
      LoggerUtils.info(
          logger,
   ...
   * @throws ReplicationSecurityException if unauthorized to stream from feeder
   */
  private NamedChannel openChannel()
      throws ConnectionException, InternalException, ReplicationSecurityException {
    ...
    try {
      ...
      final DataChannel channel =
          repImpl.getChannelFactory().connect(config.getInetSocketAddress(), connectOpts);

      ServiceDispatcher.doServiceHandshake(
          channel, FeederManager.FEEDER_SERVICE, config.getAuthInfo());
      LoggerUtils.fine(
          logger,
          envImpl,
      ...
      namedChannel =
          new NamedChannelWithTimeout(repImpl, logger, channelTimeoutTask, channel, timeoutMs);
    } catch (IOException cause) {
      /* retry if unable to connect to feeder */
      throw new ConnectionException(
          "Fail to open channel to feeder " + "due to " + cause.getMessage() + ", will retry later",
          config.getSleepBeforeRetryMs(),
          cause);
      ...
      if (cause.getResponse() == ServiceDispatcher.Response.UNKNOWN_SERVICE) {
        throw new ConnectionException(
            "Service exception: " + cause.getMessage() + ", wait longer and will retry " + "later",
            config.getSleepBeforeRetryMs(),
            cause);
      }
      ...
      if (cause.getResponse() == ServiceDispatcher.Response.INVALID) {
        throw new ReplicationSecurityException(
            "Security check failure:" + cause.getMessage(), config.getSubNodeName(), cause);
      }
  ...
  /**
   * Internal loop to dequeue message from channel to the feeder, process shutdown and heartbeat
   * messages, and relay data operations to the input queue to be consumed by input thread.
   *
   * @throws InternalException if error in reading messages from channel or enqueue message into
   *     input queue
   * @throws GroupShutdownException if receive shutdown message from feeder
   * @throws ReplicationSecurityException if output thread exits due to security check failure. In
   *     this case the main subscription need to exit without retry.
   */
  private void loopInternal()
      throws InternalException, GroupShutdownException, ReplicationSecurityException {
      ...
      while (!isShutdown()) {
        ...
        if ((message == null)) {
          LoggerUtils.info(
              logger,
              envImpl,
              "Subscription " + config.getSubNodeName() + " has nothing stream, exit loop.");
          return;
        }
        ...
        if (messageOp == Protocol.HEARTBEAT) {
          LoggerUtils.finest(
              logger, envImpl, "receive heartbeat from " + namedChannel.getNameIdPair());
          queueAck(ReplicaOutputThread.HEARTBEAT_ACK);
           ...
           */
          Protocol.ShutdownRequest req = (Protocol.ShutdownRequest) message;
          Exception exp =
              new GroupShutdownException(
                  logger,
                  repImpl,
                  config.getFeederHost(),
                  stats.getHighVLSN(),
          ...
          if (pending > stats.getMaxPendingInput().get()) {
            ...
            LoggerUtils.finest(logger, envImpl, "Max pending request log items:" + pending);
          }
        }
      }
  ...
  /**
   * Checks status of output thread and propagates RSE to main loop. If output thread exited due to
   * RSE, the main thread need to capture it to set the subscription status correctly. For other
   * exceptions, output thread uses the traditional mechanism to notify the main subscription
   * thread: simply shut down channel.
   */
  private void checkOutputThread() throws InternalException, ReplicationSecurityException {
    ...
    if (cachedOutputThread == null) {
       ...
       */
      LoggerUtils.fine(logger, envImpl, "output thread no longer exists");
      return;
    }
    ...
    if (cachedOutputThread.getException() instanceof ReplicationSecurityException) {
      ...
      LoggerUtils.warning(
          logger,
          envImpl,
          "Output thread exited due to security check " + "failure: " + rse.getMessage());
      throw rse;
    }
  }
  ...
  private class SubFeederHandshakeConfig implements ReplicaFeederHandshakeConfig {
    ...
    public RepGroupImpl getGroup() {
      ...
      RepGroupImpl repGroupImpl =
          new RepGroupImpl(
              config.getGroupName(), true, /* unknown group uuid */ repImpl.getCurrentJEVersion());
  ...
  /**
   * Thrown to indicate that the Subscriber must retry connecting to the same master, after some
   * period of time.
   */
  private class ConnectionException extends RuntimeException {
    ...
    public String getMessage() {
      return "Failed to connect, will retry after sleeping " + retrySleepMs + " ms";
    }
  }

  /** Handle exceptions uncaught in SubscriptionThread */
  private class SubscriptionThreadExceptionHandler implements UncaughtExceptionHandler {
