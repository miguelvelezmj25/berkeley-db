...
public class SubscriptionConfig implements Cloneable {
   ...
   */
  private static final int DEFAULT_MIN_PROTOCOL_VERSION = BaseProtocol.VERSION_6;
   ...
   *
   * <p>TODO: at the time of writing, only token authentication is supported. In future we may
   * support multiple authentication methods for Subscription client.
   */
  ...
  public SubscriptionConfig(
    ...
    streamMode = EntryRequestType.DEFAULT;
  ...
  ReplicationConfig createReplicationConfig() {
    ...
    ReplicationConfig repConfig =
        new ReplicationConfig(getGroupName(), getSubNodeName(), getSubNodeHostPort());
    ...
    repConfig.setConfigParam(
        RepParams.REPLICA_MESSAGE_QUEUE_SIZE.getName(), Integer.toString(getDefaultMsgQueueSize()));

    repConfig.setConfigParam(
        RepParams.REPLICA_TIMEOUT.getName(),
            getChannelTimeout(TimeUnit.MILLISECONDS) + " ms");

    repConfig.setConfigParam(
        RepParams.PRE_HEARTBEAT_TIMEOUT.getName(),
            getPreHeartbeatTimeout(TimeUnit.MILLISECONDS) + " ms");

    repConfig.setConfigParam(
        RepParams.REPSTREAM_OPEN_TIMEOUT.getName(),
            getStreamOpenTimeout(TimeUnit.MILLISECONDS) + " ms");

    repConfig.setConfigParam(
        RepParams.HEARTBEAT_INTERVAL.getName(), Integer.toString(getHeartbeatIntervalMs()));

    repConfig.setConfigParam(
        RepParams.REPLICA_RECEIVE_BUFFER_SIZE.getName(), Integer.toString(getReceiveBufferSize()));
  ...
  public long getSleepBeforeRetryMs() {
    return DbConfigManager.getDurationVal(
        props, RepParams.SUBSCRIPTION_SLEEP_BEFORE_RETRY, TimeUnit.MILLISECONDS);
  }
  ...
  public long getPollIntervalMs() {
    return DbConfigManager.getDurationVal(
        props, RepParams.SUBSCRIPTION_POLL_INTERVAL, TimeUnit.MILLISECONDS);
  }

  public long getPollTimeoutMs() {
    return DbConfigManager.getDurationVal(
        props, RepParams.SUBSCRIPTION_POLL_TIMEOUT, TimeUnit.MILLISECONDS);
  }
  ...
  public void setMinProtocolVersion(int ver) {
    if (ver < DEFAULT_MIN_PROTOCOL_VERSION) {
      throw new IllegalArgumentException(
          "Minimal HA protocol version cannot be lower than " + DEFAULT_MIN_PROTOCOL_VERSION);
    }
