...
public class ClientAuthMethod implements ServiceHandshake.AuthenticationMethod {
  ...
  static class ClientTokenOp extends ServiceHandshake.ClientInitOp {
    ...
    @Override
    public ServiceHandshake.InitResult processOp(ServiceHandshake.IOAdapter ioAdapter)
        throws IOException {
      ...
      final ServiceDispatcher.Response response = ServiceDispatcher.Response.get(responseByte[0]);
      if (response == null) {
  ...
  /**
   * Server side authentication, effectively no-op except rejecting handshake and it is not supposed
   * to be called at server-side.
   */
