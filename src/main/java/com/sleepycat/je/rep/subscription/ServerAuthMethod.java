...
public class ServerAuthMethod implements ServiceHandshake.AuthenticationMethod {
  ...
  static class ServerTokenOp extends ServiceHandshake.ServerInitOp {
    ...
    private final ByteBuffer tokenSzBuf = ByteBuffer.allocate(BUFFER_TOKEN_SIZE);
  ...
  /**
   * Client side authentication, effectively no-op except rejecting handshake and it is not supposed
   * to be called at client-side.
   */
