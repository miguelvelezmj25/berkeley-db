...
/** Object of the output thread created by subscription to respond the heartbeat ping from feeder */
class SubscriptionOutputThread extends ReplicaOutputThreadBase {
  ...
  SubscriptionOutputThread(
      SubscriptionThread parentThread,
      RepImpl repImpl,
  ...
  /**
   * Implements the reauthentication response for output thread. It sends token to server which
   * would conduct security check for the subscriber with the new token.
   *
   * @throws ReplicationSecurityException if fail to obtain a new login token by renewal or
   *     reauthentication;
  ...
  public void writeReauthentication() throws ReplicationSecurityException, IOException {
    ...
    if (authenticator != null && authenticator.hasNewToken()) {
      ...
      Protocol.ReAuthenticate response = protocol.new ReAuthenticate(authenticator.getToken());
  ...
  public void writeHeartbeat(Long txnId) throws IOException {
    ...
    HeartbeatResponse response =
        protocol.new HeartbeatResponse(VLSN.NULL_VLSN, stats.getHighVLSN());
