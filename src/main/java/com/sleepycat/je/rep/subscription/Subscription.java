...
/**
 * Object to represent a subscription to receive and process replication streams from Feeder. It
 * defines the public subscription APIs which can be called by clients.
 */
public class Subscription {
  ...
  /**
   * Start subscription main thread, subscribe from the very first VLSN from the feeder. The
   * subscriber will stay alive and consume all entries until it shuts down.
   *
   * @throws InsufficientLogException if feeder is unable to stream from start VLSN
   ...
   */
  public void start()
      throws IllegalArgumentException, InsufficientLogException, GroupShutdownException,
  ...
  /**
   * Start subscription main thread, subscribe from a specific VLSN from the feeder. The subscriber
   * will stay alive and consume all entries until it shuts down.
   *
   * @param vlsn the start VLSN of subscription. It cannot be NULL_VLSN otherwise an
  ...
  public void start(VLSN vlsn)
    ...
    subscriptionThread =
        new SubscriptionThread(dummyRepEnv, vlsn, configuration, statistics, logger);
    ...
    if (!waitForSubscriptionInitDone(subscriptionThread)) {
      ...
      throw new TimeoutException(
          "Subscription initialization timeout "
              + "after "
    ...
    switch (subscriptionThread.getStatus()) {
      ...
      default:
        ...
        throw new InternalException(
            "internal exception from " + "subscription thread, err:" + exp.getMessage(), exp);
    }
  }
  ...
  public void shutdown() {
    ...
    if (dummyRepEnv != null) {
      ...
      if (nodeType.hasTransientId() && !dummyRepEnv.isClosed()) {
        RepInternal.getNonNullRepImpl(dummyRepEnv).getNameIdPair().revertToNull();
      }
      dummyRepEnv.close();
      logger.fine(
          "Closed env "
              + dummyRepEnv.getNodeName()
              + "(forget transient id? "
  ...
  /**
   * Create a dummy replicated env used by subscription. The dummy env will be used in the
   * SubscriptionThread, SubscriptionProcessMessageThread and SubscriptionOutputThread to connect to
   * feeder.
   *
   ...
   * @throws IllegalArgumentException if env directory does not exist
   */
  private static ReplicatedEnvironment createDummyRepEnv(SubscriptionConfig conf, Logger logger)
      throws IllegalArgumentException {
    ...
    if (!envHome.exists()) {
      throw new IllegalArgumentException(
          "Env directory " + envHome.getAbsolutePath() + " does not exist.");
    }

    ret =
        RepInternal.createInternalEnvHandle(
            envHome, conf.createReplicationConfig(), conf.createEnvConfig());

    /*
     * A safety check and clear id if necessary, to prevent env with
     * existing id from failing the subscription
     */
    final NameIdPair pair = RepInternal.getNonNullRepImpl(ret).getNameIdPair();
    if (conf.getNodeType().hasTransientId() && !pair.hasNullId()) {
      logger.fine("Env has a non-null id, clear its id(name id: " + pair + ")");
      pair.revertToNull();
    }
