...
/**
 * @hidden An interface that associates a delegate socketChannel for network I/O, which provides
 *     ByteChannel, GatheringByteChannel, and ScatteringByteChannel, interfaces for callers.
 */
public interface DataChannel extends ByteChannel, GatheringByteChannel, ScatteringByteChannel {

  /**
   * Accessor for the underlying SocketChannel. Callers may used the returned SocketChannel in order
   * to query/modify connections attributes, but may not directly close, read from or write to the
   * SocketChannel.
   *
   ...
   *
   * @return true if the data channel implementation has the capability to determine trust.
   */
