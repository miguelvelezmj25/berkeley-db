...
public class RepStatManager extends StatManager {
  ...
  private final UpdateMinMax updateRepMinMaxStat =
      new UpdateMinMax(StatCaptureRepDefinitions.minStats, StatCaptureRepDefinitions.maxStats);
  ...
  public synchronized ReplicatedEnvironmentStats getRepStats(
      StatsConfig config, Integer contextKey) {
      ...
      for (StatContext context : statContextMap.values()) {
        if (context == sc) {
          ...
          if (context.getRepBase() == null) {
            ...
            // reset base
            context.setRepBase(
                computeRepIntervalStats(context.getRepBase(), cur).getStatGroupsMap());
          }
        }
      }
    }
