...
/**
 * @hidden SSL deferred This is the root class for specifying the parameters that control
 *     replication network communication within a a replicated environment. The parameters contained
 *     here are immutable.
 ...
 *     <p>ReplicationNetworkConfig follows precedence rules similar to those of {@link
 *     EnvironmentConfig}.
 *     <ol>
 *       <li>Configuration parameters specified in {@literal <environmentHome>/je.properties} take
 *           first precedence.
 *       <li>Configuration parameters set in the ReplicationNetworkConfig object used at {@code
...
public abstract class ReplicationNetworkConfig implements Cloneable, Serializable {
  ...
  /**
   * Configures the type of communication channel to use. This property is not directly setable. It
   * can be specified in a property file or property set passed to a create() method, or a direct
   * instantiation of a class derived from this class may be used. When set through one of the
   * create() methods or when read through the getChannelType() method, the valid values for this
   * parameter are:
   *
   * <ul>
   *   <li><code>basic</code>
   ...
   * </ul>
   *
   * <code>basic</code> is the standard implementation, which uses ordinary, unencrypted
   * communication, and is represented by this the {@link ReplicationBasicConfig} class.
   *
   * <p><code>ssl</code> indicates that SSL is to be used for service communication. When using SSL,
   * an instance of {@link ReplicationSSLConfig} must be used.
   *
   * <p>
   *
   * <table border="1">
   ...
   * </table>
   */
  public static final String CHANNEL_TYPE = EnvironmentParams.REP_PARAM_PREFIX + "channelType";

  /**
   * @hidden A string identifying a class to instantiate as the data channel factory. Typical
   *     product use does not require this configuration parameter, but this allows a custom data
   *     channel factory to be supplied. If supplied, it must be a fully qualified Java class name
   *     for a class that implements the {@link DataChannelFactory} interface and provides a public
   *     constructor with an argument list of the form ( {@link InstanceParams} )
   *     <p>Note: Setting this class instantiated from this parameter must be of the same
   *     configuration type as indicated by channelType().
   *     <p>
   *     <table border="1">
   * <tr><td>Name</td><td>Type</td><td>Mutable</td><td>Default</td></tr>
   * <tr>
  ...
  /**
   * @hidden A string providing factory-specific data channel configuration parameters. The encoding
   *     of parameters within the string is determined by the specific factory class implementation.
   *     As examples, it may choose to join multiple strings with a delimiter character or may allow
   *     binary data to be hex-encoded.
   *     <p>Note: Setting this parameter is ignored unless {@link #CHANNEL_TYPE
   *     je.rep.channelFactoryClass} is set.
   *     <p>
   *     <table border="1">
   * <tr><td>Name</td><td>Type</td><td>Mutable</td><td>Default</td></tr>
   * <tr>
  ...
  /**
   * A string providing a logging context identification string. This string is incorporated into
   * log messages in order to help associate messages with the configuration context.
   *
   * <p>
   *
   * <table border="1">
  ...
  /**
   * Creates an ReplicationNetworkConfig which includes the properties specified in the named
   * properties file.
   *
   * @param propFile a File from which the configuration properties will be read.
   ...
   *
   * @param properties a set of properties which which to initialize the instance properties
   */
  ...
  /**
   * @hidden Returns name of the DataChannel factory class to be used for creating new DataChannel
   *     instances
   * @return the DataChannelFactory class name, if configured
   */
  public String getChannelFactoryClass() {

    return DbConfigManager.getVal(props, RepParams.CHANNEL_FACTORY_CLASS);
  }

  /**
   * @hidden Sets the name of the DataChannelFactory class to be instantiated for creation of new
  ...
  public void setChannelFactoryClassVoid(String factoryClass) {
    ...
    DbConfigManager.setVal(props, RepParams.CHANNEL_FACTORY_CLASS, factoryClass, validateParams);
  }
  ...
  /**
   * @hidden Sets the DataChannelFactory parameters to be passed when instantiating the
   *     DataChannelFactoryClass.
   * @param factoryParams a string encoding any parameters to be passed to the class constructor.
   * @return this
   */
   ...
   * @return this
   * @throws IllegalArgumentException if the paramName or value is invalid, or if paramName is not a
   *     parameter that applies to ReplicationNetworkConfig.
   */
  public ReplicationNetworkConfig setConfigParam(String paramName, String value)
      throws IllegalArgumentException {
   ...
   *     access that are specific to this class.
   */
  ...
  /**
   * @hidden Ensure that the parameters for this and all known derived classes are registered. This
   *     is called by testing code to ensure that parameter registration happens when non-standard
   *     API access is used.
   */
  ...
  /**
   * @hidden Apply the configurations specified in sourceProps to our properties.
   * @throws IllegalArgumentException if any of the contained property entries have invalid names or
   *     invalid values
   */
  void applyRepNetProperties(Properties sourceProps) throws IllegalArgumentException {
    ...
    for (Map.Entry<Object, Object> propPair : sourceProps.entrySet()) {
      ...
      setConfigParam(props, name, value, true /* validateParams */);
    }
  }

  /**
   * @hidden Apply the configurations specified in sourceProps to updateProps, effectively copying
   *     the replication service access properties from the sourceProps hash. Only the source
   *     properties that are applicable to a ReplicationNetworkConfig are used.
   * @throws IllegalArgumentException if any of the contained property entries have invalid values
   */
  public static void applyRepNetProperties(Properties sourceProps, Properties updateProps)
      throws IllegalArgumentException {
    ...
    for (Map.Entry<Object, Object> propPair : sourceProps.entrySet()) {
      ...
      final String value = (String) propPair.getValue();
      setConfigParam(updateProps, name, value, true /* validateParams */);
    }
  }

  /** @hidden For access by ReplicationConfig and testing only */
  Properties getProps() {
