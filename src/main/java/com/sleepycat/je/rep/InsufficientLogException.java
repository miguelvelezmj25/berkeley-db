...
/**
 * This exception indicates that the log files constituting the Environment are insufficient and
 * cannot be used as the basis for continuing with the replication stream provided by the current
 * master.
 *
 * <p>This exception is typically thrown by the ReplicatedEnvironment constructor when a node has
 * been down for a long period of time and is being started up again. It may also be thrown when a
 * brand new node attempts to become a member of the group and it does not have a sufficiently
 * current set of log files. If the group experiences sustained network connectivity problems, this
 * exception may also be thrown by an active Replica that has been unable to stay in touch with the
 * members of its group for an extended period of time.
 *
 * <p>In the typical case, application handles the exception by invoking {@link
 * NetworkRestore#execute} to obtain the log files it needs from one of the members of the
 * replication group. After the log files are obtained, the node recreates its environment handle
 * and resumes participation as an active member of the group.
 *
 * @see NetworkRestore
 */
public class InsufficientLogException extends RestartRequiredException {
  ...
  /**
   * @hidden Creates an instance of the exception and packages up the information needed by
   *     NetworkRestore.
   */
  ...
  /**
   * @hidden Creates an instance of the exception and packages up the information needed by
   *     Subscription API. The target is not a replication node, so the repImpl field is a shell
   *     which represents the subscription target.
   */
  ...
  /**
   * @hidden Creates an instance of the exception when a LOG_RESTORE_REQUIRED was found at recovery,
   *     and network restore must be initiated before the recovery can succeed. The flow is: 0. A
   *     network restore is underway. It writes a marker file, essentially serializing this ILE as a
   *     property list, and writing it into the log. Something then interrupts this network restore.
   *     The process dies, so all knowledge of the interrupted network restore is lost in memory but
   *     the the marker file acts as a persistent breadcrumb. 1. Since knowledge of the network
   *     restore was lost, the application tries to open and recover the target node. The
   *     LOG_RESTORE_REQUIRED entry is seen, which means that recovery can't continue. 2. Within
   *     recovery, a new ILE is created using information from the LOG_RESTORE_REQUIRED. It's
   *     thrown, which ends recovery. 3. The caller realizes that a network restore has to be
   *     carried out before this environment can be recovered. It uses the ILE instance created in
   *     step 2 to start a new invocation of network restore. 4. Network restore starts. Since
   *     network restore needs a RepImpl, one is instantiated using info from the ILE 5. When
   *     Network restore succeeds, it removes the marker file.
   * @param helperHosts extra helper hosts are derived from those used to open the environment.
   */
  public InsufficientLogException(Properties properties, String helperHosts) {
  ...
  private void initRepImpl() {
     ...
     */
    int numLogProviders = Integer.parseInt(props.getProperty(P_NUMPROVIDERS));
    ...
    ReplicatedEnvironment restoreEnv =
        RepInternal.createInternalEnvHandle(envDir, repConfig, envConfig);
  ...
  /**
   * Returns the members of the replication group that can serve as candidate log providers to
   * supply the logs needed by this node.
   *
  ...
  /**
   * Called when network restore is complete to close the env if it was opened via this exception.
   *
   * <p>Also sets the repImpl field to null, to avoid OOME when opening the restored environment.
   * This is important whether or not the env was opened via this exception. [#26305]
   */
  void releaseRepImpl() {
    if (repImpl == null) {
      return;
