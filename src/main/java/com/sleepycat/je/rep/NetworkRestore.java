...
/**
 * Obtains log files for a Replica from other members of the replication group. A Replica may need
 * to do so if it has been offline for some time, and has fallen behind in its execution of the
 * replication stream.
 *
 * <p>During that time, the connected nodes may have reduced their log files by deleting files after
 * doing log cleaning. When this node rejoins the group, it is possible that the current Master's
 * log files do not go back far enough to adequately {@link <a
 * href="{@docRoot}/../ReplicationGuide/lifecycle.html#lifecycle-nodestartup">sync * up</a>} this
 * node. In that case, the node can use a {@code NetworkRestore} object to copy the log files from
 * one of the nodes in the group. The system tries to avoid deleting log files that either would be
 * needed for replication by current nodes or where replication would be more efficient than network
 * restore.
 *
 * <p>A Replica discovers the need for a NetworkRestore operation when a call to {@code
 * ReplicatedEnvironment()} fails with a {@link InsufficientLogException}.
 *
 * <p>A call to {@code NetworkRestore.execute()} will copy the required log files from a member of
 * the group who owns the files and seems to be the least busy. For example:
 *
 * <pre class=code>
 *  try {
 ...
 * </pre>
 *
 * @see <a href="{@docRoot}/../ReplicationGuide/logfile-restore.html">Restoring Log Files</a>
 */
public class NetworkRestore {
  ...
  /**
   * Creates an instance of NetworkRestore suitable for restoring the logs at this node. After the
   * logs are restored, the node can create a new {@link ReplicatedEnvironment} and join the group
   */
   ...
   *
   * <p>Algorithm ========= If we simply choose the server with the highest maxVSLN, we would always
   * choose the master, which is typically the server with the highest load. If we choose based on
   * load alone, we may choose a lagging replica, and this may result in syncup failing later on the
   * restored server. The compromise solution involves maxLag. We don't select servers less than
   * maxLag behind the master (the server with the highest VLSN range end) to increase the chances
   * of syncup working later, and among the non-lagging servers we choose the lowest load.
   *
   * <p>1. Collect list of servers and get their load/rangeEnd using the first part of the restore
   * protocol. For each server, its load is its number of feeders and rangeEnd is the upper end of
   * its VLSN range. Remove unresponsive servers from the list.
   *
   * <p>2. At the beginning of each round, if the server list is empty, give up.
   *
   * <p>3. Sort list by load. Let minVLSN be max(all rangeEnds) minus maxLag.
   *
   * <p>4. Attempt to perform restore in list order, refreshing each server's load/rangeEnd as we
   * go. Reject any server with a refreshed rangeEnd that is LT minVLSN or a refreshed load that is
   * GT its prior known load. Remove unresponsive servers from the list.
   *
   * <p>5. If the restore is incomplete, goto 2 and do another round.
   *
   * <p>Note that between rounds the load and minVLSN of each server can change, which is why
   * servers are not removed from the list unless they are unresponsive. The idea is to choose the
   * best server based on the information we collected in the last round, but reject servers with
   * new load or rangeEnd values that invalidate the earlier decision, and always get fresh
   * load/rangeEnd values for each server.
   *
   * @param logException the exception packing information driving the restore operation.
   * @param config may contain an explicit list of members.
   * @return the list of candidate Server instances
   * @throws IllegalArgumentException if the configured log providers are invalid
   */
  private List<Server> init(InsufficientLogException logException, NetworkRestoreConfig config)
    ...
    maxLag = repImpl.getConfigManager().getInt(RepParams.NETWORKBACKUP_MAX_LAG);
   ...
   *
   * <p>If <code>config.getLogProviders()</code> returns null, or an empty list, it uses the member
   * that is least busy as the provider of the log files. Otherwise it selects a member from the
   * list, choosing the first member that's available, to provide the log files. If the members in
   * this list are not present in <code>logException.getLogProviders()</code>, it will result in an
   * <code>IllegalArgumentException</code> being thrown. Exceptions handlers for <code>
   * InsufficientLogException</code> will typically use {@link
   * InsufficientLogException#getLogProviders()} as the starting point to compute an appropriate
   * list, with which to set up the <code>config</code> argument.
   *
   * <p>Log files that are currently at the node will be retained if they are part of a consistent
   * set of log files. Obsolete log files are either deleted, or are renamed based on the the
   * configuration of <code>config.getRetainLogFiles()</code>.
   *
   * @param logException the exception thrown by {@code ReplicatedEnvironment()} that necessitated
   *     this log refresh operation
  ...
  public synchronized void execute(
      InsufficientLogException logException, NetworkRestoreConfig config)
        ...
        for (Server server : serverList) {
          ...
          try {
            ...
            LoggerUtils.info(
                ...
                repImpl,
                String.format(
                    "Network restore completed from: %s. " + "Elapsed time: %,d s.",
            ...
            LoggerUtils.warning(
                logger, repImpl, "Backup failed from node: " + server.node + "\n" + e.getMessage());
  ...
  /**
   * @hidden for testing use only
   *     <p>Returns the member that was used to provide the log files.
   */
  ...
  /**
   * @hidden Returns the network backup statistics for the current network restore attempt, or
   *     {@code null} if a network backup is not currently underway.
  ...
  /**
   * A convenience class to help aggregate server attributes that may be relevant to ordering the
   * servers in terms of their suitability.
   */
