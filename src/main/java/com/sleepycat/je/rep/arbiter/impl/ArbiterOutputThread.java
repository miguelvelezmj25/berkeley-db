...
/**
 * The ArbiterOutputThread reads transaction identifiers from the outputQueue and writes a
 * acknowledgment response to to the network channel. Also used to write responses for heart beat
 * messages.
 */
public class ArbiterOutputThread extends ReplicaOutputThreadBase {
  private final ArbiterVLSNTracker vlsnTracker;

  public ArbiterOutputThread(
      RepImpl repImpl,
  ...
  public void writeHeartbeat(Long txnId) throws IOException {
    ...
    protocol.write(protocol.new HeartbeatResponse(VLSN.NULL_VLSN, vlsn), replicaFeederChannel);
  }
