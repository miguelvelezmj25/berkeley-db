...
/**
 * Provides a mechanism to allow write availability for the Replication group even when the number
 * of replication nodes is less than majority. The main use of an Arbiter is when the replication
 * group consists of two nodes. The addition of an Arbiter to the replication group allows for one
 * node to fail and provide write availability with ACK durability of SIMPLE_MAJORITY. The Arbiter
 * acknowledges the transaction, but does not retain a copy of the data. The Arbiter persists a
 * small amount of state to insure that only the Replication nodes that contain the Arbiter
 * acknowledged transactions may become a Master.
 *
 * <p>The Arbiter node participates in elections and may acknowledge transaction commits.
 *
 * <p>The Arbiter state is as follows: UNKNOWN [ UNKNOWN | REPLICA]+ DETACHED
 */
public class Arbiter {

  private final ArbiterImpl ai;
  ...
  /**
   * An Arbiter used in elections and transaction acknowledgments. This method returns when a
   * connection to the current master replication node is made. The Arbiter.shutdown() method is
   ...
   * @throws EnvironmentLockedException when an environment cannot be opened because another Arbiter
   *     has the environment open.
