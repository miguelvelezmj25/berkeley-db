...
public class ArbiterConfig extends ArbiterMutableConfig implements Cloneable {
  ...
  /**
   * Sets the name to be associated with this <code>Arbiter</code>. It must be unique within the
   * group. When the <code>Arbiter</code> is instantiated and joins the replication group, a check
  ...
  /**
   * Sets the hostname and port associated with this arbiter. The hostname and port combination are
  ...
  /**
   * Time to wait for the discovery of the Master during the instantiation of the Arbiter. If no
   * Master is found with in the timeout period, the Arbiter constructor return with the Arbiter in
   * the UNKNOWN state.
   *
   * @param timeout The unknown state timeout. A value of 0 turns off Unknown state timeouts. The
   *     creation of the Arbiter will wait until a Master is found.
  ...
  public ArbiterConfig setUnknownStateTimeout(long timeout, TimeUnit unit)
    ...
    DbConfigManager.setDurationVal(
        props, RepParams.ENV_UNKNOWN_STATE_TIMEOUT, timeout, unit, validateParams);
    return this;
  }
   ...
   *
   * <p>A value of 0 means Unknown state timeouts are not configured.
   *
  ...
  /**
   * @hidden The size of the message queue the Arbiter uses to read messages and used to size the
   *     output message queue for responses to the master.
  ...
  /**
   * @hidden The interval used when checking an inactive connection to the master.
  ...
  /**
   * @hidden The timeout used when waiting for the initial heartbeat when establishing a connection.
  ...
  public ArbiterConfig setPreHeartbeatTimeout(long timeout, TimeUnit unit)
      throws IllegalArgumentException {
    DbConfigManager.setDurationVal(
        props, RepParams.PRE_HEARTBEAT_TIMEOUT, timeout, unit, validateParams);
    return this;
  }
  ...
  /**
   * @hidden The size of the the TCP receive buffer associated with the socket used by the Arbiter
  ...
  /**
   * @hidden The socket timeout value used by an Arbiter when it opens a new connection to establish
   *     a stream with a feeder.
   * @param timeout maximum time to wait
   * @param unit TimeUnit
   * @return this
   * @throws IllegalArgumentException
  ...
  public ArbiterConfig setStreamOpenTimeout(long timeout, TimeUnit unit)
      throws IllegalArgumentException {
    DbConfigManager.setDurationVal(
        props, RepParams.REPSTREAM_OPEN_TIMEOUT, timeout, unit, validateParams);
    return this;
  }
  ...
  /** @hidden Get the replication service net configuration associated with this MonitorConfig. */
  public ReplicationNetworkConfig getRepNetConfig() {
    return repNetConfig;
  }

  /**
   * @hidden Set the replication service net configuration associated with this MonitorConfig.
