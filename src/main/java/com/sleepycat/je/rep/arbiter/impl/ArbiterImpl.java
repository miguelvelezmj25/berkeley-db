...
import com.sleepycat.je.rep.elections.*;
...
/**
 * The implementation of the Arbiter. The Arbiter is a participant in elections and may acknowledge
 * transaction commits.
 *
 * <p>The Arbiter persists the Arbiter's replication group node identifier and the highest commit
 * VLSN that has been acknowledged. Currently the Feeder sends commit acknowledgment requests to the
 * Arbiter if the replication factor is two and the other Rep node is not available
 * (RepImpl.preLogCommitHook).
 *
 * <p>The VLSN is used in response to an election promise request. The priority of the response is
 * lower than a RepNode. This allows the RepNode to be selected if the VLSN is equal. An Arbiter may
 * not "win" an election. If the Arbiter's VLSN is the highest in the election, the election result
 * is ignored. A NULL nodeid in a promise response is used to identify a promise response is from an
 * Arbiter.
 *
 * <p>Two pieces of information are persisted by the Arbiter. The replication group node identifier
 * is persisted because this information is located in the group database and the Arbiter does not
 * have a copy of the group database. The other is the high VLSN of a Arbiter ACKed commit.
 *
 * <p>In the future, the algorithm could be changed to request commit acknowledgments when the
 * replication factor is greater than two. This would allow for better write availability when the
 * replication factor is an even number.
 */
public class ArbiterImpl extends StoppableThread {
  ...
  /**
   * The Arbiter implementation. Uses the following replication parameters: RepParams.GROUP_NAME The
   * replication group name. RepParams.ENV_UNKNOWN_STATE_TIMEOUT Timeout used for being in the
   * unknown state. RepParams.NODE_HOST_PORT The host name and port associated with this node.
   *
  ...
  public StatGroup loadStats(StatsConfig config) {
    ...
    if (arbiterAcker == null) {
      arbStat = new StatGroup(ArbiterStatDefinition.GROUP_NAME, ArbiterStatDefinition.GROUP_DESC);
    } else {
    ...
    StatGroup trackerStats =
        arbiterVLSNTracker == null
            ? ArbiterVLSNTracker.loadEmptyStats()
  ...
  private void initialize() throws IOException {
    ...
    currentState =
        new AtomicReference<ReplicatedEnvironment.State>(ReplicatedEnvironment.State.UNKNOWN);
    ...
    serviceDispatcher = new ServiceDispatcher(getSocket(), repImpl, channelFactory);
    serviceDispatcher.start();

    masterStatus = new MasterStatus(nameIdPair);
    changeListener = new MasterChangeListener();
    File dataFile = new File(arbiterHome.getAbsolutePath() + File.separator + DATA_FILE_NAME);
    ...
    if (arbiterVLSNTracker.getCachedNodeId() != NameIdPair.NULL_NODE_ID) {
      nameIdPair.update(new NameIdPair(nameIdPair.getName(), arbiterVLSNTracker.getCachedNodeId()));
    }
  ...
  public void runArbiter() {
    ...
    elections = new Elections(new ArbElectionsConfig(), changeListener, suggestionGenerator);

    elections.startLearner();
    elections.startAcceptor();

    repGroupAdmin = new ReplicationGroupAdmin(groupName, helperSockets, channelFactory);
    ...
    int timeout = repImpl.getConfigManager().getDuration(RepParams.ENV_UNKNOWN_STATE_TIMEOUT);
    if (timeout == 0) {
    ...
    try {
       ...
       */
      getReadyLatch().awaitOrException(timeout, TimeUnit.MILLISECONDS);
      LoggerUtils.fine(logger, repImpl, "Arbiter started in " + currentState + " state.");
  ...
  public void run() {
    ...
    try {
      ...
      LoggerUtils.fine(logger, repImpl, "Arbiter main thread interrupted - " + " forced shutdown.");
    } catch (GroupShutdownException e) {
      saveShutdownException(e);
      LoggerUtils.fine(logger, repImpl, "Arbiter main thread sees group shutdown - " + e);
    } catch (InsufficientLogException e) {
      saveShutdownException(e);
    } catch (RuntimeException e) {
      LoggerUtils.fine(logger, repImpl, "Arbiter main thread sees runtime ex - " + e);
      saveShutdownException(e);
      throw e;
    } catch (Error e) {
      LoggerUtils.fine(logger, repImpl, e + " incurred during arbiter loop");
      repNodeError = e;
      repImpl.invalidate(e);
    } finally {
      LoggerUtils.info(logger, repImpl, "Arbiter main thread shutting down.");
  ...
  private void utilityServicesStart() {
    ...
    binaryStateService = new ArbBinaryStateService(serviceDispatcher, this);
  }
  ...
  private void utilityServicesShutdown() {
    ...
    if (binaryStateService != null) {
      try {
        ...
        LoggerUtils.info(
            logger, repImpl, "Error shutting down binaryStateService " + e.getMessage());
      }
    }
    ...
    if (nodeStateService != null) {
      try {
        ...
        LoggerUtils.info(logger, repImpl, "Error canceling serviceDispatch " + e.getMessage());
      }
    }
  }
  ...
  public void shutdown() {
    ...
    try {
      ...
      LoggerUtils.info(
          logger, repImpl, "Error shutting down monitor event manager " + e.getMessage());
    }
    ...
    if (arbiterAcker != null) {
      try {
        ...
        LoggerUtils.info(logger, repImpl, "Error shutting down ArbiterAcker " + e.getMessage());
      }
    }
    ...
    if (elections != null) {
      try {
        ...
        LoggerUtils.info(logger, repImpl, "Error shutting down elections " + e.getMessage());
      }
    }
  ...
  /**
   * Communicates with existing nodes in the group in order figure out who is the master. In the
   ...
   *
   * <p>We query the designated helpers for the Master information. The helpers are the ones that
   * were identified via the node's configuration.
   *
  ...
  private boolean checkGroupMasterIsAlive(NameIdPair groupMasterNameId) {
    ...
    try {
      ...
      protocol.write(
          protocol
          .new BinaryNodeStateRequest(
              groupMasterNameId.getName(), repImpl.getConfigManager().get(RepParams.GROUP_NAME)),
          channel);
  ...
  public String getHostName() {
    String hostAndPort = repImpl.getConfigManager().get(RepParams.NODE_HOST_PORT);
  ...
  private class MasterChangeListener implements Learner.Listener {
    ...
    public void notify(Proposal proposal, Value value) {
      ...
      try {
        ...
        LoggerUtils.logMsg(
            logger, formatter, Level.FINE, "Arbiter notified of new Master: " + currentMasterName);
        masterStatus.setGroupMaster(
            currentValue.getHostName(), currentValue.getPort(), currentValue.getNameId());
      } catch (Exception e) {
        LoggerUtils.logMsg(
            logger,
            formatter,
            Level.SEVERE,
            "Arbiter change event processing exception: " + e.getMessage());
      }
    }
  }
  ...
  private class MasterSuggestionGenerator implements Acceptor.SuggestionGenerator {
    @Override
    public Value get(Proposal proposal) {
      return new MasterValue(null, getPort(), NameIdPair.NULL);
    }

    @Override
    public Ranking getRanking(Proposal proposal) {
      return new Ranking(
          arbiterVLSNTracker.getDTVLSN().getSequence(), arbiterVLSNTracker.get().getSequence());
    }
  }
}
