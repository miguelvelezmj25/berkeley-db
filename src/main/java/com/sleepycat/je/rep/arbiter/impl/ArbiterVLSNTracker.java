...
/**
 * This class is used to maintain two pieces of persistent state. The replication group node
 * identifier of the Arbiter and a VLSN value that represents the highest commit record VLSN the
 * Arbiter has acknowledged.
 */
class ArbiterVLSNTracker {
  ...
  ArbiterVLSNTracker(File file) {
    ...
    stats =
        new StatGroup(
            ArbiterStatDefinition.ARBIO_GROUP_NAME, ArbiterStatDefinition.ARBIO_GROUP_DESC);
    ...
    try {
      ...
      throw new RuntimeException(
          "Unable to read the Arbiter data file " + dataFile.getAbsolutePath());
    } catch (Exception e) {
      throw new RuntimeException(
  ...
  private void writeNodeIdInternal(int id) {
    ...
    try {
      ...
      throw new RuntimeException(
          "Unable to write the Arbiter data file " + dataFile.getAbsolutePath());
    }
  }
