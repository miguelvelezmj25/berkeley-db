...
import static com.sleepycat.je.rep.arbiter.impl.ArbiterStatDefinition.*;

/**
 * The ArbiterAcker is used to acknowledge transactions. A feeder connection is established with the
 * current master. Commit and Heartbeat messages are sent by the master. The ArbiterAcker responds
 * and persistently tracks the high VLSN of the commit messages that it acknowledges.
 *
 * <p>There are configuration parameters that are used. RepParams.REPLICA_MESSAGE_QUEUE_SIZE used
 * for the replay queue size and in the computation of the output queue size.
 * RepParams.REPLICA_TIMEOUT used for the Arbiter feeder channel timeout.
 * RepParams.PRE_HEARTBEAT_TIMEOUT used for the Arbiter feeder channel timeout before the first
 * heartbeat is sent. RepParams.REPLICA_RECEIVE_BUFFER_SIZE used for the datachannel buffer size.
 * RepParams.REPSTREAM_OPEN_TIMEOUT used for the datachannel open timeout. RepParams.MAX_CLOCK_DELTA
 * - used for ReplicaFeederHandshake maximum clock delta. RepParams.HEARTBEAT_INTERVAL heartbeat
 * interval in millis. RepParams.ENABLE_GROUP_ACKS enables output thread ack grouping.
 *
 * <p>The main Arbiter thread reads messages from the feeder channel and queues the message on the
 * request queue. The request thread reads entries from the request queue. The request thread may
 * queue an entry on the output queue. The ArbiterOutputThread reads from the output queue and
 * writes to the network channel. read from network -> ArbiterAcker main thread -> requestQueue
 * requestQueue -> RequestThread -> outputQueue outputQueue -> ArbiterOutputThread -> writes to
 * network
 */
class ArbiterAcker {
  ...
  ArbiterAcker(ArbiterImpl arbiterImpl, RepImpl repImpl) {
    ...
    final int requestQueueSize =
        repImpl.getConfigManager().getInt(RepParams.REPLICA_MESSAGE_QUEUE_SIZE);
    ...
    final int outputQueueSize =
        2 * repImpl.getConfigManager().getInt(RepParams.REPLICA_MESSAGE_QUEUE_SIZE);
    outputQueue = new ArrayBlockingQueue<Long>(outputQueueSize);

    stats = new StatGroup(ArbiterStatDefinition.GROUP_NAME, ArbiterStatDefinition.GROUP_DESC);
    nReplayQueueOverflow = new LongStat(stats, ARB_N_REPLAY_QUEUE_OVERFLOW);
  ...
  private void initializeConnection() throws ConnectRetryException, IOException {
    ...
    protocol.read(arbiterFeederChannel.getChannel(), Protocol.Heartbeat.class);
  ...
  /**
   * The core Arbiter control loop. The loop exits when it encounters one of the following possible
   * conditions:
   *
   ...
   *
   * <p>3) The loop is interrupted, which is interpreted as a request to shutdown the Arbiter node
   * as a whole.
   *
   ...
   *
   * <p>Normal exit from this run loop results in the Arbiter node retrying finding the group
   * master. A thrown exception, on the other hand, results in the Arbiter node as a whole
   * terminating its operation and no longer participating in the replication group, that is, it
  ...
  void shutdown() {
    if (requestThread != null) {
      try {
        ...
        LoggerUtils.info(logger, repImpl, "Request thread error shutting down." + e);
      }
    }
  ...
  private void runArbiterAckLoopInternal() throws InterruptedException, RetryException {
    ...
    LoggerUtils.info(
        logger,
        repImpl,
        "Arbiter loop started with master: " + arbiterImpl.getMasterStatus().getNodeMasterNameId());
    try {
      ...
      if (arbiterImpl.isShutdown()) {
        LoggerUtils.info(logger, repImpl, "Arbiter loop interrupted for shutdown.");
        return;
      }
      LoggerUtils.warning(logger, repImpl, "Arbiter loop unexpected interrupt.");
      ...
      LoggerUtils.severe(
          logger,
          repImpl,
          "Arbiter unexpected exception " + e + " " + LoggerUtils.getStackTrace(e));
      throw e;
  ...
  protected void doRunArbiterLoopInternalWork() throws Exception {
    ...
    final int timeoutMs = repImpl.getConfigManager().getDuration(RepParams.REPLICA_TIMEOUT);
    ...
    try {
      while (true) {
        ...
        while (!requestQueue.offer(message, QUEUE_POLL_INTERVAL_NS, TimeUnit.NANOSECONDS)) {
          /* Offer timed out. */
        ...
        if (pending > maxPending) {
          maxPending = pending;
          LoggerUtils.fine(logger, repImpl, "Max pending request log items:" + maxPending);
        }
      }
  ...
  StatGroup loadStats(StatsConfig config) throws DatabaseException {
    masterStat.set(arbiterImpl.getMasterStatus().getNodeMasterNameId().toString());
  ...
  private void loopExitCleanup() {
    ...
    if (shutdownException != null) {
      if (shutdownException instanceof RetryException) {
        ...
        LoggerUtils.info(
            logger, repImpl, "Exiting inner Arbiter loop." + " Master requested shutdown.");
      } else {
        LoggerUtils.warning(
            logger,
            repImpl,
  ...
  /**
   * Returns a channel used by the Arbiter to connect to the Feeder. The socket is configured with a
  ...
  private void createArbiterFeederChannel() throws IOException, ConnectRetryException {
    ...
    try {
      ...
      dataChannel =
          repImpl
              .getChannelFactory()
              .connect(arbiterImpl.getMasterStatus().getNodeMaster(), connectOpts);

      arbiterFeederChannel =
          new NamedChannelWithTimeout(
              repImpl, logger, arbiterImpl.getChannelTimeoutTask(), dataChannel, timeoutMs);
  ...
  private GroupShutdownException processShutdown(ShutdownRequest shutdown) throws IOException {
    ...
    final String masterHostName = arbiterImpl.getMasterStatus().getGroupMaster().getHostName();
  ...
  private Message replayEntries(Message firstMessage) throws IOException {
    ...
    for (int i = 0; i < groupMessages.size(); i++) {
      ...
      if (messageOp == Protocol.SHUTDOWN_REQUEST) {
        ...
        InputWireRecord wireRecord = ((Protocol.Entry) message).getWireRecord();
        ...
        if (LOG_TXN_COMMIT.equalsType(entryType)) {
          ...
          if (commitEntry.getReplicaSyncPolicy() == SyncPolicy.SYNC) {
            doSync = true;
          }
          ...
          final TxnCommit masterCommit = (TxnCommit) logEntry.getMainItem();
          ...
          if (logger.isLoggable(Level.FINEST)) {
            LoggerUtils.finest(logger, repImpl, "Arbiter ack commit record " + wireRecord);
          }
        } else {
    ...
    if (doSync || (lastFSyncTime + FSYNC_INTERVAL) <= System.currentTimeMillis()) {
      doSync = true;
      lastFSyncTime = System.currentTimeMillis();
    }

    arbiterImpl.getArbiterVLSNTracker().write(new VLSN(highVLSN), new VLSN(dtvlsn), doSync);
  ...
  class RequestThread extends StoppableThread {
    ...
    public void run() {
      ...
      LoggerUtils.fine(
          logger,
          repImpl,
          "Request thread started. Message queue size:" + requestQueue.remainingCapacity());
      try {
        while (true) {
          final Message message =
              requestQueue.poll(REQUEST_QUEUE_POLL_INTERVAL_NS, TimeUnit.NANOSECONDS);

          if ((exitRequest == RequestExitType.IMMEDIATE)
              || ((exitRequest == RequestExitType.SOFT) && (message == null))
          ...
          if (shutdownMessage != null) {
            throw processShutdown((ShutdownRequest) shutdownMessage);
          }
        }
      } catch (Exception e) {
        exception = e;
         ...
         */
        LoggerUtils.fine(logger, repImpl, "closing arbiterFeederChannel = " + arbiterFeederChannel);
        RepUtils.shutdownChannel(arbiterFeederChannel);

        LoggerUtils.info(
            logger, repImpl, "ArbiterAcker thread exiting with exception:" + e.getMessage());
      }
    }
