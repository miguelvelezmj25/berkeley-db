...
/**
 * This class is used to encapsulate all access to the rep group data that is present in every
 * replicated JE environment. The rep group data exists primarily to support dynamic group
 * membership. Both read and update access must be done through the APIs provided by this class.
 *
 * <p>The database is simply a representation of the RepGroup. Each entry in the database represents
 * a node in RepGroup; the key is the String node name, and the data is the serialized
 * ReplicationNode. There is a special entry keyed by GROUP_KEY that holds the contents of the
 * RepGroup (excluding the nodes) itself.
 *
 * <p>The database may be modified concurrently by multiple transactions as a master processes
 * requests to update it. It may also be accessed by multiple overlapping transactions as a Replica
 * replays the rep stream. These updates need to be interleaved with operations like getGroup() that
 * create copies of the RepGroup instance. To avoid deadlocks, entries in the database are accessed
 * in order of ascending key. GROUP_KEY in particular is associated with the lowest key value so
 * that it's locked first implicitly as part of any iteration and any other modifications to the
 * database must first lock it before making changes to the group itself.
 *
 * <p>An instance of this class is created as part of a replication node and is retained for the
 * entire lifetime of that node.
 */
public class RepGroupDB {
  ...
  public static final long DB_ID = DbTree.NEG_DB_ID_START - 1;
  ...
  private static final Durability QUORUM_ACK_DURABILITY =
      new Durability(SyncPolicy.SYNC, SyncPolicy.SYNC, ReplicaAckPolicy.SIMPLE_MAJORITY);

  private static final TransactionConfig QUORUM_ACK = new TransactionConfig();

  private static final TransactionConfig NO_ACK = new TransactionConfig();

  /*
   * TODO: Change this when we support true read only transactions.
   */
  static final TransactionConfig READ_ONLY = NO_ACK;
  ...
  /**
   * Create an instance. Note that the database handle is not initialized at this time, since the
   * state of the node master/replica is not known at the time the replication node (and
   * consequently this instance) is created.
   *
  ...
  public RepGroupDB(RepImpl repImpl) throws DatabaseException {
    ...
    emptyGroup = new RepGroupImpl(configManager.get(GROUP_NAME), repImpl.getCurrentJEVersion());
    logger = LoggerUtils.getLogger(getClass());
  }

  /**
   * Returns all the members that are currently part of the replication group, using NO_CONSISTENCY.
   * This method can read the database directly, and can be used when the replicated environment is
   * detached and the RepNode is null. It's for the latter reason that the method reads uncommitted
   * data. In detached mode, there may be transactions on the database that were in progress when
   * the node was last shutdown. These transactions may have locks which will not be released until
   * after the node is re-attached and the replication stream is resumed. Using uncommitted reads
   * avoids use of locks in this circumstance. It's safe to read these records, since the database
   * will eventually be updated with these changes.
   *
  ...
  public static RepGroupImpl getGroup(RepImpl rImpl, String groupName) throws DatabaseException {
    ...
    if (repNode != null) {
      for (final Feeder feeder : repNode.feederManager().activeReplicasMap().values()) {
  ...
  public RepGroupImpl getGroup() throws DatabaseException {
    ...
    return getGroup(repImpl, repImpl.getConfigManager().get(GROUP_NAME));
  }

  /**
   * Sets the minimum JE version required for nodes to join the replication group and refreshes the
   * group object cached in the rep group. Throws a {@link MinJEVersionUnsupportedException} if the
   * requested version is not supported by current nodes.
   *
   * <p>If this method returns successfully, nodes that are running a JE version older than the one
   * specified will not be permitted to join the replication group in the future. Use this method to
   * implement features that require all group members to meet a minimum version requirement.
   *
   * <p>The update attempts to obtain acknowledgments from a simple majority, to make sure that
   * future masters agree that the update has taken place, but does not require this.
   *
   ...
   * @throws MinJEVersionUnsupportedException if the requested version is not supported
   */
  public void setMinJEVersion(final JEVersion newMinJEVersion)
    ...
    MasterTxn txn = new MasterTxn(repImpl, QUORUM_ACK, repImpl.getNameIdPair());
    try {
      RepGroupImpl repGroup = fetchGroupObject(txn, groupDbImpl, LockMode.RMW);
      ...
      txn = null;
      LoggerUtils.info(logger, repImpl, "Updated minimum JE group version to " + newMinJEVersion);
  ...
  /**
   * Returns a representation of the nodes of the group stored in the database, using the txn and
   * handles that were passed in.
   */
  private static RepGroupImpl fetchGroup(String groupName, DatabaseImpl dbImpl, Txn txn)
      throws DatabaseException {
    ...
    Map<Integer, RepNodeImpl> nodes = new HashMap<Integer, RepNodeImpl>();
  ...
  /**
   * Ensures that information about this node, the current master, is in the member database. If it
   * isn't, enter it into the database. If the database does not exist, create it as well.
   *
  ...
  public void addFirstNode() throws DatabaseException {
    ...
    RepGroupImpl repGroup = new RepGroupImpl(groupName, repImpl.getCurrentJEVersion());
    ...
    Cursor cursor = null;
    try {
      txn = new MasterTxn(repImpl, txnConfig, repImpl.getNameIdPair());
    ...
    ensureMember(
        new RepNodeImpl(
            nodeName, repImpl.getHostName(), repImpl.getPort(), repImpl.getCurrentJEVersion()));
  }

  /**
   * Ensures that the membership info for the replica is in the database. A call to this method is
   * initiated by the master as part of the feeder/replica handshake, where the replica provides
   * membership information as part of the handshake protocol. The membership database must already
   * exist, with the master in it, when this method is invoked.
   *
   ...
   * @throws DatabaseException when the membership info could not be entered into the membership
   *     database.
   */
  ...
  void ensureMember(RepNodeImpl ensureNode) throws DatabaseException {
    ...
    if (ensureNode.getType().hasTransientId()) {
      throw new IllegalArgumentException(
          "Attempt to call ensureMember on " + ensureNode.getType() + " node: " + ensureNode);
    }
    ...
    try {
       ...
       * acquisition.
       */
      final RepGroupImpl repGroup = fetchGroupObject(txn, groupDbImpl, LockMode.DEFAULT);
    ...
    refreshGroupAndNotifyGroupChange(ensureNode.getName(), GroupChangeType.ADD);
  }

  private void refreshGroupAndNotifyGroupChange(String nodeName, GroupChangeType opType) {
    repImpl.getRepNode().refreshCachedGroup();
    repImpl.getRepNode().getMonitorEventManager().notifyGroupChange(nodeName, opType);
  }

  /**
   * Removes a node from the replication group by marking the node's entry in the rep group db as
   * removed, and optionally deleting the entry.
   *
   * <p>This method should not be called for nodes with transient IDs.
   */
  public void removeMember(final RepNodeImpl removeNode, final boolean delete) {
    LoggerUtils.info(
        logger, repImpl, (delete ? "Deleting node: " : "Removing node: ") + removeNode.getName());
    ...
    TwoPhaseUpdate twoPhaseUpdate =
        new TwoPhaseUpdate(removeNode, true) {

          @Override
          void phase1Body() {
            final RepGroupImpl repGroup = fetchGroupObject(txn, groupDbImpl, LockMode.RMW);
          ...
          void phase2Body() {
            ...
            final Cursor cursor = makeCursor(groupDbImpl, txn, CursorConfig.DEFAULT);
            try {
              final OperationStatus status =
                  cursor.getSearchKey(nodeNameKey, new DatabaseEntry(), LockMode.RMW);
              if (status != OperationStatus.SUCCESS) {
                throw EnvironmentFailureException.unexpectedState(
                    "Node ID: " + removeNode.getNameIdPair() + " not present in group db");
              }
              cursor.delete();
            } finally {
              cursor.close();
    ...
    refreshGroupAndNotifyGroupChange(removeNode.getName(), GroupChangeType.REMOVE);

    LoggerUtils.info(logger, repImpl, "Successfully deleted node: " + removeNode.getName());
  }
  ...
  private void createMember(final RepNodeImpl node)
    ...
    LoggerUtils.fine(logger, repImpl, "Adding node: " + node.getNameIdPair());
  ...
  public void updateMember(final RepNodeImpl node, final boolean quorumAck)
    ...
    if (node.getType().hasTransientId()) {
      throw new IllegalArgumentException(
          "Attempt to call updateMember on a node of type " + node.getType() + ": " + node);
    }
   ...
   *
   * <p>In the first phase the master repeatedly tries to commit the "put" operation until it gets a
   * Quorum of acks, ensuring that the operation has been made durable. Nodes that obtain this entry
   * will start using it in elections. However, the node itself will not participate in elections
   * until it has successfully completed phase 2.
   *
   * <p>In the second phase, the entry for the member is updated to note that a quorum of acks was
   * received.
   *
   * <p>Failure leaves the database with the member info absent, or present but without the update
   * to quorumAcks indicating that a quorum has acknowledged the change.
   *
   ...
   */
  private void twoPhaseMemberUpdate(final RepNodeImpl node, final boolean quorumAck)
      throws DatabaseException {

    TwoPhaseUpdate twoPhaseUpdate =
        new TwoPhaseUpdate(node, quorumAck) {
          final int saveOrigId = node.getNameIdPair().getId();
          ...
          void phase1Body() {
            ...
            saveGroupObject(txn, repGroup, groupDbImpl);
            node.setChangeVersion(changeVersion);
            final RepNodeImpl existingNode = repGroup.getNode(node.getName());
  ...
  /**
   * This method is not used when the CBVLSN is defunct -- see GlobalCBVLSN. This method was not
   * moved to GlobalCBVLSN to avoid modularity problems.
   *
   * <p>Updates the database entry associated with the node with the new local CBVLSN, if it can do
   * so without encountering lock contention, and unless the node is a secondary, arbiter, or
   * external node. Also updates the rep node's transient group information about the global CBVLSN.
   * If it encounters contention, it returns false, and the caller must retry at some later point in
   * time.
   *
   * <p>Note that changes to the local CBVLSN do not update the group version number since they do
   * not impact group membership.
   *
   ...
   * @throws DatabaseException
   */
  public boolean updateLocalCBVLSN(
      final NameIdPair nameIdPair, final VLSN newCBVLSN, final NodeType nodeType)
      throws DatabaseException {
    ...
    try {
      ...
      /* Let's see if the entry needs updating. */
      final NodeBinding nodeBinding = new NodeBinding(repGroup.getFormatVersion());
      final RepNodeImpl node = nodeBinding.entryToObject(value);
      ...
      LoggerUtils.fine(
          logger, repImpl, "Local CBVLSN updated to " + newCBVLSN + " for node " + nameIdPair);
      ok = true;
    } finally {
      if (cursor != null) {
        cursor.close();
   ...
   */
  private RepGroupImpl fetchGroupObject(
      final Txn txn, final DatabaseImpl groupDbImpl, final LockMode lockMode)
      throws DatabaseException {
    ...
    try {
      ...
      final OperationStatus status = cursor.getSearchKey(groupKeyEntry, groupEntry, lockMode);

      if (status != OperationStatus.SUCCESS) {
        throw EnvironmentFailureException.unexpectedState(
            "Group entry key: " + GROUP_KEY + " missing from group database");
      }
    } finally {
      if (cursor != null) {
        cursor.close();
   ...
   */
  private void saveGroupObject(Txn txn, RepGroupImpl repGroup, DatabaseImpl groupDbImpl)
      throws DatabaseException {

    final GroupBinding groupBinding = new GroupBinding(repGroup.getFormatVersion());
    DatabaseEntry groupEntry = new DatabaseEntry();
    groupBinding.objectToEntry(repGroup, groupEntry);
   ...
   */
  private void saveNodeObject(
      Txn txn, RepNodeImpl node, DatabaseImpl groupDbImpl, RepGroupImpl repGroup)
      throws DatabaseException {
  ...
  /** Converts a numeric version string to a JEVersion, returning null for an empty string. */
  static JEVersion parseJEVersion(final String versionString) {
    return versionString.isEmpty() ? null : new JEVersion(versionString);
  }

  /** Converts a JEVersion to a numeric version string, returning an empty string for null. */
  ...
  public static class GroupBinding extends TupleBinding<RepGroupImpl> {
    ...
    /** Create an instance for writing using the specified group format version. */
    GroupBinding(final int writeFormatVersion) {
      if (writeFormatVersion < 0) {
        throw new IllegalArgumentException(
            "writeFormatVersion must be non-negative: " + writeFormatVersion);
      }
  ...
  /**
   * Supports the serialization/deserialization of node info into and out of the database. Nodes are
   * always saved using the current group format version, and the node's format version is checked
   * on reading to make sure it is not newer than the current group format version, although they
   * could have an older format version if they have not been saved recently.
   *
   * <p>Prior to RepGroupImpl version 3, the second field was always the ordinal value of the node
   * type, which was either 0 or 1. Starting with version 3, values greater than 1 are treated as
   * the rep group version of the format used to write the node binding, with the node type
   * following in the next field, and the jeVersion field added at the end.
   */
  public static class NodeBinding extends TupleBinding<RepNodeImpl> {
    ...
    static final int APPROX_MAX_SIZE =
        40 + /* node name (guess) */ 4 + /* node ID */ 1 + /* group version */ 1 + /* NodeType */ 1
            + /* quorumAck */ 1 + /* isRemoved */ 40 + /* hostName (guess) */ 4 + /* port */ 8
            + /* lastCBVLSN */ 8 + /* barrierTime */ 4
            + /* changeVersion */ 10; /* jeVersion (approx) */
    ...
    public RepNodeImpl entryToObject(final TupleInput input) {
      ...
      return new RepNodeImpl(
          ...
          input.readString(),
          input.readInt(),
          new CleanerBarrierState(new VLSN(input.readLong()), input.readLong()),
          input.readInt(),
    ...
    public static boolean supportsObjectToEntry(
        final RepNodeImpl node, final int groupFormatVersion) {
    ...
    @Override
    public void objectToEntry(final RepNodeImpl mi, final TupleOutput output) {
      ...
      final boolean v2 = (groupFormatVersion <= RepGroupImpl.FORMAT_VERSION_2);
  ...
  /**
   * Implements two phase updates for membership changes to the group database. It compartmentalizes
   * the retry operations and exception handling so that it's independent of the core logic.
   */
  private abstract class TwoPhaseUpdate {
    ...
    private void phase1() throws DatabaseException {
      ...
      for (int i = 0; i < QUORUM_ACK_RETRIES; i++) {
        txn = null;
        try {
          txn = new MasterTxn(repImpl, quorumAck ? QUORUM_ACK : NO_ACK, repImpl.getNameIdPair());
          phase1Body();
          txn.commit(quorumAck ? QUORUM_ACK_DURABILITY : NO_ACK_DURABILITY);
          txn = null;
          return;
          ...
          LoggerUtils.severe(logger, repImpl, "Phase 1 failed unexpectedly: " + e.getMessage());
          if (txn != null) {
            txn.abort();
          }
          throw e;
      ...
      LoggerUtils.warning(logger, repImpl, "Phase 1 failed: " + phase1Exception.getMessage());
    ...
    private void phase2() {
      try {
        ...
        LoggerUtils.severe(logger, repImpl, "Unexpected failure in Phase 2: " + e.getMessage());
        throw e;
      } finally {
        if (txn != null) {
          txn.abort();
  ...
  /**
   * An internal API used to obtain group information by opening a stand alone environment handle
   * and reading the RepGroupDB. Used for debugging and utilities.
   *
  ...
  public static RepGroupImpl getGroup(final File envDir) {
    ...
    try {
      ...
      group = fetchGroup(group.getName(), DbInternal.getDbImpl(db), DbInternal.getTxn(txn));
      txn.commit();
      txn = null;
      return group;
    } finally {
  ...
  /**
   * Deletes all the current members from the rep group database and creates a new group, with just
   * the member supplied via the configuration. This method exists to support the utility {@link
   * DbResetRepGroup}
   *
   * <p>The changes proceed in three steps:
   *
   * <p>1) Determine the node id sequence number. This is to ensure that rep node ids are not
   * reused. Old rep node ids are present in the logs as commit records.
   *
   * <p>2) A new group object, with the node id sequence number determined in step 1), is created
   * and all existing nodes are deleted.
   *
   * <p>3) The first node is added to the rep group.
   *
   * @param lastOldVLSN the VLSN used to associate the new barrier wrt this node.
   */
  public void reinitFirstNode(VLSN lastOldVLSN) {
    ...
    final boolean retainUUID = configManager.getBoolean(RESET_REP_GROUP_RETAIN_UUID);
    ...
    RepGroupImpl prevRepGroup = fetchGroupObject(txn, dbImpl, LockMode.RMW);
    txn.commit();
    ...
    RepNodeImpl firstNode =
        new RepNodeImpl(nodeName, hostname, port, repImpl.getCurrentJEVersion());
    final CleanerBarrierState barrierState =
    ...
    while (mcursor.getNext(keyEntry, value, LockMode.DEFAULT) == OperationStatus.SUCCESS) {
      ...
      if (GROUP_KEY.equals(key)) {
        ...
        if (retainUUID) {
          ...
          repGroup = new RepGroupImpl(groupName, repImpl.getCurrentJEVersion());
        }
        GroupBinding groupBinding = new GroupBinding(repGroup.getFormatVersion());
    ...
    if (firstNodeId != firstNode.getNodeId()) {
      throw new IllegalStateException(
          "Expected nodeid:" + firstNodeId + " but found:" + firstNode.getNodeId());
    }
  }
}
