...
/**
 * The protocol used to obtain backup files from a LF Feeder. The message exchange is always
 * initiated by the client.
 *
 * <p>The following describes the request/response messages exchanged between the two nodes:
 *
 * <p>FeederInfoReq -> FeederInfoResp
 *
 * <p>FileListReq -> FileListResp
 *
 * <p>FileInfoReq -> FileInfoResp
 *
 * <p>FileReq -> FileStart <byte stream> FileEnd
 *
 * <p>Done
 *
 * <p>So a complete sequence of successful request messages looks like:
 *
 * <p>FeederInfoReq FileListReq [[FileInfoReq] [FileReq] ]+ Done
 *
 * <p>A response sequence would look like:
 *
 * <p>FeederInfoResp FileListResp [[FileInfoResp] [FileStart <byte stream> FileEnd] ]+
 *
 * <p>The client may abandon its interaction with the server if it decides the server is overloaded.
 *
 * <p>The client tries to minimize the number of files it actually requests based upon its current
 * state.
 *
 * <p>When a FileReq is received by the server, other files previously requested (using FileReq) may
 * be deleted by the server. These previously requested files must not be requested again using
 * FileReq or FileReqInfo.
 */
public class Protocol extends BinaryProtocol {
  ...
  public class FileInfoResp extends FileStart {
    ...
    public FileInfoResp(
        String fileName, long fileLength, long lastModifiedTime, byte[] digestSHA1) {
    ...
    /**
     * Returns the SHA1 value if it was requested, or a zero length byte array if it was not
     * requested.
     */
   ...
   *
   * <p>FileStart <byte stream> FileEnd
   */
  ...
  /**
   * Message from client indicating it's done with all the files it needs and that the connection
   * can be terminated.
   */
