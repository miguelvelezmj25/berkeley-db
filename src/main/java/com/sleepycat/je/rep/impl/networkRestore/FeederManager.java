...
/**
 * Manages the multiple log file feeders that may be servicing requests from multiple clients
...
public class FeederManager extends StoppableThread {
   ...
   *
   * @param serviceDispatcher The service dispatcher with which the FeederManager must register
   *     itself. It's null only in a test environment.
  ...
  public FeederManager(
      ServiceDispatcher serviceDispatcher, EnvironmentImpl envImpl, NameIdPair nameIdPair) {

    super(envImpl, "Feeder Manager node: " + nameIdPair.getName());
    this.serviceDispatcher = serviceDispatcher;
    serviceDispatcher.register(
        serviceDispatcher.new LazyQueuingService(FEEDER_SERVICE, channelQueue, this));
  ...
  /**
   * Returns the number of feeders that are currently active with this node. Note that active leases
   * are included in this count, since it's expected that the clients will try to reconnect.
   */
  ...
  /**
   * Provides the lease mechanism used to maintain a handle to the DbBackup object across Server
   * client disconnects.
   */
  class Lease extends TimerTask {
     ...
     *
     * @return the dbBackup instance, if the lease hasn't already been terminated
     */
