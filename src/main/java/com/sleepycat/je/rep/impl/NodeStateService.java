...
/**
 * The service registered by a RepNode to answer the state request from another node. It can also be
 * extended to be used by "Ping" command.
 */
public class NodeStateService extends ExecutingService {
  ...
  public ResponseMessage process(NodeStateRequest stateRequest) {
    ...
    return protocol
    .new NodeStateResponse(
        repNode.getNodeName(), repNode.getMasterName(), joinTime, repNode.getRepImpl().getState());
  }
