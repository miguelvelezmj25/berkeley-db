...
import com.sleepycat.je.rep.impl.node.*;
...
import com.sleepycat.je.rep.txn.*;
...
public class RepImpl extends EnvironmentImpl implements RepEnvConfigObserver {
   ...
   *
   * <p>When a Master Transfer operation completes Phase 1, it sets a new {@code CountDownLatch} in
   * order to block the completion of transactions at the commit or abort stage. We must avoid
   * having it do so at an awkward moment. There are two (unrelated) cases:
   *
   * <ol>
   *   <li>There is a brief period between the time a transaction "awaits" the latch (in {@code
   *       checkBlock()}) and the time it publishes its VLSN. We must avoid having Master Transfer
   *       read its "ultimate goal" VLSN during that interval.
   *   <li>The Feeder input thread occasionally updates the GroupDB, upon receiving a Heartbeat
   *       response. That happens in a transaction, like any other, so it could be subject to the
   *       normal blockage in Phase 2. But the Feeder input thread is of course also the thread that
   *       we rely on for making progress towards the goal of Master Transfer; so blocking it is
   *       counterproductive.
   * </ol>
   *
   * @see MasterTransfer
  ...
  /**
   * Used for testing, to create log files with VLSN.UNINITIALIZED_VLSN_SEQUENCE as the value for
   * the dtvlsn
   */
  ...
  /** The feeder authenticator generator is instantiated by the replicated environment creator. */
  ...
  public RepImpl(
    ...
    allowConvert = RepInternal.getAllowConvert(((ReplicationConfig) repConfigProxy));

    repConfigObservers = new ArrayList<RepEnvConfigObserver>();
    addRepConfigObserver(this);

    repNetConfig = ((ReplicationConfig) repConfigProxy).getRepNetConfig();
    ...
    nodeStats = new StatGroup(RepImplStatDefinition.GROUP_NAME, RepImplStatDefinition.GROUP_DESC);
    hardRecoveryStat = new BooleanStat(nodeStats, RepImplStatDefinition.HARD_RECOVERY);
    hardRecoveryInfoStat =
        new StringStat(
            nodeStats,
            RepImplStatDefinition.HARD_RECOVERY_INFO,
            "This node did not incur a hard recovery.");

    syncupProgressListener = ((ReplicationConfig) repConfigProxy).getSyncupProgressListener();
    logRewriteListener = ((ReplicationConfig) repConfigProxy).getLogFileRewriteListener();

    feederFilter = ((ReplicationConfig) repConfigProxy).getFeederFilter();

    authenticator = ((ReplicationConfig) repConfigProxy).getAuthenticator();
  }

  /**
   * Called by the EnvironmentImpl constructor. Some rep params, preserveVLSN for example, are
   * accessed by the EnvironmentImpl via methods (getPreserveVLSN for example), so they need to be
   * initialized early.
   */
  @Override
  protected void initConfigParams(EnvironmentConfig envConfig, RepConfigProxy repConfigProxy) {
    ...
    replicaAckTimeout = configManager.getDuration(RepParams.REPLICA_ACK_TIMEOUT);
    insufficientReplicasTimeout =
        configManager.getDuration(RepParams.INSUFFICIENT_REPLICAS_TIMEOUT);
    replayTxnTimeout = configManager.getDuration(RepParams.REPLAY_TXN_LOCK_TIMEOUT);
    defaultConsistencyPolicy =
        RepUtils.getReplicaConsistencyPolicy(configManager.get(RepParams.CONSISTENCY_POLICY));
    preserveVLSN = configManager.getBoolean(RepParams.PRESERVE_RECORD_VERSION);
    cacheVLSN = configManager.getBoolean(RepParams.CACHE_RECORD_VERSION);
    allowArbiterAck = configManager.getBoolean(RepParams.ALLOW_ARBITER_ACK);
    isArbiter = configManager.getBoolean(RepParams.ARBITER_USE);
    isSubscriber = configManager.getBoolean(RepParams.SUBSCRIBER_USE);
    isNetworkBackup = configManager.getBoolean(RepParams.NETWORKBACKUP_USE);
    arbiterAckTimeout = configManager.getDuration(RepParams.ARBITER_ACK_TIMEOUT);
  }
  ...
  @Override
  protected DbConfigManager initConfigManager(
      EnvironmentConfig envConfig, RepConfigProxy repConfigProxy) {
  ...
  public void setRepMutableConfig(ReplicationMutableConfig config) throws DatabaseException {
    ...
    repConfigManager = new RepConfigManager(configManager.getEnvironmentConfig(), newConfig);
  ...
  @Override
  public void repEnvConfigUpdate(RepConfigManager configMgr, ReplicationMutableConfig newConfig)
      throws DatabaseException {

    allowArbiterAck = configMgr.getBoolean(RepParams.ALLOW_ARBITER_ACK);
    ...
    repNode
        .getElectionQuorum()
        .setElectableGroupSizeOverride(newConfig.getElectableGroupSizeOverride());
  ...
  /**
   * The VLSNIndex must be created, merged and flushed before the recovery checkpoint. This method
   * should be called even if there is no recovery checkpoint, because it sets up needed data
   * structures.
   *
   * <p>On the face of it, it seems that one could flush the VLSNIndex cache after the recovery
   * checkpoint, before the Replicator constructor returns and before any user level HA operations
   * can start. That's not sufficient because the recovery checkpoint is shortening the recovery
   * interval for future recoveries, and any information that has been garnered must be persisted.
   * Here's an example of what might happen after a series of recoveries if we fail to flush
   * VLSNIndex as part of the recovery checkpoint:
   *
   * <p>Environment recovers for first time, brand new environment recovery did not find any VLSNs
   * in log, because log is brand new recovery logs ckpt 1start recovery logs ckpt 1 end
   *
   * <p>VLSN 1 logged VLSN 2 logged VLSN 3 logged
   *
   * <p>crash .... Environment recovers recovery crawls log from ckpt 1 start onward, finds VLSNs
   * 1-3 recovery logs ckpt 2 start recovery logs ckpt 2 end VLSN index instantiated, VLSNs 1-3
   * added in but not written too disk
   *
   * <p>crash ... Environment recovers recovery crawls log from ckpt start 2 start onward, finds no
   * VLSNs.
   *
   * <p>Instead, the flushed VLSN has to be logged before the checkpoint end record that is used for
   * the next recovery.
   */
  @Override
  public void preRecoveryCheckpointInit(RecoveryInfo recoveryInfo) {
     ...
     */
    NameIdPair useNameIdPair = new NameIdPair(configManager.get(NODE_NAME));
   ...
   * Note that any method that creates or clears the repNode field must be synchronized.
   */
  public synchronized ReplicatedEnvironment.State joinGroup(
  ...
  /**
   * Initialize the DataChannelFactory in our configuration for use. This is public to allow access
   * by the ReplicatedEnvironment constructor.
   *
   * @throws IllegalArgumentException if the ReplicationNetworkConfig is invalid.
   */
      ...
      if (channelFactory == null) {
        channelFactory =
            DataChannelFactoryBuilder.construct(
                repNetConfig, DataChannelFactoryBuilder.makeLoggerFactory(this));
      }
    }
  }
  ...
  /**
   * @see EnvironmentImpl#setupClose Release all replication resources that can be released before
   *     the checkpoint. Note that any method that creates or clears the repNode field must be
   *     called from a synchronized caller.
   *     <p>Note that the vlsnIndex is closed as a callback, from postCheckpointPreEnvClose()
   * @throws DatabaseException
   */
  @Override
  protected synchronized void setupClose(PrintWriter errors) throws DatabaseException {
    ...
    if (groupDbImpl != null) {
      ...
      groupDbImpl = null;
      LoggerUtils.fine(envLogger, this, "Group member database shutdown");
    }
  ...
  /**
   * Close any resources that need to be closed after the closing checkpoint. Note that since
   * Replay.close closes open transactions, it must be invoked after the checkpoint has been
   * completed, so that the checkpoint operation can correctly account for the open transactions.
   */
  @Override
   ...
   *     <p>Note: this conversion process will iterate over all user created databases in the
   *     environment, which could be potentially be a costly affair. However, let's opt for
   *     simplicity and defer any optimizations until we see whether this is an important use case.
   */
  @Override
  ...
  /**
   * Used by error handling to forcibly close an environment, and by tests to close an environment
   * to simulate a crash. Database handles do not have to be closed before calling this method. A
   * checkpoint is not performed. The various thread pools will be shutdown abruptly.
   *
   * @throws DatabaseException
   */
  @Override
  ...
  /**
   * A replicated log entry has been written on this node. Update the VLSN->LSN mapping. Called
  ...
  /**
   * Generate the next VLSN and update the DTVLSN value in the item. Note that this method is only
   * invoked when the node is in the Master state, since the master assigns new VLSNs and DTVLSNs,
   * and the replicas simply preserve them.
   *
   * <p>The DTVLSN value must be calculated under the same latch as the updating of the VLSN to
   * ensure that the following invariants are maintained:
   *
   * <p>lsn1 > lsn2 ==> VLSN(lsn1) > VLSN(lsn2) vlsn2 > vlsn1 ==> DTVLSN(vlsn2) >= DTVLSN(vlsn1)
   *
   * <p>where vlsn2 and vlsn1 are transaction commit or abort records.
   *
   * <p>Replicas, when replaying their stream, verify that this invariant is maintained.
   *
   * <p>Commit/Abort records for Replication groups that have a single electable and durable node
   * have their dtvlsn written as the associated VLSN, that is, DTVLSN(vlsn) == vlsn. For all other
   * RG configurations, DTVLSN(vlsn) < vlsn.
   *
   * <p>Commit/Abort Log records that are created by replaying an HA stream from a pre DTVLSN
   * feeder, will have their dtvlsns set to VLSN.UNINITIALIZED_VLSN_SEQUENCE during replica replay.
   * They do not follow this code path.
   *
   * @param entry the log entry with which the VLSN will be associated. If the log entry represents
   *     a commit or abort entry, its DTVLSN is modified so that it's correct when it's serialized
   *     out.
   */
  @Override
  public VLSN assignVLSNs(LogEntry entry) {
    ...
    final VersionedWriteTxnEnd txnEnd = (VersionedWriteTxnEnd) entry.getMainItem();
    ...
    if (checkDTVLSN != VLSN.NULL_VLSN_SEQUENCE) {
      throw new IllegalStateException(
          "NULL DTVLSN expected at VLSN:" + vlsn + " not " + checkDTVLSN);
    }
  ...
  /**
   * Flush any information that needs to go out at checkpoint. Specifically, write any in-memory
   * VLSN->LSN mappings to the VLSNIndex database so we are guaranteed that the VLSNIndex database
   * will recover properly. This must be committed with noSync because - the ensuing checkpoint end
   * record will be logged with an fsync and will effectively force this out - it's important to
   * minmize lock contention on the vlsn index and any fsync done during a checkpoint will be
   * expensive, as there may be quite a lot to push to disk. We don't want to incur that cost while
   * holding locks on the vlsn index. [#20702]
   */
  @Override
  ...
  public void setChangeListener(StateChangeListener listener) {
    ...
    try {
      ...
      LoggerUtils.severe(envLogger, this, "State Change listener exception: " + e.getMessage());
  ...
  /**
   * Ensures that the environment is currently a Master before proceeding with an operation that
   * requires it to be the master.
   *
  ...
  /**
   * Create an appropriate type of ThreadLocker. Specifically, it creates an MasterThreadLocker if
   * the node is currently a Master, and a ReplicaThreadLocker otherwise, that is, if the node is a
   * Replica, or it's currently in a DETACHED state.
   *
  ...
  /**
   * Create an appropriate type of Replicated transaction. Specifically, it creates a MasterTxn, if
   * the node is currently a Master, a ReadonlyTxn otherwise, that is, if the node is a Replica, or
   * it's currently in a DETACHED state.
   *
   * <p>Note that a ReplicaTxn, used for transaction replay on a Replica is not created on this
   * path. It's created explicitly in the Replay loop by a Replica.
   *
  ...
  public Txn createRepUserTxn(TransactionConfig config) throws DatabaseException {
    ...
    return (isMaster() && !config.getReadOnly() && !config.getLocalWrite())
        ? MasterTxn.create(this, config, nameIdPair)
        : new ReadonlyTxn(this, config);
  }

  /**
   * Ensure that a sufficient number of feeders are available before proceeding with a master
   * transaction begin.
   *
   * @param txn the master transaction being initiated.
   * @throws InterruptedException
   * @throws DatabaseException if there were insufficient Replicas after the timeout period.
   */
  ...
  /**
   * Installs the commit-blocking latch that is used to halt the commit/abort of transactions, in
   * the final phase of a master transfer.
   *
  ...
  /**
   * If the GlobalCBVLSN is not defunct, updates the CBVLSN on behalf of a Feeder input thread (or
   * FeederManager running in the RepNode thread). If the GlobalCBVLSN is defunct, does nothing.
   *
   * <p>Does the update while avoiding the possibility that any resulting GroupDB update may get
   * blocked behind the final phase of a master transfer.
   *
   * <p>We skip the update if we're at the point of blocking new transactions for a master transfer.
   * And we use a read/write lock in order to be able to examine that state safely.
   */
  ...
  /**
   * This hook is used primarily to perform the final checks before allowing the commit operation to
   * proceed. The following checks are performed here:
   *
   * <p>1) Check for master 2) Check for sufficient Feeder connections to ensure that the commit
   * policy could be implemented. There is no guarantee that they will all ack the commit request.
   *
   * <p>The method also associates a latch with the transaction. The latch is used to delay the
   * commit operation until a sufficient number of commits have been received.
   *
   * <p>In addition, when mastership transfers are done, and this node is the original master,
   * commits and aborts are blocked so as to avoid hard recovery after electing a new master, see
   * [#18081].
   *
   * @param txn the master transaction being committed
   * @throws InsufficientReplicasException if the feeder is not in contact with enough replicas.
   ...
   * @throws ReplicaWriteException if the node transitioned to a Replica after the transaction was
   *     initiated.
   */
  public void preLogCommitHook(MasterTxn txn)
      throws InsufficientReplicasException, UnknownMasterException,
          ReplicaWriteException, EnvironmentFailureException {
    ...
    int activeReplicaCount = repNode.feederManager().activeAckReplicaCount();
    ReplicaAckPolicy ackPolicy = txn.getCommitDurability().getReplicaAck();
    ...
    if (requiredAckCount > activeReplicaCount) {
      ...
      if (ackPolicy.equals(ReplicaAckPolicy.SIMPLE_MAJORITY)
         ...
         * message.
         */
        final boolean includeArbiters = !ackPolicy.equals(ReplicaAckPolicy.ALL);
        ...
        if (requiredAckCount > activeAckRepSet.size()) {
          ...
          InsufficientReplicasException ire =
              new InsufficientReplicasException(txn, ackPolicy, requiredAckCount, activeAckRepSet);
          LoggerUtils.info(envLogger, this, ire.getMessage());
          throw ire;
        }

        /**
         * A new replica became active in the window between the first fast check and the second
         * slower check, just continue
         */
      }
    }
  ...
  private void checkBlock(MasterTxn txn) {
    try {
      ...
      if (txnTimeout <= 0) {
        blockTxnLatch.await();
      } else if (!blockTxnLatch.await(txnTimeout, TimeUnit.MILLISECONDS)) {
  ...
  /**
   * It ensures that the feeder obtains the requisite number of acknowledgments required for a
   * successful commit.
   *
   * @param txn The MasterTxn that was committed locally.
   * @throws InterruptedException if the thread was interrupted while waiting for acknowledgments.
   * @throws InsufficientAcksException if the master received an insufficient number of commit
   *     acknowledgments within the replica commit timeout period.
   * @throws EnvironmentFailureException
   */
  public void postLogCommitHook(MasterTxn txn, LogItem commitItem)
      throws InsufficientAcksException, InterruptedException, EnvironmentFailureException {
    final long txnTimeout = txn.getTxnTimeout();
  ...
  private void postLogCommitHookInternal(MasterTxn txn, LogItem commitItem, int ackTimeout)
      throws InsufficientAcksException, InterruptedException, EnvironmentFailureException {
    if (txn.unlockOnce()) {
    ...
    if (!isValid()) {
      final int currentRequiredAckCount =
          repNode
              .getDurabilityQuorum()
              .getCurrentRequiredAckCount(txn.getCommitDurability().getReplicaAck());
      if (currentRequiredAckCount > 0) {
        /* Throw a more actionable and accurate exception than EFE */
        final String msg = "Acks could not be obtained because the environment" + "was invalidated";
  ...
  /**
   * Invoked before aborting a MasterTxn, this happens when the master is going to be a replica
   * because of mastership transfer. We do this to make sure that the replica going to be the master
   * has the most recent log and no hard recovery would happen after its election, see SR [#18081].
   *
  ...
  public void preLogAbortHook(MasterTxn txn)
      throws EnvironmentFailureException, ReplicaWriteException, UnknownMasterException {
  ...
  /**
   * Releases the block latch lock, if held. This hook is called in the normal course of
   * Txn.abort(), once the abort log record has been written and the associated VLSN stored.
   */
  ...
  /**
   * Removes any pending acknowledgments that were registered by the preLogCommitHook. This hook is
   * called only when a {@code commit()} fails and therefore must be aborted.
   */
  public void postLogCommitAbortHook(MasterTxn txn) {
    LoggerUtils.info(envLogger, this, "post log abort hook for txn: " + txn.getId());
    if (txn.unlockOnce()) {
      blockLatchLock.readLock().unlock();
    }
  ...
  public Txn createReplayTxn(long txnId) throws DatabaseException {
    ...
    return new ReplayTxn(this, TransactionConfig.DEFAULT, txnId, envLogger);
  }

  /**
   * Used by environment recovery to get a tracker to collect VLSN-LSN mappings that are within the
   * recovery part of the log. These might not be reflected in the persistent mapping db.
   */
  @Override
  ...
  /**
   * The default consistency is not currently mutable in the API, but can be set for testing
   * purposes.
   *
   * <p>TODO: Make it mutable in the API, since Durability is mutable.
   */
  ...
  private long getLsnForVLSN(VLSN vlsn, int readBufferSize) {
    ...
    FeederReader feederReader =
        new FeederReader(this, vlsnIndex, DbLsn.makeLsn(fileNumber, 0), readBufferSize);
  ...
  /**
   * Returns the logged durable txn VLSN. The logged DTVLSN is part of the last txn commit or abort
   * entry.
   *
   * @return the persistent DTVLSN. The value may be VLSN.UNINITIALIZED_VLSN_SEQUENCE if the
   *     environment was newly created, that is, it has no transactions in it as yet, or if the last
   *     entry was created by a pre-DTVLSN master.
   * @throws FileNotFoundException if the file containing the last txn commit or abort entry does
   *     not exist
   */
  public long getLoggedDTVLSN() throws FileNotFoundException {
    ...
    final long lsn = getLsnForVLSN(lastTxnEnd, 1024 /* buffer size for txn end */);
    final TxnEnd txnEnd = (TxnEnd) getLogManager().getLogEntry(lsn).getMainItem();
    ...
    LoggerUtils.logMsg(envLogger, this, Level.INFO, "Pre DTVLSN log, starting with zero dtvlsn");
  ...
  @Override
  public void checkRulesForExistingEnv(boolean dbTreeReplicatedBit, boolean dbTreePreserveVLSN)
  ...
  /**
   * Returns the JE version that is currently running on this node, consulting the TEST_JE_VERSION
   * configuration parameter for a test override.
   */
  ...
  /**
   * Called when a node has identified itself as the master, which is when the RepNode.selfElect is
   * called. The database should not exist at this point.
   *
   * <p>Lock hierarchy: GroupDbLock -> sync on EnvironmentImpl
   *
   * @throws DatabaseException
   */
    ...
    try {
      ...
      try {
        ...
        txnConfig.setDurability(
            new Durability(SyncPolicy.SYNC, SyncPolicy.SYNC, ReplicaAckPolicy.NONE));
        txnConfig.setConsistencyPolicy(NO_CONSISTENCY);
        txn = new MasterTxn(this, txnConfig, getNameIdPair());
  ...
  public DatabaseImpl getGroupDb() throws DatabaseException {

    return openGroupDb(false /* doLockProbe */);
  }

  /**
   * Open the group db, which should exist already, using NO_CONSISTENCY. Do not wait on the group
   * db lock, return null if the databaseImpl hasn't been created and we can't obtain it.
   *
   * <p>Lock hierarchy: GroupDbLock -> sync on EnvironmentImpl
   */
  ...
  /**
   * Do the work of creating the lock and then assigning the groupDbImpl field, using
   * NO_CONSISTENCY.
   *
   * @throws DatabaseException
  ...
  private DatabaseImpl openGroupDb(final boolean doLockProbe)
    ...
    try {
      ...
      newDbImpl =
          getDbTree()
              .getDb(txn, DbType.REP_GROUP.getInternalName(), null /* databaseHandle */, false);
      if (newDbImpl == null) {
  ...
  /**
   * Return true if the node has been configured as a Designated Primary. This does not necessarily
   * mean that the node is actively operating in designated primary mode. See {@link
   * com.sleepycat.je.rep.arbitration.Arbiter#isActive}
   */
  ...
  /* For creating a rep exception from standalone code. */
  @Override
  public DatabasePreemptedException createDatabasePreemptedException(
      String msg, String dbName, Database db) {
  ...
  public void shutdownGroupSetup(long timeoutMs) {
    ...
    if (openCount > 1) {
      throw new IllegalStateException(
          "Environment has " + (openCount - 1) + " additional open handles.");
    }
  ...
  /**
   * Dump interesting aspects of the node's state. Currently for debugging use, possibly useful for
   * field support.
   */
  ...
  /**
   * Dumps the state associated with all active Feeders that supply acknowledgments, along with
   * identifying information about the node and its current HA state.
   */
  public String dumpAckFeederState() {
    return getNameIdPair() + "[" + getState() + "]" + repNode.dumpAckFeederState();
  }
   ...
   * disk. A No-Op for non-replicated systems. [#19754]
   */
  @Override
  ...
  public void setSyncupProgress(SyncupProgress progress, long n, long total) {
    ...
    if (!(syncupProgressListener.progress(progress, n, total))) {
      throw new EnvironmentFailureException(
          this,
          EnvironmentFailureReason.PROGRESS_LISTENER_HALT,
          "ReplicatedEnvironmentConfig.syncupProgressListener: ");
    }
  }

  /**
   * Test method to create pre-DTVLSN logs. When this is turned on in a test environment the dtvlsn
   * value in the log is written as a UNINITIALIZED_VLSN_SEQUENCE (zero), which is the value that
   * deserialization would assign to it if a new replica came across an older version commit or
   * abort record.
   */
  ...
  /**
   * Private class to prevent used of the close() method by the application on an internal handle.
   */
  private static class InternalReplicatedEnvironment extends ReplicatedEnvironment {

    public InternalReplicatedEnvironment(
      ...
      super(
          ...
          cloneRepConfig,
          cloneConfig,
          null /*consistencyPolicy*/,
          null /*initialElectionPolicy*/,
          false /*joinGroup*/,
          envImpl);
    }
  ...
  /**
   * RepImpl supplies the last txn abort or commit vlsn for use cases such as determining how caught
   * up a feeder or master transfer is. This info is usually obtained from the VLSNRange via the
   * VLSNIndex, but in some types of environment shutdowns, the VLSNIndex may need to be nulled out.
   * When that happens, VLSNIndexAccess will switch over from using the VLSNIndex to obtain the
   * range, to using a reference to the last known VLSNRange. Note that the VLSNRange instance held
   * within VLSNIndex is constantly being replaced when the replication stream is active., and
   * that's why LastTxnEndAccess generally obtains the range via the VLSNIndex, rather keeping a
   * reference to a VLSNRange instance.
   */
  private class VLSNIndexAccess {
    ...
    /**
     * Save the last range so the lastTxnEnd value can continue to be available, and null out the
     * vlsnIndex.
     */
   ...
   *
   * @throws IllegalStateException if any node in the group is less than JE_TTL_VERSION.
   */
  @Override
    ...
    switch (restoreRequired.getFailureType()) {
      case NETWORK_RESTORE:
        ...
        throw new InsufficientLogException(
            restoreRequired.getProperties(), configManager.get(RepParams.HELPER_HOSTS));
      default:
