...
import static com.sleepycat.je.rep.impl.networkRestore.NetworkBackupStatDefinition.*;
import static java.util.concurrent.TimeUnit.MINUTES;

/**
 * This class implements a hot network backup that permits it to obtain a consistent set of log
 * files from any running environment that provides a LogFileFeeder service. This class thus plays
 * the role of a client, and the running environment that of a server.
 *
 * <p>The log files that are retrieved over the network are placed in a directory that can serve as
 * an environment directory for a JE stand alone or HA environment. If log files are already present
 * in the target directory, it will try reuse them, if they are really consistent with those on the
 * server. Extant log files that are no longer part of the current backup file set are deleted or
 * are renamed, depending on how the backup operation was configured.
 *
 * <p>Renamed backup files have the following syntax:
 *
 * <p>NNNNNNNN.bup.<backup number>
 *
 * <p>where the backup number is the number associated with the backup attempt, rather than with an
 * individual file. That is, the backup number is increased by one each time a backup is repeated in
 * the same directory and log files actually needed to be renamed.
 *
 * <p>The implementation tries to be resilient in the face of network failures and minimizes the
 * amount of work that might need to be done if the client or server were to fail and had to be
 * restarted. Users of this API must be careful to ensure that the execute() completes successfully
 * before accessing the environment. The user fails to do this, the InsufficientLogException will be
 * thrown again when the user attempts to open the environment. This safeguard is implemented using
 * the {@link RestoreMarker} mechanism.
 */
public class NetworkBackup {
  ...
  /**
   * The number of times to retry on a digest exception. That is, when the SHA1 hash as computed by
   * the server for the file does not match the hash as computed by the client for the same file.
   */
  ...
  /**
   * Creates a configured backup instance which when executed will backup the files to the
   * environment directory.
   *
   * @param serverSocket the socket on which to contact the server
   * @param receiveBufferSize the receive buffer size to be associated with the socket used for the
   *     log file transfers.
   * @param envDir the directory in which to place the log files
   * @param clientNameId the id used to identify this client
   * @param retainLogfiles determines whether obsolete log files should be retained by renaming
   *     them, instead of deleting them.
   * @param serverLoadThreshold only backup from this server if it has fewer than this number of
   *     feeders active.
   * @param repImpl is passed in as a distinct field from the log manager and file manager because
   *     it is used only for logging and environment invalidation. A network backup may be invoked
   *     by unit tests without an enclosing environment.
   * @param minVLSN the VLSN that should be covered by the server. It ensures that the log files are
   *     sufficiently current for this client's needs.
   * @throws IllegalArgumentException if the environment directory is not valid. When used
   *     internally, this should be caught appropriately.
   */
  public NetworkBackup(
    ...
    logger = LoggerUtils.getLoggerFixedPrefix(getClass(), clientNameId.toString(), repImpl);
    statistics =
        new StatGroup(
            NetworkBackupStatDefinition.GROUP_NAME, NetworkBackupStatDefinition.GROUP_DESC);
    ...
    transferredBytes = new AtomicLongStat(statistics, TRANSFERRED_BYTES);
    transferRate = new LongAvgRateStat(statistics, TRANSFER_RATE, 10000, MINUTES);
   ...
   *
   * @see NetworkBackup(InetSocketAddress, int, File, NameIdPair, boolean, int, VLSN, RepImpl,
  ...
  public String[] execute()
    ...
    try {
      ...
      protocol = checkProtocol(new Protocol(clientNameId, Protocol.VERSION, repImpl));
      checkServer();
      ...
      LoggerUtils.info(
          ...
          "Restoring from:"
              ...
              + channel.getSocketChannel().socket().getReceiveBufferSize()
              + "("
              + receiveBufferSize
              + ")"
              + " candidate log file count:"
              + fileNames.length);
      ...
      final long fileEnd = fileManager.getNumFromName(fileNames[fileNames.length - 1]);
  ...
  /**
   * Ensures that the log file feeder is a suitable choice for this backup: The feeder's VLSN range
   * end must be GTE the minVSLN and its load must be LTE the serverLoadThreshold.
   */
  private void checkServer()
      throws IOException, ProtocolException, LoadThresholdExceededException,
    ...
    if (resp.getRangeLast().compareTo(minVLSN) < 0) {
      throw new InsufficientVLSNRangeException(
          minVLSN, resp.getRangeFirst(), resp.getRangeLast(), resp.getActiveFeeders());
    }
    if (resp.getActiveFeeders() > serverLoadThreshold) {
      throw new LoadThresholdExceededException(
          serverLoadThreshold, resp.getRangeFirst(), resp.getRangeLast(), resp.getActiveFeeders());
    }
  }

  /**
   * Delete or rename residual jdb files that are not part of the log file set. This method is only
   * invoked after all required files have been copied over from the server.
   *
   * @throws IOException
   */
  ...
  private void getFiles(String[] fileNames)
      throws IOException, DatabaseException, RestoreMarker.FileCreationException {

    LoggerUtils.info(logger, repImpl, fileNames.length + " files in backup set");

    /* Get all file transfer lengths first, so we can track progress */
    final List<FileAndLength> fileTransferLengths = getFileTransferLengths(fileNames);
      ...
      for (int i = 0; i < DIGEST_RETRIES; i++) {
        try {
          ...
          if ((i + 1) == DIGEST_RETRIES) {
            throw new IOException("Digest mismatch despite " + DIGEST_RETRIES + " attempts");
          }
  ...
  /**
   * Returns information about files that need to be transferred, and updates expectedBytes and
   * skipCount accordingly. This method tries to avoid requesting the SHA1 if the file lengths are
   * not equal, since computing the SHA1 if it's not already cached requires a pass over the log
   * file. Note that the server will always send back the SHA1 value if it has it cached.
   */
  private List<FileAndLength> getFileTransferLengths(String[] fileNames)
    ...
    for (final String fileName : fileNames) {
      ...
      FileInfoResp statResp = protocol.read(channel, FileInfoResp.class);
      ...
      if (file.exists() && (fileLength == file.length())) {
        ...
        if (statResp.getDigestSHA1().length == 0) {
          protocol.write(protocol.new FileInfoReq(fileName, true), channel);
          statResp = protocol.read(channel, FileInfoResp.class);
        }
        final byte[] digest = LogFileFeeder.getSHA1Digest(file, fileLength).digest();
  ...
  /**
   * Requests and obtains the specific log file from the server. The file is first created under a
   * name with the .tmp suffix and is renamed to its true name only after its digest has been
   * verified.
   *
   * <p>This method is protected to facilitate error testing.
   */
  protected void getFile(File file)
      throws IOException, ProtocolException, DigestException, RestoreMarker.FileCreationException {

    LoggerUtils.fine(logger, repImpl, "Requesting file: " + file);
    protocol.write(protocol.new FileReq(file.getName()), channel);
    FileStart fileResp = protocol.read(channel, FileStart.class);
     ...
     * data directories.
     */
    File tmpFile = new File(fileManager.getFullFileName(file.getName()) + FileManager.TMP_SUFFIX);
    if (tmpFile.exists()) {
     ...
     * native buffers.
     */
    final ByteBuffer buffer = ByteBuffer.allocateDirect(LogFileFeeder.TRANSFER_BYTES);
    messageDigest.reset();
    ...
    try {
      ...
      for (long bytes = fileResp.getFileLength(); bytes > 0; ) {
        ...
        if (((++count % rateInterval) == 0) || (bytes <= 0)) {
          transferRate.add(transferredBytes.get(), System.currentTimeMillis());
        }
      }
    ...
    final FileEnd fileEnd = protocol.read(channel, FileEnd.class);

    /* Check that the read is successful. */
    if (!Arrays.equals(messageDigest.digest(), fileEnd.getDigestSHA1())) {
      LoggerUtils.warning(logger, repImpl, "digest mismatch on file: " + file);
    ...
    restoreMarker.createMarkerFile(
        RestoreRequired.FailureType.NETWORK_RESTORE, exceptionProperties);
    ...
    if (!file.setLastModified(fileResp.getLastModifiedTime())) {
      throw EnvironmentFailureException.unexpectedState(
          "File.setlastModifiedTime() for:"
              + file
              + " and time "
              + new Date(fileResp.getLastModifiedTime())
              + " failed.");
    }
  }

  /**
   * Renames (or deletes) this log file, and all other files following it in the log sequence. The
   * operation is done from the highest file down to this one, to ensure the integrity of the log
   * files in the directory is always preserved. Exclude the marker file because that is meant to
   * serve as an indicator that the backup is in progress. It will be explicitly removed only when
   * the entire backup is finished.
   *
   * @param startFile the lowest numbered log file that must be renamed or deleted
   * @throws IOException
   */
  ...
  private void disposeFile(File file) {
    ...
    if (retainLogfiles) {
      ...
      try {
        renamed = fileManager.renameFile(fileNumber, FileManager.BUP_SUFFIX);
      } catch (IOException e) {
        throw EnvironmentFailureException.unexpectedState(
      ...
      LoggerUtils.fine(logger, repImpl, "Renamed log file: " + file.getPath());
    } else {
      ...
      try {
        ...
        throw EnvironmentFailureException.unexpectedException(
            "Could not delete log file " + file.getPath() + " during network restore.", e);
      }
      if (!deleted) {
        throw EnvironmentFailureException.unexpectedState(
            "Could not delete log file " + file.getPath());
      }
      LoggerUtils.fine(logger, repImpl, "deleted log file: " + file.getPath());
    }
  }
   ...
   */
  private String[] getFileList() throws IOException, ProtocolException {

    protocol.write(protocol.new FileListReq(), channel);
    FileListResp fileListResp = protocol.read(channel, FileListResp.class);
   ...
   * @throws DatabaseException
   */
  private Protocol checkProtocol(Protocol candidateProtocol) throws IOException, ProtocolException {

    candidateProtocol.write(candidateProtocol.new ClientVersion(), channel);
    ServerVersion serverVersion = candidateProtocol.read(channel, Protocol.ServerVersion.class);

    if (serverVersion.getVersion() != candidateProtocol.getVersion()) {
      String message =
          "Server requested protocol version:"
  ...
  /**
   * Exception indicating that the digest sent by the server did not match the digest computed by
   * the client, that is, the log file was corrupted during transit.
   */
  ...
  public static class RejectedServerException extends Exception {
    ...
    RejectedServerException(VLSN rangeFirst, VLSN rangeLast, int activeServers) {
  ...
  @SuppressWarnings("serial")
  public static class InsufficientVLSNRangeException extends RejectedServerException {

    /* The VLSN that must be covered by the server. */
    private final VLSN minVLSN;

    InsufficientVLSNRangeException(
        VLSN minVLSN, VLSN rangeFirst, VLSN rangeLast, int activeServers) {
  ...
  @SuppressWarnings("serial")
  public static class LoadThresholdExceededException extends RejectedServerException {

    private final int threshold;

    LoadThresholdExceededException(
        int threshold, VLSN rangeFirst, VLSN rangeLast, int activeServers) {
      super(rangeFirst, rangeLast, activeServers);
    ...
    @Override
    public String getMessage() {
      return "Active server threshold: "
          + threshold
          + " exceeded. "
          + "Active servers: "
          + activeServers;
