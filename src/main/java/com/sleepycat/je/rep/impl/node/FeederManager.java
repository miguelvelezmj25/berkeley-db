...
/**
 * FeedManager is responsible for the creation and management of the Feeders used to respond to
 * connections initiated by a Replica. runfeeders() is the central loop that listens for replica
 * connections and manages the lifecycle of individual Feeders. It's re-entered each time the node
 * becomes a Master and is exited when its status changes.
 *
 * <p>There is a single instance of FeederManager that is created for a replication node. There are
 * many instances of Feeders per FeederManager. Each Feeder instance represents an instance of a
 * connection between the node serving as the feeder and the replica.
 *
 * <p>Note that the FeederManager and the Replica currently reuse the Replication node's thread of
 * control. When we implement r2r we will need to revisit the thread management to provide for
 * concurrent operation of the FeederManger and the Replica.
 */
public final class FeederManager {
   ...
   */
  private final Set<Feeder> nascentFeeders = Collections.synchronizedSet(new HashSet<Feeder>());
  ...
  /** Used to manage the flushing of the DTVLSN via a null TXN commit when appropriate. */
  ...
  /**
   * A test hook, parameterized by feeder's Name/ID pair, that delays CBVLSN updates if it throws an
  ...
  FeederManager(RepNode repNode) {
    ...
    activeFeeders =
        new SizeAwaitMap<String, Feeder>(repNode.getRepImpl(), new MatchElectableFeeders());
    ...
    replicaDelayMap = new LongDiffMapStat(stats, REPLICA_DELAY_MAP, validityMillis);
    replicaLastCommitTimestampMap = new AtomicLongMapStat(stats, REPLICA_LAST_COMMIT_TIMESTAMP_MAP);
    replicaLastCommitVLSNMap = new AtomicLongMapStat(stats, REPLICA_LAST_COMMIT_VLSN_MAP);
    replicaVLSNLagMap = new LongDiffMapStat(stats, REPLICA_VLSN_LAG_MAP, validityMillis);
    replicaVLSNRateMap =
        new LongAvgRateMapStat(stats, REPLICA_VLSN_RATE_MAP, MOVING_AVG_PERIOD_MILLIS, MINUTES);

    pollTimeoutMs = repNode.getConfigManager().getDuration(RepParams.FEEDER_MANAGER_POLL_TIMEOUT);
    dtvlsnFlusher = new DTVLSNFlusher();
  }

  /** A SizeAwaitMap predicate that matches feeders connected to electable replicas. */
  private class MatchElectableFeeders implements Predicate<Feeder> {
    ...
    public boolean match(final Feeder value) {
      ...
      return (replica != null) && repNode.getDurabilityQuorum().replicaAcksQualify(replica);
    }
  }
  ...
  /**
   * The numbers of Replicas currently "active" with this feeder. Active currently means they are
   * connected. It does not make any guarantees about where they are in the replication stream. They
   * may, for example, be too far behind to participate in timely acks.
   *
  ...
  /**
   * Returns the set of Replicas that are currently active with this feeder. A replica is active if
   * it has completed the handshake sequence.
   *
  ...
  /**
   * Returns the set of active replicas and arbiters, that are currently active with this feeder and
   * are supplying acknowledgments. A replica is active if it has completed the handshake sequence.
   * An Arbiter is only returned if it's in active arbitration.
   *
   * @param includeArbiters include active arbiters in the list of returned node names if true;
   *     exclude arbiters otherwise.
  ...
  public Set<String> activeAckReplicas(boolean includeArbiters) {
    ...
    synchronized (activeFeeders) {
      for (final Entry<String, Feeder> entry : activeFeeders.entrySet()) {
        ...
        if (replica.getType().isArbiter()) {
          if (!includeArbiters || !feeder.getRepNode().getArbiter().isActive()) {
            /* Skip the arbiter. */
            continue;
          }
        }
  ...
  /**
   * Transitions a Feeder to being active, so that it can be used in considerations relating to
   * commit acknowledgments and decisions about choosing feeders related to system load.
   *
    ...
    synchronized (nascentFeeders) {
      synchronized (activeFeeders) {
        ...
        if (activeFeeders.remove(replicaName) != null) {
          if (arbiterFeederName != null && arbiterFeederName.equals(replicaName)) {
  ...
  /**
   * The core feeder listener loop that is run either in a Master node, or in a Replica that is
   * serving as a Feeder to other Replica nodes. The core loop accepts connections from Replicas as
   * they come in and establishes a Feeder on that connection.
   *
   * <p>The loop can be terminated for one of the following reasons:
   *
   * <p>1) A change in Masters.
   *
   * <p>2) A forced shutdown, via a thread interrupt.
   *
   * <p>3) A server socket level exception.
   *
   * <p>The timeout on the accept is used to ensure that the check is done at least once per timeout
   * period.
   */
  void runFeeders() throws DatabaseException {
    ...
    LoggerUtils.info(logger, repNode.getRepImpl(), "Feeder manager accepting requests.");
    ...
    final LocalCBVLSNUpdater updater =
        new LocalCBVLSNUpdater(repNode.getNameIdPair(), repNode.getNodeType(), repNode);
    ...
    try {
      ...
      repNode.getServiceDispatcher().register(FEEDER_SERVICE, channelQueue);
      ...
      while (true) {
        ...
        if (feederReplicaChannel == RepUtils.CHANNEL_EOF_MARKER) {
          LoggerUtils.info(logger, repNode.getRepImpl(), "Feeder manager soft shutdown.");
          return;
        }
        ...
        if (feederReplicaChannel == null) {
          if (repNode.isShutdownOrInvalid()) {
            ...
            LoggerUtils.info(logger, repNode.getRepImpl(), "Feeder manager forced shutdown.");
            return;
          }
           ...
           */
          try {
            assert TestHookExecute.doHookIfSet(delayCBVLSNUpdateHook, repNode.getNameIdPair());
        ...
        try {
           ...
           */
          LoggerUtils.fine(logger, repNode.getRepImpl(), "Feeder I/O exception: " + e.getMessage());
          try {
            feederReplicaChannel.close();
          } catch (IOException e1) {
            LoggerUtils.fine(
                logger, repNode.getRepImpl(), "Exception during cleanup." + e.getMessage());
          }
          continue;
        }
      }
    } catch (MasterSyncException e) {
      LoggerUtils.info(logger, repNode.getRepImpl(), "Master change: " + e.getMessage());

      feederShutdownException =
          new UnknownMasterException(
              "Node " + repNode.getRepImpl().getName() + " is not a master anymore");
      ...
      if (this.repNodeShutdownException != null) {
         ...
         */
        LoggerUtils.warning(logger, repNode.getRepImpl(), "Feeder manager unexpected interrupt");
        throw repNodeShutdownException; /* Terminate the rep node */
      }
      if (repNode.isShutdown()) {
        LoggerUtils.info(logger, repNode.getRepImpl(), "Feeder manager interrupted for shutdown");
        return;
      }
      ...
      LoggerUtils.info(
          logger,
          repNode.getRepImpl(),
          "Feeder manager exited. CurrentTxnEnd VLSN: " + repNode.getCurrentTxnEndVLSN());
    }
  }
  ...
  /**
   * Shuts down a specific feeder. It's typically done in response to the removal of a member from
   * the group.
   */
  ...
  /**
   * Block until the required number of electable feeders/replica connections are established. Used
   * for establishing durability quorums. Since this is counting feeder/replica connections,
   * requiredReplicaCount does not include the master.
   *
   * <p>In the future this could be improved by also taking account the position of each feeder. If
   * a feeder is lagging far behind the master and this is likely to prevent commit, we may want to
   * reject the transaction at the outset to reduce the number of wasted txns/aborts. A special case
   * is when the replica is in out-of-disk mode and not acking at all.
   */
  boolean awaitFeederReplicaConnections(int requiredReplicaCount, long insufficientReplicasTimeout)
      throws InterruptedException {
    return activeFeeders.sizeAwait(
        requiredReplicaCount, insufficientReplicasTimeout, TimeUnit.MILLISECONDS);
  }
  ...
  public String dumpState(final boolean acksOnly) {
    ...
    synchronized (activeFeeders) {
      Set<Entry<String, Feeder>> feeds = activeFeeders.entrySet();
      if (feeds.size() == 0) {
        sb.append("No feeders.");
      } else {
        sb.append("Current feeds:");
        for (Entry<String, Feeder> feedEntry : feeds) {
  ...
  /**
   * Returns a count of the number of feeders whose replicas are counted in durability decisions and
   * have acknowledged txn-end VLSNs >= the commitVLSN argument.
   *
  ...
  /**
   * Update the Master's DTVLSN if we can conclude based upon the state of the replicas that the
   * DTVLSN needs to be advanced.
   *
   * <p>This method is invoked when a replica heartbeat reports a more recent txn VLSN. This
   * (sometimes) redundant form of DTVLS update is useful in circumstances when the value could not
   * be maintained via the usual ack response processing:
   *
   * <p>1) The application is using no ack transactions explicitly.
   *
   * <p>2) There were ack transaction timeouts due to network problems and the acks were never
   * received or were received after the timeout had expired.
   */
  public void updateDTVLSN(long heartbeatVLSN) {
    ...
    final DurabilityQuorum durabilityQuorum = repNode.getDurabilityQuorum();
    final int durableAckCount =
        durabilityQuorum.getCurrentRequiredAckCount(ReplicaAckPolicy.SIMPLE_MAJORITY);
    ...
    synchronized (activeFeeders) {
      ...
      for (Feeder feeder : activeFeeders.values()) {
        ...
        if (!durabilityQuorum.replicaAcksQualify(feeder.getReplicaNode())) {
          continue;
        }

        final long replicaTxnVLSN = feeder.getReplicaTxnEndVLSN().getSequence();
  ...
  /**
   * Set a test hook, parameterized by feeder's Name/ID pair, that delays CBVLSN updates if it
  ...
  /** Convenience constant used by the DTVLSN flusher when committing the null transaction. */
  private static final TransactionConfig NULL_TXN_CONFIG = new TransactionConfig();

  static {
    NULL_TXN_CONFIG.setDurability(
        new Durability(SyncPolicy.WRITE_NO_SYNC, SyncPolicy.WRITE_NO_SYNC, ReplicaAckPolicy.NONE));
  }

  /**
   * Writes a null (no modifications) commit record when it detects that the DTVLSN is ahead of the
   * persistent DTVLSN and needs to be updated.
   *
   * <p>Note that without this mechanism, the in-memory DTVLSN would always be ahead of the
   * persisted VLSN, since in general DTVLSN(vlsn) < vlsn. That is, the commit or abort log record
   * containing the DTVLSN always has a more recent VLSN than the one it contains.
   */
  private class DTVLSNFlusher {

    /**
     * The number of feeder ticks for which the in-memory DTVLSN must be stable before it's written
     * to the log as a null TXN. We are using this "tick" indirection to avoid yet another call to
     * the clock. A "tick" in this context is the FEEDER_MANAGER_POLL_TIMEOUT.
     */
    ...
    public DTVLSNFlusher() {
      final int heartbeatMs = repNode.getConfigManager().getInt(RepParams.HEARTBEAT_INTERVAL);
      targetStableTicks = (int) Math.max(1, (2 * heartbeatMs) / pollTimeoutMs);
    }

    /**
     * Used to track whether the DTVLSN has been stable enough to write out. While it's changing
     * application commits and aborts are writing it out, so no need to write it here.
     */
    private long stableDTVLSN = VLSN.NULL_VLSN_SEQUENCE;

    /**
     * Update each time we actually persist the DTVLSN via a null txn. It represents the DTVLSN
    ...
    /**
     * Persists the DTVLSN if necessary. The DTVLSN is persisted if the version in memory is more
     * current than the version on disk and has not changed for targetStableTicks.
     */
    void flush() {
      ...
      if (dtvlsn < stableDTVLSN) {
        ...
        throw new IllegalStateException(
            "The DTVLSN sequence cannot decrease"
                + "current DTVLSN:"
                + dtvlsn
                + " previous DTVLSN:"
                + stableDTVLSN);
      ...
      if (stableDTVLSN > persistedDTVLSN) {
        if (repNode.getActiveTransfer() != null) {
          ...
          LoggerUtils.info(
              logger,
              repNode.getRepImpl(),
              "Skipped null txn updating DTVLSN: " + dtvlsn + " Master transfer in progress");
          return;
        }
        final RepImpl repImpl = repNode.getRepImpl();
        final MasterTxn nullTxn =
            MasterTxn.createNullTxn(repImpl, NULL_TXN_CONFIG, repImpl.getNameIdPair());
        /*
        ...
        try {
          ...
          LoggerUtils.fine(
              ...
              "Persist DTVLSN: "
                  + dtvlsn
                  + " at VLSN: "
                  + nullTxn.getCommitVLSN()
                  + " via null transaction:"
                  + nullTxn.getId());
          ...
          LoggerUtils.warning(
              logger,
              repNode.getRepImpl(),
              "Failed to write null txn updating DTVLSN; " + e.getMessage());
        }
      }
    }
  }
