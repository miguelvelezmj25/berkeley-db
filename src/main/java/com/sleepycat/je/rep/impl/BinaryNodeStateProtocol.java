 ...
 *
 * <p>Because this protocol has to transfer byte array between two nodes, so instead of using the
 * former NodeStateProtocol, we introduce this new protocol which inherits from BinaryProtocol.
 *
 * <p>Note: once we support active version update, we can use one protocol only.
 *
 * <p>The message request sequence: NODE_STATE_REQ -> NODE_STATE_RESP
 */
public class BinaryNodeStateProtocol extends BinaryProtocol {
  ...
  public static final MessageOp BIN_NODE_STATE_REQ =
      new MessageOp((short) 1, BinaryNodeStateRequest.class);
  public static final MessageOp BIN_NODE_STATE_RESP =
  ...
  public class BinaryNodeStateResponse extends SimpleMessage {
    ...
    public BinaryNodeStateResponse(
        String nodeName,
        String groupName,
        String masterName,
        JEVersion jeVersion,
