...
import com.sleepycat.je.rep.impl.*;
...
import static com.sleepycat.je.rep.ReplicatedEnvironment.State.*;
import static com.sleepycat.je.rep.impl.RepParams.*;

/**
 * Represents a replication node. This class is the locus of operations that manage the state of the
 * node, master, replica, etc. Once the state of a node has been established the thread of control
 * passes over to the Replica or FeederManager instances.
 *
 * <p>Note that both Feeders and the Replica instance may be active in future when we support r2r
 * replication, in addition to m2r replication. For now however, either the FeederManager is active,
 * or the Replica is and the same common thread control can be shared between the two.
 */
public class RepNode extends StoppableThread {
  ...
  /**
   * Describes the nodes that form the group. This information is dynamic it's initialized at
   * startup and subsequently as a result of changes made either directly to it, when the node is a
   * master, or via the replication stream, when it is a Replica.
   *
   * <p>Always use the setGroup() method to set this iv, so that needsAck in particular is updated
   * in unison.
   */
  private volatile RepGroupImpl group;

  /**
   * Acks needed. Determines whether durability needs acknowledgments from other nodes, that is, the
   * rep group has more than one data node that's also electable.
   *
   * <p>Only update via the setGroup method.
   */
  ...
  /**
   * Synchronize on this object when setting the minimum JE version or adding a secondary node,
   * which could change the JE versions of the nodes to check when setting a new minimum.
   *
   * @see #setMinJEVersion
  ...
  /**
   * The in-memory DTVLSN. It represents the highest transaction known to have been replicated to a
   * majority of the Replicas.
   *
   * <p>At a master, knowledge of this replication state may have been communicated explicitly due
   * to the use of SIMPLE_MAJORITY or ALL ACKs, or it may have been communicated via a heartbeat
   * indicating the progress of replication at a replica.
   *
   * <p>At a replica, this state is obtained from commit/abort records in the replication stream.
   *
   * <p>This field is initialized from its persistent value whenever the environment is first
   * opened. It may be the null VLSN value for brand new environments. This value can only advance
   * as increasing numbers of transactions are acknowledged.
   *
   * @see <a href="https://<wikihost>/trac/wiki/JEReplicationDurableTxnVLSN"> DTVLSN</a>
   */
  private final AtomicLongMax dtvlsn = new AtomicLongMax(VLSN.NULL_VLSN_SEQUENCE);

  /**
   * If not null, a test hook that is called with the name of the current node during the query for
   * group membership before the node sleeps after failing to obtain information about the group
   * master -- for unit testing.
   */
  public static volatile TestHook<String> queryGroupForMembershipBeforeSleepHook;

  /**
   * If not null, called by queryGroupForMembership with the name of the current node before
   * querying learners for the master -- for unit testing.
   */
  public static volatile TestHook<String> queryGroupForMembershipBeforeQueryForMaster;

  /**
   * If not null, a test hook that is called with the name of the current node before attempting to
   * contact each network restore supplier, for unit testing.
   */
  public static volatile TestHook<String> beforeFindRestoreSupplierHook;

  public RepNode(RepImpl repImpl, Replay replay, NodeState nodeState)
    ...
    this.serviceDispatcher =
        new ServiceDispatcher(getSocket(), repImpl, repImpl.getChannelFactory());
  ...
  /**
   * Returns the accumulated statistics for this node. The method encapsulates the statistics
   * associated with its two principal components the FeederManager and the Replica.
   */
   ...
   *
   * <p>There is a very brief period of time, during node start-up, where this can be <code>null
   * </code>. But after that it should always return a valid object.
   */
  ...
  /**
   * Returns a definitive answer to whether this node is currently the master by checking both its
   * status as a master and whether the group agrees that it is the master.
   *
   * <p>Such an authoritative answer is needed in a network partition situation to detect a master
   * that may be isolated on the minority side of a network partition.
   *
   * @return true if the node is definitely the master. False if it's not or we cannot be sure.
   */
  ...
  public int getElectionPriority() {
    ...
    return (getConfigManager().getBoolean(RepParams.DESIGNATED_PRIMARY)
        ...
        ? defaultPriority + 1
        : /* Raise its priority. */ priority; /* Explicit priority, leave it intact. */
  }
  ...
  /**
   * Returns the latest VLSN associated with a replicated commit. Note that since the lastTxnEndVLSN
   * is computed outside the write log latch, via EnvironmentImpl.registerVLSN(LogItem) it's
   * possible for it to be behind on an instantaneous basis, but it will eventually catch up when
   * the updates quiesce.
   */
  ...
  /**
   * Returns the instantaneous non-null DTVLSN value. The value should be non null once
   * initialization has been completed.
   *
   * <p>The returned value can be VLSN.UNINITIALIZED_VLSN_SEQUENCE if the node is a replica in a
   * pre-dtvlsn log segment, or a master that has not as yet seen any acknowledged transactions.
   */
  ...
  /**
   * Updates the DTVLSN with a potentially new DTVLSN value. Note that this method is only invoked
   * when the node is a Master. The Replica simply sets the DTVLSN to a specific value.
   *
   * @param candidateDTVLSN the new candidate DTVLSN
   * @return the new DTVLSN which is either the candidatDTVLSN or a more recent DTVLSN >
   *     candidateDTVLSN
   */
  ...
  /**
   * Sets the DTVLSN to a specific value. This method is used exclusively by the Replica as it
   * maintains the DTVLSN based upon the contents of the replication stream.
   *
  ...
  /**
   * Starts up the thread in which the node does its processing as a master or replica. It then
   * waits for the newly started thread to transition it out of the DETACHED state, and returns upon
  ...
  /**
   * This method must be invoked when a RepNode is first initialized and subsequently every time
   * there is a change to the replication group.
   *
   * <p>The Master should invoke this method each time a member is added or removed, and a replica
   * should invoke it each time it detects the commit of a transaction that modifies the membership
   * database.
   *
   * <p>In addition, it must be invoked after a syncup operation, since it may revert changes made
   ...
   *
   * <p>Note that names referring to removed nodes cannot be reused.
   *
   * @param nodeName identifies the node to be removed
   * @throws MemberNotFoundException if the node denoted by <code>memberName</code> is not a member
   *     of the replication group.
   * @throws MasterStateException if the member being removed is currently the Master
   * @see <a
   *     href="https://sleepycat.oracle.com/trac/wiki/DynamicGroupMembership#DeletingMembers">Member
   *     Deletion</a>
   */
  ...
  /**
   * Remove or delete a node from the group. If deleting a node, the node must not be active.
   *
   * <p>Note that names referring to removed nodes cannot be reused, but names for deleted nodes can
   * be.
   *
   ...
   * @throws MemberActiveException if {@code delete} is {@code true} and the node is currently
   *     active
   * @throws MemberNotFoundException if the node denoted by <code>memberName</code> is not a member
   *     of the replication group.
   * @throws MasterStateException if the member being removed or deleted is currently the Master
   */
   ...
   *
   * <p>Note that an alive node's address can't be updated, we'll throw an ReplicaStateException for
   * this case.
   *
   ...
   *
   * <p>We delegate most of the real work to an instance of the {@link MasterTransfer} class. Here,
   * after some simple initial validity checking, we're concerned with coordinating the potential
   * for multiple overlapping Master Transfer operation attempts. The possible outcomes are:
   *
   * <ol>
   *   <li>complete success ({@code done == true})
   *       <ul>
   *         <li>don't unblock txns here; that'll happen automatically as part of the usual handling
   *             when the environment transitions from master->replica state.
   *         <li>don't clear xfrInProgress, because we don't want to allow another attempt to
   *             supersede
   *       </ul>
   ...
   *       <ul>
   *         <li>abort existing op (if permitted), unblock txns before unleashing the new one
   *         <li>replace xfrInProgress
   *       </ul>
   *   <li>env is closed (or invalidated because of an error) during the operation
   *       <ul>
   ...
   * @param force whether to replace any existing, in-progress transfer operation
   */
    ...
    for (String rep : replicas) {
      ...
      if (node == null || node.isRemoved()) {
        throw new IllegalArgumentException(
            "Node '" + rep + "' is not currently an active member of the group");
  ...
  /**
   * Sets up a Master Transfer operation, ensuring that only one operation can be in progress at a
   * time.
   */
  private synchronized MasterTransfer setUpTransfer(
      Set<String> replicas, long timeout, boolean force) {
  ...
  /**
   * Performs some basic validity checking, common code for some Group Membership operations.
   *
   * @param nodeName name of a replica node on which an operation is to be performed
   * @param actionName textual description of the operation (for exception message)
  ...
  private RepNodeImpl checkValidity(String nodeName, String actionName)
    ...
    if (node == null) {
      throw new MemberNotFoundException(
          "Node:" + nodeName + "is not a member of the group:" + group.getName());
    }
  ...
  /**
   * Updates the cached group info for the node, avoiding a database read, if the global CBVLSN is
   * not defunct. If it is defunct, does nothing.
   *
  ...
  /**
   * When the GlobalVLSN is not defunct, recalculates it. Provoked by Replay to ensure that the
   * replica's GlobalVLSN is up to date. When the GlobalVLSN is defunct, does nothing.
   */
  ...
  private void findMaster() throws DatabaseException {
    ...
    if ((thisNode == null)
      ...
      throw EnvironmentFailureException.unexpectedState(
          "Node: " + nameIdPair.getName() + " was previously deleted.");
    } else {
  ...
  /**
   * This method enforces the requirement that all addresses within a replication group, must be
   * loopback addresses or they must all be non-local ip addresses. Mixing them means that the node
   * with a loopback address cannot be contacted by a different node. Addresses specified by
   * hostnames that currently have no DNS entries are assumed to not be loopback addresses.
   *
   * @param helperSockets the helper nodes used by this node when contacting the master.
   */
  ...
  /**
   * Communicates with existing nodes in the group in order to figure out how to start up, in the
   * case where the local node does not appear to be in the (local copy of the) GroupDB, typically
   * because the node is starting up with an empty env directory. It could be that this is a new
   * node (never before been part of the group). Or it could be a pre-existing group member that has
   * lost its env dir contents and wants to be restored via a Network Restore operation.
   *
   * <p>We first try to find a currently running master node. (An authoritative master can easily
   * handle either of the above-mentioned situations.) If we can't find a master, we look for other
   * running nodes that may know of us (by asking them for their Group information).
   *
   * <p>We query the designated helpers and all known learners. The helpers are the ones that were
   * identified via the node's configuration, while the learners are the ones currently in the
   * member database. We use both in order to cast the widest possible net.
   *
   * <p>Returns normally when the master is found.
   *
   * @throws InterruptedException if the current thread is interrupted, typically due to a shutdown
   * @throws InsufficientLogException if the environment requires a network restore
  ...
  private void queryGroupForMembership() throws InterruptedException {
    ...
    while (true) {
      assert TestHookExecute.doHookIfSet(
          queryGroupForMembershipBeforeQueryForMaster, nameIdPair.getName());
    ...
    LoggerUtils.info(
        logger,
        repImpl,
        "New node " + nameIdPair.getName() + " located master: " + groupMasterNameId);
  }

  /**
   * Check that the master found by querying other group nodes is indeed alive and that we are not
   * dealing with obsolete cached information.
   *
   * @return true if the master node could be contacted and was truly alive
   *     <p>TODO: handle protocol version mismatch here and in DbPing, also consolidate code so that
   *     a single copy is shared.
   */
  private boolean checkGroupMasterIsAlive(NameIdPair groupMasterNameId) {
    ...
    try {
      ...
      protocol.write(
          protocol.new BinaryNodeStateRequest(groupMasterNameId.getName(), group.getName()),
          channel);
      ...
      LoggerUtils.info(
          logger, repImpl, "Queried master:" + groupMasterNameId + " unavailable. Reason:" + e);
      return false;
    } finally {
      if (channel != null) {
        try {
  ...
  /**
   * Sets up a Network Restore, as part of the process of restoring an entire group from backup, by
   * producing an appropriate {@code InsufficientLogException} if possible.
   *
   * <p>Queries each of the supplied helper hosts for their notion of the group make-up. If any of
   * them consider us to be already in the group, then instead of joining the group as a new node we
   * ought to try a Network Restore; and the node(s) that do already know of us are the suitable
   * suppliers for it.
   *
   * @throws InsufficientLogException in the successful case, if one or more suitable suppliers for
   *     a Network Restore can be found; otherwise just returns.
   * @throws InterruptedException if the node was shutdown or invalidated while we were looking for
   *     a network restore supplier
   */
  public void findRestoreSuppliers(Set<InetSocketAddress> helpers) throws InterruptedException {
    ...
    RepGroupProtocol protocol =
        new RepGroupProtocol(group.getName(), nameIdPair, repImpl, repImpl.getChannelFactory());
    ...
    for (InetSocketAddress helper : helpers) {
      ...
      MessageExchange msg =
          protocol
          .new MessageExchange(helper, GroupService.SERVICE_NAME, protocol.new GroupRequest());
      ...
      if (response == null || protocol.RGFAIL_RESP.equals(response.getOp())) {
        ...
        LoggerUtils.warning(
            logger, repImpl, "Expected GROUP_RESP, got " + response.getOp() + ": " + response);
        continue;
      }
  ...
  /**
   * Elects this node as the master. The operation is only valid when the group consists of just
   * this node, and when this is an ELECTABLE node.
   *
   * @throws DatabaseException
  ...
  private void selfElect() throws DatabaseException {
    ...
    if (!nodeType.isElectable()) {
      throw new IllegalStateException(
          "Cannot elect node "
              + nameIdPair.getName()
              + " as master because its node type, "
              + nodeType
              + ", is not ELECTABLE");
    }
  ...
  /**
   * Establishes this node as the master, after re-initializing the group with this as the sole node
   * in the group. This method is used solely as part of the DbResetRepGroup utility.
   *
   * @throws IllegalStateException if the node type is not ELECTABLE
   */
  ...
  /**
   * When a disk limit is violated, the node state will transition to UNKNOWN and wait for disk to
   * become available again before it transitions to a Replica or Master State. This method will not
   * exit until one of the following occurs:
   *
   * <p>1. The disk limit violation is cleared. 2. The node is shutdown or invalidated. 3. The
  ...
  private void waitWhileDiskLimitViolation() throws InterruptedException {
    ...
    LoggerUtils.info(
        ...
        "Node waiting for disk space to become available. "
            + "Disk limit violation:"
            + getRepImpl().getDiskLimitViolation());
  ...
  /**
   * The top level Master/Feeder or Replica loop in support of replication. It's responsible for
   * driving the node level state changes resulting from elections initiated either by this node, or
   * by other members of the group.
   *
   * <p>The thread is terminated via an orderly shutdown initiated as a result of an interrupt
   * issued by the shutdown() method. Any exception that is not handled by the run method itself is
   * caught by the thread's uncaught exception handler, and results in the RepImpl being made
   * invalid. In that case, the application is responsible for closing the Replicated Environment,
   * which will provoke the shutdown.
   *
   * <p>Note: This method currently runs either the feeder loop or the replica loop. With R to R
   * support, it would be possible for a Replica to run both. This will be a future feature.
   */
  @Override
  public void run() {
    ...
    try {
      LoggerUtils.info(
          ...
          "Node "
              + nameIdPair.getName()
              + " started"
              + (!nodeType.isElectable() ? " as " + nodeType : ""));
  ...
  /**
   * Update the information stored for the master in the RepGroupDB if storing it is supported and
   * the current version is different from the recorded version.
   */
  ...
  /**
   * Returns true if the node has been shutdown or if the underlying environment has been
   * invalidated. It's used as the basis for exiting the FeederManager or the Replica.
   */
  ...
  /**
   * Used to shutdown all activity associated with this replication stream. If method is invoked
   * from different thread of control, it will wait until the rep node thread exits. If it's from
   * the same thread, it's the caller's responsibility to exit the thread upon return from this
   * method.
   *
  ...
  public void shutdown() throws InterruptedException, DatabaseException {
    ...
    LoggerUtils.info(
        logger, repImpl, "Shutting down node " + nameIdPair + " DTVLSN:" + getAnyDTVLSN());
  ...
  /**
   * Soft shutdown for the RepNode thread. Note that since the thread is shared by the FeederManager
   * and the Replica, the FeederManager or Replica specific soft shutdown actions should already
   ...
   *
   * <p>Note that the method itself does not shutdown the group. It merely sets
   * replicaCloseCatchupMs, indicating that the ensuing handle close should shutdown the Replicas.
   * The actual coordination with the closing of the handle is implemented by
   * ReplicatedEnvironment.shutdownGroup().
   *
  ...
  public void shutdownGroupOnClose(long timeoutMs) throws IllegalStateException {
    ...
    if (!nodeState.getRepEnvState().isMaster()) {
      throw new IllegalStateException(
          "Node state must be " + MASTER + ", not " + nodeState.getRepEnvState());
    }
    replicaCloseCatchupMs = (timeoutMs < 0) ? 0 : timeoutMs;
  }

  /**
   * JoinGroup ensures that a RepNode is actively participating in a replication group. It's invoked
   * each time a replicated environment handle is created.
   *
   * <p>If the node is already participating in a replication group, because it's not the first
   * handle to the environment, it will return without having to wait. Otherwise it will wait until
   * a master is elected and this node is active, either as a Master, or as a Replica.
   *
   * <p>If the node joins as a replica, it will wait further until it has become sufficiently
   * consistent as defined by its consistency argument. By default it uses PointConsistencyPolicy to
   * ensure that it is at least as consistent as the master as of the time the handle was opened.
   *
   * <p>A node can also join in the Unknown state if it has been configured to do so via
   * ENV_UNKNOWN_STATE_TIMEOUT.
   *
   * @throws UnknownMasterException If a master cannot be established within ENV_SETUP_TIMEOUT,
   *     unless ENV_UNKNOWN_STATE_TIMEOUT has been set to allow the creation of a handle while in
   *     the UNKNOWN state.
   * @return MASTER, REPLICA, or UNKNOWN (if ENV_UNKNOWN_STATE_TIMEOUT is set)
   */
  public ReplicatedEnvironment.State joinGroup(
      ReplicaConsistencyPolicy consistency, QuorumPolicy initialElectionPolicy)
      throws DatabaseException {
    ...
    try {
      for (retries = 0; retries < JOIN_RETRIES; retries++) {
        try {
          ...
          boolean done =
              getReadyLatch().awaitOrException(timeouts.getTimeout(), TimeUnit.MILLISECONDS);
          ...
          switch (finalState) {
            ...
            case DETACHED:
              throw EnvironmentFailureException.unexpectedState(
                  "Node in DETACHED state " + "while joining group.");
          }
          ...
          LoggerUtils.warning(
              logger, repImpl, "Join retry due to master transition: " + e.getMessage());
          continue;
        } catch (RestartRequiredException e) {
          LoggerUtils.warning(
              logger, repImpl, "Environment needs to be restarted: " + e.getMessage());
          throw e;
        } catch (DatabaseException e) {
          ...
          if ((cause != null) && (cause.getClass() == Replica.ConnectRetryException.class)) {
            ...
            if (timeouts.getTimeout() > 0) {
              LoggerUtils.warning(
                  logger, repImpl, "Join retry due to exception: " + cause.getMessage());
              continue;
            }
          }
          throw e;
    ...
    if (exitException != null) {
      LoggerUtils.warning(
          logger, repImpl, "Exiting joinGroup after " + retries + " retries." + exitException);
      throw exitException;
    }
  ...
  /**
   * Join the group as a Replica ensuring that the node is sufficiently consistent as defined by its
   * consistency policy.
   *
  ...
  private void joinAsReplica(ReplicaConsistencyPolicy consistency) throws InterruptedException {
    ...
    if (consistency == null) {
      ...
      consistency =
          new PointConsistencyPolicy(
              new VLSN(replica.getMasterTxnEndVLSN()), consistencyTimeout, TimeUnit.MILLISECONDS);
    }
    ...
    LoggerUtils.info(
        logger,
        repImpl,
        "Joined group as a replica. "
            + " join consistencyPolicy="
            + consistency
            + " "
  ...
  /**
   * Should be called whenever a new VLSN is associated with a log entry suitable for Replica/Feeder
   * syncup. If GlobalCBVLSN is defunct, does nothing.
   */
  ...
  /**
   * Returns a VLSN appropriate for the RestoreResponse.cbvlsn field when the GlobalCBVLSN is not
   * defunct, and otherwise a null VLSN.
   *
  ...
  /**
   * Marks the start of the search for a matchpoint that happens during a syncup. The lower bound of
   * the VLSN range must remain stable during syncup to prevent deleting files that are being read
   * by a syncup file reader.
   *
   * <p>A feeder may have multiple syncups in action. The caller should call {@link #syncupEnded}
   * when the syncup is done, passing the value returned by this method.
   *
   * @param syncupNode identifies the other node involved in the the syncup, and is the name to be
   *     used in LogSizeStats.
   * @return the ProtectedFileSet protecting the VLSNIndex range and identifying the syncup in
   *     LogSizeStats.
   */
  public ProtectedFileSet syncupStarted(NameIdPair syncupNode) {

    return getVLSNIndex().protectRangeHead(FileProtector.SYNCUP_NAME + "-" + syncupNode.toString());
  }
  ...
  /**
   * Shuts down the Network backup service *before* a rollback is initiated as part of syncup, thus
   * ensuring that NetworkRestore does not see an inconsistent set of log files. Any network backup
   * operations that are in progress at this node are aborted. The client of the service will
   * experience network connection failures and will retry with this node (when the service is
   * re-established at this node), or with some other node.
   *
   * <p>restartNetworkBackup() is then used to restart the service after it was shut down.
   */
  ...
  /**
   * Restarts the network backup service *after* a rollback has been completed and the log files are
  ...
  /**
   * Dumps the states associated with any active Feeders as well as the composition of the group
   * itself.
   */
   ...
   *
   * <p>Returns the minimum JE version that is required for all nodes that join this node's
   * replication group. The version returned is supported by all current and future group members.
   * The minimum JE version is guaranteed to only increase over time, so long as the data for the
  ...
  /**
   * Checks if all data nodes in the replication group support the specified JE version. Updates the
   * group minimum JE version, and the group format version, as needed to require all nodes joining
   * the group to be running at least the specified JE version.
   *
   * <p>This method should only be called on the master, because attempts to update the rep group DB
   * on an replica will fail.
   *
   * @param newMinJEVersion the new minimum JE version
   * @throws DatabaseException if an error occurs when accessing the replication group database
   * @throws MinJEVersionUnsupportedException if the version is not supported by one or more current
   *     group members
   */
  ...
  /**
   * Returns true if acks are needed by the group for durability. This is the case if the rep group
   * has > 1 data node that's also electable.
   */
  ...
  /**
   * Adds a transient ID node to the group. Assign a node ID and add the node to the RepGroupImpl.
   * Don't notify the monitor: transient ID nodes do not generate GroupChangeEvents.
   *
   * @param node the node
   * @throws IllegalStateException if the store does not currently support secondary nodes or the
   *     node doesn't meet the current minimum JE version
   * @throws NodeConflictException if the node conflicts with an existing persistent node
   */
  ...
  /**
   * Removes a node with transient id from the group. Remove the node from the RepGroupImpl and
   * deallocate the node ID.
   *
   * @param node the node
   */
  ...
  /**
   * Track node IDs for node with transient IDs. IDs are allocated from the specified number of
   * values at the high end of the range of integers.
   */
  static class TransientIds {
    ...
    /** Allocates a free ID, throwing IllegalStateException if none are available. */
    ...
    /**
     * Deallocates a previously allocated ID, throwing IllegalArgumentException if the argument was
     * not allocated by allocateId or if the ID is not currently allocated.
     */
