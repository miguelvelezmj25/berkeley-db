...
import com.sleepycat.je.rep.utilint.*;
...
import static com.sleepycat.je.rep.impl.node.ReplicaStatDefinition.*;

/**
 * The Replica class is the locus of the replay operations and replica transaction consistency
 * tracking and management operations at a replica node.
 *
 * <p>A single instance of this class is created when the replication node is created and exists for
 * the lifetime of the replication node, although it is only really used when the node is operating
 * as a Replica.
 *
 * <p>Note that the Replica (like the FeederManager) does not have its own independent thread of
 * control; it runs in the RepNode's thread. To make the network I/O as aync as possible, and avoid
 * stalls during network I/O the input and output are done in separate threads. The overall thread
 * and queue organization is as sketched below:
 *
 * <p>read from network -> RepNodeThread (does read) -> replayQueue replayQueue -> ReplayThread ->
 * outputQueue outputQueue -> ReplicaOutputThread (does write) -> writes to network
 *
 * <p>This three thread organization has the following benefits over a single thread replay model:
 *
 * <p>1) It makes the heartbeat mechanism used to determine whether the HA sockets are in use more
 * reliable. This is because a heartbeat response cannot be blocked by lock contention in the replay
 * thread, since a heartbeat can be sent spontaneously (without an explicit heartbeat request from
 * the feeder) by the ReplicaOutputThread, if a heartbeat had not been sent during a heartbeat
 * interval period.
 *
 * <p>2) The cpu load in the replay thread is reduced by offloading the network-specific aspects of
 * the processing to different threads. It's important to keep the CPU load in this thread at a
 * minimum so we can use a simple single thread replay scheme.
 *
 * <p>3) Prevents replay thread stalls by input and output buffering in the two threads on either
 * side of it.
 *
 * <p>With jdk 1.7 we could eliminate the use of these threads and switch over to the new aysnc I/O
 * APIs, but that involves a lot more code restructuring.
 */
public class Replica {
  ...
  private static com.sleepycat.je.utilint.TestHook<Message> initialReplayHook;
  ...
  /**
   * The message queue used for communications between the network read thread and the replay
   * thread.
   */
  ...
  /**
   * The number of times a message entry could not be inserted into the queue within the poll period
  ...
  Replica(RepNode repNode, Replay replay) {
    ...
    dbCache =
        new DbCache(
            repImpl.getDbTree(),
            configManager.getInt(RepParams.REPLAY_MAX_OPEN_DB_HANDLES),
            configManager.getDuration(RepParams.REPLAY_DB_HANDLE_TIMEOUT));
    ...
    aggProtoStats =
        new StatGroup(
            BinaryProtocolStatDefinition.GROUP_NAME, BinaryProtocolStatDefinition.GROUP_DESC);
    ...
    final int replayQueueSize =
        repNode.getConfigManager().getInt(RepParams.REPLICA_MESSAGE_QUEUE_SIZE);

    replayQueue = new ArrayBlockingQueue<>(replayQueueSize);
  }

  /**
   * Shutdown the Replica, free any threads that may have been waiting for the replica to reach some
   * degree of consistency. This method is only invoked as part of the repnode shutdown.
   *
   * <p>If the shutdown is being executed from a different thread, it attempts to interrupt the
   * thread by first shutting down the channel it may be waiting on for input from the feeder. The
   * replica thread should notice the channel shutdown and/or the shutdown state of the rep node
   * itself. The caller will use harsher methods, like an interrupt, if the rep node thread (Replica
   * or Feeder) is still active.
   */
   ...
   * serving the role of a feeder, it will run additional feeder loops in separate threads. The loop
   * exits when it encounters one of the following possible conditions:
   *
   * <p>1) The connection to the master can no longer be maintained, due to connectivity issues, or
   * because the master has explicitly shutdown its connections due to an election.
   *
   * <p>2) The node becomes aware of a change in master, that is, assertSync() fails.
   *
   * <p>3) The loop is interrupted, which is interpreted as a request to shutdown the replication
   * node as a whole.
   *
   * <p>4) It fails to establish its node information in the master as it attempts to join the
   * replication group for the first time.
   *
   * <p>Normal exit from this run loop results in the rep node retrying an election and continuing
   * in its new role as determined by the outcome of the election. A thrown exception, on the other
   * hand, results in the rep node as a whole terminating its operation and no longer participating
   * in the replication group, that is, it enters the DETACHED state.
   *
   * <p>Note that the in/out streams are handled synchronously on the replica, while they are
   * handled asynchronously by the Feeder.
   *
   * @throws InterruptedException
   * @throws DatabaseException if the environment cannot be closed/for a re-init
  ...
  private void runReplicaLoopInternal()
      throws RestartRequiredException, InterruptedException, RetryException {

    shutdownException = null;
    LoggerUtils.info(
        logger,
        repImpl,
        "Replica loop started with master: " + repNode.getMasterStatus().getNodeMasterNameId());
    if (testDelayMs > 0) {
      LoggerUtils.info(
          logger, repImpl, "Test delay of: " + testDelayMs + "ms." + " after each message sent");
    }
    try {
      initReplicaLoop();
      ...
      LoggerUtils.severe(
          logger,
          repImpl,
          "Replica unexpected exception " + e + " " + LoggerUtils.getStackTrace(e));
      throw e;
  ...
  protected void doRunReplicaLoopInternalWork() throws Exception {
    ...
    try {
      while (true) {
        ...
        while (!replayQueue.offer(
            message, ReplayThread.QUEUE_POLL_INTERVAL_NS, TimeUnit.NANOSECONDS)) {
          /* Offer timed out. */
  ...
  /**
   * Process the shutdown message from the master and return the GroupShutdownException that must be
   * thrown to exit the Replica loop.
   *
  ...
  /**
   * Initialize for replica loop entry, which involves completing the following steps successfully:
   *
   * <p>1) The replica feeder handshake. 2) The replica feeder syncup. 3) Processing the first
   * heartbeat request from the feeder.
   */
  private void initReplicaLoop()
      throws IOException, ConnectRetryException, DatabaseException, ProtocolException,
          InterruptedException, HardRecoveryElectionException {
    ...
    ReplicaFeederSyncup syncup =
        new ReplicaFeederSyncup(
            repNode, replay, replicaFeederChannel, protocol, hardRecoveryNeedsElection);
    syncup.execute(repNode.getCBVLSNTracker());

    txnEndVLSN = syncup.getMatchedVLSN();
    long matchedTxnEndTime = syncup.getMatchedVLSNTime();
    consistencyTracker.reinit(txnEndVLSN.getSequence(), matchedTxnEndTime);
    Protocol.Heartbeat heartbeat =
        protocol.read(replicaFeederChannel.getChannel(), Protocol.Heartbeat.class);
  ...
  /**
   * Process a heartbeat message. It queues a response and updates the consistency tracker with the
   * information in the heartbeat.
   *
   * @param heartbeat the heartbeat message
   * @throws IOException
   */
  private void processHeartbeat(Heartbeat heartbeat) throws IOException {
    ...
    if (shutdownException != null) {
      if (shutdownException instanceof RetryException) {
        LoggerUtils.info(
            logger,
            repImpl,
            "Retrying connection to feeder. Message: " + shutdownException.getMessage());
      } else if (shutdownException instanceof GroupShutdownException) {
        LoggerUtils.info(
            logger, repImpl, "Exiting inner Replica loop." + " Master requested shutdown.");
      } else {
        LoggerUtils.warning(
            logger,
            repImpl,
  ...
  /**
   * Invoked when this node transitions to the master state. Aborts all inflight replay transactions
   * outstanding from a previous state as a Replica, because they were initiated by a different
   * master and will never complete. Also, release any Replica transactions that were waiting on
  ...
  void masterTransitionCleanup() throws DatabaseException {
    ...
    consistencyTracker.forceTripLatches(
        new MasterStateException(repNode.getRepImpl().getStateChangeEvent()));
  }

  /**
   * Invoked when this node seamlessly changes roles from master to replica without a recovery. The
   * ability to do this transition without a recovery is desirable because it's a faster transition,
   * and avoids the GC overhead of releasing the JE cache, and the I/O overhead of recreating the
   * in-memory btree.
   *
   * <p>The two key cases where this happens are: A) a network partition occurs, and the group
   * elects a new master. The orphaned master did not crash and its environment is still valid, and
   * when it regains contact with the group, it discovers that it has been deposed. It transitions
   * into a replica status.
   *
   * <p>B) a master transfer request moves mastership from this node to another member of the group.
   * This node's environment is still valid, and it transitions to replica state.
   *
   * <p>The transition from master to replica requires resetting state so all is as expected for a
   * Replica. There are two categories of work: - network connections: shutting down feeder
   * connections and reinitializing feeder infrastructure so that a future replica->master
   * transition will work. - resetting transaction state. All MasterTxns must be transformed into
   * ReplayTxns, bearing the same transaction id and holding the same locks.
   *
   * <p>Note: since non-masters can't commit txns, the inflight MasterTxns are destined to be
   * aborted in the future. An alternative to resetting transaction state would be to mark them in
   * some way so that the later HA syncup/ replay ignores operations pertaining to these ill fated
   * txns. We didn't chose that approach because the simplicity of the replay is a plus; it is
   * almost entirely ignorant of the semantics of the replication stream. Also, replays have
   * potential for complexity, either because syncups could restart if masters change or become
   * unavailable, or because there may be future performance optimizations in that area.
   *
   * <p>Resetting transaction state is tricky because the MasterTxn is accessible to the application
   * code. While the Replay thread is attempting to transform the MasterTxn, application threads may
   * be attempting to commit or abort the transactions. Note that application threads will not be
   * trying to add locks, because the node will be in UNKNOWN state, and writes will be prohibited
   * by the MasterTxn.
   *
   * <p>MasterTransfers do impose a blocking period on transaction commits and aborts, but even
   * there, windows exist in the post-block period where the application may try to abort the
   * transaction. Network partitions do no form of blocking, and have a wider window when the
   * application and RepNode thread must be coordinated. Here's a diagram of the time periods of
   * concern
   *
   * <p>t1 - master transfer request issued (only when master transfer) t2 - user txns which attempt
   * to abort or commit are blocked on RepImpl.blockTxnLatch (only when mt) t3 - node detects that
   * it has transitioned to UNKNOWN and lost master status. MasterTxns are now stopped from
   * acquiring locks or committing and will throw UnknownMasterException. t4 - feeder connections
   * shutdown t5 - node begins conversion to replica state t6 - blockTxnLatch released (only when
   * master transfer) t7 - existing MasterTxns converted into ReplayTxns, locks moved into new
   * ReplayTxns. Blocked txns must be released before this conversion, because the application
   * thread is holding the txn mutex, and conversion needs to take that mutex.
   *
   * <p>At any time during this process, the application threads may attempt to abort or commit
   * outstanding txns, or acquire read or write locks. After t3, any attempts to lock, abort or
   * commit will throw an UnknownMasterException or ReplicaWriteException, and in the normal course
   * of events, the txn would internally abort. But once t5 is reached, we want to prevent any
   * changes to the number of write locks in the txn so as to prevent interference with the
   * conversion of the master txns and any danger of converting only part of a txn. We set the
   * volatile, transient MasterTxn.freeze field at t5 to indicate that there should be no change to
   * the contents of the transaction. When freeze is true, any attempts to abort or commit the
   * transaction will throw Unknown/ReplicaWriteException, and the txn will be put into MUST_ABORT
   * state, but the existing locks will be unchanged.
   *
   * <p>In a network partition, it's possible that the txn will be aborted or committed locally
   * before t5. In that case, there may be a hard rollback when the node syncs up with the new
   * master, and finds the anomalous abort record. In masterTransfer, the window is smaller, and the
   * blocking latch ensures that no commits can happen bettween t1-t5. After t5, the node will not
   * be a master, so there can be no commits. Aborts may happen and can cause hard rollbacks, but no
   * data will be lost.
   *
   * <p>The freeze field is similar to the blockTxnLatch, and we considered using the blockTxnLatch
   * to stabilize the txns, but ruled it out because: - the locking hierarchy where the application
   * thread holds the txn mutex while awaiting the block txn latch prevents txn conversion. - the
   * blockTxnLatch is scoped to the MasterTransfer instance, which may not be in play for network
   * partitioning.
   */
  void replicaTransitionCleanup() {
    ...
    if (repImpl.getState() == State.MASTER) {
      throw EnvironmentFailureException.unexpectedState(
          repImpl,
          "Should not be in MASTER state when converting from master " + "to replica state");
    }
    ...
    LoggerUtils.info(
        logger,
        repImpl,
        "Transitioning node to replica state, " + existingMasterTxns.size() + " txns to clean up");
    ...
    for (MasterTxn masterTxn : existingMasterTxns) {
       ...
       */
      ReplayTxn replayTxn = masterTxn.convertToReplayTxnAndClose(logger, repImpl.getReplay());

      if (replayTxn == null) {
        LoggerUtils.info(
            logger,
            repImpl,
            "Master Txn " + masterTxn.getId() + " has no locks, nothing to transfer");
      } else {
        repImpl.getTxnManager().registerTxn(replayTxn);
        LoggerUtils.info(
            logger,
            repImpl,
            "state for replay transaction " + replayTxn.getId() + " = " + replayTxn.getState());
      }
    }
  ...
  /**
   * Returns a channel used by the Replica to connect to the Feeder. The socket is configured with a
   * read timeout that's a multiple of the heartbeat interval to help detect, or initiate a change
   * in master.
   *
   * @throws IOException
  ...
  private void createReplicaFeederChannel() throws IOException, ConnectRetryException {
    ...
    final int receiveBufferSize = configManager.getInt(RepParams.REPLICA_RECEIVE_BUFFER_SIZE);
    ...
    try {
      ...
      final ConnectOptions connectOpts =
          new ConnectOptions()
              .setTcpNoDelay(true)
      ...
      dataChannel =
          repImpl
              .getChannelFactory()
              .connect(repNode.getMasterStatus().getNodeMaster(), connectOpts);

      replicaFeederChannel = new NamedChannelWithTimeout(repNode, dataChannel, timeoutMs);

      ServiceDispatcher.doServiceHandshake(dataChannel, FeederManager.FEEDER_SERVICE);
      ...
      if (e.getResponse() == Response.UNKNOWN_SERVICE) {
        throw new ConnectRetryException(
            e.getMessage(), SERVICE_UNAVAILABLE_RETRIES, CONNECT_RETRY_SLEEP_MS);
      }
      throw EnvironmentFailureException.unexpectedException(e);
    }
  }
   ...
   * Reading and replay are done in separate threads for two reasons:
   *
   * <p>1) It allows the two activities to make independent progress. The network can be read and
   * messages assembled even if the replay activity has stalled. 2) The two threads permit use of
   * two cores to perform the replay thus making it less likely that cpu is the replay bottleneck.
   *
   * <p>The inputs and outputs of this thread are schematically described as:
   *
   * <p>replayQueue -> ReplayThread -> outputQueue
   *
   * <p>It's the second component of the three thread structure outlined in the Replica's class
   * level comment.
   */
  class ReplayThread extends StoppableThread {

    /**
     * Thread exit exception. It's null if the thread exited due to an exception. It's the
     * responsibility of the main replica thread to propagate the exception across the thread
    ...
    public void run() {
      ...
      LoggerUtils.info(
          logger,
          repImpl,
          "Replay thread started. Message queue size:" + replayQueue.remainingCapacity());
      ...
      try {
        while (true) {
          ...
          final long pollIntervalNs = replay.getPollIntervalNs(QUEUE_POLL_INTERVAL_NS);

          final Message message = replayQueue.poll(pollIntervalNs, TimeUnit.NANOSECONDS);

          if ((exitRequest == ReplayExitType.IMMEDIATE)
              || ((exitRequest == ReplayExitType.SOFT) && (message == null))
         ...
         */
        LoggerUtils.info(logger, repImpl, "closing replicaFeederChannel = " + replicaFeederChannel);
        RepUtils.shutdownChannel(replicaFeederChannel);

        LoggerUtils.info(logger, repImpl, "Replay thread exiting with exception:" + e.getMessage());
      }
    }
  ...
  private class RepFeederHandshakeConfig implements ReplicaFeederHandshakeConfig {
  ...
  /**
   * Tracks the consistency of this replica wrt the Master. It provides the mechanisms that will
   * cause a beginTransaction() or a joinGroup() to wait until the specified consistency policy is
   * satisfied.
   */
  public class ConsistencyTracker {
    ...
    private final StatGroup stats =
        new StatGroup(ReplicaStatDefinition.GROUP_NAME, ReplicaStatDefinition.GROUP_DESC);
    ...
    /**
     * Invoked each time after a replica syncup so that the Replica can re-establish it's
     * consistency vis a vis the master and what part of the replication stream it considers as
     * having been replayed.
     *
     * @param matchedTxnVLSN the replica state corresponds to this txn
     * @param matchedTxnEndTime the time at which this txn was committed or aborted on the master
     */
      ...
      if (logger.isLoggable(Level.INFO)) {
        LoggerUtils.info(
            ...
            "Replica stats - Lag waits: "
                ...
                + " Lag wait time: "
                + nLagConsistencyWaitMs.get()
                + "ms. "
                + " VLSN waits: "
                + nVLSNConsistencyWaits.get()
                + " Lag wait time: "
                + nVLSNConsistencyWaitMs.get()
                + "ms.");
      }
    }
     ...
     *
     * @param exception the exception to be thrown to explain the reason behind the latches being
     *     forced.
     */
    ...
    synchronized void trackTxnEnd() {
      ...
      if ((lastReplayedTxnVLSN > masterTxnEndVLSN) && (masterTxnEndTime >= masterNow)) {
    ...
    public void awaitVLSN(long vlsn, ReplicaConsistencyPolicy consistencyPolicy)
        throws InterruptedException, DatabaseException {
     ...
     */
    private void await(
        ExceptionAwareCountDownLatch consistencyLatch, ReplicaConsistencyPolicy consistencyPolicy)
        throws DatabaseException, InterruptedException {

      if (!consistencyLatch.awaitOrException(
          consistencyPolicy.getTimeout(TimeUnit.MILLISECONDS), TimeUnit.MILLISECONDS)) {
        /* Timed out. */
    ...
    /**
     * Shutdown the consistency tracker. This is typically done as part of the shutdown of a
     * replication node. It counts down all open latches, so the threads waiting on them can make
     * progress. It's the responsibility of the waiting threads to check whether the latch countdown
     * was due to a shutdown, and take appropriate action.
     */
    public void shutdown() {
      final Exception savedShutdownException = repNode.getSavedShutdownException();
      ...
      final EnvironmentFailureException latchException =
          (savedShutdownException instanceof EnvironmentFailureException)
              ? ((EnvironmentFailureException) savedShutdownException)
              : EnvironmentFailureException.unexpectedException(
                  "Node: " + repNode.getNameIdPair() + " was shut down.", savedShutdownException);
  ...
  private abstract class OrderedLatches {
    ...
    final SortedMap<Long, ExceptionAwareCountDownLatch> latchMap = new TreeMap<>();
     ...
     *
     * @param tripValue
     * @param exception the exception to be thrown by the waiter upon exit from the await. It can be
     *     null if no exception need be thrown.
     */
  ...
  /**
   * Thrown to indicate that the Replica must retry connecting to the same master, after some period
   * of time.
   */
  @SuppressWarnings("serial")
  abstract static class RetryException extends Exception {
    final int retries;
  ...
  /**
   * Indicates that an election is needed before the hard recovery can proceed. Please see SR 20572
   * for a motivating scenario and NetworkPartitionHealingTest for an example.
   */
  @SuppressWarnings("serial")
  public static class HardRecoveryElectionException extends Exception {
    ...
    public HardRecoveryElectionException(
        NameIdPair masterNameIdPair, VLSN lastTxnEnd, VLSN matchpointVLSN) {
   ...
   * This is needed when the test hook must be installed before the {@code ReplicatedEnvironment}
   * handle constructor returns, so that a test may influence the replay of the sync-up transaction
   * backlog.
   */
  public static void setInitialReplayHook(com.sleepycat.je.utilint.TestHook<Message> hook) {
    initialReplayHook = hook;
  }

  /**
   * Set a test hook which is executed when the ReplicaFeederSyncup finishes. This differs from the
   * static method ReplicaFeederSyncup.setGlobalSyncupHook in that it sets the hook for a specific
   * node, whereas the other method is static and sets it globally.
   *
   * <p>This method is required when a test is trying to set the hook for only one node, and the
   * node already exists. The other method is useful when a test is trying to set the hook before a
   * node exists.
   */
