...
/**
 * Represents a snapshot of the Replication Group as a whole. Note that membership associated with a
 * group is dynamic and its constituents can change at any time. It's useful to keep in mind that
 * due to the distributed nature of the Replication Group all the nodes in a replication group may
 * not have the same consistent picture of the replication group at a single point in time, but will
 * converge to become consistent eventually.
 */
public class RepGroupImpl {
  ...
  /**
   * Format version introduced in JE 6.0.1 that records a node's most recent JE version, and the
   * minimum JE version required to join the group.
   */
   ...
   * This number of transient id node IDs will be reserved at the top of the node ID range.
   */
  ...
  private static final int NODE_SEQUENCE_MAX = Integer.MAX_VALUE - MAX_NODES_WITH_TRANSIENT_ID;

  /** Returns true if the node is electable. */
  private static final Predicate ELECTABLE_PREDICATE =
      new Predicate() {
        @Override
  ...
  private static final Predicate MONITOR_PREDICATE =
      new Predicate() {
        @Override
        boolean include(final RepNodeImpl n) {
  ...
  private static final Predicate SECONDARY_PREDICATE =
      new Predicate() {
        @Override
        boolean include(final RepNodeImpl n) {
  ...
  private static final Predicate EXTERNAL_PREDICATE =
      new Predicate() {
        @Override
        boolean include(final RepNodeImpl n) {
  ...
  private static final Predicate ACK_PREDICATE =
      new Predicate() {
        @Override
        boolean include(final RepNodeImpl n) {
  ...
  private static final Predicate ARBITER_PREDICATE =
      new Predicate() {
        @Override
        boolean include(final RepNodeImpl n) {
   ...
   * @param currentJEVersion if non-null, override the current JE version, for testing
   */
  ...
  /**
   * Constructor to create a group and specify if the group's UUID should be unknown or generated
   * randomly.
   */
  ...
  /**
   * Constructor used to recreate an existing RepGroup, typically as part of a deserialization
   * operation.
   *
   * @param groupName
   * @param uuid
    ...
    if (formatVersion < MIN_FORMAT_VERSION || formatVersion > MAX_FORMAT_VERSION) {
      throw new IllegalStateException(
          "Expected membership database format version between: "
              ...
              + " and "
              + MAX_FORMAT_VERSION
              + ", encountered unsupported version: "
              + formatVersion);
    }
  ...
  /**
   * Removes a member transiently from the rep group by marking it as removed and optionally
   * deleting it from the by-name and by-ID maps. This action is usually a precursor to making the
   * change persistent on disk.
   *
   * @param nodeName identifies the node being removed
   * @param delete whether to delete the node from the maps
   * @return the node that was removed
   * @throws EnvironmentFailureException if the node is not part of the group or is a node with a
   *     transient ID
   */
  ...
  /**
   * Checks for whether a new or changed node definition is in conflict with other members of the
   * group. In particular, checks that the specified node does not use the same socket address as
   * another member.
   *
   * <p>This check must be done when adding a new member to the group, or changing the network
   * address of an existing member, and must be done with the rep group entry in the database locked
   * for write to prevent race conditions.
   *
   * @param node the new node that is being checked for conflicts
   * @throws NodeConflictException if there is a conflict
   */
  public void checkForConflicts(RepNodeImpl node) throws DatabaseException {
  ...
  /**
   * Sets the nodes associated with the Rep group. Note that both nodesById and nodesByIndex are
   * initialized.
   */
  public void setNodes(final Map<Integer, RepNodeImpl> nodes) {
    ...
    synchronized (nodesById) {
      ...
      assert new HashSet<>(nodesById.values()).equals(new HashSet<>(nodesByName.values()))
  ...
  /**
   * Add a node with transient id. The caller should already have assigned the node an ID and
   * checked that the replication group supports secondary nodes.
   *
   * @param node the node with transient id
   * @throws IllegalStateException if the store does not currently support secondary nodes
  ...
  public void removeTransientNode(final RepNodeImpl node) {
    ...
    if (node.getNameIdPair().hasNullId()) {
      throw new IllegalArgumentException(
          "Attempt to call removeTransientNode on a node with no ID: " + node);
    }
    synchronized (nodesById) {
      nodesById.remove(node.getNodeId());
      nodesByName.remove(node.getName());
  ...
  /**
   * Returns the version of the instance as represented by changes to the members constituting the
   * group.
   *
  ...
  /**
   * Increments the object change version. It must be called with the group entry locked in the
   * group database.
   *
  ...
  /**
   * Set the node id sequence. This is only done in unusual circumstances, e.g. when a replication
   * group is being reset in an existing replicated environment and we want to ensure that the
   * internal node ids are not reused in the logs.
   */
   ...
   * The group object should have had its nodes fetched using the {@link RepGroupDB#fetchGroup}
   * method and should be stored to the group database after making this change. Throws a {@link
   * MinJEVersionUnsupportedException} if the requested version is not supported. Updates the group
   * format version as needed to match the JE version. Has no effect if the current minimum value is
   * already as high or higher than the requested one.
   *
   * @param newMinJEVersion the new minimum JE version
   * @throws MinJEVersionUnsupportedException if the requested version is not supported by the
   *     group's electable nodes
   */
  ...
  /**
   * Used to ensure that the ReplicationGroup value is consistent after it has been fetched via a
   * readUncommitted access to the rep group database. It does so by ensuring that the summarized
   * values match the nodes that were actually read.
   */
  ...
  /**
   * Serializes an object by converting its TupleBinding byte based representation into the hex
   * characters denoting the bytes.
   *
  ...
  /**
   * Returns a serialized character based form of the group suitable for use in subclasses of
   * SimpleProtocol. The serialized form is a multi-token string. The first token represents the
   * RepGroup object itself with each subsequent node representing a node in the group. Tokens are
   * separated by '|', the protocol separator character. The number of tokens is thus equal to the
   * number of nodes in the group + 1. Each token is itself a hex character based representation of
   * the binding used to serialize a RepGroup and store it into the database.
   *
  ...
  /**
   * Returns the serialized form of the node as a sequence of hex characters suitable for use by the
  ...
  /**
   * Carries out the two step de-serialization from hex string into a byte buffer and subsequently
  ...
  /**
   * De-serializes an array of tokens into a Rep group object and its nodes. the token at <code>
   * start</code>represents the group object and each subsequent token represents a node in the
   * group.
   *
   * @param tokens the array representing the group and its nodes
   * @param start the position in the array at which to start the de-serialization.
  ...
  /**
   * Returns the node IDs for all nodes that are currently members of the group and that act as
   * proposers, acceptors, or distinguished learners. Returns IDs for all ELECTABLE and MONITOR
   * nodes that are not removed, even if they are not acknowledged, but not for SECONDARY or
   * EXTERNAL nodes.
   */
  ...
  /**
   * Returns all nodes that are currently members of the group. Returns all ELECTABLE and MONITOR
   * nodes that are acknowledged and not removed, and SECONDARY and EXTERNAL nodes. If the predicate
   * is not null, only includes members that satisfy the predicate.
   */
  ...
  /**
   * Adds all nodes that are currently members of the group to the specified set. Adds all ELECTABLE
   * and MONITOR nodes that are not removed, even if they are not acknowledged, and SECONDARY and
   * EXTERNAL nodes. If the predicate is not null, only adds members that satisfy the predicate.
   */
  ...
  /**
   * Counts the number of nodes that are currently members of the group. Counts all ELECTABLE and
   * MONITOR nodes that are not removed, even if they are not acknowledged, and SECONDARY and
   * EXTERNAL nodes. If the predicate is not null, only counts members that satisfy the predicate.
   */
  ...
  /**
   * Adds nodes that are currently members of the group to the specified set. Adds ELECTABLE and
   * MONITOR node that are not removed and are acknowledged, and SECONDARY and EXTERNAL nodes. If
   * the predicate is not null, only adds members that satisfy the predicate.
   */
   ...
   * Returns ELECTABLE and MONITOR nodes that are not removed, even if it is not acknowledged, and
   * SECONDARY and EXTERNAL nodes.
   *
   * @return the desired node, or null if there is no such node, including if it was removed
   */
  ...
  /**
   * Returns nodes that are removed from the group. Returns ELECTABLE and MONITOR nodes that are
   * removed and are acknowledged, but not SECONDARY or EXTERNAL nodes, which are not remembered
  ...
  /**
   * Returns all electable nodes that are currently members of the group. Returns all ELECTABLE
   * nodes that are not removed, even if they are not acknowledged, but not MONITOR, SECONDARY, or
   * EXTERNAL nodes.
   */
  ...
  /**
   * Returns electable nodes that are currently members of the group. Returns ELECTABLE nodes that
   * are not removed and are acknowledged, but not MONITOR, SECONDARY, or EXTERNAL nodes.
   */
  ...
  /**
   * Adds the electable nodes that are currently members of the group to the specified set. Adds
   * ELECTABLE nodes that are not removed and are acknowledged, but not MONITOR, SECONDARY, or
   * EXTERNAL nodes.
   */
  ...
  /**
   * Returns the nodes that are currently members of the group that store replication data. Returns
   * ELECTABLE nodes that are not removed and are acknowledged, and SECONDARY nodes, but not MONITOR
   * or EXTERNAL nodes.
   */
  ...
  /**
   * Adds the nodes that are currently members of the group that store replication data to the
   * specified set. Adds ELECTABLE nodes that are not removed and are acknowledged, and SECONDARY
   * nodes, but not MONITOR or EXTERNAL nodes.
   */
  ...
  /**
   * Returns the monitor nodes that are currently members of the group. Returns MONITOR nodes that
   * are not removed and are acknowledged, but not ELECTABLE, SECONDARY, or EXTERNAL nodes.
   *
  ...
  /**
   * Adds the monitor nodes that are currently members of the group to the specified set. Adds
   * MONITOR nodes that are not removed and are acknowledged, but not ELECTABLE, SECONDARY, or
   * EXTERNAL nodes.
   */
  ...
  /**
   * Returns all the nodes that are currently members of the group that act as distinguished
   * learners to receive election results. Returns all ELECTABLE and MONITOR that are not removed,
   * even if they are not acknowledged, but not SECONDARY or EXTERNAL nodes.
   */
   ...
   *
   * <p>Returns SECONDARY nodes, but not ELECTABLE, MONITOR, or EXTERNAL nodes.
   */
   ...
   *
   * <p>Returns EXTERNAL nodes, but not ELECTABLE, MONITOR, or SECONDARY nodes.
   */
  ...
  /**
   * Adds the secondary nodes that are currently members of the group to the specified set. Adds
   * SECONDARY nodes, but not ELECTABLE, MONITOR, or EXTERNAL nodes.
   */
  ...
  /**
   * Adds the external nodes. Adds EXTERNAL nodes, but not ELECTABLE, MONITOR, or SECONDARY nodes.
   */
  public void includeExternalMembers(final Set<? super RepNodeImpl> set) {
    includeAllMembers(EXTERNAL_PREDICATE, set);
  }

  /** Returns the arbiter nodes that are currently members of the group. Returns ARBITER nodes. */
  ...
  /**
   * Adds the arbiter nodes that are currently members of the group to the specified set. Adds
   * ARBITER nodes.
   */
  ...
  /**
   * Returns the socket addresses for all nodes that are currently members of the group. Returns
   * addresses for all ELECTABLE and MONITOR nodes that are not removed, even if they are not
   * acknowledged, and for all nodes with transient id. If the predicate is not null, only returns
   * addresses for members that satisfy the predicate. ARBITER nodes are also ELECTABLE and will be
  ...
  /**
   * Return the socket addresses for all nodes that are currently members of the group and act as
   * distinguished learners to receive election results. Returns addresses for all ELECTABLE and
   * MONITOR nodes that are not removed, even if they are not acknowledged, but not for SECONDARY or
   * EXTERNAL nodes.
   *
  ...
  /**
   * Return the socket addresses for all nodes that are currently members of the group and act as
   * helpers to supply election results. Returns addresses for all ELECTABLE and MONITOR nodes that
   * are not removed, even if they are not acknowledged, and SECONDARY nodes.
   *
   ...
   * Returns addresses for all MONITOR nodes that are not removed, even if they are not
   * acknowledged, but not for ELECTABLE, SECONDARY, or EXTERNAL nodes.
   *
  ...
  /**
   * Returns the socket addresses for all nodes that are currently members of the group and act as
   * acceptors for elections. Returns addresses for all ELECTABLE nodes that are not removed, even
   * if they are not acknowledged, but not for MONITOR, SECONDARY, or EXTERNAL nodes, which do not
   * act as acceptors.
   *
  ...
  /**
   * Returns the node with the specified ID that is currently a member of the group, throwing an
   * exception if the node is found but is no longer a member. Returns ELECTABLE and MONITOR nodes
   * that are not removed, even if they are not acknowledged, and SECONDARY and EXTERNAL nodes.
   *
  ...
  /**
   * Returns the node with the specified name that is currently a member of the group, throwing an
   * exception if the node is found but is no longer a member. Returns ELECTABLE and MONITOR nodes
   * that are not removed, even if they are not acknowledged, and SECONDARY and EXTERNAL nodes.
   *
  ...
  /**
   * Returns the node with the specified ID, regardless of its membership state. Returns all
   * ELECTABLE and MONITOR nodes, even if they are removed or are not acknowledged, and SECONDARY
   * and EXTERNAL nodes.
   *
  ...
  /**
   * Returns the node with the specified name, regardless of its membership state. Returns all
   * ELECTABLE and MONITOR nodes, even if they are removed or are not acknowledged, and SECONDARY
   * and EXTERNAL nodes.
   *
   * @return the node or null
  ...
  /**
   * Returns the number of all electable nodes that are currently members of the group. Includes all
   * ELECTABLE nodes that are not removed, even if they are not acknowledged, but not MONITOR,
   * SECONDARY, or EXTERNAL nodes. Note that even unACKed nodes are considered part of the group for
   ...
   * Returns the number of all electable nodes that are currently members of the group. Includes all
   * ELECTABLE nodes that are not removed, even if they are not acknowledged, but not MONITOR,
   * ARBITER, SECONDARY, or EXTERNAL nodes. Note that even unACKed nodes are considered part of the
  ...
  public String toString() {
    ...
    sb.append(getUUID())
        .append("\n Format version: ")
        .append(getFormatVersion())
        .append("\n Change version: ")
        .append(getChangeVersion())
        .append("\n Max persist rep node ID: ")
        .append(getNodeIdSequence())
        .append("\n Min JE version: ")
        .append(minJEVersion)
        .append("\n");
