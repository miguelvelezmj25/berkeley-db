...
/**
 * The LogFileFeeder supplies log files to a client. There is one instance of this class per client
 * that's currently active. LogFileFeeders are created by the FeederManager and exist for the
 * duration of the session with the client.
 */
public class LogFileFeeder extends StoppableThread {
  ...
  private void sendRequestedFiles(Protocol protocol)
    ...
    try {
      while (true) {
        ...
        FileInfoResp cachedResp = feederManager.statResponses.get(fileName);
        byte[] cachedDigest =
            ((cachedResp != null)
   ...
   * message to maintain its position in the network stream. It expects to see a FileInfoResp once
   * it has read the agreed upon number of bytes.
   *
  ...
  private void sendFileList(Protocol protocol)
    ...
    for (int i = 0; i < files.length; i++) {
      if (files[i].contains(File.separator)) {
        files[i] = files[i].substring(files[i].indexOf(File.separator) + 1);
      }
    }
