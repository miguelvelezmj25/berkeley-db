...
/**
 * The thread used to write responses asynchronously to the network, to avoid network stalls in the
 * replica replay thread. This thread, like the Replica.ReplayThread, is created each time the node
 * establishes contact with a new feeder and starts replaying the log from it.
 *
 * <p>The inputs and outputs of this thread are schematically described as:
 *
 * <p>outputQueue -> ReplicaOutputThread (does write) -> writes to network
 *
 * <p>It's the third component of the three thread structure outlined in the Replica's class level
 * comment.
 */
public abstract class ReplicaOutputThreadBase extends StoppableThread {
  ...
  /**
   * Thread exit exception. It's non-null if the thread exited due to an exception. It's the
   * responsibility of the main replica thread to propagate the exception across the thread boundary
   * in this case.
   */
  ...
  public ReplicaOutputThreadBase(
      RepImpl repImpl,
  ...
  public void run() {
    ...
    try {
      for (Long txnId = outputQueue.poll(heartbeatMs, TimeUnit.MILLISECONDS);
          !EOF.equals(txnId);
          txnId = outputQueue.poll(heartbeatMs, TimeUnit.MILLISECONDS)) {
   ...
   *
   * <p>Returns true if it encountered an EOF or a request for a shutdown.
   */
  protected boolean groupWriteAcks(long txnId) throws IOException {
