...
import static com.sleepycat.je.log.LogEntryType.*;
import static com.sleepycat.je.rep.impl.node.ReplayStatDefinition.*;
 ...
 *
 * <p>The Replay module has a lifetime equivalent to the environment owned by this replicator. Its
 * lifetime is longer than the feeder/replica stream. For example, suppose this is nodeX:
 *
 * <p>t1 - Node X is a replica, node A is master. Replay X is alive t2 - Node X is a replica, node B
 * takes over as master. X's Replay module is still alive and has the same set of active txns. It
 * doesn't matter to X that the master has changed. t3 - Node X becomes the master. Now its Replay
 * unit is cleared, because anything managed by the Replay is defunct.
 */
public class Replay {
   ...
   * helps determine whether ack timeouts on the master are due to a slow replica, or the network.
   */
  private final long ackTimeoutLogThresholdNs;

  /**
   * ActiveTxns is a collection of txn objects used for applying replicated transactions. This
   * collection should be empty if the node is a master.
   *
   * <p>Note that there is an interesting relationship between ActiveTxns and the txn collection
   * managed by the environment TxnManager. ActiveTxns is effectively a subset of the set of txns
   * held by the TxnManager. ReplayTxns must be sure to register and unregister themselves from
   * ActiveTxns, just as all Txns must register and unregister with the TxnManager's set. One
   * implementation alternative to having an ActiveTxns map here is to search the TxnManager set
   * (which is a set, not a map) for a given ReplayTxn. Another is to subclass TxnManager so that
   * replicated nodes have their own replayTxn map, just as XATxns have a XID->Txn map.
   *
   * <p>Both alternatives seemed too costly in terms of performance or elaborate in terms of code to
   * do for the current function. It seems clearer to make the ActiveTxns a map in the one place
   * that it is currently used. This choice may change over time, and should be reevaluated if the
   * implementation changes.
   *
   * <p>The ActiveTxns key is the transaction id. These transactions are closed when: - the replay
   * unit executes a commit received over the replication stream - the replay unit executes an abort
   * received over the replication stream - the replication node realizes that it has just become
   * the new master, and was not previously the master.
   *
   * <p>Note that the Replay class has a lifetime that is longer than that of a RepNode. This means
   * in particular, that transactions may be left open, and will be resumed when a replica switches
   * from one master to another, creating a new RepNode in the process. Because of that, part of the
   * transaction may be implemented by the rep stream from one master and another part by another.
   *
   * <p>The map is synchronized, so simple get/put operations do not require additional
   * synchronization. However, iteration requires synchronization and copyActiveTxns can be used in
   * most cases.
   */
  ...
  /**
   * State that is reinitialized by the reinit() method each time a replay loop is started with a
   * new feeder.
   */

  /** All writes (predominantly acks) are queued here, so they do not block the replay thread. */
  ...
  public Replay(RepImpl repImpl, @SuppressWarnings("unused") NameIdPair nameIdPair) {
    ...
    ackTimeoutLogThresholdNs =
        MILLISECONDS.toNanos(configManager.getDuration(RepParams.REPLICA_ACK_TIMEOUT));

    /**
     * The factor of 2 below is somewhat arbitrary. It should be > 1 X so that the
     * ReplicaOutputThread can completely process the buffered messages in the face of a network
     * drop and 2X to allow for additional headroom and minimize the chances that the replay might
     * be blocked due to the limited queue length.
     */
    final int outputQueueSize = 2 * configManager.getInt(RepParams.REPLICA_MESSAGE_QUEUE_SIZE);
    ...
    statistics = new StatGroup(ReplayStatDefinition.GROUP_NAME, ReplayStatDefinition.GROUP_DESC);
    ...
    nMessageQueueOverflows = new LongStat(statistics, N_MESSAGE_QUEUE_OVERFLOWS);
    minCommitProcessingNanos = new LongMinZeroStat(statistics, MIN_COMMIT_PROCESSING_NANOS);
    maxCommitProcessingNanos = new LongMaxZeroStat(statistics, MAX_COMMIT_PROCESSING_NANOS);
    totalCommitProcessingNanos = new LongStat(statistics, TOTAL_COMMIT_PROCESSING_NANOS);
    ...
    replayLoggingThresholdNs =
        MILLISECONDS.toNanos(configManager.getDuration(RepParams.REPLAY_LOGGING_THRESHOLD));
  }
  ...
  /**
   * Actions that must be taken before the recovery checkpoint, whether the environment is
  ...
  /**
   * When mastership changes, all inflight replay transactions are aborted. Replay transactions need
   * only be aborted by the node that has become the new master (who was previously a Replica). The
   * replay transactions on the other replicas who have not changed roles are resolved by the abort
   * record issued by said new master.
   */
  public void abortOldTxns() throws DatabaseException {
    ...
    for (ReplayTxn replayTxn : copyActiveTxns().values()) {
       ...
       * written to the log.
       */
      replayTxn.abort(ReplicationContext.MASTER, masterNodeId, VLSN.NULL_VLSN_SEQUENCE);
    }
    assert activeTxns.isEmpty() : "Unexpected txns in activeTxns = " + activeTxns;
  }

  private void updateCommitStats(
      final boolean needsAck,
    ...
    if (commitNanos > ackTimeoutLogThresholdNs && logger.isLoggable(Level.INFO)) {
      LoggerUtils.info(
          logger,
          repImpl,
          "Replay commit time: "
  ...
  public void replayEntry(long startNs, Protocol.Entry entry)
      throws DatabaseException, IOException, InterruptedException, MasterSyncException {
    ...
    try {
      ...
      if (LOG_TXN_COMMIT.equalsType(entryType)) {
        ...
        final SyncPolicy txnSyncPolicy = commitEntry.getReplicaSyncPolicy();
        final SyncPolicy implSyncPolicy =
            needsAck ? groupCommit.getImplSyncPolicy(txnSyncPolicy) : noAckSyncPolicy;
        ...
        lastReplayedTxn = new TxnInfo(lastReplayedVLSN, masterCommitTimeMs);

        updateCommitStats(
            needsAck, implSyncPolicy, startNs, masterCommitTimeMs, repTxn.getEndTime());
        ...
        if (needsAck) {
           ...
           * queue it.
           */
          if (!groupCommit.bufferAck(startNs, repTxn, txnSyncPolicy)) {
            queueAck(txnId);
          }
        }
        ...
        final ReplicationContext abortContext = new ReplicationContext(wireRecord.getVLSN());
        if (logger.isLoggable(Level.FINEST)) {
          LoggerUtils.finest(
              logger,
              repImpl,
        ...
        lastReplayedTxn = new TxnInfo(lastReplayedVLSN, abort.getTime().getTime());
        if (repTxn.getRepGroupDbChange() && canRefreshGroup(repTxn)) {
      ...
      if (elapsedNs > replayLoggingThresholdNs) {
        LoggerUtils.info(
            ...
            "Replay time for entry type:"
                ...
                + "ms "
                + "exceeded threshold:"
                + NANOSECONDS.toMillis(replayLoggingThresholdNs)
                + "ms");
      }
    }
  }
  ...
  /**
   * Update the replica's in-memory DTVLSN using the value in the commit/abort entry.
   *
   * <p>In the normal course of events, DTVLSNs should not decrease. However, there is just one
   * exception: if the rep stream transitions from a post to a pre-dtvlsn stream, it will transition
   * from a positive to the UNINITIALIZED_VLSN_SEQUENCE.
   *
   * <p>A transition from a pre to a post-dtvlsn transition (from zero to some positive value),
   * observes the "DTVLSNs should not decrease" rule automatically.
   *
   * @return the DTVLSN entry in the log txnEnd record so it can be used in the commit/abort
  ...
  private long updateDTVLSN(final TxnEnd txnEnd) {
    ...
    if ((lastReplayedDTVLSN == VLSN.UNINITIALIZED_VLSN_SEQUENCE) && (txnDTVLSN > 0)) {
      LoggerUtils.info(
          logger,
          repImpl,
  ...
  private void logReplay(ReplayTxn repTxn, boolean needsAck, SyncPolicy syncPolicy) {
  ...
  /**
   * Returns true if there are no other activeTxns that have also modified the membership database
   * and are still open, since they could potentially hold write locks that would block the read
   * locks acquired during the refresh operation.
   *
   * @param txn the current txn being committed or aborted
   * @return true if there are no open transactions that hold locks on the membership database.
   */
  ...
  /**
   * Update this replica's node, txn and database sequences with any ids in this log entry. We can
   * call update, even if the replay id doesn't represent a new lowest-id point, or if the apply is
   * not successful, because the apply checks that the replay id is < the sequence on the replica.
   * We just want to ensure that if this node becomes the master, its sequences are in sync with
   * what came before in the replication stream, and ids are not incorrectly reused.
   */
    ...
    synchronized (activeTxns) {
      ...
      if (useTxn == null) {
        ...
        if (registerTxnImmediately) {
          useTxn = new ReplayTxn(repImpl, TransactionConfig.DEFAULT, txnId, activeTxns, logger);
        } else {
          useTxn =
              new ReplayTxn(repImpl, TransactionConfig.DEFAULT, txnId, activeTxns, logger) {
                @Override
   ...
   * Replays the NameLN.
   *
   * <p>Note that the operations: remove, rename and truncate need to establish write locks on the
   * database. Any open handles are closed by this operation by virtue of the ReplayTxn's
   * importunate property. The application will receive a LockPreemptedException if it subsequently
  ...
  private void applyNameLN(ReplayTxn repTxn, InputWireRecord wireRecord) throws DatabaseException {
    ...
    try {
      switch (opType) {
        case CREATE:
          {
            DatabaseConfig dbConfig = repContext.getCreateConfig().getReplicaConfig(repImpl);

            dbImpl =
                repImpl
                    .getDbTree()
                    .createReplicaDb(repTxn, databaseName, dbConfig, nameLN, repContext);
            ...
            if ((dbImpl.getId().getId() == RepGroupDB.DB_ID)
                && !DbType.REP_GROUP.getInternalName().equals(databaseName)) {
              throw EnvironmentFailureException.unexpectedState(
                  "Database: "
        ...
        case REMOVE:
          {
            ...
            try {
              repImpl.getDbTree().removeReplicaDb(repTxn, databaseName, nameLN.getId(), repContext);
        ...
        case TRUNCATE:
          {
            dbImpl = repImpl.getDbTree().getDb(repContext.getTruncateOldDbId());
            try {
              TruncateDbResult result =
                  repImpl
                      .getDbTree()
                      .truncateReplicaDb(repTxn, databaseName, false, nameLN, repContext);
        ...
        case RENAME:
          {
            ...
            try {
              dbImpl =
                  repImpl
                      .getDbTree()
                      .renameReplicaDb(repTxn, dbImpl.getName(), databaseName, nameLN, repContext);
              TriggerManager.runRenameTriggers(repTxn, dbImpl, databaseName);
            } catch (DatabaseNotFoundException e) {
              throw EnvironmentFailureException.unexpectedState(
                  "Database rename from: "
                      + dbImpl.getName()
                      + " to "
                      + databaseName
        ...
        case UPDATE_CONFIG:
          {
            ...
            repImpl.getDbTree().updateNameLN(repTxn, dbName, repContext);

            /* Set the new configurations to DatabaseImpl. */
            dbImpl.setConfigProperties(repTxn, dbName, dbConfig, repImpl);
  ...
  private void applyLN(final ReplayTxn repTxn, final InputWireRecord wireRecord)
      throws DatabaseException {
     ...
     * write operations.
     */
    final DatabaseImpl dbImpl = repImpl.getRepNode().getReplica().getDbCache().get(dbId, repTxn);

    lnEntry.postFetchInit(dbImpl);

    final ReplicationContext repContext = new ReplicationContext(wireRecord.getVLSN());

    try (final Cursor cursor = DbInternal.makeCursor(dbImpl, repTxn, null /*cursorConfig*/)) {
      ...
      if (dbImpl.getSortedDuplicates()
          && (lnEntry.isEmbeddedLN() || (ln.getData() != null && ln.getData().length > 0))) {
        ...
        throw EnvironmentFailureException.unexpectedState(
            ...
            "[#25288] emb="
                ...
                + " key="
                + Key.getNoFormatString(lnEntry.getKey())
                + " data="
                + Key.getNoFormatString(ln.getData())
                + " vlsn="
                + ln.getVLSNSequence());
      }
      ...
      if (ln.isDeleted()) {
        ...
        result =
            DbInternal.searchForReplay(
                cursor, replayKeyEntry, delDataEntry, LockMode.RMW, SearchMode.SET);
      ...
      if (result == null) {
        throw new EnvironmentFailureException(
            ...
            "Replicated operation could  not be applied. " + wireRecord);
      }
    }
  }

  /**
   * Go through all active txns and rollback up to but not including the log entry represented by
   * the matchpoint VLSN.
   *
   * <p>Effectively truncate these rolled back log entries by making them invisible. Flush the log
   * first, to make sure these log entries are out of the log buffers and are on disk, so we can
   * reliably find them through the FileManager.
   *
   * <p>Rollback steps are described in
   * https://sleepycat.oracle.com/trac/wiki/Logging#Recoverysteps. In summary,
   *
   * <p>1. Log and fsync a new RollbackStart record 2. Do the rollback in memory. There is no need
   * to explicitly log INs made dirty by the rollback operation. 3. Do invisibility masking by
   * overwriting LNs. 4. Fsync all overwritten log files at this point. 5. Write a RollbackEnd
   * record, for ease of debugging
   *
   * <p>Note that application read txns can continue to run during syncup. Reader txns cannot access
   * records that are being rolled back, because they are in txns that are not committed, i.e, they
   * are write locked. The rollback interval never includes committed txns, and we do a hard
    ...
    try {
      ...
      LogEntry rollbackStart =
          SingleItemEntry.create(
              ...
              new RollbackStart(matchpointVLSN, matchpointLsn, localActiveTxns.keySet()));
      ...
      for (ReplayTxn replayTxn : localActiveTxns.values()) {
         ...
         */
        assert checkRemoved(replayTxn) : "Should have removed " + replayTxn;
      ...
      logManager.logForceFlush(
          SingleItemEntry.create(
              LogEntryType.LOG_ROLLBACK_END, new RollbackEnd(matchpointLsn, rollbackStartLsn)),
          true, // fsyncRequired
          ReplicationContext.NO_REPLICATE);
  ...
  private boolean checkRemoved(ReplayTxn txn) {
    if (txn.isClosed()) {
        return activeTxns.get(txn.getId()) == null;
    }
  ...
  /**
   * Make a copy of activeTxns to avoid holding its mutex while iterating. Can be used whenever the
   * cost of the HashMap copy is not significant.
   */
  ...
  /**
   * Release all transactions, database handles, etc held by the replay unit. The Replicator is
   * closing down and Replay will not be invoked again.
   */
  public void close() {
    ...
    for (ReplayTxn replayTxn : copyActiveTxns().values()) {
      try {
        if (logger.isLoggable(Level.FINE)) {
          LoggerUtils.fine(logger, repImpl, "Unregistering open replay txn: " + replayTxn.getId());
        }
        replayTxn.cleanup();
      } catch (DatabaseException e) {
        LoggerUtils.fine(
            logger,
            repImpl,
            "Replay txn: " + replayTxn.getId() + " unregistration failed: " + e.getMessage());
      }
    }
  ...
  /**
   * Write out any pending acknowledgments. See GroupCommit.flushPendingAcks for details. This
   * method is invoked after each log entry is read from the replication stream.
   *
  ...
  /**
   * Implements group commit. It's really a substructure of Replay and exists mainly for modularity
   * reasons.
   *
   * <p>Since replay is single threaded, the group commit mechanism works differently in the replica
   * than in the master. In the replica, SYNC transactions are converted into NO_SYNC transactions
   * and executed immediately, but their acknowledgments are delayed until after either the
   * REPLICA_GROUP_COMMIT_INTERVAL (the max amount the first transaction in the group is delayed)
   * has expired, or the size of the group (as specified by REPLICA_MAX_GROUP_COMMIT) has been
   * exceeded.
   */
  private class GroupCommit {
    ...
    private GroupCommit(DbConfigManager configManager) {
      pendingCommitAcks = new long[configManager.getInt(RepParams.REPLICA_MAX_GROUP_COMMIT)];

      nPendingAcks = 0;

      final long groupCommitIntervalMs =
          configManager.getDuration(RepParams.REPLICA_GROUP_COMMIT_INTERVAL);

      groupCommitIntervalNs = NANOSECONDS.convert(groupCommitIntervalMs, MILLISECONDS);
      nGroupCommitTimeouts = new LongStat(statistics, N_GROUP_COMMIT_TIMEOUTS);

      nGroupCommitMaxExceeded = new LongStat(statistics, N_GROUP_COMMIT_MAX_EXCEEDED);

      nGroupCommitTxns = new LongStat(statistics, N_GROUP_COMMIT_TXNS);

      nGroupCommits = new LongStat(statistics, N_GROUP_COMMITS);
    }
    ...
    /**
     * The interval used to poll for incoming log entries. The time is lowered from the defaultNs
     * time, if there are pending acknowledgments.
     *
    ...
    /**
     * Returns the sync policy to be implemented at the replica. If group commit is active, and SYNC
     * is requested it will return NO_SYNC instead to delay the fsync.
     *
    ...
    /**
     * Buffers the acknowledgment if the commit calls for a sync, or if there are pending
     * acknowledgments to ensure that acks are sent in order.
     *
     * @param nowNs the current time
     * @param ackTxn the txn associated with the ack
     * @param txnSyncPolicy the sync policy as request by the committing txn
     * @return true if the ack has been buffered
     */
    private final boolean bufferAck(long nowNs, ReplayTxn ackTxn, SyncPolicy txnSyncPolicy)
        throws IOException {

      if (!isEnabled() || !((txnSyncPolicy == SyncPolicy.SYNC) || (nPendingAcks > 0))) {
        return false;
      }
    ...
    /**
     * Flush if there are pending acks and either the buffer limit or the group interval has been
     * reached.
     *
  ...
  public static class TxnInfo {
    ...
    public String toString() {
      return " VLSN: " + txnVLSN + " masterTxnEndTime=" + new Date(masterTxnEndTime);
    }
  }
}
