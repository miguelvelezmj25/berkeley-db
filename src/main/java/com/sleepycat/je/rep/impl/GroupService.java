...
public class GroupService extends ExecutingService {
  ...
  /**
   * List of channels for in-flight requests. The channel is in this collection while the request is
   * being processed, and must be removed before sending any response.
   *
   * @see #cancel
      ...
      try {
        ...
        if (channel.isOpen()) {
          try {
            ...
            LoggerUtils.warning(
                logger, repNode.getRepImpl(), "IO error on channel close: " + e.getMessage());
          }
        }
      }
    }
  ...
  /**
   * Wraps the replication group as currently cached on this node in a Response message and returns
   * it.
   */
   ...
   * @return EnsureOK message if the monitor node is already part of the rep group, or was just made
   *     a part of the replication group. It returns a Fail message if it could not be made part of
   *     the group. The message associated with the response provides further details.
   */
  ...
  /**
   * Deletes a current member from the group, which marks the node as removed and deletes its entry
   * from the rep group DB.
   *
   ...
   *
   * @param updateAddress the request identifying the new network address for the node.
   * @return OK message if the address is successfully updated.
   */
  public ResponseMessage process(UpdateAddress updateAddress) {
    try {
      ensureMaster();
      repNode.updateAddress(
          updateAddress.getNodeName(), updateAddress.getNewHostName(), updateAddress.getNewPort());
   ...
   *
   * @param transferMaster the request identifying nodes to be considered for the role of new master
   * @return null
   */
   ...
   *
   * <p>Before sending any response on the channel, this method must be invoked to claim ownership
   * of it. This avoids a potential race between the request processing thread in the normal case,
   * and a thread calling {@code cancel()} at env shutdown time.
   *
   * @return true, if the channel is still active (usual case); false otherwise, presumably because
