...
/**
 * ElectionQuorum centralizes decision making about what constitutes a successful election quorum
 * and the definition of an authoritative master.
 */
public class ElectionQuorum {
  ...
  public ElectionQuorum(RepImpl repImpl) {
    ...
    electableGroupSizeOverride =
        repImpl.getConfigManager().getInt(RepParams.ELECTABLE_GROUP_SIZE_OVERRIDE);
    if (electableGroupSizeOverride > 0) {
      LoggerUtils.warning(
          logger, repImpl, "Electable group size override set to:" + electableGroupSizeOverride);
    }
  }
   ...
   * status as a master and that a sufficient number of nodes agree that it's the master based on
   * the number of feeder connections to it. Currently, the sufficient number is just a simple
   * majority. Such an authoritative answer is needed in a network partition situation to detect a
   * master that may be isolated on the minority side of a network partition.
   *
  ...
  /**
   * Return the number of nodes that are required to achieve consensus on the election. Over time,
   * this may evolve to be a more detailed description than simply the size of the quorum. Instead,
   * it may return the set of possible voters.
   *
   * <p>Special situations, like an active designated primary or an election group override will
   * change the default quorum size.
   *
   * @param quorumPolicy
