...
public class DurabilityQuorum {
  ...
  /**
   * See if there are a sufficient number of replicas alive to support the commit for this
   * transaction. Used as an optimizing step before any writes are executed, to reduce the number of
   * outstanding writes that suffer from insufficient ack problems.
   *
   * <p>If this node is not the master, just return. A different kind of check will catch the fact
   * that this node cannot support writes.
   *
   * <p>TODO: Read only transactions on the master should not have to wait. In the future, introduce
   * either a read-only attribute as part of TransactionConfig or a read only transaction class to
   * optimize this.
   *
   * @param insufficientReplicasTimeout timeout in ms
   * @throws InsufficientReplicasException if there are not enough replicas connected to this feeder
   *     to be able to commit the transaction.
   */
  public void ensureReplicasForCommit(MasterTxn txn, int insufficientReplicasTimeout)
    ...
    ReplicaAckPolicy ackPolicy = txn.getDefaultDurability().getReplicaAck();
    ...
    if (repNode
        .feederManager()
        .awaitFeederReplicaConnections(requiredReplicaAckCount, insufficientReplicasTimeout)) {
      /* Wait was successful */
      return;
    }
  ...
  /**
   * Determine whether acknowledgments from the specified replica should be counted against
   * transaction durability requirements.
   *
   ...
   *
   * @throws InsufficientAcksException if the transaction's durability requirements have not been
   *     met.
   */
  public void ensureSufficientAcks(FeederTxns.TxnInfo txnInfo, int timeoutMs)
    ...
    final int requiredAcks = getCurrentRequiredAckCount(txn.getCommitDurability().getReplicaAck());
     ...
     */
    final FeederManager feederManager = repImpl.getRepNode().feederManager();
    ...
    if (currentFeederCount >= requiredAcks) {
      String msg =
          "txn "
              + txn.getId()
              + " commit vlsn:"
              + txnInfo.getCommitVLSN()
              + " acknowledged after explicit feeder check"
              + " latch count:"
              + txnInfo.getPendingAcks()
              + " state:"
              + dumpState
              + " required acks:"
              + requiredAcks;
  ...
  /**
   * Returns the minimum number of acknowledgments required to satisfy the ReplicaAckPolicy for a
   * given group size. Does not include the master. The method factors in considerations like the
   * current arbitration status of the environment and the composition of the replication group.
   *
   * <p>TODO: it seems sufficient to return a number, as opposed to a set of qualified ack nodes, as
   * long as {@link #replicaAcksQualify} will only count qualified acks against the required count.
   * That does mean that getCurrentRequiredAckCount and noteReplicaAcks for a transaction must be
   * kept consistent.
   *
