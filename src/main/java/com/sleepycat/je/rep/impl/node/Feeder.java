...
import com.sleepycat.je.rep.stream.*;
...
/**
 * There is an instance of a Feeder for each client that needs a replication stream. Either a
 * master, or replica (providing feeder services) may establish a feeder.
 *
 * <p>A feeder is created in response to a request from a Replica, and is shutdown either upon loss
 * of connectivity, or upon a change in mastership.
 *
 * <p>The protocol used to validate and negotiate a connection is synchronous, but once this phase
 * has been completed, the communication between the feeder and replica is asynchronous. To handle
 * the async communications, the feeder has two threads associated with it:
 *
 * <p>1) An output thread whose sole purpose is to pump log records (and if necessary heart beat
 * requests) down to the replica as fast as the network will allow it
 *
 * <p>2) An input thread that listens for responses to transaction commits and heart beat responses.
 *
 * <p>The feeder maintains several statistics that provide information about the replication rate
 * for each replica. By comparing this information to information about master replication
 * maintained by the FeederTxns class, it is also possible to estimate the lag between replicas and
 * the master.
 *
 * <p>The statistics facilities do not expect the set of available statistics to change dynamically.
 * To handle recording statistics about the changing set of replicas, the statistics are represented
 * as maps that associated node names with statistics. Each feeder adds individual statistics in
 * these maps at startup, and removes them at shutdown time to make sure that the statistics in the
 * map only reflect up-to-date information.
 *
 * <p>Some notes about the specific statistics:
 *
 * <dl>
 *   <dt>replicaDelay
 *   <dd>The difference between the commit times of the latest transaction committed on the master
 *       and the transaction most recently processed by the replica. The master timestamp comes from
 *       the lastCommitTimestamp statistic maintained by FeederTxns. The feeder determines the
 *       commit timestamp of the replica's most recently processed transaction by obtaining
 *       timestamps from commit records being sent to the replica, and noting the last one prior to
 *       sending a heartbeat. When a heartbeat response is received, if the latest replica VLSN
 *       included in the response is equal or greater to the one recorded when the heartbeat request
 *       was sent, then the delay is computed by comparing the commit timestamp for that most
 *       recently sent transaction with the timestamp of the master's latest transaction. Replicas
 *       can send heartbeat responses on their own, so comparing the VLSNs is necessary to make sure
 *       that the response matches the request. Note that this arrangement depends on the fact that
 *       the replica processes transactions and heartbeats in order, and only sends a heartbeat
 *       response once all preceding transactions have been processed. If the master processes
 *       transactions at a fast enough rate that additional transactions are generated while waiting
 *       for a heartbeat response, then the value of this statistic will not reach zero, but will
 *       represent the total time for sending a commit operation to the replica and receiving the
 *       associated response, including the roundtrip latency of the network and any time spent due
 *       to buffering of replication data.
 *   <dt>replicaLastCommitTimestamp
 *   <dd>The commit timestamp of the last transaction committed before the most recent heartbeat for
 *       which a heartbeat response has been received. This statistic represents the commit time on
 *       the master of the most recent data known to have been processed on the replica. It provides
 *       the information used for the replica component of the replicaDelay statistic.
 *   <dt>replicaLastCommitVLSN
 *   <dd>The VLSN of the committed transaction described for replicaLastCommitTimestamp. This
 *       statistic provides the information used for the replica component of the replicaVLSNLag
 *       statistic.
 *   <dt>replicaVLSNLag
 *   <dd>The difference between the VLSN of the latest transaction committed on the master and the
 *       one most recently processed by the replica. The master VLSN comes from the lastCommitVLSN
 *       statistic maintained by FeederTxns. This statistic is similar to replicaDelay, but provides
 *       information about the VLSN lag rather than the time delay.
 *   <dt>replicaVLSNRate
 *   <dd>An exponential moving average of the rate of change of the replicaLastCommitVLSN statistic
 *       over time, averaged over a 10 second time period. This statistic provides information about
 *       how quickly the replica is processing replication data, which can be used, along with the
 *       vlsnRate statistic maintained by FeederTxns, to estimate the amount of time it will take
 *       for the replica to catch up with the master.
 * </dl>
 */
public final class Feeder {
  ...
  /**
   * The current position of the feeder, that is, the log record with this VLSN will be sent next to
   * the Replica. Note that this does not mean that the replica has actually processed all log
   * records preceding feederVLSN. The records immediately preceding feederVLSN (down to
   * replicaAckVLSN) may be in the network, in transit to the replica.
   *
   * <p>The feederVLSN can only move forwards post feeder-replica syncup. However, it can move
   * forwards or backwards as matchpoints are negotiated during syncup.
   */
  private volatile VLSN feederVLSN = VLSN.NULL_VLSN;

  /**
   * The latest commit or abort that the replica has reported receiving, either by ack (in the case
   * of a commit), or via heartbeat response. It serves as a rough indication of the replay state of
   * the replica that is used in exception messages.
   *
   * <p>The following invariant must always hold: replicaTxnEndLSN < feederVLSN
   */
  ...
  /**
   * The replica node ID, that is, the node that is the recipient of the replication stream. Its
   * established at the time of the Feeder/Replica handshake.
   */
  private volatile NameIdPair replicaNameIdPair = NameIdPair.NULL;

  /**
   * The agreed upon log format that should be used for writing log entries to send to the replica,
  ...
  /**
   * The VLSN of the most recent log entry that committed a transaction and was sent to the replica
   * before the last heartbeat was sent, or 0 if no such log entries have been sent since the
   * previous heartbeat.
   */
  private volatile long lastHeartbeatCommitVLSN;

  /**
   * The timestamp of the most recent log entry that committed a transaction and was sent to the
   * replica before the last heartbeat was sent, or 0 if no such log entries have been sent since
  ...
  /**
   * A test hook that is called before a message is written. Note that the hook is inherited by the
   * ReplicaFeederHandshake, and will be kept in place there for the entire handshake.
   */
  private volatile TestHook<Message> writeMessageHook;

  /** A test hook that is used to set the writeMessageHook for newly created feeders. */
  ...
  private NamedChannelWithTimeout configureChannel(DataChannel channel) throws IOException {
    ...
    try {
      ...
      final boolean tcpNoDelay =
          repNode.getConfigManager().getBoolean(RepParams.FEEDER_TCP_NO_DELAY);
  ...
  Feeder(FeederManager feederManager, DataChannel dataChannel)
    ...
    securityChkIntvMs = feederManager.repNode().getSecurityCheckInterval();
  }
  ...
  /**
   * Creates the MasterFeederSource, which must be done while all files in the VLSNIndex range are
   * protected by syncup.
   */
  public void initMasterFeederSource(VLSN startVLSN) throws IOException, InterruptedException {
    ...
    feederSource =
        new MasterFeederSource(
            repNode.getRepImpl(), repNode.getVLSNIndex(), replicaNameIdPair, startVLSN);
  }

  private void initArbiterFeederSource() throws IOException, InterruptedException {
  ...
  /**
   * Returns the latest commit VLSN that was acked by the replica, or NULL_VLSN if no commit was
   * acked since the time the feeder was established.
   */
  ...
  /**
   * Returns the next VLSN that will be sent to the replica. It will return VLSN.NULL if the Feeder
   * is in the process of being created and FeederReplicaSyncup has not yet happened.
   */
  ...
  /**
   * Returns the JE version supported by the replica, or {@code null} if the value is not yet known.
   *
  ...
  /**
   * Returns a RepNodeImpl that describes the replica, or {@code null} if the value is not yet
   * known. The value will be non-null if the feeder handshake has completed successfully.
   *
  ...
  /**
   * Shutdown the feeder, closing its channel and releasing its threads. May be called internally
   * upon noticing a problem, or externally when the RepNode is shutting down.
   */
  void shutdown(Exception shutdownException) {
    ...
    if (repNode.getReplicaCloseCatchupMs() >= 0) {
      ...
      try {
        ...
        LoggerUtils.warning(
            logger, repImpl, "Interrupted while waiting to join " + "thread:" + outputThread);
      }
    }
  ...
  /** Implements the thread responsible for processing the responses from a Replica. */
  private class InputThread extends StoppableThread {
    ...
    /**
     * Does the initial negotiation to validate replication group wide consistency and establish the
     * starting VLSN. It then starts up the Output thread and enters the response loop.
     */
    @Override
    public void run() {
      ...
      try {
        FeederReplicaHandshake handshake =
            new FeederReplicaHandshake(repNode, Feeder.this, feederReplicaChannel);
        ...
        if (replicaNode.getType().isArbiter()) {
          ...
          FeederReplicaSyncup syncup =
              new FeederReplicaSyncup(Feeder.this, feederReplicaChannel, protocol);
          ...
          if (replicaNode.getType().isDataNode()) {
            replicaCBVLSN =
                new LocalCBVLSNUpdater(replicaNameIdPair, replicaNode.getType(), repNode);
          }
        ...
        replicaDelay =
            feederManager
                .getReplicaDelayMap()
                .createStat(
                    replicaNameIdPair.getName(), repNode.getFeederTxns().getLastCommitTimestamp());
        replicaLastCommitTimestamp =
            feederManager
                .getReplicaLastCommitTimestampMap()
                .createStat(replicaNameIdPair.getName());
        replicaLastCommitVLSN =
            feederManager.getReplicaLastCommitVLSNMap().createStat(replicaNameIdPair.getName());
        replicaVLSNLag =
            feederManager
                .getReplicaVLSNLagMap()
                .createStat(
                    replicaNameIdPair.getName(), repNode.getFeederTxns().getLastCommitVLSN());
        replicaVLSNRate =
            feederManager.getReplicaVLSNRateMap().createStat(replicaNameIdPair.getName());
        ...
        throw new EnvironmentFailureException(
            repNode.getRepImpl(), EnvironmentFailureReason.LOG_CHECKSUM, e);
    ...
    private void runResponseLoop() throws IOException, MasterSyncException {
      ...
      while (!checkShutdown()) {
        ...
        if (response.getOp() == Protocol.HEARTBEAT_RESPONSE) {
          ...
          LoggerUtils.info(
              logger, repImpl, "Shutdown confirmed by replica " + replicaNameIdPair.getName());
    ...
    private void processHeartbeatResponse(Message response) {
      ...
      final HeartbeatResponse hbResponse = (Protocol.HeartbeatResponse) response;
      ...
      if (replicaTxnVLSN.compareTo(repNode.getCurrentTxnEndVLSN()) >= 0) {
      ...
      final long statCommitVLSN =
          (commitVLSN <= replicaTxnVLSNSeq) ? commitVLSN : replicaTxnVLSNSeq;
  ...
  private class OutputThread extends StoppableThread {
    ...
    /**
     * Determines whether writing to the network connection for the replica suffices as a commit
     * acknowledgment.
     */
    private final boolean commitToNetwork;

    /** The threshold used to trigger the logging of transfers of commit records. */
    ...
    OutputThread() {
      ...
      if (feederManager != null) {
        ...
        StatGroup stats =
            new StatGroup(
                FeederManagerStatDefinition.GROUP_NAME, FeederManagerStatDefinition.GROUP_DESC);
    ...
    /**
     * Determines whether we should exit the output loop. If we are trying to shutdown the Replica
     * cleanly, that is, this is a group shutdown, the method delays the shutdown until the Replica
     * has had a chance to catch up to the current commit VLSN on this node, after which it sends
     * the Replica a Shutdown message.
     *
    ...
    private boolean checkShutdown() throws IOException {
      ...
      if (repNode.getReplicaCloseCatchupMs() >= 0) {
        ...
        if (!timedOut
            && !isArbiterFeeder
            && (feederVLSN.compareTo(repNode.getCurrentTxnEndVLSN()) <= 0)) {
          /*
    ...
    private void writeMessage(final Message message, final NamedChannel namedChannel)
        throws IOException {
    ...
    public void run() {
      ...
      Thread.currentThread()
          .setName("Feeder Output for " + Feeder.this.getReplicaNameIdPair().getName());
      {
    ...
    /**
     * Write as many readily "available" log entries as possible to the network. The term
     * "available" is used in the sense that these values are typically sitting around in the JE or
     * FS cache especially for messages that are recent enough to need timely acknowledgement. The
     * method tried to batch multiple entries, to minimize the number of network calls permitting
     * better packet utilization and fewer network related interrupts, since FEEDER_TCP_NO_DELAY is
     * set on the channel.
     *
     * <p>The size of the batch is limited by one of:
     *
     * <p>1) The number of "available" trailing vlsn entries between the current position of the
     * feeder and the end of the log.
     *
     * <p>2) The size of the batchWriteBuffer and
     *
     * <p>3) The time it takes to accumulate the batch without exceeding the minimum of:
     *
     * <p>a) heartbeat interval, a larger time window typically in effect when the replica is not in
     * the ack window. It effectively favors batching.
     *
     * <p>b) (batchNs + time to first ack requiring) transaction, typically in effect when the
     * replica is in the ack window and more timely acks are needed.
     *
     * <p>This adaptive time interval strategy effectively adapts the batch sizes to the behavior
     * needed of the replica at any given instant in time.
     */
    private void writeAvailableEntries()
      ...
      do {
        ...
        feederVLSN = feederVLSN.getNext();
          ...
          && /* Don't batch if set by test. */ (vlsnIndex.getLatestAllocatedVal()
              >= feederVLSN.getSequence())
    ...
    /**
     * Sends a heartbeat message, if we have exceeded the heartbeat interval.
     *
     * @throws IOException
     */
    ...
    /**
     * Converts a log entry into a specific Message to be sent out by the Feeder.
     *
    ...
    private Message createMessage(long txnId, OutputWireRecord wireRecord)
      ...
      if (ackTxn != null) {
         ...
         */
        needsAck =
            !commitToNetwork && repNode.getDurabilityQuorum().replicaAcksQualify(replicaNode);
    ...
    private void validate(OutputWireRecord record) {
      ...
      if (!record.getVLSN().equals(feederVLSN)) {
        throw EnvironmentFailureException.unexpectedState(
            "Expected VLSN:" + feederVLSN + " log entry VLSN:" + record.getVLSN());
      }

        assert repImpl.isRepConverted() || record.verifyNegativeSequences("node=" + nameIdPair);
    }
  ...
  /**
   * Defines the handler for the RepNode thread. The handler invalidates the environment by ensuring
   * that an EnvironmentFailureException is in place.
   *
   * <p>The handler communicates the cause of the exception back to the FeederManager's thread by
   * setting the repNodeShutdownException and then interrupting the FM thread. The FM thread upon
   * handling the interrupt notices the exception and propagates it out in turn to other threads
  ...
  private class IOThreadsHandler implements UncaughtExceptionHandler {
    ...
    public void uncaughtException(Thread t, Throwable e) {
      LoggerUtils.severe(
          logger,
          repImpl,
          "Uncaught exception in feeder thread "
              + t
              + e.getMessage()
  ...
  /**
   * A marker exception that wraps the real exception. It indicates that the impact of wrapped
   * exception can be contained, that is, it's sufficient cause to exit the Feeder, but does not
   * otherwise impact the RepNode.
   */
  ...
  /**
   * Set a test hook that will be called before sending a message using the protocol's write method,
   * supplying the hook with the message as an argument.
   */
  ...
  private void doSecurityCheck() throws ReplicationSecurityException {
    ...
    if ((curr - authenticator.getLastCheckTimeMs()) >= securityChkIntvMs) {
      checkAccess(authenticator, feederReplicaChannel.getNameIdPair().getName());
    }
  }
  ...
  private static void checkAccess(StreamAuthenticator authenticator, String replicaName) {
    ...
    if (!authenticator.checkAccess()) {
      final String err =
          "replica " + replicaName + "fails the security check to stream requested data.";
