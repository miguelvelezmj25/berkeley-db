 ...
 *
 * <p>Each Master Transfer operation uses a separate instance of this class. There is usually no
 * more than one instance in the lifetime of a master node, because if the transfer succeeds, the
 * old master node environment becomes invalid and must be closed. However, if an operation times
 * out, another operation can try again later. Or, a second operation can "forcibly" supersede an
 * existing operation in progress.
 *
...
public class MasterTransfer {
  ...
  /**
   * Flag that indicates we've reached the point where we're committed to proceeding with the
   * transfer: we've completed phase 2, chosen a winner, and are now notifying everyone of the new
   * (fake) election result. Once we get to this point, we can't allow a new Master Transfer
   * operation attempt to supersede us.
   */
  private volatile boolean done;

  /**
   * Queue which communicates key events of interest from Feeders regarding the progress of their
   * efforts to catch up with the end of the log. The existence of this object signifies that (1)
   * the owning Master Transfer object is viable (hasn't been superseded by a later, "forcing" MT
   * operation); and (2) we have not yet discovered a winner. Once we have chosen a winner we
   * disallow any future attempt to supersede this operation.
   *
   * @see #abort
  ...
  MasterTransfer(Set<String> replicas, long timeout, RepNode repNode) {
    ...
    eventQueue =
        new ExceptionAwareBlockingQueue<VLSNProgress>(
            repNode.getRepImpl(), new VLSNProgress(null, null));
  }

  /**
   * Aborts an existing, in-progress Master Transfer operation, if it hasn't reached the point of no
   * return.
   *
   * @return true, if the operation was cancelled, false if it's too late for a clean cancellation.
   */
  ...
  /**
   * Accepts a Progress event and posts it to our queue for processing by the Master Transfer
   * operation thread.
   */
  ...
  /**
   * Informs this Master Transfer operation that the named Feeder is shutting down, because its
   * replica connection has been lost. This of course means that we can't expect this Feeder to soon
   * catch up with our VLSN. In particular, if we have reached Phase 2 on the strength of the
   * progress of only this one Feeder, then we must revert back to Phase 1.
   *
   * <p>Actually all we do here is post a special kind of "progress" event to our queue; it gets
   * processed for real in the {@code chooseReplica()} thread, along with all the other events.
   *
   * @see #chooseReplica
   */
  ...
  private synchronized ExceptionAwareBlockingQueue<VLSNProgress> getQueue() {
    return eventQueue;
  }

  /**
   * Performs the core processing of a Master Transfer operation. We first wait for one of the
   * candidate target replica nodes to become completely synchronized. We then send a message to all
   * nodes in the group (including ourselves) announcing which node is to become the new master.
   *
   * <p>If the operation fails we release any transaction commit/abort threads that may have been
   * blocked during phase 2 of the wait. However, in the success case the release of any such
   * transaction threads is done as a natural by-product of the transition of the environment from
   * master to replica status.
   */
  String transfer() {
    try {
  ...
  /**
   * Prepares for a Master Transfer operation by waiting for one of the nominated candidate target
   * replica nodes to catch up with the master, in two phases, as described in {@link
   * ReplicatedEnvironment#transferMaster(Set, int, TimeUnit)}.
   *
   * <p>This method works by observing events generated by Feeder threads and passed to us via a
   * queue.
   *
   * @return the node name of the first replica to complete phase 2 of the preparation, or {@code
  ...
  /**
   * Broadcasts a fake election result message. This does a couple things: (1) prods the chosen
   * replica to become the new master; and (2) forces the old master to notice and shut down with a
   * master-replica transition exception.
   */
  ...
  /**
   * Enables the given {@code Feeder} to contribute to this Master Transfer operation. Called from
   * the {@code FeederManager} when a new {@code Feeder} is established during the time when a
   * Master Transfer operation is already in progress.
   */
  void addFeeder(Feeder f) {
    String name = f.getReplicaNameIdPair().getName();
    if (replicas.contains(name)) {
      LoggerUtils.info(
          logger, repNode.getRepImpl(), "Add node " + name + " to existing Master Transfer");
  ...
  /** Generates a detailed error message for the case when the operation times out. */
  private String getTimeoutMsg() {
    return "Timed out: started at "
  ...
  /**
   * An event of interest in the pursuit of our goal of completing the Master Transfer. Generally it
   * indicates that the named replica has received and processed the transaction identified by the
   * given VLSN. As a special case, an event representing the death of a Feeder is represented by a
   * {@code null} VLSN.
   */
