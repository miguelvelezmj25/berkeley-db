 ...
 *
 * <p>To support the new BinaryStateProtocol, we introduce this new BinaryNodeStateService, it's
 * used by "Ping" command.
 *
 * <p>Note: we can merge the two NodeState services together once we support acitve version updates.
 */
public class BinaryNodeStateService extends ExecutingService {
  ...
  class NodeStateServiceRunnable implements Runnable {
    ...
    private BinaryNodeStateResponse createResponse(BinaryNodeStateProtocol protocol) {
      ...
      long txnEndVLSN =
          (repNode.getCurrentTxnEndVLSN() == null
              ? 0L
      ...
      return protocol
      .new BinaryNodeStateResponse(
          repNode.getNodeName(),
          repNode.getGroup().getName(),
    ...
    public void run() {
      ...
      try {
        protocol = new BinaryNodeStateProtocol(NameIdPair.NOCHECK, repNode.getRepImpl());
        try {
          ...
          if (!repNode.getGroup().getName().equals(groupName)
            ...
            throw new ProtocolException(
                "Sending the request to" + " a wrong group or a wrong node.");
          }
          ...
          LoggerUtils.finest(
              logger, repNode.getRepImpl(), "Deal with a node state request successfully.");
