...
public class RepNodeImpl implements ReplicationNode, Serializable {
  ...
  /** The JE version most recently noted running on this node, or null if not known. */
  private volatile JEVersion jeVersion;

  /**
   * @hidden Constructor used to de-serialize a Node. All other convenience constructors funnel
   *     through this one so that argument checks can be systematically enforced.
   */
  public RepNodeImpl(
  ...
  public RepNodeImpl(final ReplicationConfig repConfig) {
    this(
        new NameIdPair(repConfig.getNodeName(), NameIdPair.NULL_NODE_ID),
  ...
  /** @hidden Convenience constructor for the above. */
  public RepNodeImpl(
      final String nodeName, final String hostName, final int port, final JEVersion jeVersion) {
    this(
        new NameIdPair(nodeName, NameIdPair.NULL.getId()),
        NodeType.ELECTABLE,
        hostName,
        port,
        jeVersion);
  ...
  /**
   * Returns whether the node was acknowledged by a quorum and its entry is therefore considered
   * durable. Secondary nodes are always considered acknowledged.
   */
  ...
  /**
   * Returns the JE Version most recently noted running on this node, or {@code null} if not known.
   */
  ...
  public String toString() {
    ...
    String info =
        String.format(
            "Node:%s %s:%d%s%s changeVersion:%d %s%s\n",
            getName(),
            getHostName(),
            getPort(),
            acked,
  ...
  /**
   * Checks if the argument represents the same node, ignoring fields that might legitimately vary
   * over time. Like the equals method, considers all fields, except ignores the quorumAck field
   * (which may change temporarily), the nodeId (since it may not have been resolved as yet), and
   * the isRemoved, barrierState, changeVersion, and jeVersion fields (which can change over time).
   *
  ...
  public boolean equals(Object obj) {
    ...
    if (jeVersion == null) {
        return other.jeVersion == null;
    } else return jeVersion.equals(other.getJEVersion());
  }
}
