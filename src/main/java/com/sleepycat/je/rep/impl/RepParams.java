...
import com.sleepycat.je.config.*;
...
public class RepParams {
  ...
  /**
   * @hidden Name of a java System property (boolean) which can be turned on in order to avoid input
   *     validation checks on node names. This is undocumented.
   *     <p>Generally users should not skip validation, because there are a few kinds of punctuation
   *     characters that would cause problems if they were allowed in node names. But in the past
   *     users might have inadvertantly created node names that do not conform to the new, stricter
   *     rules. In that case they would not be able to upgrade to the newer version of JE that now
   *     includes this checking.
   *     <p>This flag actually applies to the group name too. But for group names the new rules are
   *     actually less strict than they used to be, so there should be no problem.
   */
  public static final String SKIP_NODENAME_VALIDATION = "je.rep.skipNodenameValidation";

  /**
   * @hidden Name of a java System property (boolean) which can be turned on in order to avoid
   *     hostname resolution checks on helper host values. This is undocumented.
   *     <p>Generally users should not skip validation, because having valid helper hosts is an
   *     important path to let this node find the master in a replication group when its group
   *     membership db is not sufficient. Disabling the check should only be done in unusual cases,
   *     such as in the face of intermittent DNS failures. In that case, a hostname which seems to
   *     be invalid may actually be a transient problem, rather than a permanent configuration
   *     issue. Skipping the resolution check at config setting time may supply some degree of
   *     resilience in this unusual case.
   */
  public static final String SKIP_HELPER_HOST_RESOLUTION = "je.rep.skipHelperHostResolution";

  /** A JE/HA configuration parameter describing an Identifier name. */
  public static class IdentifierConfigParam extends ConfigParam {
    private static final String DEBUG_NAME = IdentifierConfigParam.class.getName();

    public IdentifierConfigParam(
        String configName, String defaultValue, boolean mutable, boolean forReplication) {
  ...
  /**
   * MIN_RETAINED_VLSNS was never exposed in the public API, although we did ask several users to
   * configure it in the past, so we shouldn't delete the param definition.
   *
  ...
  public static final IntConfigParam MIN_RETAINED_VLSNS =
      new IntConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "minRetainedVLSNs",
          0, // min
          null, // max
          0, // default
          false, // mutable
  ...
  /**
   * Unpublished (for now at least) parameter describing the minimum size of the VLSNIndex as a
   * number of VLSNs. Once the index grows to this size, it will not get smaller due to head
   * truncation by the cleaner.
   *
   * <p>When a disk limit is violated we will have already deleted as many reserved files as
   * possible. If space is made available and write operations can resume, we need to ensure that
   * each node has a large enough VLSNIndex to perform syncup, etc.
   *
   * <p>This limit is enforced on both master and replicas. It is particularly important on replicas
   * because feeders will not prevent VLSN index head truncation.
   */
  public static final IntConfigParam MIN_VLSN_INDEX_SIZE =
      new IntConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "minVLSNIndexSize",
          0, // min
          null, // max
          1000, // default
          false, // mutable
  ...
  /**
   * Can be used by tests to prevent the GlobalCBVLSN from being defunct, even when all nodes are
   * DEFUNCT_JE_VERSION or higher.
   */
  public static final BooleanConfigParam TEST_CBVLSN =
      new BooleanConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "testCBVLSN",
          false, // default
          false, // mutable
          true);
  ...
  public static final IntConfigParam REPLICA_RECEIVE_BUFFER_SIZE =
      new IntConfigParam(
          ReplicationConfig.REPLICA_RECEIVE_BUFFER_SIZE,
          0, // min
          null, // max
          1048576, // default
          false, // mutable
  ...
  /**
   * The size of the message queue used for communication between the thread reading the replication
   * stream and the thread doing the replay. The default buffer size has been chosen to hold 500
   * single operation transactions (the ln + commit record) assuming 1K sized LN record.
   *
   * <p>Larger values of buffer size may result in higher peak memory utilization, due to a larger
   * number of LNs sitting in the queue. The size of the queue itself is unlikely to be an issue,
   * since it's tiny relative to cache sizes. At 1000, 1kbyte LNs it raises the peak utilization by
   * 1MB which for most apps is an insignificant rise in the peak.
   *
   * <p>Note that the parameter is lazily mutable, that is, the change will take effect the next
   * time the node transitions to a replica state.
   */
  public static final IntConfigParam REPLICA_MESSAGE_QUEUE_SIZE =
      new IntConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "replicaMessageQueueSize",
          1, // min
          null, // max
          1000, // default
          true, // mutable
  ...
  public static final DurationConfigParam REPLAY_TXN_LOCK_TIMEOUT =
      new DurationConfigParam(
          ReplicationConfig.REPLAY_TXN_LOCK_TIMEOUT,
          "1 ms", // min
          "75 min", // max
          "500 ms", // default
          false, // mutable
  ...
  public static final DurationConfigParam ARBITER_ACK_TIMEOUT =
      new DurationConfigParam(
          ReplicationConfig.ARBITER_ACK_TIMEOUT,
          "10 ms", // min
          null, // max
          "2 s", // default
          false, // mutable
  ...
  /**
   * Internal parameter enable use of the group ack message. It's on by default since protocol
   * version 6.
   */
  public static final BooleanConfigParam ENABLE_GROUP_ACKS =
      new BooleanConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "enableGroupAcks",
          true, // default
          false, // mutable
          true);
  ...
  public static final LongConfigParam MAX_MESSAGE_SIZE =
      new LongConfigParam(
          ReplicationConfig.MAX_MESSAGE_SIZE,
  ...
  /**
   * Identifies the default consistency policy used by a replica. Only two policies are meaningful
   * as properties denoting environment level default policies: NoConsistencyRequiredPolicy and
   * TimeConsistencyPolicy. They can be specified as: NoConsistencyRequiredPolicy or
   * TimeConsistencyPolicy(<permissibleLag>,<timeout>). For example, a time based consistency policy
   * with a lag of 1 second and a timeout of 1 hour is denoted by the string:
  ...
  public static final IntConfigParam DEFAULT_PORT =
      new IntConfigParam(
          ReplicationConfig.DEFAULT_PORT,
  ...
  public static final ConfigParam NODE_HOST_PORT =
      new ConfigParam(
          ReplicationConfig.NODE_HOST_PORT,
          "localhost", // default
          false, // mutable
          true) { // forReplication
        ...
        public void validateValue(String hostAndPort) throws IllegalArgumentException {
          ...
          try {
            ...
            throw new IllegalArgumentException(
                "Property: " + name + " Invalid hostname: " + hostName, e);
          } catch (IOException e) {
  ...
  public static final EnumConfigParam<NodeType> NODE_TYPE =
      new EnumConfigParam<NodeType>(
          ReplicationConfig.NODE_TYPE,
  ...
  public static final BooleanConfigParam ALLOW_ARBITER_ACK =
      new BooleanConfigParam(
          ReplicationMutableConfig.ALLOW_ARBITER_ACK,
          true, // default
          true, // mutable
          true);
  ...
  public static final BooleanConfigParam FEEDER_TCP_NO_DELAY =
      new BooleanConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "feederTcpNoDelay",
          true, // default
          false, // mutable
          true);

  /**
   * The time interval in nanoseconds during which records from a feeder may be batched before being
   * written to the network.
   *
   * <p>Larger values can result in fewer network packets and lower interrupt processing overheads.
   * Since the grouping is only done when the feeder knows that the replica is not completely in
   * sync, it's unlikely to have an adverse impact on overall throughput. Consequently this
   * parameter is retained as an internal tuning knob.
   *
   * <p>The HEARTBEAT_INTERVAL parameter serves as a ceiling on this time interval. Parameter values
   * larger than HEARTBEAT_INTERVAL are truncated to HEARTBEAT_INTERVAL.
   */
  public static final IntConfigParam FEEDER_BATCH_NS =
      new IntConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "feederBatchNs",
          Integer.valueOf(0), // min
          Integer.valueOf(Integer.MAX_VALUE), // max
  ...
  /**
   * The size in KB used to batch outgoing feeder records. Upon overflow the existing buffer
   * contents are written to the network and a new batch is initiated. The default value is 64K to
   * take advantage of networks that support jumbo frames.
   */
  public static final IntConfigParam FEEDER_BATCH_BUFF_KB =
      new IntConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "feederBatchBuffKb",
  ...
  /**
   * An internal option, accessed only via the utility {@link DbResetRepGroup} utility, to reset a
   * replication group to a single new member when the replicated environment is opened.
   */
  public static final BooleanConfigParam RESET_REP_GROUP =
      new BooleanConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "resetRepGroup",
          false, // default
          false, // mutable
          true);

  /**
   * An internal option, used with {@link #RESET_REP_GROUP}, that causes the reset of the
   * replication group to retain the original group UUID and to not truncate the VLSN index. Use
   * this option when converting a SECONDARY node to an ELECTABLE node when recovering a replication
   * group.
   */
  public static final BooleanConfigParam RESET_REP_GROUP_RETAIN_UUID =
      new BooleanConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "resetRepGroupRetainUUID",
          false, // default
          false, // mutable
          true);

  /**
   * An internal option to allow converting an ELECTABLE node to a SECONDARY node by ignoring the
   * electable node ID stored in the local rep group DB.
   */
  public static final BooleanConfigParam IGNORE_SECONDARY_NODE_ID =
      new BooleanConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "ignoreSecondaryNodeId",
          false, // default
          false, // mutable
          true);
  ...
  public static final DurationConfigParam MAX_CLOCK_DELTA =
      new DurationConfigParam(
          ReplicationConfig.MAX_CLOCK_DELTA,
          null, // min
          "1 min", // max
          "2 s", // default
          false, // mutable
  ...
  public static final ConfigParam HELPER_HOSTS =
      new ConfigParam(
          ReplicationConfig.HELPER_HOSTS,
          "", // default
          true, // mutable
          true) { // forReplication
        ...
        public void validateValue(String hostPortPairs) throws IllegalArgumentException {
          ...
          for (StringTokenizer tokenizer = new StringTokenizer(hostPortPairs, ",");
              tokenizer.hasMoreTokens(); ) {
            try {
              String hostPortPair = tokenizer.nextToken();
              if (!hostPortSet.add(hostPortPair)) {
                throw new IllegalArgumentException(
                    "Property: " + name + " Duplicate specification: " + hostPortPair);
              }
              validateHostAndPort(hostPortPair, Boolean.getBoolean(SKIP_HELPER_HOST_RESOLUTION));
            } catch (IllegalArgumentException iae) {
              throw new IllegalArgumentException(
                  "Property: " + name + "Error: " + iae.getMessage(), iae);
            }
          }
        }
      };
  ...
  public static final IntConfigParam VLSN_STRIDE =
      new IntConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "vlsn.stride",
          Integer.valueOf(1), // min
          null, // max
  ...
  public static final IntConfigParam ELECTIONS_PRIMARY_RETRIES =
      new IntConfigParam(
          ReplicationConfig.ELECTIONS_PRIMARY_RETRIES, 0, Integer.MAX_VALUE, 2, false, true);
  ...
  /**
   * Used to force setting of SO_REUSEADDR to true on the HA server socket when it binds to its
   * port.
   *
   * <p>Note that the default is false, meaning that the socket has the system-specific default
   * setting associated with it. We set it to true primarily in unit tests where the interacting HA
   * processes are all on the same machine and use of this option is safe.
   *
   * <p>This option is currently intended just for internal test use.
   */
  ...
  /**
   * This option was motivated by the BDA. The BDA uses IB for intra-rack node communications and
   * 10gigE for inter-rack node communications. DNS is used to map the hostname to different IP
   * addresses depending on whether the hostname was resolved from within the rack or outside it.
   * The host thus gets HA traffic on both the IB and 10gigE interfaces and therefore needs to
   * listen on both interfaces. It does so binding its socket using a wild card address when this
   * option iks turned on.
   *
  ...
  /**
   * Determines how long to wait for a bound socket to come free. This option can be useful when
   * dealing with sockets in the TIME_WAIT state to come free so they can be reused. Attempts are
   * made to retry binding to this period at intervals of 1 second until the port is bound
   * successfully, or this wait period is exceeded.
   *
   * <p>A value of zero means that there are no retries. It does not make sense to wait too much
   * longer than the 2 min TIME_WAIT period, but we allow waiting as long as 2.5 min to account for
   * race conditions.
   *
   * <p>This option is currently intended just for internal test use.
   */
  ...
  /**
   * Internal parameter used to determine the poll timeout used when accepting incoming feeder
   * connections. This timeout also determines the frequency of various housekeeping tasks, e.g.
   * detection of a master to replica change, etc.
   */
  public static final DurationConfigParam FEEDER_MANAGER_POLL_TIMEOUT =
      new DurationConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "feederManagerPollTimeout",
          "100 ms", // min
          null, // max
          "1 s", // default
          false, // mutable
  ...
  /**
   * Used to log an info message when a commit log record exceeds this time interval from the time
   * it was created, to the time it was written out to the network.
   */
  ...
  /**
   * Used to log an info message when the time taken to replay a single log entry at a replica
  ...
  /**
   * Changes the notion of an ack. When set to true, a replica is considered to have acknowledged a
   * commit as soon as the feeder has written the commit record to the network. That is, it does not
   * wait for the replica to actually acknowledge the commit via a return message. This permits the
   * master to operate in a more async manner relative to the replica provide for higher throughput.
   *
   * <p>This config parameter is internal.
   */
  ...
  /**
   * Verifies that the port is a reasonable number. The port must be outside the range of "Well
   * Known Ports" (zero through 1024).
   *
   ...
   *
   * @param hostAndPort
   * @param skipHostnameResolution if true, don't bother checking that the hostname resolves
   * @throws IllegalArgumentException
   */
  private static void validateHostAndPort(String hostAndPort, boolean skipHostnameResolution)
      throws IllegalArgumentException {
  ...
  public static final BooleanConfigParam TXN_ROLLBACK_DISABLED =
      new BooleanConfigParam(
          ReplicationConfig.TXN_ROLLBACK_DISABLED,
          false, // default
          false, // mutable
          true); // forReplication
  ...
  public static final BooleanConfigParam ALLOW_UNKNOWN_STATE_ENV_OPEN =
      new BooleanConfigParam(
          ReplicationConfig.ALLOW_UNKNOWN_STATE_ENV_OPEN,
          false, // default
          false, // mutable
          true);
  ...
  public static final BooleanConfigParam DONT_JOIN_REP_GROUP =
      new BooleanConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "dontJoinRepGroup", false, false, true);

  /** Internal parameter used by the Arbiter. */
  public static final BooleanConfigParam ARBITER_USE =
      new BooleanConfigParam(EnvironmentParams.REP_PARAM_PREFIX + "arbiterUse", false, false, true);

  /**
   * Internal parameter used by the Subscriber.
   *
   * <p>If true, the node is a replica that operates as a subscriber.
   */
  public static final BooleanConfigParam SUBSCRIBER_USE =
      new BooleanConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "subscriberUse", false, false, true);

  /**
   * Internal parameter used by network backups.
   *
   * <p>If true, the node is used to support network backups and operates in read only mode, with
   * various daemons disabled.
   *
   * <p>TODO: would be nice to combine ARBITER_USE, SUBSCRIBER_USE and this into one concept.
   */
  public static final BooleanConfigParam NETWORKBACKUP_USE =
      new BooleanConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "networkBackupUse", false, false, true);

  /**
   * Internal parameter used by network backups.
   *
   * <p>See 'Algorithm' in {@link com.sleepycat.je.rep.NetworkRestore}. Is currently 50k, which
   * represents less than 1s of replay time.
   */
  public static final IntConfigParam NETWORKBACKUP_MAX_LAG =
      new IntConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "networkBackupMaxLag",
          0, // min
          null, // max
          50 * 1000, // default
          false, // mutable
  ...
  /**
   * Internal parameter to preserve record version (VLSN). Is immutable forever, i.e., it may not be
   * changed after the environment has been created. It has the following impacts:
   *
   * <p>. The VLSN is stored with the LN in the Btree and is available via the CursorImpl API. . The
   * VLSN is included when migrating an LN during log cleaning.
   *
   * <p>FUTURE: Expose this in ReplicationConfig and improve doc if we make record versions part of
   * the public API.
   */
  public static final BooleanConfigParam PRESERVE_RECORD_VERSION =
      new BooleanConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "preserveRecordVersion",
          false, // default
          false, // mutable
          true); // forReplication

  /**
   * Whether to cache the VLSN in the BIN after the LN has been stripped by eviction, unless caching
   * is explicitly disabled using the CACHE_RECORD_VERSION setting.
   *
   * <p>This setting has no impact if PRESERVE_RECORD_VERSION is not also enabled.
   *
   * <p>FUTURE: Expose this in ReplicationConfig and improve doc if we make record versions part of
   * the public API.
   */
  public static final BooleanConfigParam CACHE_RECORD_VERSION =
      new BooleanConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "cacheRecordVersion",
          true, // default
          false, // mutable
          true); // forReplication

  /**
   * @see ReplicationConfig#PROTOCOL_OLD_STRING_ENCODING TODO: Change default to false in JE 5.1.
   */
  ...
  public static class ChannelTypeConfigParam extends ConfigParam {
  ...
  public static final ConfigParam CHANNEL_FACTORY_CLASS =
      new ConfigParam(
          ReplicationNetworkConfig.CHANNEL_FACTORY_CLASS,
          "", // default
          false, // mutable
          true); // forReplication
  ...
  public static final ConfigParam CHANNEL_FACTORY_PARAMS =
      new ConfigParam(
          ReplicationNetworkConfig.CHANNEL_FACTORY_PARAMS,
          "", // default
          false, // mutable
          true); // forReplication
  ...
  public static final ConfigParam SSL_KEYSTORE_PASSWORD =
      new ConfigParam(
          ReplicationSSLConfig.SSL_KEYSTORE_PASSWORD,
          "", // default
          false, // mutable
          true); // forReplication
  ...
  public static final ConfigParam SSL_KEYSTORE_TYPE =
      new ConfigParam(
          ReplicationSSLConfig.SSL_KEYSTORE_TYPE,
          "", // default
          false, // mutable
          true); // forReplication
  ...
  public static final ConfigParam SSL_TRUSTSTORE_TYPE =
      new ConfigParam(
          ReplicationSSLConfig.SSL_TRUSTSTORE_TYPE,
          "", // default
          false, // mutable
          true); // forReplication
  ...
  public static final ConfigParam SSL_CIPHER_SUITES =
      new ConfigParam(
          ReplicationSSLConfig.SSL_CIPHER_SUITES,
          "", // default
          false, // mutable
          true); // forReplication
  ...
  public static final ConfigParam SSL_PROTOCOLS =
      new ConfigParam(
          ReplicationSSLConfig.SSL_PROTOCOLS,
          "", // default
          false, // mutable
          true); // forReplication
  ...
  public static final ConfigParam SSL_AUTHENTICATOR =
      new ConfigParam(
          ReplicationSSLConfig.SSL_AUTHENTICATOR,
          "", // default
          false, // mutable
          true) { // forReplication
  ...
  public static final ConfigParam SSL_HOST_VERIFIER =
      new ConfigParam(
          ReplicationSSLConfig.SSL_HOST_VERIFIER,
          "", // default
          false, // mutable
          true) { // forReplication
  ...
  public static final IntConfigParam REPLAY_COST_PERCENT =
      new IntConfigParam(
          ReplicationConfig.REPLAY_COST_PERCENT,
          0, // min
          1000, // max
          150, // default
          false, // mutable
  ...
  public static final IntConfigParam REPLAY_FREE_DISK_PERCENT =
      new IntConfigParam(
          ReplicationConfig.REPLAY_FREE_DISK_PERCENT,
          0, // min
          99, // max
          0, // default
          false, // mutable
  ...
  public static final DurationConfigParam SUBSCRIPTION_POLL_INTERVAL =
      new DurationConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "subscriptionPollInterval",
          "10 ms", // min
          null, // max
          "1 s", // default
          false, // not mutable
  ...
  public static final DurationConfigParam SUBSCRIPTION_POLL_TIMEOUT =
      new DurationConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "subscriptionPollTimeout",
          "100 ms", // min
          null, // max
          "30 s", // default
          false, // not mutable
  ...
  public static final IntConfigParam SUBSCRIPTION_MAX_CONNECT_RETRIES =
      new IntConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "subscriptionMaxConnectRetries",
          Integer.valueOf(1), // min
          null, // max
  ...
  /**
   * The amount of time that the subscription thread should sleep time before retrying a failed
   * connection.
   */
  public static final DurationConfigParam SUBSCRIPTION_SLEEP_BEFORE_RETRY =
      new DurationConfigParam(
          EnvironmentParams.REP_PARAM_PREFIX + "subscriptionSleepBeforeRetry",
          "1 s", // min
          null, // max
          "3 s", // default
          false, // not mutable
