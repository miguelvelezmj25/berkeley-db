 ...
 *
 * <p>Each time when there happens a MonitorChangeEvents, it refreshes the group information so that
 * it can send messages to current monitors.
 */
public class MonitorEventManager {
  ...
  public void notifyGroupChange(String nodeName, GroupChangeType opType) throws DatabaseException {

    RepGroupImpl repGroup = repNode.getGroup();
    GroupChange changeEvent = getProtocol(repGroup).new GroupChange(repGroup, nodeName, opType);
  ...
  public void notifyJoinGroup() throws DatabaseException {
    ...
    RepGroupImpl repGroup = repNode.getGroup();
    JoinGroup joinEvent =
        getProtocol(repGroup)
        .new JoinGroup(repNode.getNodeName(), repNode.getMasterName(), joinTime);
  ...
  public void notifyLeaveGroup(LeaveReason reason) throws DatabaseException, InterruptedException {
    ...
    Utils.checkFutures(
        compService, 10, TimeUnit.SECONDS, repNode.getLogger(), repNode.getRepImpl(), null);
  }
  ...
  private com.sleepycat.je.rep.monitor.Protocol getProtocol(RepGroupImpl repGroup) {
    ...
    return new com.sleepycat.je.rep.monitor.Protocol(
        repGroup.getName(), NameIdPair.NOCHECK, null, repNode.getRepImpl().getChannelFactory());
  }

  /* Refresh all the monitors with specified message. */
  private FutureTrackingCompService<MessageExchange> refreshMonitors(
      RepGroupImpl repGroup, RequestMessage requestMessage) {
    Set<InetSocketAddress> monitors = repGroup.getAllMonitorSockets();
    if (monitors.size() > 0) {
      LoggerUtils.info(
          repNode.getLogger(), repNode.getRepImpl(), "Refreshed " + monitors.size() + " monitors.");
    }
    /* Broadcast and forget. */
    return Utils.broadcastMessage(
        monitors,
