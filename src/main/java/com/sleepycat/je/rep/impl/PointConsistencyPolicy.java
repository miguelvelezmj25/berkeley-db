...
/**
 * This is used to ensure that the Replica has finished replaying or proceeded past the vlsn
 * specified by the policy. It's like the externally visible CommitPointConsistencyPolicy, except
 * that the latter restricts consistency points to commit vlsns, whereas this policy lets you sync
 * at uncommitted log entries.
 */
public class PointConsistencyPolicy implements ReplicaConsistencyPolicy {
  ...
  /**
   * Ensures that the replica has replayed the replication stream to the point identified by the
   * commit token. If it isn't the method waits until the constraint is satisfied by the replica.
   */
  @Override
  ...
  public boolean equals(Object obj) {
    ...
    if (targetVLSN == null) {
        return other.targetVLSN == null;
    } else return targetVLSN.equals(other.targetVLSN);
  }
