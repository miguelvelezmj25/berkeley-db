...
/**
 * Establish where the replication stream should start for a feeder and replica pair. The Feeder's
 * job is to send the replica the parts of the replication stream it needs, so that the two can
 * determine a common matchpoint.
 *
 * <p>If a successful matchpoint is found the feeder learns where to start the replication stream
 * for this replica.
 */
public class FeederReplicaSyncup {
   ...
   *
   * @throws NetworkRestoreException if sync up failed and network store is required
   * @throws ChecksumException if checksum validation failed
   */
  public void execute()
      throws DatabaseException, IOException, InterruptedException, NetworkRestoreException,
          ChecksumException {
     ...
     */
    final ProtectedFileSet protectedFileSet = repNode.syncupStarted(feeder.getReplicaNameIdPair());
    ...
    try {
      ...
      while (true) {
        ...
        if (message instanceof StartStream) {
          ...
          if (!auth.checkAccess()) {
            final String err =
                "Replica " + feeder.getReplicaNameIdPair().getName() + " fails security check.";
            LoggerUtils.warning(logger, repImpl, err);

            throw new ReplicationSecurityException(
                err, feeder.getReplicaNameIdPair().getName(), null);
          }
          break;
          ...
          throw EnvironmentFailureException.unexpectedState(
              repImpl, "Expected StartStream or EntryRequest but got " + message);
        }
      }
  ...
  private FeederSyncupReader setupReader(VLSN startVLSN) throws DatabaseException, IOException {
    ...
    return new FeederSyncupReader(
        envImpl, vlsnIndex, lastUsedLsn, readBufferSize, startVLSN, finishLsn);
  }

  private Message makeResponseToEntryRequest(
      VLSNRange range, EntryRequest request, boolean isFirstResponse)
    ...
    if (range.getLast().compareTo(requestMatchpoint) < 0) {
      ...
      if (isFirstResponse) {
        final OutputWireRecord lastSync = getMatchPtRecord(range.getLastSync());
    ...
    final OutputWireRecord matchRecord = getMatchPtRecord(requestMatchpoint);
    if (matchRecord == null) {
      throw EnvironmentFailureException.unexpectedState(
          repNode.getRepImpl(),
          "Couldn't find matchpoint " + requestMatchpoint + " in log. VLSN range=" + range);
    }
  ...
  private NetworkRestoreException answerRestore(VLSNRange range, VLSN failedMatchpoint)
      throws IOException {
    ...
    Message response =
        protocol
        .new RestoreResponse(repNode.getRestoreResponseVLSN(range), repNode.getLogProviders());
    protocol.write(response, namedChannel);

    return new NetworkRestoreException(
        failedMatchpoint, range.getFirst(), range.getLast(), feeder.getReplicaNameIdPair());
  }
  ...
  public static class NetworkRestoreException extends Exception {
    ...
    public NetworkRestoreException(
        VLSN vlsn, VLSN firstVLSN, VLSN lastVLSN, NameIdPair replicaNameIdPair) {
      this.vlsn = vlsn;
    ...
    public String getMessage() {
      return "Matchpoint vlsn "
          + vlsn
          + " requested by node: "
          + replicaNameIdPair
          + " was outside the VLSN range: "
          + "["
          + firstVLSN
          + "-"
          + lastVLSN
