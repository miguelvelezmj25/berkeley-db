...
/**
 * Object to sync-up the Feeder and Subscriber to establish the VLSN from which subscriber should
 * should start stream log entries from feeder.
 */
public class SubscriberFeederSyncup {
  ...
  /**
   * Execute sync-up to the Feeder. Request Feeder to start a replication stream from a start VLSN,
   * if it is available. Otherwise return NULL VLSN to subscriber.
   *
  ...
  public VLSN execute(VLSN reqVLSN) throws InternalException {
    ...
    LoggerUtils.info(
        logger, repImpl, "Subscriber-Feeder " + namedChannel.getNameIdPair() + " syncup started.");
    ...
    try {
      ...
      if (!startVLSN.equals(VLSN.NULL_VLSN)) {
        ...
        protocol.write(protocol.new StartStream(startVLSN, filter), namedChannel);
      } else {
        LoggerUtils.info(
            logger,
            repImpl,
  ...
  /**
   * Request a start VLSN from feeder. The feeder will return a valid start VLSN, which can be equal
   * to or earlier than the request VLSN, or null if feeder is unable to service the requested VLSN.
   *
   * @param requestVLSN start VLSN requested by subscriber
   * @return VLSN a valid start VLSN from feeder, or null if it unavailable at the feeder
  ...
  private VLSN getStartVLSNFromFeeder(VLSN requestVLSN) throws IOException, IllegalStateException {
    ...
    LoggerUtils.fine(
        logger,
        repImpl,
        "Subscriber send requested VLSN "
            + requestVLSN
            + " to feeder "
            + namedChannel.getNameIdPair());

    /* ask the feeder for the requested VLSN. */
    protocol.write(protocol.new EntryRequest(requestVLSN, type), namedChannel);
    ...
    */
    ...
    if (message instanceof Entry) {
      ...
      /* must be exact match for the default type */
        assert !type.equals(EntryRequestType.DEFAULT) || (vlsn.equals(requestVLSN));
      ...
      if (type.equals(EntryRequestType.NOW) || type.equals(EntryRequestType.AVAILABLE)) {
        String msg =
      ...
      vlsn = ((AlternateMatchpoint) message).getAlternateWireRecord().getVLSN();
