...
/**
 * Implementation of a master node acting as a FeederSource. The MasterFeederSource is stateful,
 * because it keeps its own FeederReader which acts as a cursor or scanner across the log files, so
 * it can only be used by a single Feeder.
 */
public class MasterFeederSource implements FeederSource {
  ...
  public MasterFeederSource(
      EnvironmentImpl envImpl, VLSNIndex vlsnIndex, NameIdPair replicaNameIdPair, VLSN startVLSN)
  ...
  public OutputWireRecord getWireRecord(VLSN vlsn, int waitTime)
    ...
    try {
      OutputWireRecord record = feederReader.scanForwards(vlsn, waitTime);
