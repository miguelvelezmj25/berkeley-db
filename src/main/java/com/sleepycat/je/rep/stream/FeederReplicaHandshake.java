...
/**
 * Implements the Feeder side of the handshake between the Feeder and the Replica. The
 * ReplicaFeederHandshake class takes care of the other side.
 *
 * @see <a
 *     href="https://sleepycat.oracle.com/pmwiki/pmwiki.php?n=JEHomePage.ReplicaFeederHandshake">FeederReplicaHandshake</a>
 */
public class FeederReplicaHandshake {
  ...
  /**
   * The negotiated highest version associated with log records to be sent by this feeder in the HA
   * stream.
   */
   ...
   * ReplicaFeederHandshake, and will be kept in place for the entire handshake.
   */
  ...
  /**
   * An instance of this class is created with each new handshake preceding the setting up of a
   * connection.
   *
  ...
  /**
   * Returns the replica node ID. The returned value is only valid after the handshake has been
   * executed.
   *
  ...
  /**
   * Returns the current log version for the feeder, which is the highest log version of any
   * replicable log entry supplied by this feeder. Uses LogEntryType.LOG_VERSION_HIGHEST_REPLICABLE,
   * not LOG_VERSION, since some log versions may have only applied to non-replicable log entries,
   * as was the case for log version 10.
   */
  ...
  /**
   * Return the negotiated log version that will be used for the HA stream between the feeder and
   * the replica.
   */
  ...
  /**
   * Determines log compatibility. Returns null if they are compatible or the server would like to
   * defer the rejection to the replica. Return a JEVersionsReject message if the server does not
   * wish to communicate with the replica.
   *
   * <p>This check requires the log version of the replicas to be greater than or equal to {@value
   * LogEntryType#LOG_VERSION_REPLICATE_OLDER} . Allowing the replica version to be behind the
   * feeder version supports replication during upgrades where the feeder is upgraded before the
   * replica. [#22336]
   *
   * <p>This check also requires that the JE version of the replica is at least the minJEVersion
   * specified by the RepGroupImpl, if any. This check makes sure that nodes running an older
   * software version cannot join the group after a new and incompatible feature has been used.
   */
  private JEVersionsReject checkJECompatibility(
      final Protocol protocol, final JEVersions jeVersions) {

    final int replicaLogVersion = jeVersions.getLogVersion();
    if (replicaLogVersion < LogEntryType.LOG_VERSION_REPLICATE_OLDER - 1) {
      return protocol
      .new JEVersionsReject(
   ...
   * This method should only be called after the {@link #execute} method has returned successfully.
   *
   * @return the replica's JE version or {@code null}
   */
  public JEVersion getReplicaJEVersion() {
    return (replicaJEVersions != null) ? replicaJEVersions.getVersion() : null;
  }

  /**
   * Returns a RepNodeImpl that represents the replica for a successful handshake. This method
   * should only be called after the {@link #execute} method has returned successfully, and will
  ...
  public Protocol execute()
      throws DatabaseException, IOException, ProtocolException, ExitException {

    LoggerUtils.info(logger, repNode.getRepImpl(), "Feeder-replica handshake start");
     ...
     */
    streamLogVersion = Math.min(getCurrentLogVersion(), replicaJEVersions.getLogVersion());

    writeMessage(
        protocol,
        protocol
        .new FeederJEVersions(getCurrentJEVersion(), streamLogVersion, repNode.getMinJEVersion()));
    ...
    LoggerUtils.info(
        ...
        "Feeder-replica "
            ...
            + versionMsg
            + " Stream Log: "
            + protocol.getStreamLogVersion());

    return protocol;
  }

  /** Write a protocol message to the channel. */
  private void writeMessage(final Protocol protocol, final Message message) throws IOException {
  ...
  /**
   * Verifies that the group as configured here at the Feeder matches the configuration of the
   * replica.
   *
  ...
  private void verifyMembershipInfo(Protocol protocol)
      throws IOException, DatabaseException, ExitException {

    NodeGroupInfo nodeGroup = (NodeGroupInfo) (protocol.read(namedChannel));
    ...
    try {
      ...
      if (nodeGroup.getNodeType().hasTransientId()) {
        ...
        if (nodeGroup.getNodeType().isArbiter()) {
          ...
          if (arbMembers.size() > 0) {
            throw new ExitException(
                "An Arbiter node already exists in the " + "replication group.");
          }
        }
        try {
          ...
          if (node == null) {
            throw EnvironmentFailureException.unexpectedState(
                "Node: " + nodeGroup.getNameIdPair() + " not found");
          }
      ...
      if (exception.failReplica()) {
         ...
         * environment.
         */
        writeMessage(protocol, protocol.new NodeGroupInfoReject(exception.getMessage()));
      }
      throw exception;
    }
    ...
    LoggerUtils.fine(
        logger,
        repNode.getRepImpl(),
        "Channel Mapping: " + replicaNameIdPair + " is at " + namedChannel.getChannel());
    writeMessage(protocol, protocol.new NodeGroupInfoOK(group.getUUID(), replicaNameIdPair));
  }

  /**
   * Verifies that the group related information is consistent.
   *
   * @throws ExitException if the configuration in the group db differs from the supplied config
   */
  private void doGroupChecks(NodeGroupInfo nodeGroup, final RepGroupImpl group)
      throws ExitException {

    if (nodeGroup.isDesignatedPrimary() && repNode.getRepImpl().isDesignatedPrimary()) {
      throw new ExitException(
   ...
   * @throws ExitException if the configuration in the group db differs from the supplied config
   */
  private void doNodeChecks(NodeGroupInfo nodeGroup, RepNodeImpl node) throws ExitException {
    ...
    if (nodeGroup.port() != node.getPort()) {
      throw new ExitException(
          "Conflicting ports for replica id: "
              + replicaNameIdPair
              + " Feeder thinks it uses: "
              + node.getPort()
              + " Replica is configured to use: "
              + nodeGroup.port());
    }
    ...
    if (!((NodeType.ELECTABLE == node.getType())
      ...
      throw new ExitException(
          "The replica node: " + replicaNameIdPair + " is of type: " + node.getType());
    }

    if (!nodeGroup.getNodeType().equals(node.getType())) {
      throw new ExitException(
          "Conflicting node types for: "
              + replicaNameIdPair
              + " Feeder thinks it uses: "
              + node.getType()
              + " Replica is configured as type: "
              + nodeGroup.getNodeType());
    }
    replicaNode = node;
  }

  /**
   * Update the node's JE version from the provided group info if storing the JE version is
   * supported and the current version differs from the stored one. It's OK if the attempt does not
   * fully succeed due to a lack of replicas or acknowledgments: we can try again the next time.
   *
   * @throws ExitException if the attempt fails because the updated node's socket address conflicts
   *     with another node
   */
  private void maybeUpdateJEVersion(
      final NodeGroupInfo nodeGroup, final RepGroupImpl group, final RepNodeImpl node)
      throws ExitException {
    ...
    if ((group.getFormatVersion() >= RepGroupImpl.FORMAT_VERSION_3)
       ...
       */
      try {
        repNode.getRepGroupDB().updateMember(new RepNodeImpl(nodeGroup), false);
      } catch (InsufficientReplicasException | InsufficientAcksException | LockTimeoutException e) {
        /* Ignored */
      } catch (NodeConflictException e) {
        throw new ExitException(e, true);
      }
  ...
  /**
   * Negotiates and returns the protocol that will be used in all subsequent interactions with the
   * Replica, if the replica accepts to it.
   *
  ...
  private Protocol negotiateProtocol() throws IOException, ExitException {
    ...
    Protocol defaultProtocol = Protocol.getProtocol(repNode, getCurrentProtocolVersion());
    ...
    if ((dup != null) || (message.getNameIdPair().getName().equals(feederNameIdPair.getName()))) {
      ...
      writeMessage(
          ...
          .new DuplicateNodeReject(
              "This node: " + replicaNameIdPair + " is already in active use at the feeder "));
      SocketAddress dupAddress =
          namedChannel.getChannel().getSocketChannel().socket().getRemoteSocketAddress();

      throw new ExitException(
          "A replica with the id: "
              + replicaNameIdPair
    ...
    Protocol protocol = Protocol.get(repNode, replicaVersion, getCurrentProtocolVersion());
    if (protocol == null) {
      protocol = defaultProtocol;
    }
    defaultProtocol.write(
        defaultProtocol.new FeederProtocolVersion(protocol.getVersion()), namedChannel);
    return protocol;
  }
}
