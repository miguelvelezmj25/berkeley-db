...
/**
 * Format for messages received at across the wire for replication. Instead of sending a direct copy
 * of the log entry as it is stored on the JE log files (LogEntryHeader + LogEntry), select parts of
 * the header are sent.
 *
 * <p>An InputWireRecord de-serializes the logEntry from the message bytes and releases any claim on
...
public class InputWireRecord extends WireRecord {
   ...
   */
  InputWireRecord(
      final EnvironmentImpl envImpl, final ByteBuffer msgBuffer, final BaseProtocol protocol)
      throws DatabaseException {
