...
/**
 * The FeederReader is a flavor of VLSNReader which supports replication stream feeding. It assumes
 * that reading will always go forward in the log. Special features are:
 *
 * <p>- The reader can read either from a log buffer or from the file. Sometimes log entries are
 * logged but are not yet available on disk. In general, it's better to read from the log buffers
 * rather then the file.
 *
 * <p>- The reader can block for a given time period, waiting for the next vlsn to appear
 */
public class FeederReader extends VLSNReader {
  ...
  public FeederReader(
      EnvironmentImpl envImpl, VLSNIndex vlsnIndex, long startLsn, int readBufferSize) {

    this(envImpl, vlsnIndex, startLsn, readBufferSize, false /*bypassCache*/);
  }
  ...
  /**
   * Set up the FeederReader to start scanning from this VLSN. If we find a mapping for this VLSN,
   * we'll start precisely at its LSN, else we'll have to start from an earlier location. This
   * initialization can't be done in the constructor, because the Feeder is set up to require the
   * construction of the source before we know where to start.
   *
   * @throws IOException
   * @return the startLsn
   */
  public long initScan(VLSN startVLSN) throws IOException {
  ...
  /**
   * Forward scanning for feeding the replica: get the log record for this VLSN. If the log record
   * hasn't been created yet, wait for a period specified by "waitTime".
   *
   * <p>Where possible, the FeederReader fetches the log record from the cache within the VLSNIndex.
   * (See the VLSNIndex for a description of this two level cache). If the requested VLSN is not
   * available from the cache, the reader fetches the item from the JE log -- either from the log
   * buffers or from disk.
   *
   * <p>The FeederReader is like a cursor on the log, and retains a position in the log. When there
   * are log item cache hits, the FeederReader's position can fall behind, because it is being
   * bypassed. It is possible for log cleaning to take place between the point of the FeederReader's
   * stale position and the end of the log. If so, the FeederReader must not attempt to scan from
   * its current position, because it might run afoul of gap created by the cleaned and delete log
   * files. When there have been log item cache hits, the FeederReader must jump its position
   * forward using the vlsnIndex mappings to safely skip over any cleaned gaps in the log.
   */
  public OutputWireRecord scanForwards(VLSN vlsn, int waitTime) throws InterruptedException {
    ...
    throw EnvironmentFailureException.unexpectedState(
        envImpl,
        "VLSN=" + vlsn + " repositionLsn = " + DbLsn.getNoFormatString(repositionLsn) + window);
  }

  /**
   * @throw an EnvironmentFailureException if we were scanning for a particular VLSN and we have
   *     passed it by.
   */
  private void checkForPassingTarget(int compareResult) {
    if (compareResult > 0) {
      ...
      throw EnvironmentFailureException.unexpectedState(
          "want to read " + currentVLSN + " but reader at " + currentEntryHeader.getVLSN());
    }
  }
  ...
  static class SwitchWindow extends ReadWindow {
     ...
     * @throws DatabaseException
     */
    private boolean fillFromLogBuffer(long windowFileNum, long targetOffset)
        throws DatabaseException {
     ...
     * number) if needed. Unlike other file readers, we are reading log files that are concurrently
     * growing, so this read window must also know to look in the log buffers.
     *
     * <p>The contract between the feeder reader and the VLSNIndex lets us assume that the feeder
     * reader is only active when it is sure that there is more data available somewhere -- whether
     * it's in the log buffers, write queue, or on disk.
     *
    ...
    protected boolean fillNext(boolean singleFile, int bytesNeeded)
      ...
      try {
         ...
         */
        Long nextFile = fileManager.getFollowingFileNum(currentFileNum(), true /* forward */);
        ...
        if (fillFromLogBuffer(nextFile, FileManager.firstLogEntryOffset())) {
          /*
        ...
        throw EnvironmentFailureException.unexpectedException(
            "Problem in ReadWindow.fill, reading from  = " + currentFileNum(), e);
