...
public class MatchpointSearchResults implements Serializable {
  ...
  public MatchpointSearchResults(EnvironmentImpl envImpl) {
    ...
    passedTxnLimit = envImpl.getConfigManager().getInt(RepParams.TXN_ROLLBACK_LIMIT);
  ...
  /**
   * If we see a checkpoint end record, see if it is a barrier to rolling back, and advance the file
   * reader position.
   */
  ...
  /**
   * Keep track if we have jumped over a gap in the log files, and might have missed a checkpoint
   * end.
   */
  void noteSkippedGap() {
  ...
  /**
   * At the end of the search for a matchpoint, set the matchpointLSN and adjust the debugging list
   * of passed transactions. The matchpoint entry is just before the truncation point, and does not
   * get truncated.
   */
  void setMatchpoint(long match) {
    matchpointLSN = match;
    if ((earliestPassedTxn != null) && (earliestPassedTxn.lsn == matchpointLSN)) {
      numPassedCommits--;
  ...
  /**
   * The reader saw a transaction commit; record relevant information. Note that we record all
   * transactions (replicated and non-replicated) to avoid cutting off a checkpoint. Note that the
   * stream can be a combination of log records from pre and post-dtvlsn masters. Note that we are
   * scanning the log backwards, so we are encountering commits in decreasing VLSN sequence.
   *
   * @param commit the commit being passed. Note that this object is reused and so should not be
   *     saved.
   * @param commitVLSN the VLSN associated with the commit. It could be null if the commit was local
   *     and not HA
  ...
  void notePassedCommits(TxnCommit commit, VLSN commitVLSN, long commitLSN) {
    ...
    earliestPassedTxn = new PassedTxnInfo(commitTime, txnId, commitVLSN, commitLSN, durableCommit);
  ...
  /**
   * If 1 or more commits was passed, construct a message that can be used by RollbackException and
