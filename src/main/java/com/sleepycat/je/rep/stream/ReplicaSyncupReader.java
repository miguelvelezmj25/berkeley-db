...
/**
 * The ReplicaSyncupReader scans the log backwards for requested log entries. The reader must track
 * whether it has passed a checkpoint, and therefore can not used the vlsn index to skip over
 * entries.
 *
 * <p>The ReplicaSyncupReader is not thread safe, and can only be used serially. It will stop at the
 * finishLsn, which should be set using the GlobalCBVLSN.
 */
public class ReplicaSyncupReader extends VLSNReader {
  ...
  private final LogEntry abortLogEntry = LogEntryType.LOG_TXN_ABORT.getNewLogEntry();
  ...
  public OutputWireRecord findPrevSyncEntry(boolean startAtPrev) throws DatabaseException {
    ...
    if (startAtPrev) {
      ...
      LoggerUtils.info(logger1, envImpl, "Restart ReplicaSyncupReader at " + "vlsn " + currentVLSN);
    }
  ...
  /**
   * Return true for ckpt entries, for syncable entries, and if we're in specific vlsn scan mode,
   * any replicated entry. There is an additional level of filtering in processEntry.
   */
  @Override
  protected boolean isTargetEntry() throws DatabaseException {

    if (logger1.isLoggable(Level.FINEST)) {
      LoggerUtils.finest(logger1, envImpl, " isTargetEntry " + currentEntryHeader);
    }
  ...
  /**
   * ProcessEntry does additional filtering before deciding whether to return an entry as a
   * candidate for matching.
   *
   * <p>If this is a record we are submitting as a matchpoint candidate, instantiate a WireRecord to
   * house this log entry. If this is a non-replicated entry or a txn end that follows the candidate
   * matchpoint, record whatever status we need to, but don't use it for comparisons.
   *
   * <p>For example, suppose the log is like this:
   *
   * <p>VLSN entry 10 LN 11 commit 12 LN -- ckpt end 13 commit 14 abort
   *
   * <p>And that the master only has VLSNs 1-12. The replica will suggest vlsn 14 as the first
   * matchpoint. The feeder will counter with a suggestion of vlsn 11, since it does not have vlsn
   * 14.
   *
   * <p>At that point, the ReplicaSyncupReader will scan backwards in the log, looking for vlsn 11.
   * Although the reader should only return an entry when it gets to vlsn 11. The reader must
   * process commits and ckpts that follow 11, so that they can be recorded in the searchResults, so
   * the number of rolled back commits can be accurately reported.
   */
  @Override
  protected boolean processEntry(ByteBuffer entryBuffer) {

    if (logger1.isLoggable(Level.FINEST)) {
      LoggerUtils.finest(logger1, envImpl, " syncup reader saw " + currentEntryHeader);
    }
    byte currentType = currentEntryHeader.getType();
    ...
    if (LogEntryType.LOG_CKPT_END.equalsType(currentType)) {
       ...
       */
      ckptEndLogEntry.readEntry(envImpl, currentEntryHeader, entryBuffer);

      if (logger1.isLoggable(Level.FINEST)) {
        LoggerUtils.finest(
            logger1, envImpl, " syncup reader read " + currentEntryHeader + ckptEndLogEntry);
      }

      if (((CheckpointEnd) ckptEndLogEntry.getMainItem()).getCleanedFilesToDelete()) {
    ...
    if (LogEntryType.LOG_TXN_COMMIT.equalsType(currentType)) {
      ...
      searchResults.notePassedCommits(commit, currentEntryHeader.getVLSN(), getLastLsn());

      if (logger1.isLoggable(Level.FINEST)) {
        LoggerUtils.finest(
            logger1, envImpl, "syncup reader read " + currentEntryHeader + commitLogEntry);
      }
      ...
      searchResults.notePassedAborts(abort, currentEntryHeader.getVLSN());
      if (logger1.isLoggable(Level.FINEST)) {
        LoggerUtils.finest(
            logger1, envImpl, "syncup reader read " + currentEntryHeader + abortLogEntry);
      }
    } else {
  ...
  protected void handleGapInBackwardsScan(long prevFileNum) {
    SkipGapException e = new SkipGapException(window.currentFileNum(), prevFileNum, currentVLSN);
  ...
  public static class SkipGapException extends DatabaseException {
    ...
    public SkipGapException(long currentFileNum, long nextFileNum, VLSN currentVLSN) {
      super(
          "Restarting reader in order to read backwards across gap "
              + "from file 0x"
              + Long.toHexString(currentFileNum)
              + " to file 0x"
              + Long.toHexString(nextFileNum)
              + ". Reset position to vlsn "
              + currentVLSN);
