 ...
 *
 * <p>The lastCommitVLSN, lastCommitTimestamp, and vlsnRate statistics provide general information
 * about committed transactions on the master, but are also intended to be used programmatically
 * along with other statistics for the feeder to provide information about how up-to-date the
 * replicas are. See the Feeder class for more details.
 */
public class FeederTxns {
  ...
  public FeederTxns(RepImpl repImpl) {
    ...
    statistics =
        new StatGroup(FeederTxnStatDefinition.GROUP_NAME, FeederTxnStatDefinition.GROUP_DESC);
    ...
    lastCommitVLSN = new NoClearAtomicLongStat(statistics, LAST_COMMIT_VLSN);
    lastCommitTimestamp = new NoClearAtomicLongStat(statistics, LAST_COMMIT_TIMESTAMP);
    vlsnRate =
        new LongAvgRateStat(statistics, VLSN_RATE, MOVING_AVG_PERIOD_MILLIS, TimeUnit.MINUTES);
  }
   ...
   * @return the TxnInfo associated with the txnId, if txnId needs an ack, null otherwise
   */
  public TxnInfo noteReplicaAck(final RepNodeImpl replica, final long txnId) {
    final DurabilityQuorum durabilityQuorum = repImpl.getRepNode().getDurabilityQuorum();
   ...
   * @param timeoutMs the amount of time to wait for the acknowledgments before giving up.
  ...
  /**
   * Used to track the latch and the transaction information associated with a transaction needing
   * an acknowledgment.
   */
  public class TxnInfo {
    ...
    /**
     * Returns the VLSN associated with the committed txn, or null if the txn has not yet been
     * committed.
     */
    ...
    private final boolean await(int timeoutMs, long ackAwaitStartMs) throws InterruptedException {

      boolean isZero = (latch == null) || latch.await(timeoutMs, TimeUnit.MILLISECONDS);
      if (isZero) {
        txnsAcked.increment();
