 ...
 *
 * <p>Note BaseProtocol represents a set of basic protocol operations intended to be used by
 * subclasses. For a complete description of message operations used in JE HA protocol, please see
 * the Protocol class in the same package.
 *
 * @see Protocol
 */
public abstract class BaseProtocol extends BinaryProtocol {
   ...
   */
  public static final MessageOp ENTRY = new MessageOp((short) 101, Entry.class);

  public static final MessageOp START_STREAM = new MessageOp((short) 102, StartStream.class);

  public static final MessageOp HEARTBEAT = new MessageOp((short) 103, Heartbeat.class);

  public static final MessageOp HEARTBEAT_RESPONSE =
      new MessageOp((short) 104, HeartbeatResponse.class);

  public static final MessageOp COMMIT = new MessageOp((short) 105, Commit.class);

  public static final MessageOp ACK = new MessageOp((short) 106, Ack.class);

  public static final MessageOp ENTRY_REQUEST = new MessageOp((short) 107, EntryRequest.class);

  public static final MessageOp ENTRY_NOTFOUND = new MessageOp((short) 108, EntryNotFound.class);

  public static final MessageOp ALT_MATCHPOINT =
      new MessageOp((short) 109, AlternateMatchpoint.class);

  public static final MessageOp RESTORE_REQUEST = new MessageOp((short) 110, RestoreRequest.class);

  public static final MessageOp RESTORE_RESPONSE =
      new MessageOp((short) 111, RestoreResponse.class);

  public static final MessageOp SHUTDOWN_REQUEST =
      new MessageOp((short) 112, ShutdownRequest.class);

  public static final MessageOp SHUTDOWN_RESPONSE =
      new MessageOp((short) 113, ShutdownResponse.class);

  public static final MessageOp GROUP_ACK = new MessageOp((short) 114, GroupAck.class);
  ...
  /**
   * Whether to fix the log version for log entries received from JE 7.0.x feeders that use log
   * version 12 format but are incorrectly marked with later log versions due to a bug ([#25222]).
   * The problem is that the feeder supplies an entry in log version 12 (LOG_VERSION_EXPIRE_INFO)
   * format, but says it has a later log version.
   *
   * <p>This field is only set to true by the replica, which only reads and writes it from the main
   * Replica thread, so no synchronization is needed.
   */
  private boolean fixLogVersion12Entries = false;

  /**
   * Returns a BaseProtocol object configured that implements the specified (supported) protocol
   * version.
   *
   * @param repImpl the node using the protocol
   * @param nameIdPair name-id pair of the node using the protocol
   * @param protocolVersion the version of the protocol that must be implemented by this object
   * @param maxProtocolVersion the highest supported protocol version, which may be lower than the
   *     code version, for testing purposes
   * @param streamLogVersion the log version of the format used to write log entries
   * @param protocolOps the message operations that make up this protocol
   * @param checkValidity whether to check the message operations for validity. Checks should be
   *     performed for new protocols, but suppressed for legacy ones.
   */
  BaseProtocol(
  ...
  /**
   * Returns a BaseProtocol object configured that implements the specified (supported) protocol
   * version, with enforced message operation code validity check.
   *
   * <p>This constructor enforces checking validity of message operation code. It should be used in
   * any subclass except the legacy HA protocol in je.stream.Protocol.
   *
   * @param repImpl the node using the protocol
   * @param nameIdPair name-id pair of the node using the protocol
   * @param protocolVersion the version of the protocol that must be implemented by this object
  ...
  /**
   * Invoked in cases where the stream log version is not known at the time the protocol object is
   * created and stream log version negotiations are required to determine the version that will be
   * used for log records sent over the HA stream.
   *
   * @param streamLogVersion the maximum log version associated with stream records
   */
  ...
  public class Entry extends Message {
    ...
    public ByteBuffer wireFormat() {
      ...
      final ByteBuffer messageBuffer = allocateInitializedBuffer(bodySize);
    ...
    public Entry(final ByteBuffer buffer) throws DatabaseException {

      inputWireRecord = new InputWireRecord(repImpl, buffer, BaseProtocol.this);
    }
    ...
    public boolean match(Message other) {
       ...
       */
      if (outputWireRecord == null) {
        outputWireRecord = new OutputWireRecord(repImpl, inputWireRecord);
      }
  ...
  /**
   * StartStream indicates that the replica would like the feeder to start the replication stream at
   * the proposed vlsn.
   */
  public class StartStream extends VLSNMessage {
    ...
    public StartStream(ByteBuffer buffer) {
      ...
      final byte[] filterBytes = LogUtils.readBytesNoLength(buffer, length);
      final ByteArrayInputStream bais = new ByteArrayInputStream(filterBytes);
      ...
      try {
        ...
        if (ois != null) {
          try {
            ...
            logger.finest(
                "exception raised when closing the "
    ...
    @Override
    public String toString() {
      String filterString = (feederFilter == null) ? "[no filtering]" : feederFilter.toString();
  ...
  public class Heartbeat extends Message {
    ...
    @Override
    public String toString() {
      return super.toString() + " masterNow=" + masterNow + " currentCommit=" + currentTxnEndVLSN;
    }
  }
  ...
  public class HeartbeatResponse extends Message {
    ...
    public HeartbeatResponse(ByteBuffer buffer) {
      ...
      txnEndVLSN = getVersion() >= VERSION_4 ? new VLSN(LogUtils.readLong(buffer)) : null;
    }
    ...
    public ByteBuffer wireFormat() {
      ...
      int bodySize = includeTxnEndVLSN ? 8 * 2 : 8;
      ByteBuffer messageBuffer = allocateInitializedBuffer(bodySize);
    ...
    @Override
    public String toString() {
      return super.toString() + " txnEndVLSN=" + txnEndVLSN + " syncupVLSN=" + syncupVLSN;
    }
  }
  ...
  public class Commit extends Entry {
    ...
    @Override
    public ByteBuffer wireFormat() {
      final int bodySize = super.getWireSize() + 1 /* needsAck */ + 1 /* replica sync policy */;
      final ByteBuffer messageBuffer = allocateInitializedBuffer(bodySize);
    ...
    public Commit(final ByteBuffer buffer) throws DatabaseException {

      this(getByteNeedsAck(buffer.get()), getByteReplicaSyncPolicy(buffer.get()), buffer);
    }

    private Commit(
        final boolean needsAck, final SyncPolicy replicaSyncPolicy, final ByteBuffer buffer)
  ...
  public class EntryRequest extends VLSNMessage {
    ...
    @Override
    public String toString() {
      return "entry request vlsn: " + super.toString() + ", type: " + type;
    }
  }
   ...
   *
   * <p>RV: VLSN requested by client LOW: low end of available VLSN range in vlsn index HIGH: high
   * end of available VLSN range in vlsn index
   *
   * <p>The DEFAULT mode is used by existing replication stream consumer e.g. replica, arbiter,
   * secondary nodes, etc, while the others are only used in subscription (je.rep.subscription).
   *
   * <p>------------------------------------------------------------------- MODE | RV < LOW | RV in
   * [LOW, HIGH] | RV > HIGH -------------------------------------------------------------------
   * DEFAULT | NOT_FOUND | REQUESTED ENTRY | ALT MATCH POINT AVAILABLE | LOW | REQUESTED ENTRY |
   * HIGH NOW | HIGH | HIGH | HIGH
   */
  ...
  /**
   * Response when the EntryRequest asks for a VLSN that is below the VLSN range covered by the
   * Feeder.
   */
  ...
  public class AlternateMatchpoint extends Message {
    ...
    public AlternateMatchpoint(final ByteBuffer buffer) throws DatabaseException {
      alternateInput = new InputWireRecord(repImpl, buffer, BaseProtocol.this);
    }
    ...
    public boolean match(Message other) {
      ...
      if (alternateOutput == null) {
        alternateOutput = new OutputWireRecord(repImpl, alternateInput);
      }
      return super.match(other);
    }
  }
  ...
  /**
   * Response when the replica needs information to instigate a network restore. The message
   * contains a set of nodes that could be used as the basis for a NetworkBackup so that the request
   * node can become current again.
   *
   * <p>In addition, when support is needed for older replica nodes that use the GlobalCBVLSN, a
   * vlsn is included. See {@link GlobalCBVLSN#getRestoreResponseVLSN}.
   */
  public class RestoreResponse extends SimpleMessage {
    ...
    @Override
    protected void putWireFormat(final ByteBuffer buffer, final Object obj) {
    ...
    private void putRepNodeImplArray(final ByteBuffer buffer, final RepNodeImpl[] ra) {
      LogUtils.writeInt(buffer, ra.length);
      final int groupFormatVersion = getGroupFormatVersion();
      for (final RepNodeImpl node : ra) {
        putByteArray(buffer, RepGroupImpl.serializeBytes(node, groupFormatVersion));
      }
    }
    ...
    private RepNodeImpl[] getRepNodeImplArray(final ByteBuffer buffer) {
      ...
      for (int i = 0; i < ra.length; i++) {
        ra[i] = RepGroupImpl.deserializeNode(getByteArray(buffer), groupFormatVersion);
      }
      return ra;
    }
    ...
    private int getRepNodeImplArraySize(RepNodeImpl[] ra) {
      ...
      final int groupFormatVersion = getGroupFormatVersion();
      for (final RepNodeImpl node : ra) {
        size += (4 /* Node size */ + RepGroupImpl.serializeBytes(node, groupFormatVersion).length);
      }
      return size;
    }

    /** Returns the RepGroupImpl version to use for the currently configured protocol version. */
  ...
  /**
   * Write an entry output wire record to the message buffer using the write log version format and
   * increment nEntriesWrittenOldVersion if the entry format was changed.
   */
  protected void writeOutputWireRecord(
      final OutputWireRecord record, final ByteBuffer messageBuffer) {
    final boolean changedFormat = record.writeToWire(messageBuffer, streamLogVersion);
    if (changedFormat) {
   ...
   */
  private void initializeMessageOps(MessageOp[] protocolOps, boolean checkValidity) {
  ...
  /** Returns whether the byte value specifies that an acknowledgment is needed. */
  ...
  private static boolean isValidMsgOpCode(short opId) {
    ...
    return (opId <= MAX_MESSAGE_OP_CODE_IN_SUBCLASS) && (opId >= MIN_MESSAGE_OP_CODE_IN_SUBCLASS);
  }

  /** Returns the sync policy specified by the argument. */
  private static SyncPolicy getByteReplicaSyncPolicy(final byte syncPolicyByte) {
