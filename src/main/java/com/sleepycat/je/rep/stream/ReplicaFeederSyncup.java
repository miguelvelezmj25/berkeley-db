...
/**
 * Establish where the replication stream should start for a replica and feeder pair. The replica
 * compares what is in its log with what the feeder has, to determine the latest common log entry
 * matchpoint
 *
 * <p>- If the replica has applied log entries after that matchpoint, roll them back - If a common
 * matchpoint can't be found, the replica will need to do a network restore.
 */
public class ReplicaFeederSyncup {
  ...
  public void execute(LocalCBVLSNTracker cbvlsnTracker)
      throws IOException, DatabaseException, InterruptedException, InsufficientLogException,
     ...
     */
    final ProtectedFileSet protectedFileSet = repNode.syncupStarted(namedChannel.getNameIdPair());
    ...
    try {
      ...
      protocol.write(protocol.new StartStream(startVLSN, repImpl.getFeederFilter()), namedChannel);
      LoggerUtils.info(
          logger, repImpl, "Replica-feeder " + feederName + " start stream at VLSN: " + startVLSN);
      ...
      LoggerUtils.info(
          ...
          repImpl,
          String.format(
              "Replica-feeder " + feederName + " syncup ended. Elapsed time: %,dms",
  ...
  /**
   * A matchpoint has been found. What happens next depends on the position of the matchpoint and
   * its relationship to the last transaction end record.
   *
   * <p>In following table, M = some non-null matchpoint VLSN value, T = some non-null last txn end
   * value S = some non-null last sync value
   *
   * <p>Note that currently T == S, since a sync points is always a txn end.
   *
   * <p>txn end last sync found action VLSN VLSN matchpoint ---------- --------- ---------
   * ------------------------ NULL_VLSN NULL_VLSN NULL_VLSN rollback everything NULL_VLSN NULL_VLSN
   * M can't occur NULL_VLSN S NULL_VLSN rollback everything NULL_VLSN S M rollback to M T NULL_VLSN
   * NULL_VLSN can't occur T NULL_VLSN M can't occur T S NULL_VLSN network restore, though could
   * also do hard recov T <= M S M rollback to matchpoint T > M, truncate not ok S M network restore
   * T > M, rollback limit exceeded or S M throw RollbackProhibited disabled T > M, truncate ok S M
   * hard recovery
   *
   * @throws IOException
   * @throws HardRecoveryElectionException
  ...
  private void verifyRollback(VLSNRange range)
      throws RollbackException, InsufficientLogException, HardRecoveryElectionException,
          IOException {
    ...
    LoggerUtils.finest(
        logger,
        repImpl,
        "verify rollback" + " vlsn range=" + range + " searchResults=" + searchResults);
    ...
    if (lastTxnEnd.isNull()) {
      if (range.getLastSync().isNull() && !matchpointVLSN.isNull()) {
        throw EnvironmentFailureException.unexpectedState(
            repNode.getRepImpl(),
            "Shouldn't be possible to find a "
                + "matchpoint of "
                + matchpointVLSN
                + " when the sync VLSN is null. Range="
                + range);
      }
    ...
    if (matchpointVLSN.isNull()) {
      ...
      LoggerUtils.info(
          logger,
          repImpl,
          "This node had a txn end at vlsn = " + lastTxnEnd + "but no matchpoint found.");
    ...
    if (hardRecoveryNeedsElection) {
      throw new HardRecoveryElectionException(
          repNode.getMasterStatus().getNodeMasterNameId(), lastTxnEnd, matchpointVLSN);
    }
    ...
    final int rollbackTxnLimit = configMgr.getInt(RepParams.TXN_ROLLBACK_LIMIT);
    final boolean rollbackDisabled = configMgr.getBoolean(RepParams.TXN_ROLLBACK_DISABLED);

    final int numPassedDurableCommits = searchResults.getNumPassedDurableCommits();
    final int numPassedCommits = searchResults.getNumPassedCommits();
    ...
    LoggerUtils.info(
        ...
        String.format(
            ...
            numPassedCommits, numPassedDurableCommits, dtvlsn, rollbackTxnLimit));
    ...
    if (numPassedDurableCommits > rollbackTxnLimit || rollbackDisabled) {
      ...
      throw new RollbackProhibitedException(
          repNode.getRepImpl(), rollbackTxnLimit, rollbackDisabled, matchpointVLSN, searchResults);
    }
  ...
  /**
   * Find a matchpoint, which is a log entry in the replication stream which is the same on feeder
   * and replica. Assign the matchpointVLSN field. The matchpoint log entry must be be tagged with
   * an environment id. If no matching entry is found, the matchpoint is effectively the NULL_VLSN.
   *
   * <p>To determine the matchpoint, exchange messages with the feeder and compare log entries. If
   * the feeder does not have enough log entries, throw InsufficientLogException.
   *
   * @throws InsufficientLogException
   */
  private void findMatchpoint(VLSNRange range) throws IOException, InsufficientLogException {
    ...
    int matchCounter = 0;
    repImpl.setSyncupProgress(SyncupProgress.FIND_MATCHPOINT, matchCounter++, -1);
    ...
    if (candidateMatchpoint.equals(NULL_VLSN)) {
       ...
       * initial startup.
       */
      getFeederRecord(range, VLSN.FIRST_VLSN, false /*acceptAlternative*/);
      return;
    }
    ...
    InputWireRecord feederRecord =
        getFeederRecord(range, candidateMatchpoint, true /*acceptAlternative*/);
    ...
    backwardsReader =
        setupBackwardsReader(
            candidateMatchpoint, repNode.getRepImpl().getFileManager().getLastUsedLsn());
    ...
    LoggerUtils.finest(
        logger, repImpl, "after setting  matchpoint, searchResults=" + searchResults);
  }
  ...
  /**
   * Search backwards for the replica's log record at this target VLSN. The target record is either
   * the replica's first suggestion for a matchpoint, or feeder's counter offer. We have checked
   * earlier that the counter offer is within the replica's vlsn range.
   */
  private OutputWireRecord getReplicaRecord(VLSN candidateMatchpoint) {
    ...
    do {
      try {
        replicaRecord = backwardsReader.scanBackwards(candidateMatchpoint);
        ...
        backwardsReader = setupBackwardsReader(candidateMatchpoint, startScanLsn);
        /*
  ...
  private OutputWireRecord scanMatchpointEntries() {
    ...
    do {
      try {
         ...
         */
        replicaRecord = backwardsReader.findPrevSyncEntry(firstAttempt);
        ...
        VLSN gapRepositionVLSN = e.getVLSN();
        backwardsReader =
            setupBackwardsReader(gapRepositionVLSN, vlsnIndex.getGTELsn(gapRepositionVLSN));
        firstAttempt = false;
        searchResults.noteSkippedGap();
      }
    } while (true);
  ...
  /**
   * Ask the feeder for information to add to InsufficientLogException, and then throw the
   * exception.
   *
   * <p>The endVLSN marks the last VLSN that this node will want from the network restore. That
   * information helps ensure that the restore source has enough vlsns to satisfy this replica.
   *
   * <p>The replication node list identifies possible log provider members.
   *
   * @throws IOException
   */
  ...
  /**
   * Hard recovery: truncate the files, repeat recovery. If this hard recovery came about before the
   * ReplicatedEnvironment was fully instantiated, we will recreate the environment under the
   * covers. If this came while the replica was up and supporting existing Environment handles, we
   * must invalidate the environment, and ask the application to reopen.
   */
  public RollbackException setupHardRecovery() throws IOException {

    /* Creating the exception invalidates the environment. */
    RollbackException r = new RollbackException(repImpl, matchpointVLSN, searchResults);
    LoggerUtils.severe(
        logger,
        repImpl,
    ...
    repImpl
        .getFileManager()
        .truncateLog(DbLsn.getFileNumber(matchpointLSN), DbLsn.getFileOffset(matchpointLSN));

    return r;
  }

  /**
   * Request a log entry from the feeder at this VLSN. The Feeder will only return the log record or
   * say that it isn't available.
   *
   * @throws InsufficientLogException
   */
  private InputWireRecord getFeederRecord(
      VLSNRange range, VLSN requestVLSN, boolean acceptAlternative)
    ...
    throw EnvironmentFailureException.unexpectedState(
        repNode.getRepImpl(), "Sent EntryRequest, got unexpected response of " + message);
  }
  ...
  /**
   * This interface is used instead of com.sleepycat.je.utilint.TestHook because the doHook method
   * needs to throw InterruptedException.
   */
