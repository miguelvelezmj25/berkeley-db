...
/**
 * Class used by a node to track changes in Master Status. It's updated by the Listener. It
 * represents the abstract notion that the notion of the current Replica Group is definitive and is
 * always in advance of the notion of a master at each node. A node is typically playing catch up as
 * it tries to bring its view in line with that of the group.
 */
public class MasterStatus implements Cloneable {
  ...
  public synchronized void setGroupMaster(
      String hostname, int port, NameIdPair newGroupMasterNameId) {
   ...
   *
   * @return false if the node does not know of a Master, or the group Master is different from the
   *     node's notion the master.
   */
  ...
  /**
   * An assertion form of the above. By combining the check and exception generation in an atomic
   * operation, it provides for an accurate exception message.
   *
  ...
  /**
   * Returns the Node's current idea of the Master. It may be "out of sync" with the Group's notion
   * of the Master
   */
  ...
  /**
   * Returns a socket that can be used to communicate with the group master. It can return null, if
   * there is no current group master, that is, groupMasterNameId is NULL.
   */
