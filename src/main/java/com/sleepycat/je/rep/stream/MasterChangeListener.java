...
public class MasterChangeListener implements Learner.Listener {
  ...
  /**
   * Implements the Listener protocol. The method should not have any operations that might wait,
   * since notifications are single threaded.
   */
  @Override
  public void notify(Proposal proposal, Value value) {
    ...
    try {
      ...
      LoggerUtils.info(
          logger, repNode.getRepImpl(), "Master changed to " + masterValue.getNameId().getName());

      repNode
          .getMasterStatus()
          .setGroupMaster(
              masterValue.getHostName(), masterValue.getPort(), masterValue.getNameId());
