...
/**
 * Implements the Replica side of the handshake protocol between the Replica and the Feeder. The
 * FeederReplicaHandshake class takes care of the other side.
 *
 * @see <a
...
public class ReplicaFeederHandshake {
  ...
  /** Used during testing: A non-zero value overrides the actual protocol version. */
  ...
  private static final int CLOCK_SKEW_MAX_SAMPLE_SIZE = 5;
  private static final long CLOCK_SKEW_MIN_DELAY_MS = 2;
  private final int maxClockDelta;

  /**
   * If the nodeType is an Arbiter, the SNTPRequest message is sent but the clock skew is not
   * checked.
   */
  ...
  public ReplicaFeederHandshake(ReplicaFeederHandshakeConfig conf) {
    ...
    if (nodeType.isArbiter()) {
      ...
      maxClockDelta = repImpl.getConfigManager().getDuration(MAX_CLOCK_DELTA);
      checkClockSkew = true;
    }
  ...
  private static int getCurrentLogVersion() {
    return (testCurrentLogVersion != 0) ? testCurrentLogVersion : LogEntryType.LOG_VERSION;
  }

  /**
   * Set the current log version to a different value, for testing. Specifying {@code 0} reverts to
   * the standard value.
   *
  ...
  /**
   * Set the current protocol version to a different value, for testing. Specifying {@code 0}
   * reverts to the standard value.
   */
  ...
  private Protocol negotiateProtocol() throws IOException {
    ...
    final Protocol defaultProtocol =
        Protocol.getProtocol(
            repImpl, replicaNameIdPair, clock, getCurrentProtocolVersion(), groupFormatVersion);
    ...
    if (message instanceof DuplicateNodeReject) {
      throw new EnvironmentFailureException(
          ...
          "A replica with the name: "
              + replicaNameIdPair
              + " is already active with the Feeder:"
              + feederNameIdPair);
    }
    ...
    LoggerUtils.fine(
        logger,
        repImpl,
        "Channel Mapping: " + feederNameIdPair + " is at " + namedChannel.getChannel());
    ...
    if (configuredProtocol == null) {
      ...
      throw new EnvironmentFailureException(
          ...
          "Incompatible protocol versions. "
              ...
              + " requested by the Feeder: "
              + feederNameIdPair
              + " is not supported by this Replica: "
              + replicaNameIdPair
              + " with protocol version: "
              + defaultProtocol.getVersion()
              + " introduced in JE version: "
   ...
   */
  public Protocol execute() throws IOException, ProtocolException {

    LoggerUtils.info(logger, repImpl, "Replica-feeder handshake start");
    ...
    /**
     * Note whether log entries with later log versions need to be converted to log version 12 to
     * work around [#25222].
     */
    ...
    LoggerUtils.info(
        logger, repImpl, "Replica-feeder " + feederNameIdPair.getName() + " handshake completed.");
    return protocol;
  }
  ...
  private void verifyVersions() throws IOException {
    ...
    protocol.write(
        protocol.new ReplicaJEVersions(getCurrentJEVersion(), getCurrentLogVersion()),
        namedChannel);
    ...
    if (message instanceof JEVersionsReject) {
      ...
      throw new EnvironmentFailureException(
          repImpl,
          EnvironmentFailureReason.HANDSHAKE_ERROR,
          " Feeder: " + feederNameIdPair + ". " + ((JEVersionsReject) message).getErrorMessage());
    }
    ...
    if (feederJEVersions.getLogVersion() > getCurrentLogVersion()) {
      throw new EnvironmentFailureException(
          ...
          " Feeder: "
              ...
              + ". "
              + "Feeder log version "
              + feederJEVersions.getLogVersion()
  ...
  private void verifyMembership() throws IOException {
    ...
    if (message instanceof NodeGroupInfoReject) {
      ...
      throw new EnvironmentFailureException(
          repImpl,
          EnvironmentFailureReason.HANDSHAKE_ERROR,
          " Feeder: " + feederNameIdPair + ". " + reject.getErrorMessage());
    }
    ...
    if (!(message instanceof NodeGroupInfoOK)) {
      throw new EnvironmentFailureException(
          repImpl,
          EnvironmentFailureReason.HANDSHAKE_ERROR,
          " Feeder: " + feederNameIdPair + ". " + "Protocol error. Unexpected response " + message);
    }
  ...
  /**
   * Checks for clock skew wrt the current feeder. It's important that the clock skew be within an
   * acceptable range so that replica can meet any time based consistency requirements requested by
   * transactions. The intent of this check is to draw the attention of the application or the
   * administrators to the skew, not correct it.
   *
   * <p>The scheme implemented below is a variation on the scheme used by <a
   * href="http://tools.ietf.org/html/rfc2030">SNTP</a> protocol. The Feeder plays the role of the
   * SNTP server and the replica the role of the client in this situation. The mechanism used here
   * is rough and does not guarantee the detection of a clock skew, especially since it's a one time
   * check done each time a connection is re-established with the Feeder. The clocks could be in
   * sync at the time of this check and drift apart over the lifetime of the connection. It's also
   * for this reason that we do not store the skew value and make compensations using it when
   * determining replica consistency.
   *
   * <p>Replications nodes should therefore ensure that they are using NTP or a similar time
   * synchronization service to keep time on all the replication nodes in a group in sync.
   *
   * <p>
   *
   * @throws IOException
   ...
   * @throws ProtocolException
   */
  private void checkClockSkew() throws IOException, ProtocolException {
    ...
    do {
      if (checkClockSkew) {
        ...
        isLast =
            (++sampleCount >= CLOCK_SKEW_MAX_SAMPLE_SIZE)
      ...
      SNTPResponse response = protocol.read(namedChannel, SNTPResponse.class);
    ...
    LoggerUtils.logMsg(
        logger,
        repImpl,
        (Math.abs(clockDelta) >= maxClockDelta) ? Level.SEVERE : Level.FINE,
        "Round trip delay: "
            + clockDelay
            + " ms. "
            + "Clock delta: "
