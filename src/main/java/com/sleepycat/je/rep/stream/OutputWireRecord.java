...
/**
 * Format for log entries sent across the wire for replication. In most cases, the bytes are read
 * directly from the log and never need to be serialized into the backing object.
 *
 * <p>Note that the ByteBuffer held within the OutputWireRecord has a limited lifetime. Often it's
 * just sliced, rather than copied from the underlying buffer.
 */
public class OutputWireRecord extends WireRecord {
  ...
  OutputWireRecord(
      final EnvironmentImpl envImpl, final LogEntryHeader header, final ByteBuffer readerBuffer) {
    super(header);
  ...
  /**
   * Creates an OutputWireRecord from a log item. This constructor is used when a Feeder can bypass
   * access to the log because the log item is available in the log item cache associated with the
   * VLSNIndex.
   */
  ...
  OutputWireRecord(final EnvironmentImpl envImpl, final InputWireRecord input) {
    super(input.header);
    this.envImpl = envImpl;
    this.logItem = null;
  ...
  /** Returns the shared replicable log entry associated with the log entry header. */
  ...
  /**
   * Returns a log entry corresponding to the entry buffer. Note that the log entry will only be
   * created once, at most.
   *
   * <p>When a LogItem from the LogItemCache was used to construct this record, we cache the
   * materialized entry in LogItem to try to avoid redundant materialization in multiple feeders.
   */
  public synchronized ReplicableLogEntry instantiateEntry() throws DatabaseException {
    ...
    if (!(entry instanceof ReplicableLogEntry)) {
      throw EnvironmentFailureException.unexpectedState(
          "Log entry type does not support replication: " + entry.getClass().getName());
    }
   ...
   *
   * @return true if this OutputWireRecord has the same logical contents as the InputWireRecord. The
   *     comparison will disregard portions of the logEntry that may be different, such at
   *     timestamps on a Commit entry. Must be called before the entryBuffer that backs this
   *     OutputWireRecord is reused.
   * @throws DatabaseException
   */
  public boolean match(final InputWireRecord input) throws DatabaseException {
   ...
   * For unit tests.
   *
   * @return true if this OutputWireRecord has the same logical contents as "other".
   * @throws DatabaseException
   */
  public boolean match(final OutputWireRecord otherRecord) throws DatabaseException {
    ...
    final LogEntry entry = instantiateEntry();
    final LogEntry otherEntry = otherRecord.instantiateEntry(envImpl, otherRecord.entryBuffer);
    return entry.logicalEquals(otherEntry);
  }

  /** Returns lsn if this entry was created from the log item cache, and NULL_LSN otherwise. */
  long getLogItemLSN() {
  ...
  /**
   * Returns the number of bytes needed to represent the message data for this record for the
  ...
  /**
   * Returns the number of bytes needed to represent the entry portion of the message data for this
   * record for the specified log version.
   */
  private int getEntrySize(final int logVersion) {
    return willReSerialize(logVersion) ? reSerializedSize : header.getItemSize();
  }

  /**
   * Returns whether the log entry will be re-serialized when written, due to a required format
   * change or because an optimized replication format should be used.
   *
   * <p>This method caches its result, so that it can be called twice (from getEntrySize and
   * writeToWire) and will not repeat the calculation or return different results. If true is
   * returned, reSerializedSize will also be set.
   */
    ...
    if (header.getVersion() < 8) {
      ...
      if (logEntry != null) {
        ...
        if (logEntry.hasReplicationFormat()) {
          newSize = logEntry.getSize(logVersion, true /*forReplication*/);
        ...
        reserialize =
            getSharedEntry()
                .isReplicationFormatWorthwhile(entryBuffer, header.getVersion(), logVersion);
      }
    }

    if (reserialize) {
      if (newSize == -1) {
        newSize = instantiateEntry().getSize(logVersion, true /*forReplication*/);
      }
  ...
  /**
   * Returns whether the format of the entry needs to be changed in order to be read by a replica
   * that only understands versions no later than {@code logVersion}.
   */
  ...
  /**
   * Write the log header and entry associated with this instance to the specified buffer using the
   ...
   */
  boolean writeToWire(final ByteBuffer messageBuffer, final int logVersion) {
  ...
  public long getCommitTimeStamp() throws DatabaseException {
    ...
    final TxnCommit txnCommit = (TxnCommit) instantiateEntry().getMainItem();
  ...
  public long getTimeStamp() throws DatabaseException {
    ...
    if (sharedLogEntry instanceof SingleItemEntry) {
      final Class<? extends Loggable> logClass = ((SingleItemEntry) sharedLogEntry).getLogClass();
   ...
   * Returns null for non-replicable entries and for entries not associated with a database.
   *
