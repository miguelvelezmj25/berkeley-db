...
/**
 * Format for log entries sent across the wire for replication. Instead of sending a direct copy of
 * the log entry as it is stored on the JE log files (LogEntryHeader + LogEntry), select parts of
 * the header are sent.
 *
 * @see InputWireRecord
...
abstract class WireRecord {
  ...
  /** Instantiates the log entry for this wire record using the specified environment and data. */
  LogEntry instantiateEntry(final EnvironmentImpl envImpl, final ByteBuffer buffer)
