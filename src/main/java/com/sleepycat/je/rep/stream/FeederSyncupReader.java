...
/**
 * The FeederSyncupReader scans the log backwards for requested log entries. It uses the vlsnIndex
 * to optimize its search, repositioning when a concrete vlsn->lsn mapping is available.
 *
 * <p>The FeederSyncupReader is not thread safe, and can only be used serially. It will stop at the
...
public class FeederSyncupReader extends VLSNReader {
  ...
  /**
   * Set up the FeederSyncupReader to start scanning from this VLSN. If we find a mapping for this
   * VLSN, we'll start precisely at its LSN, else we'll have to start from an earlier location.
   *
