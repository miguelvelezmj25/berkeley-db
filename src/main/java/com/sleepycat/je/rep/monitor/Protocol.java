 ...
 *     <p>Defines the protocol used by the Monitor to keep informed about group changes, and a node
 *     joins/leaves the group. The Master uses the protocol to inform all Monitors about group
 *     change and node join/leave change.
 *     <p>GCHG -> no response expected from the monitor. JoinGroup -> no response expected from the
 *     monitor. LeaveGroup -> no response expected from the monitor.
 */
public class Protocol extends TextProtocol {
  ...
  public final MessageOp GROUP_CHANGE_REQ = new MessageOp("GCHG", GroupChange.class);

  public final MessageOp JOIN_GROUP_REQ = new MessageOp("JG", JoinGroup.class);

  public final MessageOp LEAVE_GROUP_REQ = new MessageOp("LG", LeaveGroup.class);
  ...
  Protocol(
    ...
    initializeMessageOps(new MessageOp[] {GROUP_CHANGE_REQ, JOIN_GROUP_REQ, LEAVE_GROUP_REQ});
  ...
  private abstract class ChangeEvent extends RequestMessage {
    ...
    public ChangeEvent(String line, String[] tokens) throws InvalidMessageException {
  ...
  public class GroupChange extends ChangeEvent {
    ...
    public GroupChange(RepGroupImpl group, String nodeName, GroupChangeType opType) {
      super(nodeName);
      this.group = group;
      this.opType = opType;
    }

    public GroupChange(String line, String[] tokens) throws InvalidMessageException {
    ...
    public String wireFormat() {
      final int repGroupVersion =
          (Double.parseDouble(sendVersion) <= Double.parseDouble(REP_GROUP_V2_VERSION))
  ...
  private abstract class MemberEvent extends ChangeEvent {
    ...
    public MemberEvent(String line, String[] tokens) throws InvalidMessageException {
    ...
    @Override
    public String wireFormat() {
      return super.wireFormat() + masterName + SEPARATOR + joinTime;
    }
  }
  ...
  public class JoinGroup extends MemberEvent {
    ...
    public JoinGroup(String line, String[] tokens) throws InvalidMessageException {
  ...
  public class LeaveGroup extends MemberEvent {
    ...
    public LeaveGroup(String line, String[] tokens) throws InvalidMessageException {
