...
/**
 * The event used to track changes to the composition and status of the group. An instance of this
 * event is created each time there is any change to the group.
 */
...
/**
 * The event generated when the group composition changes. A new instance of this event is generated
 * each time a node is added or removed from the group. Note that SECONDARY nodes do not generate
 * these events.
 */
/*
...
public class GroupChangeEvent extends MonitorChangeEvent {
  ...
  /**
   * Returns the type of the change (the addition of a new member or the removal of an existing
   * member) made to the group. The method {@link MonitorChangeEvent#getNodeName()
   * MonitorChangeEvent.getNodeName} can be used to identify the node that triggered the event.
   *
