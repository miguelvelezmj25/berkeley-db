 ...
 *
 * <p>The following properties identify the target group.
 *
 * <ul>
 *   <li>groupName: the name of the replication group being monitored.
 *   <li>nodeName: the group-wide unique name associated with this monitor node.
 *   <li>nodeHost: the hostname and port associated with this Monitor. Used by group members to
 *       contact the Monitor.
 *   <li>helperHosts: the list of replication nodes which the Monitor uses to register itself so it
 *       can receive notifications about group status changes.
 * </ul>
 *
 * The following properties configure the daemon ping thread implemented within the Monitor. This
 * daemon thread lets the Monitor proactively find status changes that occur when the Monitor is
 * down or has lost network connectivity.
 *
 * <ul>
 *   <li>numRetries: number of times the ping thread attempts to contact a node before deeming is
 *       unreachable.
 *   <li>retryInterval: number of milliseconds between ping thread retries.
 *   <li>timeout: socketConnection timeout, in milliseconds, specified when the ping thread attempts
 *       to establish a connection with a replication node.
 * </ul>
 *
 * @since JE 5.0
 */
public class MonitorConfig implements Cloneable {
  ...
  /**
   * Sets the name for the replication group. The name must be made up of just alpha numeric
   * characters and must not be zero length.
   *
  ...
  /**
   * Sets the name to be associated with this <code>monitor</code>. It must be unique within the
   * group. When the <code>monitor</code> is instantiated and joins the replication group, a check
   * is done to ensure that the name is unique, and a {@link
   * com.sleepycat.je.rep.RestartRequiredException} is thrown if it is not.
   *
  ...
  /**
   * Sets the hostname and port associated with this monitor. The hostname and port combination are
  ...
  public void setHelperHostsVoid(String helperHosts) {
    DbConfigManager.setVal(props, RepParams.HELPER_HOSTS, helperHosts, validateParams);
  }
   ...
   *     <p>Returns the set of sockets associated with helper nodes.
   ...
   *     authoritative value for its socket value.
  ...
  /**
   * Sets the number of times a ping thread attempts to contact a node before deeming it
   * unreachable. The default value is 5.
   */
  ...
  /**
   * Returns the number of times a ping thread attempts to contact a node before deeming it
   * unreachable.
   */
  ...
  /**
   * Sets the socketConnection timeout, in milliseconds, used when the ping thread attempts to
   * establish a connection with a replication node. The default value is 10,000.
   */
  ...
  /**
   * Returns the socketConnection timeout, in milliseconds, used when the ping thread attempts to
   * establish a connection with a replication node.
   */
   ...
   *     MonitorConfig.
   * @param netConfig the new ReplicationNetworkConfig to be associated with this MonitorConfig.
