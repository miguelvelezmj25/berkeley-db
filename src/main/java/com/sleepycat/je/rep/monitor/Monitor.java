...
/**
 * Provides a lightweight mechanism to track the current master node and the members of the
 * replication group. The information provided by the monitor can be used to route update requests
 * to the node that is currently the master and distribute read requests across the other members of
 * the group.
 *
 * <p>The Monitor is typically run on a machine that participates in load balancing, request routing
 * or is simply serving as a basis for application level monitoring and does not have a replicated
 * environment. To avoid creating a single point of failure, an application may need to create
 * multiple monitor instances, with each monitor running on a distinct machine.
 *
 * <p>Applications with direct access to a {@link com.sleepycat.je.rep.ReplicatedEnvironment
 * ReplicatedEnvironment} can use its {@link <a
 * href="{@docRoot}/../ReplicationGuide/replicawrites.html">synchronous and asynchronous
 * mechanisms</a>} for determining the master node and group composition changes. The Monitor class
 * is not needed by such applications.
 *
 * <p>The Monitor generally learns about changes to group status through events issued by
 * replication group members. In addition, the Monitor maintains a daemon thread which periodically
 * pings members of the group so that the Monitor can proactively discover group status changes that
 * occur when it is down or has lost network connectivity.
 *
 * <p>The following code excerpt illustrates the typical code sequence used to initiate a Monitor.
 * Exception handling has been omitted to simplify the example.
 *
 * <pre class="code">
...
public class Monitor {
   ...
   */
  private final Set<String> groupMembers =
  ...
  /**
   * Deprecated as of JE5. Creates a monitor instance using a {@link ReplicationConfig}.
   * Monitor-specific properties that are not available in ReplicationConfig use default settings.
   *
   * @throws IllegalArgumentException if an invalid parameter is specified.
   * @deprecated As of JE 5, replaced by {@link Monitor#Monitor(MonitorConfig)}
   */
  @Deprecated
  ...
  public Monitor(MonitorConfig monitorConfig) {
    ...
    this.channelFactory =
        DataChannelFactoryBuilder.construct(monitorConfig.getRepNetConfig(), groupName);
    repGroupAdmin =
        new ReplicationGroupAdmin(groupName, monitorConfig.getHelperSockets(), channelFactory);
  ...
  /**
   * @hidden Returns the group-wide unique id associated with the monitor
  ...
  /**
   * Registers the monitor with the group so that it can be kept informed of the outcome of
   ...
   *
   * <p>A monitor must be registered at least once in order to be informed of ongoing election
   * results and group changes. Attempts to re-register the same monitor are ignored. Registration,
   * once it has been completed successfully, persists beyond the lifetime of the Monitor instance
   * and does not need to be repeated. Repeated registrations are benign and merely confirm that the
   * current monitor configuration is consistent with earlier registrations of this monitor.
   *
   ...
   *
   * <p>{@link Monitor#register} should be called before starting the listener. If the monitor has
   * not been registered, it will not be updated, and its listener will not be invoked.
   *
   * <p>Once the registration has been completed, the Monitor can start listening even if none of
   * the other nodes in the group are available. It will be contacted automatically by the other
   * nodes as they come up.
   *
   * <p>If the group has a Master, invoking <code>startListener</code> results in a synchronous
   * callback to the application via the {@link MonitorChangeListener#notify(NewMasterEvent)}
   * method. If there is no Master at this time, the callback takes place asynchronously, after the
   * method returns, when a Master is eventually elected.
   *
   * <p>Starting the listener will start the underlying ping thread, which proactively checks group
   * status for changes that might have been missed when this Monitor instance has lost network
  ...
  public void startListener(MonitorChangeListener newListener)
    ...
    serviceDispatcher = new ServiceDispatcher(monitorConfig.getNodeSocketAddress(), channelFactory);
   ...
   * JoinGroupEvents.
   */
  void disableNotify(@SuppressWarnings("hiding") final boolean disableNotify) {
    this.disableNotify = disableNotify;
  }

  /**
   * Notify JoinGroupEvents for currently active nodes in replication group, and update the list of
   * group members.
   */
  private void notifyJoinGroupEventsForActiveNodes(ReplicationGroup group) {
    NodeStateProtocol stateProtocol =
        new NodeStateProtocol(group.getName(), NameIdPair.NOCHECK, null, channelFactory);
    for (ReplicationNode repNode : group.getNodes()) {
      ...
      if (resp instanceof NodeStateResponse) {
        ...
        notifyJoin(
            new JoinGroupEvent(
                response.getNodeName(), response.getMasterName(), response.getJoinTime()));
      }
    }
  }
  ...
  private class MasterChangeListener implements Learner.Listener {
    ...
    public void notify(Proposal proposal, Value value) {
      ...
      try {
        ...
        LoggerUtils.logMsg(
            logger, formatter, Level.INFO, "Monitor notified of new Master: " + currentMasterName);
        if (monitorChangeListener == null) {
          /* No interest */
          return;
        }
        monitorChangeListener.notify(new NewMasterEvent(currentValue));
      } catch (Exception e) {
        LoggerUtils.logMsg(
            logger,
            formatter,
            Level.SEVERE,
            "Monitor change event processing exception: " + e.getMessage());
      }
    }
  }
  ...
  private class PingThread extends Thread {
    ...
    public PingThread(ReplicationGroup group) {
      ...
      this.socketConnectTimeout = monitorConfig.getSocketConnectTimeout();
      setDaemon(true);
    }
      ...
      try {
        while (running) {
          for (int i = 0; i < retries && running; i++) {
             ...
             */
            RepGroupImpl groupImpl = RepInternal.getRepGroupImpl(group);
            /*
             * The query for a master will result in
             * NewMasterEvents, if there are any changes in the
             * master.
             */
            learner.queryForMaster(groupImpl.getAllLearnerSockets());
        ...
        LoggerUtils.logMsg(
            ...
            formatter,
            Level.INFO,
            "The daemon PingThread is interrupted: " + e.getMessage());
      }
    }

    /**
     * Ping all nodes to find out about possible missed events. Manufacture notifications for any
     * missed events.
     */
    private void queryNodes() {
      ...
      for (final ReplicationNode repNode : removedNodes) {
        notifyGroupChange(
            new GroupChangeEvent(repGroup, repNode.getName(), GroupChangeType.REMOVE));
      }
      ...
      for (final String nodeName : groupMembers) {
        if (repGroup.getRepGroupImpl().getNode(nodeName) == null) {
          notifyGroupChange(new GroupChangeEvent(repGroup, nodeName, GroupChangeType.REMOVE));
        }
      }
      ...
      for (final String nodeName : joinEvents.keySet()) {
        final ReplicationNode repNode = repGroup.getRepGroupImpl().getNode(nodeName);
        if (repNode == null) {
    ...
    private void pingNode(ReplicationNode repNode, ReplicationGroup repGroup, boolean removed) {
      ...
      try {
        DbPing ping = new DbPing(repNode, getGroupName(), socketConnectTimeout, channelFactory);
        ...
        if (!groupMembers.contains(name)
            ...
            && !removed) {
          notifyGroupChange(new GroupChangeEvent(repGroup, name, GroupChangeType.ADD));
        }

        /* Send JoinGroupEvent if needed */
        if (!joinEvents.containsKey(name)) {
          notifyJoin(new JoinGroupEvent(name, state.getMasterName(), state.getJoinTime()));
        }
      } catch (IOException e) {
    ...
    private void notifyMissedLeaveEvents(String name) {
      ...
      int counter = (missedLeaveEvents.get(name) == null) ? 1 : missedLeaveEvents.get(name) + 1;
