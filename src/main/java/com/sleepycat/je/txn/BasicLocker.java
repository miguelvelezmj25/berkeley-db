...
/**
 * A non-transactional Locker that simply tracks locks and releases them when releaseNonTxnLocks or
...
public class BasicLocker extends Locker {
  ...
  protected BasicLocker(EnvironmentImpl env) {
    super(
        env, false, // readUncommittedDefault
        false, // noWait
        0); // mandatedId
  }
  ...
  public static BasicLocker createBasicLocker(EnvironmentImpl env, boolean noWait)
   ...
   *
   * <p>If we were to create a new BasicLocker here, it would not share locks with this locker,
   * which violates the definition of this method. This method is not currently called in direct
   * uses of BasicLocker and is overridden by subclasses where it is allowed (e.g., ThreadLocker and
   * ReadCommittedLocker).
   *
  ...
  protected void addLock(Long lsn, LockType type, LockGrantType grantStatus) {
    if ((ownedLock != null && ownedLock.equals(lsn))
        || (ownedLockSet != null && ownedLockSet.contains(lsn))) {
  ...
  /**
   * Stats. Note lack of synchronization while accessing Lock object. Appropriate for unit testing
   * only.
   */
  @Override
  public StatGroup collectStats() throws DatabaseException {
    ...
    StatGroup stats =
        new StatGroup("Locker lock counts", "Read and write locks held by this locker");
