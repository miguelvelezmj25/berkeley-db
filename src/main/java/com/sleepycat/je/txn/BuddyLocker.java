 ...
 *
 * <p>In general, a BuddyLocker can be used whenever the primary (API) locker is in use, and we need
 * to lock a node and release that lock before the primary locker transaction ends. In other words,
 * for this particular lock we don't want to use two-phase locking. To accomplish that we use a
 * separate BuddyLocker instance to hold the lock, while sharing locks with the primary locker. The
 * BuddyLocker can be closed to release this particular lock, without releasing the other locks held
 * by the primary locker.
 *
 * <p>In particular, a ReadCommittedLocker extends BuddyLocker. The ReadCommittedLocker keeps track
 * of read locks, while its buddy Txn keeps track of write locks. The two lockers must share locks
 * to prevent conflicts.
 *
 * <p>In addition, a BuddyLocker is used when acquiring a RANGE_INSERT lock. RANGE_INSERT only needs
 * to be held until the point we have inserted the new node into the BIN. A separate locker is
 * therefore used so we can release that lock separately when the insertion into the BIN is
 * complete. But the RANGE_INSERT lock must not conflict with locks held by the primary locker. So a
 * BuddyLocker is used that shares locks with the primary locker.
 */
public class BuddyLocker extends BasicLocker {
  ...
  public static BuddyLocker createBuddyLocker(EnvironmentImpl env, Locker buddy)
  ...
  /**
   * Forwards this call to the buddy locker. This object itself is never transactional but the buddy
   * may be.
   */
  @Override
  ...
  /**
   * Returns the transaction timeout of the buddy locker, since this locker has no independent
   * timeout.
   */
  @Override
  ...
  /**
   * Forwards to the parent buddy locker, so the buddy can check itself and all of its child
   * buddies.
   */
  @Override
