...
/**
 * Extends BuddyLocker to acquire write locks using the buddy locker (the transaction locker). This
 * is used for ReadCommitted (Degree 2) isolation.
 */
public class ReadCommittedLocker extends BuddyLocker {
   ...
   *
   * @param buddy is a transactional locker that will be used for acquiring write locks.
   */
  private ReadCommittedLocker(EnvironmentImpl env, Locker buddy) {
    ...
    super(
        env,
        (buddy instanceof ReadCommittedLocker) ? buddy.getBuddy() : buddy);
  ...
  /**
   * Returns a new ReadCommittedLocker that shares locks with this locker by virtue of both lockers
   * only holding READ locks. The buddy locker underlying both ReadCommittedLocker lockers is the
   * same transactional locker, so WRITE locks are also shared.
   */
  @Override
  ...
  /**
   * Releases the lock from this locker, or if not owned by this locker then releases it from the
   * buddy locker.
   */
  @Override
  ...
  /**
   * Forwards this method to the transactional buddy. The buddy handles write locks and therefore
   * handles delete information.
   */
  @Override
