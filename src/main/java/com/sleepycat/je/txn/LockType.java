...
/**
 * LockType is a type safe enumeration of all lock types. Methods on LockType objects can be used to
 * determine whether a type conflicts with another type or can be upgraded to another type.
 */
public class LockType {
  ...
  /**
   * NONE is used for requesting a dirty read and does not appear in the conflict or upgrade
   * matrices.
   */
  public static final LockType NONE = new LockType(5, false, "NONE");

  /**
   * RESTART is used for waiting for a restart and does not appear in the conflict or upgrade
   * matrices.
   */
  public static final LockType RESTART = new LockType(6, false, "RESTART");

  /**
   * Whenever the conflict matrix is changed be sure to update this. For every type that can cause a
   * RESTART result call setCausesRestart. This could have been determined programmatically but I
   * chose to maintain it manually to avoid extra code size.
   */
  static {
   ...
   * @see #getConflict
   */
  private static final LockConflict[][] conflictMatrix = {
   ...
   * @see #getUpgrade
   */
  private static final LockUpgrade[][] upgradeMatrix = {
    { // READ is held and there is a request for:
  ...
  /**
   * Returns true if this is a WRITE or RANGE_WRITE lock. For RANGE_INSERT, false is returned
   * because RANGE_INSERT is used to lock the key following the insertion key, not the insertion key
   * itself.
   */
  ...
  /**
   * Returns the LockConfict that results when this lock type is held and the given lock type is
   * requested by another locker.
   */
  ...
  /**
   * Returns the LockUpgrade that results when this lock type is held and the given lock type is
   * requested by the same locker.
   *
   * <p>For the returned LockUpgrade object, getIllegal will never return true because this method
   * fires an assertion if getIllegal returns true.
   */
  LockUpgrade getUpgrade(LockType requestedType) {
    LockUpgrade upgrade = upgradeMatrix[index][requestedType.index];
    assert !upgrade.getIllegal() : this + " to " + requestedType;
    return upgrade;
  }
