...
/**
 * LockInfo is a class that embodies information about a lock instance. The holding thread and the
 * locktype are all contained in the object.
 *
 * <p>This class is public for unit tests.
 */
public class LockInfo implements Cloneable {
  ...
  private static final Map<LockInfo, StackTraceAtLockTime> traceExceptionMap =
      Collections.synchronizedMap(new WeakHashMap<LockInfo, StackTraceAtLockTime>());
  ...
  /**
   * Clone from given LockInfo. Use this constructor when copying a LockInfo and its identity should
   * be copied (e.g., when mutating a thin lock to a thick lock) to ensure that debugging info is
   * retained.
   */
