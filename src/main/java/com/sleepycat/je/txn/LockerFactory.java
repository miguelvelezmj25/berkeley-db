...
public class LockerFactory {
  ...
  /**
   * Get a locker for a write operation, checking whether the db and environment is transactional or
   * not. Must return a non null locker.
   */
...
throw new UnsupportedOperationException();//    return locker;
  ...
  public static Locker getReadableLocker(
      final Database dbHandle, final Transaction userTxn, final boolean readCommittedIsolation) {

    return getReadableLocker(
        dbHandle, (userTxn != null) ? DbInternal.getLocker(userTxn) : null, readCommittedIsolation);
  }

  /** Get a locker for this database handle for a read or cursor operation. */
  public static Locker getReadableLocker(
      final Database dbHandle, Locker locker, boolean readCommittedIsolation) {
    ...
    final boolean autoTxnIsReplicated = dbImpl.isReplicated() && dbImpl.getEnv().isReplicated();

    return getReadableLocker(
        dbHandle.getEnvironment(), locker, autoTxnIsReplicated, readCommittedIsolation);
  }
   ...
   * (IllegalArgument, etc).
   */
  public static Locker getInternalReadOperationLocker(final EnvironmentImpl envImpl) {
