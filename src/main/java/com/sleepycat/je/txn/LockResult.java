...
/**
 * LockResult is the return type of Locker.lock(). It encapsulates a LockGrantType (the return type
 * of LockManager.lock()) and a WriteLockInfo.
 *
 * <p>The WriteLockInfo field is non-null if (a) the locker is transactional, and (b) the request
 * was for a WRITE or WRITE_RANGE lock, and (c) the request was not a non-blocking request that got
 * denied. If so, the WriteLockInfo is either a newly created one or a pre-existing one if the same
 * locker had write-locked the same LSN before.
 */
public class LockResult {
  private final LockGrantType grant;
  private final WriteLockInfo wli;
