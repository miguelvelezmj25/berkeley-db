...
/**
 * Based class for commit and abort records, which are replicated. The log formats for commit and
 * abort are identical.
 */
public abstract class VersionedWriteTxnEnd extends TxnEnd implements VersionedWriteLoggable {
  ...
  public int getLogSize(final int logVersion, final boolean forReplication) {
    ...
    return LogUtils.getPackedLongLogSize(id)
        ...
        + ((logVersion >= LogEntryType.LOG_VERSION_DURABLE_VLSN)
            ? LogUtils.getPackedLongLogSize(dtvlsn)
            : 0);
  }

  @Override
  public void writeToLog(
      final ByteBuffer logBuffer, final int entryVersion, final boolean forReplication) {
  ...
  public boolean isReplicationFormatWorthwhile(
    ...
    return PackedInteger.getReadLongLength(
            logBuffer.array(), logBuffer.arrayOffset() + logBuffer.position())
        > 1;
  }
