...
/**
 * Extends BasicLocker to share locks among all lockers for the same thread. This locker is used
 * when a JE entry point is called with a null transaction parameter.
 */
public class ThreadLocker extends BasicLocker {

  /**
   * Set to allow this locker to be used by multiple threads. This mode should only be set
   * temporarily, for example, while locking in lockAfterLsnChange.
   */
  ...
  public static ThreadLocker createThreadLocker(EnvironmentImpl env, boolean replicated)
  ...
  /**
   * Set the allowMultithreadedAccess mode during execution of this method because a ThreadLocker is
   * not normally allowed to perform locking from more than one thread.
   */
  @Override
   ...
   *
   * @throws IllegalStateException via all Cursor methods that use a non-transactional locker.
   */
  @Override
  ...
  /**
   * Returns a new non-transactional locker that shares locks with this locker by virtue of being a
   * ThreadLocker for the same thread.
   */
  @Override
  ...
  /**
   * Returns whether this locker can share locks with the given locker. Locks are shared when both
   * lockers are ThreadLocker instances for the same thread.
   */
  @Override
