...
/**
 * Locker instances are JE's route to locking and transactional support. This class is the abstract
 * base class for BasicLocker, ThreadLocker, Txn, MasterTxn and ReadonlyTxn. Locker instances are in
 * fact only a transaction shell to get to the lock manager, and don't guarantee transactional
 * semantics.
 *
 * <p>Txn (includes Txns marked autoTxn) MasterTxn and ReadonlyTxn instances are truly
 * transactional. They have potentially different transaction begin and end behaviors.
 */
public abstract class Locker {
  ...
  /**
   * The thread that created this locker. Used for debugging, and by the ThreadLocker subclass. Note
   * that thread may be null if the Locker is instantiated by reading the log.
   */
  protected Thread thread;

  /**
   * Set to false when close() is called. After that point no other locker operations should occur.
   */
  ...
  /**
   * Non-null if this locker is replacing another locker that is in the process of closing because a
   * cursor is being moved.
   */
  private Locker closingLocker;

  /**
   * Create a locker id. This constructor is called very often, so it should be as streamlined as
   * possible. It should never be called directly, because the mandatedId mechanism only works if
   * the generateId() method is overridden to use the mandatedId value.
   *
   * @param readUncommittedDefault if true, this transaction does read-uncommitted by default
   * @param noWait if true, non-blocking lock requests are used.
   */
  protected Locker(
      EnvironmentImpl envImpl, boolean readUncommittedDefault, boolean noWait, long mandatedId) {
  ...
  /**
   * A Locker has to generate its next id. Some subtypes, like BasicLocker, have a single id for all
   * instances because they are never used for recovery. Other subtypes ask the txn manager for an
   * id or use a specific, mandated id.
   */
   ...
   *
   * <p>WARNING: Be sure to always access the timeout with this accessor, since it is overridden in
   * BuddyLocker.
   */
   ...
   *
   * @param timeout The timeout value for the transaction lifetime, in milliseconds. A value of 0
   *     disables timeouts for the transaction.
   ...
   *
   * @param timeout The timeout value for the transaction lifetime, in microseconds. A value of 0
   *     disables timeouts for the transaction.
   * @throws IllegalArgumentException via Transaction.setLockTimeout
   */
  ...
  /** @return true if transaction was created with read-uncommitted as a default. */
  ...
  public void setPreempted() {
     ...
     */
    preemptedCause = new RuntimeException("Lock was preempted by the replication replayer");
  }

  /**
   * Called when obtaining a lock to cause a LockPreemptedException to be thrown if a lock was
   * preempted earlier.
   *
   * <p>This operation is split into two methods, checkPreempted and throwIfPreempted, so that
   * Txn.checkPreempted can call throwIfPreempted for all its BuddyLockers without causing an
   * infinite recursion. This method is overridden by BuddyLocker to forward the call to its parent
   * buddy (the Txn), and by Txn to check all its child buddies.
   *
   * @param allowPreemptedLocker is a locker that is being closed as the result of a cursor move
   *     operation. If the operation is successful then allowPreemptedLocker will be closed, and the
   *     fact that a lock has been stolen from allowPreemptedLocker can be ignored.
   */
  ...
  /**
   * Called by checkPreempted to cause a LockPreemptedException to be thrown if a lock was preempted
   * earlier. Creating the LockPreemptedException sets the txn to abort-only.
   *
   * @see #checkPreempted
   */
  final void throwIfPreempted(final Locker allowPreemptedLocker) throws OperationFailureException {

    if (this != allowPreemptedLocker && preemptedCause != null) {
  ...
  /**
   * This method is called to set the closingLocker when a cursor has been duplicated prior to being
   * moved. The new locker is informed of the old locker, so that a preempted lock taken by the old
   * locker can be ignored. When the operation is complete, this method is called to clear the
   * closingLocker so that a reference to the old closed locker is not held by this object. [#16513]
   *
   * @param closingLocker the old locker that will be closed if the new cursor (using this locker)
  ...
  /**
   * Overridden to perform actions in a non-transactional cursor when it is opened, for example,
   * ReplicaThreadLocker performs consistency checks.
   */
   ...
   *
   * <p>A replicated txn must be used for writing to a replicated DB, and a non-replicated txn must
   * be used for writing to a non-replicated DB. This is critical for avoiding corruption when HA
   * failovers occur [#23234] [#23330].
   *
   * <p>See guard in LN.logInternal.
   */
  ...
  /**
   * Returns true if writes may only be to non-replicated DBs using this locker, or false if writes
   * may only be to replicated DBs.
   *
   * <p>By default (this implementation) local-write is true, since it is allowed for all non-txnal
   * lockers and for all lockers in a standalone environment. This method is overridden and returns
   * false for for user transactions in a replicated environment that are not explicitly configured
   * for local-write.
   *
   * <p>This method is used to describe a locker's configured usage for checking the validity of an
   * API write operation. This is checked by Cursor methods at the beginning of each write
   * operation.
   */
  ...
  /**
   * Abstract method to a blocking or non-blocking lock of the given type on the given LSN. Unlike
   * the lock() method, this method does not throw LockNotAvailableException and can therefore be
   * used by nonBlockingLock to probe for a lock without the overhead of an exception stack trace.
   *
   * @param lsn is the node to lock.
   * @param lockType is the type of lock to request.
   * @param noWait is true to override the defaultNoWait setting. If true, or if defaultNoWait is
   *     true, throws LockNotAvailableException if the lock cannot be granted without waiting.
   ...
   */
  abstract LockResult lockInternal(
      long lsn,
      LockType lockType,
      boolean noWait,
   ...
   * @throws LockConflictException if a blocking lock could not be acquired.
   */
  public LockResult lock(long lsn, LockType lockType, boolean noWait, DatabaseImpl database)
      throws LockNotAvailableException, LockConflictException {

    final LockResult result =
        lockInternal(lsn, lockType, noWait, false /*jumpAheadOfWaiters*/, database);
   ...
   *
   * <p>Unlike lock(), this method returns LockGrantType.DENIED if the lock is denied rather than
   * throwing LockNotAvailableException. This method should therefore not be used as the final lock
   * for a user operation, since in that case LockNotAvailableException should be thrown for a
   * denied lock. It is normally used only to probe for a lock internally, and other recourse is
   * taken if the lock is denied.
   *
   * @param lsn is the node to lock.
   * @param lockType is the type of lock to request.
   * @param jumpAheadOfWaiters grant the lock before other waiters, if any.
   * @param database is the database containing LSN.
   */
  public LockResult nonBlockingLock(
      long lsn, LockType lockType, boolean jumpAheadOfWaiters, DatabaseImpl database) {
    final LockResult result =
        lockInternal(lsn, lockType, true /*noWait*/, jumpAheadOfWaiters, database);
    if (result.getLockGrant() != LockGrantType.DENIED) {
      checkPreempted(closingLocker);
    }
    return result;
  ...
  /**
   * Called when an LN is logged by an operation that will not hold the lock such as
   * eviction/checkpoint deferred-write logging or cleaner LN migration. We must acquire a lock on
   * the new LSN on behalf of every locker that currently holds a lock on the old LSN.
   *
   * <p>Lock is non-blocking because no contention is possible on the new LSN.
   *
   * <p>Because this locker is being used by multiple threads, this method may be called for a
   * locker that has been closed or for which the lock on the old LSN has been released. Unlike
   * other locking methods, in this case we simply return rather than report an error.
   */
  public synchronized void lockAfterLsnChange(long oldLsn, long newLsn, DatabaseImpl dbImpl) {
    ...
    final LockResult lockResult =
        nonBlockingLock(newLsn, lockType, false /*jumpAheadOfWaiters*/, dbImpl);
  ...
  /**
   * In the case where logging occurs before locking, allow lockers to reject the operation (e.g.,
   * if writing on a replica) and also prepare to undo in the (very unlikely) event that logging
   * succeeds but locking fails.
   */
  public abstract void preLogWithoutLock(DatabaseImpl database);

  /**
   * Throws ReplicaWriteException if called for a locker on a Replica. This implementation does
   * nothing but is overridden by replication lockers. [#20543]
   */
  ...
  /**
   * Returns the underlying Txn if the locker is transactional, or null if the locker is
   * non-transactional. For a Txn-based locker, this method returns 'this'. For a BuddyLocker, this
   * method may return the buddy.
   */
  ...
  /**
   * Creates a fresh non-transactional locker, while retaining any transactional locks held by this
   * locker. This method is called when the cursor for this locker is cloned.
   *
   * <p>This method must return a locker that shares locks with this locker, e.g., a ThreadLocker.
   *
   * <p>In general, transactional lockers return 'this' when this method is called, while
   * non-transactional lockers return a new instance.
   */
  public abstract Locker newNonTxnLocker() throws DatabaseException;

  /**
   * Releases any non-transactional locks held by this locker. This method is called when the cursor
   * moves to a new position or is closed.
   *
   * <p>In general, transactional lockers do nothing when this method is called, while
   * non-transactional lockers release all locks as if operationEnd were called.
   */
  public abstract void releaseNonTxnLocks() throws DatabaseException;

  /**
   * Releases locks and closes the locker at the end of a non-transactional cursor operation. For a
   * transctional cursor this method should do nothing, since locks must be held until transaction
   * end.
   */
  ...
  /**
   * Different types of transactions do different things when the operation ends. Txn does nothing,
   * auto Txn commits or aborts, and BasicLocker (and its subclasses) just releases locks.
   *
   * @param operationOK is whether the operation succeeded, since that may impact ending behavior.
   *     (i.e for an auto Txn)
   */
  public abstract void operationEnd(boolean operationOK) throws DatabaseException;

  /**
   * Should be called by all subclasses when the locker is no longer used. For Txns and auto Txns
   * this is at commit or abort. For non-transactional lockers it is at operationEnd.
   */
   ...
   *
   * <p>FUTURE: Note that this method is overridden by Txn, and Txn.abort sets the state to closed
   * when it begins rather than when it ends, but calls close() (the method above) when it ends.
   * This is not ideal and deserves attention in the future.
   */
  ...
  /**
   * Returns true if locking is required for this Locker. All Txnal lockers require it; most
   * BasicLockers do not, but BasicLockers on internal dbs do.
   */
  ...
  /**
   * Database operations like remove and truncate leave behind residual DatabaseImpls that must be
   * purged at transaction commit or abort.
   */
  public abstract void markDeleteAtTxnEnd(DatabaseImpl db, boolean deleteAtCommit)
      throws DatabaseException;
   ...
   * the lock manager will call this when its releasing a lock.
   */
  abstract void removeLock(long lsn) throws DatabaseException;
   ...
   *
   * <p>public for jca/ra/JELocalTransaction.
   *
   * <p>WARNING: Be sure to always access the timeout with this accessor, since it is overridden in
   * BuddyLocker.
   */
  ...
  /**
   * This method is safe to call without synchronizing and this fact is relied on by LockManager
   * when creating exception messages.
   */
  @Override
