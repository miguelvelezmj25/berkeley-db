...
/** A Lock embodies the lock state of a LSN. It includes a set of owners and a list of waiters. */
public // for Sizeof
class LockImpl implements Lock {
  private static final int REMOVE_LOCKINFO_OVERHEAD = 0 - MemoryBudget.LOCKINFO_OVERHEAD;

  /**
   * A single locker always appears only once in the logical set of owners. The owners set is always
   * in one of the following states.
   *
   * <p>1- Empty 2- A single writer 3- One or more readers 4- Multiple writers or a mix of readers
   * and writers, all for txns which share locks (all ThreadLocker instances for the same thread)
   *
   * <p>Both ownerSet and waiterList are a collection of LockInfo. Since the common case is that
   * there is only one owner or waiter, we have added an optimization to avoid the cost of
   * collections. FirstOwner and firstWaiter are used for the first owner or waiter of the lock, and
   * the corresponding collection is instantiated and used only if more owners arrive.
   *
   * <p>In terms of memory accounting, we count up the cost of each added or removed LockInfo, but
   * not the cost of the HashSet/List entry overhead. We could do the latter for more precise
   * accounting.
   */
  ...
  /**
   * The first waiter goes into the firstWaiter member variable. Once the waiterList is made, all
   * appended waiters go into waiterList, even after the firstWaiter goes away and leaves that field
   * null, so as to leave the list ordered.
   */
  ...
  public LockAttemptResult lock(
    ...
    LockGrantType grant =
        tryLock(newLock, jumpAheadOfWaiters || (nWaiters() == 0), mb, lockTableIndex);
  ...
  public void stealLock(Locker locker, MemoryBudget mb, int lockTableIndex) {
    ...
    if (firstOwner != null) {
      ...
      if (thisLocker != locker && thisLocker.getPreemptable()) {
  ...
  /**
   * Called from lock() to try locking a new request, and from release() to try locking a waiting
   * request.
   *
   * @param newLock is the lock that is requested.
   * @param firstWaiterInLine determines whether to grant the lock when a NEW lock can be granted,
   *     but other non-conflicting owners exist; for example, when a new READ lock is requested but
   *     READ locks are held by other owners. This parameter should be true if the requestor is the
   *     first waiter in line (or if there are no waiters), and false otherwise.
   * @param mb is the current memory budget.
   * @return LockGrantType.EXISTING, NEW, PROMOTION, WAIT_RESTART, WAIT_NEW or WAIT_PROMOTION.
   */
  private LockGrantType tryLock(
      LockInfo newLock, boolean firstWaiterInLine, MemoryBudget mb, int lockTableIndex) {
    ...
    while (owner != null) {
      ...
      if (locker == ownerLocker) {
         ...
         */
        if (!locker.sharesLocksWith(ownerLocker) && !ownerLocker.sharesLocksWith(locker)) {
  ...
  /**
   * Called from release() when a RESTART request is waiting to determine if any RANGE_INSERT owners
   * exist. We can't call tryLock for a RESTART lock because it must never be granted.
   */
  ...
  private boolean validateRequest(Locker locker) {
    if (firstWaiter != null) {
        assert firstWaiter.getLocker() != locker : "locker " + locker + " is already on waiters list.";
    }
    ...
    if (waiterList != null) {
      ...
      while (iter.hasNext()) {
        LockInfo o = iter.next();
          assert o.getLocker() != locker : "locker " + locker + " is already on waiters list.";
      }
    }
    return true;
  }
