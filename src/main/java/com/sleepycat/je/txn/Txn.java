 ...
 * This class must support multi-threaded use.
 */
public class Txn extends Locker implements VersionedWriteLoggable {
  ...
  private static final int WRITE_LOCK_OVERHEAD =
      MemoryBudget.HASHMAP_ENTRY_OVERHEAD + MemoryBudget.WRITE_LOCKINFO_OVERHEAD;
  ...
  /**
   * A non-zero mandatedId is specified only by subtypes which arbitrarily impose a transaction id
   * value onto the transaction. This is done by implementing a version of Locker.generateId() which
   * uses the proposed id.
   */
  protected Txn(
  ...
  static Txn createAutoTxn(
      EnvironmentImpl envImpl, TransactionConfig config, ReplicationContext repContext)
      throws DatabaseException {
  ...
  /**
   * True if this transaction should be registered with the transaction manager immediately at
   * startup. True for all transactions except for those ReplayTxns which were created as
   * transformed master transactions.
   */
  ...
  /**
   * Returns the durability used for the commit operation. It's only available after a commit
   * operation has been initiated.
   *
   * @return the durability associated with the commit, or null if the commit has not yet been
   *     initiated.
   */
  ...
  /**
   * @return if this transaction has ever executed a rollback. A Rollback is an undo of the
   *     transaction that can return either to the original pre-txn state, or to an intermediate
   *     intra-txn state. An abort always returns the txn to the pre-txn state.
   */
  @Override
  ...
  /**
   * Gets a lock on this LSN and, if it is a write lock, saves an abort LSN. Caller will set the
   * abortLsn later, after the write lock has been obtained.
   *
   * @throws IllegalStateException via API read/write methods if the txn is closed, in theory.
   *     However, this should not occur from a user API call, because the API methods first call
   *     Transaction.getLocker, which will throw IllegalStateException if the txn is closed. It
   *     might occur, however, if the transaction ends in the window between the call to getLocker
   *     and the lock attempt.
   * @throws OperationFailureException via API read/write methods if an OperationFailureException
   *     occurred earlier and set the txn to abort-only.
   * @see Locker#lockInternal @Override
   */
  @Override
  protected LockResult lockInternal(
      long lsn,
      LockType lockType,
      boolean noWait,
    ...
    /* Ask for the lock. */
    LockGrantType grant =
        lockManager.lock(lsn, this, lockType, timeout, useNoWait, jumpAheadOfWaiters, database);
   ...
   * Subclasses should call super.preLogWithoutLock. [#22875]
   */
  @Override
  ...
  public synchronized int prepare(Xid xid) throws DatabaseException {
    ...
    if (checkCursorsForClose()) {
      throw new IllegalStateException(
          "Transaction " + id + " prepare failed because there were open cursors.");
    }
    ...
    SingleItemEntry<TxnPrepare> prepareEntry =
        SingleItemEntry.create(LogEntryType.LOG_TXN_PREPARE, new TxnPrepare(id, xid));
    /* Flush required. */
   ...
   *
   * <p>1. Run pre-commit hook.
   *
   * <p>2. Release read locks.
   *
   * <p>3. Log a txn commit record and flush the log as indicated by the durability policy.
   *
   * <p>4. Run the post-commit hook.
   *
   * <p>5. Add deleted LN info to IN compressor queue.
   *
   * <p>6. Release all write locks
   *
   * <p>If this transaction has not made any changes to the database, that is, it is a read-only
   * transaction, no entry is made to the log. Otherwise, a concerted effort is made to log a commit
   * entry, or an abort entry, but NOT both. If exceptions are encountered and neither entry can be
   * logged, a EnvironmentFailureException is thrown.
   *
   * <p>Error conditions (in contrast to Exceptions) always result in the environment being
   * invalidated and the Error being propagated back to the application. In addition, if the
   * environment is made invalid in another thread, or the transaction is closed by another thread,
   * then we propagate the exception and we do not attempt to abort. This special handling is prior
   * to the pre-commit stage.
   *
   * <p>From an exception handling viewpoint the commit goes through two stages: a pre-commit stage
   * spanning steps 1-3, and a post-commit stage spanning steps 4-5. The post-commit stage is
   * entered only after a commit entry has been successfully logged.
   *
   * <p>Any exceptions detected during the pre-commit stage results in an attempt to log an abort
   * entry. A NULL commitLsn (and abortLsn) indicates that we are in the pre-commit stage. Note in
   * particular, that if the log of the commit entry (step 3) fails due to an IOException, then the
   * lower levels are responsible for wrapping it in a EnvironmentFailureException which is
   * propagated directly to the application.
   *
   * <p>Exceptions thrown in the post-commit stage are examined to see if they are expected and must
   * be propagated back to the caller after completing any pending cleanup; some replication
   * exceptions fall into this category. If the exception was unexpected, the environment is
   * invalidated and a EnvironmentFailureException is thrown instead. The current implementation
   * only allows propagation of exceptions from the post-commit hook, since we do not expect
   * exceptions from any of the other post-commit operations.
   *
   * <p>When there are multiple failures in commit(), we want the caller to receive the first
   * exception, to make the problem manifest. So an effort is made to preserve that primary
   * exception and propagate it instead of any following, secondary exceptions. The secondary
   * exception is always logged in such a circumstance.
   *
   * @throws IllegalStateException via Transaction.commit if cursors are open.
   * @throws OperationFailureException via Transaction.commit if an OperationFailureException
   *     occurred earlier and set the txn to abort-only.
   *     <p>Note that IllegalStateException should never be thrown by Transaction.commit because of
   *     a closed txn, since Transaction.commit and abort set the Transaction.txn to null and
   *     disallow subsequent method calls (other than abort). So in a sense the call to
   *     checkState(true) in this method is unnecessary, although perhaps a good safeguard.
   */
          ...
          try {
            ...
            if (txnState == Transaction.State.MUST_ABORT) {
              throw EnvironmentFailureException.unexpectedException(
                  "postLogCommitHook may not set MUST_ABORT", hookException);
            }
  ...
  /**
   * Aborts the current transaction and throws the pre-commit Exception, wrapped in a Database
   * exception if it isn't already a DatabaseException.
   *
   * <p>If the attempt at writing the abort entry fails, that is, if neither an abort entry, nor a
   * commit entry was successfully written to the log, the environment is invalidated and a
   * EnvironmentFailureException is thrown. Note that for HA, it's necessary that either a commit or
   * abort entry be made in the log, so that it can be replayed to the replicas and the transaction
   * is not left in limbo at the other nodes.
   *
   * @param durability used to determine whether the abort record should be flushed to the log.
  ...
  private void throwPreCommitException(Durability durability, RuntimeException preCommitException) {
    ...
    try {
      ...
      LoggerUtils.traceAndLogException(
          envImpl, "Txn", "commit", "Commit of transaction " + id + " failed", preCommitException);
    } catch (Error e) {
      envImpl.invalidate(e);
      throw e;
      ...
      String message =
          "Failed while attempting to commit transaction "
              + id
              + ". The attempt to abort also failed. "
      ...
      if ((writeInfo != null) && (abortLsn == NULL_LSN)) {
        ...
        throw new EnvironmentFailureException(
            envImpl, EnvironmentFailureReason.LOG_INCOMPLETE, message, preCommitException);
      }
   ...
   * @throws DatabaseException
   */
  private LogItem logCommitEntry(
      SyncPolicy flushSyncBehavior, Collection<WriteLockInfo> obsoleteLsns)
      throws DatabaseException {
    ...
    final CommitLogEntry commitEntry =
        new CommitLogEntry(new TxnCommit(id, lastLoggedLsn, getReplicatorNodeId(), getDTVLSN()));
    ...
    try {
      ...
      if (envImpl.isValid()) {
        throw EnvironmentFailureException.unexpectedException(
            envImpl, "Unexpected non-fatal exception while logging commit", e);
      }
      throw e;
    } catch (Error e) {
  ...
  /**
   * Pre-log check for an invalid environment or interrupted thread (this thread may have been
   * interrupted but we haven't found out yet, because we haven't done a wait or an I/O) to narrow
   * the time window where a commit could become partially durable. See getPartialDurability.
   * [#21264]
   */
  ...
  /**
   * Extract obsolete LSN info from writeInfo. Do not add a WriteInfo if a slot with a deleted LN
   * was reused (abortKnownDeleted), to avoid double counting. And count each abortLSN only once.
   */
  ...
  /**
   * Abort this transaction. This flavor does not return an LSN, nor does it require the logging of
   * a durable abort record.
   */
  ...
  /**
   * Abort this transaction. Steps are: 1. Release LN read locks. 2. Write a txn abort entry to the
   * log. This is used for log file cleaning optimization and replication, and there's no need to
   * guarantee a flush to disk. 3. Find the last LN log entry written for this txn, and use that to
   * traverse the log looking for nodes to undo. For each node, use the same undo logic as recovery
   * to undo the transaction. Note that we walk the log in order to undo in reverse order of the
   * actual operations. For example, suppose the txn did this: delete K1/D1 (in LN 10) create K1/D1
   * (in LN 20) If we process LN10 before LN 20, we'd inadvertently create a duplicate tree of "K1",
   * which would be fatal for the mapping tree. 4. Release the write lock for this LN.
   *
   * <p>An abort differs from a rollback in that the former always undoes every operation, and
   * returns it to the pre-txn state. A rollback may return the txn to an intermediate state, or to
   * the pre-txn state.
   */
   ...
   *     <p>Note that IllegalStateException should never be thrown by Transaction.abort because of a
   *     closed txn, since Transaction.commit and abort set the Transaction.txn to null and disallow
   *     subsequent method calls (other than abort). So in a sense the call to checkState(true) in
    ...
    try {
          ...
          if (updateLoggedForTxn()) {
            ...
            abortLsn =
                forceFlush
                    ? envImpl
                        .getLogManager()
                        .logForceFlush(abortEntry, true /* fsyncRequired */, repContext)
      ...
      synchronized (this) {
        ...
        if (openCursors) {
          envImpl.checkIfInvalid();
          throw new IllegalStateException(
              "Transaction " + id + " detected open cursors while aborting");
        }
      }
    } finally {
      ...
      while (undoLsn != NULL_LSN) {
        ...
        if (firstInstance(alreadyUndoneLsns, alreadyUndoneSlots, undo)) {
          ...
          RecoveryManager.abortUndo(
              envImpl.getLogger(), Level.FINER, location, undo.db, undo.logEntry, undoLsn);
  ...
  /**
   * Called by LNLogEntry.postLogWork() via the LogManager (while still under the LWL) after a
   * transactional LN is logged. Also called by the recovery manager when logging a transaction
   * aware object.
   *
   * <p>This method is synchronized by the caller, by being called within the log latch. Record the
   * last LSN for this transaction, to create the transaction chain, and also record the LSN in the
   * write info for abort logic.
   */
  ...
  /**
   * [#16861] The firstLoggedLsn field is volatile to avoid making getFirstActiveLsn synchronized,
   * which causes a deadlock in HA.
   *
   ...
   * @param deleteAtCommit true if this databaseImpl should be cleaned on commit, false if it should
   *     be cleaned on abort.
   */
  @Override
  ...
  /**
   * Cleanup leftover databaseImpls that are a by-product of database operations like
   * removeDatabase(), truncateDatabase().
   *
   * <p>This method must be called outside the synchronization on this txn, because it calls
   * finishDeleteProcessing, which gets the TxnManager's allTxns latch. The checkpointer also gets
   * the allTxns latch, and within that latch, needs to synchronize on individual txns, so we must
   * avoid a latching hiearchy conflict.
   *
   * <p>[#16861] FUTURE: Perhaps this special handling is no longer needed, now that firstLoggedLsn
  ...
  private boolean checkRepeatedDeletedDB(
      DatabaseCleanupInfo[] infoArray, DatabaseCleanupInfo info) {
  ...
  /**
   * Remove the lock from the set owned by this transaction. If specified to LockManager.release,
   * the lock manager will call this when its releasing a lock. Usually done because the transaction
   * doesn't need to really keep the lock, i.e for a deleted record.
   */
  @Override
  ...
  /**
   * Returns 'this', since this locker holds no non-transactional locks. Since this is returned,
   * sharing of locks is obviously supported.
   */
  @Override
  ...
  /**
   * Called at the end of a database open operation to add the database handle to a user txn. When a
   * user txn aborts, handles opened using that txn are invalidated.
   *
   * <p>A non-txnal locker or auto-commit txn does not retain the handle, because the open database
   * operation will succeed or fail atomically and no database invalidation is needed at a later
   * time.
   *
   * @see HandleLocker
   */
  ...
  /**
   * Check if all cursors associated with the txn are closed. If not, those open cursors will be
   * forcibly closed.
   *
  ...
  public StatGroup collectStats() {
    StatGroup stats =
        new StatGroup("Transaction lock counts", "Read and write locks held by transaction " + id);
  ...
  /**
   * Checks for preemption in this locker and all its child buddies. Does NOT call checkPreempted on
   * its child buddies, since this would cause an infinite recursion.
   */
  @Override
   ...
   *
   * <p>If calledByAbort is true, it means we're being called from abort(). But once closed, a
   * Transaction never calls abort(). See comment at the top of abortInternal.
   *
   * <p>Caller must invoke with "this" synchronized.
   */
  @Override
    ...
    switch (txnState) {
      ...
      case MUST_ABORT:
        ...
        throw onlyAbortableCause.wrapSelf(
            "Transaction " + id + " must be aborted, caused by: " + onlyAbortableCause);
  ...
  /**
   * This method is overridden by HA txn subclasses and returns the node id of the master node that
   * committed or aborted the txn.
   */
  ...
  /**
   * This method is overridden by replication txn subclasses and returns the DTVLSN associated with
   * the Txn.
   */
  ...
  public void writeToLog(
      final ByteBuffer logBuffer, final int logVersion, final boolean forReplication) {
  ...
  @Override
  public boolean isReplicationFormatWorthwhile(
      final ByteBuffer logBuffer, final int srcVersion, final int destVersion) {
    return false;
  }
   ...
   * enabled. The string construction can be numerous enough to show up on a performance profile.
   */
  ...
  /**
   * Store information about a DatabaseImpl that will have to be purged at transaction commit or
   * abort. This handles cleanup after operations like Environment.truncateDatabase,
   * Environment.removeDatabase. Cleanup like this is done outside the usual transaction commit or
   * node undo processing, because the mapping tree is always auto Txn'ed to avoid deadlock and is
  ...
  /**
   * A replicated environment introduces some new considerations when entering a transaction scope
   * via an Environment.transactionBegin() operation.
   *
   * <p>On a Replica, the transactionBegin() operation must wait until the Replica has synched up to
   * where it satisfies the ConsistencyPolicy that is in effect.
   *
   * <p>On a Master, the transactionBegin() must wait until the Feeder has sufficient connections to
   * ensure that it can satisfy the ReplicaAckPolicy, since if it does not, it will fail at commit()
   * and the work done in the transaction will need to be undone.
   *
   * <p>This hook provides the mechanism for implementing the above support for replicated
   * transactions. It ignores all non-replicated transactions.
   *
   * <p>The hook throws ReplicaStateException, if a Master switches to a Replica state while waiting
   * for its Replicas connections. Changes from a Replica to a Master are handled transparently to
   * the application. Exceptions manifest themselves as DatabaseException at the interface to
   ...
   * environment. It's invoked for transactions executed on the master or replica, but is only
   * relevant to transactions being done on the master. When invoked for a transaction on a replica
   * the implementation just returns.
   *
   * <p>The hook is invoked at a very specific point in the normal commit sequence: immediately
   * before the commit log entry is written to the log. It represents the last chance to abort the
   * transaction and provides an opportunity to make some final checks before allowing the commit
   * can go ahead. Note that it should be possible to abort the transaction at the time the hook is
   * invoked.
   *
   * <p>After invocation of the "pre" hook one of the "post" hooks: postLogCommitHook or
   * postLogAbortHook must always be invoked.
   *
   * <p>Exceptions thrown by this hook result in the transaction being aborted and the exception
   * being propagated back to the application.
   *
   * @throws DatabaseException if there was a problem and that the transaction should be aborted.
   */
  ...
  /**
   * This hook is invoked after the commit record has been written to the log, but before write
   * locks have been released, so that other application cannot see the changes made by the
   * transaction. At this point the transaction has been committed by the Master.
   *
   * <p>Exceptions thrown by this hook result in the transaction being completed on the Master, that
   ...
   * @throws DatabaseException to indicate that there was a replication related problem that needs
   *     to be communicated back to the application.
   */
  ...
  /**
   * Invoked if the transaction associated with the preLogCommitHook was subsequently aborted, for
   * example due to a lack of disk space. This method is responsible for any cleanup that may need
   * to be done as a result of the abort.
   *
   * <p>Note that only one of the "post" hooks (commit or abort) is invoked following the invocation
   * of the "pre" hook.
   */
  ...
  /**
   * Identifies exceptions that may be propagated back to the caller during the postCommit phase of
   * a transaction commit.
   *
   * @param postCommitException the exception being evaluated
   * @return true if the exception must be propagated back to the caller, false if the exception
   *     indicates there is a serious problem with the commit operation and the environment should
   *     be invalidated.
   */
  ...
  /** Use the marker Sets to record whether this is the first time we've see this logical node. */
  ...
  /**
   * Returns the set of databases for which transaction commit/abort triggers must be run. Returns
   * Null if no triggers need to be run.
   */
  ...
  /**
   * Txn freezing is used to prevent changes to transaction lock contents. A frozen transaction
   * should ignore any transaction commit/abort requests. This is used only by MasterTxns, as a way
   * of holding a transaction stable while cloning it to serve as a ReplayTxn during master->replica
   * transitions.
   *
