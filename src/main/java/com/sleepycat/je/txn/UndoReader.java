...
/**
 * Convenience class to package together the different steps and fields needed for reading a log
 * entry for undoing. Is used for both txn aborts and recovery undos.
 */
public class UndoReader {
   ...
   *
   * <p>Never returns null. The DB ID of the LN must be present in undoDatabases, or a fatal
   * exception is thrown.
   */
  public static UndoReader create(
      EnvironmentImpl envImpl, long undoLsn, Map<DatabaseId, DatabaseImpl> undoDatabases) {

    final WholeEntry wholeEntry =
        envImpl.getLogManager().getWholeLogEntryHandleFileNotFound(undoLsn);
    ...
    if (db == null) {
      throw EnvironmentFailureException.unexpectedState(
          envImpl, "DB not found during non-recovery undo/rollback, id=" + dbId);
    }
  ...
  /**
   * Set up an UndoReader when doing a recovery partial rollback. In that case, we have a file
   * reader positioned at the pertinent log entry.
   *
   * <p>This method calls DbTree.getDb. The caller is responsible for calling DbTree.releaseDb on
   * the db field.
   *
   * <p>Null is returned if the DB ID of the LN has been deleted.
   */
