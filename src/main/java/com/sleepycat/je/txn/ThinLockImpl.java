...
/**
 * Implements a lightweight Lock with no waiters and only a single Owner. If, during an operation
 * (lock) more than one owner or waiter is required, then this will mutate to a LockImpl, perform
...
public class ThinLockImpl extends LockInfo implements Lock {
  ...
  public LockAttemptResult lock(
    ...
    if (this.locker != null && this.locker != locker) {
      ...
      return newLock.lock(
          requestType, locker, nonBlockingRequest, jumpAheadOfWaiters, mb, lockTableIndex);
    }
