 ...
 *
 * <p>Note that locks are counted as taking up part of the JE cache.
 */
public abstract class LockManager implements EnvConfigObserver {
  ...
  private static final long REMOVE_TOTAL_LOCKIMPL_OVERHEAD = 0 - TOTAL_LOCKIMPL_OVERHEAD;

  private static final long REMOVE_TOTAL_THINLOCKIMPL_OVERHEAD = 0 - TOTAL_THINLOCKIMPL_OVERHEAD;
  ...
  private static final RangeRestartException rangeRestartException = new RangeRestartException();
  private static boolean lockTableDump = false;

  /**
   * Maps a thread to a set of ThreadLockers. Currently this map is only maintained (non-null) in a
   * replicated environment because it is only needed for determining when to throw
   * LockPreemptedException.
   *
   * <p>Access to the map need not be synchronized because it is a ConcurrentHashMap. Access to the
   * TinyHashSet stored for each thread need not be synchronized, since it is only accessed by a
   * single thread.
   *
   * <p>A TinyHashSet is used because typically only a single ThreadLocker per thread will be open
   * at one time.
   *
   * @see ThreadLocker#checkPreempted [#16513]
   */
  ...
  public LockManager(final EnvironmentImpl envImpl) {
    ...
    for (int i = 0; i < nLockTables; i++) {
      ...
      lockTableLatches[i] =
          LatchFactory.createExclusiveLatch(envImpl, "Lock Table " + i, true /*collectStats*/);
    }
   ...
   *     when nonBlockingRequest is true.
   * @throws LockConflictException if lock could not be acquired. If nonBlockingRequest is true, a
   *     LockConflictException is never thrown. Otherwise, if the lock acquisition would result in a
   *     deadlock, DeadlockException is thrown. Otherwise, if the lock timeout interval elapses and
   *     no deadlock is detected, LockTimeoutException is thrown.
   */
  public LockGrantType lock(
    ...
    synchronized (locker) {
      ...
      result = attemptLock(lsn, locker, type, nonBlockingRequest, jumpAheadOfWaiters);
      ...
      if (txnTimeout > 0) {
        final long txnTimeRemaining = timeRemain(txnTimeout, locker.getTxnStartMillis());
       ...
       * we're done.
       */
      if (performDeadlockDetectionDelay(lsn, locker, type, grant, timeout, startTime)) {
        return grant;
      }
    }
    ...
    while (true) {
      ...
      synchronized (locker) {
        waitResult = waitForLock(result, lsn, locker, type, lastDC, timeout, startTime, database);
      }
       ...
       * Notify the victim.
       */
      final Pair<Boolean, DeadlockChecker> nvResult =
          notifyVictim(victim, locker, lsn, type, lastDC, timeout, startTime, database);
  ...
  /**
   * Waits for a lock that was previously requested. Handles deadlocks and timeouts. However, does
   * not notify the victim when a deadlock is detected and the current locker is not the victim; in
   * that case the caller must notify the victim. This method cannot notify the victim because it is
   * synchronized on the current locker, and we can synchronize on only one locker at a time.
   *
   * <p>This method must be called while synchronized on the locker, for several reasons: 1. The
   * locker will be modified if the lock is acquired by stealLock and when finishLock is called. 2.
   * It must be synchronized when performing the lock delay.
   *
   * @return the lock result and a null victim if locking was successful, or a non-null victim if a
   *     deadlock is detected and the current locker is not the victim.
   * @throws DeadlockException when a deadlock is detected and the current locker is the victim.
   * @throws LockTimeoutException when the timeout elapses and no deadlock is detected.
   * @throws TransactionTimeoutException when the transaction time limit was exceeded.
   */
  private WaitForLockResult waitForLock(
    ...
    while (true) {
      ...
      if (envImpl.getDeadlockDetection() && !isImportunate) {
        ...
        /* Do deadlock detect */
        final DeadlockResult dlr = checkAndHandleDeadlock(locker, lsn, type, timeout, database);
      ...
      final boolean thisLockTimedOut = (!waitForever && (now - startTime) >= timeout);

      final boolean isRestart = (result.lockGrant == LockGrantType.WAIT_RESTART);
      ...
      final boolean getOwnersAndWaiters = (lockerTimedOut || thisLockTimedOut) && !isImportunate;
      ...
      if (lastDC != null) {
        throw makeDeadlockException(lastDC, locker, timeout, false /*isVictim*/, database);
      }
  ...
  private void finishLock(
      final Locker locker, final Long nid, final LockType type, final LockGrantType grant) {
  ...
  /**
   * Returns the Lockers that own a lock on the given LSN. Note that when this method returns, there
   * is nothing to prevent these lockers from releasing the lock or being closed.
   */
  ...
  List<LockInfo> getWaitersInternal(final Long lsn, final int lockTableIndex) {
    /* Get the target lock. */
    final Map<Long, Lock> lockTable = lockTables[lockTableIndex];
    final Lock useLock = lockTable.get(lsn);
    if (useLock == null) {
  ...
  /**
   * Returns the LockType if the given locker owns a lock on the given node, or null if the lock is
   * not owned.
   */
  public abstract LockType getOwnedLockType(Long lsn, Locker locker);

  LockType getOwnedLockTypeInternal(final Long lsn, final Locker locker, final int lockTableIndex) {
    /* Get the target lock. */
    final Map<Long, Lock> lockTable = lockTables[lockTableIndex];
    final Lock useLock = lockTable.get(lsn);
    if (useLock == null) {
  ...
  boolean isLockUncontendedInternal(final Long lsn, final int lockTableIndex) {
    /* Get the target lock. */
    final Map<Long, Lock> lockTable = lockTables[lockTableIndex];
    final Lock useLock = lockTable.get(lsn);
    if (useLock == null) {
  ...
  boolean ownsOrSharesLockInternal(final Locker locker, final Long lsn, final int lockTableIndex) {
    final Map<Long, Lock> lockTable = lockTables[lockTableIndex];
    final Lock useLock = lockTable.get(lsn);
    if (useLock == null) {
      return false;
    ...
    for (final LockInfo info : getOwnersInternal(lsn, lockTableIndex)) {
      ...
      if (owner == locker || owner.sharesLocksWith(locker) || locker.sharesLocksWith(owner)) {
        return true;
      }
    }
    return false;
  ...
  abstract LockAttemptResult attemptLock(
      Long lsn,
      Locker locker,
      LockType type,
  ...
  LockAttemptResult attemptLockInternal(
     ...
     */
    final LockAttemptResult lar =
        useLock.lock(
            type, locker, nonBlockingRequest, jumpAheadOfWaiters, memoryBudget, lockTableIndex);
   ...
   *
   * <p>This method must be called while synchronized on the locker because it calls locker.wait and
   * finishLock.
   *
  ...
  private boolean performDeadlockDetectionDelay(
    ...
    if (!envImpl.getDeadlockDetection() || locker.getImportunate() || ddDelay == 0) {
    ...
    if (!waitForever) {
      ddDelay = Math.min(ddDelay, timeRemain(timeout, startTime));
    }
    ...
    for (int round = 0; ; round++) {
      ...
      if (dc.hasCycle()) {
        if (dc.hasTrueDeadlock()) {
          ...
          throw makeDeadlockException(dc, locker, timeout, true /*isVictim*/, database);
  ...
  private Pair<Boolean, DeadlockChecker> notifyVictim(
    ...
    while (true) {
      ...
      if (!waitForever && timeRemain(timeout, startTime) <= 0) {
        ...
        throw makeDeadlockException(lastDC, currentLocker, timeout, false /*isVictim*/, database);
      }
      ...
      final DeadlockChecker dc = new DeadlockChecker(currentLocker, lsn, type);
  ...
  private LockConflictException makeDeadlockException(
    ...
    final LockConflictException ex = new DeadlockException(locker, msg.toString());
  ...
  static class TimeoutInfo {
    ...
    TimeoutInfo(final String message, final Set<LockInfo> owners, final List<LockInfo> waiters) {
   ...
   *     held.
   */
  public boolean release(final long lsn, final Locker locker) throws DatabaseException {

    final Set<Locker> newOwners = releaseAndFindNotifyTargets(lsn, locker);
   ...
   *
   * @return null if the lock does not exist or the given locker was not the owner, a non-empty set
   *     if owners should be notified after releasing, an empty set if no notification is required.
   */
  abstract Set<Locker> releaseAndFindNotifyTargets(long lsn, Locker locker)
      throws DatabaseException;

  /** Do the real work of releaseAndFindNotifyTargets */
  Set<Locker> releaseAndFindNotifyTargetsInternal(
      final long lsn, final Locker locker, final int lockTableIndex) {
    final Map<Long, Lock> lockTable = lockTables[lockTableIndex];
    ...
    final Set<Locker> newOwners = lock.release(locker, memoryBudget, lockTableIndex);
    ...
    if ((lock.nWaiters() == 0) && (lock.nOwners() == 0)) {
      ...
      /**
       * In the deadlock detection process, in order to check that a cycle still exists, we need to
       * detect lock release.
       *
       * <p>During release, we will create a new lock object. During deadlock detection, we compare
       * the lock reference in the cycle with the current lock reference; if they are unequal, the
       * lock was released.
       */
      if (lock.isThin()) {
  ...
  /**
   * Demote a lock from write to read. Call back to the owning locker to move this to its read
   * collection.
   *
  ...
  void demoteInternal(final long lsn, final Locker locker, final int lockTableIndex) {
  ...
  /**
   * Test the status of the lock on LSN. If any transaction holds any lock on it, true is returned.
   * If no transaction holds a lock on it, false is returned.
   *
   * <p>This method is only used by unit tests.
   *
   * @param lsn The LSN to check.
   * @return true if any transaction holds any lock on the LSN. false if no lock is held by any
   *     transaction.
   */
  ...
  boolean isOwnerInternal(
      final Long lsn, final Locker locker, final LockType type, final int lockTableIndex) {
  ...
  boolean isWaiterInternal(final Long lsn, final Locker locker, final int lockTableIndex) {
  ...
  Locker getWriteOwnerLockerInternal(final Long lsn, final int lockTableIndex) {
  ...
  boolean validateOwnershipInternal(
    ...
    if (getOwnersAndWaiters) {
       ...
       */
      if (owners != null) {
        final Set<LockInfo> localOwners = getOwnersInternal(lsn, lockTableIndex);
      ...
      if (waiters != null) {
        final List<LockInfo> localWaiters = getWaitersInternal(lsn, lockTableIndex);
  ...
  public abstract LockAttemptResult stealLock(Long lsn, Locker locker, LockType lockType)
      throws DatabaseException;

  LockAttemptResult stealLockInternal(
      final Long lsn, final Locker locker, final LockType lockType, final int lockTableIndex)
      throws DatabaseException {

    final Lock entry = lockTables[lockTableIndex].get(lsn);
    assert entry != null
        : "Lock " + DbLsn.getNoFormatString(lsn) + " for txn " + locker.getId() + " does not exist";
  ...
  /** Statistics */
  LockStats lockStat(final StatsConfig config) throws DatabaseException {

    final StatGroup latchStats = new StatGroup("Locktable latches", "Shows lock table contention");
    ...
    final StatGroup tableStats =
        new StatGroup("Locktable", "The types of locks held in the lock table");
    ...
    return new LockStats(
        stats.cloneGroup(config.getClear()),
  ...
  void dumpLockTableInternal(final StatGroup tableStats, final int i, final boolean clear) {

    final StatGroup oneTable = new StatGroup("Single lock table", "Temporary stat group");
  ...
  private void dumpToStringNoLatch(final StringBuilder sb, final int whichTable) {
  ...
  private class DeadlockChecker {
    ...
    /** The owners and waiters for the root Lock which will be contained in DeadlockException. */
     ...
     */
    DeadlockChecker(final Locker locker, final Long lsn, final LockType locktype) {
    ...
    boolean hasCycleInternal(
       ...
       */
      final CycleNode cn =
          new CycleNode(checkedLocker, lsn, checkedLock, requestLocktype, ownLockType);
      ...
      for (final LockInfo info : ownersForCheckedLock) {
        ...
        if (locker != checkedLocker) {
          ...
          if (waitsFor != null && requestType != null && ownType != null) {

            if (hasCycleInternal(locker, waitsFor, requestType, ownType)) {
              return true;
            }
          }
        }
    ...
    class CycleNode {
      ...
      /** The lsn which represents the lock. It will not change if the lock is released. */
      private final Long lsn;

      /**
       * The Lock instance. Releasing a lock will re-create a new Lock object. By comparing it with
       * the lock gotten by lsn, we can determine whether the lock was released during the deadlock
       * detection process.
       */
       ...
       */
      private final LockType ownLockType;
  ...
  private static class WaitForLockResult {
    ...
    WaitForLockResult(
        final Locker targetVictim, final DeadlockChecker dc, final LockAttemptResult result) {
  ...
  /** The result of checking for deadlocks, and handling a deadlock if one is found. */
