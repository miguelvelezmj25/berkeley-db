...
/**
 * @hidden For internal use only. It serves to shelter methods that must be public to be used by
 *     other BDB JE packages but that are not part of the public API available to applications.
 */
public class DbInternal {

  public static OperationResult DEFAULT_RESULT = new OperationResult(0, false);
  ...
  public static void setPreempted(final Database db, final String dbName, final String msg) {
   ...
   *
   * <p>This method does not check whether the returned envImpl is valid.
   *
   * <p>WARNING: This method will be phased out over time and normally getNonNullEnvImpl should be
   * called instead.
   *
   * @return the non-null underlying EnvironmentImpl, or null if the env has been closed.
   */
   ...
   *
   * <p>This method is called to access the underlying EnvironmentImpl when an env is expected to be
   * open, to guard against NPE when the env has been closed.
   *
   * <p>This method does not check whether the env is valid.
   *
  ...
  public static SecondaryAssociation getSecondaryAssociation(final Database db) {
    return db.getSecondaryAssociation();
  }

  /** Proxy to SecondaryDatabase.getPrivateSecondaryConfig */
  public static SecondaryConfig getPrivateSecondaryConfig(final SecondaryDatabase secDb) {
  ...
  public static boolean advanceCursor(
      final Cursor cursor, final DatabaseEntry key, final DatabaseEntry data) {
  ...
  public static OperationResult deleteInternal(
      final Cursor cursor, final ReplicationContext repContext) {
  ...
  public enum Search {
    ...
    /** Match the smallest value greater than or equal to the key or data param. */
    GTE,
  ...
  public static OperationStatus search(
    ...
    final OperationResult result =
        search(cursor, key, pKey, data, searchMode, getReadOptions(lockMode));

    return result != null ? OperationStatus.SUCCESS : OperationStatus.NOTFOUND;
  }

  /**
   * Finds the key according to the Search param. If dups are configured, GT and GTE will land on
   * the first dup for the matching key, while LT and LTE will land on the last dup for the matching
   * key.
   *
   * <p>search() and searchBoth() in this class may eventually be exposed as public JE Cursor
   * methods, but this isn't practical now for the following reasons:
   *
   * <p>+ The API design needs more thought. Perhaps Search.EQ should be added. Perhaps existing
   * Cursor methods should be deprecated.
   *
   * <p>+ This implementation moves the cursor multiple times and does not release locks on the
   * intermediate records.
   *
   * <p>+ This could be implemented more efficiently using lower level cursor code. For example, an
   * LTE search would actually more efficient than the existing GTE search (getSearchKeyRange and
   * getSearchBothRange).
   *
   * <p>These methods are used by KVStore.
   */
  public static OperationResult search(
      final Cursor cursor,
      final DatabaseEntry key,
    ...
    final RangeCursor rangeCursor =
        new RangeCursor(range, null, dbImpl.getSortedDuplicates(), cursor);
  ...
  public static OperationStatus searchBoth(
    ...
    final OperationResult result =
        searchBoth(cursor, key, pKey, data, searchMode, getReadOptions(lockMode));

    return result != null ? OperationStatus.SUCCESS : OperationStatus.NOTFOUND;
  }

  /**
   * Searches with the dups for the given key and finds the dup matching the pKey value, according
   * to the Search param.
   *
   * <p>See search() for more discussion.
   */
  public static OperationResult searchBoth(
    ...
    final RangeCursor rangeCursor =
        new RangeCursor(range, pKeyRange, dbImpl.getSortedDuplicates(), cursor);
  ...
  public static Cursor makeCursor(
      final DatabaseImpl databaseImpl, final Locker locker, final CursorConfig cursorConfig) {
    return makeCursor(databaseImpl, locker, cursorConfig, true);
  }

  /**
   * Create a Cursor for internal use from a DatabaseImpl for btree corruption, which can set
   * retainNonTxnLocks.
   */
  public static Cursor makeCursor(
  ...
  public static Cursor makeCursor(
      final Database dbHandle, final Locker locker, final CursorConfig cursorConfig) {
  ...
  public static void setNonCloning(final Cursor cursor, final boolean nonSticky) {
  ...
  public static void setLoadPropertyFile(
      final EnvironmentConfig config, final boolean loadProperties) {
    config.setLoadPropertyFile(loadProperties);
  }

  /** Proxy to EnvironmentConfig.setCreateUP() */
  public static void setCreateUP(final EnvironmentConfig config, final boolean val) {
  ...
  public static void setCreateEP(final EnvironmentConfig config, final boolean val) {
  ...
  public static void setCheckpointUP(final EnvironmentConfig config, final boolean checkpointUP) {
  ...
  public static void setTxnReadCommitted(
      final EnvironmentConfig config, final boolean txnReadCommitted) {
  ...
  public static EnvironmentMutableConfig cloneMutableConfig(final EnvironmentMutableConfig config) {
    return config.cloneMutableConfig();
  }

  /** Proxy to EnvironmentMutableConfig.checkImmutablePropsForEquality() */
  public static void checkImmutablePropsForEquality(
      final EnvironmentMutableConfig config, final Properties handleConfigProps)
  ...
  public static void copyMutablePropsTo(
      final EnvironmentMutableConfig config, final EnvironmentMutableConfig toConfig) {
    config.copyMutablePropsTo(toConfig);
  }

  /** Proxy to EnvironmentMutableConfig.validateParams. */
  public static void disableParameterValidation(final EnvironmentMutableConfig config) {
  ...
  public static void setUseExistingConfig(
      final DatabaseConfig config, final boolean useExistingConfig) {
    config.setUseExistingConfig(useExistingConfig);
  }

  /** Proxy to DatabaseConfig.validate(DatabaseConfig() */
  public static void validate(final DatabaseConfig config1, final DatabaseConfig config2)
      throws DatabaseException {
  ...
  public static Locker getLocker(final Transaction txn) throws DatabaseException {

    return txn.getLocker();
  }

  /** Proxy to Transaction.getEnvironment() */
  public static Environment getEnvironment(final Transaction txn) throws DatabaseException {

    return txn.getEnvironment();
  }

  /** Proxy to Environment.getDefaultTxnConfig() */
  public static TransactionConfig getDefaultTxnConfig(final Environment env) {
    return env.getDefaultTxnConfig();
  }

  public static Transaction beginInternalTransaction(
      final Environment env, final TransactionConfig config) {
    return env.beginInternalTransaction(config);
  }

  public static ExceptionEvent makeExceptionEvent(final Exception e, final String n) {
  ...
  public static DiskOrderedCursorImpl getDiskOrderedCursorImpl(final DiskOrderedCursor cursor) {
  ...
  public static OperationResult makeResult(final long time) {
    ...
    return time == 0 ? DEFAULT_RESULT : new OperationResult(time, false /*update*/);
  }

  public static OperationResult makeResult(final int expiration, final boolean expirationInHours) {

    return makeResult(TTL.expirationToSystemTime(expiration, expirationInHours));
  }

  public static OperationResult makeUpdateResult(
      final int expiration, final boolean expirationInHours) {

    return new OperationResult(
        TTL.expirationToSystemTime(expiration, expirationInHours), true /*update*/);
  }
