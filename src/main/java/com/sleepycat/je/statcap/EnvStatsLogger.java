...
public class EnvStatsLogger implements EnvConfigObserver {
  ...
  private final StatLogger stlog;
  ...
  public EnvStatsLogger(EnvironmentImpl env) {
    ...
    try {
      stlog = new StatLogger(statdirf, STATFILENAME, STATFILEEXT, MAXFILECOUNT, MAXROWCOUNT);
    } catch (IOException e) {
  ...
  public void log() {
    ...
    for (String colname : EnvironmentParams.SUPPORTED_PARAMS.keySet()) {
