...
public class StatCapture extends DaemonThread implements EnvConfigObserver {
  ...
  private SortedMap<String, String> getStats() {
    final Collection<StatGroup> envStats =
        new ArrayList<StatGroup>(statMgr.loadStats(statsConfig, statKey).getStatGroups());

    if (envImpl.isReplicated()) {
      Collection<StatGroup> rsg = envImpl.getRepStatGroups(statsConfig, statKey);
      if (rsg != null) {
        envStats.addAll(rsg);
      }
    }
  ...
  public void envConfigUpdate(DbConfigManager configMgr, EnvironmentMutableConfig unused)
