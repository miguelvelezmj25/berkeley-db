...
public class VerifyConfig implements Cloneable {
  ...
  /**
   * Configures {@link Environment#verify Environment.verify} and {@link Database#verify
   * Database.verify} to propagate exceptions found during verification.
   *
   * <p>By default this is false and exception information is printed to System.out for notification
   * but does not stop the verification activity, which continues on for as long as possible.
   *
   * <p>Note: Currently this method has no effect.
   *
   * @param propagate If set to true, configure {@link Environment#verify Environment.verify} and
   *     {@link Database#verify Database.verify} to propagate exceptions found during verification.
   * @return this
   */
  ...
  /**
   * Returns true if the {@link Environment#verify Environment.verify} and {@link Database#verify
   * Database.verify} are configured to propagate exceptions found during verification.
   *
   * <p>This method may be called at any time during the life of the application.
   *
   * @return true if the {@link Environment#verify Environment.verify} and {@link Database#verify
   *     Database.verify} are configured to propagate exceptions found during verification.
   */
  ...
  /**
   * Configures {@link Environment#verify Environment.verify} and {@link Database#verify
   * Database.verify} to perform fine granularity consistency checking that includes verifying in
   * memory constructs.
   *
   * <p>This level of checking should only be performed while the database environment is quiescent.
   *
   * <p>By default this is false.
   *
   * <p>Note: Currently, enabling aggressive verification has no additional effect.
   *
   * @param aggressive If set to true, configure {@link Environment#verify Environment.verify} and
   *     {@link Database#verify Database.verify} to perform fine granularity consistency checking
   *     that includes verifying in memory constructs.
   * @return this
   */
  ...
  /**
   * Returns true if the {@link Environment#verify Environment.verify} and {@link Database#verify
   * Database.verify} are configured to perform fine granularity consistency checking that includes
   * verifying in memory constructs.
   *
   * <p>This method may be called at any time during the life of the application.
   *
   * @return true if the {@link Environment#verify Environment.verify} and {@link Database#verify
   *     Database.verify} are configured to perform fine granularity consistency checking that
   ...
   *
   * <p>Information is printed to the {@link #getShowProgressStream()} if it is non-null, and
   * otherwise to System.err.
   *
   * <p>By default this is false.
   *
   * @param printInfo If set to true, configure {@link Environment#verify Environment.verify} and
   *     {@link Database#verify Database.verify} to print basic verification information.
   * @return this
   */
  ...
  /**
   * Configures the verify operation to display progress to the PrintStream argument. The
   * accumulated statistics will be displayed every N records, where N is the value of
  ...
  /**
   * Returns the PrintStream on which the progress messages will be displayed during long running
   * verify operations.
   */
  ...
  /**
   * When the verify operation is configured to display progress the showProgressInterval is the
   * number of LNs between each progress report.
   *
   * @return this
   */
  ...
  /**
   * Configures verification to verify secondary database integrity. This is equivalent to verifying
   * secondaries in the background Btree verifier, when {@link EnvironmentConfig#VERIFY_SECONDARIES}
   * is set to true.
   *
  ...
  /**
   * Configures verification to read and verify the leaf node (LN) of a primary data record. This is
   * equivalent to verifying data records in the background Btree verifier, when {@link
   * EnvironmentConfig#VERIFY_DATA_RECORDS} is set to true.
   *
   * <p>By default this is false.
   *
   * @return this
  ...
  /**
   * @hidden Configures verification to verify the obsolete record metadata. This is equivalent to
   *     verifying obsolete metadata in the background Btree verifier, when {@link
  ...
  /** @hidden Returns the verifyObsoleteRecords value. */
  public boolean getVerifyObsoleteRecords() {
    return verifyObsoleteRecords;
  }

  /**
   * Configures the number of records verified per batch. In order to give database remove/truncate
   * the opportunity to execute, records are verified in batches and there is a {@link
   * #setBatchDelay delay} between batches.
   *
   * <p>By default the batch size is 1000.
   *
   * <p>Note that when using the {@link EnvironmentConfig#ENV_RUN_VERIFIER background data
   * verifier}, the batch size is {@link EnvironmentConfig#VERIFY_BTREE_BATCH_SIZE}.
   *
   * @return this
   */
  ...
  /**
   * Configures the delay between batches. In order to give database remove/truncate the opportunity
   * to execute, records are verified in {@link #setBatchSize batches} and there is a delay between
   * batches.
   *
   * <p>By default the batch delay is 10 ms.
   *
   * <p>Note that when using the {@link EnvironmentConfig#ENV_RUN_VERIFIER background data
   * verifier}, the batch delay is {@link EnvironmentConfig#VERIFY_BTREE_BATCH_DELAY}.
   *
   * @param delay the delay between batches.
   * @param unit the {@code TimeUnit} of the delay value. May be null only if delay is zero.
   * @return this
   */
