 ...
 * A database handle.
 *
 * <p>Database attributes are specified in the {@link com.sleepycat.je.DatabaseConfig
 * DatabaseConfig} class. Database handles are free-threaded and may be used concurrently by
 * multiple threads.
 *
 * <p>To open an existing database with default attributes:
 *
 * <blockquote>
 *
 * <pre>
 ...
 * </blockquote>
 *
 * <p>To create a transactional database that supports duplicates:
 *
 * <blockquote>
 *
 * <pre>
...
public class Database implements Closeable {
  ...
  void initExisting(
     ...
     */
    validateConfigAgainstExistingDb(locker, databaseName, dbConfig, dbImpl);
  ...
  SecondaryAssociation makeSecondaryAssociation() {
    ...
    return new SecondaryAssociation() {
      ...
      public Collection<SecondaryDatabase> getSecondaries(
          @SuppressWarnings("unused") DatabaseEntry primaryKey) {
  ...
  /**
   * Used to remove references to this database from other objects, when this database is closed. We
   * don't remove references from cursors or secondaries here, because it's an error to close a
   * database before its cursors and to close a primary before its secondaries.
   */
  ...
  private void validateConfigAgainstExistingDb(
    ...
    if (!config.getUseExistingConfig()) {
      ...
      validatePropertyMatches("temporary", dbImpl.isTemporary(), config.getTemporary());
      /* Only check replicated if the environment is replicated. */
      if (getEnv().isReplicated()) {
        validatePropertyMatches("replicated", dbImpl.isReplicated(), config.getReplicated());
      }
    }
    ...
    if (dbImpl.hasOpenHandles()) {
      if (!config.getUseExistingConfig()) {
        validatePropertyMatches(
            "transactional", dbImpl.isTransactional(), config.getTransactional());
        validatePropertyMatches(
            "deferredWrite", dbImpl.isDurableDeferredWrite(), config.getDeferredWrite());
      }
    } else {
    ...
    if (config.getOverrideBtreeComparator()) {
      dbImplModified |=
          dbImpl.setBtreeComparator(
              config.getBtreeComparator(), config.getBtreeComparatorByClassName());
    }

    if (config.getOverrideDuplicateComparator()) {
      dbImplModified |=
          dbImpl.setDuplicateComparator(
              config.getDuplicateComparator(), config.getDuplicateComparatorByClassName());
    }

    dbImplModified |=
        dbImpl.setTriggers(
            locker, databaseName, config.getTriggers(), config.getOverrideTriggers());
    ...
    if (newNodeMaxEntries != 0 && newNodeMaxEntries != dbImpl.getNodeMaxTreeEntries()) {
      dbImplModified = true;
   ...
   *
   * <p>Any BIN-deltas in cache must be mutated to full BINs, since BIN-deltas are not allowed in DW
   * mode. This can be expensive, and is only a workaround for the underlying problem that
   * BIN-deltas are not supported in DW mode. The mutation is necessary when a db transitions from
   * non-DW to DW mode. In that case there may be BIN-deltas in cache, even if the DB has not yet
   * been opened since BIN-deltas may be placed in the Btree by recovery. This method is not
   * optimized because ultimately BIN-deltas will be supported in DW mode or DW mode will be
   * discontinued.
   *
   * <p>At the time this method is called, the DW flag on this object has already been set.
   * Therefore, this workaround doesn't guarantee that internal operations on the db (e.g.,
   * compression or eviction) won't operation on a BIN-delta in a DW DB. Therefore, we cannot
   * assume/assert that a cached BIN-delta does not exist in a DW DB. Such assertions were removed
   * as part of this workaround.
   *
   * <p>Setting the DW flag before mutating the BINs is necessary to ensure that no new BIN-deltas
   * appear in cache during this process. BINs loaded into cache for a DW db are mutated to full
   * BINs in IN.postFetchInit.
   *
  ...
  private void mutateDeferredWriteBINDeltas(DatabaseImpl dbImpl) {
    ...
    for (final IN in : getEnv().getInMemoryINs()) {
      ...
      try {
        ...
        if (ohCache == null || in.getNormalizedLevel() != 2) {
          continue;
        }
  ...
  /** @throws IllegalArgumentException via Environment.openDatabase and openSecondaryDatabase. */
  private void validatePropertyMatches(
      final String propName, final boolean existingValue, final boolean newValue) {
    if (newValue != existingValue) {
      throw new IllegalArgumentException(
          "You can't open a Database with a "
              + propName
              + " configuration of "
              + newValue
   ...
   *
   * <p>When closing the last open handle for a deferred-write database, any cached database
   * information is flushed to disk as if {@link #sync} were called.
   *
   * <p>The database handle should not be closed while any other handle that refers to it is not yet
   * closed; for example, database handles should not be closed while cursor handles into the
   * database remain open, or transactions that include operations on the database have not yet been
   * committed or aborted. Specifically, this includes {@link Cursor Cursor} and {@link
   * com.sleepycat.je.Transaction Transaction} handles.
   *
   * <p>When multiple threads are using the {@link Database Database} handle concurrently, only a
   * single thread may call this method.
   *
   * <p>When called on a database that is the primary database for a secondary index, the primary
   * database should be closed only after all secondary indices which reference it have been closed.
   *
   * <p>The database handle may not be accessed again after this method is called, regardless of the
   ...
   *     occurs.
   * @throws IllegalStateException if cursors associated with this database are still open.
   */
  public void close() {
    try {
      closeInternal(
          true /*doSyncDw*/, true /*deleteTempDb*/, DbState.CLOSED, null /*preemptedException*/);
    } catch (Error E) {
  ...
  private void closeNoSync() {
    try {
      closeInternal(
          false /*doSyncDw*/, true /*deleteTempDb*/, DbState.CLOSED, null /*preemptedException*/);
    } catch (Error E) {
      envHandle.invalidate(E);
      throw E;
    }
  ...
  /**
   * Marks the handle as preempted when the handle lock is stolen by the HA replayer, during replay
   * of a naming operation (remove, truncate or rename). This causes DatabasePreemptedException to
   * be thrown on all subsequent use of the handle or cursors opened on this handle. [#17015]
   */
  synchronized void setPreempted(final String dbName, final String msg) {
    ...
    final OperationFailureException preemptedException =
        dbImpl.getEnv().createDatabasePreemptedException(msg, dbName, this);

    closeInternal(
        false /*doSyncDw*/, false /*deleteTempDb*/, DbState.PREEMPTED, preemptedException);
  }
   ...
   *
   * <p>Note that this method (unlike close) does not perform sync and removal of DW DBs. A DW DB
   * cannot be transactional.
   */
  synchronized void invalidate() {
    closeInternal(
        false /*doSyncDw*/, false /*deleteTempDb*/, DbState.INVALID, null /*preemptedException*/);
  }
  ...
  private void closeInternal(
    ...
    try {
      ...
      try {
        closeInternalWork(doSyncDw, deleteTempDb, newState, preemptedException);
      } finally {
  ...
  /**
   * Nulls-out indirect references to the environment, to allow GC. It also sets the state to the
   * given non-open state, if the state is currently open. We must set the state to non-open before
   * setting references to null.
   *
   * <p>The app may hold the Database references longer than expected. In particular during an
   * Environment re-open we need to give GC a fighting chance while handles from two environments
   * are temporarily referenced.
   *
   * <p>Note that this is needed even when the db or env is invalid.
   */
  synchronized void minimalClose(
      final DbState newState, final OperationFailureException preemptedException) {
  ...
  private void closeInternalWork(
    ...
    synchronized (this) {
      ...
      if (state == DbState.CLOSED
          || state == DbState.INVALID
          || state == DbState.PREEMPTED
          || state == null) {
        return;
      }
      ...
      if (newState == DbState.CLOSED) {
        if (openCursors.get() != 0) {
          handleRefErrors.append(" ").append(openCursors.get()).append(" open cursors.");
        }
        if (simpleAssocSecondaries != null && simpleAssocSecondaries.size() > 0) {
          handleRefErrors
              .append(" ")
              .append(simpleAssocSecondaries.size())
              .append(" associated SecondaryDatabases.");
        }
        if (foreignKeySecondaries != null && foreignKeySecondaries.size() > 0) {
          handleRefErrors
              .append(" ")
              .append(foreignKeySecondaries.size())
              .append(" associated foreign key SecondaryDatabases.");
        }
      }
    ...
    if (handleRefErrors.length() > 0) {
      throw new IllegalStateException(
          "Database closed while still referenced by other handles." + handleRefErrors);
    }
  ...
  /**
   * Flushes any cached information for this database to disk; only applicable for deferred-write
   * databases.
   *
   * <p>Note that deferred-write databases are automatically flushed to disk when the {@link #close}
   * method is called.
   *
   * @see DatabaseConfig#setDeferredWrite DatabaseConfig.setDeferredWrite
   ...
   * @throws EnvironmentFailureException if an unexpected, internal or environment-wide failure
   *     occurs.
   * @throws UnsupportedOperationException if this is not a deferred-write database, or this
   ...
   * @throws SequenceExistsException if the sequence record already exists and the {@code
   *     SequenceConfig ExclusiveCreate} parameter is true.
   * @throws SequenceNotFoundException if the sequence record does not exist and the {@code
   *     SequenceConfig AllowCreate} parameter is false.
   * @throws OperationFailureException if one of the <a
   *     href="../je/OperationFailureException.html#readFailures">Read Operation Failures</a>
   *     occurs. If the sequence does not exist and the {@link SequenceConfig#setAllowCreate
   ...
   * @throws EnvironmentFailureException if an unexpected, internal or environment-wide failure
   *     occurs.
   * @throws UnsupportedOperationException if this database is read-only, or this database is
   *     configured for duplicates.
   * @throws IllegalStateException if the Sequence record is deleted by another thread during this
   *     method invocation, or the database has been closed.
   * @throws IllegalArgumentException if an invalid parameter is specified, for example, an invalid
   *     {@code SequenceConfig} parameter.
   */
  public Sequence openSequence(
      final Transaction txn, final DatabaseEntry key, final SequenceConfig config) {
    try {
      checkEnv();
  ...
  /**
   * Removes the sequence from the database. This method should not be called if there are open
   * handles on this sequence.
   *
   * @param txn For a transactional database, an explicit transaction may be specified, or null may
   *     be specified to use auto-commit. For a non-transactional database, null must be specified.
   * @param key The key {@link com.sleepycat.je.DatabaseEntry DatabaseEntry} of the sequence.
   * @throws OperationFailureException if one of the <a
   ...
   *
   * @param txn the transaction used to protect all operations performed with the cursor, or null if
   *     the operations should not be transaction protected. If the database is non-transactional,
   *     null must be specified. For a transactional database, the transaction is optional for
   *     read-only access and required for read-write access.
   * @param cursorConfig The cursor attributes. If null, default attributes are used.
  ...
  /**
   * Create a DiskOrderedCursor to iterate over the records in 'this' Database. Because the
  ...
  public DiskOrderedCursor openCursor(DiskOrderedCursorConfig cursorConfig) {
    try {
      ...
      trace(Level.FINEST, "Database.openForwardCursor", null, cursorConfig);
   ...
   * @deprecated in favor of {@link #populateSecondaries(Transaction, DatabaseEntry, DatabaseEntry,
   *     long, CacheMode)}.
   */
  public void populateSecondaries(
  ...
  /**
   * @hidden For internal use only.
   *     <p>Given the {@code key}, {@code data} and {@code expirationTime} for a locked primary DB
   *     record, update the corresponding secondary database (index) records, for secondaries
   *     enabled for incremental population.
   *     <p>The secondaries associated the primary record are determined by calling {@link
   *     SecondaryAssociation#getSecondaries}. For each of these secondaries, {@link
   *     SecondaryDatabase#isIncrementalPopulationEnabled} is called to determine whether
   *     incremental population is enabled. If so, appropriate secondary records are inserted and
   *     deleted so that the index accurately reflects the current state of the primary record.
   *     <p>Note that for a given primary record, this method will not modify the secondary database
   *     if the secondary has already been updated for the primary record, due to concurrent primary
   *     write operations. Due to this behavior, certain integrity checks are not performed as
   *     documented in {@link SecondaryDatabase#startIncrementalPopulation}.
   *     <p>The primary record must be locked (read or write locked) when this method is called.
   *     Therefore, the caller should not use dirty-read to read the primary record. The simplest
   *     way to ensure that the primary record is locked is to use a cursor to read primary records,
   *     and call this method while the cursor is still positioned on the primary record.
   *     <p>It is the caller's responsibility to pass all primary records to this method that
   *     contain index keys for a secondary DB being incrementally populated, before calling {@link
   *     SecondaryDatabase#endIncrementalPopulation} on that secondary DB.
   * @param txn is the transaction to be used to write secondary records. If null and the secondary
   *     DBs are transactional, auto-commit will be used.
   * @param key is the key of the locked primary record.
   * @param data is the data of the locked primary record.
   * @param expirationTime the expiration time of the locked primary record. This can be obtained
   *     from {@link OperationResult#getExpirationTime()} when reading the primary record.
  ...
  /**
   * Removes records with a given key from the database. In the presence of duplicate keys, all
   * records associated with the given key will be removed. When the database has associated
   * secondary databases, this method also deletes the associated index records.
   *
   * @param txn For a transactional database, an explicit transaction may be specified, or null may
   *     be specified to use auto-commit. For a non-transactional database, null must be specified.
   ...
   * @return the OperationResult if the record is deleted, else null if the given key was not found
   *     in the database.
   * @throws OperationFailureException if one of the <a
   *     href="../je/OperationFailureException.html#writeFailures">Write Operation Failures</a>
   *     occurs.
   ...
   *     null input key parameter, an input key parameter with a null data array, or a partial key
   *     input parameter.
   * @since 7.0
   */
   ...
   * secondaries and enforce foreign key constraints. Deletes all duplicates.
   */
  OperationResult deleteInternal(
      final Locker locker, final DatabaseEntry key, final CacheMode cacheMode) {
    ...
    try (final Cursor cursor = new Cursor(this, locker, null)) {
      ...
      OperationResult searchResult =
          cursor.search(key, noData, lockMode, cacheMode, SearchMode.SET, false);
      ...
      while (searchResult != null) {
        ...
        final OperationResult deleteResult =
            cursor.deleteInternal(dbImpl.getRepContext(), cacheMode);
   ...
   * secondary databases, this method also deletes the associated index records.
   *
   * <p>Calling this method is equivalent to calling {@link #delete(Transaction, DatabaseEntry,
   * WriteOptions)}.
   *
   * @param txn For a transactional database, an explicit transaction may be specified, or null may
   *     be specified to use auto-commit. For a non-transactional database, null must be specified.
   * @param key the key used as <a href="DatabaseEntry.html#inParam">input</a>.
   * @return The method will return {@link com.sleepycat.je.OperationStatus#NOTFOUND
   *     OperationStatus.NOTFOUND} if the given key is not found in the database; otherwise {@link
   ...
   * ReadOptions#getLockMode() lock mode} specified, the key and/or data will be returned via the
   * (non-null) DatabaseEntry parameters, and a non-null OperationResult will be returned. If the
   * operation fails because the record requested is not found, null is returned.
   *
   * <p>The following table lists each allowed operation and whether the key and data parameters are
   * <a href="DatabaseEntry.html#params">input or output parameters</a>. See the individual {@link
   * Get} operations for more information. <div>
   *
   ...
   *     array, and a partial key/data input parameter.
   * @since 7.0
   */
  public OperationResult get(
    ...
    try {
      ...
      trace(Level.FINEST, "Database.get", String.valueOf(getType), txn, key, null, lockMode);
   ...
   *
   * <p>Calling this method is equivalent to calling {@link #get(Transaction, DatabaseEntry,
   * DatabaseEntry, Get, ReadOptions)} with {@link Get#SEARCH}.
   *
   * @param txn For a transactional database, an explicit transaction may be specified to
   *     transaction-protect the operation, or null may be specified to perform the operation
   *     without transaction protection. For a non-transactional database, null must be specified.
  ...
  public OperationStatus get(
      final Transaction txn, final DatabaseEntry key, final DatabaseEntry data, LockMode lockMode) {
    final OperationResult result =
        get(txn, key, data, Get.SEARCH, DbInternal.getReadOptions(lockMode));

    return result == null ? OperationStatus.NOTFOUND : OperationStatus.SUCCESS;
  }

  /**
   * Retrieves the key/data pair with the given key and data value, that is, both the key and data
   * items must match.
   *
   * <p>Calling this method is equivalent to calling {@link #get(Transaction, DatabaseEntry,
   * DatabaseEntry, Get, ReadOptions)} with {@link Get#SEARCH_BOTH}.
   *
   * @param txn For a transactional database, an explicit transaction may be specified to
   *     transaction-protect the operation, or null may be specified to perform the operation
   *     without transaction protection. For a non-transactional database, null must be specified.
  ...
  public OperationStatus getSearchBoth(
      final Transaction txn, final DatabaseEntry key, final DatabaseEntry data, LockMode lockMode) {
    final OperationResult result =
        get(txn, key, data, Get.SEARCH_BOTH, DbInternal.getReadOptions(lockMode));
  ...
  /**
   * Creates a specialized join cursor for use in performing equality or natural joins on secondary
   * indices.
   *
   * <p>Each cursor in the <code>cursors</code> array must have been initialized to refer to the key
   * on which the underlying database should be joined. Typically, this initialization is done by
   * calling {@link Cursor#getSearchKey Cursor.getSearchKey}.
   *
   * <p>Once the cursors have been passed to this method, they should not be accessed or modified
   * until the newly created join cursor has been closed, or else inconsistent results may be
   * returned. However, the position of the cursors will not be changed by this method or by the
  ...
  /**
   * Preloads the cache. This method should only be called when there are no operations being
   * performed on the database in other threads. Executing preload during concurrent updates may
   * result in some or all of the tree being loaded into the JE cache. Executing preload during any
   * other types of operations may result in JE exceeding its allocated cache size. preload()
   * effectively locks the entire database and therefore will lock out the checkpointer, cleaner,
   * and compressor, as well as not allow eviction to occur.
   *
   * @deprecated As of JE 2.0.83, replaced by {@link Database#preload(PreloadConfig)}.
   * @param maxBytes The maximum number of bytes to load. If maxBytes is 0, je.evictor.maxMemory is
   *     used.
   * @throws OperationFailureException if one of the <a
   *     href="OperationFailureException.html#readFailures">Read Operation Failures</a> occurs.
   * @throws EnvironmentFailureException if an unexpected, internal or environment-wide failure
   ...
   * and compressor, as well as not allow eviction to occur.
   *
   * @deprecated As of JE 2.0.101, replaced by {@link Database#preload(PreloadConfig)}.
   * @param maxBytes The maximum number of bytes to load. If maxBytes is 0, je.evictor.maxMemory is
   *     used.
   * @param maxMillisecs The maximum time in milliseconds to use when preloading. Preloading stops
   *     once this limit has been reached. If maxMillisecs is 0, preloading can go on indefinitely
   *     or until maxBytes (if non-0) is reached.
   * @throws OperationFailureException if one of the <a
   *     href="OperationFailureException.html#readFailures">Read Operation Failures</a> occurs.
   * @throws EnvironmentFailureException if an unexpected, internal or environment-wide failure
   *     occurs.
   ...
   * other types of operations may result in JE exceeding its allocated cache size. preload()
   * effectively locks the entire database and therefore will lock out the checkpointer, cleaner,
   * and compressor, as well as not allow eviction to occur. If the database is replicated and the
   * environment is in the replica state, then the replica may become temporarily disconnected from
   * the master if the replica needs to replay changes against the database and is locked out
   * because the time taken by the preload operation exceeds {@link
   * com.sleepycat.je.rep.ReplicationConfig#FEEDER_TIMEOUT}.
   *
   * <p>While this method preloads a single database, {@link Environment#preload} lets you preload
   * multiple databases.
   *
   ...
   *     occurs.
   * @throws IllegalStateException if the database has been closed.
   * @throws IllegalArgumentException if {@code PreloadConfig.getMaxBytes} is greater than size of
   *     the JE cache.
   */
  ...
  /**
   * Counts the key/data pairs in the database. This operation is faster than obtaining a count from
   * a cursor based scan of the database, and will not perturb the current contents of the cache.
   * However, the count is not guaranteed to be accurate if there are concurrent updates. Note that
   * this method does scan a significant portion of the database and should be considered a fairly
   * expensive operation.
   *
   * <p>This operation uses the an internal infrastructure and algorithm that is similar to the one
   * used for the {@link DiskOrderedCursor}. Specifically, it will disable deletion of log files by
   * the JE log cleaner during its execution and will consume a certain amount of memory (but
   * without affecting the memory that is available for the JE cache). To avoid excessive memory
   * consumption (and a potential {@code OutOfMemoryError}) this method places an internal limit on
   * its memory consumption. If this limit is reached, the method will still work properly, but its
   * performance will degrade. To specify a different memory limit than the one used by this method,
   * use the {@link Database#count(long memoryLimit)} method.
   *
   * <p>Currently, the internal memory limit is calculated as 10% of the difference between the max
   * JVM memory (the value returned by Runtime.getRuntime().maxMemory()) and the configured JE cache
   * size.
   *
   ...
   *     occurs.
   * @throws IllegalStateException if the database has been closed.
   * @see <a href="EnvironmentStats.html#cacheUnexpectedSizes">Cache Statistics: Unexpected
   *     Sizes</a>
   */
  public long count() {
    checkEnv();
   ...
   * the JE log cleaner during its execution and will consume a certain amount of memory (but
   * without affecting the memory that is available for the JE cache). To avoid excessive memory
   * consumption (and a potential {@code OutOfMemoryError}) this method takes as input an upper
   * bound on the memory it may consume. If this limit is reached, the method will still work
   * properly, but its performance will degrade.
   *
   * @param memoryLimit The maximum memory (in bytes) that may be consumed by this method.
   * @return The count of key/data pairs in the database.
   * @throws OperationFailureException if one of the <a
   *     href="OperationFailureException.html#readFailures">Read Operation Failures</a> occurs.
   * @throws EnvironmentFailureException if an unexpected, internal or environment-wide failure
   ...
   *
   * <p>If this method has not been configured to avoid expensive operations (using the {@link
   * com.sleepycat.je.StatsConfig#setFast StatsConfig.setFast} method), it will access some of or
   * all the pages in the database, incurring a severe performance penalty as well as possibly
   * flushing the underlying cache.
   *
   * <p>In the presence of multiple threads or processes accessing an active database, the
   ...
   * and debugging.
   *
   * @param config Configures the verify operation; if null, the default operation is performed.
   * @return Database statistics.
   * @throws OperationFailureException if one of the <a
   *     href="OperationFailureException.html#readFailures">Read Operation Failures</a> occurs.
   ...
   *
   * <p>This may differ from the configuration used to open this object if the database existed
   * previously.
   *
   * <p>Unlike most Database methods, this method may be called after the database is closed.
   *
  ...
  /**
   * Returns the {@link com.sleepycat.je.Environment Environment} handle for the database
   * environment underlying the {@link Database Database}.
   *
   * <p>This method may be called at any time during the life of the application.
   *
   * @return The {@link com.sleepycat.je.Environment Environment} handle for the database
   *     environment underlying the {@link Database Database}.
   */
  ...
  /**
   * Returns a list of all {@link com.sleepycat.je.SecondaryDatabase SecondaryDatabase} objects
   * associated with a primary database.
   *
   * <p>If no secondaries are associated with this database, an empty list is returned.
   */
  /*
  ...
  /**
   * Compares two keys using either the default comparator if no BTree comparator has been set or
   * the BTree comparator if one has been set.
   *
   * @return -1 if entry1 compares less than entry2, 0 if entry1 compares equal to entry2, 1 if
   *     entry1 compares greater than entry2
   * @throws IllegalStateException if the database has been closed.
   * @throws IllegalArgumentException if either entry is a partial DatabaseEntry, or is null.
   */
  ...
  /**
   * Compares two data elements using either the default comparator if no duplicate comparator has
   * been set or the duplicate comparator if one has been set.
   *
   * @return -1 if entry1 compares less than entry2, 0 if entry1 compares equal to entry2, 1 if
   *     entry1 compares greater than entry2
   * @throws IllegalStateException if the database has been closed.
   ...
   *
   * <p>This method should always be called to access the databaseImpl, to guard against NPE when
   * the database has been closed after the initial checks.
   *
   * <p>However, callers should additionally call checkOpen at API entry points to reject the
   * operation as soon as possible. Plus, if the database has been closed, this method may return
   * non-null because the databaseImpl field is not volatile.
   *
   * @throws IllegalStateException if the database has been closed since checkOpen was last called.
   */
  ...
  @SuppressWarnings("unused")
  void removeCursor(final ForwardCursor ignore) throws DatabaseException {
  ...
  void checkLockModeWithoutTxn(final Transaction userTxn, final LockMode lockMode) {
    if (userTxn == null && LockMode.RMW.equals(lockMode)) {
      throw new IllegalArgumentException(
          lockMode
              + " is meaningless and can not be specified "
  ...
  void trace(
    ...
    if (logger.isLoggable(level)) {
      ...
      LoggerUtils.logMsg(logger, getEnv(), level, sb.toString());
    }
  }
   ...
   */
  void trace(
      final Level level, final String methodName, final Transaction txn, final Object config) {
   ...
   *
   * <p>This method is in the Database class, rather than in SecondaryDatabase, to support joins
