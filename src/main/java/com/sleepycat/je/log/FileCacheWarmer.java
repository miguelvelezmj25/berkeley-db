...
/**
 * Warm-up the file system cache during startup, for some portion of the log that is not being read
 * by recovery.
 *
 * <p>See EnvironmentConfig.LOG_FILE_WARM_UP_SIZE (until we publish it, this is in
 * EnvironmentParams).
 */
public class FileCacheWarmer extends Thread {
  ...
  /**
   * Stops this thread. At most one read will occur after calling this method, and then the thread
   * will exit.
   */
  void shutdown() {
    stop = true;
  ...
  public void run() {
    try {
      ...
      LoggerUtils.traceAndLogException(
          envImpl,
          FileCacheWarmer.class.getName(),
          "run",
          "Unable to warm file system cache due to exception",
          e);
  ...
  private void doRun() throws Throwable {
    ...
    long remaining =
        (warmUpSize * ONE_MB) - DbLsn.getTrueDistance(recoveryStartLsn, endOfLogLsn, fm);
