...
/**
 * The LogManager supports reading and writing to the JE log. The writing of data to the log is
 * serialized via the logWriteMutex. Typically space is allocated under the LWL. The client computes
 * the checksum and copies the data into the log buffer (not holding the LWL).
 */
public class LogManager {
  ...
  /**
   * Called at the end of recovery to begin expiration tracking using the given tracker. During
   * recovery we are single threaded, so we can set the field without taking the LWL.
   */
  ...
  /**
   * Reset the pool when the cache is resized. This method is called after the memory budget has
   * been calculated.
   */
   ...
   */
  public long logForceFlush(LogEntry entry, boolean fsyncRequired, ReplicationContext repContext)
  ...
  /**
   * Log an item, first logging any items on the lazyLogQueue, and finally flushing and sync'ing (if
   * requested).
   */
  public LogItem log(LogParams params) throws DatabaseException {
    ...
    try {
      ...
      for (LazyQueueEntry lqe = lazyLogQueue.poll(); lqe != null; lqe = lazyLogQueue.poll()) {
      ...
      if (params.repContext.inReplicationStream()) {
        ...
        assert (item.header.getVLSN() != null)
            : "Unexpected null vlsn: " + item.header + " " + params.repContext;
    ...
    if (params.backgroundIO) {
      envImpl.updateBackgroundWrites(item.size, logBufferPool.getLogBufferSize());
    }
  ...
  private void logItem(final LogItem item, final LogParams params)
    ...
    if (params.expirationTrackerCompleted != null) {
      envImpl.getExpirationProfile().addCompletedTracker(params.expirationTrackerCompleted);
    }
  }

  /**
   * This method handles exceptions to be certain that the Environment is invalidated when any
   * exception occurs in the critical write path, and it checks for an invalid environment to be
   * sure that no subsequent write is allowed. [#21929]
   *
   * <p>Invalidation is necessary because a logging operation does not ensure that the internal
   * state -- correspondence of LSN pointer, log buffer position and file position, and the
   * integrity of the VLSN index [#20919] -- is maintained correctly when an exception occurs.
   * Allowing a subsequent write can cause log corruption.
   */
  private LogWriteInfo serialLog(
      final LogItem item, final LogParams params, final UtilizationTracker tracker)
      throws IOException {
  ...
  /**
   * This method is used as part of writing data to the log. Called under the LogWriteLatch. Data is
   * either written to the LogBuffer or allocates space in the LogBuffer. The LogWriteInfo object is
   * used to save information about the space allocate in the LogBuffer. The caller uses the object
   * to copy data into the underlying LogBuffer. A null value returned indicates that the item was
   * written to the log. This occurs when the data item is too big to fit into an empty LogBuffer.
   *
   * @param params log params.
   * @param tracker utilization.
   * @return a LogWriteInfo object used to access allocated LogBuffer space. If null, the data was
   *     written to the log.
   * @throws IOException
   */
  private LogWriteInfo serialLogWork(
    ...
    */
     ...
     * getSizeForWrite.
     */
    tracker.countNewLogEntry(currentLsn, entryType, entrySize, params.nodeDb);
     ...
     */
    final LogBuffer lastLogBuffer = logBufferPool.getWriteBuffer(entrySize, flippedFile);
     ...
     *    flippedFile is true.
     */
    final long prevOffset = fileManager.advanceLsn(currentLsn, entrySize, flippedFile);
    ...
    try {
      ...
      if (useBuffer != null) {
        ...
        final boolean flushWriteQueue = params.flushRequired && !params.fsyncRequired;

        fileManager.writeLogBuffer(new LogBuffer(item.buffer, currentLsn), flushWriteQueue);
  ...
  ByteBuffer putIntoBuffer(LogEntry entry, long prevLogEntryOffset) {
    LogEntryHeader header =
        new LogEntryHeader(entry, Provisional.NO, ReplicationContext.NO_REPLICATE);
  ...
  public LogEntry getLogEntry(long lsn) throws FileNotFoundException {
    ...
    return getLogEntry(lsn, 0, false /*invisibleReadAllowed*/).getEntry();
  }
  ...
  /**
   * Instantiate all the objects in the log entry at this LSN. Allow the fetch of invisible log
   * entries if we are in recovery.
   */
   ...
   * @param invisibleReadAllowed true if it's expected that the target log entry might be invisible.
   *     Correct the known-to-be-bad checksum before proceeding.
   * @return log entry that embodies all the objects in the log entry.
   */
  private WholeEntry getLogEntry(long lsn, int lastLoggedSize, boolean invisibleReadAllowed)
      throws FileNotFoundException {
    ...
    try {
      ...
      try {
        ...
        long fileLength = -1;
        try {
          fileHandle = fileManager.getFileHandle(DbLsn.getFileNumber(lsn));
        ...
        if (fileHandle == null || fileLength <= DbLsn.getFileOffset(logBuffer.getFirstLsn())) {
          ...
          throw EnvironmentFailureException.unexpectedException(
              envImpl, "Corruption detected in log buffer, " + "but was not written to disk.", ce);
        }
         ...
         */
        final FileSource fileSource = new FileHandleSource(fileHandle, readBufferSize, fileManager);
        try {
          return getLogEntryFromLogSource(lsn, lastLoggedSize, fileSource, invisibleReadAllowed);
        } finally {
          fileSource.release();
        }
      }
  ...
  /**
   * Throws ChecksumException rather than translating it to EnvironmentFailureException and
   * invalidating the environment. Used instead of getLogEntry when a ChecksumException is handled
   * specially.
   */
  LogEntry getLogEntryAllowChecksumException(long lsn)
    ...
    try {
      return getLogEntryFromLogSource(lsn, 0, logSource, false /*invisibleReadAllowed*/).getEntry();
    } finally {
      logSource.release();
    }
  }

  LogEntry getLogEntryAllowChecksumException(long lsn, RandomAccessFile file, int logVersion)
      throws ChecksumException, DatabaseException {

    final LogSource logSource =
        new FileSource(file, readBufferSize, fileManager, DbLsn.getFileNumber(lsn), logVersion);
  ...
  /**
   * Gets log entry from the given source; the caller is responsible for calling logSource.release
   * and handling ChecksumException.
   *
   * <p>Is non-private for unit testing.
   *
   * @param lsn location of entry in log
   * @param lastLoggedSize is the entry size if known, or zero if unknown.
   * @param invisibleReadAllowed if true, we will permit the read of invisible log entries, and we
   *     will adjust the invisible bit so that the checksum will validate
   * @return log entry that embodies all the objects in the log entry
   */
  WholeEntry getLogEntryFromLogSource(
      long lsn, int lastLoggedSize, LogSource logSource, boolean invisibleReadAllowed)
      throws ChecksumException, DatabaseException {
    ...
    if (entryBuffer.remaining() < LogEntryHeader.MIN_HEADER_SIZE) {
      throw new ChecksumException(
          "Incomplete log entry header in "
              + logSource
              + " needed="
    ...
    if (entryBuffer.remaining() < itemSize) {
      entryBuffer = logSource.getBytes(fileOffset + header.getSize(), itemSize);
      if (entryBuffer.remaining() < itemSize) {
        throw new ChecksumException(
            "Incomplete log entry item in "
                + logSource
                + " needed="
                + itemSize
                + " remaining="
    ...
    if (header.isInvisible() && !invisibleReadAllowed) {
      throw new EnvironmentFailureException(
          ...
          "Read invisible log entry at " + DbLsn.getNoFormatString(lsn) + " " + header);
    }
    ...
    if (logBuffer == null) {
      try {
        ...
        return new FileHandleSource(
            fileManager.getFileHandle(fileNum), readBufferSize, fileManager);
  ...
  /**
   * Reads a log entry using a FileSource, and returns null (rather than throwing a
   * ChecksumException) if the entry exists in a log buffer that was not flushed.
   *
   * <p>Used to check whether an in-memory corruption is persistent.
   *
  ...
  public WholeEntry getLogEntryDirectFromFile(long lsn) throws ChecksumException {
    ...
    if (logSource instanceof LogBuffer) {
       ...
       */
      fileSource = new FileHandleSource(fileHandle, readBufferSize, fileManager);
    } else {
      fileSource = (FileSource) logSource;
    }

    try {
      return getLogEntryFromLogSource(lsn, 0, fileSource, false /*invisibleReadAllowed*/);
    } finally {
      fileSource.release();
    }
  }
  ...
  /**
   * Flush log buffers, but do not flush the write queue. This is used only by FsyncManager, just
   * prior to an fsync. When FsyncManager performs the fsync, the write queue will be flushed by
   * FileManager.fsyncLogEnd.
   */
   ...
   *
   * <p>Flushing logically means flushing all write buffers to the file system, so flushWriteQueue
   * should be false only when this method is called just before an fsync (FileManager.syncLogEnd
   * will flush the write queue).
   */
  ...
  /**
   * Return the current number of cache misses in a lightweight fashion, without incurring the cost
   * of loading all the stats, and without clearing any stats.
   */
      ...
      if (params.obsoleteWriteLockInfo != null) {
        for (WriteLockInfo info : params.obsoleteWriteLockInfo) {
          tracker.countObsoleteNode(
              info.getAbortLsn(), null /*type*/, info.getAbortLogSize(), info.getDb());
        }
      }
    }
  }
  ...
  /**
   * Count node as obsolete under the log write latch. This is done here because the log write latch
   * is managed here, and all utilization counting must be performed under the log write latch.
   */
  public void countObsoleteNode(
      long lsn, LogEntryType type, int size, DatabaseImpl nodeDb, boolean countExact) {
    synchronized (logWriteMutex) {
  ...
  /**
   * A flavor of countObsoleteNode which does not fire an assert if the offset has already been
   * counted. Called through the LogManager so that this incidence of all utilization counting can
   * be performed under the log write latch.
   */
  public void countObsoleteNodeDupsAllowed(
      long lsn, LogEntryType type, int size, DatabaseImpl nodeDb) {
    synchronized (logWriteMutex) {
      UtilizationTracker tracker = envImpl.getUtilizationTracker();
  ...
  public void countObsoleteDb(DatabaseImpl db) {
    synchronized (logWriteMutex) {
      db.countObsoleteDb(envImpl.getUtilizationTracker(), DbLsn.NULL_LSN /*mapLnLsn*/);
    }
  }
  ...
  private class LogWriteInfo {
    ...
    LogWriteInfo(final LogBufferSegment bs, final VLSN vlsn, final long fileOffset) {
      lbs = bs;
      this.vlsn = vlsn;
