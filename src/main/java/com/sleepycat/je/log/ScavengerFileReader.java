...
/**
 * A ScavengerFileReader reads the log backwards. If it encounters a checksum error, it goes to the
 * start of that log file and reads forward until it encounters a checksum error. It then continues
 * the reading backwards in the log.
 *
 * <p>The caller may set "dumpCorruptedBounds" to true if information about the start and finish of
 * the corrupted portion should be displayed on stderr.
 *
 * <p>The caller is expected to implement processEntryCallback. This method is called once for each
...
public abstract class ScavengerFileReader extends FileReader {
  ...
  public ScavengerFileReader(
      EnvironmentImpl env, int readBufferSize, long startLsn, long finishLsn, long endOfFileLsn)
  ...
  protected void handleGapInBackwardsScan(long prevFileNum) {
    if (!resyncReader(DbLsn.makeLsn(prevFileNum, DbLsn.MAX_FILE_OFFSET), false)) {
      throw new EnvironmentFailureException(
          envImpl,
  ...
  /** @return true if this reader should process this entry, or just skip over it. */
  @Override
