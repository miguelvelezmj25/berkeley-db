...
public class CleanerFileReader extends FileReader {
  ...
  /**
   * Process the header to track the last VLSN and count true utilization. Then read the entry and
   * return true if the LogEntryType is of interest.
   *
   * <p>We don't override isTargetEntry so it always returns true and we can count utilization
   * correctly here in processEntry. We call getLastLsn to count utilization and this is not allowed
   * from isTargetEntry.
   */
  @Override
    ...
    if (currentEntryHeader.getReplicated()) {
      ...
      if (vlsn != null) {
        ...
        assert (vlsn.compareTo(lastVLSN) > 0)
            : "vlsns out of order, last=" + lastVLSN + " current=" + vlsn;
        lastVLSN = vlsn;
      }
    }
