...
/**
 * Flush the log buffers (and write queue) periodically to disk and to the file system, as specified
 * by {@link com.sleepycat.je.EnvironmentConfig#LOG_FLUSH_SYNC_INTERVAL} and {@link
 * com.sleepycat.je.EnvironmentConfig#LOG_FLUSH_NO_SYNC_INTERVAL}.
 *
 * <p>Currently flushing occurs if any transactions were committed during the interval. In the
 * future we may want to flush if there were no writes or fynscs in the interval, to allow
 * specifying an even smaller interval for NO_SYNC flushing. This would mean that the wakeup
 * interval should be the config interval divided by 2.
 */
public class LogFlusher {
  ...
  public LogFlusher(EnvironmentImpl envImpl) {
    ...
    this.timer =
        new Timer(envImpl.makeDaemonThreadName(Environment.LOG_FLUSHER_NAME), true /*isDaemon*/);
  }

  /**
   * Applies the new configuration, then cancels and reschedules the flush tasks as needed.
   *
   * @throws IllegalArgumentException if an illegal combination of old and new flush params were
   *     specified.
   */
    ...
    if (configMgr.isSpecified(OLD_REP_RUN_LOG_FLUSH_TASK)
      ...
      if (configMgr.isSpecified(LOG_FLUSH_SYNC_INTERVAL)
        ...
        throw new IllegalArgumentException(
            "When "
                + OLD_REP_RUN_LOG_FLUSH_TASK.getName()
                + " is set to false, "
      ...
      if (configMgr.isSpecified(OLD_REP_LOG_FLUSH_TASK_INTERVAL)) {
        ...
        if (configMgr.isSpecified(LOG_FLUSH_SYNC_INTERVAL)) {
          ...
          throw new IllegalArgumentException(
              "Both "
                  + OLD_REP_LOG_FLUSH_TASK_INTERVAL.getName()
                  + " and "
                  + LOG_FLUSH_SYNC_INTERVAL
                  + " must not be specified.");
        }
