...
/**
 * LogBuffers hold outgoing, newly written log entries. Space is allocated via the allocate() method
 * that returns a LogBufferSegment object. The LogBuffer.writePinCount is incremented each time
 * space is allocated. Once the caller copies data into the log buffer, the pin count is decremented
 * via the free() method. Readers of a log buffer wait until the pin count is zero.
 *
 * <p>The pin count is incremented under the readLatch. The pin count is decremented without holding
 * the latch. Holding the readLatch will prevent the pin count from being incremented.
 *
 * <p>Apart from the pin count, access to the buffer is protected by the readLatch and the LWL: -
 * Write access requires holding both the LWL and the readLatch. - Read access requires holding
 * either the LWL or the readLatch.
 *
 * <p>Of course, for buffers outside the buffer pool, or in the process of being constructed, these
 * rules do not apply and no latching is necessary.
 *
 * <p>TODO: Although the above statement about latching reflects the current implementation, it
 * would be better if we can remove the reliance on the LWL and protect all access to the buffer
 * using the readLatch. To do this, the callers of getFirstLsn and hasRoom will have to acquire the
 * readLatch.
 *
 * @see LogBufferPool
 */
public class LogBuffer implements LogSource {
  ...
  private final AtomicInteger writePinCount = new AtomicInteger();
  private byte[] data;
  ...
  LogBuffer(int capacity, EnvironmentImpl env) throws DatabaseException {
    ...
    readLatch = LatchFactory.createExclusiveLatch(env, DEBUG_NAME, false /*collectStats*/);
    this.env = env;
    reinit();
  }
   ...
   *
   * <p>The LWL or readLatch must be held.
   */
  ...
  /**
   * Register the LSN for a buffer segment that has been allocated in this buffer.
   *
   * <p>The LWL and readLatch must be held.
   */
  void registerLsn(long lsn) {
    assert readLatch.isExclusiveOwner();

      assert lastLsn == DbLsn.NULL_LSN || (DbLsn.compareTo(lsn, lastLsn) > 0) : "lsn=" + lsn + " lastlsn=" + lastLsn;
  ...
  /**
   * Support for reading out of a still-in-memory log. Can be used to determine if a log entry with
   * a given LSN is contained in this buffer, or whether an arbitrary LSN location is present in the
   * buffer.
   *
   * <p>No latches need be held. The buffer is latched for read if true is returned.
   *
   * <p>This method must wait until the buffer's pin count goes to zero. When writing is active and
   * this is the currentWriteBuffer, it may have to wait until the buffer is full.
   *
   * @return true if this buffer holds the data at this LSN location. If true is returned, the
   *     buffer will be latched for read. Returns false if LSN is not here, and releases the read
   *     latch.
   */
  ...
  /**
   * Acquires the readLatch, providing exclusive access to the buffer. When modifying the buffer,
   * both the LWL and buffer latch must be held.
   *
   * <p>Note that containsLsn() acquires the latch for reading.
   *
   * <p>Call release() to release the latch.
   *
   * <p>TODO: It would be possible to use a shared buffer latch to allow concurrent access by
   * multiple readers. The access rules for would then be: - Write access requires holding both the
   * LWL and the buffer latch EX. - Read access requires holding either the LWL or the buffer latch
   * SH. Note that LogBufferPool.bumpCurrent calls latchForWrite, but it may actually only need read
   * access.
   */
   ...
   * @return null if not enough room, otherwise a LogBufferSegment for the data.
   */
  public LogBufferSegment allocate(int size) {
    assert readLatch.isExclusiveOwner();

    if (hasRoom(size)) {
      ByteBuffer buf = ByteBuffer.wrap(data, buffer.position(), size);
    ...
    while (!done) {
      if (writePinCount.get() > 0) {
        ...
        if (Thread.interrupted()) {
          throw new ThreadInterruptedException(env, "Interrupt during read operation");
        }
      } else {
        readLatch.acquireExclusive();
        if (writePinCount.get() == 0) {
  ...
  /**
   * Make a copy of this buffer (doesn't copy data, only buffer state) and position it to read the
   * requested data.
   *
   * <p>The LWL or readLatch must be held.
   *
  ...
  public String toString() {
    return "[LogBuffer firstLsn=" + DbLsn.getNoFormatString(firstLsn) + "]";
  }
}
