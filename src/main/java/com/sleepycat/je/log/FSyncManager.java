...
class FSyncManager {
  ...
  FSyncManager(EnvironmentImpl envImpl) {
    ...
    grpcInterval =
        envImpl.getConfigManager().getDurationNS(EnvironmentParams.LOG_GROUP_COMMIT_INTERVAL);
    grpcThreshold = envImpl.getConfigManager().getInt(EnvironmentParams.LOG_GROUP_COMMIT_THRESHOLD);
      grpWaitOn = grpcInterval != 0 && grpcThreshold != 0;
    ...
    stats =
        new StatGroup(LogStatDefinition.FSYNCMGR_GROUP_NAME, LogStatDefinition.FSYNCMGR_GROUP_DESC);
  ...
  /**
   * Request to flush the log buffer and optionally fsync to disk. This thread may or may not
   * actually execute the flush/fsync, but will not return until a flush/fsync has been issued and
   * executed on behalf of its write. There is a timeout period specified by
   * EnvironmentParam.LOG_FSYNC_TIMEOUT that ensures that no thread gets stuck here indefinitely.
   *
   * <p>When a thread comes in, it will find one of two things. 1. There is no work going on right
   * now. This thread should go ahead and become the group leader. The leader may wait and executes
   * the flush/fsync. 2. There is work going on, wait on the next group commit.
   *
   * <p>When a work is going on, all those threads that come along are grouped together as the
   * nextFsyncWaiters. When the current work is finished, one of those nextFsyncWaiters will be
   * selected as a leader to issue the next flush/fsync. The other members of the group will merely
   * wait until the flush/fsync done on their behalf is finished.
   *
   * <p>When a thread finishes a flush/fsync, it has to: 1. wake up all the threads that were
   * waiting in the group. 2. wake up one member of the next group of waiting threads (the
   * nextFsyncWaiters) so that thread can become the new leader and issue the next flush/fysnc call.
   *
   * <p>If a non-leader member of the nextFsyncWaiters times out, it will issue its own flush/fsync
   * anyway, in case something happened to the leader.
   *
            ...
            if (interval < grpcInterval) {
              try {
                ...
                throw new ThreadInterruptedException(
                    envImpl, "Unexpected interrupt while " + "waiting for write or fsync", e);
              }
            }
  ...
  static class FSyncGroup {
    ...
    /**
     * Wait for either a turn to execute a fsync, or to find out that a fsync was done on your
     * behalf.
     *
     * @return true if the fsync wasn't done, and this thread needs to execute a fsync when it wakes
     *     up. This may be true because it's the leader of its group, or because the wait timed out.
     */
