...
/**
 * A FileReader is an abstract class that traverses the log files, reading-in chunks of the file at
 * a time. The class provides a iterator interface, via its readNextEntry() method. Concrete
 * subclasses must (a) provide the public methods that allow their users to examine the contents of
 * the logrec that the iterator is currently positioned at, and (b) implement the (non-public)
 * isTarget() and processEntry() methods that may filter-out logrecs that do not need to be seen by
 * the caller of readNextEntry(), or perform specific actions before each logrec is "returned" to
 * the caller of readNextEntry().
 */
public abstract class FileReader {
  ...
  private final boolean doChecksumOnRead; // Validate checksums
  ...
  /**
   * Scans the log files until either it has reached the end of the log or has hit an invalid
   * portion.
   *
   * @return true if an element has been read, false at end-of-log.
   * @throws EnvironmentFailureException if a ChecksumException, FileNotFoundException, or another
  ...
  private void readBasicHeader(ByteBuffer dataBuffer) throws ChecksumException, DatabaseException {
    ...
    currentEntryHeader = new LogEntryHeader(dataBuffer, window.logVersion, window.getCurrentLsn());
  ...
  private void copyToSaveBuffer(int bytesNeeded) {
    ...
    if (saveBuffer.capacity() - saveBuffer.position() < bytesFromThisBuffer) {
  ...
  /**
   * This method is called by readNextEntry() after the header of the current logrec has been
   * de-serialized, but not the body. Based on header info only, it may perform some actions and
   * then decide whether the rest of the logrec should be de-serialized or just skipped.
   *
   * @return true if this reader should process the current logrec further, via the processEntry()
   *     method. A logrec must be passed to processEntry if the full logrec (not just the header)
   *     must be de-serialized for further processing. Return false if no further processing is
   *     needed, in which case the current logrec will be skipped (i.e, not returned to the caller
   *     of readNextEntry().
   ...
   *
   * @param entryBuffer A ByteBuffer that the logrec data and is positioned at the start of the
   *     logrec body (i.e., just after the logrec header).
   * @return true if this entry should be returned to the caller of readNextEntry().
   */
  ...
  protected static class ReadWindow {
    ...
    /**
     * Ensure that whenever we change the fileNum, the logVersion is also updated. The fileNum and
     * logVersion fields should be kept private.
     */
    ...
    boolean containsOffset(long targetOffset) {
      return (targetOffset >= startOffset) && (targetOffset < endOffset);
    }
    ...
    public boolean containsLsn(long targetFileNumber, long targetOffset) {
      return ((fileNum == targetFileNumber) && containsOffset(targetOffset));
    }
