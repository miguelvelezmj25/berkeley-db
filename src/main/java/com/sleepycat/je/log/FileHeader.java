...
public class FileHeader implements Loggable {
  ...
  int validate(EnvironmentImpl envImpl, String fileName, long expectedFileNum)
      throws DatabaseException {

    if (logVersion > LogEntryType.LOG_VERSION) {
      throw new VersionMismatchException(
          envImpl,
          "Expected log version "
              + LogEntryType.LOG_VERSION
              + " or earlier but found "
              + logVersion);
    }
  ...
  /**
   * @see Loggable#writeToLog Serialize this object into the buffer. Update cksum with all the bytes
   *     used by this object
  ...
  /**
   * @see Loggable#readFromLog Initialize this object from the data in itemBuf.
