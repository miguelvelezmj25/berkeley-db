...
public class LogUtils {
  ...
  /**
   * Marshall a long into the next 4 bytes in this buffer. Necessary when the long is used to hold
   * an unsigned int.
   */
  ...
  /**
   * The byte[]'s in Xid's are known to be 255 or less in length. So instead of using
   * read/writeByteArray(), we can save 6 bytes per record by making the byte[] length be 1 byte
   * instead of 4.
   */
  ...
  public static class XidImpl implements Xid {
    ...
    public boolean equals(Object o) {
      ...
      if (xid.getFormatId() != formatId) {
        ...
        return compareByteArrays(xid.getGlobalTransactionId(), gid)
