 ...
 *
 * <p>- BINDeltaLogEntry subclasses INLogEntry and is used to read/write BIN-delta logrecs for log
 * versions 9 or later.
 *
 * <p>- OldBINDeltaLogEntry is used to read/write BIN-delta logrecs for log versions earlier than 9.
 * OldBINDeltaLogEntry is not a subclass of INLogEntry.
 *
 * <p>On disk, a full IN logrec contains:
 *
 * <pre>
 * (3 <= version < 6)
 *        IN
...
public class INLogEntry<T extends IN> extends BaseEntry<T> implements LogEntry, INContainingEntry {
  ...
  /** If non-null, used to write a pre-serialized log entry. In this case the 'in' field is null. */
  ...
  /**
   * Returns the main item BIN if it has any slots with expiration times. Must only be called if
   * this entry's type is BIN or BIN_DELTA.
   *
   * <p>This method is called for expiration tracking because getMainItem and getIN cannot be called
   * on an INLogEntry logging parameter, since it may be in pre-serialize form when it appears in
   * the off-heap cache.
   */
  ...
  /**
   * INs from two different environments are never considered equal, because they have lsns that are
