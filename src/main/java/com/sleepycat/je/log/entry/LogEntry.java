...
/**
 * A Log entry allows you to read, write and dump a database log entry. Each entry may be made up of
 * one or more loggable items.
 *
 * <p>The log entry on disk consists of a log header defined by LogManager and the specific contents
 * of the log entry.
 *
 * <p>Log entries that support replication are required to implement {@link ReplicableLogEntry}.
 */
public interface LogEntry extends Cloneable {
  ...
  /**
   * Construct a complete item from a item entry, fetching additional log entries as needed to
   * ensure that a usable main object is available.
   *
   * <p>For an OldBINDeltaLogEntry, fetches the full BIN and merges the delta information. This is
   * necessary to return a Node main object. However, for the new BINDeltaLogEntry, the full BIN is
   * not fetched, since the partial BIN (the delta) is usable as a Node.
   */
  Object getResolvedItem(DatabaseImpl dbImpl);

  /**
   * @return the ID of the database containing this entry, or null if this entry type is not part of
   *     a database.
   */
  DatabaseId getDbId();
  ...
  boolean isImmediatelyObsolete(DatabaseImpl dbImpl);
  ...
  /** @return true if these two log entries are logically the same. Used for replication. */
