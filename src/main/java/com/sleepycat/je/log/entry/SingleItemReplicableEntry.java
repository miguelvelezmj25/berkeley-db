...
/**
 * A basic implementation of a replicable log entry that has a single loggable item and provides for
 * writing in a single format by default. Starting with log version 9, entry classes whose log
 * format has changed since the previous log version will need to override the {@link #getSize(int,
 * boolean)} and {@link #writeEntry(ByteBuffer, int, boolean)} methods to support writing the entry
 * in earlier log formats.
 *
