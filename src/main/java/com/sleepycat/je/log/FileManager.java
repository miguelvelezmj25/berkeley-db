...
public class FileManager {
  ...
  public enum FileMode {
    ...
    private final String fileModeValue;
    private final boolean isWritable;

    FileMode(String fileModeValue, boolean isWritable) {
  ...
  public FileManager(EnvironmentImpl envImpl, File dbEnvHome, boolean readOnly)
    ...
    stats =
        new StatGroup(LogStatDefinition.FILEMGR_GROUP_NAME, LogStatDefinition.FILEMGR_GROUP_DESC);
    ...
    try {
      ...
      fSyncTimeLimit = configManager.getDuration(EnvironmentParams.LOG_FSYNC_TIME_LIMIT);

      nDataDirs = configManager.getInt(EnvironmentParams.LOG_N_DATA_DIRECTORIES);
      ...
      if (!envImpl.isMemOnly()) {
         ...
         */
        boolean isReadOnly = !envImpl.isArbiter() && readOnly;
      ...
      perFileLastUsedLsn = Collections.synchronizedMap(new HashMap<Long, Long>());
      ...
      if (stopOnWriteActionProp != null) {
        if (stopOnWriteActionProp.compareToIgnoreCase("throw") == 0) {
          ...
          throw new IllegalArgumentException(
              "Unknown value for: " + stopOnWriteActionName + stopOnWriteActionProp);
        }
      }

      final Boolean logFileDeleteDetect =
          configManager.getBoolean(EnvironmentParams.LOG_DETECT_FILE_DELETE);
      if (!envImpl.isMemOnly() && logFileDeleteDetect) {
        fdd = new FileDeletionDetector(dbEnvHome, dbEnvDataDirs, envImpl);
      } else {
        fdd = null;
      }
   ...
   * @param prevOffset value to use for the prevOffset of the next entry. If the beginning of the
   *     file, this is 0.
   */
  ...
  /**
   * May be used to disable sync at file end to speed unit tests. Must only be used for unit
   * testing, since log corruption may result.
   */
  ...
  /**
   * Returns the highest (current) file number. Note that this is unsynchronized, so if it is called
   * outside the log write latch it is only valid as an approximation.
   */
   ...
   *
   * @param curFile the file we're at right now. Note that it may not exist, if it's been cleaned
   *     and renamed.
   * @param forward if true, we want the next larger file, if false we want the previous file
   ...
   *
   * @return index into dbEnvDataDirs of this fileName's data directory. -1 if multiple data
   *     directories are not being used.
   */
  private int getDataDirIndexFromName(String fileName) {
    ...
    return Integer.valueOf(Integer.parseInt(dataDir.substring("data".length())));
  }
   ...
   *     <p>Used by unit tests so package protection.
   */
  ...
  /**
   * Find .jdb files which are >= the minimimum file number and <= the maximum file number. Return
   * names sorted in ascending fashion.
   *
   * @return array of file names
   */
    ...
    if (envMultiSubDir) {
      for (File file : envDirFile.listFiles()) {
        if (file.isDirectory() && file.getName().startsWith("data")) {
          File[] subFiles = file.listFiles(new JEFileFilter(suffixes));
  ...
  private File getDataDir(long fileNum) {
    return (nDataDirs == 0) ? dbEnvHome : dbEnvDataDirs[((int) (fileNum % nDataDirs))];
  }
  ...
  public String getFullFileName(String fileName) {
    ...
    String suffix = fileName.substring(suffixStartPos);
    assert suffix != null;
    String fileNum = fileName.substring(0, suffixStartPos);

    return getFullFileName(Long.valueOf(Long.parseLong(fileNum, 16)), suffix);
  }
  ...
  /**
   * HexFormatter generates a 0 padded string starting with 0x. We want the right most 8 digits, so
   * start at 10.
   */
  ...
  /**
   * @return true if successful, false if File.renameTo returns false, which can occur on Windows if
  ...
  /**
   * Rename this file to NNNNNNNN.suffix. If that file already exists, try NNNNNNNN.suffix.1, etc.
   * Used for deleting files or moving corrupt files aside.
   *
   * @param fileNum the file we want to move
   * @param newSuffix the new file suffix
   * @param subDir the data directory sub-directory to rename the file into. The subDir must already
   *     exist. May be null to leave the file in its current data directory.
   * @return renamed File if successful, or null if File.renameTo returns false, which can occur on
   *     Windows if the file was recently closed.
   */
  public File renameFile(final long fileNum, final String newSuffix, final String subDir)
      throws IOException {
    ...
    final File newDir = (subDir != null) ? (new File(oldDir, subDir)) : oldDir;
   ...
   *
   * @param fileNum the file we want to move
   * @return true if successful, false if File.delete returns false, which can occur on Windows if
   *     the file was recently closed.
   */
  ...
  /**
   * Return a read only file handle that corresponds to this file number. Retrieve it from the cache
   * or open it anew and validate the file header. This method takes a latch on this file, so that
   * the file descriptor will be held in the cache as long as it's in use. When the user is done
   * with the file, the latch must be released.
   *
      ...
      while (true) {
        ...
        if (newHandle) {
          ...
          if (!fileHandle.latchNoWait()) {
            ...
            fileHandle =
                new FileHandle(envImpl, fileId, getFileNumberString(fileId)) {
                  @Override
            ...
            openFileHandle(fileHandle, FileMode.READ_MODE, existingHandle);
          }
        }
  ...
  /**
   * Creates a new FileHandle and adds it to the cache, but does not open the file.
   *
  ...
  /** Creates a new handle and opens it. Does not add the handle to the cache. */
   ...
   *
   * @param existingHandle is an already open handle for the same file or null. If non-null it is
   *     used to avoid the cost of reading the file header.
   */
  private void openFileHandle(FileHandle fileHandle, FileMode mode, FileHandle existingHandle)
      throws FileNotFoundException, ChecksumException {
    ...
    try {
      ...
      if (newFile.length() == 0) {
        ...
        if (mode.isWritable()) {
          ...
          if ((headerPrevOffset == 0) && (fileNum > 1) && syncAtFileEnd) {
      ...
      throw new EnvironmentFailureException(
          envImpl, EnvironmentFailureReason.LOG_READ, "Couldn't open file " + fileName, e);
   ...
   */
  private int readAndValidateFileHeader(RandomAccessFile file, String fileName, long fileNum)
    ...
    LogEntry headerEntry =
        logManager.getLogEntryAllowChecksumException(
            DbLsn.makeLsn(fileNum, 0), file, LogEntryType.UNKNOWN_FILE_HEADER_VERSION);
  ...
  private void writeFileHeader(
      RandomAccessFile file, String fileName, FileHeader header, long fileNum)
    ...
    ByteBuffer headerBuf =
        envImpl.getLogManager().putIntoBuffer(headerLogEntry, 0); // prevLogEntryOffset
    ...
    try {
       ...
       * header. [#20732]
       */
      bytesWritten = writeToFile(file, headerBuf, 0, fileNum, true /*flushRequired*/);
  ...
  long getFileHeaderPrevOffset(long fileNum) throws ChecksumException, DatabaseException {
    ...
    try {
      LogEntry headerEntry =
          envImpl.getLogManager().getLogEntryAllowChecksumException(DbLsn.makeLsn(fileNum, 0));
      FileHeader header = (FileHeader) headerEntry.getMainItem();
  ...
  boolean shouldFlipFile(long size) {
    return forceNewFile || (DbLsn.getFileOffset(nextAvailableLsn) + size) > maxFileSize;
  }

  /** Calculates LSN of next entry to be logged. */
  long calculateNextLsn(boolean flippedFile) {
    return flippedFile
        ? DbLsn.makeLsn(currentFileNum + 1, FileManager.firstLogEntryOffset())
        : nextAvailableLsn;
  }
   ...
   *
   * <p>When flippedFile is true, this method must be called after flushing the prior file. We
   * guarantee that certain volatile LSN fields (currentFileNumber, nextAvailableLsn, lastUsedLsn)
   * are not updated until after flushing the prior file.
   *
   ...
   * @return the file offset of the previous LSN that was used. Needed for constructing the header
   *     of the log entry for currentLsn. If the previous LSN was in the previous file, or this is
   *     the very first LSN of the env, zero is returned.
   */
  long advanceLsn(long currentLsn, long size, boolean flippedFile) {
    ...
    if (flippedFile) {
      ...
      prevOffset = (lastUsedLsn == DbLsn.NULL_LSN) ? 0 : DbLsn.getFileOffset(lastUsedLsn);
    }

    forceNewFile = false;
    lastUsedLsn = currentLsn;

    nextAvailableLsn = DbLsn.makeLsn(currentFileNum, DbLsn.getFileOffset(currentLsn) + size);
   ...
   * @param flushWriteQueue true if this write can not be queued on the Write Queue.
   */
  void writeLogBuffer(LogBuffer fullBuffer, boolean flushWriteQueue) throws DatabaseException {
      ...
      try {
        ...
        if (LOGWRITE_EXCEPTION_TESTING) {
          generateLogWriteException(
              file, data, DbLsn.getFileOffset(firstLsn), DbLsn.getFileNumber(firstLsn));
        }
        writeToFile(
            file,
            data,
         ...
         */
        throw new ThreadInterruptedException(
            envImpl, "File closed, may be due to thread interrupt", e);
      } catch (IOException e) {
        throw new LogWriteException(envImpl, e);
      }
  ...
  private int writeToFile(
    ...
    if (lastFileNumberTouched == fileNum
        && (Math.abs(destOffset - lastFileTouchedOffset) < ADJACENT_TRACK_SEEK_DELTA)) {
    ...
    if (!fsyncLatchAcquired && useWriteQueue && !flushWriteQueue) {
      enqueueSuccess =
          endOfLog.enqueueWrite(fileNum, data.array(), destOffset, pos + data.arrayOffset(), size);
    }
  ...
  /**
   * Read a buffer from a file at a given offset. We know that the desired data exists in this file.
   * There's no need to incur extra costs such as checks of the file length, nor to return status as
   * to whether this file contains the data.
   */
  void readFromFile(RandomAccessFile file, ByteBuffer readBuffer, long offset, long fileNo)
      throws DatabaseException {
    readFromFile(file, readBuffer, offset, fileNo, true /* dataKnownToBeInFile */);
  }

  /**
   * Read a buffer from a file at a given offset.
   *
   * @return true if the read buffer is filled, false, if there is nothing left in the file to read
   */
  ...
  private void readFromFileInternal(
      RandomAccessFile file, ByteBuffer readBuffer, long offset, long fileNum) throws IOException {
    ...
    synchronized (file) {
      ...
      if (lastFileNumberTouched == fileNum
          && (Math.abs(offset - lastFileTouchedOffset) < ADJACENT_TRACK_SEEK_DELTA)) {
  ...
  private void printLogBuffer(ByteBuffer entryBuffer, long lsn) {
    ...
    while (entryBuffer.remaining() > 0) {
      ...
      try {
        header = new LogEntryHeader(entryBuffer, LogEntryType.LOG_VERSION, lsn);
      } catch (ChecksumException e) {
  ...
  private void verifyChecksums(ByteBuffer entryBuffer, long lsn, String comment) {
    ...
    try {
      while (entryBuffer.remaining() > 0) {
        ...
        LogEntryHeader header = new LogEntryHeader(entryBuffer, LogEntryType.LOG_VERSION, lsn);
  ...
  private void verifyChecksum(
      ByteBuffer entryBuffer, LogEntryHeader header, long lsn, String comment)
  ...
  /**
   * Sync the end of the log, close off this log file. Should only be called under the log write
   * latch.
   */
  ...
  public void startFileCacheWarmer(final long recoveryStartLsn) {
    ...
    final int warmUpSize = cm.getInt(EnvironmentParams.LOG_FILE_WARM_UP_SIZE);

    if (warmUpSize == 0) {
      return;
    }

    final int bufSize = cm.getInt(EnvironmentParams.LOG_FILE_WARM_UP_BUF_SIZE);

    fileCacheWarmer =
        new FileCacheWarmer(envImpl, recoveryStartLsn, lastUsedLsn, warmUpSize, bufSize);
  ...
  /**
   * Lock the environment. Return true if the lock was acquired. If exclusive is false, then this
   * implements a single writer, multiple reader lock. If exclusive is true, then implement an
   * exclusive lock.
   *
   * <p>There is a lock file and there are two regions of the lock file: byte 0, and byte 1. Byte 0
   * is the exclusive writer process area of the lock file. If an environment is opened for write,
   * then it attempts to take an exclusive write lock on byte 0. Byte 1 is the shared reader process
   * area of the lock file. If an environment is opened for read-only, then it attempts to take a
   * shared lock on byte 1. This is how we implement single writer, multi reader semantics.
   *
   * <p>The cleaner, each time it is invoked, attempts to take an exclusive lock on byte 1. The
   * owning process already either has an exclusive lock on byte 0, or a shared lock on byte 1. This
   * will necessarily conflict with any shared locks on byte 1, even if it's in the same process and
   * there are no other holders of that shared lock. So if there is only one read-only process, it
   * will have byte 1 for shared access, and the cleaner can not run in it because it will attempt
   * to get an exclusive lock on byte 1 (which is already locked for shared access by itself). If a
   * write process comes along and tries to run the cleaner, it will attempt to get an exclusive
   * lock on byte 1. If there are no other reader processes (with shared locks on byte 1), and no
   * other writers (which are running cleaners on with exclusive locks on byte 1), then the cleaner
   * will run.
   */
      ...
      if (lockFile == null) {
        lockFile =
            new RandomAccessFile(
                new File(dbEnvHome, LOCK_FILE), FileMode.READWRITE_MODE.getModeValue());
      }
  ...
  /**
   * Ensure that if the environment home dir is on readonly media or in a readonly directory that
   * the environment has been opened for readonly access.
   *
  ...
  private boolean checkEnvHomePermissionsSingleEnvDir(File dbEnvHome, boolean rdOnly)
      throws DatabaseException {
  ...
  /**
   * Truncate a log at this position. Used by recovery to a timestamp utilities and by recovery to
   * set the end-of-log position, see LastFileReader.setEndOfFile().
   *
   * <p>This method forces a new log file to be written next, if the last file (the file truncated
   * to) has an old version in its header. This ensures that when the log is opened by an old
   * version of JE, a version incompatibility will be detected. [#11243]
   */
  ...
  public void truncateLog(long fileNum, long offset) throws IOException, DatabaseException {
    ...
    for (long i = getLastFileNum(); i >= fileNum; i--) {
      ...
      assert deleted : "File " + getFullFileName(i, JE_SUFFIX) + " not deleted during truncateLog";
    }
  }

  /**
   * Mark the specified log entries as invisible and obsolete. The entries are written here, but are
   * fsync'ed later. If there is any problem or exception during the setting, the method will throw
   * an EnvironmentFailureException.
   *
   * <p>These changes are made directly to the file, but recently logged log entries may also be
   * resident in the log buffers. The caller must take care to call LogManager.flush() before this
   * method, to ensure that all entries are on disk.
   *
   * <p>In addition, we must ensure that after this step, the affected log entries will only be read
   * via a FileReader, and will not be faulted in by the LogManager. Entries may be present in the
   * log and in the log buffers, but only the on disk version is modified by this method. The
   * LogManager can read directly from the log buffers and may read the incorrect, non-invisible
   * version of the log entry, rather than the invisible version from the file. This should not be
   * an issue, because invisible log entries should be detached from the in-memory tree before they
   * are made invisible.
   *
   * @param fileNum target file.
   * @param lsns The list of LSNs to make invisible, must be sorted in ascending order.
   */
      ...
      for (Long lsn : lsns) {
        if (DbLsn.getFileNumber(lsn) != fileNum) {
          ...
          throw new EnvironmentFailureException(
              ...
              "LSN of " + DbLsn.getNoFormatString(lsn) + " did not match file number" + fileNum);
        }
  ...
  public void force(Set<Long> fileNums) {
    for (long fileNum : fileNums) {
      ...
      try {
        ...
        throw new EnvironmentFailureException(
            envImpl, EnvironmentFailureReason.LOG_WRITE, "Invisible fsyncing file " + fileNum, e);
      } finally {
        if (file != null) {
          try {
            file.close();
  ...
  /**
   * Return the next available LSN in the log. Note that this is unsynchronized, so if it is called
   * outside the log write latch it is only valid as an approximation.
   */
  ...
  /**
   * Return the last allocated LSN in the log. Note that this is unsynchronized, so if it is called
   * outside the log write latch it is only valid as an approximation.
   */
  ...
  private static class FileCache {
    ...
    /**
     * Take any file handles corresponding to this file name out of the cache. A file handle could
     * be there twice, in rd only and in r/w mode.
     */
  ...
  /**
   * The LogEndFileDescriptor is used to write and fsync the end of the log. Because the JE log is
   * append only, there is only one logical R/W file descriptor for the whole environment. This
   * class actually implements two RandomAccessFile instances, one for writing and one for fsyncing,
   * so the two types of operations don't block each other.
   *
   * <p>The write file descriptor is considered the master. Manipulation of this class is done under
   * the log write latch. Here's an explanation of why the log write latch is sufficient to
   * safeguard all operations.
   *
   * <p>There are two types of callers who may use this file descriptor: the thread that is
   * currently writing to the end of the log and any threads that are fsyncing on behalf of the
   * FSyncManager.
   *
   * <p>The writing thread appends data to the file and fsyncs the file when we flip over to a new
   * log file. The file is only instantiated at the point that it must do so -- which is either when
   * the first fsync is required by JE or when the log file is full and we flip files. Therefore,
   * the writing thread has two actions that change this descriptor -- we initialize the file
   * descriptor for the given log file at the first write to the file, and we close the file
   * descriptor when the log file is full. Therefore is a period when there is no log descriptor --
   * when we have not yet written a log buffer into a given log file.
   *
   * <p>The fsyncing threads ask for the log end file descriptor asynchronously, but will never
   * modify it. These threads may arrive at the point when the file descriptor is null, and
   * therefore skip their fysnc, but that is fine because it means a writing thread already flipped
   * that target file and has moved on to the next file.
   *
   * <p>Time Activity 10 thread 1 writes log entry A into file 0x0, issues fsync outside of log
   * write latch, yields the processor 20 thread 2 writes log entry B, piggybacks off thread 1 30
   * thread 3 writes log entry C, but no room left in that file, so it flips the log, and fsyncs
   * file 0x0, all under the log write latch. It nulls out endOfLogRWFile, moves onto file 0x1, but
   * doesn't create the file yet. 40 thread 1 finally comes along, but endOfLogRWFile is null-- no
   * need to fsync in that case, 0x0 got fsynced.
   *
   * <p>If a write is attempted and an fsync is already in progress, then the information pertaining
   * to the data to be written (data, offset, length) is saved away in the "queuedWrites" array.
   * When the fsync completes, the queuedWrites buffer is emptied. This ensures that writes continue
   * to execute on file systems which block all IO calls during an fsync() call (e.g. ext3).
   */
  class LogEndFileDescriptor {
     ...
     */
    private final byte[] queuedWrites = useWriteQueue ? new byte[writeQueueSize] : null;
     ...
     */
    /* private */
    boolean checkWriteCache(
        final ByteBuffer readBuffer, final long requestedOffset, final long fileNum) {
      ...
      synchronized (queuedWrites) {
        ...
        if (requestedOffset < qwStartingOffset
            || (qwStartingOffset + queuedWritesPosition) <= requestedOffset) {
          return false;
        }
     ...
     *
     * <p>Typically, endOfLogRWFile is not null. Hence the fsyncFileSynchronizer does not need to be
     * locked (which would block the write queue from operating.
     */
    ...
    private void force() throws DatabaseException, IOException {
      ...
      try {
        ...
        if (file != null) {
          ...
          if (nFSyncMaxTime.setMax(fSyncMs) && fSyncTimeLimit != 0 && fSyncMs > fSyncTimeLimit) {

            LoggerUtils.warning(
                envImpl.getLogger(),
                envImpl,
                String.format(
                    "FSync time of %d ms exceeds limit (%d ms)", fSyncMs, fSyncTimeLimit));
          }
    ...
    /**
     * Close the end of the log file descriptor. Use atomic assignment to ensure that we won't force
     * and close on the same descriptor.
     */
  ...
  private static final String RRET_PROPERTY_NAME = "je.logwrite.exception.testing";
  ...
  private void generateLogWriteException(
      RandomAccessFile file, ByteBuffer data, long destOffset, long fileNum)
    ...
    if (logWriteExceptionCounter == logWriteExceptionRandom.nextInt(LOGWRITE_EXCEPTION_MAX)) {
      ...
      if (len > 0) {
        ...
        writeToFile(file, buf, destOffset, fileNum, false /*flushRequired*/);
      }
  ...
  /**
   * The factory interface for creating RandomAccessFiles. For production use, the default factory
   * is always used and a DefaultRandomAccessFile is always created. For testing, the factory can be
   * overridden to return a subclass of DefaultRandomAccessFile that overrides methods and injects
   * faults, for example.
   */
  public interface FileFactory {

    /**
     * @param envHome can be used to distinguish environments in a test program that opens multiple
     *     environments. Not for production use.
     * @param fullName the full file name to be passed to the RandomAccessFile constructor.
     * @param mode the file mode to be passed to the RandomAccessFile constructor.
     */
  ...
  /**
   * The RandomAccessFile for production use. Tests that override the default FileFactory should
   * return a RandomAccessFile that subclasses this class to inherit workarounds such as the
   * overridden length method.
   */
  public static class DefaultRandomAccessFile extends RandomAccessFile {
    ...
    /**
     * RandomAccessFile.length() is not thread safe and side-effects the file pointer if interrupted
     * in the middle. It is synchronized here to work around that problem.
     */
    @Override
  ...
  /**
   * The factory instance used to create RandomAccessFiles. This field is intentionally public and
   * non-static so it may be set by tests. See FileFactory.
   */
