...
class FileDeletionDetector {
   ...
   * action happens.
   */
  private final Map<WatchKey, File> fileDeletionWatchKeys;

  FileDeletionDetector(
      final File dbEnvHome, final File[] dbEnvDataDirs, final EnvironmentImpl envImpl) {
     ...
     */
    try {
      fileDeletionWatcher = FileSystems.getDefault().newWatchService();
      ...
      throw new EnvironmentFailureException(
          ...
          "Can not register " + dbEnvHome.toString() + " or its sub-directories to WatchService.",
          ie);
    }

    DbConfigManager configManager = envImpl.getConfigManager();
    final int interval =
        configManager.getDuration(EnvironmentParams.LOG_DETECT_FILE_DELETE_INTERVAL);
    /* Periodically detect unexpected log file deletion. */
    fileDeletionTimer =
        new Timer(envImpl.makeDaemonThreadName(Environment.FILE_DELETION_DETECTOR_NAME), true);
  ...
  private void handleUnexpectedThrowable(Thread t, Throwable e) {
    StoppableThread.handleUncaughtException(envImpl.getLogger(), envImpl, t, e);
  }
  ...
  private void processLogFileDeleteEvents() throws Exception {
    ...
    while (true) {
        ...
        for (final File file : fileDeletionWatchKeys.values()) {
          if (!file.exists()) {
            ...
            throw new IOException(
                "Directory " + dir + " does not exist now. " + "Something abnormal may happen.");
          }
        }
        break;
      }
        ...
        if (fileName.endsWith(FileManager.JE_SUFFIX)) {
          synchronized (filesDeletedByJE) {
            if (!filesDeletedByJE.contains(fileName)) {
              ...
              throw new EnvironmentFailureException(
                  envImpl,
                  EnvironmentFailureReason.LOG_UNEXPECTED_FILE_DELETION,
                  "Log file " + fileName + " was deleted unexpectedly.");
            }
      ...
      if (!key.reset()) {
         ...
         */
        final String dir = fileDeletionWatchKeys.get(key).getCanonicalPath();
        throw new IOException(
            "Watch Key corresponding to "
                + dir
                + " return false "
