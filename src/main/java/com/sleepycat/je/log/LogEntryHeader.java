...
public class LogEntryHeader {
  ...
  /**
   * Persistent fields. Layout on disk is (invariant) checksum - 4 bytes (invariant) entry type - 1
   * byte (invariant) entry flags - 1 byte (invariant) offset of previous log entry - 4 bytes
   * (invariant) item size (not counting header size) - 4 bytes (optional) vlsn - 8 bytes
   *
   * <p>Flags: The provisional bit can be set for any log type in the log. It's an indication to
   * recovery that the entry shouldn't be processed when rebuilding the tree. See
   * com.sleepycat.je.log.Provisional.java for the reasons why it's set.
   *
   * <p>The replicated bit is set when this particular log entry is part of the replication stream
   * and contains a VLSN in the header.
   *
   * <p>The invisible bit is set when this log entry has been rolled back as part of replication
   * syncup. The ensuing log entry has not been checksum-corrected, and to read it, the invisible
   * bit must be cloaked.
   *
   * <p>The VLSN_PRESENT bit is set when a VLSN is present for log version 8+, and is set when the
   * replicated bit is *not* set in the case of a cleaner migrated LN. Prior to version 8, the
   * replicated bit alone indicates that a VLSN is present. For all versions, if the replicated bit
   * is set then a VLSN is always present. [#19476]
   *
   * <p>first version of migrated LN a replicated LN --------------- ----------- log version 7-
   * replicated = true replicated = false (JE 4.1 and vlsn present = false vlsn present = false
   * earlier) vlsn exists in header no vlsn in header
   *
   * <p>log version 8+ replicated = true replicated = false preserve record vlsn present = true vlsn
   * present = false version = false vlsn exists in header no vlsn in header
   *
   * <p>log version 8+ replicated = true replicated = false preserve record vlsn present = true vlsn
   * present = true version = true vlsn exists in header vlsn exists in header
   */
  ...
  private static final byte VERSION_6_FLAGS =
      PROVISIONAL_ALWAYS_MASK | PROVISIONAL_BEFORE_CKPT_END_MASK | REPLICATED_MASK;
   ...
   *
   * @param entryBuffer the buffer containing at least the first MIN_HEADER_SIZE bytes of the entry
   *     header.
   * @param logVersion is the log version of the file that contains the given buffer, and is
   *     obtained from the file header. Note that for the file header entry itself,
   *     UNKNOWN_FILE_HEADER_VERSION may be passed.
  ...
  public LogEntryHeader(ByteBuffer entryBuffer, int logVersion, long lsn) throws ChecksumException {
    ...
    if (entryType == FILE_HEADER_TYPE_NUM) {
      ...
      if (logVersion == LogEntryType.UNKNOWN_FILE_HEADER_VERSION) {
        ...
        assert logVersion != 6 || (entryVersion == 6);
      } else {
  ...
  public LogEntryHeader(LogEntry entry, Provisional provisional, ReplicationContext repContext) {
    ...
    assert (!((!logEntryType.isReplicationPossible()) && repContext.inReplicationStream()))
   ...
   * See FileHeaderEntry.readEntry. [#16939]
   */
  ...
  /** @return the number of bytes used to store the header, excepting the checksum field. */
  ...
  /**
   * Assumes this is called directly after the constructor, and that the entryBuffer is positioned
   * right before the VLSN.
   */
  ...
  /**
   * Serialize this object into the buffer and leave the buffer positioned in the right place to
   * write the following item. The checksum, prevEntry, and vlsn values will filled in later on.
   *
   * <p>public for unit tests.
   */
  ...
  /**
   * Add those parts of the header that must be calculated later to the entryBuffer, and also assign
   * the fields in this class. That's - the prev offset, which must be done within the log write
   * latch to be sure what that lsn is - the VLSN, for the same reason - the checksumVal, which must
   * be added last, after all other fields are marshalled. (public for unit tests)
   */
  ...
  /**
   * Dump the header without enclosing <header> tags. Used for DbPrintLog, to make the header
   * attributes in the <entry> tag, for a more compact rendering.
   *
   * @param sb destination string buffer
   * @param verbose if true, dump the full, verbose version
   */
  ...
  /**
   * For use in special case where commits are transformed to aborts because of i/o errors during a
   * logBuffer flush. See [11271]. Assumes that the entryBuffer is positioned at the start of the
   * item. Return with the entryBuffer positioned to the end of the log entry.
   */
  void convertCommitToAbort(ByteBuffer entryBuffer) {
    ...
    checksum.update(
        entryBuffer.array(), entryTypePosition + entryBuffer.arrayOffset(), checksumSize);
  ...
  /**
   * @return true if two log headers are logically the same. This check will ignore the log version.
   *     <p>Used by replication.
   */
  public boolean logicalEqualsIgnoreVersion(LogEntryHeader other) {
    ...
    return ((getVLSN().equals(other.getVLSN()))
        ...
        && (LogEntryType.compareTypeAndVersion(
            getVersion(), getType(), other.getVersion(), other.getType())));
  }

  /**
   * May be called after reading MIN_HEADER_SIZE bytes to determine whether more bytes
   * (getVariablePortionSize) should be read.
   */
   ...
   *
   * @param logHeaderStartPosition the byte position of the start of the log entry header.
   */
