...
/**
 * LogBufferPool manages a circular pool of LogBuffers. The currentWriteBuffer is the buffer that is
 * currently used to add data. When the buffer is full, the next (adjacent) buffer is made available
 * for writing. The buffer pool has a dirty list of buffers. A buffer becomes a member of the dirty
 * list when the currentWriteBuffer is moved to another buffer. Buffers are removed from the dirty
 * list when they are written. The dirtyStart/dirtyEnd variables indicate the list of dirty buffers.
 * A value of -1 for either variables indicates that there are no dirty buffers. These variable are
 * synchronized via the LogBufferPool.bufferPoolLatch. The LogManager.logWriteLatch (aka LWL) is
 * used to serialize access to the currentWriteBuffer, so that entries are added in write/LSN order.
 *
 * <p>A buffer used for writing is accessed by the getWriteBuffer method. This method must be called
 * while holding the LWL. The buffer returned is not latched.
 *
 * <p>A LogBuffer has a pin count (LogBuffer.writePinCount) associated with it. The pin count is
 * incremented when space is allocated in the buffer. The allocation of space is serialized under
 * the LWL. Threads will add data to the buffer by latching the buffer, but without holding the LWL.
 * After the data is added, the pin count is decremented. A buffer cannot be used for reading unless
 * the pin count is zero. It should be noted that the increment of the pin count is done with the
 * buffer latched. The decrement does not latch the buffer.
 *
 * <p>Read access to a log buffer is allowed only if the buffer is latched and the pin count is
 * zero. A thread that attempt to access a log buffer for reading will latch and check the pin
 * count. If the pin count is not zero, the latch is released and the process is retried. The thread
 * attempting to access the log buffer for reading may be delayed. The worst case is when the reader
 * has to wait until the buffer is filled (the pin count would be zero).
 *
 * @see LogBuffer
 */
class LogBufferPool {
  ...
  LogBufferPool(FileManager fileManager, EnvironmentImpl envImpl) throws DatabaseException {
    ...
    bufferPoolLatch =
        LatchFactory.createExclusiveLatch(
    ...
    stats = new StatGroup(LogStatDefinition.LBF_GROUP_NAME, LogStatDefinition.LBF_GROUP_DESC);
  ...
  /**
   * Initialize the pool at construction time and when the cache is resized. This method is called
   * after the memory budget has been calculated.
   *
   * <p>The LWL must be held when the buffer pool is not being constructed.
   */
   ...
   *
   * <p>If sizeNeeded will fit in currentWriteBuffer, currentWriteBuffer is returned without
   * requiring any flushing. The caller can allocate the entry in the buffer returned.
   *
   * <p>If sizeNeeded won't fit in currentWriteBuffer, but is LTE the LogBuffer capacity, we bump
   * the buffer to get an empty currentWriteBuffer. If there are no free write buffers, then all
   * dirty buffers must be flushed. The caller can allocate the entry in the buffer returned.
   *
   * <p>If sizeNeeded is greater than the LogBuffer capacity, flush all dirty buffers and return an
   * empty (but too small) currentWriteBuffer. The caller must then write the entry to the file
   * directly.
   *
   * <p>The LWL must be held.
   *
   * @param sizeNeeded size of the entry to be written.
   * @param flippedFile if true, always flush all dirty buffers and get an empty currentWriteBuffer,
   *     and fsync/finish the log file.
   * @return currentWriteBuffer, which may or may not have enough room for sizeNeeded.
   */
  LogBuffer getWriteBuffer(int sizeNeeded, boolean flippedFile)
    ...
    if (flippedFile) {
      ...
      if (!bumpCurrent(sizeNeeded) || !currentWriteBuffer.hasRoom(sizeNeeded)) {
   ...
   *
   * <p>The LWL must be held to ensure that, if there are no free buffers, we can write the dirty
   * buffers and have a free one, which is required to bump the current write buffer.
   *
   * @param sizeNeeded used only if running in memory. Size of the log buffer buffer that is needed.
   * @param flushWriteQueue true if data is written to log, otherwise data may be placed on the
   *     write queue.
   */
  void bumpAndWriteDirty(int sizeNeeded, boolean flushWriteQueue) {
    ...
    if (!bumpCurrent(sizeNeeded)) {
      ...
      if (bumpCurrent(sizeNeeded)) {
        ...
        throw EnvironmentFailureException.unexpectedState(envImpl, "No free log buffers.");
      }
  ...
  /**
   * Returns the next buffer slot number from the input buffer slot number. The slots are are a
   * circular buffer.
   *
   * <p>The bufferPoolLatch must be held.
   *
   ...
   *
   * <p>No latches need be held.
   *
   * <p>Note that if no buffers are dirty, nothing will be written, even when data is buffered in
   * the write queue and flushWriteQueue is true. If we were to allow a condition where 1) the write
   * queue is non-empty, 2) there are no dirty log buffers, and 3) the current write buffer is
   * empty, then a flushNoSync at that time won't flush the write queue. This should never happen
   * because: a) flushNoSync leaves the write queue empty, b) LogManager.serialLogWork leaves the
   * current write buffer non-empty, and c) writeDirty(false) doesn't change the state of the
   * current write buffer. TODO: Confirm this with a more detailed analysis.
   *
   * @param flushWriteQueue true then data is written to file otherwise the data may be placed on
   *     the FileManager WriteQueue.
   */
   ...
   * Writes a log buffer.
   *
   * <p>The LWL or buffer latch must be held.
   *
   * @param latchedBuffer buffer to write
   * @param flushWriteQueue true then data is written to file otherwise the data may be placed on
   *     the FileManager WriteQueue.
   */
  private void writeBufferToFile(LogBuffer latchedBuffer, boolean flushWriteQueue) {
    ...
    try {
      ...
      try {
        ...
        if (t instanceof EnvironmentFailureException) {
          ...
          throw EnvironmentFailureException.unexpectedException(envImpl, (Exception) t);
        } else {
          throw EnvironmentFailureException.unexpectedException(envImpl, t.getMessage(), null);
        }
      }
    } finally {
  ...
  /**
   * Move the current write buffer to the next. Will not bump the current write buffer if the buffer
   * is empty.
   *
   * <p>The LWL must be held.
   *
   * @param sizeNeeded used only if running in memory. Size of the log buffer that is needed.
   * @return false when the buffer needs flushing, but there are no free buffers. Returns true when
   *     when running in memory, when the buffer is empty, or when the buffer is non-empty and is
   *     bumped.
   */
  boolean bumpCurrent(int sizeNeeded) {
    ...
    try {
      ...
      LogBuffer newInitialBuffer = bufferPool.get(getNextSlot(currentWriteBufferIndex));
      nextToUse.reinit();
  ...
  /**
   * Set minBufferLsn to start of new initial buffer. The update occurs only after cycling once
   * through the buffers in the pool. This is a simple implementation, and waiting until we've
   * filled the buffer pool to initialize it is sufficient for reducing read contention in
   ...
   * <p>No latches need be held.
   *
   * @return the buffer that contains the given LSN location, latched and ready to read, or return
   *     null.
   */
  ...
  /**
   * Return the current nCacheMiss statistic in a lightweight fashion, without perturbing other
   * statistics or requiring synchronization.
   */
