...
/**
 * Summarizes the utilized and unutilized portion of each log file by examining each log entry. Does
 * not use the Cleaner UtilizationProfile information in order to provide a second measure against
 * which to evaluation the UtilizationProfile accuracy.
 *
 * <p>Limitations =========== BIN-deltas are all considered obsolete, as an implementation short cut
 * and for efficiency. 90% (by default) of deltas are obsolete anyway, and it would be expensive to
 * fetch the parent BIN to find the lookup key.
 *
 * <p>Assumes that any currently open transactions will be committed. For example, if a deletion or
 * update has been performed but not yet committed, the old record will be considered obsolete.
 * Perhaps this behavior could be changed in the future by attempting to lock a record
 * (non-blocking) and considering a locked record to be non-obsolete; this might make it match live
 * utilization counting more closely.
 *
 * <p>Accesses the Btree, using JE cache memory if necessary and contending with other accessors, to
 * check whether an entry is active.
 *
 * <p>Historical note: This implementation, which uses the Btree to determine whether a node is
 * active, replaced an earlier implementation that attempted to duplicate the Btree in memory and
 * read the entire log. This older implementation had inaccuracies and was less efficient. With the
 * new implementation it is also possible to calculation utilization for a range of LSNs, reading
 * only that portion of the log. [#22208]
 */
public class UtilizationFileReader extends FileReader {
  ...
  private UtilizationFileReader(
      EnvironmentImpl envImpl, int readBufferSize, long startLsn, long finishLsn)
  ...
  protected boolean processEntry(ByteBuffer entryBuffer) throws DatabaseException {
    ...
    if (entry instanceof LNLogEntry) {
      ...
      final boolean isActive =
          (dbImpl != null) && !lnEntry.isImmediatelyObsolete(dbImpl) && isLNActive(lnEntry, dbImpl);
  ...
  private boolean isLNActive(LNLogEntry<?> lnEntry, DatabaseImpl dbImpl) {
    ...
    final boolean parentFound =
        tree.getParentBINForChildLN(
            location, key, false /*splitsAllowed*/, false /*blindDeltaOps*/, CacheMode.DEFAULT);
  ...
  private boolean isINActive(INLogEntry<?> inEntry, DatabaseImpl dbImpl) {
    ...
    final SearchResult result =
        tree.getParentINForChildIN(
            logIn, true, /*useTargetLevel*/ true, /*doFetch*/ CacheMode.DEFAULT);
  ...
  /**
   * Creates a UtilizationReader, reads the log, and returns the resulting Map of Long file number
   * to FileSummary.
   */
  ...
  public static Map<Long, FileSummary> calcFileSummaryMap(
      EnvironmentImpl envImpl, long startLsn, long finishLsn) {

    final int readBufferSize =
        envImpl.getConfigManager().getInt(EnvironmentParams.LOG_ITERATOR_READ_SIZE);

    final UtilizationFileReader reader =
        new UtilizationFileReader(envImpl, readBufferSize, startLsn, finishLsn);
    try {
  ...
  private static class ExtendedFileSummary extends FileSummary {
    ...
    /** Overrides the LN size calculation to return the recalculated number of obsolete LN bytes. */
    @Override
    public int getObsoleteLNSize() {
      return recalcObsoleteLNSize;
    }

    /** Overrides the IN size calculation to return the recalculated number of obsolete IN bytes. */
    @Override
