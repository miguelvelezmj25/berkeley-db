...
/**
 * The StatsFileReader generates stats about the log entries read, such as the count of each type of
 * entry, the number of bytes, minimum and maximum sized log entry.
 */
public class StatsFileReader extends DumpFileReader {
  ...
  class CheckpointInfoCSVFormatter extends CheckpointInfoTextFormatter {
  ...
  private void summarizeCSV() {
    Iterator<Map.Entry<LogEntryType, EntryInfo>> iter = entryInfoMap.entrySet().iterator();
    ...
    sb.append(((double) (realTotalKeyBytes * 100) / totalLogBytes));
    System.out.println(sb);
    ...
    sb.append((double) (realTotalDataBytes * 100) / totalLogBytes);
    System.out.println(sb);
  ...
  private void summarizeText() {
    ...
    long keySize = (realTotalKeyCount == 0) ? 0 : (realTotalKeyBytes / realTotalKeyCount);
    double keyPct =
        (totalLogBytes == 0) ? 0 : (((double) (realTotalKeyBytes * 100)) / totalLogBytes);
    ...
    long dataSize = (realTotalDataCount == 0) ? 0 : (realTotalDataBytes / realTotalDataCount);
    double dataPct =
        (totalLogBytes == 0) ? 0 : (((double) (realTotalDataBytes * 100))) / totalLogBytes;
  ...
  private void summarizeCheckpointInfo(CheckpointInfoTextFormatter f) {
    ...
    while (iter.hasNext()) {
      ...
      if (prevCounter == null) {
        endToEndDistance = DbLsn.getWithCleaningDistance(end, firstLsnRead, logFileMax, fileMgr);
      } else {
        endToEndDistance =
            DbLsn.getWithCleaningDistance(end, prevCounter.endCkptLsn, logFileMax, fileMgr);
      }
      ...
      if (prevCounter == null) {
        endToStartDistance =
            DbLsn.getWithCleaningDistance(start, firstLsnRead, logFileMax, fileMgr);
      } else {
        endToStartDistance =
            DbLsn.getWithCleaningDistance(start, prevCounter.endCkptLsn, logFileMax, fileMgr);
      }
      ...
      if ((c.startCkptLsn != DbLsn.NULL_LSN) && (c.endCkptLsn != DbLsn.NULL_LSN)) {
        startToEndDistance =
            DbLsn.getWithCleaningDistance(c.endCkptLsn, c.startCkptLsn, logFileMax, fileMgr);
      }
