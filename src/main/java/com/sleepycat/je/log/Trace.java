...
/**
 * Trace logs event tracing messages into .jdb files. Only critical messages that should always be
 * included in a log should use this functionality.
 */
public class Trace extends BasicVersionedWriteLoggable {
  ...
  public static long trace(EnvironmentImpl envImpl, Trace traceMsg) {
    if (isWritePermitted(envImpl)) {
      return envImpl
          .getLogManager()
          .log(new TraceLogEntry(traceMsg), ReplicationContext.NO_REPLICATE);
    }
  ...
  /**
   * Convenience method to create a log entry (lazily) containing this trace msg. Lazy tracing is
   * used when tracing is desired, but the .jdb files are not initialized.
   */
  public static void traceLazily(EnvironmentImpl envImpl, String message) {
    if (isWritePermitted(envImpl)) {
      envImpl
          .getLogManager()
          .logLazily(new TraceLogEntry(new Trace(message)), ReplicationContext.NO_REPLICATE);
    }
  }
