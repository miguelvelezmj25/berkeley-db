...
/**
 * LNFileReader scans log files for LNs. Also, if it's going backwards for the undo phase in
 * recovery, it reads transaction commit entries.
 */
public class LNFileReader extends FileReader {
   ...
   * @param redo If true, we're going to go forward from the start LSN to the end of the log. If
   *     false, we're going backwards from the end of the log to the start LSN.
   * @param finishLsn the last LSN to read in the log. May be null if we want to read to the end of
   *     the log.
   * @param endOfFileLsn the virtual LSN that marks the end of the log. (The one off the end of the
   *     log). Only used if we're reading backwards. Different from the startLsn because the
   *     startLsn tells us where the beginning of the start entry is, but not the length/end of the
   *     start entry. May be null if we're going foward.
   */
  public LNFileReader(
      EnvironmentImpl env,
      int readBufferSize,
  ...
  /**
   * Get the last LN log entry seen by the reader. Note that LNLogEntry.postFetchInit must be called
   * before calling certain LNLogEntry methods.
   */
