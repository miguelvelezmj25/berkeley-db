...
/**
 * The configuration properties of a <code>SecondaryDatabase</code> extend those of a primary <code>
 * Database</code>. The secondary database configuration is specified when calling {@link
 * Environment#openSecondaryDatabase Environment.openSecondaryDatabase}.
 *
 * <p>To create a configuration object with default attributes:
 *
 * <pre>
 *     SecondaryConfig config = new SecondaryConfig();
 * </pre>
 *
 * <p>To set custom attributes:
 *
 * <pre>
 *     SecondaryConfig config = new SecondaryConfig();
...
public class SecondaryConfig extends DatabaseConfig {
   ...
   *
   * <p>Unless the primary database is read-only, a key creator is required when opening a secondary
   * database. Either a KeyCreator or MultiKeyCreator must be specified, but both may not be
   * specified.
   *
   * <p>Unless the primary database is read-only, a key creator is required when opening a secondary
   * database.
   *
   * <p><em>WARNING:</em> Key creator instances are shared by multiple threads and key creator
   * methods are called without any special synchronization. Therefore, key creators must be thread
   * safe. In general no shared state should be used and any caching of computed values must be done
   * with proper synchronization.
   *
   * @param keyCreator the user-supplied object used for creating single-valued secondary keys.
   * @return this
   */
   ...
   * with proper synchronization.
   *
   * @param multiKeyCreator the user-supplied object used for creating multi-valued secondary keys.
   * @return this
   */
   ...
   *
   * <p>If automatic population is allowed, when the secondary database is opened it is checked to
   * see if it is empty. If it is empty, the primary database is read in its entirety and keys are
   * added to the secondary database using the information read from the primary.
   *
   * <p>If this property is set to true and the database is transactional, the population of the
   * secondary will be done within the explicit or auto-commit transaction that is used to open the
   * database.
   *
  ...
  /**
   * Returns whether automatic population of the secondary is allowed. If {@link #setAllowPopulate}
   * has not been called, this method returns false.
   *
   ...
   *
   * <p>If this property is non-null, a record must be present in the specified foreign database for
   * every record in the secondary database, where the secondary key value is equal to the foreign
   * database key value. Whenever a record is to be added to the secondary database, the secondary
   * key is used as a lookup key in the foreign database. If the key is not found in the foreign
   * database, a {@link ForeignConstraintException} is thrown.
   *
   * <p>The foreign database must not have duplicates allowed. If duplicates are allowed, an
   * IllegalArgumentException will be thrown when the secondary database is opened.
   *
   * @param foreignKeyDatabase the database used to check the foreign key integrity constraint, or
   *     null if no foreign key constraint should be checked.
   * @return this
   */
  ...
  /**
   * Returns the database used to check the foreign key integrity constraint, or null if no foreign
   * key constraint will be checked.
   *
   ...
   *
   * <p>This property is ignored if the foreign key database property is null.
   *
   * @param foreignKeyDeleteAction the action taken when a referenced record in the foreign key
   *     database is deleted.
   ...
   *
   * <p>This method may <em>not</em> be used along with {@link #setMultiKeyCreator}. When using a
   * multi-key creator, use {@link #setForeignMultiKeyNullifier} instead.
   *
   * <p>If the foreign key database property is non-null and the foreign key delete action is <code>
   * NULLIFY</code>, this property is required to be non-null; otherwise, this property is ignored.
   *
   * <p><em>WARNING:</em> Key nullifier instances are shared by multiple threads and key nullifier
   * methods are called without any special synchronization. Therefore, key creators must be thread
   * safe. In general no shared state should be used and any caching of computed values must be done
   * with proper synchronization.
   *
   * @param foreignKeyNullifier the user-supplied object used for setting single-valued foreign keys
   *     to null.
   * @see ForeignKeyNullifier @see ForeignKeyDeleteAction#NULLIFY @see #setForeignKeyDatabase
   * @return this
   */
   ...
   *
   * @return the user-supplied object used for setting single-valued foreign keys to null.
   ...
   * with proper synchronization.
   *
   * @param foreignMultiKeyNullifier the user-supplied object used for setting multi-valued foreign
   *     keys to null.
   * @see ForeignMultiKeyNullifier @see ForeignKeyDeleteAction#NULLIFY @see #setForeignKeyDatabase
   * @return this
   */
   ...
   *
   * @return the user-supplied object used for setting multi-valued foreign keys to null.
   ...
   *     <p>Specifies whether the key extractor/creator will only use the primary key.
   *     <p>Specifying that only the primary key is needed can be used to optimize primary database
   *     updates and deletions. If a primary record is updated or deleted, and all associated
   *     secondaries have this property set to true, then the existing primary record will not be
   *     read. This potentially saves an I/O. When this property is <em>not</em> set to true (it is
   *     false by default), the existing primary record must be read if it is not already in cache,
   *     order to pass the primary data to the key extractor/creator.
   *     <p>Note that if this property is true, either null or a non-null value may be passed to the
   *     key extractor/creator for the primary data parameter. The key extractor/creator is expected
   *     to ignore this parameter.
   *     <p>
   * @param extractFromPrimaryKeyOnly whether the key extractor/creator will only use the primary
   *     key.
   * @return this
   */
  public SecondaryConfig setExtractFromPrimaryKeyOnly(boolean extractFromPrimaryKeyOnly) {
  ...
  /** @hidden The void return setter for use by Bean editors. */
  public void setExtractFromPrimaryKeyOnlyVoid(boolean extractFromPrimaryKeyOnly) {
  ...
  /**
   * @hidden For internal use only.
   *     <p>Returns whether the key extractor/creator will only use the primary key. If {@link
   *     #setExtractFromPrimaryKeyOnly} has not been called, this method returns false.
   ...
   *
   * <p>Specifying that a secondary key is immutable can be used to optimize updates when the
   * secondary key in a primary record will never be changed after that primary record is inserted.
   * For immutable secondary keys, a best effort is made to avoid calling <code>
   * SecondaryKeyCreator.createSecondaryKey</code> when a primary record is updated. This
   * optimization may reduce the overhead of an update operation significantly if the <code>
   * createSecondaryKey</code> operation is expensive.
   *
   * <p>Be sure to set this property to true only if the secondary key in the primary record is
   * never changed. If this rule is violated, the secondary index will become corrupted, that is, it
   * will become out of sync with the primary.
   *
  ...
  /**
   * Returns whether the secondary key is immutable. If {@link #setImmutableSecondaryKey} has not
   * been called, this method returns false.
   *
  ...
  void validate(DatabaseConfig configArg) throws DatabaseException {
    ...
    boolean fmknMatch =
        equalOrBothNull(config.getForeignMultiKeyNullifier(), foreignMultiKeyNullifier);
    ...
    String message =
        genSecondaryConfigMismatchMessage(
            config, kcMatch, mkcMatch, fkdMatch, fkdaMatch, fknMatch, fmknMatch, imskMatch);
  ...
  /** @hidden For internal use only. */
  @Override
  public DatabaseConfig setOverrideTriggers(@SuppressWarnings("unused") boolean override) {
    throw new IllegalArgumentException("Triggers may only be associated with a Primary database");
  }
