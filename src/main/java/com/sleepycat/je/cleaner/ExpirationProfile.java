...
/**
 * A cache of the histograms for all files, except for the last file. Also caches the number of
 * bytes expired in the current interval.
 *
 * <p>No memory budgeting is performed because the size of these data structures is small compared
 * to the Btree they represent. The serialized form of the histogram is cached, which is a small
 * number of bytes per file. If no data in a file expires, it will not have a cache entry.
 *
 * <p>Possible future optimization: If there is contention on this data structure, the refresh
 * method could create a read-only map containing the current number of expired bytes, for access by
 * getExpiredBytes without synchronization.
 */
public class ExpirationProfile {

  private static final Pair<Integer, Integer> PAIR_OF_ZEROS = new Pair<>(0, 0);
  ...
  /**
   * Called at the end of recovery to open the expiration DB, and cache its records in the profile's
   * map.
   *
   * <p>Also collects expiration info for any complete file having expiration info that was not
   * written to the DB earlier, due to a crash for example.
   *
   * <p>Also initializes the tracker for the current file in the log manager, reading/tracking the
   * existing entries in that file.
   */
  public void populateCache(
      final StartupTracker.Counter counter, final ProgressListener<RecoveryProgress> listener) {
  ...
  /**
   * Writes a record in the expiration summary DB for the given tracker, and (if there is any data
   * with an expiration time) adds it to the map.
   *
   * <p>Because this method and {@link #removeFile} perform Btree operations while synchronized on
   * the 'map', it is important that an IN is not latched when calling these methods, which could
   * cause a deadlock. Also, an IN latch should not be held while calling a method that synchronizes
   * on {@link FileSelector}, since methods in this class are called while synchronized on
   * FileSelector, and this could cause a 3-way deadlock [#25613].
   */
  ...
  /**
   * Called after a file flip. The tracker is completed in the sense that the file is completely
   * written, but there may be pending calls to {@link ExpirationTracker#track(int, boolean, int)}
   * for some writing threads. This is because track is not called under the LWL.
   */
  ...
  /**
   * Periodically, and when refreshing the profile, we process completed trackers that were added to
   * the completedTrackers queue at the time of a file flip. If truly complete, they are added to
   * the profile DB and map.
   */
  ...
  /**
   * Updates the expired bytes in the expiration profile according to the data that has expired at
   * the given time. Should be called periodically, and before calling {@link #getExpiredBytes}.
   *
   * <p>Also processes any completed trackers by adding them to the DB and to the histogram map.
   *
   * <p>This method only does any real work once per hour, on hour boundaries, since data expires on
   * (at most) hour boundaries.
   */
  public void refresh(final long time) {
    ...
    synchronized (map) {
      ...
      for (final ExpInfo info : map.values()) {
        ...
        info.currentExpiredBytes =
            ExpirationTracker.getExpiredBytes(info.serializedForm, dayLimit, hourLimit);
      }
    }
  }

  /**
   * Returns the number of expired bytes for the given file. Uses the value calculated by the last
  ...
  /**
   * Returns the number of expired bytes for the given file. Two values are returned: the total
   * expired at the given time, and a potentially smaller amount that gradually expires over the
   * current hour or day interval. Uses the values calculated by the last call to {@link #refresh}.
   *
   * <p>The amount that gradually expires is the amount that expires in the current time interval,
   * which is one day if all data in the profile expires on day boundaries, and otherwise is one
   * hour. If this is the first interval for the file (after a restart or a revisal run), all
   * expired bytes are considered expired in the current interval and expire gradually.
   *
  ...
  public Pair<Integer, Integer> getExpiredBytes(final long fileNum, final long time) {
    ...
    synchronized (map) {
      ...
      final int gradualBytes =
          info.previousExpiredBytes + ((int) ((newlyExpiredBytes * currentMs) / intervalMs));
  ...
  private static class ExpInfo {
    ...
    /**
     * Cached serialized form, use to recompute the current expired bytes for a given expiration
     * time.
     */
    final byte[] serializedForm;

    /**
     * The number of expired bytes for the given file. This is the value calculated by the last call
     * to {@link ExpirationProfile#refresh}.
     */
    int currentExpiredBytes = 0;

    /**
     * The number of bytes that expired prior to the current interval. Calculated by the previous
     * refresh for which the interval changed.
     */
