...
/**
 * Iterator over files that should be migrated by cleaning them, even if they don't need to be
 * cleaned for other reasons.
 *
 * <p>Files are migrated either because they are named in the CLEANER_FORCE_CLEAN_FILES parameter or
 * their log version is prior to the CLEANER_UPGRADE_TO_LOG_VERSION parameter.
 *
 * <p>An iterator is used rather than finding the entire set at startup to avoid opening a large
 * number of files to examine their log version. For example, if all files are being migrated in a
 * very large data set, this would involve opening a very large number of files in order to read
 * their header. This could significantly delay application startup.
 *
 * <p>Because we don't have the entire set at startup, we can't select the lowest utilization file
 * from the set to clean next. Inteaad we iterate in file number order to increase the odds of
 * cleaning lower utilization files first.
 */
class FilesToMigrate {
  ...
  /**
   * An array of pairs of file numbers, where each pair is a range of files to be force cleaned.
   * Index i is the from value and i+1 is the to value, both inclusive.
   */
  ...
  FilesToMigrate(EnvironmentImpl env) {
    ...
    upgradeToVersion =
        env.getConfigManager().getInt(EnvironmentParams.CLEANER_UPGRADE_TO_LOG_VERSION);
  ...
  /**
   * Returns whether there are more files to be migrated. Must be called while synchronized on the
   * UtilizationProfile.
   */
  boolean hasNext(SortedMap<Long, FileSummary> fileSummaryMap, Set<Long> inProgressFiles) {
    if (nextAvailable) {
  ...
  /**
   * Returns the next file file to be migrated. Must be called while synchronized on the
   * UtilizationProfile.
   */
  long next(SortedMap<Long, FileSummary> fileSummaryMap, Set<Long> inProgressFiles)
  ...
  private void parseForceCleanFiles(String propValue) throws IllegalArgumentException {
    ...
    if (propValue == null || propValue.length() == 0) {
      ...
      StringTokenizer tokens = new StringTokenizer(propValue, ",-", true /*returnDelims*/);
