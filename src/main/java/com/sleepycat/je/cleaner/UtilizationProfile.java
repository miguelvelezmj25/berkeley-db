 ...
 *
 * <p>Unlike the UtilizationTracker, the UP is not accessed under the log write latch and is instead
 * synchronized on itself for protecting the cache. It is not accessed during the primary data
 * access path, except for when flushing (writing) file summary LNs. This occurs in the following
 * cases:
 *
 * <ol>
 *   <li>The summary information is flushed at the end of a checkpoint. This allows tracking to
 *       occur in memory in between checkpoints, and replayed during recovery.
 *   <li>When committing the truncateDatabase and removeDatabase operations, the summary information
 *       is flushed because detail tracking for those operations is not replayed during recovery
 *   <li>The evictor will ask the UtilizationTracker to flush the largest summary if the memory
 *       taken by the tracker exeeds its budget.
 * </ol>
 *
 * <p>The cache is populated by the RecoveryManager just before performing the initial checkpoint.
 * The UP must be open and populated in order to respond to requests to flush summaries and to evict
 * tracked detail, even if the cleaner is disabled.
 *
 * <p>WARNING: While synchronized on this object, eviction is not permitted. If it were, this could
 * cause deadlocks because the order of locking would be the UP object and then the evictor. During
 * normal eviction the order is to first lock the evictor and then the UP, when evicting tracked
 * detail.
 *
 * <p>The methods in this class synchronize to protect the cached summary information. Some methods
 * also access the UP database. However, because eviction must not occur while synchronized, UP
 * database access is not performed while synchronized except in one case: when inserting a new
 * summary record. In that case we disallow eviction during the database operation.
 */
public class UtilizationProfile {
  ...
  /**
   * Gets the base summary from the cached map. Add the tracked summary, if one exists, to the base
   * summary. Sets all entries obsolete, if the file is in the migrateFiles set.
   */
  ...
  /** Flush a MapLN for each database that has dirty utilization in the given tracker. */
  ...
  /**
   * Returns a copy of the current file summary map, optionally including tracked summary
   * information, for use by the DbSpace utility and by unit tests. The returned map's key is a Long
   * file number and its value is a FileSummary.
   */
  ...
  /** Gets the size of the file. If the file does not exist in fileSummaryMap, then return -1. */
  ...
  /**
   * Returns a simplified copy of the current file summary map, i.e. the value is only the total
   * size of the file.
   *
   * <p>Besides, we are not sure whether the FileSummary for the current last file is complete, so
   * we remove the entry for the last file.
   */
  ...
  /**
   * Clears the cache of file summary info. The cache is not automatically repopulated, so this
   * method should currently be called only by close.
   */
  ...
  /**
   * Updates the reserved file db and file summary db after removing a set of files from the
   * FileSelector and changing their status to reserved in the FileProtector. 1. Insert reserved
   * file record for each file. 2. Remove files from database (MapLN) metadata. 3. Delete file
   * summary db records for each file.
   *
   * <p>See populateCache for how a crash during these steps is handled.
   */
  void reserveFiles(final Map<Long, FileSelector.FileInfo> reservedFiles) {

    final Set<DatabaseId> dbIds = new HashSet<>();

    for (final Map.Entry<Long, FileSelector.FileInfo> entry : reservedFiles.entrySet()) {
  ...
  /** Stores a reserved file db record for a file that has been cleaned and is ready-to-deleted. */
  private void putReservedFileRecord(final Long file, final FileSelector.FileInfo fsInfo) {

    final DatabaseEntry keyEntry = new DatabaseEntry();
    ReservedFileInfo.keyToEntry(file, keyEntry);

    final ReservedFileInfo rfInfo =
        new ReservedFileInfo(fsInfo.firstVlsn, fsInfo.lastVlsn, fsInfo.dbIds);
    ...
    try {
      try (Cursor cursor =
          DbInternal.makeCursor(reservedFilesDb, locker, null, false /*retainNonTxnLocks*/)) {
  ...
  void deleteReservedFileRecord(final Long file) {
    ...
    final ReadOptions readOptions = new ReadOptions().setLockMode(LockMode.RMW);

    final Locker locker = BasicLocker.createBasicLocker(env, false /*noWait*/);
    try {
      try (Cursor cursor = DbInternal.makeCursor(reservedFilesDb, locker, null)) {
  ...
  /**
   * Removes a file from the MapLN utilization info, the utilization database and the profile, after
   * it has been determined that the file does not exist. This is unusual, so the inefficiency of
   * removeAndFlushDbMetadata is acceptable.
   */
  ...
  /**
   * Remove newly reserved files from the utilization profile cache. This is called while
   * synchronized on the FileSelector to prevent a window where the file could be selected for
   * cleaning again.
   *
   * <p>This method does not delete the utilization db records.
   */
  ...
  /**
   * Removes a set of files from the DatabaseImpl utilization info, dirtying the MapLNs in the
   * process. The MapLNs will be flushed by the checkpointer. This method performs eviction and is
   * not synchronized.
   *
   * <p>For a given file, this method must be called after inserting its reserved db record. If
   * there is a crash and recovery finds the reserved db record in the checkpointer interval,
   * recovery will redo this action.
   *
   * <p>This method is optimally called for multiple files at a time, since this updates the MapLNs
   * more efficiently than if it were done one file at a time.
   */
  private boolean removeDbMetadata(
      final Collection<Long> fileNums, final Set<DatabaseId> databases) {
  ...
  /**
   * Like removeDbMetadata but operates on a single file and also flushes the updated MapLNs
   * immediately. This is very inefficient and is meant only for infrequent cleanup.
   */
  private void removeAndFlushDbMetadata(Long fileNum) throws DatabaseException {
    ...
    boolean logRoot = logManager.removeDbFileSummaries(idDatabase, fileNums);
  ...
  /**
   * Deletes all FileSummaryLNs for the file. This method performs eviction and is not synchronized.
   *
   * <p>For a given file, this method should be called after calling removeDbMetadata as described
   * in {@link #populateCache}.
   */
  ...
  /**
   * Updates and stores the FileSummary for a given tracked file, if flushing of the summary is
   * allowed.
   */
  ...
  /**
   * Updates and stores the FileSummary for a given tracked file. This method is synchronized and
   * may not perform eviction.
   */
   ...
   *
   * @param logUpdate if true, log any updates to the utilization profile. If false, only retrieve
   *     the new information.
   */
  PackedOffsets getObsoleteDetailPacked(Long fileNum, boolean logUpdate) throws DatabaseException {
  ...
  /**
   * Populate the profile for file selection. This method performs eviction and is not synchronized.
   * It must be called before recovery is complete so that synchronization is unnecessary. It should
   * be called before the recovery checkpoint so that the checkpoint can flush dirty metadata.
   *
   * <p>After a file is cleaned, at checkpoint end it is moved to reserved status and these steps
   * are taken to delete its metadata: 1. The file is cleaned and a checkpoint flushes the updated
   * INs. 2. Utilization info in MapLNs referencing the file are updated in cache and the MapLNs are
   * dirtied. At some point later, the dirty MapLNs are flushed by a normally scheduled checkpoint.
   * This could happen after any of the steps below. 3. A record is inserted for the file in the
   * reserved file db. 4. All file summary db records for the file are deleted.
   *
   * <p>When the reserved file is deleted, this is done in these steps. 5. The file itself is
   * deleted. Note that because we do not flush the log before deleting the file, step 3 and 4 may
   * not be durable. 6. The files's record in the reserved file db is deleted.
   *
   * <p>Data file deletion and file metadata deletion cannot be performed atomically. In fact none
   * of the steps are grouped atomically into transactions, and no-sync logging is used when writing
   * to the file summary db and the reserved file db. A crash can occur at any point, and we must
   * handle that in some way, either in recovery or here (in the populateCache method).
   *
   * <p>- A crash prior to durable completion of step 3 will be handled naturally by cleaning the
   * file again, since it is present in the file summary db but not in the reserved file db.
   *
   * <p>- If step 3 completes durably but a crash occurs before the dirty MapLNs are flushed, this
   * is indicated by the presence of a reserved file record in the recovery interval, and step 2
   * will be repeated here during recovery.
   *
   * <p>- If step 3 completes durably but not step 4 or 5, then step 4 is completed here.
   *
   * <p>- If step 3 and 5 complete durably but not step 4 and 6, then steps 4 and 6 are completed
   * here.
   *
   * <p>- If step 4 and 5 complete durably but not step 6, then step 6 is completed here.
   *
   * <p>Prior to log version 15, the reserved file db did not exist and reserved files were
   * re-cleaned if they were not deleted before a crash. The old steps were: A. The file is cleaned
   * and a checkpoint flushes the updated INs. B. If the file is reserved, this is tracked in memory
   * but nothing is logged.
   *
   * <p>When deleting a file: C. The file itself is deleted. D. MapLNs are updated and flushed. E.
   * All file summary db records for the file are deleted.
   *
   * <p>This is handled now as follows, which is the same as the old approach.
   *
   * <p>- A crash prior to step C results in re-cleaning the file.
   *
   * <p>- If step C completes but step E does not, step D and E are performed here. MapLNs must be
   * flushed immediately in this case, which is inefficient but hopefully very infrequent.
   */
  public boolean populateCache(
     ...
     */
    final boolean forceCheckpoint =
        removeDbMetadata(recoveryReservedFiles, recoveryReservedFileDbs);
    ...
    if (openReservedFilesDatabase()) {
      ...
      final Locker locker = BasicLocker.createBasicLocker(env, false /*noWait*/);
      try {
        final ReadOptions options = new ReadOptions().setLockMode(LockMode.READ_UNCOMMITTED);
        ...
        try (final Cursor cursor =
          ...
          while (cursor.get(keyEntry, dataEntry, Get.NEXT, options) != null) {
            ...
            final Long file = ReservedFileInfo.entryToKey(keyEntry);

            reservedFileRecords.add(file);

            final ReservedFileInfo info = ReservedFileInfo.entryToObject(dataEntry);
    ...
    final int oldMemorySize = fileSummaryMap.size() * MemoryBudget.UTILIZATION_PROFILE_ENTRY;
    ...
    try {
      ...
      cursor =
          new CursorImpl(
              fileSummaryDb, locker, false /*retainNonTxnLocks*/, false /*isSecondaryCursor*/);
      ...
      if (cursor.positionFirstOrLast(true)) {
        ...
        while (result != null) {
          ...
          final boolean isOldVersion = FileSummaryLN.hasStringKey(keyBytes);
      ...
      final int newMemorySize = fileSummaryMap.size() * MemoryBudget.UTILIZATION_PROFILE_ENTRY;
  ...
  /**
   * If the reserved files db is already open, return, otherwise attempt to open it. If the
   * environment is read-only and the database doesn't exist, return false. If the environment is
   * read-write the database will be created if it doesn't exist.
   */
  private boolean openReservedFilesDatabase() throws DatabaseException {
    ...
    reservedFilesDb = env.getDbTree().openNonRepInternalDB(DbType.RESERVED_FILES);

    return (reservedFilesDb != null);
  }

  /**
   * If the file summary db is already open, return, otherwise attempt to open it. If the
   * environment is read-only and the database doesn't exist, return false. If the environment is
   * read-write the database will be created if it doesn't exist.
   */
  ...
  /**
   * Insert the given LN with the given key values. This method is synchronized and may not perform
   * eviction.
   *
   * <p>Is public only for unit testing.
   */
  synchronized boolean insertFileSummary(FileSummaryLN ln, long fileNum, int sequence)
      throws DatabaseException {
    ...
    try {
      ...
      boolean inserted =
          cursor.insertRecord(
              keyBytes, ln, false /*blindInsertion*/, ReplicationContext.NO_REPLICATE);
  ...
  /**
   * Checks that all FSLN offsets are indeed obsolete. Assumes that the system is quiesent (does not
   * lock LNs). This method is not synchronized (because it doesn't access fileSummaryMap) and
   * eviction is allowed.
   *
