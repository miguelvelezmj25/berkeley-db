...
/**
 * Reads all entries in a log file and either determines them to be obsolete or active. Active LNs
 * are migrated immediately (by logging them). Active INs are marked for migration by setting the
 * dirty flag.
 *
 * <p>May be invoked explicitly by calling doClean, or woken up if used as a daemon thread.
 */
public class FileProcessor extends DaemonThread {

  /**
   * The number of LN log entries after we process pending LNs. If we do this too seldom, the
   * pending LN queue may grow large, and it isn't budgeted memory. If we process it too often, we
   * will repeatedly request a non-blocking lock for the same locked node.
   */
  ...
  /**
   * The thread is woken, either by an explicit notify (call to {@link Cleaner#wakeupActivate()}, or
   * when the timed wakeup interval elapses.
   *
   * <p>In the former case (a call to wakeupActivate), the 'activate' field will be true and the
   * doClean method is called here. This happens when the number of bytes written exceeds the
   * cleanerBytesInterval, a config change is made that could impact cleaning, etc.
   *
   * <p>In the latter case (the wakeup interval elapsed), 'activate' will be false. In this case,
   * when there has been no writing since the last wakeup, we perform cleaning and checkpointing, if
   * needed to reclaim space. This handles the situation where writing stops, but cleaning or
   * checkpointing or reserved file deletion may be needed. See {@link
   * com.sleepycat.je.EnvironmentConfig#CLEANER_WAKEUP_INTERVAL}.
   *
   * <p>In all cases, when a disk limit is in violation we always call the doClean method to ensure
   * that {@link Cleaner#manageDiskUsage()} is called in this situation. This is important to free
   * disk space whenever possible.
   */
  @Override
  protected synchronized void onWakeup() {
    ...
    doClean(true /*invokedFromDaemon*/, true /*cleanMultipleFiles*/, false /*forceCleaning*/);

    activate = false;
  }

  /**
   * Selects files to clean and cleans them. It returns the number of successfully cleaned files.
   * May be called by the daemon thread or programatically.
   *
  ...
  synchronized int doClean(
      boolean invokedFromDaemon, boolean cleanMultipleFiles, boolean forceCleaning) {
    ...
    while (true) {
      ...
      final Pair<Long, Integer> result =
          fileSelector.selectFileForCleaning(calculator, fileSummaryMap, forceCleaning);
      ...
      try {
        ...
        if (twoPass) {
          ...
          final ExpirationTracker expTracker = new ExpirationTracker(fileNumValue);

          processFile(fileNum, recalcSummary, new INSummary(), expTracker);

          final int expiredSize = expTracker.getExpiredBytes(TTL.currentSystemTime());

          final int obsoleteSize = recalcSummary.getObsoleteSize();

          final int recalcUtil =
              FileSummary.utilization(obsoleteSize + expiredSize, recalcSummary.totalSize);
        ...
        final FileSummary estimatedFileSummary =
            fileSummaryMap.containsKey(fileNum) ? fileSummaryMap.get(fileNum).clone() : null;
        ...
        if (!processFile(fileNum, recalculatedFileSummary, inSummary, null)) {
        ...
        if (logger.isLoggable(Level.INFO)) {
          ...
          final int estUtil =
              (estimatedFileSummary != null) ? estimatedFileSummary.utilization() : -1;

          final int recalcUtil = recalculatedFileSummary.utilization();
        ...
        LoggerUtils.logMsg(
            ...
            envImpl,
            Level.INFO,
            "Missing file 0x" + Long.toHexString(fileNum) + " ignored by cleaner");

      } catch (IOException e) {

        LoggerUtils.traceAndLogException(envImpl, "Cleaner", "doClean", "", e);
  ...
  /**
   * Calculates expired bytes without performing any migration or other side effects. The expired
   * sizes will not overlap with obsolete data, because expired sizes are accumulated only for
  ...
  public ExpirationTracker countExpiration(long fileNum) {
    ...
    try {
      final boolean result = processFile(fileNum, new FileSummary(), new INSummary(), tracker);

      assert result;

    } catch (IOException e) {

      LoggerUtils.traceAndLogException(envImpl, "Cleaner", "countExpiration", "", e);
   ...
   *
   * <p>Note that we gather obsolete offsets at the beginning of the method and do not check for
   * obsolete offsets of entries that become obsolete while the file is being processed. An entry in
   * this file can become obsolete before we process it when normal application activity deletes or
   * updates the entry. Also, large numbers of entries also become obsolete as the result of LN
   * migration while processing the file, but these Checking the TrackedFileSummary while processing
   * the file would be expensive if it has many entries, because we perform a linear search in the
   * TFS. There is a tradeoff between the cost of the TFS lookup and its benefit, which is to avoid
   * a tree search if the entry is obsolete. Many more lookups for non-obsolete entries than
   * obsolete entries will typically be done. Because of the high cost of the linear search,
   * especially when processing large log files, we do not check the TFS. [#19626]
   *
   ...
   * @param expTracker if non-null, enables countOnly mode, in which expiration info is returned via
   *     this param, obsolete info returned via fileSummary does not include expired data, and no
   *     migration is performed, i.e., there are no side effects.
   * @return false if we aborted file processing because the environment is being closed.
   */
  private boolean processFile(
      Long fileNum, FileSummary fileSummary, INSummary inSummary, ExpirationTracker expTracker)
      throws IOException {
    ...
    final int lookAheadCacheSize = countOnly ? 0 : cleaner.lookAheadCacheSize;
     ...
     */
    final int adjustMem = (2 * readBufferSize) + obsoleteOffsets.getLogSize() + lookAheadCacheSize;
    final MemoryBudget budget = envImpl.getMemoryBudget();
     ...
     */
    final LookAheadCache lookAheadCache = countOnly ? null : new LookAheadCache(lookAheadCacheSize);
    ...
    final CleanerFileReader reader =
        new CleanerFileReader(
            envImpl,
            readBufferSize,
    ...
    try {
      ...
      while (reader.readNextEntryAllowExceptions()) {
         ...
         * [#21405]
         */
        if (!isObsolete && isOldBINDelta && fileLogVersion < 8) {
          isObsolete = true;
        }
         ...
         */
        if (!isObsolete && isIN && db.getSortedDuplicates() && fileLogVersion < 8) {
          isObsolete = true;
        }
        ...
        if (!isObsolete && isLN) {
          ...
          if (!isObsolete && (db.isLNImmediatelyObsolete() || lnEntry.isEmbeddedLN())) {
            isObsolete = true;
          }
          ...
          if (!isObsolete && !countOnly) {
            ...
            expirationTime =
                TTL.expirationToSystemTime(lnEntry.getExpiration(), lnEntry.isExpirationInHours());

            if (envImpl.expiresWithin(expirationTime, 0 - envImpl.getTtlLnPurgeDelay())) {

              if (!lockManager.isLockUncontended(logLsn)) {
                fileSelector.addPendingLN(
                    logLsn, new LNInfo(null /*LN*/, dbId, lnEntry.getKey(), expirationTime));
                nLNsLockedThisRun++;
                continue;
              }
              ...
              localTracker.countObsoleteNodeInexact(
                  logLsn, null /*type*/, reader.getLastEntrySize(), db);
            }
          }
        }
        ...
        if (isLN) {
          ...
          if (lookAheadCache.isFull()) {
            processLN(fileNum, location, lookAheadCache, dbCache, checkPendingDbSet);
          }
      ...
      for (final DatabaseId pendingDbId : checkPendingDbSet) {
        final DatabaseImpl db = dbMapTree.getDb(pendingDbId, cleaner.lockTimeout, dbCache);
    ...
    fileSelector.addCleanedFile(
        fileNum, databases, reader.getFirstVLSN(), reader.getLastVLSN(), budget);

    return true;
  }

  /**
   * Processes the first LN in the look ahead cache and removes it from the cache. While the BIN is
   * latched, look through the BIN for other LNs in the cache; if any match, process them to avoid a
   * tree search later.
   */
  private void processLN(
    ...
    final DatabaseImpl db = envImpl.getDbTree().getDb(dbId, cleaner.lockTimeout, dbCache);
    ...
    try {
      ...
      LNInfo pendingLN = processFoundLN(info, logLsn, bin.getLsn(index), bin, index);
      ...
      for (int i = 0; i < bin.getNEntries(); i += 1) {
        ...
        if (i != index
          ...
          if (myInfo != null) {
            ...
            pendingLN = processFoundLN(myInfo, binLsn, binLsn, bin, i);
      ...
      if (pendingLNs != null) {
        for (Map.Entry<Long, LNInfo> entry : pendingLNs.entrySet()) {
          fileSelector.addPendingLN(entry.getKey(), entry.getValue());
        }
      }

      if (processedHere) {
        cleaner.logFine(
            Cleaner.CLEAN_LN, lnFromLog, logLsn, completed, obsolete, false /*migrated*/);
      }
    }
  }

  /**
   * Processes an LN that was found in the tree. Lock the LN's LSN and then migrates the LN, if the
   * LSN of the LN log entry is the active LSN in the tree.
   *
   * @param info identifies the LN log entry.
   * @param logLsn is the LSN of the log entry.
   * @param treeLsn is the LSN found in the tree.
   * @param bin is the BIN found in the tree; is latched on method entry and exit.
   * @param index is the BIN index found in the tree.
   * @return a non-null LNInfo if it should be added to the pending LN list, after releasing the BIN
   *     latch.
   */
  private LNInfo processFoundLN(
      final LNInfo info, final long logLsn, final long treeLsn, final BIN bin, final int index) {
    ...
    try {
       ...
       * of that record.
       */
      if (lnFromLog.isDeleted() && treeLsn == logLsn && fileLogVersion <= 2) {
      ...
      if (!isTemporary) {
        ...
        if (lockRet.getLockGrant() == LockGrantType.DENIED) {
          ...
          return new LNInfo(null /*LN*/, db.getId(), key, info.getExpirationTime());
        }
      ...
      if (db.getId().equals(DbTree.ID_DB_ID)) {
        ...
        bin.updateEntry(index, logItem.lsn, targetLn.getVLSNSequence(), logItem.size);
        ...
        CursorImpl.lockAfterLsnChange(db, logLsn, logItem.lsn, locker /*excludeLocker*/);
      ...
      cleaner.logFine(Cleaner.CLEAN_LN, lnFromLog, logLsn, completed, obsolete, migrated);
    }
  }

  /**
   * If this OldBINDelta is still in use in the in-memory tree, dirty the associated BIN. The next
   * checkpoint will log a new delta or a full version, which will make this delta obsolete.
   *
   * <p>For OldBINDeltas, we do not optimize and must fetch the BIN if it is not resident.
   */
  private void processOldBINDelta(OldBINDelta deltaClone, DatabaseImpl db, long logLsn) {
    ...
    final BIN treeBin = db.getTree().search(searchKey, Cleaner.UPDATE_GENERATION);
  ...
  /**
   * If this BIN-delta is still in use in the in-memory tree, dirty the associated BIN. The next
   * checkpoint will log a new delta or a full version, which will make this delta obsolete.
   *
   * <p>We optimize by placing the delta from the log into the tree when the BIN is not resident.
   */
  private void processBINDelta(BIN deltaClone, DatabaseImpl db, long logLsn) {
    ...
    final SearchResult result =
        db.getTree()
            .getParentINForChildIN(
                deltaClone, true /*useTargetLevel*/, true /*doFetch*/, CacheMode.UNCHANGED);
    ...
    try {
       ...
       */
      BIN treeBin = (BIN) result.parent.loadIN(result.index, CacheMode.UNCHANGED);
      ...
      if (treeBin == null) {
        ...
        result.parent.attachNode(result.index, treeBin, null /*lnSlotKey*/);
      } else {
  ...
  /**
   * If an IN is still in use in the in-memory tree, dirty it. The checkpoint invoked at the end of
   * the cleaning run will end up rewriting it.
   */
  private void processIN(IN inClone, DatabaseImpl db, long logLsn) {
    ...
    try {
      ...
      cleaner.logFine(Cleaner.CLEAN_IN, inClone, logLsn, completed, obsolete, dirtied);
    }
  }

  /**
   * Given a clone of an IN that has been taken out of the log, try to find it in the tree and
   * verify that it is the current one in the log. Returns the node in the tree if it is found and
   * it is current re: LSN's. Otherwise returns null if the clone is not found in the tree or it's
   * not the latest version. Caller is responsible for unlatching the returned IN.
   */
  private IN findINInTree(Tree tree, DatabaseImpl db, IN inClone, long logLsn) {
    ...
    try {
      result =
          tree.getParentINForChildIN(
              inClone, true /*useTargetLevel*/, true /*doFetch*/, Cleaner.UPDATE_GENERATION);
       ...
       */
      final BIN bin = (BIN) parent.fetchIN(result.index, Cleaner.UPDATE_GENERATION);
  ...
  /** Get the current root in the tree, or null if the inClone is not the current root. */
  private static class RootDoWork implements WithRootLatched {
    ...
    public IN doWork(ChildReference root) {
      ...
      if (root == null
          ...
          || // deferred write root
          (((IN) root.fetchTarget(db, null)).getNodeId() != inClone.getNodeId())) {
        return null;
      }
  ...
  /**
   * Check if the cloned IN is the same node as the root in tree. Return the real root if it is,
   * null otherwise. If non-null is returned, the returned IN (the root) is latched -- caller is
   * responsible for unlatching it.
   */
  ...
  /**
   * A cache of LNInfo by LSN offset. Used to hold a set of LNs that are to be processed. Keeps
   * track of memory used, and when full (over budget) the next offset should be queried and
   * removed.
   */
