 ...
 *
 * <p>The methods in this class for reading obsolete offsets may be used by multiple threads without
 * synchronization even while another thread is adding offsets. This is possible because elements
 * are never deleted from the lists. The thread adding obsolete offsets does so under the log write
...
public class TrackedFileSummary extends FileSummary {
  ...
  private int memSize;
  private final boolean trackDetail;
  ...
  /**
   * Returns whether this summary is allowed or prohibited from being flushed or evicted during
  ...
  /**
   * Allows or prohibits this summary from being flushed or evicted during cleaning. By default,
   * flushing is allowed.
   */
  ...
  /**
   * Return the total memory size for this object. We only bother to budget obsolete detail, not the
   * overhead for this object, for two reasons: 1) The number of these objects is very small, and 2)
   * unit tests disable detail tracking as a way to prevent budget adjustments here.
   */
  int getMemorySize() {
    return memSize;
  }
