...
public class DbFileSummaryMap {
  ...
  private final Map<Long, DbFileSummary> map;
  private int memSize;
  private MemoryBudget budget;

  /**
   * Creates a map of Long file number to DbFileSummary. The init() method must be called after
   * creating this object.
   *
   * <p>Always counts this object and its contained objects in the memory budget. If
   * countParentMapEntry is true, also counts a single HashMap entry that contains this object. This
   * option allows all memory budget adjustments for LocalUtilizationTracker to be contained in this
   * class.
   */
  ...
  /**
   * Starts memory budgeting. The map and its entries will be counted in the budget. When adding
   * entries via the get() method prior to calling this method, the adjustMemBudget parameter must
   * be false. After calling this method, the adjustMemBudget parameter must be true.
   *
   * <p>This method is separate from the constructor so that the map may be read from the log
   * without having the EnvironmentImpl object available.
   */
   ...
   * @param checkResurrected is true if this method should check fileNum and return null if the file
   *     does not exist. When checkResurrected is false, the expensive call to File.exists will not
   *     be made.
   * @param fileManager is used to check for resurrected files and may be null if checkResurrected
   *     is false.
   */
  public DbFileSummary get(
      Long fileNum, boolean adjustMemBudget, boolean checkResurrected, FileManager fileManager) {
  ...
  public Map<Long, DbFileSummary> cloneMap() {
    final Map<Long, DbFileSummary> clone = new HashMap<Long, DbFileSummary>(map.size());
    final Iterator<Map.Entry<Long, DbFileSummary>> i = map.entrySet().iterator();
    while (i.hasNext()) {
