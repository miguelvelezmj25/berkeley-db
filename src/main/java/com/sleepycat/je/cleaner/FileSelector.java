...
public class FileSelector {
  ...
  /**
   * Each file for which cleaning is in progress has one of the following status values. Files
   * numbers migrate from one status to another, in the order declared below.
   */
  enum FileStatus {

    /**
     * A file's status is initially TO_BE_CLEANED when it is selected as part of a batch of files
     * that, when deleted, will bring total utilization down to the minimum configured value. All
     * files with this status will be cleaned in lowest-cost-to-clean order. For two files of equal
     * cost to clean, the lower numbered (oldest) files is selected; this is why the fileInfoMap is
     * sorted by key (file number).
     */
    TO_BE_CLEANED,

    /**
     * When a TO_BE_CLEANED file is selected for processing by FileProcessor, it is moved to the
     * BEING_CLEANED status. This distinction is used to prevent a file from being processed by more
     * than one thread.
     */
    BEING_CLEANED,

    /**
     * A file is moved to the CLEANED status when all its log entries have been read and processed.
     * However, entries needing migration will be marked with the BIN entry MIGRATE flag, entries
     * that could not be locked will be in the pending LN set, and the DBs that were pending
     * deletion will be in the pending DB set.
     */
    CLEANED,

    /**
     * A file is moved to the CHECKPOINTED status at the end of a checkpoint if it was CLEANED at
     * the beginning of the checkpoint. Because all dirty BINs are flushed during the checkpoints,
     * no files in this set will have entries with the MIGRATE flag set. However, some entries may
     * be in the pending LN set and some DBs may be in the pending DB set.
     */
    CHECKPOINTED,

    /**
     * A file is moved from the CHECKPOINTED status to the FULLY_PROCESSED status when the pending
     * LN/DB sets become empty. Since a pending LN was not locked successfully, we don't know its
     * original file. But we do know that when no pending LNs are present for any file, all log
     * entries in CHECKPOINTED files are either obsolete or have been migrated. Note, however, that
     * the parent BINs of the migrated entries may not have been logged yet.
     *
     * <p>No special handling is required to coordinate syncing of deferred write databases for
     * pending, deferred write LNs, because non-temporary deferred write DBs are always synced
     * during checkpoints, and temporary deferred write DBs are not recovered. Note that although DW
     * databases are non-txnal, their LNs may be pended because of lock collisions.
     */
    FULLY_PROCESSED,
  }
  ...
  /**
   * Information about files being cleaned, keyed by file number. The map is sorted by file number
   * to clean older files before newer files.
   */
  private final SortedMap<Long, FileInfo> fileInfoMap;

  /**
   * Pending LN info, keyed by original LSN. These are LNs that could not be locked, either during
   * processing or during migration.
   */
  private final Map<Long, LNInfo> pendingLNs;

  /**
   * For processed entries with DBs that are pending deletion, we consider them to be obsolete but
   * we store their DatabaseIds in a set. Until the DB deletion is complete, we can't delete the log
   * files containing those entries.
   */
  private final Set<DatabaseId> pendingDBs;

  /**
   * If during a checkpoint there are no pending LNs or DBs added, we can move CLEANED files
   * directly to reserved status at the end of the checkpoint. This is an optimization that allows
   * deleting files more quickly when possible. In particular this impacts the checkpoint during
   * environment close, since no user operations are active during that checkpoint; this
   * optimization allows us to delete all cleaned files after the final checkpoint.
   */
   ...
   *
   * @param forceCleaning is true to always select a file, even if its utilization is above the
   *     minimum utilization threshold.
   * @return {file number, required utilization for 2-pass cleaning}, or null if no file qualifies
   *     for cleaning.
   */
  ...
  /**
   * Moves a file to a given status, adding the file to the fileInfoMap if necessary.
   *
   * <p>This method must be called while synchronized.
   */
  ...
  /**
   * Moves a collection of files to a given status, adding the files to the fileInfoMap if
   * necessary.
   *
   * <p>This method must be called while synchronized.
   */
  ...
  /**
   * Asserts that a file has a given status. Should only be called under an assertion to avoid the
   * overhead of the method call and synchronization. Always returns true to enable calling it under
   * an assertion.
   *
   * <p>This method must be called while synchronized.
   */
  ...
  synchronized void addCleanedFile(
      Long fileNum, Set<DatabaseId> databases, VLSN firstVlsn, VLSN lastVlsn, MemoryBudget budget) {
    assert checkStatus(fileNum, FileStatus.BEING_CLEANED);
  ...
  synchronized CheckpointStartCleanerState getFilesAtCheckpointStart() {
    ...
    return new CheckpointStartCleanerState(
        getFiles(FileStatus.CLEANED), getFiles(FileStatus.FULLY_PROCESSED));
  }

  /**
   * Returns whether any files are cleaned or fully-processed, meaning that a checkpoint is needed
  ...
  /**
   * When a checkpoint is complete, move the previously CLEANED and FULLY_PROCESSED files to the
   * CHECKPOINTED and reserved status. Reserved files are removed from the FileSelector and their
   * reserved status is maintained in FileProtector.
   *
   * @return map of {fileNum, FileInfo} for the files whose status was changed to reserved.
   */
  synchronized Map<Long, FileInfo> updateFilesAtCheckpointEnd(
      final EnvironmentImpl env, final CheckpointStartCleanerState info) {
    ...
    final Set<Long> previouslyProcessedFiles = info.getFullyProcessedFiles();
  ...
  /**
   * Returns a map of LNInfo for LNs that could not be migrated in a prior cleaning attempt, or null
   * if no LNs are pending.
   */
  ...
  /**
   * Returns an array of DatabaseIds for DBs that were pending deletion in a prior cleaning attempt,
   * or null if no DBs are pending.
   */
  ...
  /**
   * If there are no pending LNs or DBs outstanding, move the CHECKPOINTED files to FULLY_PROCESSED.
   * The check for pending LNs/DBs and the copying of the CHECKPOINTED files must be done atomically
   * in a synchronized block. All methods that call this method are synchronized.
   */
  ...
  /**
   * Adjust the memory budget when an entry is added to or removed from the cleanedFilesDatabases
   * map.
   */
  private void adjustMemoryBudget(
      MemoryBudget budget, Set<DatabaseId> oldDatabases, Set<DatabaseId> newDatabases) {
    long adjustMem = 0;
  ...
  /**
   * Returns the size of a HashMap entry that contains the given set of DatabaseIds. We don't count
   * the DatabaseId size because it is likely that it is also stored (and budgeted) in the
   * DatabaseImpl.
   */
  ...
  public static class CheckpointStartCleanerState {
    ...
    private final Set<Long> cleanedFiles;
    private final Set<Long> fullyProcessedFiles;
