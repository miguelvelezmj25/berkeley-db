 ...
 *
 * <p>Note that we do clean files that are protected from deletion by HA/DataSync. If we did not
 * clean them and a large number of files were to become unprotected at once, a large amount of log
 * cleaning may suddenly be necessary. Cleaning the files avoids this. Better still would be to
 * delete the metadata, but that would require writing a log entry to indicate the file is ready to
 * be deleted, to avoid cleaning from scratch after a crash. [#16643] [#19221]
 *
 * <p>Historical note: Prior to JE 6.0, LN utilization adjustments were needed because the LN last
 * logged size was not stored in the BIN [#18633]. Originally in JE 5, the corrected average LN size
 * was used to adjust utilization. This was changed later in JE 5 to a correction factor since
 * different log files may have different average LN sizes [#21106]. Then in JE 6.0 the last logged
 * size was added to the BIN, the default for {@link
 * com.sleepycat.je.EnvironmentConfig#CLEANER_ADJUST_UTILIZATION} was changed to false and a warning
 * was added that the feature will be removed in the future [#22275]. Finally in JE 6.3 the LN
 * adjustment code and data in CheckpointEnd were removed and the parameter was deprecated [#24090].
 *
 * <p>Unlike with LNs, we do not store the last logged size of INs, so their obsolete size is
 * computed as an average and this has the potential to cause over/under-cleaning. This problem is
 * not known to occur, but if there are over/under-cleaning problems we should examine the
 * recalculated info that is output as part of the CleanerRun INFO message.
 *
 * <p>=== Expired Data and Utilization ===
 *
 * <p>Per-file histograms are calculated by the {@link ExpirationTracker} and stored in an internal
 * database and cache by the {@link ExpirationProfile}. The histograms are used to calculate the
 * expired bytes for a file at a particular time. Since obsolete (not expired) data can overlap with
 * expired data, upper and lower bounds for overall utilization are determined. When the lower bound
 * is below minUtilization, cleaning occurs.
 *
 * <p>The file that has the lowest average utilization (midway between its upper and lower bounds)
 * is selected for cleaning. If the file's upper and lower bounds are not close together (or the
 * same), and the upper bound is above a threshold, then two-pass cleaning is performed. See {@link
 * EnvironmentParams#CLEANER_TWO_PASS_GAP} and {@link EnvironmentParams#CLEANER_TWO_PASS_THRESHOLD}.
 *
 * <p>The first pass of two-pass cleaning reads the file but doesn't do any real cleaning (no side
 * effects). If this pass finds that the true utilization of the file is above the threshold (the
 * same threshold as above), then cleaning does not take place and instead the histogram is updated.
 * In this case the obsolete and expired data (in the old histogram) overlap, and it is the overlap
 * that caused utilization to be estimated incorrectly. The new/updated histogram is calculated such
 * that there is no overlap, improving utilization accuracy. If the first pass finds that true
 * utilization is below the threshold, then normal cleaning (pass two) occurs. Two-pass cleaning
 * protects against "over cleaning".
 *
 * <p>The use of the overall utilization lower bound to drive cleaning is considered sufficient to
 * protect against "under cleaning". Therefore, a disk space threshold is unnecessary.
 *
 * <p>Gradual expiration is used to prevent cleaning spikes on day or hour boundaries. For purposes
 * of driving cleaning, the utilization lower bound is calculated by distributing the bytes that
 * expired in the current day/hour period evenly over that day/hour.
 */
public class UtilizationCalculator implements EnvConfigObserver {
   ...
   *
   * <p>This method is called by FileSelector and synchronization order is: 1-FileSelector,
   * 2-UtilizationCalculator, 3-ExpirationProfile.
   *
  ...
  synchronized Pair<Long, Integer> getBestFile(
      final SortedMap<Long, FileSummary> fileSummaryMap, final boolean forceCleaning) {

    /* Paranoia.  There should always be 1 file. */
    if (fileSummaryMap.size() == 0) {
      LoggerUtils.logMsg(logger, env, Level.SEVERE, "Can't clean, map is empty.");
      return null;
    }
    ...
    for (final Map.Entry<Long, FileSummary> entry : fileSummaryMap.entrySet()) {
      ...
      if (expirationEnabled) {
        ...
        expiredGradualSize =
            gradualExpiration ? Math.min(expiredSizes.second(), summary.totalSize) : expiredSize;
      ...
      final int minObsoleteSize = Math.max(obsoleteSize, expiredSize);

      final int maxObsoleteSize = Math.min(obsoleteSize + expiredSize, summary.totalSize);

      final int minGradualObsoleteSize = Math.max(obsoleteSize, expiredGradualSize);

      final int maxGradualObsoleteSize =
          Math.min(obsoleteSize + expiredGradualSize, summary.totalSize);
       ...
       * utilization so far.
       */
      final int thisMinUtil = FileSummary.utilization(maxObsoleteSize, summary.totalSize);

      final int thisMaxUtil = FileSummary.utilization(minObsoleteSize, summary.totalSize);
       ...
       */
      final int thisGradualMaxUtil =
          FileSummary.utilization(minGradualObsoleteSize, summary.totalSize);

      if (bestGradualFile == null || thisGradualMaxUtil < bestGradualFileMaxUtil) {
    ...
    final int currentMinUtil = FileSummary.utilization(currentMaxObsoleteSize, currentTotalSize);

    final int currentMaxUtil = FileSummary.utilization(currentMinObsoleteSize, currentTotalSize);

    final int predictedMinUtil =
        FileSummary.utilization(predictedMaxObsoleteSize, predictedTotalSize);

    final int predictedMaxUtil =
        FileSummary.utilization(predictedMinObsoleteSize, predictedTotalSize);
    ...
    if (logger.isLoggable(logLevel)) {
      ...
      LoggerUtils.logMsg(
          ...
          logLevel,
          "Clean file "
              + ((fileChosen != null) ? ("0x" + Long.toHexString(fileChosen)) : "none")
              + ": "
              + reason
              + twoPassMsg
              + ", current util min: "
    ...
    return (fileChosen != null) ? new Pair<>(fileChosen, pass1RequiredUtil) : null;
  }

  /**
   * Process notifications of mutable property changes.
   *
   * @throws IllegalArgumentException via FilesToMigrate ctor and parseForceCleanFiles.
   */
