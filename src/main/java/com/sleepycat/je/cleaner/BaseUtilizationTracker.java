...
/**
 * Shared implementation for all utilization trackers. The base implementation keeps track of
 * per-file utilization info only. Subclasses keep track of per-database info.
 */
public abstract class BaseUtilizationTracker {
  ...
  /**
   * The tracked files are maintained in a volatile field Map that is "copied on write" whenever an
   * element is added or removed. Add and remove are called only under the log write latch, but get
   * and iteration may be performed at any time because the map is read-only.
   */
  ...
  /**
   * Returns a snapshot of the files being tracked as of the last time a log entry was added. The
   * summary info returned is the delta since the last checkpoint, not the grand totals, and is
   * approximate since it is changing in real time. This method may be called without holding the
   * log write latch.
   *
   * <p>If files are added or removed from the collection of tracked files in real time, the
   * returned collection will not be changed since it is a snapshot. But the objects contained in
   * the collection are live and will be updated in real time under the log write latch. The
   * collection and the objects in the collection should not be modified by the caller.
   */
  ...
  /**
   * Returns one file from the snapshot of tracked files, or null if the given file number is not in
   * the snapshot array.
   *
   * @see #getTrackedFiles
   */
   ...
   *
   * <p>For the global tracker, must be called under the log write latch.
   */
  final void countNew(long lsn, Object databaseKey, LogEntryType type, int size) {
    assert type != null;
    ...
    if (trackObsoleteInfo(type)) {
      assert databaseKey != null
          : "No DB for lsn=" + DbLsn.getNoFormatString(lsn) + " type: " + type;
      DbFileSummary dbFileSummary = getDbFileSummary(databaseKey, fileNum);
      if (isLN) {
  ...
  /**
   * Counts an obsolete node by incrementing the obsolete count and size. Tracks the LSN offset if
   * trackOffset is true and the offset is non-zero.
   *
   * <p>For the global tracker, must be called under the log write latch.
   */
  ...
  /**
   * Counts all active LSNs in a database as obsolete in the per-file utilization summaries. This
   * method is called during database remove/truncate or when replaying those operations during
   * recovery.
   *
   * <p>For the global tracker, must be called under the log write latch.
   *
   * @param dbFileSummaries the map of Long file number to DbFileSummary for a database that is
   *     being deleted.
   * @param mapLnLsn is the LSN of the MapLN when recovery is replaying the truncate/remove, or
   *     NULL_LSN when called outside of recovery; obsolete totals should only be counted when this
   *     LSN is prior to the LSN of the FileSummaryLN for the file being counted.
   */
  public void countObsoleteDb(DbFileSummaryMap dbFileSummaries, long mapLnLsn) {
    ...
    for (Map.Entry<Long, DbFileSummary> entry : dbFileSummaries.entrySet()) {
      ...
      if (isFileUncounted(fileNum, mapLnLsn)) {
        ...
        TrackedFileSummary fileSummary = getFileSummary(fileNum.longValue());
         ...
         */
        int lnObsoleteSizeCounted =
            lnObsoleteCount + (dbFileSummary.obsoleteLNCount - dbFileSummary.obsoleteLNSizeCounted);
  ...
  /**
   * Returns whether file summary information for the given LSN is not already counted. Outside of
   * recovery, always returns true. For recovery, is overridden by RecoveryUtilizationTracker and
   * returns whether the FileSummaryLN for the given file is prior to the given LSN. .
   */
  ...
  /**
   * Returns a DbFileSummary for the given database key and file number, adding an empty one if the
   * file is not already being tracked.
   *
   * <p>This method is implemented by subclasses which interpret the databaseKey as either the
   * DatabaseImpl or a DatabaseId.
   *
   * <p>For the global tracker, must be called under the log write latch.
   *
   * @return the summary, or null if the DB should not be tracked because the file has been deleted.
   */
  abstract DbFileSummary getDbFileSummary(Object databaseKey, long fileNum);

  /**
   * Returns a tracked file for the given file number, adding an empty one if the file is not
  ...
  TrackedFileSummary getFileSummary(long fileNum) {
    ...
    if (file == null) {
      ...
      Map<Long, TrackedFileSummary> newFiles = new HashMap<Long, TrackedFileSummary>(fileSummaries);
   ...
   *
   * <p>We keep the active file summary in the tracked file map, but we remove older files to
  ...
  void resetFile(TrackedFileSummary fileSummary) {
    ...
    if (fileSummary.getFileNumber() < activeFile && fileSummary.getAllowFlush()) {
      /* Assign fileSummaries field after modifying the new map. */
      Map<Long, TrackedFileSummary> newFiles = new HashMap<Long, TrackedFileSummary>(fileSummaries);
  ...
  /**
   * Returns whether obsoleteness is tracked for the given type. Obsoleteness is tracked for node
   * types and BIN-deltas. A null type is assumed to be an LN.
   */
