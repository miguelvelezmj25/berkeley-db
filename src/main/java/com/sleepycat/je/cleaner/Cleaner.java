...
/**
 * The Cleaner is responsible for effectively garbage collecting the JE log. It selects the least
 * utilized log file for cleaning (see FileSelector), reads through the log file (FileProcessor) and
 * determines whether each entry is obsolete (no longer relevant) or active (referenced by the
 * Btree). Entries that are active are migrated (copied) to the end of the log, and finally the
 * cleaned file is deleted.
 *
 * <p>The migration of active entries is a multi-step process that can be configured to operate in
 * different ways. Eviction and checkpointing, as well as the cleaner threads (FileProcessor
 * instances) are participants in this process. Migration may be immediate or lazy.
 *
 * <p>Active INs are always migrated lazily, which means that they are marked dirty by the
 * FileProcessor, and then logged later by an eviction or checkpoint. Active LNs are always migrated
 * immediately by the FileProcessor by logging them.
 *
 * <p>When the FileProcessor is finished with a file, all lazy migration for that file is normally
 * completed by the end of the next checkpoint, if not sooner via eviction. The checkpoint/recovery
 * mechanism will ensure that obsolete entries will not be referenced by the Btree. At the end of
 * the checkpoint, it is therefore safe to delete the log file.
 *
 * <p>There is one exception to the above paragraph. When attempting to migrate an LN, if the LN
 * cannot be locked then we must retry the migration at a later time. Also, if a database removal is
 * in progress, we consider all entries in the database obsolete but cannot delete the log file
 * until database removal is complete. Such "pending" LNs and databases are queued and processed
 * periodically during file processing and at the start of a checkpoint; see processPending(). In
 * this case, we may have to wait for more than one checkpoint to occur before the log file can be
 * deleted. See FileSelector and the use of the pendingLNs and pendingDBs collections.
 */
public class Cleaner implements DaemonRunner, EnvConfigObserver {
  ...
  /**
   * The CacheMode to use for Btree searches. This is currently UNCHANGED because we update the
   * generation of the BIN when we migrate an LN. In other other cases, it is not desirable to keep
   * INs in cache.
   */
  static final CacheMode UPDATE_GENERATION = CacheMode.UNCHANGED;

  /**
   * Whether the cleaner should participate in critical eviction. Ideally the cleaner would not
   * participate in eviction, since that would reduce the cost of cleaning. However, the cleaner can
   * add large numbers of nodes to the cache. By not participating in eviction, other threads could
   * be kept in a constant state of eviction and would effectively starve. Therefore, this setting
  ...
  private static final long MAX_CLEANER_BYTES_INTERVAL = 100L << 20;

    /* 10GB is the lower threshold for adjusting MAX_DISK. */
  private static final long MAX_DISK_ADJUSTMENT_THRESHOLD = 10L * 1024L * 1024L * 1024L;
  ...
  public Cleaner(EnvironmentImpl env, String name) {
    ...
    nPendingLNsProcessed = new LongStat(statGroup, CLEANER_PENDING_LNS_PROCESSED);
    nMarkedLNsProcessed = new LongStat(statGroup, CLEANER_MARKED_LNS_PROCESSED);
    nToBeCleanedLNsProcessed = new LongStat(statGroup, CLEANER_TO_BE_CLEANED_LNS_PROCESSED);
    nClusterLNsProcessed = new LongStat(statGroup, CLEANER_CLUSTER_LNS_PROCESSED);
    ...
    if (env.isMemOnly()) {
      ...
      try {
        fileStoreInfo = FileStoreInfo.getInfo(env.getEnvironmentHome().getAbsolutePath());
      } catch (IOException e) {
  ...
  private void setMutableProperties(final DbConfigManager cm) {
    ...
    useDeletedDir = cm.getBoolean(EnvironmentParams.CLEANER_USE_DELETED_DIR);

    twoPassGap = cm.getInt(EnvironmentParams.CLEANER_TWO_PASS_GAP);

    twoPassThreshold = cm.getInt(EnvironmentParams.CLEANER_TWO_PASS_THRESHOLD);

    if (twoPassThreshold == 0) {
      twoPassThreshold = cm.getInt(EnvironmentParams.CLEANER_MIN_UTILIZATION) - 5;
    }

    gradualExpiration = cm.getBoolean(EnvironmentParams.CLEANER_GRADUAL_EXPIRATION);

    dbCacheClearCount = cm.getInt(EnvironmentParams.ENV_DB_CACHE_CLEAR_COUNT);
    ...
    if (cleanerBytesInterval == 0) {
      ...
      cleanerBytesInterval = Math.min(cleanerBytesInterval, MAX_CLEANER_BYTES_INTERVAL);
    }

    final int wakeupInterval = cm.getDuration(EnvironmentParams.CLEANER_WAKEUP_INTERVAL);
    ...
    fetchObsoleteSize = cm.getBoolean(EnvironmentParams.CLEANER_FETCH_OBSOLETE_SIZE);
    ...
    if (env.isMemOnly()) {
      ...
      if (replayFreeDiskPct == 0) {
        ...
        freeDiskLimit = (getDiskTotalSpace() * replayFreeDiskPct) / 100;
      }
  ...
  /**
   * If the number of bytes written since the last activation exceeds the cleaner's byte interval,
   * wakeup the file processor threads in activate mode.
   */
  public void wakeupAfterWrite(int writeSize) {

    if (bytesWrittenSinceActivation.addAndGet(writeSize) > cleanerBytesInterval) {
  ...
  /**
   * Wakeup the file processor threads in activate mode, meaning that FileProcessor.doClean will be
   * called.
   *
  ...
  /**
   * Cleans selected files and returns the number of files cleaned. This method is not invoked by a
   * deamon thread, it is programatically.
   *
   * @param cleanMultipleFiles is true to clean until we're under budget, or false to clean at most
   *     one file.
   * @param forceCleaning is true to clean even if we're not under the utilization threshold.
   * @return the number of files cleaned, not including files cleaned unsuccessfully.
   */
  public int doClean(boolean cleanMultipleFiles, boolean forceCleaning) {

    FileProcessor processor = createProcessor();

    return processor.doClean(false /*invokedFromDaemon*/, cleanMultipleFiles, forceCleaning);
  }

  public FileProcessor createProcessor() {
    return new FileProcessor("", false, env, this, profile, calculator, fileSelector);
  }
  ...
  public StatGroup loadStats(StatsConfig config) {
    ...
    new IntStat(stats, CLEANER_MIN_UTILIZATION, calculator.getCurrentMinUtilization());
    new IntStat(stats, CLEANER_MAX_UTILIZATION, calculator.getCurrentMaxUtilization());
    new IntStat(stats, CLEANER_PENDING_LN_QUEUE_SIZE, fileSelector.getPendingLNQueueSize());
    ...
    synchronized (statGroup) {
      new LongStat(stats, CLEANER_ACTIVE_LOG_SIZE, logSizeStats.activeSize);
      new LongStat(stats, CLEANER_RESERVED_LOG_SIZE, logSizeStats.reservedSize);
      new LongStat(stats, CLEANER_PROTECTED_LOG_SIZE, logSizeStats.protectedSize);
      new LongStat(stats, CLEANER_AVAILABLE_LOG_SIZE, availableLogSize);
      new LongStat(stats, CLEANER_TOTAL_LOG_SIZE, totalLogSize);

      final AtomicLongMapStat protectedSizeMap =
          new AtomicLongMapStat(stats, CLEANER_PROTECTED_LOG_SIZE_MAP);

      for (final Map.Entry<String, Long> entry : logSizeStats.protectedSizeMap.entrySet()) {
  ...
  /**
   * Enables or disabling processing of safe-to-delete files, including the truncation of the VLSN
   * index. Disabling this is needed for tests, when VLSNIndex changes should be prevented.
   */
  ...
  /**
   * Updates log size stats and deletes unprotected reserved files in order to stay within disk
   * limits.
   *
   * <p>This method must be called frequently enough to maintain disk usage safely below the limits.
   * For an HA env this is particularly important since we retain all reserved files until we
   * approach the disk limits. For this, calling this method at least every CLEANER_BYTES_INTERVAL
   * should suffice.
   *
   * <p>It is also important to call this method based on a time interval when writing stops, to
   * retry deletions when files are protected or the env is locked by read-only processes. For this,
   * calling this method at least every CLEANER_WAKEUP_INTERVAL should suffice.
   */
  public void manageDiskUsage() {
    ...
    try {
      ...
      if (fileProtector.getNReservedFiles() > 0) {
        ...
        if (env.isReplicated()) {
          ...
          final long origBytesNeeded =
              ((maxDiskLimit > 0) ? Math.max(maxDiskOverage, freeDiskShortage) : freeDiskShortage)
                  + Math.max(1L << 20, 3 * cleanerBytesInterval);
           ...
           */
          if (bytesNeeded > 0 && env.tryVlsnHeadTruncate(bytesNeeded)) {
           ...
           * files.
           */
          final long bytesNeeded = deleteUnprotectedFiles(Long.MAX_VALUE);
      ...
      LoggerUtils.logMsg(
          ...
          env,
          Level.SEVERE,
          "Could not delete files due to read-only processes. " + diskUsageMessage);
  ...
  /**
   * Deletes unprotected reserved files in an attempt to free bytesNeeded. In a non-HA env, attempts
   * to delete all reserved files, irrespective of bytesNeeded.
   *
   * <p>An exclusive environment is held while deleting the files to lock out read-only processes.
   * The lock is held while deleting the reserved file records as well, but this is inexpensive and
   * should not cause long delays for read-only processes.
   *
   * @param bytesNeeded the amount of space we need to reclaim to stay within disk limits.
   * @return number of bytes we could not reclaim due to protected files, or zero if we deleted
   *     files totaling bytesNeeded or more.
   * @throws EnvLockedException if we can't get an exclusive environment lock because the env is
   *     locked by read-only processes, and therefore no files can be deleted.
   */
  private long deleteUnprotectedFiles(long bytesNeeded) throws EnvLockedException {
    ...
    try {
      ...
      while (bytesNeeded > 0 || !env.isReplicated()) {
        ...
        final Pair<Long, Long> pair = fileProtector.takeCondemnedFile(file + 1);
      ...
      if (!deletedFiles.isEmpty()) {
        ...
        final StringBuilder sb = new StringBuilder("Cleaner deleted files:");
        ...
        LoggerUtils.traceAndLog(logger, env, Level.INFO, sb.toString());
      }
    }
  ...
  private boolean deleteFile(final Long file) {
    ...
    try {
      if (expunge) {
        ...
        final File newFile =
            fileManager.renameFile(
                file, FileManager.DEL_SUFFIX, useDeletedDir ? DELETED_SUBDIR : null);
      ...
      throw new EnvironmentFailureException(
          env, EnvironmentFailureReason.LOG_WRITE, "Unable to " + expungeLabel + " " + file, e);
    }
    ...
    LoggerUtils.traceAndLog(
        ...
        "Log file 0x"
            ...
            + " could not be "
            + expungedLabel
            + ". The deletion will be retried later.");

    return false;
  }

  /**
   * Updates the cached set of log size stats, including maxDiskOverage, freeDiskShortage,
   * diskUsageMessage and diskUsageViolationMessage.
   *
   * <p>Normally this should only be called by manageDiskUsage while holding the
   * manageDiskUsageLock. However, it may be called directly during (single threaded) recovery.
   */
  public void freshenLogSizeStats() {

    recalcLogSizeStats(fileProtector.getLogSizeStats(), getDiskFreeSpace());
  }

  /** Implementation of freshenLogSizeStats. Exposed for testing. */
  void recalcLogSizeStats(final FileProtector.LogSizeStats stats, final long diskFreeSpace) {
    ...
    final long availBytes = freeBytes2 + stats.reservedSize - stats.protectedSize;
    ...
    for (final Map.Entry<String, Long> entry : stats.protectedSizeMap.entrySet()) {
  ...
  /**
   * Returns a message describing disk space limits and usage, regardless of whether the limit is
   * violated or not. If there is a limit violation, returns the same value as {@link
   * #getDiskLimitViolation()}. Does not return null.
   */
   ...
   *
   * <p>If non-null is returned, the checkpoint should flush an extra level, and
  ...
  /**
   * When a checkpoint is complete, update the files that were returned at the beginning of the
   * checkpoint.
   */
  ...
  /**
   * If any LNs or databases are pending, process them. This method should be called often enough to
   * prevent the pending LN set from growing too large.
   */
  void processPending() {
    ...
    try {
      ...
      final LockManager lockManager = env.getTxnManager().getLockManager();
      ...
      if (pendingLNs != null) {
        ...
        for (final Map.Entry<Long, LNInfo> entry : pendingLNs.entrySet()) {
          ...
          if (env.expiresWithin(info.getExpirationTime(), 0 - env.getTtlLnPurgeDelay())) {
  ...
  /**
   * Processes a pending LN, getting the lock first to ensure that the overhead of retries is
   * minimal.
   */
  private void processPendingLN(
    ...
    try {
      ...
      final LockResult lockRet =
          locker.nonBlockingLock(logLsn, LockType.READ, false /*jumpAheadOfWaiters*/, db);
      ...
      lockDenied = migratePendingLN(db, logLsn, bin.getLsn(index), bin, index);
      ...
      LoggerUtils.traceAndLogException(
          env, "com.sleepycat.je.cleaner.Cleaner", "processLN", "Exception thrown: ", e);
      throw e;
    } finally {
      if (bin != null) {
        bin.releaseLatch();
  ...
  /**
   * Migrate a pending LN in the given BIN entry, if it is not obsolete. The BIN must be latched on
   * entry and is left latched by this method.
   *
   * @return whether migration could not be completed because the LN lock was denied.
   */
  private boolean migratePendingLN(
    ...
    try {
      ...
      if (logLsn != treeLsn) {
        ...
        final LockResult lockRet =
            locker.nonBlockingLock(treeLsn, LockType.READ, false /*jumpAheadOfWaiters*/, db);
        ...
        throw EnvironmentFailureException.unexpectedState(
            ...
            "LN is embedded although its associated logrec (at "
                + treeLsn
                + " does not have the embedded flag on");
      }
      ...
      CursorImpl.lockAfterLsnChange(db, treeLsn, logItem.lsn, null /*excludeLocker*/);
      ...
      logFine(CLEAN_PENDING_LN, ln, treeLsn, completed, obsolete, migrated);
    }
  }

  /**
   * Returns the ReplicationContext to use for migrating the given LN. If VLSNs are preserved in
   * this Environment then the VLSN is logically part of the data record, and LN.getVLSNSequence
   * will return the VLSN, which should be included in the migrated LN.
   */
  static ReplicationContext getMigrationRepContext(LN ln) {
    ...
    return new ReplicationContext(new VLSN(vlsnSeq), false /*inReplicationStream*/);
  }
  ...
  void addPendingDB(DatabaseImpl db) {
    if (db != null && db.isDeleted() && !db.isDeleteFinished()) {
      ...
      if (fileSelector.addPendingDB(id)) {
        LoggerUtils.logMsg(logger, env, Level.FINE, "CleanAddPendingDB " + id);
      }
    }
  }
  ...
  /**
   * Release resources and update memory budget. Should only be called when this environment is
   * closed and will never be accessed again.
   */
  public void close() {
    profile.close();
    tracker.close();
