...
public class VerifyUtils {
   ...
   * Assumes that the database and environment is quiescent, and that there is no current cleaner
   * activity.
   */
  ...
  public static void checkLsns(DatabaseImpl dbImpl, PrintStream out) throws DatabaseException {
    ...
    List<DatabaseException> savedExceptions = new ArrayList<DatabaseException>();
    ...
    while (iter.hasNext()) {
      ...
      PackedOffsets obsoleteOffsets = profile.getObsoleteDetailPacked(fileNum, false /*logUpdate*/);
      PackedOffsets.Iterator obsoleteIter = obsoleteOffsets.iterator();
      while (obsoleteIter.hasNext()) {
        long offset = obsoleteIter.next();
        Long oneLsn = Long.valueOf(DbLsn.makeLsn(fileNum.longValue(), offset));
        obsoleteLsns.add(oneLsn);
        if (DEBUG) {
          out.println("Adding 0x" + Long.toHexString(oneLsn.longValue()));
        }
      }
    }
    ...
    while (iter.hasNext()) {
      ...
      if (obsoleteLsns.contains(lsn)) {
        out.println(
            "Obsolete LSN set contains valid LSN " + DbLsn.getNoFormatString(lsn.longValue()));
        error = true;
      }
    }
    ...
    while (iter.hasNext()) {
      ...
      if (lsnsInTree.contains(lsn)) {
        out.println("Tree contains obsolete LSN " + DbLsn.getNoFormatString(lsn.longValue()));
        error = true;
      }
    }
    ...
    if (error) {
      throw new EnvironmentFailureException(
          envImpl, EnvironmentFailureReason.LOG_INTEGRITY, "Lsn mismatch");
    }

    if (savedExceptions.size() > 0) {
      throw new EnvironmentFailureException(
          envImpl, EnvironmentFailureReason.LOG_INTEGRITY, "Sorted LSN Walk problem");
    }
  }
  ...
  private static class GatherLSNs implements TreeNodeProcessor {
    ...
    public void processLSN(
        long childLSN, LogEntryType childType, Node ignore, byte[] ignore2, int ignore3) {
  ...
  /**
   * Compare utilization as calculated by UtilizationProfile to utilization as calculated by
   * UtilizationFileReader. Also check that per-database and per-file utilization match.
   *
   * @throws EnvironmentFailureException if there are mismatches
   */
  public static void verifyUtilization(
    ...
    Map<Long, FileSummary> profileMap =
        envImpl.getCleaner().getUtilizationProfile().getFileSummaryMap(true);
     ...
     */
    Iterator<Map.Entry<Long, FileSummary>> i = profileMap.entrySet().iterator();
    while (i.hasNext()) {
      ...
      //*/
      check(fileStr, recalcSummary.totalCount == profileSummary.totalCount);
      check(fileStr, recalcSummary.totalSize == profileSummary.totalSize);
      check(fileStr, recalcSummary.totalINCount == profileSummary.totalINCount);
      check(fileStr, recalcSummary.totalINSize == profileSummary.totalINSize);
      check(fileStr, recalcSummary.totalLNCount == profileSummary.totalLNCount);
      check(fileStr, recalcSummary.totalLNSize == profileSummary.totalLNSize);
       ...
       */
      if (false) {
        check(fileStr, recalcSummary.obsoleteINCount == profileSummary.obsoleteINCount);
      }
       ...
       */
      if (expectAccurateObsoleteLNCount) {
        check(fileStr, recalcSummary.obsoleteLNCount == profileSummary.obsoleteLNCount);
        ...
        if (expectAccurateObsoleteLNSize) {
          check(fileStr, recalcSummary.getObsoleteLNSize() == profileSummary.obsoleteLNSize);
        }
      }
      ...
      if (expectAccurateDbUtilization) {
        ...
        check(fileStr, profileSummary.totalINCount == dbSummary.totalINCount);
        check(fileStr, profileSummary.totalLNCount == dbSummary.totalLNCount);
        check(fileStr, profileSummary.totalINSize == dbSummary.totalINSize);
        check(fileStr, profileSummary.totalLNSize == dbSummary.totalLNSize);
         ...
         */
        if (false) {
          check(fileStr, profileSummary.obsoleteINCount == dbSummary.obsoleteINCount);
        }
        if (expectAccurateObsoleteLNCount) {
          check(fileStr, profileSummary.obsoleteLNCount == dbSummary.obsoleteLNCount);
          if (expectAccurateObsoleteLNSize) {
            check(fileStr, profileSummary.obsoleteLNSize == dbSummary.obsoleteLNSize);
            check(fileStr, profileSummary.obsoleteLNSizeCounted == dbSummary.obsoleteLNSizeCounted);
          }
        }
      }
    }
  ...
  /**
   * Adds up the per-file totals from the utilization information for each database to make a total
   * per-file count.
   *
   * @return aggregation of per-file information.
   */
  private static Map<Long, DbFileSummary> calcDbDerivedUtilization(EnvironmentImpl envImpl)
      throws DatabaseException {
  ...
  /**
   * Walk through the DbFileSummaryMap associated with a single database and aggregate all the
   * per-file/per db information into a single per-file grandTotals map.
   */
  private static void addDbDerivedTotals(
      DatabaseImpl dbImpl, Map<Long, DbFileSummary> grandTotalsMap) {
