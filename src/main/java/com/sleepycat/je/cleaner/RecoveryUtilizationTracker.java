 ...
 *
 * <p>Per-database information is keyed by DatabaseId because the DatabaseImpl is not always
 * available during recovery. In fact this is the only reason that a "local" tracker is used during
 * recovery -- to avoid requiring that the DatabaseImpl is available, which is necessary to use the
 * "global" UtilizationTracker. There is no requirement to accumulate totals locally, since recovery
 * is single threaded.
 *
 * <p>When finished with this object, its information should be added to the Environment's
 * UtilizationTracker and DatabaseImpl objects by calling transferToUtilizationTracker. This is done
 * at the end of recovery, just prior to the checkpoint. It does not have to be done under the log
 * write latch, since recovery is single threaded.
 */
public class RecoveryUtilizationTracker extends BaseLocalUtilizationTracker {
   ...
   *
   * <p>Even when trackOffset is true, duplicate offsets are not checked (no assertion is fired)
   * because recovery is known to count the same LSN offset twice in certain circumstances.
   */
  public void countObsoleteUnconditional(
      long lsn, LogEntryType type, int size, DatabaseId dbId, boolean trackOffset) {
  ...
  /**
   * Counts the oldLsn of a node obsolete if it has not already been counted at the point of lsn in
   * the log.
   *
   * <p>Even when trackOffset is true, duplicate offsets are not checked (no assertion is fired)
   * because recovery is known to count the same LSN offset twice in certain circumstances.
  ...
  public boolean countObsoleteIfUncounted(
      long oldLsn, long newLsn, LogEntryType type, int size, DatabaseId dbId, boolean trackOffset) {
  ...
  /**
   * Overrides this method for recovery and returns whether the most recently seen FileSummaryLN for
   * the given file is prior to the given LSN.
   */
  @Override
