...
/**
 * Stores a sorted list of LSN offsets in a packed short representation. Each stored value is the
 * difference between two consecutive offsets. The stored values are stored as one or more shorts
 * where each short holds 0x7fff values. Shorts are in LSB order. The value is negated if more
 * shorts for the same offset follow; this works because offsets are always positive values.
 */
public class PackedOffsets implements Loggable {
  ...
  /**
   * Copies the given value as a packed long to the array starting at the given index. Returns the
   * index of the next position in the array.
   */
  ...
  /**
   * Return the extra memory used by this object when the pack() method has been called to allocate
   * the data array.
   */
