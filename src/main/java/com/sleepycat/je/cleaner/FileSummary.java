...
/**
 * Per-file utilization counters. The UtilizationProfile stores a persistent map of file number to
 * FileSummary.
 */
public class FileSummary implements Loggable, Cloneable {
  ...
  /**
   * Returns the approximate byte size of all obsolete LN entries, using the average LN size for LN
   * sizes that were not counted.
   */
  public int getObsoleteLNSize() {
    ...
    final int obsLNSizeCounted = Math.min(obsoleteLNSizeCounted, obsLNCount);
   ...
   *
   * <p>In FileSummaryLN version 3 and greater the obsolete size is normally counted, but not in
   * exceptional circumstances such as recovery. If it is not counted, obsoleteLNSizeCounted will be
   * less than obsoleteLNCount.
   *
   * <p>In log version 8 and greater, we don't count the size when the LN is not resident in cache
   * during update/delete, and CLEANER_FETCH_OBSOLETE_SIZE is false (the default setting).
   *
   * <p>We added maxLNSize in version 8 for use in estimating obsolete LN sizes.
   *
   * <p>To compute the average LN size, we only consider the LNs (both obsolete and non-obsolete)
   * for which the size has not been counted. This increases accuracy when counted and uncounted LN
   * sizes are not uniform. An example is when large LNs are inserted and deleted. The size of the
   * deleted LN log entry (which is small) is always counted, but the previous version (which has a
   * large size) may not be counted.
   */
  ...
  public int getMaxObsoleteLNSize() {
    ...
    if (obsCountNotCounted > 0) {
      ...
      final long maxObsSizeNotCounted =
          totalLNSize - obsLNSize - ((totalLNCount - obsLNCount) * LNLogEntry.MIN_LOG_SIZE);
  ...
  /**
   * Returns an estimate of the total bytes that are obsolete, using getObsoleteLNSize instead of
  ...
  /**
   * Returns an estimate of the total bytes that are obsolete, using getMaxObsoleteLNSize instead of
   * getObsoleteLNSize.
   */
  ...
  private int calculateObsoleteSize(int lnObsoleteSize) {
    ...
    int obsoleteSize = lnObsoleteSize + getObsoleteINSize() + leftoverSize;
  ...
  /**
   * Returns the total number of entries counted. This value is guaranteed to increase whenever the
   * tracking information about a file changes. It is used a key discriminator for FileSummaryLN
   * records.
   */
