 ...
 *
 * <p>In JE test mode (when the JE_TEST system property is set), TRACK_LATCHES will be true and
 * related debugging methods may be used to check the number of Btree latches held.
 *
 * <p>CAPTURE_OWNER is also set to true if the system property JE_CAPTURE_LATCH_OWNER is defined to
 * true. This will capture a stack trace when a latch is acquired exclusively, and the stack trace
 * will be included in all error messages. Capturing the stack trace is expensive so this is off by
...
public class LatchSupport {
  ...
  public static void expectBtreeLatchesHeld(final int expectNLatches, final String msg) {
  ...
  /**
   * Should be called when closing the environment, so that residual latches don't impact another
   * environment that is opened
   */
  ...
  static void trackAcquire(final Latch latch, final LatchContext context) {
    ...
    if (!latchTable.add(latch)) {
      throw unexpectedState("Latch already held." + latch.debugString());
    }
  }
  ...
  static void trackRelease(final Latch latch, final LatchContext context) {
    ...
    if (!latchTable.remove(latch)) {
      throw unexpectedState("Latch not held." + latch.debugString());
    }
  }

  static String toString(
      final Latch latch, final LatchContext context, final OwnerInfo lastOwnerInfo) {
    final StringBuilder builder = new StringBuilder();
    builder
        .append(context.getLatchName())
  ...
  static EnvironmentFailureException handleTimeout(final Latch latch, final LatchContext context) {
    ...
    LoggerUtils.logMsg(
        logger, envImpl, Level.SEVERE, "Thread dump follows for latch timeout: " + msg);

    LoggerUtils.fullThreadDump(logger, envImpl, Level.SEVERE);

    return unexpectedState(envImpl, "Latch timeout. " + msg);
  }
}
