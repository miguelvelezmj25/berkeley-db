 ...
 *
 * <p>SharedLatch (not just Latch) is implemented to support exclusive-only BIN latches.
 */
...
public class LatchImpl extends ReentrantLock implements SharedLatch {
  ...
  public void acquireExclusive() {
    ...
    if (isHeldByCurrentThread()) {
      throw unexpectedState(
          context.getEnvImplForFatalException(), "Latch already held: " + debugString());
    }
  ...
  public void release() {
    if (!isHeldByCurrentThread()) {
      throw unexpectedState(
          context.getEnvImplForFatalException(), "Latch not held: " + debugString());
    }
    if (LatchSupport.TRACK_LATCHES) {
