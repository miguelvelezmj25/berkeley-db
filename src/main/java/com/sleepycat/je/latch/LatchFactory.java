...
public class LatchFactory {
   ...
   *
   * @param exclusiveOnly indicates whether this latch can only be set exclusively (not shared).
   */
  public static SharedLatch createSharedLatch(
      final LatchContext context, final boolean exclusiveOnly) {
    if (exclusiveOnly) {
   ...
   */
  public static SharedLatch createSharedLatch(
      final EnvironmentImpl envImpl, final String name, final boolean exclusiveOnly) {
    if (exclusiveOnly) {
   ...
   *
   * @param collectStats is true to collect stats. If false, a smaller and faster implementation is
   *     used.
   */
   ...
   *     used.
   */
  public static Latch createExclusiveLatch(
      final EnvironmentImpl envImpl, final String name, final boolean collectStats) {
    if (collectStats) {
