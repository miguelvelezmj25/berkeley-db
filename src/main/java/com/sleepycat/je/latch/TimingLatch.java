...
/**
 * A subclass of Latch that may be used for debugging performance issues. This latch can be used in
 * place of an exclusive latch or object mutex in order to see who is waiting for a latch
 * acquisition, how long they're waiting, and who the previous holder was. It crudely writes to
 * System.out, but this can easily be changed to a java.util.Log or EventTrace as desired. You can
 * specify a threshold for the wait and previous holder time (nanos).
 *
 * <p>Note that this class has not recently been used because it is not implemented for shared
 * (Btree) latches. The next time it is used, it should be integrated with the LatchFactory.
 */
