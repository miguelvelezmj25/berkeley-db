...
import static com.sleepycat.je.dbi.SequenceStatDefinition.*;

/**
 * A Sequence handle is used to manipulate a sequence record in a database. Sequence handles are
 * opened using the {@link Database#openSequence Database.openSequence} method.
 */
public class Sequence implements Closeable {
  ...
  private final TransactionConfig autoCommitConfig;
      ...
      boolean isWritableLocker =
          !db.getConfig().getTransactional()
              || (locker.isTransactional()
                  && !DbInternal.getNonNullEnvImpl(db.getEnvironment()).isReplicated());
   ...
   *
   * <p>The sequence handle may not be used again after this method has been called, regardless of
   * the method's success or failure.
   *
  ...
  /**
   * Reads persistent fields from the sequence record. Throws an exception if the key is not present
   * in the database.
   */
