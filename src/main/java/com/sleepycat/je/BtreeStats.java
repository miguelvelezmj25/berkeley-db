...
public class BtreeStats extends DatabaseStats {
   ...
   *
   * <p>The information is included only if the {@link Database#getStats Database.getStats} call was
   * not configured by the {@link com.sleepycat.je.StatsConfig#setFast StatsConfig.setFast} method.
   *
  ...
  /**
   * Returns the number of deleted data records in the database tree that are pending removal by the
   * compressor.
   *
   * <p>The information is included only if the {@link Database#getStats Database.getStats} call was
   * not configured by the {@link com.sleepycat.je.StatsConfig#setFast StatsConfig.setFast} method.
   *
   * @return number of deleted data records in the database tree that are pending removal by the
   *     compressor.
   */
   ...
   * This is calculated without locks or transactions, and therefore is only an accurate count of
   * the current number of records when the database is quiescent.
   *
   * <p>The information is included only if the {@link Database#getStats Database.getStats} call was
   * not configured by the {@link com.sleepycat.je.StatsConfig#setFast StatsConfig.setFast} method.
   *
   * @return number of leaf nodes in the database tree, which can equal the number of records. This
   *     is calculated without locks or transactions, and therefore is only an accurate count of the
   *     current number of records when the database is quiescent.
   */
  ...
  /**
   * Returns an array representing a histogram of the number of Bottom Internal Nodes with various
   * percentages of non-deleted entry counts. The array is 10 elements and each element represents a
   * range of 10%.
   *
   * <pre>
   ...
   * </pre>
   *
   * @return an array representing a histogram of the number of BINs with various percentages of
   *     non-deleted entries.
   */
  ...
  /**
   * Returns the number of latch upgrades (relatches) required while operating on this database's
   * BTree. Latch upgrades are required when an operation assumes that a shared (read) latch will be
  ...
  public String toStringVerbose() {
    return dbImplStats.toStringVerbose() + "\n" + treeStats.toStringVerbose();
  }
}
