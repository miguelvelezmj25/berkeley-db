...
/**
 * TinyHashSet is used to optimize (for speed, not space) the case where a HashSet generally holds
 * one or two elements. This saves us the cost of creating the HashSet and related elements as well
 * as call Object.hashCode(). It was designed for holding the cursors of a BIN, which are often no
 * more than two in number.
 *
 * <p>If (elem1 != null || elem2 != null), they are the only elements in the TinyHashSet. If (set !=
 * null) then only the set's elements are in the TinyHashSet.
 *
 * <p>It should never be true that: (elem1 != null || elem2 != null) and (set != null).
 *
 * <p>This class does not support adding null elements, and only supports a few of the methods in
 * the Set interface.
 */
public class TinyHashSet<T> implements Iterable<T> {
  ...
  public boolean contains(T o) {
    ...
    if (elem1 != null && (elem1 == o || elem1.equals(o))) {
      ...
      return elem2 != null && (elem2 == o || elem2.equals(o));
  }
  ...
  private static class TwoElementIterator<T> implements Iterator<T> {
    ...
    /**
     * Examine elements in the reverse order they were returned, to remove the last returned element
     * when both elements were returned.
     */
    public void remove() {
