...
/**
 * DbLsn is a class that operates on Log Sequence Numbers (LSNs). An LSN is a long comprised of a
 * file number (32b) and offset within that file (32b) which references a unique record in the
 * database environment log. While LSNs are represented as long's, we operate on them using an
 * abstraction and return longs from these methods so that we don't have to worry about the lack of
 * unsigned quantities.
 */
public class DbLsn {
  ...
  /**
   * This flavor of makeLsn is used when the file offset has been stored in 32 bits, as is done in
   * the VLSNBucket.
   */
  ...
  public static int compareTo(long lsn1, long lsn2) {
    if (lsn1 == NULL_LSN || lsn2 == NULL_LSN) {
      throw EnvironmentFailureException.unexpectedState(
          "NULL_LSN lsn1=" + getNoFormatString(lsn1) + " lsn2=" + getNoFormatString(lsn2));
    }
  ...
  /**
   * Return the logsize in bytes between these two LSNs. This is an approximation; the logs might
   * actually be a little more or less in size. This assumes that no log files have been cleaned.
   */
  ...
  /**
   * Return the logsize in bytes between these two LSNs. This is an approximation; the logs might
   * actually be a little more or less in size. This assumes that log files might have been cleaned.
   */
  public static long getWithCleaningDistance(
      long thisLsn, long otherLsn, long logFileSize, FileManager fileManager) {
    long diff = 0;
  ...
  private static long calcDiff(
      long fileDistance, long logFileSize, long laterLsn, long earlierLsn) {
   ...
   * Assumes that all files in the LSN range are currently protected from cleaner deletion, e.g.,
   * during recovery. Uses File.length and does not perturb the FileManager's file handle cache.
   */
  public static long getTrueDistance(
      final long thisLsn, final long otherLsn, final FileManager fileManager) {
