...
public class PropUtil {
  ...
  /**
   * Converts the given duration (interval value plus unit) to milliseconds, ensuring that any given
   * value greater than zero converts to at least one millisecond to avoid a zero millisecond
   * result, since Object.wait(0) waits forever.
   *
   * @throws IllegalArgumentException if the duration argument is illegal. Thrown via API setter
   *     methods such as Transaction.setLockTimeout.
   */
   ...
   *
   * @throws IllegalArgumentException if the duration string is illegal. Thrown via the Environment
   *     ctor and setMutableConfig, and likewise for a ReplicatedEnvironment.
   */
  ...
  public static long parseDurationNS(final String property) {
    ...
    try {
      ...
      throw new IllegalArgumentException(
          "Duration argument does not start with a long integer: " + property);
    }
    ...
    if (tokens.hasMoreTokens()) {
      ...
      if (tokens.hasMoreTokens()) {
        throw new IllegalArgumentException(
            "Duration argument has extra characters after unit: " + property);
      }
      try {
        ...
        try {
          ...
          throw new IllegalArgumentException(
              "Duration argument has unknown unit name: " + property);
        }
      }
    } else {
  ...
  /**
   * Support for conversion of IEEE time units. Although names are defined in uppercase, we
   * uppercase the input string before calling IEEETimeUnit.valueOf, in order to support input names
