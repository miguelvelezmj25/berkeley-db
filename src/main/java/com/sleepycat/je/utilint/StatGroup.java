...
/**
 * The Stats infrastructure provides context for JE statistics. Each statistic has these attributes:
 * - metadata - specifically, a name and description - each statistic is associated with a parent
 * stat group, which itself has a name and description. - support for the StatsConfig.clear
 * semantics - a way to print statistics in a user friendly way.
 *
 * <p>To create a statistic variable, instantiate one of the concrete subclasses of Stat. Each
 * concrete subclass should hold the methods that are needed to best set and display the value. For
 * example, instead of using LongStat to hold a timestamp or LSN value, use TimestampStat or
 * LSNStat. A Stat instance needs to specify a StatDefinition. There may be multiple Stat variables
 * in different components that share a StatDefinition. They are differentiated when displayed by
 * their parent StatGroup.
 *
 * <p>Each Stat instance is associated with a StatGroup, which holds the collection of stats that
 * belong to a given component. Each member of the StatGroup has a unique StatDefinition. StatGroups
 * can be combined, in order to accumulate values. For example, the LockManager may have multiple
 * lock tables. Each lock table keeps its own latch statistics. When LockStats are generated, the
 * StatsGroup for each latch is collected and rolled up into a single StatGroup, using the
 * addAll(StatGroup) method.
 *
 * <p>The Stats infrastructure is for internal use only. Public API classes like EnvironmentStats,
 * LockStats, etc, contain StatGroups. A call to retrieve stats is implemented by getting a clone of
 * the StatGroups held by the components like the cleaner, the incompressor, the LockManager, etc.
 * The public API classes provide getter methods that reach into the StatGroups to return the
 * specific stat value.
 *
 * <p>To add a statistic, create the Stat variable in the component where it is being used and
 * associate it with a StatGroup. The Stat infrastructure does the rest of the work for plumbing
 * that statistic up to the public API class. Each API class must provide a getter method to access
 * the specific statistic. Currently, this is done manually.
 */
public class StatGroup implements Serializable {
  ...
  private StatGroup(
      String groupName, String groupDescription, Map<StatDefinition, Stat<?>> values) {
  ...
  /**
   * Returns a synchronized, unmodifiable view of the stats in this group. Note that the returned
   * set can still be modified by other threads, so the caller needs to take that into account.
   */
  ...
  /**
   * Add all the stats from the other group into this group. If both groups have the same stat, add
   * the values. The caller must make sure that no stats are added to or removed from the argument
   * during this call.
   *
   * @throws ConcurrentModificationException if the addition or removal of stats in the argument is
   *     detected
   */
  ...
  /**
   * The caller must make sure that no stats are added to or removed from this stat group while this
   * method is being called.
   *
   * @throws ConcurrentModificationException if the addition or removal of stats in this group is
   *     detected
   */
  @SuppressWarnings("unchecked")
  ...
  public int getInt(StatDefinition definition) {
    ...
    if (s == null) {
      ...
      assert false
          : "Internal error calling getInt with"
