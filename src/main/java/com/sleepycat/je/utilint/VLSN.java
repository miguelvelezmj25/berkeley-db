...
public class VLSN implements Loggable, Comparable<VLSN>, Serializable {
  ...
  /**
   * Return true if this VLSN's sequence directly follows the "other" VLSN. This handles the case
   * where "other" is a NULL_VLSN.
   */
  public boolean follows(VLSN other) {
    return ((other.isNull() && sequence == 1)
        || ((!other.isNull()) && (other.getSequence() == (sequence - 1))));
  }

  /**
   * Compares this VLSN's sequence with the specified VLSN's sequence for order. Returns a negative
   * integer, zero, or a positive integer as this sequence is less than, equal to, or greater than
   ...
   * @return the smaller of {@code a} and {@code b}, ignoring NULL_VLSN unless both are NULL_VLSN
