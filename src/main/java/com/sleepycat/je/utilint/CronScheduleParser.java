...
/**
 * This class aims to parser {@link EnvironmentConfig#VERIFY_SCHEDULE} which is a cron-style
 * expression.
 *
 * <p>The cron-style expression can be very complicate, for example containing *, ?, / and so on.
 * But now we only handle the simplest situation. We will continually add the code to handle more
 * complicated situations if needed in the future.
 *
 * <p>Constraint for current version:
 * <li>The standard string should be "* * * * *", i.e. there are 5 fields and 4 blank space
 * <li>Each filed can only be an int value or *.
 * <li>Can not specify dayOfMonth and dayOfWeek simultaneously
 * <li>Can not specify dayOfMonth or month. Because if so, we will need to consider the days of that
 *     month and furthermore whether that year is leap year for February. The difference of the
 *     number of days for each month make it complicate to calculate the delay and the interval.
 * <li>If the field is an int value, then the value should be in the correct range, i.e.
 *     minute(0-59), hour(0-23), dayOfWeek(0-6), where dayOfMonth(1-31) and month(1-12) can not be
 *     specified.
 * <li>If dayOfWeek is a concrete value, then minute or hour can not be '*'. For example, we can not
 *     use "0 * * * 5", i.e. we can not specify that we want the verifier to run every hour only on
 *     Friday. Because it may be complicate to calculate the stop time point and at least we need to
 *     add a variable.
...
public class CronScheduleParser {
  private static final String errorMess =
      "The style of " + EnvironmentConfig.VERIFY_SCHEDULE + " is not right. ";
  ...
  /**
   * The construction function will first validate the cron-style string and then parser the string
   * to get the interval of the cron-style task represented by the string and to get the
   * wait-time(delay) to first time start the cron-style task.
   *
  ...
  /**
   * Check whether two cron-style strings are same, i.e. both are null or the content of the two
   * strings are same.
   *
  ...
  /**
   * @return delay The wait-time to first time start the cron-style task represented by the
   *     cron-style string.
   */
  ...
  /** @return interval The interval of the cron-style task represented by the cron-style string. */
    ...
    if (!timeArray[4].equals("*")) {
      ...
      if (dayOfWeek < 0 || dayOfWeek > 6) {
        throw new IllegalArgumentException(
            errorMess + cons5 + "The day of the week should" + "be (0-6).");
      }
    }
