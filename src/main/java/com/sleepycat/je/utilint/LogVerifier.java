...
/**
 * Verifies the checksums in the contents of a log file in a JE {@code Environment}.
 *
 * <p>The caller supplies the contents of the log file by passing arrays of bytes in a series of
 * calls to the {@link #verify} method, which verifies the checksums for log records, and by calling
 * the {@link #verifyAtEof} when the entire contents are complete, to detect incomplete entries at
 * the end of the file. The primary intended use of this class is to verify the contents of log
 * files that are being copied as part of a programmatic backup. It is critical that invalid files
 * are not added to a backup set, since then both the live environment and the backup will be
 * invalid.
 *
...
public class LogVerifier {
   ...
   *     LogVerificationException}. This should be a simple file name of the form {@code
   *     NNNNNNNN.jdb}, where NNNNNNNN is the file number in hexadecimal format.
  ...
  /**
   * Creates a log verifier for use with an internal environment. If {@code fileNum} is less than
   * zero, it is derived from {@code fileName}.
   *
   * @param envImpl the {@code EnvironmentImpl} associated with the log
   * @param fileName the file name of the log, for reporting in the {@code
   *     LogVerificationException}. This should be a simple file name of the form {@code
   ...
   */
  public LogVerifier(final EnvironmentImpl envImpl, final String fileName, final long fileNum) {
    this.envImpl = envImpl;
    this.fileName = fileName;
    this.fileNum = (fileNum >= 0) ? fileNum : envImpl.getFileManager().getNumFromName(fileName);
    state = State.INIT;
    entryStart = 0L;
    prevEntryStart = 0L;
     ...
     */
    headerBuf =
        ByteBuffer.allocate(Math.max(LogEntryHeader.MAX_HEADER_SIZE, FileHeader.entrySize()));
   ...
   * @throws LogVerificationException if a checksum cannot be verified or a log entry is determined
   *     to be invalid by examining its contents
   * @throws EnvironmentFailureException if an unexpected, internal or environment-wide failure
   *     occurs
   */
  ...
  /**
   * Checks that the log file ends with a complete log entry, after having completed verifying the
   * log file contents through calls to {@link #verify}.
   *
   * @throws LogVerificationException if the stream does not end with a complete log entry
   * @throws EnvironmentFailureException if an unexpected, internal or environment-wide failure
   *     occurs
   */
  ...
  /**
   * Initializes all state variables before the start of a log entry. Moves the state to
   * FIXED_HEADER, the first part of a log entry.
   */
  ...
  /**
   * Processes the fixed initial portion of a log entry. After all bytes for the fixed portion are
   * read, moves the state to VARIABLE_HEADER if the header contains a variable portion, or to ITEM
   * if it does not.
   */
  private int processFixedHeader(final byte[] buf, final int curOffset, final int remaining)
      throws LogVerificationException {

    assert header == null;

    final int maxSize = LogEntryHeader.MIN_HEADER_SIZE;
    final int processSize = Math.min(remaining, maxSize - headerBuf.position());
    ...
    if (headerBuf.position() == maxSize) {
      headerBuf.flip();
      try {
        header = new LogEntryHeader(headerBuf, logVersion, DbLsn.makeLsn(fileNum, entryStart));
      } catch (ChecksumException e) {
        throw newVerifyException("Invalid header bytes=" + Arrays.toString(headerBuf.array()), e);
      }
  ...
  /**
   * Processes the variable portion of a log entry. After all bytes for the variable portion are
   * read, moves the state to ITEM.
   */
  private int processVariableHeader(final byte[] buf, final int curOffset, final int remaining) {
    assert header != null;
  ...
  private int processFileHeaderItem(final byte[] buf, final int curOffset, final int remaining)
      throws LogVerificationException {
  ...
  /**
   * Processes the item portion of a log entry. After all bytes for the item are read, moves the
   * state back to INIT and bumps the entryStart.
   */
  private int processItem(final byte[] buf, final int curOffset, final int remaining)
      throws LogVerificationException {
  ...
  private LogVerificationException newVerifyException(String reason, Throwable cause) {
    ...
    if (header != null) {
      logEntrySize = String.valueOf(header.getSize() + header.getItemSize());
    } else {
