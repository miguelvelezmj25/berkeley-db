...
public class JVMSystemUtils {
  ...
  private static final String ZING_ACCESS_ERROR =
      "Could not access Zing management bean." + " Make sure -XX:+UseZingMXBeans was specified.";
  ...
  /**
   * Returns the max amount of memory in the heap available, using an approach that depends on the
   * JVM vendor, OS, etc.
   *
   * <p>May return Long.MAX_VALUE if there is no inherent limit.
   */
  public static long getRuntimeMaxMemory() {
    ...
    if (ZING_JVM) {
      try {
        ...
        final Method getMaxMemoryMethod =
            beanClass.getMethod("getApplicationObjectHeapUsableMemory");
  ...
  /**
   * Returns the size of the System Zing Memory pool. This is the max memory for all running Zing
   * JVMs.
   */
  public static long getSystemZingMemorySize() {
    try {
      ...
      final List<?> pools = (List<?>) getPoolsMethod.invoke(null);
