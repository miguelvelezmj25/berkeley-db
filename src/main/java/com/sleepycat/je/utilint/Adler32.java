 ...
 *
 * <p>This class is used rather than the native java.util.zip.Adler32 class because we have seen a
 * JIT problem when calling the Adler32 class using the Server JVM on Linux and Solaris.
 * Specifically, we suspect this may be Bug Parade number 4965907. See SR [#9376]. We also believe
 * that this bug is fixed in Java 5 and therefore only use this class conditionally if we find that
 * we're in a 1.4 JVM. [#13354].
 *
 * <p>The Adler32 checksum is discussed in RFC1950. The sample implementation from this RFC is shown
 * below:
 *
 * <pre>
 ...
 * </pre>
 *
 * The NMAX optimization is so that we don't have to do modulo calculations on every iteration. NMAX
 * is the max number of additions to make before you have to perform the modulo calculation.
 */
  ...
  public static class ChunkingAdler32 extends java.util.zip.Adler32 {
    ...
    public void update(byte[] b, int off, int len) {
      ...
      while (i < len) {
        ...
        int nBytesThisChunk = Math.min(bytesRemaining, adler32ChunkSize);
