 ...
 * Reports 0 if the value is greater than the stat value. The computed difference remains valid for
 * a specified amount of time, which should represent the maximum amount of time expected to elapse
 * between when new values are provided. If no value is specified within the validity interval, then
 * the difference is recomputed using the current base stat value and the last specified value. The
 * idea is to treat the specified value as up-to-date for a certain period of time, and then
 * represent that the lack of updates means it is falling behind.
 */
public class LongDiffStat extends MapStatComponent<Long, LongDiffStat> {
  ...
  /** The previous value, or 0. Synchronize on this instance when accessing this field. */
  private long prevValue;

  /**
   * The time in milliseconds of the previous value, or 0. Synchronize on this instance when
   * accessing this field.
   */
  private long prevTime;

  /** The last computed difference, or 0. Synchronize on this instance when accessing this field. */
  private long diff;
   ...
   * @param validityMillis the amount of time, in milliseconds, which a computed difference remains
   *     valid
   */
