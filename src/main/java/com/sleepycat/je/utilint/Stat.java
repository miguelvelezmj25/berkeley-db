...
/**
 * Base class for all JE statistics. A single Stat embodies a value and definition. See StatGroup
 * for a description of how to create and display statistics.
 *
 * <p>Note that Stat intentionally does not contain the statistics value itself. Instead, the
 * concrete subclass will implement the value as the appropriate primitive type. That's done to
 * avoid wrapper classes like Integer and Long, and to keep the overhead of statistics low.
 */
public abstract class Stat<T> extends BaseStat<T> implements Cloneable {
  ...
  /**
   * Creates an instance without registering it with the owning group, for creating copies without
   * using clone. For constructing an unregistered instance.
   */
