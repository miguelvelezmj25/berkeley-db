...
public abstract class MapStatComponent<T, C extends MapStatComponent<T, C>> extends BaseStat<T> {
  ...
  /**
   * Returns the value of the statistic as a formatted string, either using or not using commas as
   * requested. Implementations should make sure that the result does not contain commas when
   * useCommas is false, because the value will be used in a comma-separated value file, where
   * embedded commas would cause problems.
   *
