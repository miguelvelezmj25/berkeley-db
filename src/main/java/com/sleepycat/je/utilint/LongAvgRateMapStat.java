...
/**
 * A JE stat that maintains a map of individual {@link LongAvgRate} values which can be looked up
 * with a String key, and that returns results as a formatted string.
 */
public final class LongAvgRateMapStat extends MapStat<Long, LongAvgRate> {
  ...
  /**
   * The time the last stat was removed. This value is used to determine which entries should be
   * included when calling computeInterval. Synchronize on this instance when accessing this field.
   */
  private long removeStatTimestamp;

  /**
   * Creates an instance of this class. The definition type must be INCREMENTAL.
   *
   ...
   */
  public LongAvgRateMapStat(
      StatGroup group, StatDefinition definition, long periodMillis, TimeUnit reportTimeUnit) {
  ...
  public synchronized LongAvgRate createStat(String key) {
    assert key != null;
    final LongAvgRate stat =
        new LongAvgRate(definition.getName() + ":" + key, periodMillis, reportTimeUnit);
    statMap.put(key, stat);
    return stat;
  }

  /**
   * Note the removal time, so that computeInterval can tell if an empty map is newer than a
   * non-empty one.
   */
  @Override
  ...
  /**
   * Creates a new map that contains entries for all keys that appear in whichever of this map or
   * the argument is newer, with those entries updated with any values from both maps. Treats this
   * map as newest if both have the same timestamp. This method does not compute negative intervals,
   * since negation does not work properly for this non-additive stat. The base argument must be a
   * LongAvgRateMapStat.
   */
  @Override
  public LongAvgRateMapStat computeInterval(Stat<String> base) {
    ...
    final LongAvgRateMapStat baseCopy = (LongAvgRateMapStat) base.copy();
  ...
  /**
   * Update this map to reflect changes from the argument, including merging latest changes,
   * removing entries not in the argument, and adding ones not in this instance.
   */
  private synchronized LongAvgRateMapStat updateLatest(final LongAvgRateMapStat latest) {

    synchronized (latest) {
      for (final Iterator<Entry<String, LongAvgRate>> i = statMap.entrySet().iterator();
          i.hasNext(); ) {
        final Entry<String, LongAvgRate> e = i.next();
        final LongAvgRate latestStat = latest.statMap.get(e.getKey());
        if (latestStat != null) {
      ...
      for (final Entry<String, LongAvgRate> e : latest.statMap.entrySet()) {
  ...
  /**
   * Returns the most recent time any component stat was modified, including the time of the latest
   * stat removal.
   */
