...
public abstract class DaemonThread implements DaemonRunner, Runnable {
  ...
  /**
   * If run is true, starts the thread if not started or unpauses it if already started; if run is
   * false, pauses the thread if started or does nothing if not started.
   *
   * <p>Note that no thread is created unless run is true at some time. That way, threads are
   * conserved in cases where the app wants to run their own threads. This can be important when
   * many JE envs are in the same process, in which case a shared cache is often used.
   */
  ...
  /**
   * If Daemon Thread throws errors and exceptions, this function will catch it and throw a
   * EnvironmentFailureException, and fail the test.
   *
   * <p>Only used during testing.
   */
  public boolean checkErrorListener(Throwable e) {
    if (Boolean.getBoolean(ERROR_LISTENER)) {
      ...
      new EnvironmentFailureException(
          ...
          "Daemon thread failed during testing",
          e);
    }
  ...
  /**
   * Returns whether shutdown has been requested. This method should be used to to terminate daemon
   * loops.
   */
  ...
  /**
   * Returns whether the daemon is currently paused/disabled. This method should be used to to
   * terminate daemon loops.
   */
  ...
  /**
   * Returns whether the onWakeup method is currently executing. This is only an approximation and
   * is used to avoid unnecessary wakeups.
   */
