...
/**
 * An Environment that implements XAResource. If JE is used in an XA environment, this class should
 * be used instead of Environment so that appropriate XA functions are available.
 */
public class XAEnvironment extends Environment implements XAResource {
  ...
  public Transaction getXATransaction(Xid xid) {
    ...
    if (ret == null || ret instanceof PreparedTxn) {
      return null;
    }
  ...
  public void setXATransaction(Xid xid, Transaction txn) {
    getNonNullEnvImpl().getTxnManager().registerXATxn(xid, txn.getTxn(), false);
  }
  ...
  public boolean isSameRM(XAResource rm) throws XAException {
    ...
    return envImpl == DbInternal.getNonNullEnvImpl((XAEnvironment) rm);
  }
  ...
  public void start(Xid xid, int flags) throws XAException {
    ...
    try {
      ...
      if (flags == XAResource.TMNOFLAGS) {
        ...
        if (txnMgr.getTxnForThread() != null || txn.getPrepared()) {
          throw new XAException(XAException.XAER_PROTO);
        }
      } else if (tmResume) {
        if (txn == null) {
