 ...
 *
 * <p>TODO: Need a description of the recovery algorithm here. For some related information, see the
 * Checkpointer class comments.
 *
 * <p>Recovery, the INList and Eviction ================================= There are two major steps
 * in recovery: 1) recover the mapping database and the INs for all other databases, 2) recover the
 * LNs for the other databases. In the buildTree method, step 1 comes before the call to buildINList
 * and step 2 comes after that. The INList is not maintained in step 1.
 *
 * <p>The INList is not maintained in the step 1 because there is no benefit -- we cannot evict
 * anyway as explained below -- and there are potential drawbacks to maintaining it: added
 * complexity and decreased performance. The drawbacks are described in more detail further below.
 *
 * <p>Even if the INList were maintained in step 1, eviction could not be enabled until step 2,
 * because logging is not allowed until all the INs are in place. In principle we could evict
 * non-dirty nodes in step 1, but since recovery is dirtying the tree as it goes, there would be
 * little or nothing that is non-dirty and could be evicted.
 *
 * <p>Therefore, the INList has an 'enabled' mode that is initially false (in step 1) and is set to
 * true by buildINList, just before step 2. The mechanism for adding nodes to the INList is skipped
 * when it is disabled. In addition to enabling it, buildINList populates it from the contents of
 * the Btrees that were constructed in step 1. In step 2, eviction is invoked explicitly by calling
 * EnvironmentImpl.invokeEvictor often during recovery. This is important since the background
 * evictor thread is not yet running.
 *
 * <p>An externally visible limitation created by this situation is that the nodes placed in the
 * Btree during step 1 must all fit in memory, since no eviction is performed. So memory is a
 * limiting factor in how large a recovery can be performed. Since eviction is allowed in step 2,
 * and step 2 is where the bulk of the recovery is normally performed, this limitation of step 1
 * hasn't been a critical problem.
 *
 * <p>Maintaining the INList ---------------------- In this section we consider the impact of
 * maintaining the INList in step 1, if this were done in a future release. It is being considered
 * for a future release so we can rely on the INList to reference INs by node ID in the in-memory
 * representation of an IN (see the Big Memory SR [#22292]).
 *
 * <p>To maintain the INList in step 1, when a branch of a tree (a parent IN) is spliced in, the
 * previous branch (all of the previous node's children) would have to be removed from the INList.
 * Doing this incorrectly could cause an OOME, and it may also have a performance impact.
 *
 * <p>The performance impact of removing the previous branch from the INList is difficult to
 * estimate. In the normal case (recovery after normal shutdown), very few nodes will be replaced
 * because normally only nodes at the max flush level are replayed, and the slots they are placed
 * into will be empty (not resident). Here is description of a worst case scenario, which is when
 * there is a crash near the end of a checkpoint:
 *
 * <p>+ The last checkpoint is large, includes all nodes in the tree, is mostly complete, but was
 * not finished (no CkptEnd). The middle INs (above BIN and below the max flush level) must be
 * replayed (see Checkpointer and Provisional.BEFORE_CKPT_END).
 *
 * <p>+ For these middle INs, the INs at each level are placed in the tree and replace any IN
 * present in the slot. For the bottom-most level of middle INs (level 2), these don't replace a
 * node (the slot will be empty because BINs are never replayed). But for the middle INs in all
 * levels above that, they replace a node that was fetched earlier; it was fetched because it is the
 * parent of a node at a lower level that was replayed.
 *
 * <p>+ In the worst case, all INs from level 3 to R-1, where R is the root level, would be replayed
 * and replace a node. However, it seems the replaced node would not have resident children in the
 * scenario described, so the cost of removing it from the INList does not seem excessive.
 *
 * <p>+ Here's an example illustrating this scenario. The BINs and their parents (as a sub-tree) are
 * logged first, followed by all dirty INs at the next level, etc.
 *
 * <p>0050 CkptStart 0100 BIN level 1 0200 BIN level 1 ... 1000 IN level 2, parent of 0100, 0200,
 * etc. 1100 BIN level 1 1200 BIN level 1 ... 2000 IN level 2, parent of 1100, 1200, etc. ... 7000
 * IN level 2, last level 2 IN logged 8000 IN level 3, parent of 1000, 2000, etc. ... 9000 IN level
 * 4, parent of 8000, etc. ...
 *
 * <p>9000 level 4 / ----8000---- level 3 / / \ 1000 2000 ...... level 2
 *
 * <p>BINs not shown
 *
 * <p>Only the root (if it happens to be logged right before the crash) is non-provisional. We'll
 * assume in this example that the root was not logged. Level 2 through R-1 are logged as
 * Provisional.BEFORE_CKPT_END, and treated as non-provisional (replayed) by recovery because there
 * is no CkptEnd.
 *
 * <p>When 1000 (and all other nodes at level 2) is replayed, it is placed into an empty slot.
 *
 * <p>When 8000 (and all other INs at level 3 and higher, below the root) is replayed, it will
 * replace a resident node that was fetched and placed in the slot when replaying its children. The
 * replaced node is one not shown, and assumed to have been logged sometime prior to this
 * checkpoint. The replaced node will have all the level 2 nodes that were replayed earlier (1000,
 * 2000, etc.) as its resident children, and these are the nodes that would have to be removed from
 * the INList, if recovery were changed to place INs on the INList in step 1.
 *
 * <p>So if the INs were placed on the INList, in this worst case scenario, all INs from level 3 to
 * R-1 will be replayed, and all their immediate children would need to be removed from the INList.
 * Grandchildren would not be resident. In other words, all nodes at level 2 and above (except the
 * root) would be removed from the INList and replaced by a node being replayed.
 *
 * <p>When there is a normal shutdown, we don't know of scenarios that would cause this sort of
 * INList thrashing. So perhaps maintaining the INList in step 1 could be justified, if the
 * additional recovery cost after a crash is acceptable.
 *
 * <p>Or, a potential solution for the worst case scenario above might be to place the resident
 * child nodes in the new parent, rather than discarding them and removing them from the INList.
 * This would have the benefit of populating the cache and not wasting the work done to read and
 * replay these nodes. OTOH, it may cause OOME if too much of the tree is loaded in step 1.
 */
public class RecoveryManager {
  ...
  /**
   * Look for an existing log and use it to create an in memory structure for accessing existing
   * databases. The file manager and logging system are only available after recovery.
   *
  ...
  public RecoveryInfo recover(boolean readOnly) throws DatabaseException {
    ...
    try {
      ...
      if (fileManager.filesExist()) {
        ...
        String endOfLogMsg =
            "Recovery underway, valid end of log = "
       ...
       */
      if (!readOnly
          && ((envImpl.getLogManager().getLastLsnAtRecovery() != info.checkpointEndLsn)
              || forceCheckpoint)) {
        ...
        startupTracker.setStats(
            Phase.CKPT, envImpl.getCheckpointer().loadStats(StatsConfig.DEFAULT));
        ...
        envImpl
            .getCheckpointer()
            .initIntervals(
                info.checkpointStartLsn, info.checkpointEndLsn, System.currentTimeMillis());
      }
    } catch (IOException e) {
      LoggerUtils.traceAndLogException(
          envImpl, "RecoveryManager", "recover", "Couldn't recover", e);
  ...
  /**
   * Find the end of the log, initialize the FileManager. While we're perusing the log, return the
   * last checkpoint LSN if we happen to see it.
   */
  private void findEndOfLog(boolean readOnly) throws IOException, DatabaseException {
    ...
    assert (reader.getLastValidLsn() != reader.getEndOfLog())
        : "lastUsed="
            + DbLsn.getNoFormatString(reader.getLastValidLsn())
            + " end="
    ...
    envImpl
        .getFileManager()
        .setLastPosition(info.nextAvailableLsn, info.lastUsedLsn, reader.getPrevOffset());
    startupTracker.stop(Phase.FIND_END_OF_LOG);
  }

  /**
   * Find the last checkpoint and establish the firstActiveLsn point, checkpoint start, and
   * checkpoint end.
   */
  private void findLastCheckpoint() throws IOException, DatabaseException {
    ...
    if (info.checkpointEndLsn == DbLsn.NULL_LSN) {
      ...
      CheckpointEnd checkpointEnd =
          (CheckpointEnd) (envImpl.getLogManager().getEntry(info.checkpointEndLsn));
  ...
  /**
   * Should be called when performing operations that may add to the cache, but only after all INs
   * are in place and buildINList has been called.
   */
  ...
  /**
   * Perform two passes for the INs of a given level. Root INs must be processed first to account
   * for splits/compressions that were done during/after a checkpoint [#14424] [#24663].
   *
   * <p>Splits and compression require logging up to the root of the tree, to ensure that all INs
   * are properly returned to the correct position at recovery. In other words, splits and
   * compression ensure that the creation and deletion of all nodes is promptly logged.
   *
   * <p>However, checkpoints are not propagated to the top of the tree, in order to conserve on
   * logging. Because of that, a great-aunt situation can occur, where an ancestor of a given node
   * can be logged without referring to the latest on-disk position of the node, because that
   * ancestor was part of a split or compression.
   *
   * <p>Take this scenario: Root-A / \ IN-B IN-C / / | \ BIN-D / LN-E
   *
   * <p>1) LN-E is logged, BIN-D is dirtied 2) BIN-D is logged during a checkpoint, IN-B is dirtied
   * 3) IN-C is split and Root-A is logged 4) We recover using Root-A and the BIN-D logged at (2) is
   * lost
   *
   * <p>At (3) when Root-A is logged, it points to an IN-B on disk that does not point to the most
   * recent BIN-D
   *
   * <p>At (4) when we recover, although we will process the BIN-D logged at (2) and splice it into
   * the tree, the Root-A logged at (3) is processed last and overrides the entire subtree
   * containing BIN-D
   *
   * <p>This could be addressed by always logging to the root at every checkpoint. Barring that, we
   * address it by replaying the root INs first, and then all non-root INs.
   *
   * <p>It is important that no IN is replayed that would cause a fetch of an older IN version which
   * has been replaced by a newer version in the checkpoint interval. If the newer version were
   * logged as the result of log cleaning, and we attempt to fetch the older version, this would
   * cause a LOG_FILE_NOT_FOUND exception. The replay of the root INs in the first pass is safe,
   * because it won't cause a fetch [#24663]. The replay of INs in the second pass is safe because
   * only nodes at maxFlushLevel were logged non-provisionally and only these nodes are replayed.
   *
   ...
   */
  private void readRootINsAndTrackIds(Counter counter) throws DatabaseException {
  ...
  private void readRootINs(Counter counter) throws DatabaseException {
  ...
  private void readNonRootINs(boolean mappingTree, Counter counter) throws DatabaseException {
   ...
   *
   * <p>inFromLog should be latched upon entering this method and it will not be latched upon
   * exiting.
   *
   * @param inFromLog - the new node to put in the tree. The identifier key and node ID are used to
   *     find the existing version of the node.
  ...
  private void recoverIN(DatabaseImpl db, IN inFromLog, long logLsn) throws DatabaseException {
    ...
    try {
      ...
      LoggerUtils.traceAndLogException(
          ...
          "RecoveryManager",
          "recoverIN",
          " lsnFromLog: " + DbLsn.getNoFormatString(logLsn) + " " + trace,
          e);
      ...
      if (LatchSupport.TRACK_LATCHES) {
        LatchSupport.expectBtreeLatchesHeld(
            0, "LSN = " + DbLsn.toString(logLsn) + " inFromLog = " + inFromLog.getNodeId());
      }
    }
  }

  /**
   * If the root of this tree is null, use this IN from the log as a root. Note that we should
   * really also check the LSN of the mapLN, because perhaps the root is null because it's been
   * deleted. However, the replay of all the LNs will end up adjusting the tree correctly.
   *
   * <p>If there is a root, check if this IN is a different LSN and if so, replace it.
   */
  private void recoverRootIN(DatabaseImpl db, IN inFromLog, long lsn) throws DatabaseException {
    ...
    try {
      ...
      throw new EnvironmentFailureException(
          envImpl,
          EnvironmentFailureReason.LOG_INTEGRITY,
          "lsnFromLog=" + DbLsn.getNoFormatString(lsn),
          e);
    } finally {
   ...
   *
   * <p>Note that this method never inserts a slot for an IN, it only replaces the node in a slot
   * under certain conditions. Insertion of slots is unnecessary because splits are logged all the
   * way to the root, and therefore inserted slots are always visible via the parent node. In fact,
   * it is critical that splits are not allowed during this phase of recovery, because that might
   * require splits and logging is not allowed until the INs are all in place.
   */
  private void recoverChildIN(
      DatabaseImpl db, IN inFromLog, long logLsn, List<TrackingInfo> trackingList)
      throws DatabaseException {
   ...
   * belong to a committed txn AND (2) aren't part of a prepared txn AND (3) shouldn't be
   * resurrected as part of a replication ReplayTxn.
   *
   * <p>LNs that are part of a rollback period do need to be undone, but in a different way from the
   * other LNs. They are rolledback and take a different path.
   *
   * <p>To find these LNs, walk the log backwards, using log entry commit record to create a
   * collection of committed txns. If we see a log entry that doesn't fit the criteria above, undo
   * it.
   *
   * @param firstUndoPass is true if this is the first time that undoLNs is called. This is a little
   *     awkward, but is done to explicitly indicate to the rollback tracker that this is the
   *     tracker's construction phase. During this first pass, RollbackStart and RollbackEnd are in
   *     the target log types, and the rollback period map is created. We thought that it was better
   *     to be explicit than to reply on checking the logTypes parameter to see if
   *     RollbackStart/RollbackEnd is included.
   */
  private void undoLNs(Set<LogEntryType> logTypes, boolean firstUndoPass, Counter counter)
      throws DatabaseException {
    ...
    try {
      ...
      while (reader.readNextEntry()) {
        ...
        if (reader.isLN()) {
          ...
          committedTxnIds.put(reader.getTxnCommitId(), reader.getLastLsn());
          counter.incNumAux();
          ...
          throw EnvironmentFailureException.unexpectedState(
              envImpl, "LNreader should not have picked up type " + reader.dumpCurrentHeader());
        }
      } /* while */
      counter.setRepeatIteratorReads(reader.getNRepeatIteratorReads());
  ...
  /**
   * Uncommitted, unaborted LNs that belong to a replicated txn are resurrected rather than undone.
   * This means that the LN is also replicated.
   */
  ...
  /**
   * When recovering a replicated environment, all uncommitted, replicated transactions are
   * resurrected much the same way as a prepared transaction. If the node turns out to be a new
   * master, by definition those txns won't resume, and the code path for new master setup will
   * abort these transactions. If the node is a replica, the transactions will either resume or
   * abort depending on whether the originating master is alive or not.
   */
  ...
  /**
   * The entry just read is a prepare record. Setup a PrepareTxn that will exempt any of its
   * uncommitted LNs from undo. Instead, uncommitted LNs that belong to a PrepareTxn are redone.
   */
   ...
   * </pre>
   *
   * @param location holds state about the search in the tree. Passed in from the recovery manager
   *     to reduce objection creation overhead.
   * @param logLsn is the LSN from the just-read log entry
   *     <p>Undo can take place for regular recovery, for aborts, and for recovery rollback
   *     processing. Each flavor has some slight differences, and are factored out below.
   */
  private void recoveryUndo(
      TreeLocation location, DatabaseImpl db, LNLogEntry<?> lnEntry, long logLsn) {
    ...
    undo(
        ...
        false /*revertPD*/,
        lnEntry.getAbortKey(),
        lnEntry.getAbortData(),
        lnEntry.getAbortVLSN(),
        lnEntry.getAbortExpiration(),
        lnEntry.isAbortExpirationInHours());
  }
  ...
  public static void rollbackUndo(
    ...
    undo(
        ...
        revertTo.revertData,
        revertTo.revertVLSN,
        revertTo.revertExpiration,
        revertTo.revertExpirationInHours);
  }
  ...
  /**
   * Redo all LNs that should be revived. That means - all committed LNs - all prepared LNs - all
   * uncommitted, replicated LNs on a replicated node.
   */
  private void redoLNs(Set<LogEntryType> lnTypes, Counter counter) throws DatabaseException {
      ...
      while (reader.readNextEntry()) {
        ...
        if (db == null) {
          ...
          tracker.countObsoleteIfUncounted(
              logrecLsn, logrecLsn, null, reader.getLastEntrySize(), dbId, false /*trackOffset*/);
  ...
  /**
   * These categories of LNs are redone: - LNs from committed txns between the ckpt start and end of
   * log - non-transactional LNs between the ckpt start and end of log - LNs from prepared txns
   * between the first active LSN and end of log - LNs from replicated, uncommitted, unaborted txns
   * between the first active LSN and end of log that are NOT invisible.
   *
   * <p>LNs that are in a rollback part of the log are invisible and will not be redone.
   */
  ...
  private void trackReservedFileRecords(LNLogEntry<?> logrec) {
    ...
    if (!LogEntryType.LOG_RESERVED_FILE_LN.equals(logrec.getLogType()) || logrec.isDeleted()) {
      return;
    }

    Long file = ReservedFileInfo.entryToKey(new DatabaseEntry(logrec.getKey()));

    ReservedFileInfo info = ReservedFileInfo.entryToObject(new DatabaseEntry(logrec.getData()));
   ...
   * active txn.
   */
  private void relock(Txn txn, long logLsn, LNLogEntry<?> logrec, DatabaseImpl db)
      throws DatabaseException {
    ...
    final LockResult result =
        txn.nonBlockingLock(logLsn, LockType.WRITE, false /*jumpAheadOfWaiters*/, db);

    if (result.getLockGrant() == LockGrantType.DENIED) {
      throw EnvironmentFailureException.unexpectedState(
          "Resurrected lock denied txn="
              + txn.getId()
              + " logLsn="
    ...
    result.setAbortInfo(
        logrec.getAbortLsn(),
        logrec.getAbortKnownDeleted(),
        logrec.getAbortKey(),
        logrec.getAbortData(),
        logrec.getAbortVLSN(),
        logrec.getAbortExpiration(),
        logrec.isAbortExpirationInHours(),
        db);
    ...
    if (wli == null) {
      throw EnvironmentFailureException.unexpectedState(
          "Resurrected lock has no write info txn="
              + txn.getId()
              + " logLsn="
              + DbLsn.getNoFormatString(logLsn)
              + " abortLsn="
   ...
   *
   * <p>Let R and T be the record and locker associated with the current logrec L. Let TL be the
   * logrec pointed to by the BIN slot for R (if any) just before the call to the redo() method on
   * R. Let TT be the locker that wrote TL.
   *
   * <p>R slot found | L vs TL | L is | action in tree | | deletion | --------------+--------
   * +----------+------------------------ Y | L <= TL | | no action
   * --------------+---------+----------+------------------------ Y | L > TL | N | replace w/log LSN
   * --------------+---------+----------+------------------------ Y | L > TL | Y | replace w/log
   * LSN, put | | | on compressor queue --------------+---------+----------+------------------------
   * N | n/a | N | insert into tree --------------+---------+----------+------------------------ N |
   * n/a | Y | no action --------------+---------+----------+------------------------
   *
   * @param location Used to return to the caller info about the R slot found in the tree (if any)
   *     before the slot is updated by this method. It is allocated once by the caller and reset by
   *     this method; this way the overhead of object creation is avoided.
   * @param logrec the LN logrec L that is being redone.
   * @param logrecLsn the LSN of L.
   * @param logrecSize the on-disk size of L.
   * @return the LSN found in the tree, or NULL_LSN if the tree did not contain any slot for the
   *     record.
   */
  private long redo(
      DatabaseImpl db,
    ...
    boolean blindInsertions =
        (configManager.getBoolean(EnvironmentParams.BIN_DELTA_BLIND_OPS)
            && eligible.isCommitted()
    ...
    try {
      ...
      if (foundNotKD) {
        ...
        index =
            bin.insertEntry1(
                null /*ln*/, logrecKey, logrecData, logrecLsn, true /*blindInsertion*/);
        ...
        if (!isDeletion) {
          ...
          index =
              bin.insertEntry1(null, logrecKey, logrecData, logrecLsn, false /*blindInsertion*/);
   ...
   *
   * <p>Let R and T be the record and txn associated with the current logrec L. Let TL be the logrec
   * pointed to by the BIN slot for R (if any) just before the call to the redo() method on R. Let
   * TT be the txn that wrote TL. Let AL by the logrec whose LSN is stored as the abortLSN in L (if
   * L.abortLsn != NULL).
   *
   * <p>This method considers whether L, TL, or AL should be counted as obsolete, and if so, it does
   * the counting.
   *
   * @param logrec The deserialized logrec L that is being processed.
   * @param logrecSize The on-disk size of L.
   * @param logrecLsn The LSN of L.
   * @param treeLsn The LSN of TL. Will be NULL_LSN if there was no R slot in the BTree, or the R
   *     slot was a KD slot with a NULL_LSN.
   * @param treeLsnIsImmediatelyObsolete True if (a) the DB is a dups DB with all immediatelly
   *     obsolete LNs, or (b) treeLsn is NULL_LSN, or (c) TL is an embedded logrec (as indicated by
   *     the embedded flag in the slot).
   * @param treeLNLoggedSize The on-disk size of TL
   * @param commitLsn The commitLSN of T, if T is a Txn that did commit.
   * @param isCommitted True if T is non-transactional or a Txn that did commit.
   * @param db The DatabaseImpl obj for the DB containing R.
   *     <p>There are cases where we do not count the previous version of an LN as obsolete when
   *     that obsolete LN occurs prior to the recovery interval. This happens when a later version
   *     of the LN is current in the tree because its parent IN has been flushed non-provisionally
   *     after it. The old version of the LN is not in the tree so we never encounter it during
   *     recovery and cannot count it as obsolete. For example:
   *     <p>100 LN-A checkpoint occurred (ckpt begin, flush, ckpt end) 200 LN-A 300 BIN parent of
   *     200 400 IN parent of 300, non-provisional via a sync
   *     <p>no utilization info is flushed no checkpoint crash and recover
   *     <p>200 is the current LN-A in the tree. When we redo 200 we do not count anything as
   *     obsolete because the log and tree LSNs are equal. 100 is never counted obsolete because it
   *     is not in the recovery interval.
   *     <p>The same thing occurs when a deleted LN is replayed and the old version is not found in
   *     the tree because it was compressed and the IN was flushed non-provisionally.
   *     <p>In these cases we may be able to count the abortLsn as obsolete but that would not work
   *     for non-transactional entries.
   */
  private void redoUtilizationInfo(
    ...
    if (logrec.isImmediatelyObsolete(db)) {
      tracker.countObsoleteIfUncounted(
          logrecLsn, logrecLsn, null, logrecSize, db.getId(), false /*trackOffset*/);
    }
    ...
    if (commitLsn != DbLsn.NULL_LSN
      ...
      if (treeLsn == DbLsn.NULL_LSN
        ...
        tracker.countObsoleteIfUncounted(
            abortLsn, commitLsn, null, 0, db.getId(), true /*trackOffset*/);
      }
    }
  }
   ...
   *
   * <p>Let R and T be the record and txn associated with the current logrec L. L is for sure
   * obsolete. It may or may have been counted as such already. Consider the following cases:
   *
   * <p>1. L is an immediately obsolete logrec. Then, L was counted as obsolete during normal
   * processing and it should be counted here only if its obsoleteness was not made durable before
   * the crash, i.e., if it is after the latest FSLN for the containing log file. No need to record
   * L's LSN in the tracker, because L is immediately obsolete.
   *
   * <p>2. L is not an immediately obsolete logrec.
   *
   * <p>2.1. L is the last logrec for R by T. In this case, L was not counted as obsolete during
   * normal processing. L is made obsolete here by the fact that it is undone.
   *
   * <p>2.2 L is not the last logrec for R by T. In this case, L was counted as obsolete during
   * normal processing it should be counted here only if its obsoleteness was not made durable
   * before the crash.
   *
   * <p>Unfortunately, we cannot differentiate between cases 2.1 and 2.2, so the code below calls
   * tracker.countObsoleteUnconditional() for both of those cases, which can result to some double
   * counting in case 2.2. However, it is not very common for a txn to update the same record
   * multiple times, so this should not be a big issue.
   */
  private void undoUtilizationInfo(
      LNLogEntry<?> logrec, DatabaseImpl db, long logrecLsn, int logrecSize) {
    ...
    if (logrec.isImmediatelyObsolete(db)) {
      ...
      tracker.countObsoleteUnconditional(
          logrecLsn, null /*logEntryType*/, logrecSize, db.getId(), true /*trackOffset*/);
    }
  }
  ...
  /** Build the in memory inList with INs that have been made resident by the recovery process. */
  ...
  /**
   * Remove all temporary databases that were encountered as MapLNs during recovery undo/redo. A
   * temp DB needs to be removed when it is not closed (closing a temp DB removes it) prior to a
   * crash. We ensure that the MapLN for every open temp DBs is logged each checkpoint interval.
   */
  ...
  /**
   * For committed truncate/remove NameLNs with no corresponding deleted MapLN found, delete the
   * MapLNs now. MapLNs are deleted by truncate/remove operations after logging the Commit, so it is
   * possible that a crash occurs after logging the Commit of the NameLN, and before deleting the
   * MapLN. [#20816]
   */
  ...
  private void checkLogVersion8UpgradeViolations() throws EnvironmentFailureException {
    ...
    if (v8DupNodes || v8Deltas) {
      ...
      throw EnvironmentFailureException.unexpectedState(
          illegalEntries
              + " were found in the recovery interval. "
   ...
   * enabled. This is used to construct verbose trace messages for individual log entry processing.
   */
  ...
  private void traceAndThrowException(long badLsn, String method, Exception originalException)
    ...
    LoggerUtils.traceAndLogException(
        envImpl, "RecoveryManager", method, "last LSN = " + badLsnString, originalException);
    throw new EnvironmentFailureException(
        envImpl,
        EnvironmentFailureReason.LOG_INTEGRITY,
