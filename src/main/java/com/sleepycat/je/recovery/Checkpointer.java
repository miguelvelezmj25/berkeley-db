 ...
 * Checkpoint flushes must be done in ascending order from the bottom of the tree up.
 *
 * <p>Checkpoint and IN Logging Rules ------------------------------- The checkpoint must log, and
 * make accessible via non-provisional ancestors, all INs that are dirty at CkptStart. If we crash
 * and recover from that CkptStart onward, any IN that became dirty (before the crash) after the
 * CkptStart must become dirty again as the result of replaying the action that caused it to
 * originally become dirty.
 *
 * <p>Therefore, when an IN is dirtied at some point in the checkpoint interval, but is not logged
 * by the checkpoint, the log entry representing the action that dirtied the IN must follow either
 * the CkptStart or the FirstActiveLSN that is recorded in the CkptEnd entry. The FirstActiveLSN is
 * less than or equal to the CkptStart LSN. Recovery will process LNs between the FirstActiveLSN and
 * the end of the log. Other entries are only processed from the CkptStart forward. And provisional
 * entries are not processed.
 *
 * <p>Example: Non-transactional LN logging. We take two actions: 1) log the LN and then 2) dirty
 * the parent BIN. What if the LN is logged before CkptStart and the BIN is dirtied after CkptStart?
 * How do we avoid breaking the rules? The answer is that we log the LN while holding the latch on
 * the parent BIN, and we don't release the latch until after we dirty the BIN. The construction of
 * the checkpoint dirty map requires latching the BIN. Since the LN was logged before CkptStart, the
 * BIN will be dirtied before the checkpointer latches it during dirty map construction. So the BIN
 * will always be included in the dirty map and logged by the checkpoint.
 *
 * <p>Example: Abort. We take two actions: 1) log the abort and then 2) undo the changes, which
 * modifies (dirties) the BIN parents of the undone LNs. There is nothing to prevent logging
 * CkptStart in between these two actions, so how do we avoid breaking the rules? The answer is that
 * we do not unregister the transaction until after the undo phase. So although the BINs may be
 * dirtied by the undo after CkptStart is logged, the FirstActiveLSN will be prior to CkptStart.
 * Therefore, we will process the Abort and replay the action that modifies the BINs.
 *
 * <p>Exception: Lazy migration. The log cleaner will make an IN dirty without logging an action
 * that makes it dirty. This is an exception to the general rule that actions should be logged when
 * they cause dirtiness. The reasons this is safe are: 1. The IN contents are not modified, so there
 * is no information lost if the IN is never logged, or is logged provisionally and no ancestor is
 * logged non-provisionally. 2. If the IN is logged non-provisionally, this will have the side
 * effect of recording the old LSN as being obsolete. However, the general rules for checkpointing
 * and recovery will ensure that the new version is used in the Btree. The new version will either
 * be replayed by recovery or referenced in the active Btree via a non-provisional ancestor.
 *
 * <p>Checkpoint Algorithm TODO update this -------------------- The final checkpointDirtyMap field
 * is used to hold (in addition to the dirty INs) the state of the checkpoint and highest flush
 * levels. Access to this object is synchronized so that eviction and checkpointing can access it
 * concurrently. When a checkpoint is not active, the state is CkptState.NONE and the dirty map is
 * empty. When a checkpoint runs, we do this:
 *
 * <p>1. Get set of files from cleaner that can be deleted after this checkpoint. 2. Set
 * checkpointDirtyMap state to DIRTY_MAP_INCOMPLETE, meaning that dirty map construction is in
 * progress. 3. Log CkptStart 4. Construct dirty map, organized by Btree level, from dirty INs in
 * INList. The highest flush levels are calculated during dirty map construction. Set
 * checkpointDirtyMap state to DIRTY_MAP_COMPLETE. 5. Flush INs in dirty map. + First, flush the
 * bottom two levels a sub-tree at a time, where a sub-tree is one IN at level two and all its BIN
 * children. Higher levels (above level two) are logged strictly by level, not using subtrees. o If
 * je.checkpointer.highPriority=false, we log one IN at a time, whether or not the IN is logged as
 * part of a subtree, and do a Btree search for the parent of each IN. o If
 * je.checkpointer.highPriority=true, for the bottom two levels we log each sub-tree in a single
 * call to the LogManager with the parent IN latched, and we only do one Btree search for each level
 * two IN. Higher levels are logged one IN at a time as with highPriority=false. + The Provisional
 * property is set as follows, depending on the level of the IN: o level is max flush level:
 * Provisional.NO o level is bottom level: Provisional.YES o Otherwise (middle levels):
 * Provisional.BEFORE_CKPT_END 6. Flush VLSNIndex cache to make VLSNIndex recoverable. 7. Flush
 * UtilizationTracker (write FileSummaryLNs) to persist all tracked obsolete offsets and utilization
 * summary info, to make this info recoverable. 8. Log CkptEnd 9. Delete cleaned files from step 1.
 * 10. Set checkpointDirtyMap state to NONE.
 *
 * <p>Per-DB Highest Flush Level -------------------------- As mentioned above, when the dirty map
 * is constructed we also determine the highest flush level for each database. This is the maximum
 * Btree level at which a dirty node exists in the DB.
 *
 * <p>When logging a node below the maxFlushLevel, we add the parent to the dirty map. It may or may
 * not have been added when the dirty map was constructed. The idea is to flush all ancestors of all
 * nodes in the dirty map, up to and including the maxFlushLevel, even if those ancestors were not
 * dirty when the dirty map was constructed.
 *
 * <p>This is done to avoid orphaning a dirty node as shown in this example.
 *
 * <p>IN-A (root level=4) / \ (d) IN-B IN-C (maxFlushLevel=3) \ (d) IN-D
 *
 * <p>IN-C is not dirty (d) when the dirty map is constructed, but it will be logged because its
 * child (IN-D) is dirty, and it is not above maxFlushLevel.
 *
 * <p>If IN-C were not logged, and there were a crash after the checkpoint, the changes to IN-D
 * would be lost. IN-D would not be replayed by recovery because it is logged provisionally, and it
 * would not be accessible via its parent. This is because only nodes at maxFlushLevel are logged
 * non-provisionally. The actions that led to the changes in IN-D may not be replayed either,
 * because they may appear before the firstActiveLsn associated with the checkpoint.
 *
 * <p>When log files are to be deleted at the end of the checkpoint (after being processed by the
 * log cleaner), the maxFlushLevel is increased by one. This is to ensure that LSNs in deleted files
 * will not be fetched during recovery. Such files are in the FileSelector.CLEANED state, which
 * means they have been processed by the cleaner since the last checkpoint.
 *
 * <p>TODO: Document circumstances and motivation for the extra flush level.
 *
 * <p>Lastly, for Database.sync or a checkpoint with MinimizeRecoveryTime configured, we will flush
 * all the way to the root rather than using the maxFlushLevel computed as described above.
 *
 * <p>Provisional.BEFORE_CKPT_END --------------------------- See Provisional.java for a description
 * of the relationship between the checkpoint algorithm above and the BEFORE_CKPT_END property.
 *
 * <p>Coordination of Eviction and Checkpointing ------------------------------------------ Eviction
 * can proceed concurrently with all phases of a checkpoint, and eviction may take place
 * concurrently in multiple threads. This concurrency is crucial to avoid blocking application
 * threads that perform eviction and to reduce the amount of eviction required in application
 * threads.
 *
 * <p>Eviction calls Checkpointer.coordinateEvictionWithCheckpoint, which calls
 * DirtyINMap.coordinateEvictionWithCheckpoint, just before logging an IN.
 * coordinateEvictionWithCheckpoint returns whether the IN should be logged provisionally
 * (Provisional.YES) or non-provisionally (Provisional.NO).
 *
 * <p>Other coordination necessary depends on the state of the checkpoint: + NONE: No additional
 * action. o return Provisional.NO + DIRTY_MAP_INCOMPLETE: The parent IN is added to the dirty map,
 * exactly as if it were encountered as dirty in the INList during dirty map construction. o IN is
 * root: return Provisional.NO o IN is not root: return Provisional.YES + DIRTY_MAP_COMPLETE: o IN
 * level GTE highest flush level: return Provisional.NO o IN level LT highest flush level: return
 * Provisional.YES
 *
 * <p>In general this is designed so that eviction will use the same provisional value that would be
 * used by the checkpoint, as if the checkpoint itself were logging the IN. However, there are
 * several conditions where this is not exactly the case.
 *
 * <p>1. Eviction may log an IN with Provisional.YES when the IN was not dirty at the time of dirty
 * map creation, if it became dirty afterwards. In this case, the checkpointer would not have logged
 * the IN at all. This is safe because the actions that made that IN dirty are logged in the
 * recovery period. 2. Eviction may log an IN with Provisional.YES after the checkpoint has logged
 * it, if it becomes dirty again. In this case the IN is logged twice, which would not have been
 * done by the checkpoint alone. This is safe because the actions that made that IN dirty are logged
 * in the recovery period. 3. An intermediate level IN (not bottom most and not the highest flush
 * level) will be logged by the checkpoint with Provisional.BEFORE_CKPT_END but will be logged by
 * eviction with Provisional.YES. See below for why this is safe. 4. Between checkpoint step 8 (log
 * CkptEnd) and 10 (set checkpointDirtyMap state to NONE), eviction may log an IN with
 * Provisional.YES, although a checkpoint is not strictly active during this interval. See below for
 * why this is safe.
 *
 * <p>It is safe for eviction to log an IN as Provisional.YES for the last two special cases,
 * because this does not cause incorrect recovery behavior. For recovery to work properly, it is
 * only necessary that:
 *
 * <p>+ Provisional.NO is used for INs at the max flush level during an active checkpoint. +
 * Provisional.YES or BEFORE_CKPT_END is used for INs below the max flush level, to avoid replaying
 * an IN during recovery that may depend on a file deleted as the result of the checkpoint.
 *
 * <p>You may ask why we don't use Provisional.YES for eviction when a checkpoint is not active.
 * There are two reason, both related to performance:
 *
 * <p>1. This would be wasteful when an IN is evicted in between checkpoints, and that portion of
 * the log is processed by recovery later, in the event of a crash. The evicted INs would be ignored
 * by recovery, but the actions that caused them to be dirty would be replayed and the INs would be
 * logged again redundantly. 2. Logging a IN provisionally will not count the old LSN as obsolete
 * immediately, so cleaner utilization will be inaccurate until the a non-provisional parent is
 * logged, typically by the next checkpoint. It is always important to keep the cleaner from
 * stalling and spiking, to keep latency and throughput as level as possible.
 *
 * <p>Therefore, it is safe to log with Provisional.YES in between checkpoints, but not desirable.
 *
 * <p>Although we don't do this, it would be safe and optimal to evict with BEFORE_CKPT_END in
 * between checkpoints, because it would be treated by recovery as if it were Provisional.NO. This
 * is because the interval between checkpoints is only processed by recovery if it follows the last
 * CkptEnd, and BEFORE_CKPT_END is treated as Provisional.NO if the IN follows the last CkptEnd.
 *
 * <p>However, it would not be safe to evict an IN with BEFORE_CKPT_END during a checkpoint, when
 * logging of the IN's ancestors does not occur according to the rules of the checkpoint. If this
 * were done, then if the checkpoint completes and is used during a subsequent recovery, an obsolete
 * offset for the old version of the IN will mistakenly be recorded. Below are two cases where
 * BEFORE_CKPT_END is used correctly and one showing how it could be used incorrectly.
 *
 * <p>1. Correct use of BEFORE_CKPT_END when the checkpoint does not complete.
 *
 * <p>050 BIN-A 060 IN-B parent of BIN-A 100 CkptStart 200 BIN-A logged with BEFORE_CKPT_END 300
 * FileSummaryLN with obsolete offset for BIN-A at 050 Crash and recover
 *
 * <p>Recovery will process BIN-A at 200 (it will be considered non-provisional) because there is no
 * following CkptEnd. It is therefore correct that BIN-A at 050 is obsolete.
 *
 * <p>2. Correct use of BEFORE_CKPT_END when the checkpoint does complete.
 *
 * <p>050 BIN-A 060 IN-B parent of BIN-A 100 CkptStart 200 BIN-A logged with BEFORE_CKPT_END 300
 * FileSummaryLN with obsolete offset for BIN-A at 050 400 IN-B parent of BIN-A, non-provisional 500
 * CkptEnd Crash and recover
 *
 * <p>Recovery will not process BIN-A at 200 (it will be considered provisional) because there is a
 * following CkptEnd, but it will process its parent IN-B at 400, and therefore the BIN-A at 200
 * will be active in the tree. It is therefore correct that BIN-A at 050 is obsolete.
 *
 * <p>3. Incorrect use of BEFORE_CKPT_END when the checkpoint does complete.
 *
 * <p>050 BIN-A 060 IN-B parent of BIN-A 100 CkptStart 200 BIN-A logged with BEFORE_CKPT_END 300
 * FileSummaryLN with obsolete offset for BIN-A at 050 400 CkptEnd Crash and recover
 *
 * <p>Recovery will not process BIN-A at 200 (it will be considered provisional) because there is a
 * following CkptEnd, but no parent IN-B is logged, and therefore the IN-B at 060 and BIN-A at 050
 * will be active in the tree. It is therefore incorrect that BIN-A at 050 is obsolete.
 *
 * <p>This last case is what caused the LFNF in SR [#19422], when BEFORE_CKPT_END was mistakenly
 * used for logging evicted BINs via CacheMode.EVICT_BIN. During the checkpoint, we evict BIN-A and
 * log it with BEFORE_CKPT_END, yet neither it nor its parent are part of the checkpoint. After
 * being counted obsolete, we crash and recover. Then the file containing the BIN (BIN-A at 050
 * above) is cleaned and deleted. During cleaning, it is not migrated because an obsolete offset was
 * previously recorded. The LFNF occurs when trying to access this BIN during a user operation.
 *
 * <p>CacheMode.EVICT_BIN ------------------- Unlike in JE 4.0 where EVICT_BIN was first introduced,
 * in JE 4.1 and later we do not use special rules when an IN is evicted. Since concurrent eviction
 * and checkpointing are supported in JE 4.1, the above rules apply to EVICT_BIN as well as all
 * other types of eviction.
 */
public class Checkpointer extends DaemonThread implements EnvConfigObserver {

  /**
   * For unit testing only. Called before we flush the max level. This field is static because it is
   * called from the static flushIN method.
   */
  ...
  /**
   * The DirtyINMap for checkpointing is created once and is reset after each checkpoint is
   * complete. Access to this object is synchronized so that eviction and checkpointing can access
   * it concurrently.
   */
  ...
  public Checkpointer(EnvironmentImpl envImpl, long waitTime, String name) {
    ...
    logSizeBytesInterval =
        envImpl.getConfigManager().getLong(EnvironmentParams.CHECKPOINTER_BYTES_INTERVAL);
  ...
  public void envConfigUpdate(DbConfigManager cm, EnvironmentMutableConfig ignore) {
    highPriority = cm.getBoolean(EnvironmentParams.CHECKPOINTER_HIGH_PRIORITY);
  }

  /**
   * Initializes the checkpoint intervals when no checkpoint is performed while opening the
   * environment.
   */
  ...
  /**
   * Coordinates an eviction with an in-progress checkpoint and returns whether provisional logging
   * is needed.
   *
   * @return the provisional status to use for logging the target.
   */
  public Provisional coordinateEvictionWithCheckpoint(
      final DatabaseImpl db, final int targetLevel, final IN parent) {

    return checkpointDirtyMap.coordinateEvictionWithCheckpoint(db, targetLevel, parent);
  }
   ...
   *
   * @throws IllegalArgumentException via Environment ctor and setMutableConfig.
   */
  ...
  protected void onWakeup() {
    ...
    doCheckpoint(CheckpointConfig.DEFAULT, "daemon", true /*invokedFromDaemon*/);

    wakeupAfterNoWrites = false;
  }

  /**
   * Wakes up the checkpointer if a checkpoint log interval is configured and the number of bytes
   * written since the last checkpoint exceeds the size of the interval.
   */
  public void wakeupAfterWrite() {
    ...
    if ((logSizeBytesInterval != 0) && !isRunning()) {
      ...
      if (DbLsn.getNoCleaningDistance(nextLsn, lastCheckpointStart, logFileMax)
          >= logSizeBytesInterval) {
  ...
  /**
   * Wakes up the checkpointer if a checkpoint is needed to reclaim disk space for already cleaned
   * files. This method is called after an idle period with no writes.
   */
  ...
  private boolean isRunnable(CheckpointConfig config) {
    ...
    try {
      ...
      if (useBytesInterval != 0) {
         ...
         */
        final long lastUsedLsn = envImpl.getFileManager().getLastUsedLsn();
  ...
  /**
   * The real work to do a checkpoint. This may be called by the checkpoint thread when waking up,
   * or it may be invoked programatically through the api.
   *
   * @param invokingSource a debug aid, to indicate who invoked this checkpoint. (i.e. recovery, the
   *     checkpointer daemon, the cleaner, programatically)
   */
  public synchronized void doCheckpoint(
      CheckpointConfig config, String invokingSource, boolean invokedFromDaemon) {
    ...
    final CheckpointStartCleanerState cleanerState = cleaner.getFilesAtCheckpointStart();
    ...
    try {
      ...
      final SingleItemEntry<CheckpointStart> startEntry =
          SingleItemEntry.create(
              LogEntryType.LOG_CKPT_START, new CheckpointStart(checkpointId, invokingSource));

      final long checkpointStart = logManager.log(startEntry, ReplicationContext.NO_REPLICATE);
      ...
      /* Flush IN nodes. */
      flushDirtyNodes(envImpl, checkpointDirtyMap, checkpointStart, highPriority, flushStats);
      ...
      envImpl
          ...
          .flushFileUtilization(envImpl.getUtilizationTracker().getTrackedFiles());

      final DbTree dbTree = envImpl.getDbTree();
      final boolean willDeleteFiles = !cleanerState.isEmpty();

      final CheckpointEnd ckptEnd =
          new CheckpointEnd(
              invokingSource,
              checkpointStart,
              envImpl.getRootLsn(),
              ...
              dbTree.getLastLocalDbId(),
              dbTree.getLastReplicatedDbId(),
      ...
      lastCheckpointInterval =
          DbLsn.getNoCleaningDistance(checkpointStart, lastCheckpointStart, logFileMax);
      ...
      lastCheckpointEnd =
          logManager.logForceFlush(
              endEntry, true /*fsyncRequired*/, ReplicationContext.NO_REPLICATE);
      ...
      LoggerUtils.logMsg(
          ...
          "Ckpt id="
              ...
              + " success="
              + success
              + " aborted because of disk limit violation: "
              + e);
      ...
      LoggerUtils.traceAndLogException(
          envImpl, "Checkpointer", "doCheckpoint", "checkpointId=" + checkpointId, e);
      throw e;
    } finally {
  ...
  /**
   * Flush a given database to disk. Like checkpoint, log from the bottom up so that parents
   * properly represent their children.
   */
  public void syncDatabase(EnvironmentImpl envImpl, DatabaseImpl dbImpl, boolean flushLog) {
    ...
    try {
      ...
      if (dirtyMap.getNumEntries() > 0) {
        ...
        flushDirtyNodes(
            envImpl, dirtyMap, DbLsn.NULL_LSN /*ckptStart*/, false /*highPriority*/, fstats);
      ...
      LoggerUtils.traceAndLogException(
          envImpl, "Checkpointer", "syncDatabase", "of " + dbImpl.getDebugName(), e);
      throw e;
    } finally {
      dirtyMap.reset();
    }
   ...
   * specified, we need only cascade up to the highest level set at the start of checkpointing.
   *
   * <p>Note that all but the top level INs are logged provisionally. That's because we don't need
   * to process lower INs during recovery because the higher INs will end up pointing at them.
   */
  private static void flushDirtyNodes(
    ...
    try {
      while (dirtyMap.getNumLevels() > 0) {
        ...
        while (true) {
          final CheckpointReference targetRef = dirtyMap.removeNextNode(currentLevel);
           ...
           */
          final DatabaseImpl db = dbTree.getDb(targetRef.dbId, -1 /*lockTimeout*/, dbCache);

          if (db != null && !db.isDeleted()) {

            /* Flush if we're below maxFlushLevel. */
            final int maxFlushLevel = dirtyMap.getHighestFlushLevel(db);
   ...
   * Flush the target IN.
   *
   * <p>Where applicable, also attempt to flush the subtree that houses this target, which means we
   * flush the siblings of this target to promote better cleaning throughput. The problem lies in
   * the fact that provisionally logged nodes are not available for log cleaning until their parent
   * is logged non-provisionally. On the other hand, we want to log nodes in provisional mode as
   * much as possible, both for recovery performance, and for correctness to avoid fetches against
   * cleaned log files. (See [#16037].) These conflicting goals are reconciled by flushing nodes in
   * subtree grouping, because writing the non-provisional parent of a set of provisionally written
   * nodes frees the cleaner to work on that set of provisional nodes as soon as possible. For
   * example, if a tree consists of:
   *
   * <p>INa +------+-------+ INb INc +-----+----+ +-----+ BINd BINe BINf BINg BINh
   *
   * <p>It is more efficient for cleaning throughput to log in this order: BINd, BINe, BINf, INb,
   * BINg, BINh, INc, INa rather than: BINd, BINe, BINf, BINg, BINh, INb, INc, INa
   *
   * <p>Suppose the subtree in question is INb->{BINd, BINe, BINf}
   *
   * <p>Suppose we see BINd in the dirty map first, before BINe and BINf. - flushIN(BINd) is called
   * - we fetch and latch its parent, INb
   *
   * <p>If this is a high priority checkpoint, we'll hold the INb latch across the time it takes to
   * flush all three children. In flushIN(BINd), we walk through INb, create a local map of all the
   * siblings that can be found in the dirty map, and then call logSiblings with that local map.
   * Then we'll write out INb.
   *
   * <p>If high priority is false, we will not hold the INb latch across multiple IOs. Instead, we -
   * write BINd out, using logSiblings - while still holding the INb latch, we create a list of
   * dirty siblings - release the INb latch - call flushIN() recursively on each entry in the local
   * sibling map, which will result in a search and write of each sibling. These recursive calls to
   * flushIN are called with the allowLogSubtree parameter of false to halt the recursion and
   * prevent a repeat of the sibling examination. - write INb
   */
  private static void flushIN(
    ...
    if (targetRef.isRoot) {
      ...
      final RootFlusher flusher = new RootFlusher(db, targetRef.nodeId);
    ...
    try {
       ...
       */
      final boolean bottomLevelTarget = ((parentLevel & IN.LEVEL_MASK) == 2);
      ...
      if (!result.exactParentFound) {
        if (parentLevel > targetLevel) {
          dirtyMap.addIN(
              parent, -1 /*index*/, false /*updateFlushLevels*/, true /*updateMemoryBudget*/);
        }
        return;
      }
      ...
      for (int i = 0; i < parent.getNEntries(); i += 1) {
        ...
        final CheckpointReference childRef = dirtyMap.removeNode(targetLevel, childLsn, childId);
      ...
      if (parentLevel <= maxFlushLevel) {
        parentRef = dirtyMap.removeNode(parentLevel, parent.getLastLoggedLsn(), parent.getNodeId());
      } else {
        parentRef = null;
      }
    } finally {
    ...
    if (logSiblingsSeparately != null) {
      for (final CheckpointReference childRef : logSiblingsSeparately) {
        flushIN(
            db, childRef, dirtyMap, maxFlushLevel, highPriority, fstats, false /*allowLogSubtree*/);
      }
    }
    ...
    if (parentRef != null) {
      flushIN(
          db, parentRef, dirtyMap, maxFlushLevel, highPriority, fstats, false /*allowLogSubtree*/);
    }
  }

  /**
   * Note that if this method is called, the parent must also be logged. This is true even if this
   * method finds that the child is not dirty. In that case the child has already been flushed
   * (e.g., by eviction) and the parent must be logged according to the rule for max flush level.
   */
  private static void logDirtyIN(
    ...
    if (child != null) {
      ...
      final INLogEntry<BIN> logEntry = ohCache.createBINLogEntryForCheckpoint(parent, index);
      ...
      newLsn = IN.logEntry(logEntry, provisional, true /*backgroundIO*/, parent);
