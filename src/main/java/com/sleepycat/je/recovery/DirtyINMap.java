...
/**
 * Manages the by-level map of checkpoint references that are to be flushed by a checkpoint or
 * Database.sync, the MapLNs to be flushed, the highest level by database to be flushed, and the
 * state of the checkpoint.
 *
 * <p>An single instance of this class is used for checkpoints and has the same lifetime as the
 * checkpointer and environment. An instance per Database.sync is created as needed. Only one
 * checkpoint can occur at a time, but multiple syncs may occur concurrently with each other and
 * with the checkpoint.
 *
 * <p>The methods in this class are synchronized to protect internal state from concurrent access by
 * the checkpointer and eviction, and to coordinate state changes between the two. Eviction must
 * participate in the checkpoint so that INs cascade up properly; see
 * coordinateEvictionWithCheckpoint.
 *
 * <p>When INs are latched along with synchronization on a DirtyINMap, the order must be: 1) IN
 * latches and 2) synchronize on DirtyINMap. For example, the evictor latches the parent and child
 * IN before calling the synchronized method coordinateEvictionWithCheckpoint, and
 * selectDirtyINsForCheckpoint latches the IN before calling the synchronized method
 * selectForCheckpoint.
 */
class DirtyINMap {
  ...
  private final SortedMap<
          Integer, Pair<Map<Long, CheckpointReference>, Map<Long, CheckpointReference>>>
      levelMap;
  ...
  /**
   * Coordinates an eviction with an in-progress checkpoint and returns whether or not provisional
   * logging is needed.
   *
  ...
  synchronized Provisional coordinateEvictionWithCheckpoint(
     ...
     */
    if (ckptState == CkptState.DIRTY_MAP_INCOMPLETE && parent != null) {
     ...
     */
    if (ckptState == CkptState.DIRTY_MAP_COMPLETE && targetLevel < getHighestFlushLevel(db)) {
      return Provisional.YES;
    }
   ...
   *
   * <p>TODO: Is it necessary to perform MapLN flushing for nodes logged by a split (and not just
   * the new sibling)?
   *
  ...
  /**
   * Must be called before starting a checkpoint, and must not be called for Database.sync. Updates
   * memory budget and sets checkpoint state.
   */
  ...
  /**
   * Must be called after a checkpoint or Database.sync is complete. Updates memory budget and
  ...
  /**
   * Scan the INList for all dirty INs, excluding temp DB INs. Save them in a tree-level ordered map
   * for level ordered flushing.
   *
   * <p>Take this opportunity to recalculate the memory budget tree usage.
   *
   * <p>This method itself is not synchronized to allow concurrent eviction. Synchronization is
   * performed on a per-IN basis to protect the data structures here, and eviction can occur in
   * between INs.
   */
  ...
  /**
   * Add the IN to the dirty map if dirty map construction is in progress and the IN is not in a
   * temp DB. If added, the highest flush level map is also updated.
   */
  synchronized void selectForCheckpoint(final IN in, final int index) {
    ...
    addIN(in, index, true /*updateFlushLevels*/, false /*updateMemoryBudget*/);
  }

  /**
   * Adds the the dirty child BINs of the 'in' if dirty map construction is in progress and the IN
   * is not in a temp DB.
   *
   * <p>Main cache resident BINs are added when their parent is encountered in the INList iteration,
   * rather than when the BIN is encountered in the iteration. This is because a BIN can transition
   * between main and off-heap caches during the construction of the dirty map. When a BIN is loaded
   * from off-heap and added to the main cache, it is added to the INList at that time, and such a
   * BIN may not be encountered in the iteration. (ConcurrentHashMap iteration only guarantees that
   * nodes will be encountered if they are present when the iterator is created). So if we relied on
   * encountering BINs in the iteration, some might be missed.
   *
   * <p>Note that this method is not synchronized because it latches the BIN children. IN latching
   * must come before synchronizing on 'this'. The selectForCheckpoint method, which is called after
   * latching the BIN, is synchronized.
   */
  ...
  private void updateFlushLevels(
      Integer level, final DatabaseImpl db, final boolean isBIN, final boolean isRoot) {
  ...
  /**
   * Scan the INList for all dirty INs for a given database. Arrange them in level sorted map for
   * level ordered flushing.
   *
   * <p>This method is not synchronized to allow concurrent eviction. Coordination between eviction
   * and Database.sync is not required.
   */
  void selectDirtyINsForDbSync(DatabaseImpl dbImpl) {
      ...
      if (in.getDatabaseId().equals(dbId)) {
        ...
        try {
          if (in.getInListResident() && in.getDirty()) {
            addIN(in, -1 /*index*/, false /*updateFlushLevels*/, false /*updateMemoryBudget*/);
          }
        } finally {
          in.releaseLatch();
        }
     ...
     * be flushed.
     */
    highestFlushLevels.put(dbImpl, envImpl.getDbTree().getHighestLevel(dbImpl));
  ...
  private synchronized void addCostToMemoryBudget() {
    ...
    final long cost = ((long) numEntries) * MemoryBudget.CHECKPOINT_REFERENCE_SIZE;
   ...
   *
   * @param in is the IN to add, or the parent of an off-heap IN to add when index >= 0.
   * @param index is the index of the off-heap child to add, or -1 to add the 'in' itself.
   * @param updateMemoryBudget if true then update the memory budget as the map is changed; if false
   *     then addCostToMemoryBudget must be called later.
   */
  synchronized void addIN(
      final IN in,
      final int index,
    ...
    Pair<Map<Long, CheckpointReference>, Map<Long, CheckpointReference>> pairOfMaps =
        levelMap.get(level);
    ...
    final CheckpointReference ref =
        new CheckpointReference(db.getId(), nodeId, level, isRoot, idKey, lsn);
  ...
  synchronized CheckpointReference removeNode(final int level, final long lsn, final long nodeId) {

    final Pair<Map<Long, CheckpointReference>, Map<Long, CheckpointReference>> pairOfMaps =
        levelMap.get(level);
  ...
  synchronized CheckpointReference removeNextNode(Integer level) {
    ...
    final Iterator<Map.Entry<Long, CheckpointReference>> iter = map.entrySet().iterator();
  ...
  /**
   * If the given IN is a BIN for the ID mapping database, saves all dirty/temp MapLNs contained in
   * it.
   */
   ...
   *
   * <p>If dirty, a MapLN must be flushed at each checkpoint to record updated utilization info in
   * the checkpoint interval. If it is a temporary DB, the MapLN must be flushed because all temp
   * DBs must be encountered by recovery so they can be removed if they were not closed (and
   * removed) by the user. This method is not synchronized because it takes the Btree root latch,
   * and we must never latch something in the Btree after synchronizing on DirtyINMap; see class
   * comments. Special synchronization is performed for accessing internal state; see below.
   *
   * @param checkpointStart start LSN of the checkpoint in progress. To reduce unnecessary logging,
   *     the MapLN is only flushed if it has not been written since that LSN.
   */
      ...
      for (DatabaseId dbId : mapLNsCopy) {
        ...
        try {
          if (db != null && !db.isDeleted() && db.isCheckpointNeeded()) {
            ...
            dbTree.modifyDbRoot(db, checkpointStart /*ifBeforeLsn*/, true /*mustExist*/);
          }
        } finally {
          dbTree.releaseDb(db);
        }
  ...
  /**
   * Flushes the DB mapping tree root at the end of the checkpoint, if either mapping DB is dirty
   * and the root was not flushed previously during the checkpoint.
   *
   * <p>This method is not synchronized because it does not access internal state. Also, it takes
   * the DbTree root latch and although this latch should never be held by eviction, for consistency
   * we should not latch something related to the Btree after synchronizing on DirtyINMap; see class
   * comments.
   *
   * @param checkpointStart start LSN of the checkpoint in progress. To reduce unnecessary logging,
   *     the Root is only flushed if it has not been written since that LSN.
   */
  ...
  private void traceDirtySet() {
    ...
    LoggerUtils.logMsg(envImpl.getLogger(), envImpl, Level.INFO, sb.toString());
  }
}
