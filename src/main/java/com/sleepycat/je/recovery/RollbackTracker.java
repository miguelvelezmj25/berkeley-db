...
/**
 * RollbackTracker is used to detect rollback periods in the log that are the result of HA replica
 * syncups. These rollback periods affect how LNs should be processed at recovery. Rollbacks differ
 * from aborts in that a rollback returns a LN to its previous version, whether intra or
 * inter-txnal, while an abort always returns an LN to its pre-txn version.
 *
 * <p>What is a Rollback Period? -------------------------- The rollback represents the logical
 * truncation of the log. Any transactional LNs in that rollback period should be undone, even if
 * they are ultimately part of a committed transaction. See the wiki page on Syncup Recovery for the
 * full design. See com.sleepycat.je.rep.impl.node.Replay.rollback for the steps taken at the time
 * of the rollback.
 *
 * <p>A RollbackStart record is logged at the start of any rollback, and a RollbackEnd is logged at
 * the completion of a rollback. RollbackStarts refer to a matchpoint and the area between the
 * matchpoint and the RollbackStart is the rollback period.The RollbackTracker peruses
 * RollbackStarts and Ends and generates a map of the rollback periods.
 *
 * <p>RollbackStarts and their starting Matchpoints can be nested or can be distinct, but several
 * invariants are in place and can be enforced. For example:
 *
 * <p>LSN --- 100 txnA commit
 *
 * <p>200 txnB abort 250 LN for txnC 300 txnC abort .. 400 RollbackStart A (starting matchpoint =
 * 200) 500 RollbackEnd A ... 600 RollbackStart B (starting matchpoint = 200) 700 RollbackStart C
 * (starting matchpoint = 100) 800 RollbackEnd C 900 txnD abort 1000 RollbackStart D (starting
 * matchpoint = 900)
 *
 * <p>This log creates four rollback periods 1) LSN 100 -> 700 (defined by RollbackStart C). This
 * has two rollback periods nested within. 2) LSN 200 -> 400, (defined by RollbackStart A) nested
 * within B 3) LSN 200 -> 600, (defined by RollbackStart B) nested within C 4) LSN 1000 -> 900
 * (defined by RolbackStart D)
 *
 * <p>- There can be no commits or aborts within a rollback period, because we shouldn't have
 * executed a soft recovery that undid a commit or abort. in the rollback period.
 *
 * <p>- There can be no LN_TXs between a RollbackStart and its matching RollbackEnd (should be no LN
 * write operations happening during the syncup.) However, there might be INs written by a
 * checkpoint, and eviction.
 *
 * <p>- The recovery period should never see a RollbackEnd without its matching RollbackStart
 * record, though it is possible to see a RollbackStart that has no RollbackEnd.
 *
 * <p>- There can never be any overlapping, or intersection of periods, because a rollback period is
 * supposed to be like a truncation of the log. Since that log is "gone", a subsequent rollback
 * shouldn't find a matchpoint inside another rollback period.
 *
 * <p>- A child period must be wholly contained between the parent's matchpoint and RollbackStart.
 * This is simply due to the way rollbacks occur. A parent rollback has a Matchpoint <= the child's
 * Matchpoint or it wouldn't be nested. The parent's RollbackStart > the child's RollbackEnd, since
 * the parent occurs after the child in time.
 *
 * <p>The Rollback tracker keeps a list of all the rollback periods. Some are distinct, some are
 * nested.
 *
 * <p>Recovery processing and rollback periods ---------------------------------------- The actions
 * taken at a rollback may not have been made persistent to the log, so at recovery, we literally
 * mimic and replay these two steps: (a) make sure invisible log entries have their invisible bit on
 * and (b) make sure all INs reflect the correct LNs. All use of the rollback periods and tracker
 * take place on the backwards scans. The RollbackStart and End entries are read during the first
 * recovery undo pass When a rollback period is found, a transaction chain is constructed for each
 * transaction that was active in the period, to support a repeat of the actions taken originally.
 *
 * <p>The first undo pass, for the mapping tree, has to construct a map of recovery periods. Since
 * the mapping tree only has MapLNs, and we never write any txnal MapLNs, that first pass does not
 * encounter any txnal LNs. The next two undo passes consult the rollback period map to determine if
 * an LN needs to be rolledback, or just treated like other LNs.
 *
 * <p>Rollback periods that precede the checkpoint start can be ignored, because we can be assured
 * that all the INs and LNs modified by that rollback were made persistent by the checkpoint.
 * Ignoring such periods is required, and is not just an optimization, because it guarantees that we
 * will not need to create a transaction chain that needs to traverse the log beyond the first
 * active lsn. A rollback period precedes the checkpoint if its RollbackEnd is before the checkpoint
 * start.
 *
 * <p>When a rollback period overlaps CkptStart and we recover, we are guaranteed that the undo
 * passes will process all LNs in the rollback period, because they are >= to the firstActiveLEnd of
 * the checkpoint.
 *
 * <p>The lastActiveLSN for the checkpoint will be <= the LSN of the first LN of any transaction
 * that is being rolled back at the time of CkptStart, since these transactions were still active at
 * that time.
 *
 * <p>No file containing a transaction rolled back in the recovery interval, or a file containing
 * the abortLSN of such a transaction, will be deleted by the cleaner. An active transaction
 * prevents cleaning of its first logged entry and beyond. The LN of the abortLSN will be locked,
 * which prevents it from being cleaned.
 *
 * <p>All the work lies on the undo side. Recovery redo only needs to ignore invisible log entries,
 * because we know that the undo pass applied the invisible bit where needed. Note that the undo
 * pass must be sure to write the invisible bits after the pass, before redo attempts to read the
 * log.
 *
 * <p>Each rollback LN_TX belongs to a single rollback period. When periods are nested, the LN_TX
 * belongs to the closest rollback period that encompasses it. Using the example above, a LN at lsn
 * 350 belongs to rollback period A a LN at lsn 550 belongs to rollback period B a LN at lsn 650
 * belongs to rollback period C It uses its rollback period's txn chain to find its previous
 * version.
 */
public class RollbackTracker {
  ...
  void register(RollbackEnd rollbackEnd, long rollbackEndLSN) {
    ...
    if ((underConstructionPeriod != null)
        && (underConstructionPeriod.makeNestedPeriod(rollbackEnd, rollbackEndLSN))) {
      return;
    }

    underConstructionPeriod = new RollbackPeriod(this, rollbackEnd, rollbackEndLSN);
    periodList.add(underConstructionPeriod);
  }

  /**
   * Construction Pass: A RollbackStart is seen. Might be the matching one for the current period,
   * or it might be a new period.
   */
  void register(RollbackStart rollbackStart, long rollbackStartLSN) {
    ...
    if ((underConstructionPeriod != null)
        && (underConstructionPeriod.makeNestedPeriod(rollbackStart, rollbackStartLSN))) {
      return;
    }

    underConstructionPeriod = new RollbackPeriod(this, rollbackStart, rollbackStartLSN);
    periodList.add(underConstructionPeriod);
  }

  /**
   * A TxnCommit showed up on the construction pass. If it's a replicated txn, check if it's in a
   * valid place. It should not be within the rollback period.
   *
   * <p>Omit commits for internal, non-replicated transactions from this check.
   */
  void checkCommit(long commitLSN, long txnId) {
    ...
    if (underConstructionPeriod.contains(commitLSN)) {
      underConstructionPeriod.fail(
          "Commit at " + DbLsn.getNoFormatString(commitLSN) + " is within rollback period.");
    }
  }
  ...
  /**
   * Flip the invisible bit for each lsn in rollbackLsns. Collect the corresponding unique set of
   * file numbers and add them to fileNums.
   */
  private static void setInvisible(
      EnvironmentImpl envImpl, List<Long> rollbackLsns, Set<Long> filesToFsync) {
  ...
  /**
   * At the end of a recovery pass, write out all invisible bits, save a set of file numbers to
   * fsync, and reinitialize the per-pass list for the next round.
   */
  void singlePassSetInvisible() {
    ...
    setInvisible(envImpl, singlePassInvisibleLsns, recoveryFilesToSync);
    singlePassInvisibleLsns = new ArrayList<Long>();
  }
   ...
   *
   * <p>Use inexact counting. Since invisible entries are not processed by the cleaner, recording
   * the obsolete offset would be a waste of resources. Since we don't count offsets, we don't need
   * to worry about duplicate offsets.
   *
   * <p>Some entries may be double counted if they were previously counted obsolete, for example,
   * when multiple versions of an LN were logged. This is tolerated for an exceptional situation
   * like rollback.
   */
  private void countObsolete(long undoLsn, UndoReader undo, RecoveryUtilizationTracker tracker) {
    tracker.countObsoleteUnconditional(
        undoLsn, null /*type*/, undo.logEntrySize, undo.db.getId(), false /*countExact*/);
  }
  ...
  /**
   * A Scanner is to process LNs during a recovery pass. It determines whether this log entry is
   * within the rollback period, and should be accordingly undone or ignored. It serves as a sort of
   * cursor or iterator that works with the rollback tracker.
   */
  abstract class Scanner {
    ...
    /**
     * Return true if this transactional log entry is something that should be rolled back in this
     * rollback period. The Scanner's position can be changed by this call. Update the target field
     * if necessary.
     */
    abstract boolean positionAndCheck(long lsn, long txnId);

    /**
     * Rollback the filereader's current LN_TX. This assumes that the the caller has ascertained
     * that the LN is contained within this rollback period.
     */
  ...
  class UnderConstructionScanner extends Scanner {
    ...
    public boolean positionAndCheck(long lsn, long txnId) {
      ...
      assert underConstructionPeriod.notInRollbackStartAndEnd(lsn, txnId)
          : underConstructionPeriod.bracketFailure(lsn);

      target = underConstructionPeriod.getScannerTarget(lsn);
        return (target != null) && (target.containsLN(lsn, txnId));
    }
  }

  /**
   * In a backward scanner, the currentPeriod field is always pointing to the period that contains
   * this lsn. If the lsn is not in a period, the currentPeriod is the period that is just before
   * this lsn. If there is no period before this lsn, the currentPeriod field is null.
   */
  class BackwardScanner extends Scanner {
    ...
    public boolean positionAndCheck(long lsn, long txnId) {
      ...
      assert currentPeriod.notInRollbackStartAndEnd(lsn, txnId) : currentPeriod.bracketFailure(lsn);
  ...
  static class RollbackPeriod {
    ...
    RollbackPeriod(
        long matchpointLSN, long rollbackStartLSN, long rollbackEndLSN, long checkpointStart) {
      this(
          null,
          matchpointLSN,
          rollbackStartLSN,
     ...
     *
     * @return true if the RollbackEnd belongs to a period nested within the current period. Return
     *     false if the RollbackEnd belongs to new, distinct, different period, and the current
     *     period is closed.
     */
     ...
     *
     * @return true if the RollbackStart belongs to a period nested within the current period, or if
     *     it is the current period. Return false if the RollbackStart belongs to new, distinct,
     *     different period, and this current period is closed.
     */
    ...
    private boolean contained(RollbackEnd foundRBEnd, long foundLSN) {
      ...
      if (DbLsn.compareTo(foundLSN, rollbackStartLSN) >= 0) {
        fail(
            "Should not be two RollbackEnds in a row. "
                + "New RollbackEnd at "
       ...
       */
      if (!((DbLsn.compareTo(foundRBEnd.getMatchpoint(), matchpointLSN) >= 0)
          && (DbLsn.compareTo(foundRBEnd.getRollbackStart(), rollbackStartLSN) < 0))) {
        fail(
            "RollbackEnd intersects current rollback period "
                + foundRBEnd
                + " at "
                + DbLsn.getNoFormatString(foundLSN));
      }
    ...
    private boolean contained(RollbackStart foundRBStart, long foundLSN) {
      ...
      if (!((DbLsn.compareTo(foundRBStart.getMatchpoint(), matchpointLSN) >= 0)
          && (DbLsn.compareTo(foundLSN, rollbackStartLSN) < 0))) {
        fail(
            "RollbackStart intersects current rollback period "
                + foundRBStart
                + " at "
                + DbLsn.getNoFormatString(foundLSN));
      }
    ...
    /**
     * @return true if this RollbackStart entry is the one that is the RollbackStart for this open
     *     period.
     */
    ...
    private void makeChild(RollbackEnd foundRBEnd, long foundLSN) {
      currentChild = new RollbackPeriod(tracker, foundRBEnd, foundLSN);
      children.add(currentChild);
    }

    private void makeChild(RollbackStart foundRBStart, long foundLSN) {
      currentChild = new RollbackPeriod(tracker, foundRBStart, foundLSN);
      children.add(currentChild);
    }

    /**
     * Return the period that should own this foundRBEnd. That may be either a nested period, or
     * this period.
     */
    RollbackPeriod getNewPeriodTarget(RollbackEnd foundRBEnd, long foundLSN) {
      if (currentChild != null) {
        final RollbackPeriod target = currentChild.getNewPeriodTarget(foundRBEnd, foundLSN);
        if (target != null) {
          return target;
        }
      }
    ...
    /**
     * Return the period that should own this foundRBStart. That may be either a nested period, or
     * this period.
     */
    RollbackPeriod getNewPeriodTarget(RollbackStart foundRBStart, long foundLSN) {
      if (currentChild != null) {
        final RollbackPeriod target = currentChild.getNewPeriodTarget(foundRBStart, foundLSN);
        if (target != null) {
          return target;
        }
      }
    ...
    /**
     * This log entry belongs to this rollback period if it lies between the matchpoint and the
     * RollbackStart. We don't use RollbackEnd, because there may not be a RollbackEnd. Also, by
     * definition, anything whose rollback fate is define by this period must have happened before
     * the RollbackStart.
     */
    ...
    RollbackPeriod findTarget(long lsn, long txnId) {
      ...
      if (currentChild != null) {
        final RollbackPeriod candidate = currentChild.findTarget(lsn, txnId);
        if (candidate != null) {
          return candidate;
        }
      }
    ...
    boolean notInRollbackStartAndEnd(long lsn, long txnId) {
      ...
      if (rollbackEndLSN == DbLsn.NULL_LSN) return true;
    ...
    /** @return true if this rollback period is after, and does not contain the lsn. */
    boolean follows(long lsn) {
      return DbLsn.compareTo(matchpointLSN, lsn) > 0;
    }

    /** @return true if this rollback period is before, and does not contain the lsn. */
    ...
    TxnChain getChain(long txnId, long undoLsn, EnvironmentImpl envImpl) {
      ...
      if (chain == null) {
        chain = new TxnChain(undoLsn, txnId, matchpointLSN, envImpl);
