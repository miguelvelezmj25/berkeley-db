...
/**
 * An OldBINDelta contains the information needed to create a partial (delta) BIN log entry. It also
 * knows how to combine a full BIN log entry and a delta to generate a new BIN.
 *
 * <p>An OldBINDelta is no longer written by this version of JE, but it may be read from a log file
...
public class OldBINDelta implements Loggable {
  ...
  /**
   * @return the prior delta version of this BIN, or NULL_LSN if the prior version is a full BIN.
   *     The returned value is the LSN that is obsoleted by this delta.
   */
  ...
  /**
   * Returns a key that can be used to find the BIN associated with this delta. The key of any slot
   * will do.
   */
  ...
  /**
   * Returns the number of bytes occupied by this object. Deltas are not stored in the Btree, but
   * they are budgeted during a SortedLSNTreeWalker run.
   */
  public long getMemorySize() {
    long size =
        MemoryBudget.BINDELTA_OVERHEAD
