...
/**
 * The abstract class that defines the various formats used to represent the keys associated with
 * the IN node. The class is also used to store embedded records, where the actual key and the data
 * portion of a record are stored together as a single byte sequence.
 *
 * <p>There are currently two supported representations:
 *
 * <ol>
 *   <li>A default representation <code>Default</code> that's capable of holding any set of keys.
 *   <li>A compact representation <code>MaxKeySize</code> that's more efficient for holding small
 *       keys (LTE 16 bytes) in length. If key prefixing is in use this represents the unprefixed
 *       part of the key, since that's what is stored in this array.
 * </ol>
 *
 * <p>The choice of representation is made when an IN node is first read in from the log. The <code>
 * MaxKeySize</code> representation is only used when it is more storage efficient than the default
 * representation for the set of keys currently associated with the IN.
 *
 * <p>Note that no attempt is currently made to optimize the storage representation as keys are
 * added to, or removed from, the <code>Default</code> representation to minimize the chances of
 * transitionary "back and forth" representation changes that could prove to be expensive.
 */
public abstract class INKeyRep extends INArrayRep<INKeyRep, INKeyRep.Type, byte[]> {
  ...
  /**
   * Returns true if the key bytes mem usage is accounted for internally here, or false if each key
   * has a separate byte array and its mem usage is accounted for by the parent.
   */
  ...
  public abstract byte[] getFullKey(byte[] prefix, int idx, boolean embeddedData);
  ...
  public static class Default extends INKeyRep {
    ...
    public int compareKeys(
      ...
      cmp =
          Key.compareUnsignedBytes(
              searchKey, 0, Math.min(searchKey.length, prefix.length), prefix, 0, prefix.length);
      ...
      if (cmp == 0) {
        ...
        return compareSuffixes(searchKey, searchKeyOffset, searchKeyLen, idx, embeddedData);
      }
    ...
    private int compareSuffixes(
        byte[] searchKey, int searchKeyOff, int searchKeyLen, int idx, boolean embeddedData) {
    ...
    /**
     * Evolves to the MaxKeySize representation if that is more efficient for the current set of
     * keys. Note that since all the keys must be examined to make the decision, there is a
     * reasonable cost to the method and it should not be invoked indiscriminately.
     */
    @Override
    public INKeyRep compact(IN parent) {
      ...
      if (defaultSizeWithKeys > MaxKeySize.calculateMemorySize(keys.length, maxKeyLength)) {
    ...
    private MaxKeySize compactToMaxKeySizeRep(int maxKeyLength, IN parent) {
      ...
      MaxKeySize newRep = new MaxKeySize(keys.length, (short) maxKeyLength);
    ...
    public long calculateMemorySize() {
       ...
       */
      return MemoryBudget.DEFAULT_KEYVALS_OVERHEAD + MemoryBudget.objectArraySize(keys.length);
    }
    ...
    void updateCacheStats(
        @SuppressWarnings("unused") boolean increment,
  ...
  /**
   * The compact representation that can be used to represent keys LTE 16 bytes in length. The keys
   * are all represented inside a single byte array instead of having one byte array per key. Within
   * the array, all keys are assigned a storage size equal to that taken up by the longest key, plus
   * one byte to hold the actual key length. This makes key retreival fast. However, insertion and
   * deletion for larger keys moves bytes proportional to the storage length of the keys. This is
   * why the representation is restricted to keys LTE 16 bytes in size.
   *
   * <p>On a 32 bit VM the per key overhead for the Default representation is 4 bytes for the
   * pointer + 16 bytes for each byte array key object, for a total of 20 bytes/key. On a 64 bit
   * machine the overheads are much larger: 8 bytes for the pointer plus 24 bytes per key.
   *
   * <p>The more fully populated the IN the more the savings with this representation since the
   * single byte array is sized to hold all the keys regardless of the actual number of keys that
   * are present.
   *
   * <p>It's worth noting that the storage savings here are realized in addition to the storage
   * benefits of key prefixing, since the keys stored in the key array are the smaller key values
   * after the prefix has been stripped, reducing the length of the key and making it more likely
   * that it's small enough for this specialized representation.
   */
  public static class MaxKeySize extends INKeyRep {
    ...
    @Override
    public INKeyRep resize(int capacity) {
      return new MaxKeySize(Arrays.copyOfRange(keys, 0, capacity * fixedKeyLen), fixedKeyLen);
    }
    ...
    public int compareKeys(
      ...
      if (cmp == 0) {
        ...
        return compareSuffixes(searchKey, searchKeyOff, searchKeyLen, idx, embeddedData);
      }
    ...
    private int compareSuffixes(
      ...
      if (keys[myKeyOff] != NULL_KEY) {
        ...
        if (embeddedData) {
          myKeyLen = DupKeyData.getKeyLength(keys, myKeyOff, myKeyLen);
        }
      } else {
        assert (!embeddedData);
        myKeyOff += LENGTH_BYTES;
      ...
      return Key.compareUnsignedBytes(
          searchKey, searchKeyOff, searchKeyLen, keys, myKeyOff, myKeyLen);
    }

    @Override
    public INKeyRep copy(int from, int to, int n, IN parent) {
      System.arraycopy(keys, (from * fixedKeyLen), keys, (to * fixedKeyLen), n * fixedKeyLen);
      return this;
    }
    ...
    @Override
    public long calculateMemorySize() {
      return MemoryBudget.MAX_KEY_SIZE_KEYVALS_OVERHEAD + MemoryBudget.byteArraySize(keys.length);
    }

    private static long calculateMemorySize(int maxKeys, int maxKeySize) {
      return MemoryBudget.MAX_KEY_SIZE_KEYVALS_OVERHEAD
          + MemoryBudget.byteArraySize(maxKeys * (maxKeySize + LENGTH_BYTES));
    }
