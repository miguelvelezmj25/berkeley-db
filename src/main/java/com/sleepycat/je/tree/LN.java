...
public class LN extends Node implements VersionedWriteLoggable {
  ...
  /**
   * Create an empty LN, to be filled in from the log. If VLSNs are preserved for this environment,
   * a VersionedLN will be created instead.
   */
  public LN() {
    this.data = null;
  }
   ...
   *
   * <p>Does NOT copy the byte array, so after calling this method the array is "owned" by the Btree
  ...
  /**
   * Does NOT copy the byte array, so after calling this method the array is "owned" by the Btree
   * and should not be modified.
   */
  LN(final byte[] data) {
   ...
   *
   * <p>If VLSNs are not preserved for this environment, returns -1 which is the sequence for
   * VLSN.NULL_VLSN.
   *
   * <p>If VLSNs are preserved for this environment, this method is overridden by VersionedLN which
   * returns the VLSN sequence.
   */
   ...
   * after reading the LN with the VLSN from the log entry header.
   *
   * <p>If VLSNs are not preserved for this environment, does nothing.
   *
   * <p>If VLSNs are preserved for this environment, this method is overridden by VersionedLN which
   * stores the VLSN sequence.
   */
  ...
  /**
   * Returns true by default, but is overridden by MapLN to prevent eviction of open databases. This
   * method is meant to be a guaranteed check and is used after a BIN has been selected for LN
   * stripping but before actually stripping an LN. [#13415]
   *
  ...
  /**
   * Sets data to empty and returns old data. Called when converting an old format LN in a
   * duplicates DB.
   */
  ...
  /**
   * Release the memory budget for any objects referenced by this LN. For now, only release
   * treeAdmin memory, because treeMemory is handled in aggregate at the IN level. Over time,
   * transition all of the LN's memory budget to this, so we update the memory budget counters more
   * locally. Called when we are releasing a LN for garbage collection.
   */
   ...
   *
   * <p>For a deferred-write database, the logging will not actually occur and a transient LSN will
   * be created and returned if the currLsn is NULL; otherwise the currLsn is returned.
   *
   * <p>However, if the embeddedness of the LN changes, we must create a real logrec for the op. The
   * following scenario is an example of what can go wrong if we don't log embeddedness changes:
   *
   * <p>- R1 exists before DB is opened in DW mode. - R1 is embedded, so its on-disk image has been
   * counted obsolete and might have been deleted already. - R1 is updated in DW mode, and its new
   * version, R2, is not embedded (and not logged). Furthermore, the slot LSN points to R1 still. -
   * R2 gets evicted and logged. As a result, R1 will be counted as obsolete again, because the slot
   * points to R1 and says R1 is not embedded.
   *
   * <p>If we do log embeddedness changes, then in the above scenario, R2 will be logged and this
   * logging will not count R1 as obsolete, because the embedded flag in the slot still says that R1
   * is embedded. After R2 is logged, the slot is updated (both the slot LSN and the embedded flag).
   *
   * <p>In general, the embedded flag in the slot must be in accord with the embedded flag in the
   * last logged version of a record, even if multiple updates were done on the record since the
   * last time it was logged and the current logging event.
   */
  ...
  /** Convenience logging method, used to migrate an LN during cleaning. See logInternal. */
  public LogItem log(
      EnvironmentImpl envImpl,
      DatabaseImpl dbImpl,
      Locker locker,
  ...
  /**
   * Generate and write to the log a logrec describing an operation O that is being performed on a
   * record R with key K. O may be an insertion, update, deletion, migration, or in the case of a DW
   * DB, an eviction or checkpoint of a dirty LN.
   *
   * <p>Let T be the locker performing O. T is null in case of DW eviction/ckpt. Otherwise, T holds
   * a lock on R and it will keep that lock until it terminates. In case of a CUD op, the lock is an
   * exclusive one; in case of LN migration, it's a shared one (and T is non-transactional).
   *
   * <p>- Let Rc be the current version of R (before O). The absence of R from the DB is considered
   * as a special "deleted" version. Rc may be the deleted version. - If T is a Txn, let Ra be the
   * version of R before T write-locked R. Ra may be the deleted version. Ra and Rc will be the same
   * if O is the very 1st op on R by T. - Let Rn be R's new version (after O). Rc and Rn will be the
   * same if O is migration or DW eviction/ckpt.
   *
   * <p>- Let Ln be the LSN of the logrec that will be generated here to describe O. - Let Lc be the
   * current LSN value in R's slot, or NULL if no such slot exists currently. If an R slot exists,
   * then for a non-DW DB, Lc points to Rc, or may be NULL if Rc is the deleted version. But for a
   * DW DB, Lc may point to an older version than Rc, or it may be transient. - If T is a Txn, let
   * La be the LSN value in R's slot at the time T write-locked R, or NULL if no such slot existed
   * at that time.
   *
   * @param isInsertion Whether this CUDop is an insertion (possibly with slot reuse.
   * @param locker The locker T. If non-null, a write lock will be acquired by T on Ln's LSN.
   *     <p>WARNING: Be sure to pass null for the locker param if the new LSN should not be locked.
   * @param writeLockInfo It is non-null if and only if T is a Txn. It contains info that must be
   *     included in Ln to make it undoable if T aborts. Specifically, it contains:
   *     <p>- abortKD : True if Ra is the deleted version; false otherwise. - abortLSN : The La LSN
   *     as defined above. - abortKey : The key of Ra, if Ra was embedded in the parent BIN and the
   *     containing DB allows key updates. - abortData : The data of Ra, if Ra was embedded in the
   *     parent BIN.
   *     <p>When the new LSN is write-locked, a new WriteLockInfo is created and the above info is
   *     copied into it. Normally this parameter should be obtained from the prepareForInsert or
   *     prepareForUpdate method of CursorImpl.LockStanding.
   * @param newEmbeddedLN Whether Rn will be embedded into the parent BIN. If true, Ln will be
   *     counted as an "immediately obsolete" logrec.
   * @param newKey Rn's key. Note: Rn's data is not passed as a parameter to this method because it
   *     is stored in this LN. Rn (key and data) will be stored in Ln. Rn's key will also be stored
   *     in the parent BIN, and if newEmbeddedLN is true, Rn's data too will be stored there.
   ...
   * @param currEmbeddedLN Whether Rc's data is embedded into the parent BIN. If true, Lc has
   *     already been counted obsolete.
   * @param currLsn The Lc LSN as defined above. Is given as a param to this method to count the
   *     associated logrec as obsolete (which must done under the LWL), if it has not been counted
   *     already.
   * @param currSize The size of Lc (needed for obsolete counting).
   * @param isInsertion True if the operation is an insertion (including slot reuse). False
   *     otherwise.
   */
  private LogItem logInternal(
    ...
    try {
      ...
      if (envImpl.isValid()) {
        throw new EnvironmentFailureException(
            envImpl, EnvironmentFailureReason.LOG_INCOMPLETE, "LN could not be logged", e);
      } else {
        throw e;
      }
    } finally {
    ...
    /**
     * Lock the new LSN immediately after logging, with the BIN latched. Lock non-blocking, since no
     * contention is possible on the new LSN. If the locker is transactional, a new WriteLockInfo is
     * created for the new LSN and stored in the locker. lockResult points to that WriteLockInfo.
     * Since this new WriteLockInfo and the WriteLockInfo given as input to this method refer to the
     * same logical record, the info from the given WriteLockInfo is copied to the new one.
     */
    if (locker != null) {
      final long newLsn = item.lsn;

      final LockResult lockResult =
          locker.nonBlockingLock(newLsn, LockType.WRITE, false /*jumpAheadOfWaiters*/, dbImpl);
    ...
    if (dbImpl.getSortedDuplicates() && (newEmbeddedLN || (data != null && data.length > 0))) {
      ...
      throw EnvironmentFailureException.unexpectedState(
          ...
          "[#25288] emb="
              + newEmbeddedLN
              + " key="
              + Key.getNoFormatString(newKey)
              + " data="
              + Key.getNoFormatString(data)
              + " vlsn="
              + item.header.getVLSN()
              + " lsn="
  ...
  /**
   * The first time we optionally-log an LN in a DeferredWrite database, oldLsn will be NULL_LSN and
   * we'll assign a new transient LSN. When we do subsequent optional-log operations, the old LSN
   * will be non-null and to conserve transient LSNs we'll continue to use the previously assigned
   * LSN rather than assigning a new one. And of course, when old LSN is persistent we'll continue
   * to use it.
   *
   * <p>If locker is non-null, this method write-locks the new LSN, whether it has been assigned by
   * this method or not.
   */
  private long assignTransientLsn(
      EnvironmentImpl envImpl, DatabaseImpl dbImpl, long oldLsn, Locker locker) {
    final long newLsn;
    ...
    /**
     * Lock immediately after assigning a new LSN, with the BIN latched. Lock non-blocking, since no
     * contention is possible on the new LSN.
     */
    if (locker != null) {
      final LockResult lockResult =
          locker.nonBlockingLock(newLsn, LockType.WRITE, false /*jumpAheadOfWaiters*/, dbImpl);
  ...
  /**
   * Calculates log size based on given dataLen, which is negative to calculate the size of a
   * deleted LN.
   */
  ...
  /**
   * Copies the given byte array to the given destination entry, copying only partial data if the
   * entry is specified to be partial. If the byte array is null, clears the entry.
   */
  ...
  /**
   * Copies the given source entry to the given destination entry, copying only partial data if the
   * destination entry is specified to be partial.
   */
  ...
  /**
   * Merges the partial entry with the given byte array, effectively applying a partial entry to an
   * existing record, and returns a enw byte array.
   */
