...
/**
 * Estimates the number of non-deleted BIN entries between two end points, using information
 * returned in TrackingInfo from Tree.getParentINForChildIN. Used for estimating dup counts, e.g.,
 * for join query optimization. Accuracy is limited by the number of samples taken to compute the
 * average number of entries at each level. Currently only two samples (at the end points) are
 * taken, and because the tree is not balanced making the number of entries highly variable, the
 * count can easily be off by a factor of two.
 */
public class CountEstimator {
  ...
  /**
   * Returns an estimate of the number of records between two end points specified by begin/end
   * cursor positions.
   */
  ...
  /**
   * Find the common ancestor node for the two end points, which we'll call the root level. If no
   * common ancestor can be found, return false to restart the process, because a split must have
   * occurred in between getting the two stacks for the end points.
   */
  ...
  /** This method starts with a preliminary average using just the two end points. */
  ...
  /**
   * At each level we compute the average number of entries. This will be used as a multipler to
   * estimate the number of nodes for any IN at that level.
   */
