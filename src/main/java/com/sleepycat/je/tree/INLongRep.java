 ...
 *
 * <p>Zero is the default value and is returned when no value has been set.
 *
 * <p>The EMPTY_REP is used at first, and is mutated as necessary as values are set. A minimum
 * number of bytes per value is used, based on the largest value passed to set().
 *
 * <p>Optionally, a sparse rep is used when a value is set for EMPTY_REP. Up to 4 values are stored
 * along with their indexes. When the 5th values is set, the rep is mutated to the default rep.
 *
 * <p>This object calls IN.updateMemorySize to track the memory it uses. EMPTY_REP uses no memory
...
public abstract class INLongRep {
  ...
  /**
   * Initially empty (all values are zero) but will mutate as needed when non-zero values are passed
   * to set().
   */
  ...
  public static class DefaultRep extends INLongRep {
    ...
    public DefaultRep resize(int capacity) {
      return new DefaultRep(
          Arrays.copyOfRange(byteArray, 0, capacity * bytesPerValue), bytesPerValue);
    }
    ...
    public INLongRep clear(IN parent, EmptyRep emptyRep) {
      ...
      parent.updateMemorySize(getMemorySize(), emptyRep.getMemorySize());
    ...
    public INLongRep copy(int from, int to, int n, IN parent) {
      System.arraycopy(
          byteArray, from * bytesPerValue, byteArray, to * bytesPerValue, n * bytesPerValue);
      return this;
    }

    @Override
    public long getMemorySize() {
      return MemoryBudget.DEFAULT_LONG_REP_OVERHEAD + MemoryBudget.byteArraySize(byteArray.length);
    }
  }
  ...
  public static class SparseRep extends DefaultRep {
    ...
    public INLongRep set(int idx, long val, IN parent) {
      ...
      /* It's full, mutate it. */
      INLongRep newRep = new DefaultRep(parent.getMaxEntries(), bytesPerValue);
