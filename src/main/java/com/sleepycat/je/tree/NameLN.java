...
public final class NameLN extends LN {
  ...
  /**
   * In the ideal world, we'd have a base LN class so that this NameLN doesn't have a superfluous
   * data field, but we want to optimize the LN class for size and speed right now.
   */
  ...
  public boolean logicalEquals(Loggable other) {
    ...
    if (!(id.equals(otherLN.id))) {
      ...
      return deleted == otherLN.deleted;
  }
  ...
  LNLogEntry<?> createLogEntry(
    ...
    return new NameLNLogEntry(
        entryType, dbImpl.getId(), txn, abortLsn, abortKD, newKey, this, repContext);
  }
}
