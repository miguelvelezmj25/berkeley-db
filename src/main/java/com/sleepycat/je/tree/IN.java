 ...
 *
 * <p>Explanation of KD (KnownDeleted) and PD (PendingDelete) entry flags
 * ===================================================================
 *
 * <p>PD: set for all LN entries that are deleted, even before the LN is committed. Is used as an
 * authoritative (transactionally correct) indication that an LN is deleted. PD will be cleared if
 * the txn for the deleted LN is aborted.
 *
 * <p>KD: set under special conditions for entries containing LNs which are known to be obsolete.
 * Not used for entries in an active/uncommitted transaction.
 *
 * <p>First notice that IN.fetchLN will allow a FileNotFoundException when the PD or KD flag is set
 * on the entry. And it will allow a NULL_LSN when the KD flag is set.
 *
 * <p>KD was implemented first, and was originally used when the cleaner attempts to migrate an LN
 * and discovers it is deleted (see Cleaner.migrateLN). We need KD because the INCompressor may not
 * have run, and may not have compressed the BIN. There's the danger that we'll try to fetch that
 * entry, and that the file was deleted by the cleaner.
 *
 * <p>KD was used more recently when an unexpected exception occurs while logging an LN, after
 * inserting the entry. Rather than delete the entry to clean up, we mark the entry KD so it won't
 * cause a fetch error later. In this case the entry LSN is NULL_LSN. See Tree.insertNewSlot.
 *
 * <p>PD is closely related to the first use of KD above (for cleaned deleted LNs) and came about
 * because of a cleaner optimization we make. The cleaner considers all deleted LN log entries to be
 * obsolete, without doing a tree lookup, and without any record of an obsolete offset. This makes
 * the cost of cleaning of deleted LNs very low. For example, if the log looks like this:
 *
 * <p>100 LNA 200 delete of LNA
 *
 * <p>then LSN 200 will be considered obsolete when this file is processed by the cleaner. After
 * all, only two things can happen: (1) the txn commits, and we don't need LSN 200, because we can
 * wipe this LN out of the tree, or (2) the txn aborts, and we don't need LSN 200, because we are
 * going to revert to LSN 100/LNA.
 *
 * <p>We set PD for the entry of a deleted LN at the time of the operation, and we clear PD if the
 * transaction aborts. There is no real danger that this log entry will be processed by the cleaner
 * before it's committed, because cleaning can only happen after the first active LSN.
 *
 * <p>Just as in the first use of KD above, setting PD is necessary to avoid a fetch error, when the
 * file is deleted by the cleaner but the entry containing the deleted LN has not been deleted by
 * the INCompressor.
 *
 * <p>PD is also set in replication rollback, when LNs are marked as invisible.
 *
 * <p>When LSN locking was implemented (see CursorImpl.lockLN), the PD flag took on additional
 * meaning. PD is used to determine whether an LN is deleted without fetching it, and therefore is
 * relied on to be transactionally correct.
 *
 * <p>In addition to the setting and use of the KD/PD flags described above, the situation is
 * complicated by the fact that we must restore the state of these flags during abort, recovery, and
 * set them properly during slot reuse.
 *
 * <p>We have been meaning to consider whether PD and KD can be consolidated into one flag: simply
 * the Deleted flag. The Deleted flag would be set in the same way as PD is currently set, as well
 * as the second use of KD described above (when the LSN is NULL_LSN after an insertion error). The
 * use of KD and PD for invisible entries and recovery rollback should also be considered.
 *
 * <p>If we consolidate the two flags and set the Deleted flag during a delete operation (like PD),
 * we'll have to remove optimizations (in CursorImpl for example) that consider a slot deleted when
 * KD is set. Since KD is rarely set currently, this shouldn't have a noticeable performance impact.
 */
public class IN extends Node implements Comparable<IN>, LatchContext {
   ...
   *
   * <p>minLength is 3 because BIN IDs are LRU list indexes. Initially 100k indexes are allocated
   * and the largest values are used first.
   *
   * <p>allowSparseRep is true because some workloads will only load BIN IDs for a subset of the
   * BINs in the IN.
   */
  ...
  /**
   * References to the next and previous nodes in an LRU list. If the node is not in any LRUList,
   * both of these will be null. If the node is at the front/back of an LRUList,
   * prevLRUNode/nextLRUNode will point to the node itself.
   */
  ...
  public IN(@SuppressWarnings("unused") SizeofMarker marker) {
    ...
    latch =
        LatchFactory.createSharedLatch(
  ...
  protected void init(
      DatabaseImpl db,
      @SuppressWarnings("hiding") byte[] identifierKey,
      int initialCapacity,
      @SuppressWarnings("hiding") int level) {
  ...
  /**
   * Latch this node if it is not latched by another thread. Update the LRU using the given
   * cacheMode if the latch succeeds.
   */
   ...
   *
   * @param db is passed in order to initialize the database for an uninitialized node, which is
   *     necessary in order to latch it.
   */
  ...
  /**
   * This method should be used carefully. Unless this node and the parent are already known to be
   * latched, call latchParent instead to access the parent safely.
   */
  public IN getParent() {
    return parent;
  }
  ...
  /**
   * Latches the parent exclusively, leaving this node latched. The parent must not already be
   * latched.
   *
   * <p>This node must be latched on entry and will be latched on exit. This node's latch may be
   * released temporarily, in which case it will be ex-latched (since the parent is ex-latched, this
   * isn't a drawback).
   *
   * <p>Does not perform cache mode processing, since this node is already latched.
   *
   * @return the ex-latched parent, for which calling getKnownChildIndex with this node is
   *     guaranteed to succeed.
   * @throws EnvironmentFailureException (fatal) if the parent latch is already held.
   */
  ...
  /**
   * Returns the index of the given child. Should only be called when the caller knows that the
   * given child is resident.
   */
  ...
  /**
   * We would like as even a hash distribution as possible so that the Evictor's LRU is as accurate
   * as possible. ConcurrentHashMap takes the value returned by this method and runs its own hash
   * algorithm on it. So a bit complement of the node ID is sufficient as the return value and is a
   * little better than returning just the node ID. If we use a different container in the future
   * that does not re-hash the return value, we should probably implement the Wang-Jenkins hash
   * function here.
   */
  @Override
  ...
  /**
   * Indicates that the BIN was fetched from disk, or loaded from the off-heap cache, using
   * CacheMode.UNCHANGED, and has not been accessed with another CacheMode. BINs in this state
   * should be evicted from main cache as soon as they are no longer referenced by a cursor. If they
   * were loaded from off-heap cache, they should be stored off-heap when they are evicted from
   * main. The FetchedColdOffHeap flag indicates whether the BIN was loaded from off-heap cache.
   */
   ...
   * @param makeDirty should normally be true, but may be false when an expired slot containing the
   *     identifier key has been deleted.
   */
   ...
   *     <p>Overriden by TestIN in INEntryTestBase.java
   */
  ...
  /**
   * @return true if the idx'th entry has been deleted, although the transaction that performed the
   *     deletion may not be committed.
   */
  ...
  /**
   * @return true if the idx'th entry is deleted for sure. If a transaction performed the deletion,
  ...
  /**
   * Set knownDeleted flag to true and evict the child LN if cached. The child LN is evicted to save
   * memory, since it will never be fetched again.
   */
  ...
  /**
   * Return the idx'th key. If prefixing is enabled, construct a new byte[] containing the prefix
   * and suffix. If prefixing is not enabled, just return the current byte[] in entryKeys.
   */
  ...
  /**
   * Returns the size of the key that is stored persistently, which will be the combined key-data
   * for an embedded LN or duplicated DB record.
   */
  ...
  /**
   * Updates the key in the idx-th slot of this BIN, if the DB allows key updates and the new key is
   * not identical to the current key in the slot. It also updates the data (if any) that is
   * embedded with the key in the idx-slot, or embeds new data in that slot, is the "data" param is
   * non-null, or removes embedded data, if "data" is null. Finally, it sets the EMBEDDED_LN_BIT and
   * NO_DATA_LN_BIT flags in the slot's state.
   *
   * @param key is the key to set in the slot and is the LN key.
   * @param data If the data portion of a record must be embedded in this BIN, "data" stores the
   *     record's data. Null otherwise. See also comment for the keyEntries field.
   * @return true if a multi-slot change was made and the complete IN memory size must be updated.
   */
  private boolean updateLNSlotKey(int idx, byte[] key, byte[] data) {
     ...
     */
    if (key != null
        && (databaseImpl.allowsKeyUpdates() || DupConvert.needsConversion(databaseImpl))
  ...
  private boolean idKeyIsSlotKey() {
    ...
    for (int i = 0; i < nEntries; i += 1) {
      ...
      if (entryKeys.compareKeys(
              identifierKey, keyPrefix, i, haveEmbeddedData(i), databaseImpl.getKeyComparator())
          == 0) {
  ...
  /**
   * This method inserts or updates a key at a given slot. In either case, the associated embedded
   * data (if any) is inserted or updated as well, and the key prefix is adjusted, if necessary.
   *
   * <p>In case of insertion (indicated by a true value for the isInsertion param), it is assumed
   * that the idx slot does not store any valid info, so any change to the key prefix (if any) is
   * due to the insertion of this new new key and not to the removal of the current key at the idx
   * slot.
   *
   * <p>In case of update, the method does not check if the current key is indeed different from the
   * new key; it just updates the key unconditionally. If the slot has embedded data, that data will
   * also be updated (if the data param is not null), or be removed (if the data param is null). If
   * the slot does not have embedded data and the data param is not null, the given data will be
   * embedded.
   *
   * <p>Note: For BINs, the maintenance of the EMBEDDED_LN_BIT andNO_DATA_LN_BIT is done by the
  ...
  public boolean setKey(int idx, byte[] key, byte[] data, boolean isInsertion) {
    ...
    if (databaseImpl.getKeyPrefixing() && keyPrefix != null) {
      ...
      if (newPrefixLen < keyPrefix.length) {
         ...
         */
        byte[] newPrefix =
            (isInsertion ? Key.createKeyPrefix(keyPrefix, key) : computeKeyPrefix(idx));
  ...
  /**
   * Forces computation of the key prefix, without requiring a split. Is public for use by
   * DbCacheSize.
   */
  ...
  /**
   * Returns whether the given key is greater than or equal to the first key in the IN and less than
   * or equal to the last key in the IN. This method is used to determine whether a key to be
   * inserted belongs in this IN, without doing a tree search. If false is returned it is still
   * possible that the key belongs in this IN, but a tree search must be performed to find out.
   */
  ...
  /**
   * Set the LSN of the idx'th slot, mark the slot dirty, and update memory consuption. Throw
   * exception if the update is not legitimate.
   */
  ...
  /**
   * Set the LSN of the idx'th slot, mark the slot dirty, and update memory consuption. If "check"
   * is true, throw exception if the update is not legitimate.
   */
   ...
   * Also ensures that a transient LSN is not accidentally assigned to a persistent entry.
   *
   * <p>Because this method uses strict checking, prepareForSlotReuse must sometimes be called when
   * a new logical entry is being placed in a slot, e.g., during an IN split or an LN slot reuse.
   *
   * <p>The following transition is a NOOP and the LSN is not set: Any LSN to same value. The
   * following transitions are allowed and cause the LSN to be set: Null LSN to transient LSN Null
   * LSN to persistent LSN Transient LSN to persistent LSN Persistent LSN to new persistent LSN The
   * following transitions should not occur and throw an exception: Transient LSN to null LSN
   * Transient LSN to new transient LSN Persistent LSN to null LSN Persistent LSN to transient LSN
   *
   * <p>The above imply that a transient or null LSN can overwrite only a null LSN.
   */
      ...
      if (oldLsn != DbLsn.NULL_LSN && DbLsn.isTransientOrNull(newLsn)) {
        throw unexpectedState(
            "DeferredWrite LSN update not allowed"
                + " oldLsn = "
                + DbLsn.getNoFormatString(oldLsn)
                + " newLsn = "
   ...
   *
   * <p>WARNING: This method does not update the memory budget. The caller must update the budget.
   */
   ...
   *
   * <p>This method does not load children from off-heap cache, so it always returns null when then
   * child is not in main cache. Note that when children are INs (this is not a BIN), when this
   * method returns null it is does not imply that the child is non-dirty, because dirty BINs are
   * stored off-heap. To fetch the current version from off-heap cache in that case, call loadIN
   * instead.
   */
  ...
  /**
   * Returns the idx-th child of "this" upper IN, fetching the child from the log and attaching it
   * to its parent if it is not already attached. This method is used during tree searches.
   *
   * <p>On entry, the parent must be latched already.
   *
   * <p>If the child must be fetched from the log, the parent is unlatched. After the disk read is
   * done, the parent is relatched. However, due to splits, it may not be the correct parent
   * anymore. If so, the method will return null, and the caller is expected to restart the tree
   * search.
   *
   * <p>On return, the parent will be latched, unless null is returned or an exception is thrown.
   *
   * <p>The "searchKey" param is the key that the caller is looking for. It is used by this method
   * in determining if, after a disk read, "this" is still the correct parent for the child.
   * "searchKey" may be null if the caller is doing a LEFT or RIGHT search.
   */
  ...
  /**
   * This variant of fetchIN() takes a SearchResult as a param, instead of an idx (it is used by
   * Tree.getParentINForChildIN()). The ordinal number of the child to fetch is specified by
   * result.index. result.index will be adjusted by this method if, after a disk read, the ordinal
   * number of the child changes due to insertions, deletions or splits in the parent.
   */
  ...
  private IN fetchINWithNoLatch(
      int idx, byte[] searchKey, SearchResult result, CacheMode cacheMode) {
      ...
      try {
        ...
        if (ohBytes != null) {
          ...
          final WholeEntry wholeEntry =
              envImpl
                  .getLogManager()
                  .getLogEntryAllowInvisibleAtRecovery(lsn, getLastLoggedSize(idx));
        ...
        throw new EnvironmentFailureException(
            ...
            makeFetchErrorMsg(null, lsn, idx),
            e);
        ...
        throw new EnvironmentFailureException(
            ...
            makeFetchErrorMsg(e.toString(), lsn, idx),
            e);
      } finally {
        /*
  ...
  /**
   * Returns the idx-th child of "this" upper IN, fetching the child from the log and attaching it
   * to its parent if it is not already attached.
   *
   * <p>On entry, the parent must be EX-latched already and it stays EX-latched for the duration of
   * this method and on return (even in case of exceptions).
   *
  ...
  /**
   * Returns the idx-th child of "this" upper IN, loading the child from off-heap and attaching it
   * to its parent if it is not already attached and is cached off-heap. This method does not fetch
   * from disk, and will return null if the child is not in the main or off-heap cache.
   *
   * <p>On entry, the parent must be EX-latched already and it stays EX-latched for the duration of
   * this method and on return (even in case of exceptions).
   *
   ...
   *
   * <p>Null is returned in the following cases:
   *
   * <p>1. if the LSN is null and the KnownDeleted flag is set; or 2. if the LSN's file has been
   * cleaned and: a. the PendingDeleted or KnownDeleted flag is set, or b. the entry is "probably
   * expired".
   *
   * <p>Note that checking for PD/KD before calling this method is not sufficient to ensure that
   * null is not returned, because null is also returned for expired records.
   *
   * <p>When null is returned, the caller must treat the record as deleted.
   *
   * <p>Note that null can only be returned for a slot that could contain an LN, not other node
   * types and not a DupCountLN since DupCountLNs are never deleted or expired.
   *
   * <p>An exclusive latch must be held on this BIN.
   *
  ...
  /**
   * Return the idx'th LN target, enforcing rules defined by the cache modes for the LN. This method
   * should be called instead of getTarget when a cache mode applies to user operations such as
   * reads and updates.
   */
   ...
   *
   * <p>Does not call setLastLoggedLsn because materialization of the off-heap BIN initializes all
   * fields including the last logged/delta LSNs.
   */
  ...
  /**
   * Needed only during duplicates conversion, not during normal operation. The needDupKeyConversion
   * field will only be true when first upgrading from JE 4.1. After the first time an IN is
   * converted, it will be written out in a later file format, so the needDupKeyConversion field
   * will be false and this method will do nothing. See DupConvert.convertInKeys.
   */
   ...
   *
   * <p>Currently uses a binary search, but eventually, this may use binary or linear search
   * depending on key size, number of entries, etc.
   *
   * <p>This method guarantees that the key parameter, which is the user's key parameter in
   * user-initiated search operations, is always the left hand parameter to the Comparator.compare
   * method. This allows a comparator to perform specialized searches, when passed down from upper
   * layers.
   *
   * <p>This is public so that DbCursorTest can access it.
   *
   * <p>Note that the 0'th entry's key is treated specially in an IN. It always compares lower than
   * any other key.
   *
   * @param key - the key to search for.
   * @param indicateIfDuplicate - true if EXACT_MATCH should be or'd onto the return value if key is
   *     already present in this node.
   * @param exact - true if an exact match must be found.
   * @return offset for the entry that has a key LTE the arg. 0 if key is less than the 1st entry.
   *     -1 if exact is true and no exact match is found. If indicateIfDuplicate is true and an
   *     exact match was found then EXACT_MATCH is or'd onto the return value.
   */
  public final int findEntry(
      byte[] key, boolean indicateIfDuplicate, boolean exact, Comparator<byte[]> comparator) {
    ...
    while (low <= high) {
      ...
      if (middle == 0 && entryZeroSpecialCompare) {
        ...
        s = entryKeys.compareKeys(key, keyPrefix, middle, haveEmbeddedData(middle), comparator);
      }
  ...
  /**
   * Inserts a slot with the given key, lsn and child node into this IN, if a slot with the same key
   * does not exist already. The state of the new slot is set to DIRTY. Assumes this node is already
   * latched by the caller.
   *
   * @return true if the entry was successfully inserted, false if it was a duplicate.
   ...
   * @param data If the data portion of a record must be embedded in this BIN, "data" stores the
   *     record's data. Null otherwise. See also comment for the keyEntries field.
   * @return either (1) the index of location in the IN where the entry was inserted |'d with
   *     INSERT_SUCCESS, or (2) the index of the duplicate in the IN if the entry was found to be a
   *     duplicate.
   * @throws EnvironmentFailureException if the node is full (it should have been split earlier).
   */
  public final int insertEntry1(
      Node child, byte[] key, byte[] data, long childLsn, boolean blindInsertion) {

    return insertEntry1(child, key, data, childLsn, EntryStates.DIRTY_BIT, blindInsertion);
  }

  /**
   * Inserts a slot with the given key, lsn, state, and child node into this IN, if a slot with the
   * same key does not exist already. Assumes this node is already latched by the caller.
   *
   * <p>This returns a failure if there's a duplicate match. The caller must do the processing to
   * check if the entry is actually deleted and can be overwritten. This is foisted upon the caller
   * rather than handled in this object because there may be some latch releasing/retaking in order
   * to check a child LN.
   *
   * @param data If the data portion of a record must be embedded in this BIN, "data" stores the
   *     record's data. Null otherwise. See also comment for the keyEntries field.
   ...
   */
  public final int insertEntry1(
      Node child, byte[] key, byte[] data, long childLsn, byte state, boolean blindInsertion) {
        ...
        if (traceDeltas) {
          LoggerUtils.envLogMsg(
              ...
              Thread.currentThread().getId()
                  ...
                  + "-"
                  + getEnv().getName()
                  + (blindInsertion ? " Blind insertion in BIN-delta " : " Blind put in BIN-delta ")
                  + getNodeId()
                  + " nEntries = "
                  + nEntries
                  + " max entries = "
  ...
  /**
   * Removes the slot at index from this IN. Assumes this node is already latched by the caller.
   *
  ...
  /**
   * Variant that allows specifying whether the IN is dirtied and whether validation takes place.
   * 'validate' should be false only in tests.
   *
   * <p>See BIN.compress and INCompressor for a discussion about why slots can be deleted without
   * dirtying the BIN, and why the next delta is prohibited when the slot is dirty.
   */
  ...
  /**
   * WARNING: clearEntry() calls entryTargets.set() directly, instead of setTarget(). As a result,
   * the hasCachedChildren flag of the IN is not updated here. The caller is responsible for
   * updating this flag, if needed.
   */
  ...
  /**
   * This method is called after the idx'th child of this node gets logged, and changes position as
   * a result.
   *
   * @param newLSN The new on-disk position of the child.
   * @param newVLSN The VLSN of the logrec at the new position. For LN children only.
   * @param newSize The size of the logrec at the new position. For LN children only.
   */
  ...
  /**
   * This method is called only from BIN.applyDelta(). It applies the info extracted from a delta
   * slot to the corresponding slot in the full BIN.
   *
   * <p>Unlike other update methods, the LSN may be NULL_LSN if the KD flag is set. This allows
   * applying a BIN-delta with a NULL_LSN and KD, for an invisible log entry for example.
   *
   * <p>No need to do memory counting in this method because the BIN is not yet attached to the
   * tree.
   */
  final void applyDeltaSlot(
      int idx, Node node, long lsn, int lastLoggedSize, byte state, byte[] key, byte[] data) {
  ...
  /**
   * Update the idx slot of this BIN to reflect a record insertion in an existing KD slot. It is
   * called from CursorImpl.insertRecordInternal(), after logging the insertion op.
   *
   ...
   * @param newKey The value for the record's key. It is equal to the current key value in the slot,
   *     but may not be identical to that value if a custom comparator is used.
   * @param newData If the record's data must be embedded in this BIN, "data" stores the record's
   *     data. Null otherwise. See also comment for the keyEntries field.
   */
  ...
  /**
   * Update the idx slot of this BIN to reflect an update of the associated record. It is called
   * from CursorImpl.updateRecordInternal(), after logging the update op.
   *
   * @param oldMemSize If the child LN was cached before the update op, it has already been updated
   *     in-place by the caller. In this case, oldMemSize stores the size of the child LN before the
   *     update, and it is used to do memory counting. Otherwise oldMemSize is 0 and the newly
   *     created LN has not been attached to the tree; it will be attached later by the caller, if
   *     needed.
   ...
   * @param newKey The new value for the record's key. It is equal to the current value, but may not
   *     be identical to the current value if a custom comparator is used. It may be null, if the
   *     caller knows for sure that the key does not change.
   * @param newData If the record's data must be embedded in this BIN, "data" stores the record's
   *     data. Null otherwise. See also comment for the keyEntries field.
   */
  ...
  /**
   * Update the idx slot slot of this BIN to reflect a deletion of the associated record. It is
   * called from CursorImpl.deleteCurrentRecord(), after logging the deletion op.
   *
   * @param oldMemSize If the child LN was cached before the deletion, it has already been updated
   *     in-place by the caller (the ln contents have been deleted). In this case, oldMemSize stores
   *     the in-memory size of the child LN before the update, and it is used to do memory counting.
   *     Otherwise oldMemSize is 0 and the newly created LN has not been attached to the tree; it
   *     will be attached later by the caller, if needed.
  ...
  /**
   * This method is used by the RecoveryManager to change the current version of a record, either to
   * a later version (in case of redo), or to an earlier version (in case of undo). The current
   * version may or may not be cached as a child LN of this BIN (it may be only in case of txn abort
   * during normal processing). If it is, it is evicted. The new version is not attached to the
   * in-memory tree, to save memory during crash recovery.
   *
   * @param idx The BIN slot for the record.
   * @param lsn The LSN of the new record version. It may be null in case of undo, if the logrec
   *     that is being undone is an insertion and the record did not exist at all in the DB before
   *     that insertion.
   * @param knownDeleted True if the new version is a committed deletion.
   * @param pendingDeleted True if the new version is a deletion, which may or may not be committed.
   * @param key The key of the new version. It is null only if we are undoing and the revert-to
   *     version was not embedded (in this case the key of the revert-to version is not stored in
   *     the logrec). If it is null and the DB allows key updates, the new record version is fetched
   *     from disk to retrieve its key, so that the key values stored in the BIN slots are always
   *     transactionally correct.
   * @param data The data of the new version. It is non-null if and only if the new version must be
   *     embedded in the BIN.
   * @param vlsn The VLSN of the new version.
   * @param logrecSize The on-disk size of the logrec corresponding to the new version. It may be 0
   *     (i.e. unknown) in case of undo.
   */
  ...
  /**
   * Update the cached-child and LSN properties of the idx-th slot. This method is used by the
   * RecoveryManager.recoverChildIN() to change the version of a child IN, a later version The child
   * IN may or may not be already attached to the tree.
   */
  ...
  /**
   * Attach the given node as the idx-th child of "this" node. If the child node is an LN, update
   * the key of the parent slot to the given key value, if that value is non-null and an update is
   * indeed necessary.
   *
   * <p>This method is called after the child node has been either (a) fetched in from disk and is
   * not dirty, or (b) is a newly created instance that will be written out later by something like
   * a checkpoint. In either case, the slot LSN does not need to be updated.
   *
   * <p>Note: does not dirty the node unless the LN slot key is changed.
   */
  ...
  /**
   * This method is used in DupConvert, where it is called to convert the keys of an upper IN that
   * has just been fetched from the log and is not attached to in-memory tree yet.
   */
  ...
  /**
   * Return true if this node needs splitting. For the moment, needing to be split is defined by
   * there being no free entries available.
   */
  ...
  /**
   * Split this into two nodes. Parent IN is passed in parent and should be latched by the caller.
   *
   * <p>childIndex is the index in parent of where "this" can be found.
   */
  ...
  /**
   * Called when we know we are about to split on behalf of a key that is the minimum (leftSide) or
   * maximum (!leftSide) of this node. This is achieved by just forcing the split to occur either
   * one element in from the left or the right (i.e. splitIndex is 1 or nEntries - 1).
   */
  IN splitSpecial(
      IN parent,
      int parentIndex,
  ...
  final IN splitInternal(
    ...
    try {
      ...
      /**
       * Log the parent. Note that the root slot or grandparent slot is not updated with the
       * parent's LSN here; this is done by Tree.forceSplit.
       */
      if (parent.isRoot()) {
  ...
  /**
   * Update a slot that is being split. The slot to be updated here is the one that existed before
   * the split.
   *
   * @param child The new child to be placed under the slot. May be the newly created sibling or the
   *     pre-existing sibling.
   * @param lsn The new lsn of the child (the child was logged just before calling this method, so
   *     its slot lsn must be updated)
   * @param key The new key for the slot. We should not actually update the slot key, because its
   *     value is the lower bound of the key range covered by the slot, and this lower bound does
   *     not change as a result of the split (the new slot created as a result of the split is
   *     placed to the right of the pre-existing slot). There is however one exception: the key can
   *     be updated if "idx" is the 0-slot. The 0-slot key is not a true lower bound; the actual
   *     lower bound for the 0-slot is the key in the parent slot for this IN. So, in this case, if
   *     the given key is less than the current one, it is better to update the key in order to
   *     better approximate the real lower bound (and thus make the isKeyInBounds() method more
   *     effective).
   */
  ...
  /**
   * Shift entries to the right by one position, starting with (and including) the entry at index.
   * Increment nEntries by 1. Called in insertEntry1()
   *
  ...
  /**
   * Shift entries starting at the byHowMuch'th element to the left, thus removing the first
   * byHowMuch'th elements of the entries array. This always starts at the 0th entry. Caller is
   * responsible for decrementing nEntries.
   *
   * @param byHowMuch - The number of entries to remove from the left side of the entries array.
   */
   ...
   *
   * <p>Necessary to support assertions for transient LSNs in shouldUpdateLsn. Examples: LN slot
   * reuse, and splits where a new node is placed in an existing slot.
   *
   * <p>Also needed to free the off-heap BIN associated with the old node.
   *
   * <p>TODO: This method is no longer used for LN slot reuse, and freeing of the off-heap BIN could
   * be done by the only caller, splitInternal, and then this method could be removed.
   */
  ...
  /**
   * Compute the current memory consumption of this node, after putting its keys in their compact
  ...
  /**
   * Count up the memory usage attributable to this node alone. LNs children are counted by their
   * BIN parents, but INs are not counted by their parents because they are resident on the IN list.
   * The identifierKey is "intentionally" not kept track of in the memory budget.
   */
   ...
   *
   * <p>Called by the evictor to reduce memory usage. Should not be called too often (e.g., every
   * CRUD operation), since this could cause lots of memory allocations as the representations
   * contract and expend, resulting in expensive GC.
   *
  ...
  /**
   * Called as part of a memory budget reset (during a checkpoint) to clear the accumulated delta
   * and return the total memory size.
   */
  ...
  void updateMemorySize(long delta) {
    ...
    if (getInListResident()) {
       ...
       * [#21929]
       */
      assert inMemorySize >= getFixedMemoryOverhead() || !getEnv().isValid()
          : "delta: "
              + delta
              + " inMemorySize: "
              + inMemorySize
  ...
  /**
   * Returns the treeAdmin memory in objects referenced by this IN. Specifically, this refers to the
   * DbFileSummaryMap held by MapLNs
   */
  ...
  protected long printMemorySize() {
    ...
    final int keyPrefixOverhead =
        (keyPrefix != null) ? MemoryBudget.byteArraySize(keyPrefix.length) : 0;

    final int provisionalOverhead =
        (provisionalObsolete != null) ? provisionalObsolete.getMemorySize() : 0;
  ...
  /**
   * Adds (increments) or removes (decrements) the cache stats for the key and target
   * representations. Used when rep objects are being replaced with a new instance, rather than by
   * calling their mutator methods. Specifically, it is called when mutating from full bin to bin
   * delta or vice-versa.
   */
   ...
   *
   * <p>Used by the evictor to reclaim memory by some means short of evicting the entire node. If a
   * positive value is returned, the evictor will postpone full eviction of this node.
   */
  ...
  /**
   * Disallow delta on next log. Set to true (a) when we we delete a slot from a BIN, (b) when the
   * cleaner marks a BIN as dirty so that it will be migrated during the next checkpoint.
   */
  ...
  /**
   * Check if this node fits the qualifications for being part of a deletable subtree. It can only
   * have one IN child and no LN children.
   *
   * <p>Note: the method is overwritten by BIN and LN. BIN.isValidForDelete() will not fetch any
   * child LNs. LN.isValidForDelete() simply returns false.
   *
   * <p>We assume that this is only called under an assert.
   */
  @Override
   ...
   *
   * <p>It is called from IN.postFetch/RecoveryInit(). If the logrec we have just read was a
   * BINDelta, this.lastFullVersion has already been set (in BINDeltaLogEntry.readMainItem() or in
   * OldBinDelta.reconstituteBIN()). So, this method will set this.lastDeltaVersion. Otherwise, if
   * the logrec was a full BIN, this.lastFullVersion has not been set yet, and it will be set here.
   * In this case, this.lastDeltaVersion will remain NULL.
   */
  ...
  /**
   * Returns the last delta version LSN, or NULL_LSN if a delta was not last logged. For BINs, it
   * just returns the value of the lastDeltaVersion field. Public for unit testing.
   */
  ...
  /**
   * When splits and checkpoints intermingle in a deferred write databases, a checkpoint target may
   * appear which has a valid target but a null LSN. Deferred write dbs are written out in
   * checkpoint style by either Database.sync() or a checkpoint which has cleaned a file containing
   * deferred write entries. For example, INa | BINb
   *
   * <p>A checkpoint or Database.sync starts The INList is traversed, dirty nodes are selected BINb
   * is bypassed on the INList, since it's not dirty BINb is split, creating a new sibling, BINc,
   * and dirtying INa INa is selected as a dirty node for the ckpt
   *
   * <p>If this happens, INa is in the selected dirty set, but not its dirty child BINb and new
   * child BINc.
   *
   * <p>In a durable db, the existence of BINb and BINc are logged anyway. But in a deferred write
   * db, there is an entry that points to BINc, but no logged version.
   *
   * <p>This will not cause problems with eviction, because INa can't be evicted until BINb and BINc
   * are logged, are non-dirty, and are detached. But it can cause problems at recovery, because INa
   * will have a null LSN for a valid entry, and the LN children of BINc will not find a home. To
   * prevent this, search for all dirty children that might have been missed during the selection
   * phase, and write them out. It's not sufficient to write only null-LSN children, because the
   * existing sibling must be logged lest LN children recover twice (once in the new sibling, once
   * in the old existing sibling.
   *
   * <p>TODO: Would the problem above be solved by logging dirty nodes using a tree traversal
   * (post-order), rather than using the dirty map?
   *
   * <p>Overriden by BIN class.
   */
  ...
  public final long log(
      boolean allowDeltas, boolean isProvisional, boolean backgroundIO, IN parent) {
  ...
  public final long log(
      boolean allowDeltas, Provisional provisional, boolean backgroundIO, IN parent) {

    return logInternal(
        this, null, allowDeltas, true /*allowCompress*/, provisional, backgroundIO, parent);
  }
  ...
  public static long logEntry(
      INLogEntry<BIN> logEntry, Provisional provisional, boolean backgroundIO, IN parent) {
   ...
   *
   * <p>If 'node' is non-null, 'logEntry' must be null. If 'node' is null, 'logEntry' and 'parent'
   * must be non-null.
   *
   * <p>When 'logEntry' is non-null we are logging an off-heap BIN, and it is not resident in the
   * main cache. The lastFull/DeltaLsns are not updated here, and this must be done instead by the
   * caller.
   *
   * <p>When 'node' is non-null, 'parent' may or may not be null. It must be non-null when logging
  ...
  /**
   * Adds the given obsolete LSN and any tracked obsolete LSNs for the given child IN to this IN's
   * tracking list. This method is called to track obsolete LSNs when a child IN is logged
   * provisionally. Such LSNs cannot be considered obsolete until an ancestor IN is logged
   * non-provisionally.
   */
  void trackProvisionalObsolete(final IN childIN, final long obsoleteLsn) {
    ...
    updateMemorySize(
        oldMemSize, (provisionalObsolete != null) ? provisionalObsolete.getMemorySize() : 0);
  }

  /**
   * Discards the provisional obsolete tracking information in this node after it has been counted
   * in the live tracker. This method is called after this node is logged non-provisionally.
   */
   ...
   *     <p>Overrriden by DIN and DBIN classes.
   */
  @Override
  ...
  public final int getLogSize(boolean deltasOnly) {
    ...
    final int maxEntriesToWrite =
        (!deltasOnly ? getMaxEntries() : bin.getDeltaCapacity(nEntriesToWrite));
    ...
    for (int i = 0; i < nEntries; i++) { // entries
      ...
      size +=
          LogUtils.getByteArrayLogSize(entryKeys.get(i))
              + // key
              (compactLsnsRep ? LogUtils.INT_BYTES : LogUtils.getLongLogSize())
              + // LSN
              1; // state
  ...
  /**
   * WARNING: In the case of BINs this method is not only used for logging but also for off-heap
   * caching. Therefore, this method should not have side effects unless the clearDirtyBits param is
   * true.
   */
  ...
  /**
   * WARNING: In the case of BINs this method is used not only for logging but also for off-heap
   * caching. Therefore, this method should not have side effects unless the clearDirtyBits param is
   * true or an older log version is passed (off-heap caching uses the current version).
   */
  public final void materialize(
      ByteBuffer itemBuffer, int entryVersion, boolean deltasOnly, boolean clearDirtyBits) {
  ...
  /** @see Loggable#logicalEquals Always return false, this item should never be compared. */
  @Override
  ...
  /**
   * Allows subclasses to add additional fields before the end tag. If they just overload dumpLog,
   * the xml isn't nested.
   */
  ...
  /**
   * Converts to an identifying string that is safe to output in a log. Keys are not included for
