...
/**
 * The abstract class that defines the various representations used to represent an array of target
 * pointers to children of an IN node. These arrays can be sparse, so the non-default
 * representations are designed to make efficient representations for the sparse cases. Each
 * specialized representation is a subclass of INTargetReps.
 *
 * <p>A new IN node starts out with the None representation and grows through a sparse into the full
 * default representation. Subsequently, the default representation can be <i>compacted</i> into a
 * Sparse or None representation whenever an IN is stripped. Note that representations do not
 * currently move to more compact forms when entries are nulled to minimize the possibility of
 * tansitionary representation changes, since each representation change has a cpu cost and a gc
...
public abstract class INTargetRep extends INArrayRep<INTargetRep, INTargetRep.Type, Node> {
  ...
  /**
   * Representation used when 1-4 children are cached. Note that the IN itself may have more
   * children, but they are not currently cached. The INArrayRep is represented by two parallel
   * arrays: an array of indices (idxs) and an array of values (targets). All elements that are not
   * explicitly represented are null.
   */
  ...
  public static class None extends INTargetRep {
    ...
    @Override
    public INTargetRep copy(
        @SuppressWarnings("unused") int from,
