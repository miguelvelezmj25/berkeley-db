 ...
 *
 * <p>BIN-deltas ========== A BIN-delta is a BIN with the non-dirty slots omitted. A "full BIN",
 * OTOH contains all slots. On disk and in memory, the format of a BIN-delta is the same as that of
 * a BIN. In memory, a BIN object is actually a BIN-delta when the BIN-delta flag is set
 * (IN.isBINDelta). On disk, the NewBINDelta log entry type (class BINDeltaLogEntry) is the only
 * thing that distinguishes it from a full BIN, which has the BIN log entry type.
 *
 * <p>BIN-deltas provides two benefits: Reduced writing and reduced memory usage.
 *
 * <p>Reduced Writing --------------- Logging a BIN-delta rather a full BIN reduces writing
 * significantly. The cost, however, is that two reads are necessary to reconstruct a full BIN from
 * scratch. The reduced writing is worth this cost, particularly because less writing means less log
 * cleaning.
 *
 * <p>A BIN-delta is logged when 25% or less (configured with EnvironmentConfig TREE_BIN_DELTA) of
 * the slots in a BIN are dirty. When a BIN-delta is logged, the dirty flag is cleared on the the
 * BIN in cache. If more slots are dirtied and another BIN-delta is logged, it will contain all
 * entries dirtied since the last full BIN was logged. In other words, BIN-deltas are cumulative and
 * not chained, to avoid reading many (more than two) log entries to reconstruct a full BIN. The
 * dirty flag on each slot is cleared only when a full BIN is logged.
 *
 * <p>In addition to the cost of fetching two entries on a BIN cache miss, another drawback of the
 * current approach is that dirtiness propagates upward in the Btree due to BIN-delta logging,
 * causing repeated logging of upper INs. The slot of the parent IN contains the LSN of the most
 * recent BIN-delta or full BIN that was logged. A BINDeltaLogEntry in turn contains the LSN of the
 * last full BIN logged.
 *
 * <p>Historical note: The pre-JE 5 implementation of OldBINDeltas worked differently and had a
 * different cost/benefit trade-off. When an OldBINDelta was logged, its dirty flag was not cleared,
 * causing it to be logged repeatedly at every checkpoint. A full BIN was logged after 10 deltas, to
 * prevent endless logging of the same BIN. One benefit of this approach is that the BIN's parent IN
 * was not dirtied when logging the OldBINDelta, preventing dirtiness from propagating upward.
 * Another benefit is that the OldBINDelta was only processed by recovery, and did not have to be
 * fetched to reconstruct a full BIN from scratch on a cache miss. But the cost (the logging of an
 * OldBINDelta every checkpoint, even when it hadn't changed since the last time logged) outweighed
 * the benefits. When the current approach was implemented in JE 5, performance improved due to less
 * logging.
 *
 * <p>In JE 6, deltas were also maintained in the Btree cache. This was done to provide the reduced
 * memory benefits described in the next section. The log format for a delta was also changed. The
 * OldBINDelta log format is different (not the same as the BIN format) and is supported for
 * backward compatibility as the OldBINDeltaLogEntry. Its log entry type name is still BINDelta,
 * which is why the new type is named NewBINDelta (for backward compatibility, log entry type names
 * cannot be changed.) This is also why the spelling "BIN-delta" is used to refer to deltas in the
 * new approach. The old BINDelta class was renamed to OldBINDelta and there is no longer a class
 * named BINDelta.
 *
 * <p>Reduced Memory Usage -------------------- In the Btree cache, a BIN may be represented as a
 * full BIN or a BIN-delta. Eviction will mutate a full BIN to a BIN-delta in preference to
 * discarding the entire BIN. A BIN-delta in cache occupies less memory than a full BIN, and can be
 * exploited as follows:
 *
 * <p>- When a full BIN is needed, it can be constructed with only one fetch rather than two,
 * reducing IO overall. IN.fetchIN implements this optimization.
 *
 * <p>- Certain operations can sometimes be performed using the BIN-delta alone, allowing such
 * operations on a given data set to take place using less less IO (for a given cache size).
 *
 * <p>The latter benefit is not yet implemented. No user CRUD operations are currently implemented
 * using BIN-deltas. In the future we plan to implement the following operations using the BIN-delta
 * alone.
 *
 * <p>- Consider recording deletions in a BIN-delta. Currently, slot deletion prohibits a BIN-delta
 * from being logged. To record deletion in BIN-deltas, slot deletion will have to be deferred until
 * a full BIN is logged.
 *
 * <p>- User reads by key, updates and deletions can be implemented if the key happens to appear in
 * the BIN-delta.
 *
 * <p>- The Cleaner can migrate an LN if its key happens to appear in the BIN-delta. This is similar
 * to a user update operation, but in a different code path.
 *
 * <p>- Insertions, deletions and updates can always be performed in a BIN-delta during replica
 * replay, since the Master operation has already determined whether the key exists.
 *
 * <p>- Recovery LN redo could also apply insertions, updates and inserts in the manner described.
 *
 * <p>- Add idempotent put/delete operations, which can always be applied in a BIN-delta.
 *
 * <p>- Store a hash of the keys in the full BIN in the BIN-delta and use it to perform the
 * following in the delta: - putIfAbsent (true insertion) - get/delete/putIfPresent operations that
 * return NOTFOUND - to avoid accumulating unnecessary deletions
 *
 * <p>However, some internal operations do currently exploit BIN-deltas to avoid unnecessary IO. The
 * following are currently implemented.
 *
 * <p>- The Evictor and Checkpointer log a BIN-delta that is present in the cache, without having to
 * fetch the full BIN.
 *
 * <p>- The Cleaner can use the BIN-delta to avoid fetching when processing a BIN log entry (delta
 * or full) and the BIN is not present in cache,
 *
 * <p>To support BIB-delta-aware operations, the IN.fetchIN() and IN.getTarget() methods may return
 * a BIN delta. IN.getTarget() will return whatever object is cached under the parent IN, and
 * IN.fetchIN() will do a single I/O to fetch the most recently log record for the requested BIN,
 * which may be a full BIN or a delta. Callers of these methods must be prepared to handle a BIN
 * delta; either doing their operation directly on the delta, if possible, or mutating the delta to
 * a full BIN by calling BIN.mutateToFullBIN().
 */
public class BIN extends IN {
   ...
   *
   * <p>minLength is 1 because log sizes are unpredictable.
   *
   * <p>allowSparseRep is false because all slots have log sizes and less mutation is better.
   */
   ...
   *
   * <p>minLength is 5 because VLSNS grow that large fairly quickly, and less mutation is better.
   * The value 5 accomodates data set sizes up to 100 billion. If we want to improve memory
   * utilization for smaller data sets or reduce mutation for larger data sets, we could dynamically
   * determine a value based on the last assigned VLSN.
   *
   * <p>allowSparseRep is false because either all slots typically have VLSNs, or none do, and less
   * mutation is better.
   */
   ...
   *
   * <p>minLength is 8 because memory IDs are 64-bit pointers.
   *
   * <p>allowSparseRep is true because some workloads will only load LN IDs for a subset of the LNs
   * in the BIN.
   */
   ...
   *
   * <p>minLength is 1 because we expect most expiration values, which are an offset from a base
   * day/hour, to fit in one byte.
   *
   * <p>allowSparseRep is true because some workloads only set TTLs on some of the LNs in a BIN.
   */
  private static final INLongRep.EmptyRep EMPTY_EXPIRATION = new INLongRep.EmptyRep(1, true);
  ...
  /**
   * When some LNs are in the off-heap cache, the offHeapLruId is this BIN's index in the off-heap
   * LRU list.
   */
  ...
  /**
   * An expirationValues slot value is one more than the number of days/hours to add to the
   * expirationBase to get the true expiration days/hours. A slot value of zero means no expiration,
   * and a non-zero slot value is one greater than the actual offset to be added. The base is the
  ...
  void setCachedVLSNUnconditional(int idx, long vlsn) {
    vlsnCache = vlsnCache.set(idx, (vlsn == VLSN.NULL_VLSN_SEQUENCE ? 0 : vlsn), this);
  }
  ...
  /**
   * Returns the VLSN. VLSN.NULL_VLSN_SEQUENCE (-1) is returned in two cases: 1) This is a
   * standalone environment. 2) The VLSN is not cached (perhaps VLSN caching is not configured), and
   * the allowFetch param is false.
   *
   * <p>WARNING: Because the vlsnCache is only updated when an LN is evicted, it is critical that
   * getVLSN returns the VLSN for a resident LN before getting the VLSN from the cache.
   */
  ...
  /**
   * The last logged size is never needed when the LN is counted obsolete immediately, since it is
   * only needed for counting an LN obsolete during an update or deletion.
   *
   * <p>This method may not be called until after the database is initialized, i,e., it may not be
   * called during readFromLog.
   */
  @Override
  ...
  boolean mayHaveLastLoggedSizeStored() {
    ...
    if (DatabaseUtil.TEST) {
      ...
      if (TEST_NO_LAST_LOGGED_SIZES && !databaseImpl.getDbType().isInternal()) {
        return false;
      }
    }
   ...
   *
   * <p>This method does not dirty the IN because the caller methods dirty it, for example, when
   * setting the LSN, key, or node.
   *
   * <p>This method is sometimes called to add the logged size for a pre log version 9 BIN, for
   * example, during fetchTarget and preload. This makes the logged size available for obsolete
   * counting but does not dirty the IN, since that could cause an unexpected write of the IN being
   * read.
   *
   * @param lastLoggedSize is positive if the size is known, zero if the size is unknown, or -1 if
   *     the size should not be changed because logging of the LN was deferred.
   */
  @Override
   ...
   *
   * <p>This method is used when reading from the log because the databaseImpl is not yet
   * initialized and isLastLoggedSizeStored cannot be called. It is also called for efficiency
   * reasons when it is known that storing the logged size is necessary, for example, when copying
  ...
  /**
   * Returns the expiration time for a slot. The return value is in days or hours, depending on
   * isExpirationTimeInHours.
   */
  ...
  public boolean isProbablyExpired(int idx) {
    ...
    return getEnv()
        .expiresWithin(getExpiration(idx), isExpirationInHours(), getEnv().getTtlClockTolerance());
  }
  ...
  void appendEntryFromOtherNode(IN from, int fromIdx) {
    ...
    setExpiration(idx, fromBin.getExpiration(fromIdx), fromBin.isExpirationInHours());

    final OffHeapCache ohCache = getOffHeapCache();

    if (ohCache.isEnabled()) {

      offHeapLNIds = offHeapLNIds.set(idx, fromBin.offHeapLNIds.get(fromIdx), this);
   ...
   * @param newSiblingLow
   * @param newSiblingHigh - the low and high entry of "this" that were moved into newSibling.
   */
  @Override
  ...
  /**
   * For each cursor in this BIN's cursor set, ensure that the cursor is actually referring to this
   * BIN.
   */
   ...
   *
   * <p>This must not be a BIN-delta. No cursors can be present on the BIN. Caller is responsible
   * for latching and unlatching this node.
   *
   * <p>If the slot containing the identifier is removed, the identifier key will be changed to the
   * key in the first remaining slot.
   *
   * <p>Normally when a slot is removed, the IN is dirtied. However, during compression the BIN is
   * not dirtied when a slot is removed. This is safe for the reasons described below. Note that the
   * BIN being compressed is always a full BIN, not a delta.
   *
   * <p>+ If the BIN is not dirty and it does not become dirty before shutdown, i.e., it is not
   * logged, then it is possible that this compression will be "lost". However, the state of the
   * slot on disk is expired/deleted, and when the BIN is later fetched from disk, this state will
   * be restored and the compression will be performed again.
   *
   * <p>+ If the slot is dirty, the BIN may also be dirty or may become dirty later, and be logged.
   * Logging a delta would cause the information in the dirty slot to be lost. Therefore, when a
   * dirty slot is removed, we set a flag that prohibits the next BIN logged from being a delta.
   *
   * <p>This optimization (that we don't dirty the BIN and we allow logging a delta after removing a
   * non-dirty slot) has one minor and one major impact:
   *
   * <p>1. When a slot is removed for a deleted record, normally the slot and the BIN will be dirty.
   * Although it is unusual, we may encounter a non-dirty slot for a deleted record. This happens if
   * the slot could not be removed by this method when a full BIN is logged, due to a lock or a
   * cursor, and we compress the full BIN later.
   *
   * <p>2. When a slot is removed for an expired record, it is common that the slot will not be be
   * dirty. In this case, without the optimization, the removal of expired slots would cause more
   * logging and less deltas would be logged.
   *
   * @param localTracker is used only for temporary DBs, and may be specified to consolidate
   *     multiple tracking operations. If null, the tracking is performed immediately in this
   *     method.
   * @return true if all deleted and expired slots were compressed, or false if one or more slots
   *     could not be compressed because we were unable to obtain a lock. A false return value means
   *     "try again later".
   */
  public boolean compress(boolean compressDirtySlots, final LocalUtilizationTracker localTracker) {
    ...
    for (int i = 0; i < getNEntries(); i++) {
      ...
      final boolean expired = envImpl.isExpired(getExpiration(i), isExpirationInHours());
      ...
      if (lsn != DbLsn.NULL_LSN && !lockManager.isLockUncontended(lsn)) {
      ...
      if (entryKeys.compareKeys(
              getIdentifierKey(), keyPrefix, i, haveEmbeddedData(i), getKeyComparator())
          == 0) {
      ...
      if (!expired && db.isDeferredWriteMode()) {
        ...
        if (ln != null && ln.isDirty() && !DbLsn.isTransient(lsn)) {
          ...
          if (db.isTemporary()) {
            ...
            if (localTracker != null) {
              localTracker.countObsoleteNode(lsn, ln.getGenericLogType(), getLastLoggedSize(i), db);
            } else {
              envImpl
                  .getLogManager()
                  .countObsoleteNode(
                      lsn, ln.getGenericLogType(), getLastLoggedSize(i), db, true /*countExact*/);
            }
          } else {
    ...
    if (getMaxEntries() > configuredCapacity && getNEntries() < configuredCapacity) {
  ...
  /**
   * This method is called opportunistically at certain places where a deleted slot is observed
   * (when the slot's PendingDeleted or KnownDeleted flag is set), to ensure that the slot is
   * compressed away. This is an attempt to process slots that were not compressed during the
   * mainstream record deletion process because of cursors on the BIN during compress, or a crash
  ...
  /**
   * Check if this node fits the qualifications for being part of a deletable subtree. It may not
   * have any LN children.
   *
  ...
  boolean isValidForDelete() throws DatabaseException {
    ...
    if (numValidEntries > 0) { // any valid entries, not eligible
      ...
      // cursors on BIN, not eligible
      return nCursors() <= 0;// 0 entries, no cursors
  }
  ...
  protected long printMemorySize() {
    ...
    final long binTotal = inTotal + vlsnCacheOverhead + logSizesOverhead + offHeapLNIdOverhead;

    System.out.format(
        "BIN: %d vlsns: %d logSizes: %d expiration: %d offHeapLNIds: %d %n",
        binTotal, vlsnCacheOverhead, logSizesOverhead, expirationOverhead, offHeapLNIdOverhead);
  ...
  /**
   * Returns the treeAdmin memory in objects referenced by this BIN. Specifically, this refers to
   * the DbFileSummaryMap held by MapLNs
   */
  @Override
  ...
  /**
   * Reduce memory consumption. Note that evicting deferred-write LNs may require logging them,
   * which will mark this BIN dirty. Compression of deleted slots will also mark the BIN dirty.
   *
   * <p>The BIN should be latched by the caller.
   *
   * @return a long number encoding (a) the number of evicted bytes, and (b) whether this BIN is
   *     evictable. (b) will be false if the BIN has any cursors on it, or has any non-evictable
   *     children.
   */
  @Override
  ...
  /**
   * Reduce memory consumption by evicting all LN targets. Note that this may cause LNs to be
   * logged, which will mark this BIN dirty.
   *
   * <p>The BIN should be latched by the caller.
   *
   * @return a long number encoding (a) the number of evicted bytes, and (b) whether this BIN is
  ...
  public long evictLNs() throws DatabaseException {
    ...
    assert isLatchExclusiveOwner() : "BIN must be latched before evicting LNs";
   ...
   *
   * @param ifFetchedCold If true, evict the LN only if it has the FetchedCold flag set.
  ...
  private void logDirtyLN(int idx, LN ln, boolean allowEviction) throws DatabaseException {
    ...
    final boolean force = getDatabase().isDeferredWriteMode() && DbLsn.isTransientOrNull(currLsn);
    ...
    if (force || ln.isDirty()) {
      ...
      CursorImpl.lockAfterLsnChange(dbImpl, currLsn, logItem.lsn, null /*excludeLocker*/);

      /*
       * It is desirable to evict a non-dirty LN that is immediately
       * obsolete, because it will never be fetched again.
       */
      if (allowEviction && (databaseImpl.isLNImmediatelyObsolete() || isEmbeddedLN(idx))) {
        evictLN(idx);
      }
    }
  }
  ...
  int getDeltaCapacity(int numDirtyEntries) {
    ...
    boolean blindOps = (getEnv().allowBlindOps() || getEnv().allowBlindPuts());
  ...
  /**
   * Decide whether to log a full or partial BIN, depending on the ratio of the delta size to full
   * BIN size.
   *
   * <p>Other factors are taken into account: + a delta cannot be logged if the BIN has never been
   * logged before + deltas are not currently supported for DeferredWrite databases + this
   * particular delta may have been prohibited because the cleaner is migrating the BIN or a dirty
   * slot has been removed + if there are no dirty slots, we might as well log a full BIN
   *
   * <p>The restriction on using BIN-deltas for deferred-write DBs is for reasons that are probably
   * no longer relevant. However, we have not tested deltas with DW, so we still prohibit them.
   * Because BIN-deltas may be in cache at the time a DB is opened in DW mode, a workaround is
  ...
  public boolean shouldLogDelta() {
    ...
    final int deltaLimit = (getNEntries() * databaseImpl.getBinDeltaPercent()) / 100;
   ...
   *
   * <p>This method must be called with this node latched exclusively, and canMutateToBINDelta must
   * return true.
   *
  ...
  /**
   * This method assumes that "this" BIN is a delta and creates a clone of it. It is currently used
   * by the DiskOrderedScanner only. The method does not clone the targets array.
   */
  public BIN cloneBINDelta() {

    assert (isBINDelta());

    final BIN bin = new BIN(databaseImpl, getIdentifierKey(), 0 /*capacity*/, getLevel());
  ...
  private void initBINDelta(
      final BIN destBIN, final int nDeltas, final int capacity, final boolean copyTargets) {
  ...
  /**
   * Replaces the contents of this BIN with the given contents. Used in mutating a full BIN to a
   * BIN-delta or for creating a new BIN delta with the given content.
   */
  ...
  /**
   * Fetch the full BIN and apply the deltas in this BIN to it, then use the merged result to
   * replace the contents of this BIN.
   *
   * <p>This method must be called with this node latched exclusively. If 'this' is not a delta,
  ...
  /**
   * Mutates this delta to a full BIN by applying this delta to the fullBIN param and then replacing
   * this BIN's contents with it.
   *
   * <p>This method must be called with this node latched exclusively. 'this' must be a delta.
   *
   * <p>After mutation, the full BIN is compressed and compacted. The compression is particularly
   * important, since BIN-deltas in cache cannot be compressed.
   *
   * <p>The method is public because it is called directly from FileProcessor when it finds a BIN
   * that must be migrated. In that case, fullBIN is a full BIN that has just been read from the
   * log, and it is not part of the memory-resident tree.
   */
      ...
      for (CursorImpl cursor : cursorSet) {
        ...
        if (keys[i] != null) {
          ...
          if ((index & IN.EXACT_MATCH) == 0) {
            throw EnvironmentFailureException.unexpectedState(
                getEnv(),
                "Failed to reposition cursor during " + "mutation of a BIN delta to a full BIN");
          }
  ...
  private BIN fetchFullBIN(DatabaseImpl dbImpl) {
    ...
    try {
      ...
      throw new EnvironmentFailureException(
          ...
          makeFetchErrorMsg(e.toString(), lsn, -1),
          e);
    }
  }

  /**
   * Replaces the contents of this BIN with the contents of the given BIN, including lsns, states,
   * keys and targets. Key prefixing and key/target representations will also be those of the given
   * BIN.
   */
  ...
  private void resize(final int newCapacity) {
    ...
    if (entryLsnByteArray != null) {
      entryLsnByteArray = Arrays.copyOfRange(entryLsnByteArray, 0, newCapacity * 4);
    }
    if (entryLsnLongArray != null) {
      entryLsnLongArray = Arrays.copyOfRange(entryLsnLongArray, 0, newCapacity);
    }
    if (entryStates != null) {
  ...
  /**
   * Create a BIN by fetching its most recent full version from the log and applying to it the
   * deltas in this BIN delta. The new BIN is not added to the INList or the BTree.
   *
   * <p>Called from DiskOrderedScanner.fetchAndProcessBINs() and DiskOrderedScanner.accumulateLNs()
   *
  ...
  /**
   * Given a full version BIN, apply to it the deltas in this BIN delta. The fullBIN will then be
   * complete, but its memory will not be compacted.
   *
   * <p>Called from mutateToFullBIN() above and from SortedLSNTreewalker.
   *
   * @param leaveFreeSlot should be true if a slot will be inserted into the resulting full BIN,
   *     without first checking whether the full BIN must be split, and performing the split if
   *     necessary. If this param is true, the returned BIN will contain at least one free slot. If
   *     this param is false, a BIN with no free slots may be returned. For example, it is important
   *     that false is passed when a split will be performed, since if true were passed, the BIN
   *     would grow beyond its bounds unnecessarily.
   */
  public void reconstituteBIN(DatabaseImpl dbImpl, BIN fullBIN, boolean leaveFreeSlot) {
    ...
    try {
      ...
      if (!dbImpl.getEnv().isInInit()) {
        fullBIN.compress(false /*compressDirtySlots*/, null /*localTracker*/);
      }
      int nInsertions = leaveFreeSlot ? 1 : 0;
      for (int i = 0; i < getNEntries(); i += 1) {
        final int foundIndex = fullBIN.findEntry(getKey(i), true, false);
  ...
  /**
   * Apply (insert, update) a given delta slot in this full BIN. Note: also called from OldBINDelta
   * class.
   */
  void applyDelta(
      final byte[] key,
    ...
    if (foundIndex >= 0 && (foundIndex & IN.EXACT_MATCH) != 0) {
       ...
       */
      applyDeltaSlot(foundIndex, child, lsn, lastLoggedSize, state, key, data);
       ...
       * [#20737]
       */
      final int result = insertEntry1(child, key, data, lsn, state, false /*blindInsertion*/);
