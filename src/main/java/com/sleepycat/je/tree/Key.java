...
/**
 * Key represents a JE B-Tree Key. Keys are immutable. Within JE, keys are usually represented as
 * byte arrays rather than as Key instances in order to reduce the in-memory footprint. The static
 * methods of this class are used to operate on the byte arrays.
 *
 * <p>One exception is when keys are held within a collection. In that case, Key objects are
 * instantiated so that keys are hashed and compared by value.
 */
public final class Key implements Comparable<Key> {
  ...
  public abstract static class DumpType {
    ...
    public static final DumpType BINARY =
        new DumpType("BINARY") {
          @Override
    ...
    public static final DumpType HEX =
        new DumpType("HEX") {
          @Override
          void dumpByteArrayInternal(StringBuilder sb, byte[] b) {
            for (int i = 0; i < b.length; i++) {
    ...
    public static final DumpType TEXT =
        new DumpType("TEXT") {
          @Override
          void dumpByteArrayInternal(StringBuilder sb, byte[] b) {
    ...
    public static final DumpType OBFUSCATE =
        new DumpType("OBFUSCATE") {
          @Override
          void dumpByteArrayInternal(StringBuilder sb, byte[] b) {
            int len = b.length;
   ...
   *
   * <p>Note that any configured user comparison function is not used, and therefore this method
   * should not be used for comparison of keys during Btree operations.
   */
  ...
  public static int compareKeys(
    ...
    if (comparator == null) {
      return compareUnsignedBytes(key1, off1, len1, key2, off2, len2);
    }
  ...
  /** Compare keys with an optional comparator. */
  public static int compareKeys(
      DatabaseEntry entry1, DatabaseEntry entry2, Comparator<byte[]> comparator) {
  ...
  /** Compare using a default unsigned byte comparison. */
  public static int compareUnsignedBytes(
      byte[] key1, int off1, int len1, byte[] key2, int off2, int len2) {
