...
import static com.sleepycat.je.dbi.BTreeStatDefinition.*;

/**
 * Tree implements the JE B+Tree.
 *
 * <p>A note on tree search patterns: There's a set of Tree.search* methods. Some clients of the
 * tree use those search methods directly, whereas other clients of the tree tend to use methods
 * built on top of search.
 *
 * <p>The semantics of search* are they leave you pointing at a BIN or IN they don't tell you where
 * the reference of interest is. The semantics of the get* methods are: they leave you pointing at a
 * BIN or IN they return the index of the slot of interest they traverse down to whatever level is
 * needed they are built on top of search* methods. For the future: Over time, we need to clarify
 * which methods are to be used by clients of the tree. Preferably clients that call the tree use
 * get*, although their are cases where they need visibility into the tree structure.
 *
 * <p>Also, search* should return the location of the slot to save us a second binary search.
 *
 * <p>Search Method Call Hierarchy ---------------------------- getFirst/LastNode search CALLED BY:
 * CursorImpl.getFirstOrLast
 *
 * <p>getNext/PrevBin getParentINForChildIN searchSubTree CALLED BY: DupConvert CursorImpl.getNext
 *
 * <p>getParentINForChildIN IN.findParent does not use shared latching CALLED BY:
 * Checkpointer.flushIN (doFetch=false, targetLevel=-1) FileProcessor.processIN (doFetch=true,
 * targetLevel=LEVEL) Evictor.evictIN (doFetch=true, targetLevel=-1)
 * RecoveryManager.replaceOrInsertChild (doFetch=true, targetLevel=-1) getNext/PrevBin
 * (doFetch=true, targetLevel=-1)
 *
 * <p>search searchSubTree CALLED BY: CursorImpl.searchAndPosition INCompressor to find BIN
 *
 * <p>searchSubTree uses shared grandparent latching
 *
 * <p>getParentBINForChildLN searchSplitsAllowed CALLED BY: RecoveryManager.redo
 * RecoveryManager.recoveryUndo search CALLED BY: RecoveryManager.abortUndo
 * RecoveryManager.rollbackUndo FileProcessor.processLN Cleaner.processPendingLN
 * UtilizationProfile.verifyLsnIsObsolete (utility)
 *
 * <p>findBinForInsert searchSplitsAllowed CALLED BY: CursorImpl.putInternal
 *
 * <p>searchSplitsAllowed uses shared non-grandparent latching CALLED BY: DupConvert (instead of
 * findBinForInsert, which needs a cursor)
 *
 * <p>Possible Shared Latching Improvements ------------------------------------- By implementing
 * shared latching for BINs we would get better concurrency in these cases: Reads when LN is in
...
public final class Tree implements Loggable {
   ...
   * [#13354].
   */
  private static final SplitRequiredException splitRequiredException = new SplitRequiredException();
  ...
  /**
   * Embodies an enum for the type of search being performed. NORMAL means do a regular search down
   * the tree. LEFT/RIGHT means search down the left/right side to find the first/last node in the
   * tree.
   */
      ...
      if (getTarget() == null && !rootLatch.isExclusiveOwner()) {
        ...
        if (this != root) {
          throw EnvironmentFailureException.unexpectedState(
              database.getEnv(), "Root changed while unlatched, dbId=" + database.getId());
        }
      }
  ...
  public void setDatabase(DatabaseImpl database) {
     ...
     */
    final LatchContext latchContext =
        new LatchContext() {
          @Override
  ...
  /**
   * Called when latching a child and the parent is latched. Used to opportunistically validate the
   * parent pointer.
   */
  ...
  public boolean rootExists() {
    if (root == null) {
      ...
      return (root.getTarget() != null) || (root.getLsn() != DbLsn.NULL_LSN);
  }

  /**
   * Perform a fast check to see if the root IN is resident. No latching is performed. To ensure
   * that the root IN is not loaded by another thread, this method should be called while holding a
   * write lock on the MapLN. That will prevent opening the DB in another thread, and potentially
  ...
  /**
   * Helper to obtain the root IN with shared root latching. Optionally updates the generation of
   * the root when latching it.
   */
  ...
  /**
   * Helper to obtain the root IN with exclusive root latching. Optionally updates the generation of
   * the root when latching it.
   */
  ...
  /**
   * Deletes a BIN specified by key from the tree. If the BIN resides in a subtree that can be
   * pruned away, prune as much as possible, so we don't leave a branch that has no BINs.
   *
   * <p>It's possible that the targeted BIN will now have entries, or will have resident cursors.
   * Either will prevent deletion (see exceptions).
   *
   * <p>Unlike splits, IN deletion does not immediately log the subtree parent or its ancestors. It
   * is sufficient to simply dirty the subtree parent. Logging is not necessary for correctness, and
   * if a checkpoint does not flush the subtree parent then recovery will add the BINs to the
   * compressor queue when redoing the LN deletions.
   *
   * @param idKey - the identifier key of the node to delete.
   * @throws NodeNotEmptyException if the BIN is not empty. The deletion is no longer possible.
   * @throws CursorsExistException is the BIN has cursors. The deletion should be retried later by
   *     the INCompressor.
   */
  public void delete(byte[] idKey) throws NodeNotEmptyException, CursorsExistException {
    ...
    try {
      ...
      if (logger.isLoggable(Level.FINEST)) {
        LoggerUtils.envLogMsg(
            ...
            "Tree.delete() "
                ...
                + " from parent node: "
                + branchParent.getNodeId()
                + " parent has "
                + branchParent.getNEntries()
                + " children");
      }
      ...
      if (logger.isLoggable(Level.FINE)) {
        LoggerUtils.envLogMsg(
            Level.FINE, envImpl, "SubtreeRemoval: subtreeRoot = " + branchRoot.getNodeId());
      }
  ...
  /**
   * Search down the tree using a key, but instead of returning the BIN that houses that key, find
   * the point where we can detach a deletable subtree. A deletable subtree is a branch where each
   * IN has one child, and the bottom BIN has no entries and no resident cursors. That point can be
   * found by saving a pointer to the lowest node in the path with more than one entry.
   *
   * <p>INa / \ INb INc | | INd .. / \ INe .. | BINx (suspected of being empty)
   *
   * <p>In this case, we'd like to prune off the subtree headed by INe. INd is the parent of this
   * deletable subtree.
   *
   * <p>The method returns a list of parent/child/index structures. In this example, the list will
   * hold: INd/INe/index INe/BINx/index All three nodes will be EX-latched.
   *
   * @return null if the entire Btree is empty, or a list of SplitInfo for the branch to be deleted.
   *     If non-null is returned, the INs in the list will be EX-latched; otherwise, no INs will be
   *     latched.
  ...
  /**
   * Release latched acquired by searchDeletableSubTree. Each child is latched, plus the parent of
   * the first node (the branch parent).
   */
   ...
   *
   * @return the leftmost node in the tree, null if the tree is empty. The returned node is latched
   *     and the caller must release it.
   */
  public BIN getFirstNode(CacheMode cacheMode) throws DatabaseException {

    BIN bin =
        search(null /*key*/, SearchType.LEFT, null /*binBoundary*/, cacheMode, null /*comparator*/);
   ...
   *
   * @return the rightmost node in the tree, null if the tree is empty. The returned node is latched
   *     and the caller must release it.
   */
  public BIN getLastNode(CacheMode cacheMode) throws DatabaseException {
    ...
    BIN bin =
        search(
            null /*key*/, SearchType.RIGHT, null /*binBoundary*/, cacheMode, null /*comparator*/);
   ...
   * @return The next BIN, or null if there are no more. The returned node is latched and the caller
   *     must release it. If null is returned, the argument BIN remains latched.
   */
   ...
   *
   * @param bin The BIN to find the next BIN for. This BIN is latched.
   * @return The previous BIN, or null if there are no more. The returned node is latched and the
   *     caller must release it. If null is returned, the argument bin remains latched.
   */
  ...
  /**
   * Returns the next IN in the tree before/after the given IN, and at the same level. For example,
   * if a BIN is passed in the prevIn parameter, the next BIN will be returned.
   *
   * <p>TODO: A possible problem with this method is that we don't know for certain whether it works
   * properly in the face of splits. There are comments below indicating it does. But the
   * Cursor.checkForInsertion method was apparently added because getNextBin/getPrevBin didn't work
   * properly, and may skip a BIN. So at least it didn't work properly in the distant past.
   * Archeology and possibly testing are needed to find the truth. Hopefully it does now work, and
   * Cursor.checkForInsertion can be removed.
   *
   * <p>TODO: To eliminate EX latches on upper INs, a new getParentINForChildIN is needed, which
   * will return with both the parent and the grandparent SH-latched. If we do this, then in
   * Tree.getNextIN() the call to searchSubtree() will be able to do grandparent latching, and the
   * call to parent.fetchIN(index) will also be replace with a local version of grandparent
   * latching.
   */
    ...
    try {
      while (true) {
        ...
        final SearchResult result =
            getParentINForChildIN(curr, false, /*useTargetLevel*/ true, /*doFetch*/ cacheMode);
        ...
        if (moreEntriesThisIn) {
          ...
          if (nextIN.getLevel() == targetLevel) {
            ...
            if (ret.getLevel() == targetLevel) {
              ...
              throw EnvironmentFailureException.unexpectedState(
                  "subtree did not have a IN at level " + targetLevel);
            }
          }
        }
  ...
  /**
   * Search for the parent P of a given IN C (where C is viewed as a logical node; not as a java
   * obj). If found, P is returned latched exclusively. The method is used when C has been accessed
   * "directly", i.e., not via a tree search, and we need to perform an operation on C that requires
   * an update to its parent. Such situations arise during eviction (C has been accessed via the LRU
   * list), checkpointing, and recovery (C has been read from the log and is not attached to the
   * in-memory tree).
   *
   * <p>The method uses C's identifierKey to search down the tree until:
   *
   * <p>(a) doFetch is false and we need to access a node that is not cached. In this case, we are
   * actually looking for the cached copies of both C and its parent, so a cache miss on the path to
   * C is considered a failure. This search mode is used by the evictor: to evict C (which has been
   * retrieved from the LRU), its parent must be found and EX-latched; however, if the C has been
   * evicted already by another thread, there is nothing to do (C will GC-ed). or (b) We reach a
   * node whose node id equals the C's node id. In this case, we know for sure that C still belongs
   * to the BTree and its parent has been found. or (c) useTargetLevel is true and we reach a node P
   * that is at one level above C's level. We know that P contains a slot S whose corresponding key
   * range includes C's identifierKey. Since we haven't read the child node under S to check its
   * node id, we cannot know for sure that C is still in the tree. Nevertheless, we consider this
   * situation a success, i.e., P is the parent node we are looking for. In this search mode, after
   * this method returns, the caller is expected to take further action based on the info in slot S.
   * For example, if C was created by reading a log entry at LSN L, and the LSN at slot S is also L,
   * then we know P is the real parent (and we have thus saved a possible extra I/O to refetch the C
   * node from the log to check its node id). This search mode is used by the cleaner. or (d) None
   * of the above conditions occur and the bottom of the BTree is reached. In this case, no parent
   * exists (the child node is an old version of a node that has been removed from the BTree).
   *
   * @param child The child node for which to find the parent. This node is latched by the caller
   *     and is unlatched by this function before returning to the caller.
   * @param useTargetLevel If true, the search is considered successful if a node P is reached at
   *     one level above C's level. P is the parent to return to the caller.
   * @param doFetch if false, stop the search if we run into a non-resident child and assume that no
   *     parent exists.
   * @param cacheMode The CacheMode for affecting the hotness of the nodes visited during the
   *     search.
   * @return a SearchResult object. If the parent has been found, result.foundExactMatch is true,
   *     result.parent refers to that node, and result.index is the slot for the child IN inside the
   *     parent IN. Otherwise, result.foundExactMatch is false and result.parent is null.
   */
  public SearchResult getParentINForChildIN(
      IN child, boolean useTargetLevel, boolean doFetch, CacheMode cacheMode)
  ...
  /**
   * This version of getParentINForChildIN does the same thing as the version above, but also adds a
   * "trackingList" param. If trackingList is not null, the LSNs of the parents visited along the
   * way are added to the list, as a debug tracing mechanism. This is meant to stay in production,
  ...
  /**
   * This version of getParentINForChildIN() is the actual implementation of the previous 2 versions
   * (read the comments there), but it also implements one additional use cases via the extra
   * "requireExactMatch" param.
   *
   * <p>requireExactMatch == false && doFetch == false In this case we are actually looking for the
   * lowest cached ancestor of the C node. The method will always return a node (considered as the
   * "parent") unless the BTree is empty (has no nodes at all). The returned node must be latched,
   * but not necessarily in EX mode. This search mode is used by the checkpointer.
   *
   * <p>The exclusiveLevel param: In general, if exclusiveLevel == L, nodes above L will be SH
   * latched and nodes at or below L will be EX-latched. In all current use cases, L is set to 1 +
   * C.level. Note that if doFetch == false, the normalized exclusiveLevel must be >= 2 so that
   * loadIN can be called.
   */
  public SearchResult getParentINForChildIN(
      long targetNodeId,
      byte[] targetKey,
    ...
    assert (rootIN.getLevel() >= exclusiveLevel)
        : " rootLevel=" + rootIN.getLevel() + " exLevel=" + exclusiveLevel;
  ...
  /**
   * Return a reference to the parent of this LN. This searches through the tree and allows splits,
   * if the splitsAllowed param is true. Set the tree location to the proper BIN parent whether or
   * not the LN child is found. That's because if the LN is not found, recovery or abort will need
   * to place it within the tree, and so we must point at the appropriate position.
   *
   * <p>When this method returns with location.bin non-null, the BIN is latched and must be
   * unlatched by the caller. Note that location.bin may be non-null even if this method returns
   * false.
   *
   * @param location a holder class to hold state about the location of our search. Sort of an
   *     internal cursor.
   * @param key key to navigate through main key
   * @param splitsAllowed true if this method is allowed to cause tree splits as a side effect. In
   *     practice, recovery can cause splits, but abort can't.
   * @param blindDeltaOps Normally, if this method lands on a BIN-delta and the search key is not in
   *     that delta, it will mutate the delta to a full BIN to make sure whether the search key
   *     exists in the tree or not. However, by passing true for blindDeltaOps, the caller indicates
   *     that it doesn't really care whether the key is in the tree or not: it is going to insert
   *     the key in the BIN-delta, if not already there, essentially overwritting the slot that may
   *     exist in the full BIN. So, if blindDeltaOps is true, the method will not mutate a BIN-delta
   *     parent (unless the BIN-delta has no space for a slot insertion).
   * @param cacheMode The CacheMode for affecting the hotness of the tree.
   * @return true if node found in tree. If false is returned and there is the possibility that we
   *     can insert the record into a plausible parent we must also set - location.bin (may be null
   *     if no possible parent found) - location.lnKey (don't need to set if no possible parent).
   */
  ...
  /**
   * Find the BIN that is relevant to the insert. If the tree doesn't exist yet, then create the
   * first IN and BIN. On return, the cursor is set to the BIN that is found or created, and the BIN
   * is latched.
   */
   ...
   *
   * <p>When this returns, parent will be unlatched unless parent is the returned IN.
   */
  private BIN searchSplitsAllowed(
      IN rootIN, byte[] key, CacheMode cacheMode, Comparator<byte[]> comparator)
  ...
  /**
   * Do pre-emptive splitting: search down the tree until we get to the BIN level, and split any
   * nodes that fit the splittable requirement except for the root. If the root needs splitting, a
   * splitRequiredException is thrown and the root split is handled at a higher level.
   *
   * <p>Note that more than one node in the path may be splittable. For example, a tree might have a
   * level2 IN and a BIN that are both splittable, and would be encountered by the same insert
   * operation.
   *
   * <p>Splits cause INs to be logged in all ancestors, including the root. This is to avoid the
   * "great aunt" problem described in LevelRecorder.
   *
   * <p>INs below the root are logged provisionally; only the root is logged non-provisionally.
   * Provisional logging is necessary during a checkpoint for levels less than maxFlushLevel.
   *
   * <p>This method acquires and holds this.rootLatch in EX mode during its whole duration (so
   * splits are serialized). The rootLatch is released on return.
   *
   * @return the tree root node, latched in EX mode. This may be different than the tree root when
   *     this method was called, because no latches are held on entering this method.
   *     <p>All latches are released in case of exception.
   */
      ...
      for (int i = nodeLadder.size() - 1; i >= 0; i -= 1) {
        ...
        if (child.needsSplitting()) {
          ...
          if (allLeftSideDescent || allRightSideDescent) {
            child.splitSpecial(
                parent, index, grandParent, maxTreeEntriesPerNode, key, allLeftSideDescent);
          } else {
  ...
  /**
   * Search the tree, starting at the root. Depending on search type either (a) search for the BIN
   * that *should* contain a given key, or (b) return the right-most or left-most BIN in the tree.
   *
   * <p>Preemptive splitting is not done during the search.
   *
   * @param key - the key to search for, or null if searchType is LEFT or RIGHT.
   * @param searchType - The type of tree search to perform. NORMAL means we're searching for key in
   *     the tree. LEFT/RIGHT means we're descending down the left or right side, resp.
   * @param binBoundary - If non-null, information is returned about whether the BIN found is the
   *     first or last BIN in the database.
   * @return - the BIN that matches the criteria, if any. Returns null if the root is null. BIN is
   *     latched (unless it's null) and must be unlatched by the caller. In a NORMAL search, it is
   *     the caller's responsibility to do the findEntry() call on the key and BIN to locate the
   *     entry (if any) that matches key.
   */
  public BIN search(
      byte[] key,
      ...
      if (!success) {
        ...
        try {
          ...
          LoggerUtils.traceAndLogException(
              database.getEnv(), "Tree", "searchSubTreeInternal", "", e);
        }
      }
    }
  }
  ...
  /**
   * Debugging check that all resident nodes are on the INList and no stray nodes are present in the
   * unused portion of the IN arrays.
   */
   ...
   * enabled.
   */
  private void traceSplitRoot(
      Level level, String splitType, IN newRoot, long newRootLsn, IN oldRoot, long oldRootLsn) {
