 ...
 *
 * <p>Obsolete in log version 8, only used by DupConvert and some log readers.
 */
public final class DIN extends IN {
  ...
  /**
   * Full key for this set of duplicates. For example, if the tree contains k1/d1, k1/d2, k1/d3, the
   * dupKey = k1.
   */
  private byte[] dupKey;
   ...
   * BIN/DIN parents, but INs are not counted by their parents because they are resident on the IN
   * list.
   */
  @Override
  public long computeMemorySize() {
    ...
    if (dupCountLNRef != null) {
      ...
      if (dupCountLNRef.getTarget() != null) {
        size += dupCountLNRef.getTarget().getMemorySizeIncludedByParent();
      }
    }
    return size;
  }
  ...
  protected long printMemorySize() {
    ...
    if (dupCountLNRef != null) {
      dupKeySize = MemoryBudget.byteArraySize(dupCountLNRef.getKey().length);
      if (dupCountLNRef.getTarget() != null) {
        dupLNSize = dupCountLNRef.getTarget().getMemorySizeIncludedByParent();
      }
    }
