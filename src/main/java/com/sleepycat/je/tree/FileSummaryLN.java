 ...
 *
 * <p>The contents of the FileSummaryLN are not fixed until the moment at which the LN is added to
 * the log. A base summary object contains the summary last added to the log. A tracked summary
 * object contains live summary info being updated in real time. The tracked summary is added to the
 * base summary just before logging it, and then the tracked summary is reset. This ensures that the
 * logged summary will accurately reflect the totals calculated at the point in the log where the LN
 * is added.
 *
 * <p>This is all done in the writeToLog method, which operates under the log write latch. All
 * utilization tracking must be done under the log write latch.
 *
 * <p>In record version 1, obsolete offset tracking was added and multiple records are stored for a
 * single file rather than a single record. Each record contains the offsets that were tracked since
 * the last record was written.
 *
 * <p>The key is 8 bytes: 4 bytes for the file number followed by 4 bytes for the sequence number.
 * The lowest valued key for a given file contains the most recent summary information, while to get
 * a complete list of obsolete offsets all records for the file must be read. A range search using
 * just the first 4 bytes can be used to find the most recent record -- this is possible because the
 * sequence number values are decreasing over time for a given file. Here are example keys for three
 ...
 * </pre>
 *
 * <p>The sequence number is the number of obsolete entries counted so far, subtracted from
 * Integer.MAX_VALUE to cause the latest written record to have the lowest key.
 *
 * <h3>Log version information</h3>
 *
 * <p>Version 0: Keys are old format strings. No obsolete detail is present.
 *
 * <p>Version 1: Keys are two 4 byte integers: {file, sequence}. Obsolete detail is present. Some
 * offsets may be invalid if RMW was used.
 *
 * <p>Version 2: The RMW problem with invalid offsets was corrected. There is no data format change;
 * all versions of JE 2.0.x can read version 1.
 *
...
public final class FileSummaryLN extends LN {
   ...
   *
   * @param deletedMarker makes this constructor signature unique, the value passed doesn't matter.
   */
  ...
  /**
   * Sets the live summary object that will be added to the base summary at the time the LN is
   * logged.
   */
  ...
  /**
   * Returns true if the given key for this LN is a String file number key. For the old version of
   * the LN there will be a single record per file.
   *
   * <p>If this is a version 0 log entry, the key is a string. However, such an LN may be migrated
   * by the cleaner, in which case the version will be 1 or greater [#13061]. In the latter case, we
   * can distinguish a string key by:
   *
   * <p>1) If the key is not 8 bytes long, it has to be a string key.
   *
   * <p>2) If the key is 8 bytes long, but bytes[4] is ascii "0" to "9", then it must be a string
   * key. bytes[4] to bytes[7] are a sequence number that is the number of log entries counted. For
   * this number to be greater than 0x30000000, the binary value of 4 digits starting with ascii
   * "0", over 400 million log entries would have to occur in a single file; this should never
   * happen.
   *
   * <p>Note that having to rely on method (2) is unlikely. A string key will only be 8 bytes if the
   * file number reach 8 decimal digits (10,000,000 to 99,999,999). This is a very large file number
   * and unlikely to have occurred using JE 1.7.1 or earlier.
   *
   * <p>In summary, the only time the algorithm here could fail is if there were more than 400
   * million log entries per file, and more than 10 million were written with JE 1.7.1 or earlier.
   */
  ...
  /**
   * Convert a FileSummaryLN key from a byte array to a long. The file number is the first 4 bytes
   * of the key.
   */
  ...
  /**
   * Get the sequence number from the byte array. The sequence number is the last 4 bytes of the
   * key.
   */
  ...
  /**
   * Returns the first 4 bytes of the key for the given file number. This can be used to do a range
   * search to find the first LN for the file.
   */
  ...
  /**
   * Returns the full two-part key for a given file number and unique sequence. This can be used to
   * insert a new LN.
   *
   * @param sequence is a unique identifier for the LN for the given file, and must be greater than
   *     the last sequence.
   */
  ...
  public void postFetchInit(DatabaseImpl db, long sourceLsn) throws DatabaseException {
    ...
    if (entryVersion == 1 && db.getEnv().getCleaner().isRMWFixEnabled()) {
      obsoleteOffsets = new PackedOffsets();
    }
  }
   ...
   *
   * <p>Note: FileSummaryLN will never be transactional.
   */
  @Override
  ...
  /**
   * Adds the extra memory used by obsoleteOffsets to the parent BIN memory size. Must be called
   * after LN is inserted into the BIN and logged, while the cursor is still positioned on the
   * inserted LN. The BIN must be latched. [#17462]
   *
   * <p>The obsoleteOffsets memory size is not intially budgeted in the usual way because
   * PackedOffsets.pack (which changes the memory size) is called during marshalling (see
   * getOffset). This amount is not counted in the parent IN size in the usual way, because LN
   * logging / marshalling occurs after the LN is inserted in the BIN and its memory size has been
   * counted (see CursorImpl.putInternal).
   *
   * <p>Note that the tree memory usage cannot be updated directly in getOffsets because the tree
   * memory usage must always be the sum of all IN sizes, and it is reset to this sum each
   * checkpoint.
   */
  @Override
  ...
  public void dumpKey(StringBuilder sb, byte[] key) {
    sb.append("<fileSummaryLNKey fileNumber=\"0x" + Long.toHexString(getFileNumber(key)) + "\" ");
    sb.append("sequence=\"0x" + Long.toHexString(getSequence(key)) + "\"/>");
