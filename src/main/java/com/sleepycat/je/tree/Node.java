...
public abstract class Node implements Loggable {
  ...
  /**
   * Since DIN/DBIN/DupCountLN are no longer used in the Btree, this method should normally only be
   * used by dup conversion or entities that do not access records via the Btree.
   *
   * @return true if this node is a duplicate-bearing node type, false if otherwise.
   */
  ...
  /**
   * Return the approximate size of this node in memory, if this size should be included in its
   * parents memory accounting. For example, all INs return 0, because they are accounted for
   * individually. LNs must return a count, they're not counted on the INList.
   */
  ...
  /**
   * Returns the generic LogEntryType for this node. Returning the actual type used to log the node
   * is not always possible. Specifically, for LN nodes the generic type is less specific than the
   * actual type used to log the node: + A non-transactional type is always returned. + LOG_INS_LN
   * is returned rather than LOG_UPD_LN. + LOG_DEL_LN is returned rather than LOG_DEL_DUPLN.
   */
