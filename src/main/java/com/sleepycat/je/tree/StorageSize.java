 ...
 *
 * <p>Currently this only applies to KVS because we assume that VLSNs are preserved.
 */
public class StorageSize {
  ...
  /**
   * Returns the estimated disk storage size for the record in the given BIN slot. This method does
   * not fetch the LN.
   *
   * <p>For KVS, a formula that customers will use to predict the storage for a given set of
   * records, not including obsolete size (size available for reclamation by the cleaner), is as
   * follows.
   *
   * <p>The storage overhead for a single Row (JE primary record) is:
   *
   * <pre>
   ...
   * </pre>
   *
   * This method returns the size estimate for an actual record based on the use of that formula,
   * getting the key and data size (or lastLoggedSize) from the BIN. The amount calculated using the
   * formula above will normally be larger than the size returned by this method, for several
   * reasons:
   *
   * <ul>
   *   <li>This method uses the key size after it is reduced by prefix compression.
   *   <li>For a separate (non-embedded) LN, this method uses the lastLoggedSize rather than adding
   *       LN_OVERHEAD to the data size (this is why LN_OVERHEAD is not referenced in code here).
   *       This is more accurate since the actual LN overhead is reduced due to integer packing,
   *       etc. Also, this method cannot fetch the LN, so the data size is unknown.
   *   <li>For an embedded LN in a primary DB, the returned size does not include the LN size, since
   *       the LN is always obsolete. This means the primary key size is not counted redundantly and
   *       the LN_OVERHEAD is not included in the return value, as they are in the formula. These
   *       are significant differences, but since embedded LNs require a data size LTE 16, this is
   *       not expected to be a common use case. If it becomes common, we should add a new case for
   *       this to the customer formula.
   * </ul>
   *
   * In addition, the size returned by this method will normally be larger than the actual storage
   * size on disk. This is because this method uses PRI_SLOT_OVERHEAD and SEC_SLOT_OVERHEAD to
   * calculate the Btree slot space, rather than using the serialized size of the slot. These
   * constant values are somewhat larger than the actual overheads, since they do not take into
   * account integer packing, etc. See the comments above these constants. The serialized slot size
   * was not used here for simplicity and speed, plus this additional size compensates for uncounted
   * sizes such as per-BIN and UIN overhead.
   *
