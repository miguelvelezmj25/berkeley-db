...
/**
 * A ChildReference is a reference in the tree from parent to child. It contains a node reference,
 * key, and LSN.
 */
public class ChildReference implements Loggable {
  ...
  /**
   * Fetch the target object that this ChildReference refers to. If the object is already in VM,
   * then just return the reference to it. If the object is not in VM, then read the object from the
   * log. If the object has been faulted in and the in arg is supplied, then the total memory size
   * cache in the IN is invalidated.
   *
   * @param database The database that this ChildReference resides in.
   * @param parent The IN that this ChildReference lives in. If the target is fetched (i.e. it is
   *     null on entry), then the total in memory count is invalidated in the IN. May be null. For
   *     example, the root is a ChildReference and there is no parent IN when the rootIN is fetched
   *     in.
   * @return the Node object representing the target node in the tree, or null if there is no target
   *     of this ChildReference, or null if a pendingDelete or knownDeleted entry has been cleaned.
   */
  ...
  /**
   * Sets the target for this ChildReference. No need to make dirty, that state only applies to key
   * and LSN.
   */
  ...
  /**
   * Clear the target for this ChildReference. No need to make dirty, that state only applies to key
   * and LSN. This method is public because it's safe and used by RecoveryManager. This can't
   * corrupt the tree.
   */
  ...
  /**
   * @return true if the entry has been deleted, although the transaction the performed the deletion
