...
public abstract class Stats extends JPanel {
  ...
  private final Object[] envStatParams = new Object[] {true /* setClear */, true /* setFast */};
  ...
  public Stats(MBeanServerConnection connection) {
    ...
    statsCollector.scheduleAtFixedRate(new StatsCollectionTask(), 0, statsIntervalMillis);
  ...
  private void initContainers() {
    ...
    for (ObjectName name : getBeansNames()) {
      Map<String, String> storeMap = new LinkedHashMap<String, String>();
  ...
  private JPanel createMBeanPanel() {
    ...
    return generatePanel(
        new JComponent[] {new JLabel("Choose JE Environment:"), mBeansComboBox},
        new FlowLayout(),
        "Choose JE MBean");
  }
  ...
  private String getMBeanComboBoxString(ObjectName name) {
    String envHome =
        name.toString().substring(name.toString().indexOf("(") + 1, name.toString().length() - 1);
    if (envHome.length() > 40) {
      envHome =
          envHome.substring(0, 19)
              + "..."
              + envHome.substring(envHome.length() - 20);
    }
  ...
  private JPanel createLogPanel() {
    startLogButton = createButton("Start Recording", new StartLogListener());
    ...
    saveLogButton = createButton("Record Statistics To ...", new SaveLogListener(this));

    return generatePanel(
        new JComponent[] {startLogButton, stopLogButton, saveLogButton},
        new FlowLayout(),
        "Record Statistics");
  }
  ...
  private JPanel createStatsPanel() {
    ...
    return generatePanel(new JComponent[] {splitPane}, new FlowLayout(), "JE Stats Table");
  }
  ...
  public void removeGraphFrame(ObjectName beanName, String graphStats) {
    CopyOnWriteArrayList<GraphFrame> list = new CopyOnWriteArrayList<GraphFrame>(frameList);
    for (GraphFrame frame : list) {
      if (frame.getBeanName().equals(beanName) && frame.getStatsName().equals(graphStats)) {
  ...
  private synchronized Map<String, String> generateStats(ObjectName name) {
    ...
    try {
      if (connection != null && connection.queryNames(name, null) != null) {
        String status = (String) connection.invoke(name, opName, envStatParams, signature);
  ...
  private void writeToLog(Map<String, String> map, ObjectName currentName) {
    try {
      ...
      if (item.getLogName() == null) return;
  ...
  private void getCSVOutput(
      StringBuilder buffer, Map<String, String> map, boolean init, ObjectName name) {
    ...
    if (map.size() > 0) {
      ...
      int count = 1;
      for (Map.Entry<String, String> entry : map.entrySet()) {
        String title = entry.getKey().substring(0, entry.getKey().indexOf(":")).trim();
        if (!init) {
          title = "\"" + entry.getValue() + "\"";
        }
        if (count < map.size()) title = title + ",";
        buffer.append(title);
        count++;
      }
    }
  ...
  private class LogObject {
    ...
    private final ArrayList<String> turnOffIndex = new ArrayList<String>();
  ...
  public class StatsTableModel extends AbstractTableModel {
    ...
    private final String[] columnNames = {"Stat Name", "Value"};
  ...
  private class StringRenderer extends DefaultTableCellRenderer {
    ...
    public Component getTableCellRendererComponent(
       ...
       */
      if (column == 1) {
        String newValue = valueStore.get(objName).get(table.getValueAt(row, 0));
        if (newValue != null && !newValue.equals(table.getValueAt(row, 1))) {

          valueStore
              .get(objName)
              .put(table.getValueAt(row, 0).toString(), table.getValueAt(row, 1).toString().trim());
    ...
    public void approveSelection() {
      ...
      if (file != null) {
        ...
        if (filter != null && filter instanceof FileNameExtensionFilter) {
          ...
          boolean goodExt = extensions.length > 0;
  ...
  private class StartLogListener implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      ...
      enableComponent(new JComponent[] {saveLogButton, startLogButton, logMenuItem}, false);
      doLog = true;
    }
  }
  ...
  private class StopLogListener implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      enableComponent(new JComponent[] {saveLogButton, logMenuItem}, true);
      doLog = false;
      startLogButton.setEnabled(false);
      stopLogButton.setEnabled(false);
  ...
  private class SaveLogListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (ret == JFileChooser.APPROVE_OPTION) {
        logMap.get(objName).setLogName(fileChooser.getSelectedFile().getAbsolutePath());
  ...
  private class BeanComboBoxListener implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      ...
      objName = comboToObjects.get(mBeansComboBox.getSelectedItem());
      /* Repaint the tab. */
      drawNewStats.run();
    }
  }
  ...
  private class StatsTypeListener implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      shownStats.put(
          ((JCheckBox) e.getSource()).getText(), ((JCheckBox) e.getSource()).isSelected());
      /* Repaint the tab. */
      drawNewStats.run();
    }
  }
  ...
  private class LogMenuListener implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      ...
      if (selectedRow > -1) {
        String title = statsTable.getValueAt(selectedRow, 0).toString().trim();

        if (!logMenuItem.getState() && !currentBean.getTurnOff().contains(title)) {
  ...
  private class GraphMenuListener implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      if (selectedRow > -1) {
        String graphStats = statsTable.getValueAt(selectedRow, 0).toString().trim();
        if (!isTitleEqual(graphStats)) {
          boolean initialized = false;
          for (GraphFrame frame : frameList) {
            if (frame.getBeanName().equals(objName) && frame.getStatsName().equals(graphStats)) {
              initialized = true;
              break;
            }
          }
    ...
    public void mouseClicked(MouseEvent e) {
      if (e.getButton() == MouseEvent.BUTTON3) {
        ...
        if (row >= 0) {
          ...
          final String statName = statsTable.getValueAt(row, 0).toString().trim();
            logMenuItem.setState(!logMap.get(objName).getTurnOff().contains(statName));

          /* Check which stats shouldn't be graphed. */
          try {
            String statValue = getParsedValue(statsTable.getValueAt(row, 1).toString().trim());
   ...
   *
   * <p>Note: stats whose type is float are not supported currently.
   */
  private class GraphFrame extends JFrame {
    ...
    private final PlotterPanel plotterPanel;
    ...
    public GraphFrame(ObjectName beanName, String statsName) {
      ...
      plotterPanel.getPlotter().createSequence(stats, stats, statsColor, true);
      TimeComboBox timeComboBox = new TimeComboBox(plotterPanel.getPlotter());
    ...
    public void writeData(Map<String, String> map) {
      for (Map.Entry<String, String> entry : map.entrySet()) {
        if (entry.getKey().indexOf(":") > 0) {
          String realKey = entry.getKey().substring(0, entry.getKey().indexOf(":")).trim();
          ...
          if (stats.equals(realKey)) {
            ...
            plotterPanel.getPlotter().addValues(System.currentTimeMillis(), Long.valueOf(value));
          }
        }
      }
    }
  ...
  private class StatsCollectionTask extends TimerTask {
    ...
    public void run() {
      ...
      for (ObjectName objectName : objectNames) {
        ...
        Map<String, String> copiedStats = new LinkedHashMap<String, String>();
