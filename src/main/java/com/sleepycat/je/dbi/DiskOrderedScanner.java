 ...
 *
 * <p>Unlike SortedLSNTreeWalker, for which the primary use case is preload, this class notifies the
 * callback while holding a latch only if that can be done without blocking (e.g., when the callback
 * can buffer the data without blocking). In other words, while holding a latch, the callback will
 * not be notified if it might block. This is appropriate for the DOS (DiskOrderedCursor) use case,
 * since the callback will block if the DOS queue is full, and the user's consumer thread may not
 * empty the queue as quickly as it can be filled by the producer thread. If the callback were
 * allowed to block while a latch is held, this would block other threads attempting to access the
 * database, including JE internal threads, which would have a very detrimental impact.
 *
 * <p>Algorithm =========
 *
 * <p>Terminology ----------- callback: object implementing the RecordProcessor interface process:
 * invoking the callback with a key-data pair iteration: top level iteration consisting of phase I
 * and II phase I: accumulate LSNs phase II: sort, fetch and process LSNs
 *
 * <p>Phase I and II -------------- To avoid processing resident nodes (invoking the callback with a
 * latch held), a non-recursive algorithm is used. Instead of recursively accumulating LSNs in a
 * depth-first iteration of the tree (like the SortedLSNTreeWalker algorithm), level 2 INs are
 * traversed in phase I and LSNs are accumulated for LNs or BINs (more on this below). When the
 * memory or LSN batch size limit is exceeded, phase I ends and all tree latches are released.
 * During phase II the previously accumulated LSNs are fetched and the callback is invoked for each
 * key or key-data pair. Since no latches are held, it is permissible for the callback to block.
 *
 * <p>One iteration of phase I and II processes some subset of the database. Since INs are traversed
 * in tree order in phase I, this subset is described by a range of keys. When performing the next
 * iteration, the IN traversal is restarted at the highest key that was processed by the previous
 * iteration. The previous highest key is used to avoid duplication of entries, since some overlap
 * between iterations may occur.
 *
 * <p>LN and BIN modes ---------------- As mentioned above, we accumulate LSNs for either LNs or
 * BINs. The BIN accumulation mode provides an optimization for key-only traversals and for all
 * traversals of duplicate DBs (in a dup DB, the data is included in the key). In these cases we
 * never need to fetch the LN, so we can sort and fetch the BIN LSNs instead. This supports at least
 * some types of traversals that are efficient when all BINs are not in the JE cache.
 *
 * <p>We must only accumulate LN or BIN LSNs, never both, and never the LSNs of other INs (above
 * level 1). If we broke this rule, there would be no way to constrain memory usage in our
 * non-recursive approach, since we could not easily predict in advance how much memory would be
 * needed to fetch the nested nodes. Even if we were able predict the memory needed, it would be of
 * little advantage to sort and fetch a small number of higher level nodes, only to accumulate the
 * LSNs of their descendants (which are much larger in number). The much smaller number of higher
 * level nodes would likely be fetched via random IO anyway, in a large data set anyway.
 *
 * <p>The above justification also applies to the algorithm we use in LN mode, in which we
 * accumulate and fetch only LN LSNs. In this mode we always fetch BINs explicitly (not in LSN
 * sorted order), if they are not resident, for the reasons stated above.
 *
 * <p>Furthermore, in BIN mode we must account for BIN-deltas. Phase I must keep a copy any
 * BIN-deltas encountered in the cache. And phase II must make two passes for the accumulated LSNs:
 * one pass to load the deltas and another to load the full BINs and merge the previously loaded
 * deltas. Unfortunately we must budget memory for the deltas during phase I; since most BIN LSNs
 * are for deltas, not full BINs, we assume that we will need to temporarily save a delta for each
 * LSN. This two pass approach is not like the recursive algorithm we rejected above, however, in
 * two respects: 1) we know in advance (roughly anyway) how much memory we will need for both
 * passes, and 2) the number of LSNs fetched in each pass is roughly the same.
 *
 * <p>Data Lag -------- In phase I, as an exception to what was said above, we sometimes process
 * nodes that are resident in the Btree (in the JE cache) if this is possible without blocking. The
 * primary intention of this is to provide more recent data to the callback. When accumulating BINs,
 * if the BIN is dirty then fetching its LSN later means that some recently written LNs will not be
 * included. Therefore, if the callback would not block, we process the keys in a dirty BIN during
 * phase I. Likewise, when accumulating LNs in a deferred-write database, we process dirty LNs if
 * the callback would not block. When accumulating LN LSNs for a non-deferred-write database, we can
 * go further and process all resident LNs, as long as the callback would not block, since we know
 * that no LNs are dirty.
 *
 * <p>In spite of our attempt to process resident nodes, we may not be able to process all of them
 * if doing so would cause the callback to block. When we can't process a dirty, resident node, the
 * information added (new, deleted or updated records) since the node was last flushed will not be
 * visible to the callback.
 *
 * <p>In other words, the data presented to the callback may lag back to the time of the last
 * checkpoint. It cannot lag further back than the last checkpoint, because: 1) the scan doesn't
 * accumulate LSNs any higher than the BIN level, and 2) checkpoints flush all dirty BINs. For a
 * DOS, the user may decrease the likelihood of stale data by increasing the DOS queue size,
 * decreasing the LSN batch size, decreasing the memory limit, or performing a checkpoint
 * immediately before the start of the scan. Even so, it may be impossible to guarantee that all
 * records written at the start of the scan are visible to the callback.
 */
public class DiskOrderedScanner {
  ...
  interface RecordProcessor {
    ...
    /**
     * Process a key-data pair, in user format (dup DB keys are already split into key and data).
     *
     * @param key always non-null.
     * @param data is null only in keys-only mode.
     */
    void process(int dbIdx, byte[] key, byte[] data, int expiration, boolean expirationInHours);

    /**
     * Returns whether process() can be called nRecords times, immediately after calling this
     * method, without any possibility of blocking. For example, with DOS this method returns true
     * if the DOS queue has nRecords or more remaining capacity.
     */
  ...
  public static class OffHeapBinRef {
    ...
    OffHeapBinRef(
        DiskOrderedScanner scanner, DBContext ctx, long binLsn, long fullBinLsn, int ohBinId) {
  ...
  public static class DeferredLsnsBatch {
    ...
    static final int LSN_MEM_OVERHEAD =
        (MemoryBudget.HASHSET_ENTRY_OVERHEAD + MemoryBudget.LONG_OVERHEAD);
  ...
  private static final LogEntryType[] LN_ONLY =
      new LogEntryType[] {LogEntryType.LOG_INS_LN /* Any LN type will do. */};
  ...
  private static final LogEntryType[] BIN_OR_DELTA =
      new LogEntryType[] {
        LogEntryType.LOG_BIN, LogEntryType.LOG_BIN_DELTA, LogEntryType.LOG_OLD_BIN_DELTA,
      };
  ...
  private static final int ACCUMULATED_MEM_LIMIT = 100000; // bytes

  private static final int SUSPENSION_INTERVAL = 50; // in milliseconds
  ...
  /**
   * Returns whether phase I should terminate because the memory or LSN batch size limit has been
   * exceeded.
   *
   * <p>This method need not be called every LN processed; exceeding the limits by a reasonable
   * amount should not cause problems, since the limits are very approximate measures anyway. It is
   * acceptable to check for exceeded limits once per BIN, and this is currently how it is used.
   */
  ...
  /**
   * Called to perform a disk-ordered scan. Returns only when the scan is complete; i.e, when all
   * records in the database have been passed to the callback.
   */
  void scan(final String protectedFilesNamePrefix, final long protectedFilesId) {

    final FileProtector.ProtectedFileSet protectedFileSet =
        env.getFileProtector()
            .protectActiveFiles(protectedFilesNamePrefix + "-" + protectedFilesId);
    try {
      if (scanSerial) {
        scanSerial();
      } else {
  ...
  private void scanInterleaved() {
    ...
    while (true) {
      ...
      try {
        ...
        for (int dbidx = 0; dbidx < numDBs; ++dbidx) {
          if (dbs[dbidx].parent != null && dbs[dbidx].parentIsLatched) {
    ...
    if (debug) {
      System.out.println("Producer done in " + nIterations + " iterations");
    }
  }
      ...
      if (binsOnly && ctx.lastBinLsn != DbLsn.NULL_LSN) {
        ...
        for (DeferredLsnsBatch batch : deferredLsns) {
          ...
          if (batch.containsLsn(ctx.lastBinLsn)) {
            ...
            BIN bin = (BIN) fetchItem(ctx.lastBinLsn, BIN_OR_DELTA);
            ...
            assert (Key.compareKeys(
                    ctx.newEndingKey, ctx.prevEndingKey, ctx.dbImpl.getKeyComparator())
                >= 0);
  ...
  /**
   * Implements guts of phase I in binsOnly mode. Accumulates BIN deltas and BIN LSNs for the
   * children of the given level 2 IN parent, and processes resident BINs under certain conditions;
   * see algorithm at top of file.
   */
    ...
    while (ctx.pidx < ctx.parent.getNEntries()) {
      ...
      if (ctx.prevEndingKey == null
          || (!ctx.safeToUseCachedDelta
              && ctx.pidx > 0
              && Key.compareKeys(
                      ctx.prevEndingKey, ctx.parent.getKey(ctx.pidx), ctx.dbImpl.getKeyComparator())
                  < 0)) {
      ...
      try {
        if (bin != null || ohBinId >= 0) {
          ...
          if (isBinDelta) {
            ...
            if (bin != null) {
              ...
              if (ctx.parent.isOffHeapBINDirty(ctx.pidx) || !ctx.safeToUseCachedDelta) {

                addDirtyDeltaRef(ohCache.loadBIN(env, ohBinId));
              } else {
                addCleanDeltaOffHeapRef(ctx, binLsn, ohInfo.fullBINLsn, ohBinId, ohBinPri2);
              }
            }
          ...
          if (ctx.prevEndingKey != null) {
            int cmp =
                Key.compareKeys(
                    bin.getKey(binNEntries - 1), ctx.prevEndingKey, ctx.dbImpl.getKeyComparator());
        ...
        if (bin == null || processor.getCapacity() < binNEntries) {
          ...
          if (debug) {
            System.out.println("Phase 1." + nIterations + ": accumulated bin lsn: " + binLsn);
          }

        } else if (processor.canProcessWithoutBlocking(binNEntries)) {

          if (debug) {
            System.out.println("Phase 1." + nIterations + ": Processing bin: " + bin.getNodeId());
          }
  ...
  void addCleanDeltaOffHeapRef(
      DBContext ctx, long binLsn, long fullBINLsn, int ohBinId, boolean ohBinPri2) {

    binDeltas.add(new OffHeapBinRef(this, ctx, binLsn, fullBINLsn, ohBinId));
  ...
  void addDirtyDeltaRef(BIN delta) {
    ...
    if (debug) {
      System.out.println(
          "Phase 1." + nIterations + ": copied dirty or unsafe bin delta " + delta.getNodeId());
    }
  }

  /**
   * Implements guts of phase I in LNs-only mode (binsOnly is false). Accumulates LN LSNs for the
   * BIN children of the given level 2 IN parent, and processes resident LNs under certain
   * conditions; see algorithm at top of file.
   */
    ...
    while (ctx.pidx < parent.getNEntries()) {
      ...
      try {
        ...
        if (waitForConsumer) {
          ...
          boolean exact = (bidx >= 0 && ((bidx & IN.EXACT_MATCH) != 0));
        ...
        for (; bidx < bin.getNEntries(); ++bidx) {
          ...
          if (processor.canProcessWithoutBlocking(1)) {
            ...
            processRecord(
                ctx, ctx.binKey, data, bin.getExpiration(bidx), bin.isExpirationInHours());
            continue;
          }
  ...
  boolean skipParentSlot(DBContext ctx) {
    ...
    if (ctx.checkLevel2Keys
        ...
        && Key.compareKeys(
                ctx.prevEndingKey, ctx.parent.getKey(ctx.pidx + 1), ctx.dbImpl.getKeyComparator())
            >= 0) {
      return true;
    }
  ...
  void resumeParent(DBContext ctx) {
    ...
    if (ctx.pidx >= ctx.parent.getNEntries()
        || ctx.plsn != ctx.parent.getLsn(ctx.pidx)
        || (ctx.dbImpl.isDeferredWriteMode() && ctx.parent.getTarget(ctx.pidx) != null)) {
  ...
  private void fetchAndProcessBINs() {
    ...
    if (deferredBatch != null) {
      ...
      for (Long lsn : deferredBatch.lsns) {
        ...
        if (debug) {
          System.out.println("Phase 2." + nIterations + " Found deferred LSN: " + lsn);
        }
    ...
    if (debug) {
      System.out.println(
          "Phase 2." + nIterations + " Num LSNs to read during phase 2a: " + lsns.length);
    }
    ...
    for (long lsn : lsns) {
      ...
      if (bin.isBINDelta(false /*checkLatched*/)) {
        ...
        isDeferred = (deferredBatch != null && deferredBatch.removeLsn(this, lsn, memSize));
      ...
      isDeferred = (deferredBatch != null && deferredBatch.removeLsn(this, lsn, 0));
    ...
    for (int i = 0; i < nDeltas; i += 1) {
      ...
      if (o instanceof OldBINDelta) {
        ...
        BIN bin = env.getOffHeapCache().loadBINIfLsnMatches(env, ref.ohBinId, ref.binLsn);

        if (bin == null) {
          nextDeferredBatch = addDeferredLsn(nextDeferredBatch, ref.binLsn, ref.memSize);
  ...
  private void processBINInternal(DBContext ctx, BIN bin, boolean isDeferred) {
    ...
    */
    ...
    for (int i = 0; i < bin.getNEntries(); i += 1) {
      ...
      processRecord(ctx, key, data, bin.getExpiration(i), bin.isExpirationInHours());
    }
  }
  ...
  private void fetchAndProcessLNs() {
    ...
    for (long lsn : lsns) {
      ...
      final LNLogEntry<?> entry = (LNLogEntry<?>) fetchEntry(lsn, LN_ONLY);
      ...
      processRecord(
          ctx, entry.getKey(), ln.getData(), entry.getExpiration(), entry.isExpirationInHours());
    }
  ...
  /**
   * Invokes the callback to process a single key-data pair. The parameters are in the format stored
   * in the Btree, and are translated here to user-format for dup DBs.
   */
  private void processRecord(
      DBContext ctx, byte[] treeKey, byte[] treeData, int expiration, boolean expirationInHours) {
    ...
    if (dupDBs && !countOnly) {
      ...
      final DatabaseEntry dataEntry = (keysOnly ? null : new DatabaseEntry());
    ...
    if (ctx.newEndingKey == null
        || Key.compareKeys(ctx.newEndingKey, treeKey, ctx.dbImpl.getKeyComparator()) < 0) {
   ...
   *
   * @param expectTypes is used to validate the type of the entry; an internal exception is thrown
   *     if the log entry does not have one of the given types.
   */
  ...
  private LogEntry fetchEntry(long lsn, LogEntryType[] expectTypes) {
    ...
    final LogEntry entry = logManager.getLogEntryHandleFileNotFound(lsn);
  ...
  private static int getDeltaMemSize(DBContext ctx, long lsn) {
     ...
     */
    final float avgINSize = (((float) summary.totalINSize) / summary.totalINCount);
  ...
  private boolean isBinProcessedBefore(DBContext ctx, BIN bin) {
    ...
    if (ctx.prevEndingKey != null && bin.getNEntries() > 0) {
      final byte[] firstKey = bin.getKey(0);
        ...
        return Key.compareKeys(firstKey, ctx.prevEndingKey, ctx.dbImpl.getKeyComparator()) <= 0;
    }
  ...
  /**
   * Returns whether to skip a BIN slot because its LN is deleted or expired, or its key has already
   * been processed in a previous iteration.
   */
  private boolean skipSlot(DBContext ctx, BIN bin, int index, boolean checkBinKeys) {
    ...
    return ctx.prevEndingKey != null
        && checkBinKeys
        && Key.compareKeys(ctx.prevEndingKey, ctx.binKey, ctx.dbImpl.getKeyComparator()) >= 0;
  }

  /**
   * Moves to the first level 2 IN in the database if searchKey is null (signifying the first
   * iteration), or the level 2 IN containing searchKey if it is non-null.
   *
   * <p>We take the liberty of fetching the BIN (first BIN or BIN for the searchKey), when it is not
   * resident, although in an ideal world no BINs would be added to the cache. Since this only
   * occurs once per iteration, it is considered to be acceptable.
   */
  private void getFirstIN(DBContext ctx, byte[] searchKey) {
    ...
    throw EnvironmentFailureException.unexpectedState(
        "Unable to find BIN for key: " + Arrays.toString(searchKey));
  }
  ...
  private void getNextIN(DBContext ctx) {
    ...
    ctx.parent =
        ctx.dbImpl
            .getTree()
            .getNextIN(ctx.parent, true /*forward*/, true /*latchShared*/, CacheMode.UNCHANGED);
  ...
  public void evictBinRefs() {
    ...
    for (Object o : binDeltas) {
      if (o instanceof OffHeapBinRef) {
        ...
        env.getOffHeapCache().evictBINIfLsnMatch(env, ohRef.ohBinId, ohRef.binLsn);
