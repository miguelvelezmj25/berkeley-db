...
/**
 * Class that invokes the triggers associated with a database. It encapsulates the mechanics
 * associated with actually invoking a trigger.
 */
public class TriggerManager {
  ...
  /**
   * Returns the environment handle that will be passed in as an argument to a database open
   * trigger.
   *
   * <p>To ensure that an environment handle is always available, an internal handle is created and
   * stored in the EnvironmentImpl. The lifetime of the internal handle (Environment or
   * ReplicatedEnvironment) roughly aligns with the lifetime of the underlying EnvironmentImpl, or
   * it's subtype RepImpl.
   *
   * <p>For standalone environments, using explicit transactions, the environment handle that's
   * passed as the argument is the one used to initiate the transaction. When using AutoTransactions
   * to open a database, the environment argument to the trigger is the internal environment handle.
   *
   * <p>For replicated environments, the argument to the trigger is the internal environment handle
   * in all cases. This is done to make the behavior of the parameter deterministic and independent
   * of the interaction of the application level database open operations with those initiated from
   * the "replay" stream.
   *
  ...
  /**
   * Invokes the trigger methods associated with the closing of a database. Note that this also
   * results in the invocation of removeTrigger methods, for transient triggers.
   */
  ...
  /**
   * Invokes the trigger methods associated with the removal of a database. Note that this also
   * results in the invocation of removeTrigger methods.
   */
  ...
  /**
   * Invokes the trigger methods associated with the commit of a transaction. Trigger methods are
   * only invoked if the txn was associated with a trigger invocation.
   */
  ...
  /**
   * Invokes the trigger methods associated with the abort of a transaction. Trigger methods are
   * only invoked if the txn was associated with a trigger invocation.
   */
  ...
  public static void runPutTriggers(
      Locker locker,
      DatabaseImpl dbImpl,
  ...
  public static void runDeleteTriggers(
      Locker locker, DatabaseImpl dbImpl, final DatabaseEntry key, final DatabaseEntry oldData) {
    assert key != null;
  ...
  /**
   * Generic method for invoking any trigger operation. It iterates over all the triggers associated
   * with the database and if the trigger fails invalidates the environment.
   *
   ...
   */
  private static void runTriggers(
      final DatabaseImpl dbImpl, final Locker locker, TriggerInvoker invoker) {
    ...
    Transaction triggerTransaction =
        (locker instanceof Txn) ? locker.getTransaction() : null;
  ...
  private abstract static class TriggerInvoker {
    /*
  ...
  /**
   * Invoke the triggers associated with the addition or removal of the trigger itself. They are
   * typically invoked upon database open, or database removal.
   *
   ...
   */
  public static void invokeAddRemoveTriggers(
      Locker locker, List<Trigger> oldTriggers, List<Trigger> newTriggers) {

    Set<String> oldNames =
        new MapOver<String, Trigger>(oldTriggers) {
          @Override
    ...
    Set<String> newNames =
        new MapOver<String, Trigger>(newTriggers) {
          @Override
          protected String fun(Trigger e) {
            return e.getName();
          }
    ...
    Transaction txn = (locker instanceof Txn) ? locker.getTransaction() : null;
   ...
   */
  public abstract static class MapOver<R, E> {
