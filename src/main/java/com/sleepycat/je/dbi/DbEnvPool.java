...
/**
 * Singleton collection of environments. Responsible for environment open and close, supporting this
 * from multiple threads by synchronizing on the pool.
 *
 * <p>To avoid multiple environment openings from being blocked by recovery in getEnvironment(), the
 * EnvironmentImpl constructor is broken into two parts, with the second part
 * (EnvironmentImpl.finishInit) doing the recovery.
 *
 * <p>When synchronizing on two or more of the following objects the synchronization order must be
 * as follows. Synchronization is not performed in constructors, of course, because no other thread
 * can access the object.
 *
 * <p>Synchronization order: Environment, DbEnvPool, EnvironmentImpl, Evictor
 */
public class DbEnvPool {
  /* Singleton instance. */
  private static final DbEnvPool pool = new DbEnvPool();
    ...
    synchronized (this) {
      ...
      if (envImpl != null) {
        ...
        if (envImpl.isArbiter()) {
          throw new UnsupportedOperationException(
              "An Arbiter is currently using " + "this directory. " + envHome.getAbsolutePath());
        }
        ...
        envImpl =
            (repConfigProxy == null)
                ...
                : loadRepImpl(envHome, config, sharedCacheEnv, repConfigProxy);
  ...
  /**
   * Use reflection to create a RepImpl, to avoid introducing HA compilation dependencies to
   * non-replication code.
   */
  private EnvironmentImpl loadRepImpl(
      File envHome,
      EnvironmentConfig config,
  ...
  /**
   * Called by EnvironmentImpl.setMutableConfig to perform the setMutableConfig operation while
   * synchronized on the DbEnvPool.
   *
   * <p>In theory we shouldn't need to synchronize here when envImpl.getSharedCache() is false;
   * however, we synchronize unconditionally to standardize the synchronization order and avoid
   * accidental deadlocks.
   */
  synchronized void setMutableConfig(
      EnvironmentImpl envImpl, EnvironmentMutableConfig mutableConfig) throws DatabaseException {
  ...
  /**
   * Called by EnvironmentImpl.close to perform the close operation while synchronized on the
   * DbEnvPool.
   *
   * <p>Synchronization on this DbEnvPool during the close is used to protect its data structures.
   * Unfortunately, this means that a long checkpoint during a close will block other closes and
   * opens. We may want to improve this in the future. However, at least there is a user workaround:
   * perform a full checkpoint before closing the environment.
   */
  synchronized void closeEnvironment(
      EnvironmentImpl envImpl, boolean doCheckpoint, boolean isAbnormalClose) {
  ...
  /**
   * Called by EnvironmentImpl.closeAfterInvalid to perform the close operation while synchronized
   * on the DbEnvPool.
   */
  ...
  /**
   * Removes an EnvironmentImpl from the pool after it has been closed. This method is called while
   * synchronized. Note that the environment was removed from the SharedEvictor by
   * EnvironmentImpl.shutdownEvictor.
   */
  private void removeEnvironment(EnvironmentImpl envImpl) {

    final String environmentKey = getEnvironmentMapKey(envImpl.getEnvironmentHome());
   ...
   *     unchanged. A total greater than zero is given when it has changed via setMutableConfig.
   * @param skipEnv is an environment that should not be reset, or null. Non-null is passed when an
   *     environment has already been reset because it was just created or the target of
   *     setMutableConfig.
   */
  private void resetSharedCache(long newMaxMemory, EnvironmentImpl skipEnv)
    ...
    for (EnvironmentImpl envImpl : sharedCacheEnvs) {
      ...
      if (envImpl != skipEnv && envImpl.isValid()) {
        envImpl.getMemoryBudget().reset(newMaxMemory, false /*newEnv*/, envImpl.getConfigManager());
      }
    }
  }
}
