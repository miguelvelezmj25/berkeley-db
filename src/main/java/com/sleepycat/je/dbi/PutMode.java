...
public enum PutMode {
  ...
  /**
   * User operation: Cursor.putCurrent. Replace data at current position. Return KEYEMPTY if record
   * at current position is deleted.
   */
  CURRENT,
  ...
  /**
   * User operation: Cursor.put. Insert if key (for non-duplicates DBs) or key/data (for duplicates
   * DBs) does not already exist; otherwise, overwrite key and data.
   */
  OVERWRITE,
}
