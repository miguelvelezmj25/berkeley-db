 ...
 *
 * <p>For an explanation of the 'enabled' mode, see RecoveryManager class comments.
 */
public class INList implements Iterable<IN> {
  ...
  /**
   * We use a Map of INs because there is no ConcurrentHashSet, only a ConcurrentHashMap. But this
   * map is treated as a set of INs with the same object as the key and the value.
   */
  private final ConcurrentMap<IN, IN> ins;

  /**
   * Stats about the composition of the INList must be kept in this class rather than the evictor
   * because a sharedEnvCache encompasses many INLists. Note that we are keeping a true StatGroup
   * instance/Stat fields. That's because these values are "instantaneous", and don't need to obey
   * the accumulate/clear semantics of stats. When stats are loaded, we'll just create a new stats
   * group to pass back.
   */
  ...
  public StatGroup loadStats() {
    StatGroup stats =
        new StatGroup(EvictorStatDefinition.GROUP_NAME, EvictorStatDefinition.GROUP_DESC);
  ...
  public void add(IN in) {
    ...
    if (oldValue != null) {
      throw EnvironmentFailureException.unexpectedState(
          envImpl,
          "Failed adding new IN node="
              + in.getNodeId()
              + " dbIdentity="
              + System.identityHashCode(in.getDatabase())
              + " db="
              + in.getDatabase().dumpString(0)
              + "\nExisting IN node="
              + oldValue.getNodeId()
              + " dbIdentity="
              + System.identityHashCode(oldValue.getDatabase())
              + " db="
              + oldValue.getDatabase().dumpString(0));
    }
  ...
  /**
   * Return an iterator over the main 'ins' set. Returned iterator may or may not show elements
   * added or removed after the iterator is created.
   *
  ...
  /**
   * A direct Iterator on the INList may return INs that have been removed, since the underlying
   * ConcurrentHashMap doesn't block changes to the list during the iteration. This Iterator
   * implementation wraps a direct Iterator and returns only those INs that are on the INList.
   *
   * <p>Note that this doesn't guarantee that an IN will not be removed from the INList after being
   * returned by this iterator. But filtering out the INs already removed will avoid wasting effort
   * in the evictor, checkpointer, and other places where INs are iterated and processed.
   */
  ...
  /**
   * We are starting the iteration of the INList. Flip the INList toggle and set the total amount to
   * zero.
   *
   * <p>After calling this method, memRecalcEnd must be called in a finally block. If it is not
   * called, internal state will be invalid.
   */
  ...
  /**
   * An IN was encountered during the iteration through the entire INList. Add its size to the
   * recalc total if we have not already processed it, and mark it as processed. If it was already
   * processed, memRecalcAdd must have been called for the IN when it was added to the INList during
   * the iteration.
   */
  public void memRecalcIterate(IN in) {
    assert recalcInProgress;
    if (recalcConsistent && recalcToggle != in.getRecalcToggle()) {
  ...
  /**
   * An IN is being added to the INList. Add its size to the recalc total and mark it as processed.
   * It cannot have already been processed since it is a new IN.
   */
  private void memRecalcAdd(IN in, long size) {
    if (recalcInProgress && recalcConsistent) {
  ...
  /**
   * An IN is being removed from the INList. Add the delta to the recalc total if it was already
   * processed, and mark it as processed. If we have not yet processed it, it is not included in the
   * total.
   */
  ...
  /**
   * The size of an IN is changing. Add the delta to the recalc total if it have already processed
   * the IN. If we have not yet processed it, its total size will be added by memRecalcIterate.
   */
  public void memRecalcUpdate(IN in, long delta) {
    if (recalcInProgress && recalcConsistent && recalcToggle == in.getRecalcToggle()) {
      recalcTotal.addAndGet(delta);
    }
  }

  /**
   * The reset operation is over. Only update the tree budget if the iteration was completed and the
   * state was consistent prior to this reset operation.
   */
  public void memRecalcEnd(boolean completed) {
    assert recalcInProgress;
    if (completed && recalcConsistent) {
      envImpl.getMemoryBudget().refreshTreeMemoryUsage(recalcTotal.get());
    }
    recalcInProgress = false;
