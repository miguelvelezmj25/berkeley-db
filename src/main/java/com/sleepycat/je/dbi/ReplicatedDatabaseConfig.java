...
/**
 * This class contains all fields of the database configuration which are persistent. This class is
 * logged as part of a nameLN so that databases can be created on replica nodes with the correct
 * configuration.
 */
public class ReplicatedDatabaseConfig extends BasicVersionedWriteLoggable {
  ...
  /**
   * Create a database config for use on the replica which contains all the configuration options
   * that were conveyed by way of this class.
   */
  public DatabaseConfig getReplicaConfig(EnvironmentImpl envImpl) {
    ...
    replicaConfig.setSortedDuplicates(DatabaseImpl.getSortedDuplicates(flags));
     ...
     * Replica has been.
     */
    replicaConfig.setKeyPrefixing(
        DatabaseImpl.getKeyPrefixing(flags) || DatabaseImpl.getSortedDuplicates(flags));
    ...
    DatabaseImpl.ComparatorReader reader =
        new DatabaseImpl.ComparatorReader(btreeComparatorBytes, "Btree", envImpl.getClassLoader());
    if (reader.isClass()) {
    ...
    reader =
        new DatabaseImpl.ComparatorReader(
            duplicateComparatorBytes, "Duplicate", envImpl.getClassLoader());
    if (reader.isClass()) {
    ...
    replicaConfig.setTriggers(
        TriggerUtils.unmarshallTriggers(null, triggerBytes, envImpl.getClassLoader()));
  ...
  public void readFromLog(ByteBuffer itemBuffer, int entryVersion) {
    ...
    maxTreeEntriesPerNode = LogUtils.readInt(itemBuffer, false /*unpacked*/);
    if (entryVersion < 8) {
      /* Discard maxDupTreeEntriesPerNode. */
      LogUtils.readInt(itemBuffer, false /*unpacked*/);
    }
    btreeComparatorBytes = LogUtils.readByteArray(itemBuffer, false /*unpacked*/);
    duplicateComparatorBytes = LogUtils.readByteArray(itemBuffer, false /*unpacked*/);
  ...
  public void dumpLog(StringBuilder sb, boolean verbose) {
    ...
    sb.append(" btcmpSet=\"").append(btreeComparatorBytes != LogUtils.ZERO_LENGTH_BYTE_ARRAY);
    sb.append("\" dupcmpSet=\"")
        .append(duplicateComparatorBytes != LogUtils.ZERO_LENGTH_BYTE_ARRAY)
        .append("\"");
  ...
  public boolean logicalEquals(Loggable other) {
    ...
    if (maxTreeEntriesPerNode != otherConfig.maxTreeEntriesPerNode) {
      return false;
    }

    if (!Arrays.equals(btreeComparatorBytes, otherConfig.btreeComparatorBytes)) {
      return false;
    }

      return Arrays.equals(duplicateComparatorBytes, otherConfig.duplicateComparatorBytes);
  }
}
