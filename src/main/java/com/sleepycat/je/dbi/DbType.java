...
/**
 * Classifies all databases as specific internal databases or user databases. This can be thought of
 * as a substitute for having DatabaseImpl subclasses for different types of databases. It also
 * identifies each internal database by name.
 */
public enum DbType {
  ID("_jeIdMap") {
    @Override
   ...
   * CursorImpl.putCurrentAlreadyLatchedAndLocked. Without this method it would be necessary to
   * fetch the existing LN and call LN.modify.
   *
  ...
  /**
   * Creates a deleted LN for use in an optimization in CursorImpl.delete. Without this method it
   * would be necessary to fetch the existing LN and call LN.delete.
   *
   * @throws EnvironmentFailureException if this is not allowed.
   */
  public LN createDeletedLN(EnvironmentImpl envImpl) {
