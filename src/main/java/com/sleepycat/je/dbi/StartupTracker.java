...
/**
 * Store and calculate elapsed time, counts, and other statistics about environment open. No
 * synchronization is used, which generally works because with the exception of replication,
 * environment startup is currently a serial, single threaded event. Replicated environments must be
 * sure to record startup times only at thread safe points.
 */
public class StartupTracker {
  ...
  public void stop(Phase phase) {
     ...
     */
    int dumpThreshold =
        envImpl.getConfigManager().getDuration(EnvironmentParams.STARTUP_DUMP_THRESHOLD);
  ...
  public void setProgress(RecoveryProgress progress) {
    ProgressListener<RecoveryProgress> progressListener = envImpl.getRecoveryProgressListener();
  ...
  /**
   * Generate a description of the four recovery locations (firstActive, ckptStart, ckptend, end of
   * Log) and the distance inbetween.
   */
  private String displayRecoveryInterval() {
    ...
    long fileMax = envImpl.getConfigManager().getLong(EnvironmentParams.LOG_FILE_MAX);

    long useStart = info.checkpointStartLsn == DbLsn.NULL_LSN ? 0 : info.checkpointStartLsn;
    long head = DbLsn.getNoCleaningDistance(useStart, info.firstActiveLsn, fileMax);

    long useEnd = info.checkpointEndLsn == DbLsn.NULL_LSN ? 0 : info.checkpointEndLsn;
    long ckpt = DbLsn.getNoCleaningDistance(useEnd, info.checkpointStartLsn, fileMax);

    long useLast = info.lastUsedLsn == DbLsn.NULL_LSN ? 0 : info.lastUsedLsn;
    long tail = DbLsn.getNoCleaningDistance(useLast, info.checkpointEndLsn, fileMax);
    returnInfo.append(
        "firstActive["
    ...
    f.format(
        "%24s bytes = %,d\n%24s bytes = %,d\n%24s bytes = %,d",
        "firstActive->ckptStart", head, "ckptStart->ckptEnd", ckpt, "ckptEnd->end bytes", tail);
  ...
  /** Display a phase and its children, showing elapsed time as a percentage of the phases' root. */
  private void displayPhaseSubtree(
      PrintStream stream, Phase parent, Elapsed parentTime, Elapsed rootElapsed) {
    ...
    stream.printf(parentFormat, parent, parentTime.getPercentage(rootElapsed), parentTime);
    ...
    for (Phase child : parent.children) {
      ...
      stream.printf(dataFormat, child, time.getPercentage(rootElapsed), time);
    }
  }
  ...
  private static class Elapsed {
    ...
    public String toString() {
      ...
      if (end != 0) {
        ...
        if (start != 0) {
          f.format("%13s  %tD,%tH:%tM:%tS:%tL", "started at", start, start, start, start, start);
        } else {
          f.format("%13s", "none");
        }
      }
  ...
  public static class Counter {
    ...
    /**
     * Keep track of auxiliary log entries processed during this pass. For example, LNs are the main
     * target of the undoLN pass, but we also read aborts and commits.
     */
    ...
    public String toString() {
      ...
      f.format(
          ...
          numDeleted,
          numAux,
          numRepeatIteratorReads,
          (endCacheMiss - startCacheMiss));
      return sb.toString();
    }
