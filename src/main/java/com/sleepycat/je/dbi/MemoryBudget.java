...
/**
 * MemoryBudget calculates the available memory for JE and how to apportion it between cache and log
 * buffers. It is meant to centralize all memory calculations. Objects that ask for memory budgets
 * should get settings from this class, rather than using the configuration parameter values
 * directly.
 */
public class MemoryBudget implements EnvConfigObserver {
  ...
  // 12
  private static final int BIN_FIXED_OVERHEAD_32 = 223;
  private static final int BIN_FIXED_OVERHEAD_64 = 352;
  private static final int BIN_FIXED_OVERHEAD_OOPS = 232;
  ...
  // 52
  private static final int DEFAULT_LONG_REP_OVERHEAD_32 = 16;
  private static final int DEFAULT_LONG_REP_OVERHEAD_64 = 32;
  private static final int DEFAULT_LONG_REP_OVERHEAD_OOPS = 24;

  // 59
  private static final int SPARSE_LONG_REP_OVERHEAD_32 = 24;
  private static final int SPARSE_LONG_REP_OVERHEAD_64 = 40;
  private static final int SPARSE_LONG_REP_OVERHEAD_OOPS = 24;
  ...
  // 13
  private static final int IN_FIXED_OVERHEAD_32 = 248;
  private static final int IN_FIXED_OVERHEAD_64 = 392;
  private static final int IN_FIXED_OVERHEAD_OOPS = 256;
  ...
  // 37
  private static final int WRITE_LOCKINFO_OVERHEAD_32 = 48;
  private static final int WRITE_LOCKINFO_OVERHEAD_64 = 72;
  private static final int WRITE_LOCKINFO_OVERHEAD_OOPS = 56;
  ...
  private static final int TXN_OVERHEAD_64 = 361;
  private static final int TXN_OVERHEAD_OOPS = 240;

  // 26
  private static final int CHECKPOINT_REFERENCE_SIZE_32 = 40 + HASHSET_ENTRY_OVERHEAD_32;
  private static final int CHECKPOINT_REFERENCE_SIZE_64 = 56 + HASHSET_ENTRY_OVERHEAD_64;
  private static final int CHECKPOINT_REFERENCE_SIZE_OOPS = 48 + HASHSET_ENTRY_OVERHEAD_OOPS;
  ...
  private static final int UTILIZATION_PROFILE_ENTRY_64 = 160;
  ...
  // 33
  private static final int LN_INFO_OVERHEAD_32 = 32;
  private static final int LN_INFO_OVERHEAD_64 = 48;
  private static final int LN_INFO_OVERHEAD_OOPS = 30;
  ...
  // 55
  private static final int DOS_WEAK_BINREF_OVERHEAD_32 = 48;
  private static final int DOS_WEAK_BINREF_OVERHEAD_64 = 72;
  private static final int DOS_WEAK_BINREF_OVERHEAD_OOPS = 48;

  // 56
  private static final int DOS_OFFHEAP_BINREF_OVERHEAD_32 = 32;
  private static final int DOS_OFFHEAP_BINREF_OVERHEAD_64 = 40;
  private static final int DOS_OFFHEAP_BINREF_OVERHEAD_OOPS = 40;

  // 57
  private static final int DOS_DEFERRED_LSN_BATCH_OVERHEAD_32 = 88;
  private static final int DOS_DEFERRED_LSN_BATCH_OVERHEAD_64 = 128;
  private static final int DOS_DEFERRED_LSN_BATCH_OVERHEAD_OOPS = 88;

  // 58
  private static final int DOS_DEFERRED_DELTAREF_OVERHEAD_32 = 16;
  private static final int DOS_DEFERRED_DELTAREF_OVERHEAD_64 = 24;
  private static final int DOS_DEFERRED_DELTAREF_OVERHEAD_OOPS = 16;

  // 27 minus zero length Object array
  private static final int EMPTY_OBJ_ARRAY = objectArraySize(0);
  ...
  static {
    ...
    final boolean useCompressedOops =
        COMPRESSED_OOPS_KNOWN ? COMPRESSED_OOPS_KNOWN_ON : COMPRESSED_OOPS_REQUESTED;
    ...
    if (useCompressedOops) {
      ...
      SPARSE_TARGET_ENTRY_OVERHEAD = SPARSE_TARGET_ENTRY_OVERHEAD_OOPS;
      DEFAULT_TARGET_ENTRY_OVERHEAD = DEFAULT_TARGET_ENTRY_OVERHEAD_OOPS;
      ...
      DOS_DEFERRED_LSN_BATCH_OVERHEAD = DOS_DEFERRED_LSN_BATCH_OVERHEAD_OOPS;
      ...
      DEFAULT_TARGET_ENTRY_OVERHEAD = DEFAULT_TARGET_ENTRY_OVERHEAD_64;
      DEFAULT_KEYVALS_OVERHEAD = DEFAULT_KEYVALS_OVERHEAD_64;
      MAX_KEY_SIZE_KEYVALS_OVERHEAD = MAX_KEY_SIZE_KEYVALS_OVERHEAD_64;
      ...
      DOS_DEFERRED_LSN_BATCH_OVERHEAD = DOS_DEFERRED_LSN_BATCH_OVERHEAD_64;
      ...
      DEFAULT_TARGET_ENTRY_OVERHEAD = DEFAULT_TARGET_ENTRY_OVERHEAD_32;
      DEFAULT_KEYVALS_OVERHEAD = DEFAULT_KEYVALS_OVERHEAD_32;
      MAX_KEY_SIZE_KEYVALS_OVERHEAD = MAX_KEY_SIZE_KEYVALS_OVERHEAD_32;
      ...
      DOS_DEFERRED_LSN_BATCH_OVERHEAD = DOS_DEFERRED_LSN_BATCH_OVERHEAD_32;
      DOS_DEFERRED_DELTAREF_OVERHEAD = DOS_DEFERRED_DELTAREF_OVERHEAD_32;
    }
  }

  /* public for unit tests. */
  public static final long MIN_MAX_MEMORY_SIZE = 96 * 1024;
  public static final String MIN_MAX_MEMORY_SIZE_STRING = Long.toString(MIN_MAX_MEMORY_SIZE);

  /* This value prevents cache churn for apps with a high write rate. */
  @SuppressWarnings("unused")
  private static final int DEFAULT_MIN_BTREE_CACHE_SIZE = 500 * 1024;

  private static final long N_64MB = (1 << 26);
  ...
  MemoryBudget(
      EnvironmentImpl envImpl, EnvironmentImpl sharedCacheEnv, DbConfigManager configManager)
  ...
  /**
   * Logs a SEVERE message if compressed oops was specified but did not take effect. Must be called
   * after the environment is initialized so the message makes it to the output file.
   */
   ...
   *
   * @param newMaxMemory is the new total cache budget or is less than 0 if the total should remain
   *     unchanged.
   * @param newEnv is true if this is the first time we are resetting the budget for a new
   *     environment. Note that a new environment has not yet been added to the set of shared cache
   *     environments.
   */
  void reset(long newMaxMemory, boolean newEnv, DbConfigManager configManager)
      throws DatabaseException {
  ...
  /**
   * Initialize the starting environment memory state. We really only need to recalibrate the tree
   * and treeAdmin categories, since there are no locks and txns yet, and the items in the admin
   * category are cleaner items and aren't affected by the recovery splicing process.
   */
  ...
  /**
   * Returns whether eviction of INList information is allowed. To prevent extreme cache churn,
   * eviction of Btree information is prohibited unless the tree memory usage is above this minimum
   * value.
   */
  ...
  public void updateTreeAdminMemoryUsage(long increment) {
    updateCounter(increment, treeAdminMemoryUsage, "treeAdmin", DEBUG_TREEADMIN);
  }
  ...
  private void updateCounter(long increment, AtomicLong counter, String debugName, boolean debug) {
    if (increment != 0) {
      ...
      assert (sizeNotNegative(newSize)) : makeErrorMessage(debugName, newSize, increment);
  ...
  public void updateLockMemoryUsage(long increment, int lockTableIndex) {
    if (increment != 0) {
      ...
      assert lockMemoryUsage.get() >= 0
          : makeErrorMessage("lockMem", lockMemoryUsage.get(), increment);
      if (DEBUG_LOCK) {
        if (increment > 0) {
          System.err.println(
              "INC-------- ="
          ...
          System.err.println(
              "-------DEC=" + increment + " lock[" + lockTableIndex + "] " + lockMemoryUsage.get());
        }
      }
  ...
  public long getLockMemoryUsage() {
    long accLockMemoryUsage = txnMemoryUsage.get() + lockMemoryUsage.get();
  ...
  public static int tupleOutputSize(TupleOutput o) {
    return TUPLE_OUTPUT_OVERHEAD + byteArraySize(o.getBufferBytes().length);
  }

  /**
   * Returns the memory size occupied by a byte array of a given length. All arrays (regardless of
   * element type) have the same overhead for a zero length array. On 32b Java, there are 4 bytes
   * included in that fixed overhead that can be used for the first N elements -- however many fit
   * in 4 bytes. On 64b Java, there is no extra space included. In all cases, space is allocated in
   * 8 byte chunks.
   */
  ...
  StatGroup loadStats() {
    ...
    new LongStat(
        stats, MB_SHARED_CACHE_TOTAL_BYTES, totals.isSharedCache() ? totals.getCacheUsage() : 0);
    new LongStat(stats, MB_TOTAL_BYTES, getLocalCacheUsage());
    new LongStat(stats, MB_DATA_BYTES, treeMemoryUsage.get() + treeAdminMemoryUsage.get());
  ...
  /**
   * Common base class for shared and private totals. This abstraction allows most other classes to
   * be unaware of whether we're using a SharedEvictor or PrivateEvictor.
   */
