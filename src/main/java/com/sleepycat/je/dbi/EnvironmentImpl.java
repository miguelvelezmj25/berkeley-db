...
/**
 * Underlying Environment implementation. There is a single instance for any database environment
...
public class EnvironmentImpl implements EnvConfigObserver {
  ...
  /** The recovery progress listener for this environment, if any has been specified. */
  ...
  /**
   * DbPrintLog doesn't need btree and dup comparators to function properly don't require any
   * instantiations. This flag, if true, indicates that we've been called from DbPrintLog or a
   * similar utility.
   */
   ...
   * invalidated.
   */
  private final AtomicReference<EnvironmentFailureException> invalidatingEFE =
      new AtomicReference<>();
  ...
  /**
   * To support platforms that do not have any javax.management classes, we load JEMonitor
   * dynamically to ensure that there are no explicit references to com.sleepycat.je.jmx.*.
   */
  public interface MBeanRegistrar {
    void doRegister(Environment env) throws Exception;
  ...
  private final LongStat relatchesRequired;
  ...
  public EnvironmentImpl(File envHome, EnvironmentConfig envConfig, EnvironmentImpl sharedCacheEnv)
   ...
   * @param envConfig is the configuration to be used. It's already had the je.properties file
   *     applied, and has been validated.
   * @param sharedCacheEnv if non-null, is another environment that is sharing the cache with this
   *     environment; if null, this environment is not sharing the cache or is the first environment
   *     to share the cache.
  ...
  protected EnvironmentImpl(
    ...
    try {
      ...
      mapTreeRootLatch =
          LatchFactory.createExclusiveLatch(this, "MapTreeRoot", false /*collectStats*/);

      /* Do the stats definition. */
      envStats = new StatGroup(ENV_GROUP_NAME, ENV_GROUP_DESC);

      relatchesRequired = new LongStat(envStats, ENV_RELATCHES_REQUIRED);

      creationTime = System.currentTimeMillis();

      binDeltaGets = new AtomicLongStat(envStats, ENV_BIN_DELTA_GETS);
      binDeltaInserts = new AtomicLongStat(envStats, ENV_BIN_DELTA_INSERTS);
      binDeltaUpdates = new AtomicLongStat(envStats, ENV_BIN_DELTA_UPDATES);
      binDeltaDeletes = new AtomicLongStat(envStats, ENV_BIN_DELTA_DELETES);

      thrputStats = new StatGroup(THROUGHPUT_GROUP_NAME, THROUGHPUT_GROUP_DESC);

      priSearchOps = new AtomicLongStat(thrputStats, THROUGHPUT_PRI_SEARCH);
      priSearchFailOps = new AtomicLongStat(thrputStats, THROUGHPUT_PRI_SEARCH_FAIL);
      secSearchOps = new AtomicLongStat(thrputStats, THROUGHPUT_SEC_SEARCH);
      secSearchFailOps = new AtomicLongStat(thrputStats, THROUGHPUT_SEC_SEARCH_FAIL);
      priPositionOps = new AtomicLongStat(thrputStats, THROUGHPUT_PRI_POSITION);
      secPositionOps = new AtomicLongStat(thrputStats, THROUGHPUT_SEC_POSITION);
      priInsertOps = new AtomicLongStat(thrputStats, THROUGHPUT_PRI_INSERT);
      priInsertFailOps = new AtomicLongStat(thrputStats, THROUGHPUT_PRI_INSERT_FAIL);
      secInsertOps = new AtomicLongStat(thrputStats, THROUGHPUT_SEC_INSERT);
      priUpdateOps = new AtomicLongStat(thrputStats, THROUGHPUT_PRI_UPDATE);
      secUpdateOps = new AtomicLongStat(thrputStats, THROUGHPUT_SEC_UPDATE);
      priDeleteOps = new AtomicLongStat(thrputStats, THROUGHPUT_PRI_DELETE);
      priDeleteFailOps = new AtomicLongStat(thrputStats, THROUGHPUT_PRI_DELETE_FAIL);
      secDeleteOps = new AtomicLongStat(thrputStats, THROUGHPUT_SEC_DELETE);
      ...
      checkpointer =
          new Checkpointer(
              this, Checkpointer.getWakeupPeriod(configManager), Environment.CHECKPOINTER_NAME);

      inCompressor =
          new INCompressor(
              this,
              configManager.getDuration(EnvironmentParams.COMPRESSOR_WAKEUP_INTERVAL),
      ...
      statCapture =
          new StatCapture(
              ...
              configManager.getDuration(EnvironmentParams.STATS_COLLECT_INTERVAL),
              envConfig.getCustomStats(),
      ...
      secondaryAssociationLock = new ReentrantReadWriteLock(false /*fair*/);
  ...
  /**
   * Create a config manager that holds the configuration properties that have been passed in. These
   * properties are already validated, and have had the proper order of precedence applied; that is,
   * the je.properties file has been applied. The configuration properties need to be available
   * before the rest of environment creation proceeds.
   *
   * <p>This method is overridden by replication environments.
   *
   * @param envConfig is the environment configuration to use
   * @param repParams are the replication configurations to use. In this case, the Properties bag
   *     has been extracted from the configuration instance, to avoid crossing the compilation
   *     firewall.
   */
  protected DbConfigManager initConfigManager(
      EnvironmentConfig envConfig, RepConfigProxy repParams) {
   ...
   *
   * <p>This method is overridden by RepImpl to get init params also. This allows certain rep params
   * to be accessed from the EnvironmentImpl constructor using methods such as getPreserveVLSN. The
   * overridden method calls this method first.
   *
  ...
  protected void initConfigParams(EnvironmentConfig envConfig, RepConfigProxy repConfigProxy) {
    ...
    useOffHeapChecksums = configManager.getBoolean(EnvironmentParams.OFFHEAP_CHECKSUM);
    ...
    compactMaxKeyLength = configManager.getInt(EnvironmentParams.TREE_COMPACT_MAX_KEY_LENGTH);

    latchTimeoutMs = configManager.getDuration(EnvironmentParams.ENV_LATCH_TIMEOUT);

    ttlClockTolerance = configManager.getDuration(EnvironmentParams.ENV_TTL_CLOCK_TOLERANCE);

    ttlMaxTxnTime = configManager.getDuration(EnvironmentParams.ENV_TTL_MAX_TXN_TIME);

    ttlLnPurgeDelay = configManager.getDuration(EnvironmentParams.ENV_TTL_LN_PURGE_DELAY);

    allowBlindOps = configManager.getBoolean(EnvironmentParams.BIN_DELTA_BLIND_OPS);

    allowBlindPuts = configManager.getBoolean(EnvironmentParams.BIN_DELTA_BLIND_PUTS);

    maxEmbeddedLN = configManager.getInt(EnvironmentParams.TREE_MAX_EMBEDDED_LN);

    deadlockDetection = configManager.getBoolean(EnvironmentParams.LOCK_DEADLOCK_DETECT);

    deadlockDetectionDelay =
        configManager.getDuration(EnvironmentParams.LOCK_DEADLOCK_DETECT_DELAY);
   ...
   *
   * <p>Note that this method should be called even when opening additional handles for an already
   * initialized environment. If initialization is still in progress then this method will block
   * until it is finished.
   *
   * @return true if we are opening the first handle for this environment and recovery is run (when
   *     ENV_RECOVERY is configured to true); false if we are opening an additional handle and
   *     recovery is not run.
   */
  public synchronized boolean finishInit(EnvironmentConfig envConfig) throws DatabaseException {
    ...
    try {
       ...
       */
      final boolean doRecovery = configManager.getBoolean(EnvironmentParams.ENV_RECOVERY);
      if (doRecovery) {
          ...
          try {
            ...
            if (recoverySuccess) {
              throw new EnvironmentFailureException(
                  this, EnvironmentFailureReason.LOG_INTEGRITY, e);
            }
          } catch (Exception e) {
            if (recoverySuccess) {
  ...
  private void doRegisterMBean(String className, Environment env) throws DatabaseException {
    ...
    try {
      ...
      throw new EnvironmentFailureException(
          DbInternal.getNonNullEnvImpl(env), EnvironmentFailureReason.MONITOR_REGISTRATION, e);
    }
  }
  ...
  public void envConfigUpdate(DbConfigManager mgr, EnvironmentMutableConfig newConfig) {
    ...
    expirationEnabled = mgr.getBoolean(EnvironmentParams.ENV_EXPIRATION_ENABLED);

    exposeUserData = mgr.getBoolean(EnvironmentParams.ENV_EXPOSE_USER_DATA);

    if (mgr.getBoolean(EnvironmentParams.STATS_COLLECT)) {
      if (envStatLogger == null && !isReadOnly() && !isMemOnly()) {
        envStatLogger = new EnvStatsLogger(this);
        addConfigObserver(envStatLogger);
  ...
  private void runOrPauseDaemons(DbConfigManager mgr) {
    ...
    statCapture.runOrPause(mgr.getBoolean(EnvironmentParams.STATS_COLLECT));
  ...
  /**
   * Return the incompressor. In general, don't use this directly because it's easy to forget that
   * the incompressor can be null at times (i.e during the shutdown procedure. Instead, wrap the
   * functionality within this class, like lazyCompress.
   */
  ...
  /**
   * Returns the default cache mode for this environment. If the environment has a null cache mode,
   * CacheMode.DEFAULT is returned. Null is never returned.
   */
  ...
  /**
   * If a background read limit has been configured and that limit is exceeded when the cumulative
   * total is incremented by the given number of reads, increment the sleep backlog to cause a sleep
   * to occur. Called by background activities such as the cleaner after performing a file read
   * operation.
   *
  ...
  /**
   * If a background write limit has been configured and that limit is exceeded when the given
   * amount written is added to the cumulative total, increment the sleep backlog to cause a sleep
   * to occur. Called by background activities such as the checkpointer and evictor after performing
   * a file write operation.
   *
   * <p>The number of writes is estimated by dividing the bytes written by the log buffer size.
   * Since the log write buffer is shared by all writers, this is the best approximation possible.
   *
   * @see #sleepAfterBackgroundIO
   */
  ...
  /**
   * If the sleep backlog is non-zero (set by updateBackgroundReads or updateBackgroundWrites),
   * sleep for the configured interval and decrement the backlog.
   *
   * <p>If two threads call this method and the first call causes a sleep, the call by the second
   * thread will block until the first thread's sleep interval is over. When the call by the second
   * thread is unblocked, if another sleep is needed then the second thread will sleep again. In
   * other words, when lots of sleeps are needed, background threads may backup. This is intended to
   * give foreground threads a chance to "catch up" when background threads are doing a lot of IO.
   */
  ...
  /**
   * Logs the map tree root, but only if its current LSN is before the ifBeforeLsn parameter or
   * ifBeforeLsn is NULL_LSN.
   */
  ...
  /**
   * Tells the asynchronous IN compressor thread about a collections of BINReferences with deleted
   * entries.
   */
  ...
  public void resetLoggingLevel(String changedLoggerName, Level level) {
    ...
    while (loggers.hasMoreElements()) {
      ...
      if ("all".equals(changedLoggerName)
          ...
          || loggerName.endsWith(changedLoggerName + LoggerUtils.NO_ENV)
          || loggerName.endsWith(changedLoggerName + LoggerUtils.FIXED_PREFIX)
    ...
    if (!validName) {
      throw new IllegalArgumentException(
          "The logger name parameter: " + changedLoggerName + " is invalid!");
    }
  }
  ...
  private FileHandler initFileHandler() throws DatabaseException {
     ...
     */
    boolean setupLoggers = configManager.getBoolean(EnvironmentParams.ENV_SETUP_LOGGER);
    if ((envHome == null) || (!envHome.isDirectory()) || (isReadOnly && !setupLoggers)) {
  ...
  /**
   * Invalidate the environment. Done when a fatal exception (EnvironmentFailureException) is
   * thrown.
   */
  ...
  /**
   * @return true if environment is fully open (not being constructed and not closed), and has not
   *     been invalidated by an EnvironmentFailureException.
   */
  ...
  /**
   * When a EnvironmentFailureException occurs or the environment is closed, further writing can
   * cause log corruption.
   */
  ...
  /**
   * Decrements the reference count and closes the environment when it reaches zero. A checkpoint is
   * always performed when closing.
   */
  public void close() throws DatabaseException {

    /* Calls doClose while synchronized on DbEnvPool. */
    DbEnvPool.getInstance()
        .closeEnvironment(this, true /*doCheckpoint*/, false /*isAbnormalClose*/);
  }

  /**
   * Decrements the reference count and closes the environment when it reaches zero. A checkpoint
   * when closing is optional.
   */
  public void close(boolean doCheckpoint) throws DatabaseException {

    /* Calls doClose while synchronized on DbEnvPool. */
    DbEnvPool.getInstance().closeEnvironment(this, doCheckpoint, false /*isAbnormalClose*/);
  }
  ...
  public void abnormalClose() throws DatabaseException {
    ...
    if (openCount1 > 1) {
      throw EnvironmentFailureException.unexpectedState(
          this,
          "Abnormal close assumes that the open count on " + "this handle is 1, not " + openCount1);
    }

    /* Calls doClose while synchronized on DbEnvPool. */
    DbEnvPool.getInstance()
        .closeEnvironment(this, false /*doCheckpoint*/, true /*isAbnormalClose*/);
  }
   ...
   * @throws EnvironmentFailureException if leaks or other problems are detected while closing.
   */
  synchronized void doClose(boolean doCheckpoint, boolean isAbnormalClose) {
    ...
    try {
      ...
      if (!isAbnormalClose && (envState != DbEnvState.INVALID)) {
    ...
    if (errorStringWriter.getBuffer().length() > 0 && invalidatingEFE.get() == null) {
  ...
  /**
   * Release any resources from a subclass that need to be released before close is called on
   * regular environment components.
   *
   * @throws DatabaseException
   */
  protected synchronized void setupClose(@SuppressWarnings("unused") PrintWriter errors)
      throws DatabaseException {}
  ...
  /**
   * Called after recovery but before any other initialization. Is overridden by ReplImpl to convert
   * user defined databases to replicated after doing recovery.
   */
  ...
  /** Returns a count of open environment handles, not including the internal handle. */
   ...
   * Because the internal environment handle is not included in the openCount, this method is
   * currently equivalent to getOpenCount.
   *
  ...
  private void checkLeaks() throws DatabaseException {
    ...
    if (!clean && assertionsEnabled) {
      throw EnvironmentFailureException.unexpectedState(
          "Lock, transaction, latch or memory " + "left behind at environment close");
    }
  }
  ...
  public void invokeCheckpoint(CheckpointConfig config, String invokingSource) {
    checkpointer.doCheckpoint(config, invokingSource, false /*invokedFromDaemon*/);
  }
  ...
  public Provisional coordinateWithCheckpoint(
      final DatabaseImpl dbImpl, final int targetLevel, final IN parent) {

    return checkpointer.coordinateEvictionWithCheckpoint(dbImpl, targetLevel, parent);
  }
  ...
  /**
   * Flip the log to a new file, forcing an fsync. Return the LSN of the trace record in the new
   * file.
   */
  public long forceLogFileFlip() throws DatabaseException {

    return logManager.logForceFlip(new TraceLogEntry(new Trace("File Flip")));
  }

  /** Invoke a compress programmatically. Note that only one compress may run at a time. */
  ...
  public int invokeCleaner(boolean cleanMultipleFiles) {
    ...
    if (isReadOnly || isMemOnly) {
      throw new UnsupportedOperationException(
          "Log cleaning not allowed in a read-only or memory-only " + "environment");
    }

    return cleaner.doClean(cleanMultipleFiles, false /*forceCleaning*/);
  }
   ...
   * {@link #getName()} should almost always be used instead for messages, exceptions, etc.
   */
   ...
   *
   * <p>The configuration can change, but changes are made by replacing the config manager object
   * with a enw one. To use a consistent set of properties, call this method once and query the
   * returned manager repeatedly for each property, rather than getting the config manager via this
   * method for each property individually.
   */
  ...
  /**
   * Uses cached disk usage info to determine whether disk space limits are currently violated. This
   * method simply returns a volatile field. The cached information is updated frequently enough to
   * prevent violating the limits by a large amount.
   *
   * @return a non-null message (appropriate for an exception) if a disk limit is currently
   *     violated, else null.
   */
  ...
  /**
   * Uses cached disk usage info to determine whether disk space limits are currently violated. This
   * method simply checks a volatile field. The cached information is updated frequently enough to
   * prevent violating the limits by a large amount.
   *
  ...
  public EnvironmentStats loadStatsInternal(StatsConfig config) throws DatabaseException {
    ...
    synchronized (statSynchronizer) {
      ...
      stats.setMBAndEvictorStats(memoryBudget.loadStats(), evictor.loadStats(config));
      ...
      stats.setThroughputStats(thrputStats.cloneGroup(config.getClear()));
    }
    return stats;
  }
  ...
  /**
   * Closes the internally maintained environment handle. If the close is an abnormal close, it just
   * does cleanup work instead of trying to close the internal environment handle which may result
   * in further errors.
   */
  ...
  /**
   * Get an environment name, for tagging onto logging and debug message. Useful for multiple
   * environments in a JVM, or for HA.
   */
  ...
  /**
   * Returns whether a given expiration time precedes the current system time, i.e., the expiration
   * time has passed.
   */
  public boolean isExpired(final int expiration, final boolean hours) {
    return expirationEnabled && TTL.isExpired(expiration, hours);
  }
  ...
  public boolean isExpired(final long expirationTime) {
    return expirationEnabled && TTL.isExpired(expirationTime);
  }

  /**
   * Returns whether a given expiration time precedes the current system time plus withinMs, i.e.,
   * the expiration time will pass within withinMs, or earlier. If withinMs is negative, this is
   * whether the expiration time passed withinMs ago, or earlier.
   */
  public boolean expiresWithin(final int expiration, final boolean hours, final long withinMs) {
    return expirationEnabled && TTL.expiresWithin(expiration, hours, withinMs);
  }

  /** Same as {@link #expiresWithin(int, boolean, long)} but with a single expirationTime param. */
  public boolean expiresWithin(final long expirationTime, final long withinMs) {
    return expirationEnabled && TTL.expiresWithin(expirationTime, withinMs);
  }
  ...
  /**
   * Wake up the eviction threads when the main cache is full or close to full. We do not wake up
   * the off-heap evictor threads since the off-heap budget is maintained internally by the off-heap
   * evictor.
   */
  void alertEvictor() {
    evictor.alert();
  ...
  /**
   * Performs critical eviction if necessary. Is called before and after each cursor operation. We
   * prefer to have the application thread do as little eviction as possible, to reduce the impact
   * on latency, so critical eviction has an explicit set of criteria for determining when this
   * should run.
   *
   * <p>WARNING: The action performed here should be as inexpensive as possible, since it will
   * impact app operation latency. Unconditional synchronization must not be performed, since that
   * would introduce a new synchronization point for all app threads.
   *
   * <p>An overriding method must call super.criticalEviction.
   *
   * <p>No latches are held or synchronization is in use when this method is called.
   */
  ...
  /**
   * Do eviction if the memory budget is over. Called by JE daemon threads that do not have the same
   * latency concerns as application threads.
   */
  ...
  /**
   * Performs special eviction (eviction other than standard IN eviction) for this environment. This
   * method is called once per eviction batch to give other components an opportunity to perform
   * eviction. For a shared cached, it is called for only one environment (in rotation) per batch.
   *
   * <p>An overriding method must call super.specialEviction and return the sum of the long value it
   * returns and any additional amount of budgeted memory that is evicted.
   *
   * <p>No latches are held when this method is called, but it is called while synchronized on the
   * evictor.
   *
  ...
  /**
   * Returns true if the VLSN is preserved as the record version. Always false in a standalone
   * environment. Overridden by RepImpl.
   */
  ...
  /**
   * Returns true if the VLSN is both preserved and cached. Always false in a standalone
   * environment. Overridden by RepImpl.
   */
  ...
  /**
   * True if ReplicationConfig set allowConvert as true. Standalone environment is prohibited from
   * doing a conversion, return false.
   */
  ...
  /**
   * Computes and assigns VLSNs as needed to this log item for a replicated log record. This method
   * must be invoked under the LWL to ensure that the VLSNs it generates are correctly serialized
   * with respect to their locations in the log.
   *
   * <p>The method must be invoked before any calls are made to determine the log entry size, since
   * some of the underlying values used to determine the size of the entry will only be finalized
   * after this call has completed.
   *
   * <p>This method is only invoked when the log is being written as the master, since the replica
   * merely reuses the VLSN values computed by the master.
   *
   * <p>Since this method is being written under the LWL it must not block.
   *
   * @param entry the log entry, an in/out argument: the entry is modified with an updated DTVLSN
   *     for commit and abort log entries.
  ...
  /**
   * Do any work that must be done before the checkpoint end is written, as as part of the
   * checkpoint process.
   */
  ...
  /** For replicated environments only; only the overridden method should ever be called. */
  public OperationFailureException createDatabasePreemptedException(
  ...
  private static class InternalEnvironment extends Environment {
    ...
    public InternalEnvironment(
        File envHome, EnvironmentConfig configuration, EnvironmentImpl envImpl)
        throws
  ...
  /**
   * Undeclared exception used to throw through SortedLSNTreeWalker code when preload has either
   * filled the user's max byte or time request.
   */
  ...
  public PreloadStats preload(final DatabaseImpl[] dbImpls, final PreloadConfig config)
      throws DatabaseException {
    ...
    try {
      ...
      PreloadProcessor callback =
          new PreloadProcessor(this, maxBytes, useOffHeapCache, targetTime, pstats, config);
      ...
      SortedLSNTreeWalker walker =
          new PreloadLSNTreeWalker(dbImpls, rootLsns, useOffHeapCache, callback, config);
  ...
  private static class PreloadProcessor implements TreeNodeProcessor {
    ...
    public void processLSN(
      ...
      if (progressListener != null) {
        progressCounter += 1;
        if (!progressListener.progress(PreloadConfig.Phases.PRELOAD, progressCounter, -1)) {
      ...
      } else if (childType.equals(LogEntryType.LOG_DUPCOUNTLN_TRANSACTIONAL)
    ...
    @Override
    public void processDirtyDeletedLN(
        @SuppressWarnings("unused") long childLsn,
        @SuppressWarnings("unused") LN ln,
        @SuppressWarnings("unused") byte[] lnKey) {}
    ...
    public void close() {
      ...
      if (progressListener != null) {
        progressListener.progress(PreloadConfig.Phases.PRELOAD, progressCounter, progressCounter);
      }
    }
  }
  ...
  private class PreloadLSNTreeWalker extends SortedLSNTreeWalker {
    ...
    PreloadLSNTreeWalker(
      ...
      super(
          ...
          callback,
          null,
          null); /* savedException, exception predicate */
    ...
    public void walk() throws DatabaseException {
      ...
      try {
        try {
          ...
          throw EnvironmentFailureException.unexpectedException(
              EnvironmentImpl.this, "Couldn't latch all DatabaseImpls during preload", e);
        }
    ...
    IN getRootIN(DatabaseImpl dbImpl, @SuppressWarnings("unused") long rootLsn) {
      return dbImpl.getTree().getRootINRootAlreadyLatched(CacheMode.UNCHANGED, false /*exclusive*/);
    }
  ...
  /**
   * Recovery encountered a RestoreRequired marker file, so recovery is halted and some intervention
   * must be taken.
   *
   * @param restoreRequired getFailureType() is used to indicate the how the environment can be
   *     healed.
   */
