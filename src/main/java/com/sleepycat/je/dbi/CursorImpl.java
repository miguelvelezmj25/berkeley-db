...
public class CursorImpl implements Cloneable {
  ...
  /**
   * Creates a cursor with retainNonTxnLocks=true, isSecondaryCursor=false. These are the standard
   * settings for an internal cursor.
   */
  public CursorImpl(DatabaseImpl database, Locker locker) {
    this(database, locker, true /*retainNonTxnLocks*/, false /*isSecondaryCursor*/);
  }

  /**
   * Creates a cursor.
   *
   * <p>A cursor always retains transactional locks when it is reset or closed. Non-transaction
   * locks may be retained or not, depending on the retainNonTxnLocks parameter value.
   *
   * <p>Normally a user-created non-transactional Cursor releases locks on reset and close, and a
   * ThreadLocker is normally used. However, by passing true for retainNonTxnLocks a ThreadLocker
   * can be made to retain locks; this capability is used by SecondaryCursor.readPrimaryAfterGet.
   *
   * <p>For internal (non-user) cursors, a BasicLocker is often used and locks are retained. In
   * these internal use cases the caller explicitly calls BasicLocker.operationEnd() after the
   * cursor is closed, and retainNonTxnLocks is set to true to prevent the locks acquired by the
   * BasicLocker from being released when the cursor is closed.
   *
   * <p>BasicLocker is also used for NameLN operations while opening a Database handle. Database
   * handle locks must be retained, even if the Database is opened non-transactionally.
   *
   * @param retainNonTxnLocks is true if non-transactional locks should be retained (not released
   *     automatically) when the cursor is reset or closed.
   * @param isSecondaryCursor whether to treat this cursor as a secondary cursor, e.g., secondary
   *     records don't have record versions.
   */
  public CursorImpl(
      DatabaseImpl dbImpl, Locker locker, boolean retainNonTxnLocks, boolean isSecondaryCursor) {
   ...
   *
   * @param samePosition If true, this cursor's position is used for the new cursor, and addCursor
   *     is called on the new cursor to register it with the current BIN. If false, the new cursor
   *     will be uninitialized.
   */
  public CursorImpl cloneCursor(final boolean samePosition) {

    assert assertCursorState(false /*mustBeInitialized*/, false /*mustNotBeInitialized*/);
  ...
  /**
   * Called when a cursor has been duplicated prior to being moved. The new locker is informed of
   * the old locker, so that a preempted lock taken by the old locker can be ignored. [#16513]
   *
   * @param closingCursor the old cursor that will be closed if the new cursor is moved
   *     successfully.
   */
  ...
  /**
   * Called when a cursor move operation is complete. Clears the closingLocker so that a reference
   * to the old closed locker is not held.
   */
  ...
  /**
   * Sets the effective cache mode to use for the next operation. The cacheMode field will never be
   * set to null, and can be passed directly to latching methods.
   *
  ...
  public void assertBIN(BIN bin) {
    assert this.bin == bin : "nodeId=" + bin.getNodeId() + " cursor=" + dumpToString(true);
  }
  ...
  /**
   * Change cursor to point to the given BIN/index. If the new BIN is different, then old BIN must
   * be unlatched and the new BIN must be latched.
   */
  ...
  /**
   * Returns the key at the current position, regardless of whether the record is defunct. Does not
   * lock. The key returned is not a copy and may not be returned directly to the user without
   * copying it first.
   *
   * <p>The cursor must be initialized.
   *
   * <p>TODO: The returned byte array is normally, but not always a copied, and then copied again
   * into the user's DatabaseEntry. If this method always returns a copy, the extra copy into
   * DatabaseEntry could be avoided.
   */
  ...
  public long getExpirationTime() {
    latchBIN();
    try {
      return TTL.expirationToSystemTime(bin.getExpiration(index), bin.isExpirationInHours());
    } finally {
      releaseBIN();
    }
  }
  ...
  /**
   * For a non-sticky cursor, this method is called when the cursor is initialized and an advancing
   * operation (next/prev/skip) is about to be performed. The cursor position is not reset as it
   * would be if the operation were a search or an insertion, for example.
   */
  public void beforeNonStickyOp() {
     ...
     * is evicted later.
     */
    if (cacheMode != CacheMode.DEFAULT && cacheMode != CacheMode.KEEP_HOT) {
  ...
  /**
   * For a non-sticky cursor, this method is called after a successful operation. The cursor
   ...
   * Close a cursor.
   *
   * @param newCursor is another cursor that is kept open by the parent Cursor object, or null if no
   *     other cursor is kept open.
   */
  ...
  /**
   * Performs cache mode-based eviction but for the prior BIN only. This is called after a
   * successful operation using a non-sticky cursor. The prior BIN is evicted only if the BIN has
   * changed.
   */
  ...
  /**
   * Disables or enables eviction during cursor operations. For example, a cursor used to implement
   * eviction (e.g., in some UtilizationProfile and most DbTree and VLSNIndex methods) should not
   * itself perform eviction, but eviction should be enabled for user cursors. Eviction is disabled
   * by default.
   */
  ...
  /**
   * When multiple operations are performed, CacheMode-based eviction is performed for a given
   * operation at the end of the next operation, which calls close() or reset() on the CursorImpl of
   * the previous operation. Eviction for the last operation (including when only one operation is
   * performed) also occurs during Cursor.close(), which calls CursorImpl.close().
   *
   * <p>By default, the CacheMode returned by DatabaseImpl.getCacheMode is used, and the defaults
   * specified by the user for the Database or Environment are applied. However, the default mode
   * can be overridden by the user by calling Cursor.setCacheMode, and the mode may be changed prior
   * to each operation, if desired.
   *
   * <p>To implement a per-operation CacheMode, two CacheMode fields are maintained.
   * Cursor.cacheMode is the mode to use for the next operation. CursorImpl.cacheMode is the mode
   * that was used for the previous operation, and that is used for eviction when that CursorImpl is
   * closed or reset.
   *
   * <p>This method must be called with the BIN latched but may release it, namely when the BIN is
   * evicted.
   */
  ...
  /**
   * Performs the LN portion of CacheMode eviction. The BIN is latched on method entry and exit.
   * Must be called only for CacheMode.EVICT_LN, EVICT_BIN, UNCHANGED and MAKE_COLD.
   */
  ...
  /**
   * Performs the BIN portion of CacheMode eviction. The BIN is latched on method entry, but may or
   * may not be latched on exit. Must be called only for CacheMode.EVICT_BIN, UNCHANGED and
   * MAKE_COLD.
   */
  ...
  /**
   * Evict the given BIN. Must already be latched. The latch will be released inside the
   * doCacheModeEvict() call.
   */
  ...
  /**
   * Delete the item pointed to by the cursor. If the item is already defunct, return KEYEMPTY.
   * Returns with nothing latched.
   */
  public OperationResult deleteCurrentRecord(ReplicationContext repContext) {

    assert assertCursorState(true /*mustBeInitialized*/, false /*mustNotBeInitialized*/);
    ...
    try {
      ...
      final LockStanding lockStanding =
          lockLN(LockType.WRITE, true /*allowUncontended*/, false /*noWait*/);
       ...
       */
      bin.deleteRecord(index, oldLNMemSize, logItem.lsn, ln.getVLSNSequence(), logItem.size);
      ...
      if (success && !dbImpl.isInternalDb() && bin != null && bin.isBINDelta()) {
   ...
   * @return OperationResult, or null if an expired LN was purged and a partial 'data' param was
   *     supplied.
   */
  public OperationResult updateCurrentRecord(
    ...
    try {
      /* Get a write lock. */
      lockStanding = lockLN(LockType.WRITE, true /*allowUncontended*/, false /*noWait*/);
   ...
   *
   * <p>The cursor must initially be uninitialized.
   *
   * <p>This method is called directly internally for putting tree map LNs and file summary LNs.
   */
  public boolean insertRecord(
      byte[] key, LN ln, boolean blindInsertion, ReplicationContext repContext) {

    assert assertCursorState(false /*mustBeInitialized*/, true /*mustNotBeInitialized*/);
    ...
    try {
      final Pair<LockStanding, OperationResult> result =
          insertRecordInternal(
              key, ln, null /*expirationInfo*/, blindInsertion, null /*returnNewData*/, repContext);
  ...
  /**
   * Insert or update a given record. The method searches for the record using its key. It will
   * perform an update if the record is found, otherwise an insertion.
   *
   * <p>The cursor must initially be uninitialized.
   *
   * <p>Called by all the Cursor.putXXX() ops, except putCurrent().
   *
   ...
   *     "returnNewData" may be different than "data" only if "data" is partial.
   * @return OperationResult where isUpdate() distinguishes insertions and updates. Is null only if
   *     an expired LN was purged and a partial 'data' param was supplied.
   */
  public OperationResult insertOrUpdateRecord(
    ...
    try {
      ...
      Pair<LockStanding, OperationResult> insertResult =
          insertRecordInternal(
              keyCopy, ln, expInfo, false /*blindInsertion*/, returnNewData, repContext);
  ...
  /**
   * Try to insert the key/data pair as a new record. Will succeed if a non-defunct record does not
   * exist already with the given key.
   *
   * <p>The cursor must initially be uninitialized.
   *
   * <p>On return, this.bin is latched.
   *
   * @return a non-null pair of LockStanding and OperationResult.
   *     <p>+ LockStanding will be non-null if a slot with the given key already exists, whether or
   *     not we reuse the slot for this record (i.e., whether or not the result is non-null). In
   *     other words, we always lock the record in an existing slot for the give key.
   *     <p>+ OperationResult will be non-null if we inserted a slot or reused a slot having a
   *     defunct record, or null if the insertion failed because a non-defunct record exists with
   *     the given key.
   */
  private Pair<LockStanding, OperationResult> insertRecordInternal(
    ...
    if (lockStanding == null) {
      ...
      bin.setExpiration(index, expInfo.expiration, expInfo.expirationInHours);
    ...
    return new Pair<>(
        lockStanding, DbInternal.makeResult(expInfo.expiration, expInfo.expirationInHours));
  }

  /**
   * Update the record where the cursor is currently positioned at. The cursor is registered with
   * this position, the associated bin is latched, the BIN slot is not defunct, and it has been
   * locked in WRITE mode.
   *
   * @param returnOldData if non-null, will be filled in with the pre-existing record's data.
   *     However, if an expired LN was purged, it will not be filled in and the caller should expect
   *     this; see {@link Cursor#putNotify}.
   * @return OperationResult, or null if an expired LN was purged and a partial 'data' param was
   *     supplied.
   */
  private OperationResult updateRecordInternal(
     ...
     */
    if (key != null && Key.compareKeys(currKey, key, dbImpl.getKeyComparator()) != 0) {
    ...
    final boolean shouldCache =
        (dbImpl.isDeferredWriteMode() || (!dbImpl.getSortedDuplicates() && !newEmbeddedLN));
  ...
  /**
   * Position the cursor at the first or last record of the dbImpl. It's okay if this record is
   * defunct.
   *
   * <p>The cursor must initially be uninitialized.
   *
   * <p>Returns with the target BIN latched!
   *
   * @return true if a first or last position is found, false if the tree being searched is empty.
   */
  ...
  /**
   * Position this cursor on the slot whose key is the max key less or equal to the given search
   * key.
   *
   * <p>To be more precise, let K1 be search key. The method positions the cursor on the BIN that
   * should contain K1. If the BIN does contain K1, this.index is set to the containing slot.
   * Otherwise, this.index is set to the right-most slot whose key is < K1, or to -1 if K1 is < than
   * all keys in the BIN.
   *
   * <p>The cursor must initially be uninitialized.
   *
   * <p>The method returns with the BIN latched, unless an exception is raised.
   *
   * <p>The method returns an integer that encodes the search outcome: If the FOUND bit is not set,
   * the tree is completely empty (has no BINs). If the FOUND bit is set, the EXACT_KEY bit says
   * whether K1 was found or not and the FOUND_LAST bit says whether the cursor is positioned to the
   * very last slot of the BTree (note that this state can only be counted on as long as the BIN is
   * latched).
   *
   * <p>Even if the search returns an exact result, the record may be defunct. The caller must
   * therefore check whether the cursor is positioned on a defunct record.
   *
   * <p>This method does not lock the record. The caller is expected to call lockAndGetCurrent to
   * perform locking.
   */
  public int searchRange(DatabaseEntry searchKey, Comparator<byte[]> comparator) {
    ...
    try {
      ...
      bin =
          dbImpl.getTree().search(key, Tree.SearchType.NORMAL, binBoundary, cacheMode, comparator);
      ...
      if (bin != null) {
        ...
        index = bin.findEntry(key, true /*indicateIfExact*/, false /*exact*/, comparator);

        if (bin.isBINDelta()
            && (index < 0 || (index & IN.EXACT_MATCH) == 0 || binBoundary.isLastBin)) {
        ...
        if (index >= 0) {
          ...
          foundLast = (binBoundary.isLastBin && index == bin.getNEntries() - 1);
        }
  ...
  /**
   * Position this cursor on the slot (if any) whose key matches the given search key. If no such
   * slot is found or the slot does not hold a "valid" record, return null. Otherwise, lock the
   * found record with the specified lock type (which may be NONE) and return the LockStanding obj
   * that was created by the locking op. Whether the slot contains a "valid" record or not depends
   * on the slot's KD/PD flags and the lockType and dirtyReadAll parameters. Four cases are
   * considered; they are described in the lockLNAndCheckDefunct() method.
   *
   * <p>The cursor must initially be uninitialized.
   *
   * <p>The method returns with the BIN latched, unless an exception is raised.
   *
   * <p>In all cases, the method registers the cursor with the BIN that contains or should contain
   * the search key.
   *
   * @return the LockStanding for the found record, or null if no record was found.
   */
  public LockStanding searchExact(
    ...
    try {
      ...
      if (bin != null) {
        ...
        if (index >= 0) {
          lockStanding = lockLNAndCheckDefunct(lockType, dirtyReadAll, dataRequested);
        }
      }
  ...
  /**
   * Lock and copy current record into the key and data DatabaseEntry. When calling this method,
   * this.bin should not be latched already. On return, this.bin is unlatched.
   */
  public OperationResult lockAndGetCurrent(
      DatabaseEntry foundKey, DatabaseEntry foundData, final LockType lockType) {

    return lockAndGetCurrent(foundKey, foundData, lockType, false, false, true);
  }

  /**
   * Let S be the slot where this cursor is currently positioned on. If S does not hold a "valid"
   * record, return null. Otherwise, lock the record in S with the specified lock type(which may be
   * NONE), copy its key and data into the key and data DatabaseEntries, and return SUCCESS. Whether
   * the slot contains a "valid" record or not depends on the slot's KD/PD flags, the lockType and
   * dirtyReadAll parameters, and whether the record has expired. For details see {@link
   * #lockLNAndCheckDefunct}.
   *
   * <p>On entry, the isLatched param says whether this.bin is latched or not. On return, this.bin
   * is unlatched if the unlatch param is true or an exception is thrown.
   *
   * @return OperationResult, or null if the LN has been cleaned and cannot be fetched.
   */
  public OperationResult lockAndGetCurrent(
    ...
    try {
       ...
       */
      if (index >= 0 && index < bin.getNEntries() && bin.isDeleted(index)) {
       ...
       */
      if (index < 0 || index >= bin.getNEntries() || bin.isEntryKnownDeleted(index)) {
      ...
      final boolean dataRequested =
          (foundData != null && (!foundData.getPartial() || foundData.getPartialLength() != 0));

      if (lockLNAndCheckDefunct(lockType, dirtyReadAll, dataRequested) == null) {
        if (treeStatsAccumulator != null) {
          treeStatsAccumulator.incrementDeletedLNCount();
        }
        success = true;
  ...
  /**
   * Let S be the slot where this cursor is currently positioned on. The method locks S (i.e. its
   * LSN), and depending on S's KD/PD flags and expired status, it returns either null or the
   * LockStanding obj that was created by the locking op. The following 4 cases are considered. By
   * "defunct" below we mean S is KD/PD or expired.
   *
   * <p>1. If S is not defunct, return the LockStanding obj. In this case, we know that S holds a
   * valid (non-defunct) record.
   *
   * <p>2. If S is defunct, and the lock type is not NONE, return null. In this case, we know that
   * the record that used to be in S is definitely defunct.
   *
   * <p>3. If S is defunct, the lock kind is NONE, and dirtyReadAll is false, return null. This case
   * corresponds to the READ_UNCOMMITTED LockMode. The record in S is defunct, but the deleting txn
   * may be active still, and if it aborts later, the record will be restored. To avoid a
   * potentially blocking lock, in READ_UNCOMMITTED mode we consider the record to be non-existing
   * and return null.
   *
   * <p>4. If S is defunct, the lock kind is NONE, and dirtyReadAll is true, lock the record in READ
   * mode. This case corresponds to the READ_UNCOMMITTED_ALL LockMode, which requires that we do not
   * skip "provisionally defunct" records. There are two sub-cases:
   *
   * <p>4a. If dataRequested is true, we wait until the deleting txn finishes. In this case the READ
   * lock is blocking. If after the lock is granted S is still defunct, release the lock and return
   * null. Otherwise, release the lock and return the LockStanding obj.
   *
   * <p>4b. If dataRequested is false, then we check whether the deleting txn is still open by
   * requested a non-blocking READ lock. If the lock is granted then the writing txn is closed or
   * this cursor's locker is the writer, and we proceed as if the READ lock was granted in 4a. If
   * the lock is denied then the deleting txn is still open, and we return the LockStanding obj so
   * that the record is not skipped.
   *
   * <p>The BIN must be latched on entry and is latched on exit.
   *
   * @param dirtyReadAll is true if using LockMode.READ_UNCOMMITTED_ALL.
   * @param dataRequested is true if the read operation should return the record data, meaning that
   *     a blocking lock must be used for dirtyReadAll. Is ignored if dirtyReadAll is false. Is
   *     always false for a dup DB, since data is never requested for dup DB ops at the CursorImpl
   *     level.
   */
  private LockStanding lockLNAndCheckDefunct(
      final LockType lockType, final boolean dirtyReadAll, final boolean dataRequested) {
     ...
     */
    standing = lockLN(LockType.READ, false /*allowUncontended*/, !dataRequested /*noWait*/);
   ...
   * @return OperationResult, or null if the LN has been cleaned and cannot be fetched.
   */
  public OperationResult getCurrent(final DatabaseEntry foundKey, final DatabaseEntry foundData) {
    ...
    final long vlsn =
        (ln != null ? ln.getVLSNSequence() : bin.getVLSN(index, false /*allowFetch*/, cacheMode));

    setCurrentVersion(vlsn, bin.getLsn(index));
    setStorageSize();

    return DbInternal.makeResult(bin.getExpiration(index), bin.isExpirationInHours());
  }
  ...
  /**
   * Returns the VLSN and LSN for the record at the current position. Must be called when the cursor
   * is positioned on a record.
   *
   * <p>If this method is called on a secondary cursor, the version of the associated primary record
   * is returned. In that case, the allowFetch parameter is ignored, and the version is available
   * only if the primary record was retrieved (see setPriInfo).
   *
   * @param allowFetch is true to fetch the LN to get the VLSN, or false to return -1 for the VLSN
   *     if both the LN and VLSN are not cached.
   * @throws IllegalStateException if the cursor is closed or uninitialized, or this is a secondary
   *     cursor and the version is not cached.
   */
  public RecordVersion getCurrentVersion(boolean allowFetch) {

    /* Ensure cursor is open and initialized. */
    checkCursorState(true /*mustBeInitialized*/, false /*mustNotBeInitialized*/);
     ...
     */
    if (currentRecordVersion != null) {
      if ((currentRecordVersion.getVLSN() != VLSN.NULL_VLSN_SEQUENCE)
          || !allowFetch
    ...
    latchBIN();
    try {
      setCurrentVersion(bin.getVLSN(index, allowFetch, cacheMode), bin.getLsn(index));
    } finally {
      releaseBIN();
    }
    return currentRecordVersion;
  ...
  /**
   * Returns the estimated disk storage size for the record at the current position. The size
   * includes an estimation of the JE overhead for the record, in addition to the user key/data
   * sizes. But it does not include obsolete overhead related to the record, i.e., space that could
   * potentially be reclaimed by the cleaner.
   *
   * <p>This method does not fetch the LN. Must be called when the cursor is positioned on a record.
   *
   * <p>When called on a secondary cursor that was used to return the primary data, the size of the
   * primary record is returned by this method. Otherwise the size of the record at this cursor
   * position is returned.
   *
   * @return the estimated storage size, or zero when the size is unknown because a non-embedded LN
   *     is not resident and the LN was logged with a JE version prior to 6.0.
  ...
  /**
   * When the primary record is read during a secondary operation, this method is called to copy the
   * primary version and storage size here. This allows the secondary cursor API to return the
   * version and size of the primary record. Note that a secondary record does not have a version of
   * its own.
   *
   * @param sourceCursor contains the primary info, but may be a primary or secondary cursor.
   */
  ...
  /**
   * Returns the number of secondary records written by the last put/delete operation at the current
   * cursor position.
   *
   * <p>NOTE: this method does not work (returns 0) if primary deletions are performed via a
   * secondary (SecondaryDatabase/SecondaryCursor.delete).
   *
   * @return number of writes, or zero if a put/delete operation was not performed.
   */
  ...
  /**
   * Advance a cursor. Used so that verify can advance a cursor even in the face of an exception
   * [12932].
   *
  ...
  /**
   * Move the cursor forward and return the next "valid" record. Whether a slot contains a "valid"
   * record or not depends on the slot's KD/PD flags and the lockType and dirtyReadAll parameters.
   * Four cases are considered; they are described in the lockLNAndCheckDefunct() method.
   *
   * <p>This will cross BIN boundaries. On return, no latches are held. If no exceptions, the cursor
   * is registered with its new location.
   *
   ...
   * @param isLatched if true, the bin that we're on is already latched.
   * @param rangeConstraint if non-null, is called to determine whether a key is out of range.
   */
    ...
    try {
      while (bin != null) {
        ...
        if ((forward && ++index < bin.getNEntries()) || (!forward && --index > -1)) {
          ...
          if (rangeConstraint != null && !rangeConstraint.inBounds(bin.getKey(index))) {
  ...
  /**
   * Used to detect phantoms during "get next" operations with serializable isolation. If this
   * method returns true, the caller should restart the operation from the prior position.
   *
   * <p>Something may have been added to the original cursor (cursorImpl) while we were getting the
   * next BIN. cursorImpl would have been adjusted properly but we would have skipped a BIN in the
   * process. This can happen when all INs are unlatched in Tree.getNextBin. It can also happen
   * without a split, simply due to inserted entries in the previous BIN.
   *
   * @return true if an unaccounted for insertion happened.
   *     <p>TODO: Unfortunately, this method doesn't cover all cases where a phantom may have been
   *     inserted. Another case is described below.
   *     <p>IN-0 ---------------------- | | 50 | 100 | | ---------------------- / / \ \ / \ /----
   *     ----\ IN-1 / \ IN-2 ---------------- ---------------- | 60 | 70 | 80 | | | | |
   *     ---------------- ---------------- / | \ / / | \ / ---------------- ----------------- | 81 |
   *     83 | 85 | | 110 | | | ---------------- ----------------- BIN-3 BIN-4
   *     <p>Initially, the tree looks as above and a cursor (C) is located on the last slot of
   *     BIN-3. For simplicity, assume no duplicates and no serializable isolation. Also assume that
   *     C is a sticky cursor.
   *     <p>1. Thread 1 calls C.getNext(), which calls retrieveNextAllowPhantoms(), which duplicates
   *     C's cursorImpl, and calls dup.getNext().
   *     <p>dup.getNext() latches BIN-3, sets dup.binToBeRemoved to BIN-3 and then calls
   *     Tree.getNextBin(BIN-3).
   *     <p>Tree.getNextBin(BIN-3) does the following: - sets searchKey to 85 - calls
   *     Tree.getParentINForChildIN(BIN-3). - Tree.getParentINForChildIN(BIN-3) unlatches BIN-3 and
   *     searches for BIN-3 parent, thus reaching IN-1. - IN-1.findEntry(85) sets "index" to 2, -
   *     "index" is incremented, - "moreEntriesThisIn" is set to false, - "next" is set to IN-1, . -
   *     Tree.getParentINForChildIN(IN-1) is called and unlatches IN-1.
   *     <p>Assume at this point thread 1 looses the cpu.
   *     <p>2. Thread 2 inserts keys 90 and 95, causing a split of both BIN-3 and IN-1. So the tree
   *     now looks like this:
   *     <p>IN-0 --------------------------- | | 50 | 80 | 100 | | --------------------------- / / |
   *     \ \ / | \ /--------- | ----------\ IN-1 / | \ IN-2 / IN-5 | \ ----------- -----------
   *     ---------------- | 60 | 70 | | 80 | 90 | | | | | ----------- ----------- ---------------- /
   *     | / \ / / | / \ / ---------------- ----------- ----------------- | 81 | 83 | 85 | | 90 | 95
   *     | | 110 | | | ---------------- ----------- ----------------- BIN-3 BIN-6 BIN-4
   *     <p>Notice that C.cursorImpl still points to the last slot of BIN-3.
   *     <p>3. Thread 1 resumes:
   *     <p>- Tree.getParentINForChildIN(IN-1) reaches IN-0. - IN-0.findEntry(85) sets "index" to 2,
   *     - "index" is incremented, - "nextIN" is set to IN-2, which is latched. -
   *     Tree.searchSubTree(IN-2, LEFT) is called, and returns BIN-4. - BIN-4 is the result of
   *     Tree.getNextBin(BIN-3), i.e., BIN-6 was skipped
   *     <p>Now we are back in dup.getNext(): - dup.bin is set to BIN-4, dup.index to -1, and dup is
   *     added to BIN-4 - the while loop repeats, dup.index is set to 0, the 1st slot of BIN-4 is
   *     locked, and dup.getNext() returns SUCCESS.
   *     <p>Now we are back in C.retrieveNextAllowPhantoms(): - C.checkForInsertion() is called -
   *     C.cursorImpl and dup are on different BINs, but the condition: origBIN.getNEntries() - 1 >
   *     origCursor.getIndex() is false, so C.checkForInsertion() returns false.
   *     <p>The end result is that BIN-6 has been missed. This is not be a "bug" for
   *     non-serializable isolation, but the above scenario applies to serializable isolation as
   *     well, and in that case, BIN-6 should really not be missed. This could be solved by
   *     re-implementing Tree.getNext/PrevBIN() do a more "logical" kind of search.
   */
  public boolean checkForInsertion(final GetMode getMode, final CursorImpl dupCursor) {
  ...
  /**
   * Skips over entries until a boundary condition is satisfied, either because maxCount is reached
   * or RangeConstraint.inBounds returns false.
   *
   * <p>If a maxCount is passed, this allows advancing the cursor quickly by N entries. If a
   * rangeConstraint is passed, this allows returning the entry count after advancing until the
   * predicate returns false, e.g., the number of entries in a key range. In either case, the number
   * of entries advanced is returned.
   *
   * <p>Optimized to scan using level two of the tree when possible, to avoid calling
   * getNextBin/getPrevBin for every BIN of the database. All BINs beneath a level two IN can be
   * skipped quickly, with the level two parent IN latched, when all of its children BINs are
   * resident and can be latched without waiting. When a child BIN is not resident or latching
   * waits, we revert to the getNextBin/getPrevBin approach, to avoid keeping the parent IN latched
   * for long time periods.
   *
   * <p>Although this method positions the cursor on the last non-defunct entry seen (before the
   * boundary condition is satisfied), because it does not lock the LN it is possible that it is
   * made defunct by another thread after the BIN is unlatched.
   *
   * @param forward is true to skip forward, false to skip backward.
   * @param maxCount is the maximum number of non-defunct entries to skip, and may be LTE zero if no
   *     maximum is enforced.
   * @param rangeConstraint is a predicate that returns false at a position where advancement should
   *     stop, or null if no predicate is enforced.
  ...
  /**
   * Use this cursor to reference the current BIN in the traversal, to prevent the current BIN from
   * being compressed away. But set the given finalPositionCursor (the 'user' cursor) position only
   * at non-defunct entries, since it should be positioned on a valid entry when this method
   * returns.
   */
  private long skipInternal(
    ...
    try {
      while (true) {
        ...
        count =
            skipEntries(
                forward, maxCount, rangeConstraint, finalPositionCursor, curBin, curIndex, count);
        ...
        final byte[] idKey =
            (curBin.getNEntries() == 0
                ...
                : (forward ? curBin.getKey(curBin.getNEntries() - 1) : curBin.getKey(0)));

        final SearchResult result =
            tree.getParentINForChildIN(
                curBin, false, /*useTargetLevel*/ true, /*doFetch*/ CacheMode.DEFAULT);
  ...
  /**
   * Skip entries in curBin from one past curIndex and onward. Returns non-negative count if
   * skipping should continue, or negative count if bounds is exceeded.
   */
  ...
  /**
   * Returns the stack of ancestor TrackingInfo for the BIN at the cursor, or null if a split occurs
   * and the information returned would be inconsistent.
   *
   * <p>Used by CountEstimator.
   */
  public List<TrackingInfo> getAncestorPath() {
    ...
    final SearchResult result =
        tree.getParentINForChildIN(
            origBin, false, /*useTargetLevel*/ true /*doFetch*/, CacheMode.UNCHANGED, trackingList);
    ...
    final long binLsn;
    try {
      if (origBin != result.parent.getTarget(result.index) || origBin != bin) {
        /* Must have been a split. */
        return null;
      }
    ...
    try {
      ...
      final TrackingInfo info = new TrackingInfo(binLsn, bin.getNodeId(), binEntries, binIndex);
  ...
  /**
   * Search for the next key following the given key, and acquire a range insert lock on it. If
   * there are no more records following the given key, lock the special EOF node for the dbImpl.
   */
      ...
      while (true) {
        ...
        if ((searchResult & FOUND) != 0 && (searchResult & FOUND_LAST) == 0) {
          ...
          if (result != null) {
            ...
            Comparator<byte[]> comparator = dbImpl.getKeyComparator();
  ...
  /**
   * Holds the result of a lockLN operation. A lock may not actually be held (getLockResult may
   * return null) if an uncontended lock is allowed.
   */
  public static class LockStanding {
    ...
    /**
     * Called by update and delete ops, after lockLN() and before logging the LN and updating the
     * BIN. It returns a WriteLockInfo that is meant to be passed to the LN logging method, where
     * its info will be included in the LN log entry and also copied into the new WriteLockInfo that
     * will be created for the new LSN.
     *
     * <p>If the locker is not transactional, or the current LSN has not been write-locked before by
     * this locker, a new WriteLockInfo is created here and its abortLsn and abortKD fields are set.
     * (note: even though lockLN() is called before prepareForUpdate(), it may not actually acquire
     * a lock because of the uncontended optimization).
     *
     * <p>Otherwise, a WriteLockInfo exists already. It may have been created by the lockLN() call
     * during the current updating op, or a lockLN() call during an earlier updating op by the same
     * txn. In the later case, the abortLsn and abortKD have been set already and should not be
     * overwriten here.
     */
    public WriteLockInfo prepareForUpdate(BIN bin, int idx) {
      ...
      if (bin.isEmbeddedLN(idx)) {
        ...
        abortVLSN = bin.getVLSN(idx, false /*allowFetch*/, null /*cacheMode*/);
      ...
      WriteLockInfo wri = (lockResult == null ? null : lockResult.getWriteLockInfo());
      if (wri == null) {
    ...
    /**
     * Creates WriteLockInfo that is appropriate for a newly inserted slot. The return value is
     * meant to be passed to an LN logging method and copied into the WriteLockInfo for the new LSN.
     * This method is static because lockLN is never called prior to logging an LN for a newly
     * inserted slot.
     */
  ...
  /**
   * Locks the LN at the cursor position. Attempts to use a non-blocking lock to avoid
   * unlatching/relatching.
   *
   * <p>Retries if necessary, to handle the case where the LSN is changed while the BIN is
   * unlatched. Because it re-latches the BIN to check the LSN, this serializes access to the LSN
   * for locking, guaranteeing that two lockers cannot obtain conflicting locks on the old and new
   * LSNs.
   *
   * <p>Preconditions: The BIN must be latched.
   *
   * <p>Postconditions: The BIN is latched.
   *
   * <p>LN Locking Rules ---------------- The lock ID for an LN is its LSN in the parent BIN slot.
   * Because the LSN changes when logging the LN, only two methods of locking an LN may be used to
   * support concurrent access:
   *
   * <p>1. This method may be called to lock the old LSN. For read operations, that is all that is
   * necessary. For write operations, the new LSN must be locked after logging it, which is done by
   * all the LN logging methods. Be sure to pass a non-null locker to the LN logging method to lock
   * the LN, unless locking is not desired.
   *
   * <p>2. A non-blocking lock may be obtained on the old LSN (using Locker.nonBlockingLock rather
   * than this method), as long as the lock is released before the BIN latch is released. In this
   * case a null locker is passed to the LN logging method; locking the new LSN is unnecessary
   * because no other thread can access the new LSN until the BIN latch is released.
   *
   * <p>The first method is used for all user operations. The second method is used by the cleaner,
   * when flushing dirty deferred-write LNs, and by certain btree operations.
   *
   * <p>Uncontended Lock Optimization ----------------------------- The allowUncontended param is
   * passed as true for update and delete operations as an optimization for the case where no lock
   * on the old LSN is held by any locker. In this case we don't need to lock the old LSN at all, as
   * long as we log the new LSN before releasing the BIN latch.
   *
   * <p>1. Latch BIN 2. Determine that no lock/waiter exists for oldLsn 3. Log LN and get lsn 4.
   * Lock lsn 5. Update BIN 6. Release BIN latch
   *
   * <p>The oldLsn is never locked, saving operations on the lock table. The assumption is that
   * another locker will first have to latch the BIN to get oldLsn, before requesting a lock.
   *
   * <p>A potential problem is that the other locker may release the BIN latch before requesting the
   * lock.
   *
   * <p>This Operation Another Operation -------------- ----------------- Latch BIN, get oldLsn,
   * release BIN latch Step 1 and 2 Request lock for oldLsn, granted Step 3 and 4
   *
   * <p>Both operations now believe they have an exclusive lock, but they have locks on different
   * LSNs.
   *
   * <p>However, this problem is handled as long as the other lock is performed using a lockLN
   * method in this class, which will release the lock and retry if the LSN changes while acquiring
   * the lock. Because it re-latches the BIN to check the LSN, this will serialize access to the LSN
   * for locking, guaranteeing that two conflicting locks cannot be granted on the old and new LSNs.
   *
   * <p>Deferred-Write Locking ---------------------- When one of the LN optionalLog methods is
   * called, a deferred-write LN is dirtied but not actually logged. In order to lock an LN that has
   * been inserted but not yet assigned a true LSN, a transient LSNs is assigned. These LSNs serve
   * to lock the LN but never appear in the log. See LN.assignTransientLsn.
   *
   * <p>A deferred-write LN is logged when its parent BIN is logged, or when the LN is evicted. This
   * will replace transient LSNs with durable LSNs. If a lock is held by a cursor on a
   * deferred-write LN when it is logged, the same lock is acquired on the new LSN by the cursor.
   * See lockAfterLsnChange.
   *
   * <p>Cleaner Migration Locking ------------------------- The cleaner takes a non-blocking read
   * lock on the old LSN before migrating/logging the LN, while holding the BIN latch. It does not
   * take a lock on the new LSN, since it does not need to retain a lock after releasing the BIN
   * latch.
   *
   * <p>Because a read, not write, lock is taken, other read locks may be held during migration.
   * After logging, the cleaner calls lockAfterLsnChange to lock the new LSN on behalf of other
   * lockers.
   *
   * <p>For more info on migration locking, see HandleLocker.
   *
   * <p>Expired Record Locking ---------------------- To support repeatable-read semantics when a
   * record expires after being locked, we must check whether a record was previously locked before
   * attempting to lock it. If it was previously locked, then it is treated as not expired, even if
   * its expiration time has passed.
   *
   * <p>By was previously "locked" here we mean that any lock type is held, or shared with its
   * owner, by this cursor's locker. Since a read lock will prevent modification of the expiration
   * time, any lock type is adequate. A shared lock is considered adequate to account for the case
   * where multiple lockers are used internally for a single virtual locker, as seen by the user.
   * This is the case when using a read-committed locker or a thread-locker, for example.
   *
   * <p>To avoid unnecessary added overhead, we do not check whether a record was previously locked
   * except when expiration is imminent, which is defined as expiring within {@link
   * EnvironmentParams#ENV_TTL_MAX_TXN_TIME}. The ENV_TTL_MAX_TXN_TIME buffer is used because the
   * expiration time may pass while waiting for a lock.
   *
   * <p>Another case to account for is when the expiration time of the record changes while waiting
   * for the lock. This can happen if the record is updated or an update is aborted. In this case we
   * can assume that the was not previously locked, since that would have prevented the update.
   *
   * <p>Note that when an uncontended lock applies, the expiration of the record with the current
   * LSN cannot change. It is possible that the update or deletion requesting the uncontended lock
   * will be aborted, and the LSN of an expired record will be reinstated in the BIN, but this does
   * not create a special case.
   *
   * <p>Historical Notes ---------------- In JE 4.1 and earlier, each LN had a node ID that was used
   * for locking, rather than using the LSN. The node ID changed only if a deleted slot was reused.
   * The node ID was stored in the LN, requiring that the LN be fetched when locking the LN. With
   * LSN locking a fetch is not needed.
   *
   * <p>When LN node IDs were used, deferred-write LNs were not assigned an LSN until they were
   * actually logged. Deferred-write LNs were initially assigned a null LSN and transient LSNs were
   * not needed.
   *
   * @param lockType the type of lock requested.
   * @param allowUncontended is true to return immediately (no lock is taken) when no locker holds
   *     or waits for the lock.
   * @param noWait is true to perform a no-wait lock request while keeping the BIN latched. The
   *     caller must check the lock result to see whether the lock was granted.
   * @return all information about the lock; see LockStanding.
   * @throws LockConflictException if the lsn is non-null, the lock is contended, and a lock could
   *     not be obtained by blocking.
   */
  public LockStanding lockLN(
      final LockType lockType, final boolean allowUncontended, final boolean noWait)
    ...
    if (envImpl.expiresWithin(
        prevExpiration, prevExpirationInHours, dbImpl.getEnv().getTtlMaxTxnTime())) {
    ...
    if (locker.getDefaultNoWait()) {
      try {
        standing.lockResult = locker.lock(standing.lsn, lockType, true /*noWait*/, dbImpl);
      ...
      standing.lockResult =
          locker.nonBlockingLock(standing.lsn, lockType, false /*jumpAheadOfWaiters*/, dbImpl);
    }
    ...
    if (standing.lockResult.getLockGrant() != LockGrantType.DENIED) {
      ...
      standing.defunct = wasLockedAndExpiresSoon ? bin.isDeleted(index) : bin.isDefunct(index);
    ...
    while (true) {
      ...
      standing.lockResult = locker.lock(standing.lsn, lockType, false /*noWait*/, dbImpl);
  ...
  /**
   * After logging a deferred-write LN during eviction/checkpoint or a migrated LN during cleaning,
   * for every existing lock on the old LSN held by another locker, we must lock the new LSN on
   * behalf of that locker.
   *
   * <p>This is done while holding the BIN latch so that the new LSN does not change during the
   * locking process. The BIN must be latched on entry and is left latched by this method.
   *
   * <p>We release the lock on the oldLsn to prevent locks from accumulating over time on a
   * HandleLocker, as the cleaner migrates LNs, because Database handle locks are legitimately very
   * long-lived. It is important to first acquire all lsn locks and then release the oldLsn locks.
   * Releasing an oldLsn lock might allow another locker to acquire it, and then acquiring another
   * lsn lock may encounter a conflict. [#20617]
   *
   * @see com.sleepycat.je.txn.HandleLocker
   * @see #lockLN
   */
  public static void lockAfterLsnChange(
      DatabaseImpl dbImpl, long oldLsn, long newLsn, Locker excludeLocker) {

    final LockManager lockManager = dbImpl.getEnv().getTxnManager().getLockManager();
    ...
    for (LockInfo lockInfo : owners) {
      final Locker locker = lockInfo.getLocker();
      if (locker != excludeLocker && locker.allowReleaseLockAfterLsnChange()) {
  ...
  /**
   * Calls checkCursorState and asserts false if an exception is thrown. Otherwise returns true, so
   * it can be called under an assertion.
   */
  private boolean assertCursorState(boolean mustBeInitialized, boolean mustNotBeInitialized) {
    ...
    try {
      ...
      assert false : e + " " + dumpToString(true);
   ...
   *
   * @throws IllegalStateException via all Cursor methods that call Cursor.checkState (all get and
   *     put methods, plus more).
   */
  public void checkCursorState(boolean mustBeInitialized, boolean mustNotBeInitialized) {
    ...
    switch (status) {
      ...
      default:
        throw EnvironmentFailureException.unexpectedState("Unknown cursor status: " + status);
    }
  }

  /**
   * Checks that LN deletedness matches KD/PD flag state, at least when the LN is resident. Should
   * only be called under an assertion.
   */
  private boolean verifyPendingDeleted(LockType lockType) {
    ...
    assert ((lnDeleted && (kd || pd)) || (!lnDeleted && !pd))
        : "Deleted state mismatch LNDeleted = " + lnDeleted + " PD = " + pd + " KD = " + kd;
    return true;
  }
  ...
  /**
   * Return this lock to its prior status. If the lock was just obtained, release it. If it was
   * promoted, demote it.
   */
  ...
  public void lockEof(LockType lockType) {
    ...
    locker.lock(dbImpl.getEofLsn(), lockType, false /*noWait*/, dbImpl);
  }
  ...
  /**
   * Enumerates all records in a dbImpl non-transactionally and calls the withCursor method for each
   * record. Stops the enumeration if the callback returns false.
   *
   * @param db DatabaseImpl to traverse.
   * @param lockType non-null LockType for reading records.
   * @param allowEviction should normally be true to evict when performing multiple operations, but
   *     may be false if eviction is disallowed in a particular context.
   * @param withCursor callback object.
   */
  public static void traverseDbWithCursor(
      DatabaseImpl db, LockType lockType, boolean allowEviction, WithCursor withCursor) {
      ...
      if (cursor.positionFirstOrLast(true /*first*/)) {
        ...
        OperationResult result =
            cursor.lockAndGetCurrent(
                key, data, lockType, false /*dirtyReadAll*/, true /*isLatched*/, true /*unlatch*/);
  ...
  /**
   * Dump the cursor for debugging purposes. Dump the bin that the cursor refers to if verbose is
   * true.
   */
   ...
   */
  private void trace(
      Level level, String changeType, BIN theBin, int lnIndex, long oldLsn, long newLsn) {
