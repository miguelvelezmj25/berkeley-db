...
public class DatabaseImpl implements Loggable, Cloneable {
  ...
  /**
   * For unit testing, setting this field to true will force a walk of the tree to count utilization
   * during truncate/remove, rather than using the per-database info. This is used to test the "old
   * technique" for counting utilization, which is now used only if the database was created prior
   * to log version 6.
   */
   ...
   */
  private final AtomicReference<List<Trigger>> triggers = new AtomicReference<List<Trigger>>(null);
...
//    maxTreeEntriesPerNode = dbConfig.getNodeMaxEntries();
  ...
  private void commonInit() {
    ...
    referringHandles = Collections.synchronizedSet(new HashSet<Database>());
    dbFileSummaries = new DbFileSummaryMap(false /* countParentMapEntry */);
  }
  ...
  /**
   * Returns the DB name for debugging and error messages. This method should be called rather than
   * getName to avoid accessing the db mapping tree in error situations The name may not be
   * transactionally correct, and may be unknown under certain circumstances (see
   * DbTree.setDebugNameForDatabaseImpl) in which case a string containing the DB ID is returned.
   */
  ...
  /**
   * Initialize configuration settings when creating a new instance or after reading an instance
   * from the log. The envImpl field must be set before calling this method.
   */
  private void initWithEnvironment() {
    ...
    assert !(replicatedBitSet() && notReplicatedBitSet())
        : "The replicated AND notReplicated bits should never be set " + " together";
    ...
    */
    ...
    if (maxTreeEntriesPerNode == 0) {
      maxTreeEntriesPerNode = configMgr.getInt(EnvironmentParams.NODE_MAX);
    }
    ...
    if (!envImpl.getNoComparators()) {
      ...
      ComparatorReader reader =
          new ComparatorReader(btreeComparatorBytes, "Btree", envImpl.getClassLoader());
      btreeComparator = reader.getComparator();
      btreeComparatorByClassName = reader.isClass();
      btreePartialComparator = btreeComparator instanceof PartialComparator;
      btreeBinaryEqualityComparator =
          (btreeComparator == null || btreeComparator instanceof BinaryEqualityComparator);

      reader =
          new ComparatorReader(duplicateComparatorBytes, "Duplicate", envImpl.getClassLoader());
      duplicateComparator = reader.getComparator();
      duplicateComparatorByClassName = reader.isClass();
      duplicatePartialComparator = duplicateComparator instanceof PartialComparator;
      duplicateBinaryEqualityComparator =
          (duplicateComparator == null || duplicateComparator instanceof BinaryEqualityComparator);
  ...
  /**
   * Create a clone of this database that can be used as the new empty database when truncating this
   * database. setId and setTree must be called on the returned database.
   */
  public DatabaseImpl cloneDatabase() {
    ...
    newDb.dbFileSummaries = new DbFileSummaryMap(false /*countParentMapEntry*/);
  ...
  /** @return true if this database was user configured for durable deferred write mode. */
  public boolean isDurableDeferredWrite() {
    return durableDeferredWrite;
  }

  /**
   * @return true if write operations are not logged immediately. This is true if the user
   *     configured a durable DW database or a temporary database.
   */
  ...
  /**
   * Returns whether all LNs in this DB are "immediately obsolete", meaning two things: 1) They are
   * counted obsolete when logged and can be ignored by the cleaner entirely. 2) As a consequence,
   * they cannot be fetched by LSN, except under special circumstances where they are known to
   * exist.
   *
   * <p>Currently, this is synonymous with whether all LNs in this DB must have zero length data,
   * and partial comparators are not used. Currently only duplicate DBs are known to have zero
   * length LNs, since there is no way in the API to specify that LNs are immutable. In the future
   * we will also support "immediately obsolete" LNs that are mutable and embedded in the BIN in
   * other ways, e.g., tiny data may be stored with the key.
   *
   * <p>Note that deleted LNs (the logged deletion, not the prior version) are always immediately
   * obsolete also. See LNLogEntry.isImmediatelyObsolete.
   */
  public boolean isLNImmediatelyObsolete() {
    return getSortedDuplicates() && !btreePartialComparator && !duplicatePartialComparator;
  }

  /**
   * This method should be the only method used to obtain triggers after reading the MapLN from the
   * log. It unmarshalls the triggers lazily here to avoid a call to getName() during recovery, when
   * the DbTree is not yet instantiated.
   */
  public List<Trigger> getTriggers() {
     ...
     */
    myTriggers = TriggerUtils.unmarshallTriggers(getName(), triggerBytes, envImpl.getClassLoader());
    if (myTriggers == null) {
  ...
  /**
   * Returns true if the flagVal enables the KeyPrefixing, used to create ReplicatedDatabaseConfig
   * after reading a NameLNLogEntry.
   */
  ...
  /**
   * @return true if this database is replicated. Note that we only need to check the
   *     IS_REPLICATED_BIT, because we require that we never have both IS_REPLICATED and
   *     NOT_REPLICATED set at the same time.
   */
  ...
  public boolean unknownReplicated() {
    return ((flags & IS_REPLICATED_BIT) == 0) && ((flags & NOT_REPLICATED_BIT) == 0);
  }
  ...
  /**
   * Used to determine whether to throw ReplicaWriteException when a write to this database is
   * attempted. For the most part, writes on a replica are not allowed to any replicated DB.
   * However, an exception is the DB naming DB. The naming DB contains a mixture of LNs for
   * replicated and non-replicated databases. Here, we allow all writes to the naming DB. DB naming
   * operations for replicated databases on a replica, such as the creation of a replicated DB on a
   * replica, are prohibited by DbTree methods (dbCreate, dbRemove, etc). [#20543]
   */
  ...
  /**
   * Returns the default cache mode for this database. If the database has a null cache mode and is
   * not an internal database, the Environment default is returned. Null is never returned.
   * CacheMode.DYNAMIC may be returned.
   */
   ...
   *
   * <p>This is a start at budgeting per-Database memory. For future reference, other things that
   * could be budgeted are: - debugDatabaseName as it is set - Database handles as they are
   * added/removed in referringHandles
   */
  public int getAdditionalTreeMemorySize() {
    ...
    if (btreeComparator != null) {
      val += 2 * MemoryBudget.byteArraySize(btreeComparatorBytes.length);
    }
    if (duplicateComparator != null) {
      val += 2 * MemoryBudget.byteArraySize(duplicateComparatorBytes.length);
    }
   ...
   */
  public boolean setDuplicateComparator(Comparator<byte[]> comparator, boolean byClassName)
      throws DatabaseException {

    final byte[] newBytes = comparatorToBytes(comparator, byClassName, "Duplicate");
   ...
   *
   * @param dbName pass it in since it may not be available during database creation
   * @param newTriggers the triggers to associate with the database
   * @param overridePersistentTriggers whether to overwrite persistent triggers
   * @return true if a {@link PersistentTrigger} was changed, and therefore may need to be stored.
   */
  public boolean setTriggers(
      Locker locker, String dbName, List<Trigger> newTriggers, boolean overridePersistentTriggers) {
    ...
    if (overridePersistentTriggers) {
      if (newTriggers == null) {
        ...
        if (nTriggers == 0) {
          ...
          for (Trigger trigger : newTriggers) {
            if (trigger instanceof PersistentTrigger) {
              newTriggerBytes[i++] = objectToBytes(trigger, "trigger " + trigger.getName());
              trigger.setDatabaseName(dbName);
            }
          }
          persistentChange = !Arrays.equals(triggerBytes, newTriggerBytes);
        }
      }
    } else {
    ...
    if (newTriggers == null) {
      ...
      final Map<Trigger, Object> diffs = new IdentityHashMap<Trigger, Object>();
      for (Trigger trigger : newTriggers) {
        if (!(trigger instanceof PersistentTrigger)) {
      ...
      } else if (transientTriggers.size() != newTransientTriggers.size()) {
    ...
    if (persistentChange || transientChange) {
      TriggerManager.invokeAddRemoveTriggers(locker, getTriggers(), newTriggers);
      /* Don't change fields until after getTriggers() call above. */
      triggerBytes = newTriggerBytes;
      transientTriggers =
          ((newTransientTriggers != null) && (newTransientTriggers.size() > 0))
  ...
  /**
   * Called when a database is closed to clear all transient triggers and call their 'removeTrigger'
   * methods.
   */
  private void clearTransientTriggers() {
    ...
    setTriggers(
        null /*locker*/, null /*dbName*/, newTriggers, false /*overridePersistentTriggers*/);
  }
   ...
   */
  public boolean setBtreeComparator(Comparator<byte[]> comparator, boolean byClassName)
      throws DatabaseException {

    final byte[] newBytes = comparatorToBytes(comparator, byClassName, "Btree");
  ...
  /**
   * This comparator should not be used directly for comparisons. Use getKeyComparator instead.
   *
  ...
  private void resetKeyComparator() {
    ...
    if (btreeComparator instanceof DatabaseComparator) {
      ((DatabaseComparator) btreeComparator).initialize(envImpl.getClassLoader());
    }
    if (duplicateComparator instanceof DatabaseComparator) {
      ((DatabaseComparator) duplicateComparator).initialize(envImpl.getClassLoader());
    }

    /* Create derived comparator for duplicate database. */
    if (getSortedDuplicates()) {
      keyComparator = new DupKeyData.TwoPartKeyComparator(btreeComparator, duplicateComparator);
    } else {
   ...
   *
   * <p>For a duplicates database, the data is part two of the two-part database key. Therefore, the
   * duplicates comparator and btree comparator are used for comparing keys. This synthetic
   * comparator will call both of the other two user-defined comparators as necessary.
   */
  public Comparator<byte[]> getKeyComparator() {
    return keyComparator;
  }

  /** @return whether Comparator is set by class name, not by serializable Comparator object. */
  ...
  public void handleClosed(boolean doSyncDw, boolean deleteTempDb) throws DatabaseException {
    ...
    if (referringHandles.isEmpty()) {
      ...
      if (deleteTempDb && isTemporary()) {
        Locker locker = BasicLocker.createBasicLocker(envImpl, true /* noWait */);
  ...
  /**
   * Figure out how much memory is used by the DbFileSummaryMap. Usually this number is built up
   * over time by the DbFileSummaryMap itself and added to the memory budget, but in this case we
   * need to reinitialize it after recovery, when DbFileSummaryMaps may be cut adrift by the process
  ...
  /**
   * Update memory budgets when this databaseImpl is closed and will never be accessed again or when
   * it is still open when its owning MapLN will be garbage collected, due to eviction or recovery.
   */
  ...
  /**
   * Increments the use count of this DB to prevent it from being evicted. Called by the
   * DbTree.createDb/getDb methods that return a DatabaseImpl. Must be called while holding a lock
   * on the MapLN. See isInUse. [#13415]
   */
   ...
   * Called via DbTree.releaseDb to release a DatabaseImpl that was returned by a
   * DbTree.createDb/getDb method. See isInUse. [#13415]
   */
  ...
  /**
   * Returns whether this DB is in use and cannot be evicted. Called by MapLN.isEvictable while
   * holding a write-lock on the MapLN and a latch on its parent BIN. [#13415]
   *
   * <p>When isInUse returns false (while holding a write-lock on the MapLN and a latch on the
   * parent BIN), it guarantees that the database object is not in use and cannot be acquired by
   * another thread (via DbTree.createDb/getDb) until both the MapLN lock and BIN latch are
   * released. This guarantee is due to the fact that DbTree.createDb/getDb only increment the use
   * count while holding a read-lock on the MapLN. Therefore, it is safe to evict the MapLN when
   * isInUse returns false.
   *
   * <p>When isInUse returns true, it is possible that another thread may decrement the use count at
   * any time, since no locking or latching is performed when calling DbTree.releaseDb (which calls
   * decrementUseCount). Therefore, it is not guaranteed that the MapLN is in use when isInUse
   * returns true. A true result means: the DB may be in use, so it is not safe to evict it.
   */
  ...
  /**
   * For this secondary database return the primary that it is associated with, or null if not
   * associated with any primary. Note that not all handles need be associated with a primary.
   */
  ...
  /**
   * Returns the DbFileSummary for the given file, allocates it if necessary and budgeted memory for
   * any changes.
   *
  ...
  public DbFileSummary getDbFileSummary(Long fileNum, boolean willModify) {
     ...
     */
    return dbFileSummaries.get(
        fileNum, true /*adjustMemBudget*/, true /*checkResurrected*/, envImpl.getFileManager());
  }
   ...
   * <p>Must be called under the log write latch.
   *
   * @return whether a DbFileSummary for any of the given files was present and was removed.
   */
  ...
  /**
   * Returns whether this database has new (unflushed) utilization info or the root LSN was modified
   * after it was last logged.
   */
   ...
   *
   * <p>This flag is used when utilization is changed, the root LSN is changed, etc, in order to
   * cause the MapLN to be flushed during the next checkpoint, or when utilization info is logged.
   */
  ...
  /** @return true if this database is deleted. Delete cleanup may still be in progress. */
  ...
  /**
   * The delete cleanup is starting. Set this before releasing any write locks held for a db
   * operation.
   */
  ...
  /**
   * Perform the entire two-step database deletion. This method is used at non-transactional
   * operation end. When a transaction is used (see Txn), startDeleteProcessing is called at commit
   * before releasing write locks and finishDeleteProcessing is called after releasing write locks.
   */
  ...
  /**
   * Release the INs for the deleted database, count all log entries for this database as obsolete,
   * delete the MapLN, and set the state to DELETED.
   *
   * <p>Used at transaction end or non-transactional operation end in these cases: - purge the
   * deleted database after a commit of Environment.removeDatabase - purge the deleted database
   * after a commit of Environment.truncateDatabase - purge the newly created database after an
   * abort of Environment.truncateDatabase
   *
   * <p>Note that the processing of the naming tree means the MapLN is never actually accessible
   * from the current tree, but deleting the MapLN will do two things: (a) mark it properly obsolete
   * (b) null out the database tree, leaving the INList the only reference to the INs.
   */
  public void finishDeleteProcessing() throws DatabaseException {
    ...
    try {
      ...
      if (createdAtLogVersion >= 6 && !forceTreeWalkForTruncateAndRemove) {
        ...
        boolean fetchLNSize = envImpl.getCleaner().getFetchObsoleteSize(this);
        ...
        SortedLSNTreeWalker walker =
            new ObsoleteTreeWalker(this, rootLsn, fetchLNSize, obsoleteProcessor, rootIN);
      ...
      try {
        ...
        while (iter.hasNext()) {
          ...
          if (thisIN.getDatabase() == this) {
            iter.remove();
            memoryChange += (0 - thisIN.getBudgetedMemorySize());
          }
        }
      } finally {
   ...
   *
   * @param mapLnLsn is the LSN of the MapLN when called via recovery, otherwise is NULL_LSN.
   *     <p>Must be called under the log write latch or during recovery.
   */
  ...
  private static class ObsoleteTreeWalker extends SortedLSNTreeWalker {
    ...
    @Override
    IN getResidentRootIN(@SuppressWarnings("unused") DatabaseImpl ignore) {
      if (rootIN != null) {
        rootIN.latchShared();
      }
      return rootIN;
  ...
  private static class ObsoleteProcessor implements TreeNodeProcessor {
    ...
    public void processLSN(
        long childLsn, LogEntryType childType, Node node, byte[] lnKey, int lastLoggedSize) {
      ...
      localTracker.countObsoleteNodeInexact(childLsn, childType, size, db);
    }

    @Override
    public void processDirtyDeletedLN(
        long childLsn, LN ln, @SuppressWarnings("unused") byte[] lnKey) {
      assert ln != null;
       ...
       */
      localTracker.countObsoleteNodeInexact(childLsn, ln.getGenericLogType(), 0, db);
    }
  ...
  private static class DOSCountCallback implements DiskOrderedScanner.RecordProcessor {
    ...
    @Override
    public void process(
        int dbIdx, byte[] key, byte[] data, int expiration, boolean expirationInHours) {
  ...
  public long count(long memoryLimit) throws DatabaseException {
    ...
    try {
       ...
       */
      if (memoryLimit <= 0) {
        memoryLimit = (JVMSystemUtils.getRuntimeMaxMemory() - mb.getMaxMemory()) / 10;
      }
      ...
      DiskOrderedScanner scanner =
          new DiskOrderedScanner(
              ...
              true /*keyOnly*/,
              true /*countOnly*/,
              Long.MAX_VALUE /*lsnBatchSize*/,
              memoryLimit,
              false /*debug*/);

      scanner.scan(
          FileProtector.DATABASE_COUNT_NAME, envImpl.getNodeSequence().getNextDatabaseCountId());
   ...
   *
   * <p>Counts entries in a key range by positioning a cursor on the beginning key and skipping
   * entries until the ending key is encountered.
   */
  private long count(
      DatabaseEntry beginKey, boolean beginInclusive, DatabaseEntry endKey, boolean endInclusive) {
    ...
    try {
      final Cursor c = DbInternal.makeCursor(this, locker, null /*cursorConfig*/);
      ...
      try {
        ...
        if (beginKey != null) {
          ...
          if (c.getSearchKeyRange(key, noData, lockMode) != OperationStatus.SUCCESS) {
            return 0;
          }

          if (!beginInclusive && key.equals(beginKey)) {
            if (c.getNext(key, noData, lockMode) != OperationStatus.SUCCESS) {
              return 0;
            }
          }
        } else {
          if (c.getFirst(key, noData, lockMode) != OperationStatus.SUCCESS) {
            return 0;
          }
        }
        ...
        return 1
            + DbInternal.getCursorImpl(c).skip(true /*forward*/, 0 /*maxCount*/, rangeConstraint);
  ...
  public String dumpString(int nSpaces) {
    ...
    if (btreeComparator != null) {
      sb.append(" btc=\"");
      sb.append(getComparatorClassName(btreeComparator, btreeComparatorBytes));
      sb.append("\"");
    ...
    if (duplicateComparator != null) {
      sb.append(" dupc=\"");
      sb.append(getComparatorClassName(duplicateComparator, duplicateComparatorBytes));
      sb.append("\"");
    ...
    if (dbFileSummaries != null) {
      Iterator<Map.Entry<Long, DbFileSummary>> entries = dbFileSummaries.entrySet().iterator();
  ...
  public int getLogSize() {
    ...
    Iterator<Map.Entry<Long, DbFileSummary>> i = dbFileSummaries.entrySet().iterator();
    while (i.hasNext()) {
      Map.Entry<Long, DbFileSummary> entry = i.next();
      Long fileNum = entry.getKey();
      DbFileSummary summary = entry.getValue();
      size += LogUtils.getPackedLongLogSize(fileNum.longValue()) + summary.getLogSize();
    }
  ...
  public void readFromLog(ByteBuffer itemBuffer, int entryVersion) {
    ...
    if (entryVersion >= 2) {
      btreeComparatorBytes = LogUtils.readByteArray(itemBuffer, !version6OrLater);
      duplicateComparatorBytes = LogUtils.readByteArray(itemBuffer, !version6OrLater);
    } else {
      ...
      if (btreeClassName.length() == 0) {
        ...
        btreeComparatorBytes = objectToBytes(btreeClassName, "Btree");
      }
      if (dupClassName.length() == 0) {
        duplicateComparatorBytes = LogUtils.ZERO_LENGTH_BYTE_ARRAY;
      } else {
        duplicateComparatorBytes = objectToBytes(dupClassName, "Duplicate");
      }
    }

    if (entryVersion >= 1) {
      maxTreeEntriesPerNode = LogUtils.readInt(itemBuffer, !version6OrLater);
      if (entryVersion < 8) {
    ...
    if (version6OrLater) {
      ...
      for (int i = 0; i < nFiles; i += 1) {
        ...
        DbFileSummary summary =
            dbFileSummaries.get(
                Long.valueOf(fileNum),
                false /*adjustMemBudget*/,
    ...
    triggerBytes = (entryVersion < 8) ? null : TriggerUtils.readTriggers(itemBuffer, entryVersion);
  ...
  static void dumpFlags(StringBuilder sb, @SuppressWarnings("unused") boolean verbose, byte flags) {
    sb.append(" dupsort=\"").append((flags & DUPS_ENABLED) != 0);
    sb.append("\" replicated=\"").append((flags & IS_REPLICATED_BIT) != 0);
    sb.append("\" temp=\"").append((flags & TEMPORARY_BIT) != 0).append("\" ");
  }
  ...
  private static String getComparatorClassName(
      Comparator<byte[]> comparator, byte[] comparatorBytes) {

    if (comparator != null) {
      return comparator.getClass().getName();
    } else if (comparatorBytes != null && comparatorBytes.length > 0) {
  ...
  public static Comparator<byte[]> instantiateComparator(
      Class<? extends Comparator<byte[]>> comparatorClass, String comparatorType) {
    ...
    try {
      ...
      throw EnvironmentFailureException.unexpectedException(
          "Exception while trying to load " + comparatorType + " Comparator class.", e);
    }
  }

  /** Used to validate a comparator when set in DatabaseConfig. */
  @SuppressWarnings("unchecked")
  public Comparator<byte[]> instantiateComparator(
      Comparator<byte[]> comparator, String comparatorType) throws DatabaseException {

    if (comparator == null) {
      return null;
    }

    return (Comparator<byte[]>)
        bytesToObject(
            objectToBytes(comparator, comparatorType), comparatorType, envImpl.getClassLoader());
  }

  /**
   * Converts a comparator object to a serialized byte array, converting to a class name String
   * object if byClassName is true.
   *
   * @throws EnvironmentFailureException if the object cannot be serialized.
   */
  public static byte[] comparatorToBytes(
      Comparator<byte[]> comparator, boolean byClassName, String comparatorType) {
    if (comparator == null) {
      return LogUtils.ZERO_LENGTH_BYTE_ARRAY;
    }

    final Object obj = byClassName ? comparator.getClass().getName() : comparator;

    return objectToBytes(obj, comparatorType);
  }

  /**
   * Converts an arbitrary object to a serialized byte array. Assumes that the object given is
   * non-null.
   */
  ...
  /**
   * Converts an arbitrary serialized byte array to an object. Assumes that the byte array given is
   * non-null and has a non-zero length.
   */
  ...
  static class ComparatorReader {
    ...
    public ComparatorReader(byte[] comparatorBytes, String type, ClassLoader loader) {
      ...
      if (obj instanceof String) {
        ...
        try {
          comparatorClass =
              (Class<? extends Comparator<byte[]>>) ClassResolver.resolveClass(className, loader);
      ...
      throw EnvironmentFailureException.unexpectedState(
          "Expected class name or Comparator instance, got: " + obj.getClass().getName());
    }
  ...
  /**
   * Return a ReplicationContext that will indicate if this operation should broadcast data records
   * for this database as part the replication stream.
   */
  ...
  /**
   * Return a ReplicationContext that includes information on how to logically replicate database
   * operations. This kind of replication context must be used for any api call which logging a
   * NameLN for that represents a database operation. However, NameLNs which are logged for other
   * reasons, such as cleaner migration, don't need this special replication context.
   */
  DbOpReplicationContext getOperationRepContext(DbOperationType operationType, DatabaseId oldDbId) {
     ...
     */
    DbOpReplicationContext context = new DbOpReplicationContext(isReplicated(), operationType);
