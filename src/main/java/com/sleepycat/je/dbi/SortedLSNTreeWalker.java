...
/**
 * SortedLSNTreeWalker uses ordered disk access rather than random access to iterate over a database
 * tree. Faulting in data records by on-disk order can provide much improved performance over
 * faulting in by key order, since the latter may require random access. SortedLSN walking does not
 * obey cursor and locking constraints, and therefore can only be guaranteed consistent for a
 * quiescent tree which is not being modified by user or daemon threads.
 *
 * <p>The class walks over the tree using sorted LSN fetching for parts of the tree that are not in
 * memory. It returns LSNs for each node in the tree, <b>except</b> the root IN, in an arbitrary
 * order (i.e. not key order). The caller is responsible for getting the root IN's LSN explicitly.
 *
 * <p>A callback function specified in the constructor is executed for each LSN found.
 *
 * <p>The walker works in two phases. The first phase is to gather and return all the resident INs
 * using the roots that were specified when the SLTW was constructed. For each child of each root,
 * if the child is resident it is passed to the callback method (processLSN). If the child was not
 * in memory, it is added to a list of LSNs to read. When all of the in-memory INs have been passed
 * to the callback for all LSNs collected, phase 1 is complete.
 *
 * <p>In phase 2, for each of the sorted LSNs, the target is fetched, the type determined, and the
 * LSN and type passed to the callback method for processing. LSNs of the children of those nodes
 * are retrieved and the process repeated until there are no more nodes to be fetched for this
 * database's tree. LSNs are accumulated in batches in this phase so that memory consumption is not
 * excessive. For instance, if batches were not used then the LSNs of all of the BINs would need to
 * be held in memory.
 */
public class SortedLSNTreeWalker {
  ...
  public interface TreeNodeProcessor {
    void processLSN(
        long childLSN, LogEntryType childType, Node theNode, byte[] lnKey, int lastLoggedSize)
  ...
  /**
   * Supplements INEntry with BIN-delta information. When a BIN-delta is encountered during the
   * fetching process, we cannot immediately place it in the tree. Instead we queue a DeltaINEntry
   * for fetching the full BIN, in LSN order as usual. When the full BIN is fetched, the
   * DeltaINEntry is used to apply the delta and place the result in the tree.
   */
  public static class DeltaINEntry extends INEntry {
    ...
    long getMemorySize() {
      ...
      return deltaSize + MemoryBudget.HASHMAP_ENTRY_OVERHEAD + MemoryBudget.DELTAINENTRY_OVERHEAD;
    }
  }
  ...
  public SortedLSNTreeWalker(
    ...
    if (dbImpls == null || dbImpls.length < 1) {
      throw EnvironmentFailureException.unexpectedState(
          "DatabaseImpls array is null or 0-length for " + "SortedLSNTreeWalker");
    }
    ...
    for (DatabaseImpl di : dbImpls) {
      ...
      if (ei == null) {
        throw EnvironmentFailureException.unexpectedState(
            "environmentImpl is null for target db " + di.getDebugName());
      }
  ...
  /**
   * Find all non-resident nodes, and execute the callback. The root IN's LSN is not returned to the
   * callback.
   */
  public void walk() {
    walkInternal();
  ...
  private void accumulateLSNs(
    ...
    for (int i = 0; i < parent.getNEntries(); i += 1) {
      ...
      final byte[] lnKey =
          (allChildrenAreLNs || (childCached && child.isLN())) ? parent.getKey(i) : null;
      ...
      if (parentBin != null && parentBin.isDefunct(i)) {
      ...
      } else if (!childCached && parentBin != null && parentBin.getOffHeapLNId(i) != 0) {
        ...
        processChild(lsn, child, lnKey, parent.getLastLoggedSize(i), pendingLSNs, null, -1);

      } else if (!childCached && parent.getOffHeapBINId(i) >= 0) {

        child = ohCache.materializeBIN(envImpl, ohCache.getBINBytes(parent, i));
        ...
        try {
          if (bin.isBINDelta()) {
            ...
            processChild(lsn, bin, lnKey, parent.getLastLoggedSize(i), pendingLSNs, parent, i);
          }
        } finally {
          if (isLatched) {
            bin.releaseLatch();
      ...
      } else if (accumulateChildren && !childCached && lsn != DbLsn.NULL_LSN) {
        ...
        if (!parent.isEmbeddedLN(i)) {
          ...
          processChild(
              DbLsn.NULL_LSN, null /*child*/, lnKey, 0 /*lastLoggedSize*/, pendingLSNs, null, -1);
        }
         ...
         */
        processChild(
            lsn, null /*child*/, lnKey, parent.getLastLoggedSize(i), pendingLSNs, null, -1);
      }
       ...
       */
      final boolean internalMemoryExceeded = internalMemoryUsage > internalMemoryLimit;

      if (pendingLSNs.getNTotalEntries() > lsnBatchSize || internalMemoryExceeded) {
  ...
  private void fetchAndProcessLSN(long lsn, LSNAccumulator pendingLSNs) {
    ...
    final FetchResult result = fetchLSNHandleExceptions(lsn, lnKeyEntry, pendingLSNs);
    ...
    try {
      callProcessLSNHandleExceptions(
          lsn,
          result.node.getGenericLogType(),
          result.node,
          lnKeyEntry.getData(),
          result.lastLoggedSize);

      if (isIN) {
        accumulateLSNs(in, pendingLSNs, result.ohBinParent, result.ohBinIndex);
      }
    } finally {
      if (isIN) {
        in.releaseLatch();
  ...
  private FetchResult fetchLSNHandleExceptions(
      long lsn, DatabaseEntry lnKeyEntry, LSNAccumulator pendingLSNs) {
    ...
    try {
      ...
      if (excPredicate == null || !excPredicate.ignoreException(e)) {
        dbe = e;
      }
    }
  ...
  private void callProcessLSNHandleExceptions(
      long childLSN, LogEntryType childType, Node theNode, byte[] lnKey, int lastLoggedSize) {
    DatabaseException dbe = null;

    try {
      callback.processLSN(childLSN, childType, theNode, lnKey, lastLoggedSize);
  ...
  /**
   * Returns the root IN, latched shared. Allows subclasses to override getResidentRootIN and/or
   * getRootIN to modify behavior. getResidentRootIN is called first,
   */
  ...
  /**
   * The default behavior fetches the rootIN from the log and latches it shared. Classes extending
   * this may fetch (and latch) the root from the tree.
   */
  IN getRootIN(DatabaseImpl dbImpl, long rootLsn) {
    final IN root = (IN) envImpl.getLogManager().getEntryHandleFileNotFound(rootLsn);
    if (root == null) {
      return null;
    }
  ...
  /**
   * The default behavior returns (and latches shared) the IN if it is resident in the Btree, or
   * null otherwise. Classes extending this may return (and latch) a known IN object.
   */
  ...
  private static class FetchResult {
    ...
    FetchResult(
        final Node node, final int lastLoggedSize, final IN ohBinParent, final int ohBinIndex) {
      this.node = node;
   ...
   * sorted LSN order.
   */
  private FetchResult fetchLSN(long lsn, DatabaseEntry lnKeyEntry, LSNAccumulator pendingLSNs) {
    ...
    try {
      ...
      if (isOffHeapBinInTree && deltaObject == null) {
        ...
        final BIN ohBin = ohCache.materializeBIN(envImpl, ohCache.getBINBytes(in, index));
      ...
      if (residentNode != null) {
        ...
        if (!residentNode.isBINDelta()) {
          ...
          return new FetchResult(residentNode, in.getLastLoggedSize(index), null, -1);
        }
      ...
      try {
        ...
        final String msg =
            (fetchAndInsertIntoTree() ? "Preload failed" : "SortedLSNTreeWalker failed")
                + " dbId="
                + dbImpl.getId()
                + " isOffHeapBinInTree="
                + isOffHeapBinInTree
        ...
        throw new EnvironmentFailureException(
            ...
            in.makeFetchErrorMsg(msg, lsn, index),
            e);
      }
      ...
      if (mutateResidentDeltaToFullBIN) {
        ...
        if (fetchAndInsertIntoTree()) {
          ...
          return new FetchResult(residentNode, lastLoggedSize, ohBinParent, ohBinIndex);
        } else {
          delta.reconstituteBIN(dbImpl, fullBIN, false /*leaveFreeSlot*/);

          return new FetchResult(ret, lastLoggedSize, ohBinParent, ohBinIndex);
        }
      }
      ...
      if (fetchAndInsertIntoTree()) {
        ...
        if (isOffHeapBinInTree || (storeOffHeap && !ret.isUpperIN())) {
          ...
          if (in.isBIN()) {
            final CacheMode mode = in.getDatabase().getDefaultCacheMode();
      ...
      return new FetchResult(ret, lastLoggedSize, retOhBinParent, retOhBinIndex);
