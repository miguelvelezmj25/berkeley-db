...
/**
 * A struct for passing record expiration info to a 'put' operation, and returning the old
 * expiration time plus whether it was updated/changed.
 */
public class ExpirationInfo {
  ...
  public ExpirationInfo(
      final int expiration, final boolean expirationInHours, final boolean updateExpiration) {
   ...
   * @return ExpirationInfo, or null if WriteOptions.getTTL is zero and WriteOptions.getUpdateTTL is
   *     false, meaning we will not add or update the TTL.
   */
