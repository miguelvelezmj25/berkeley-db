...
import static com.sleepycat.je.log.entry.DbOperationType.*;

/**
 * DbTree represents the database directory for this environment. DbTree is itself implemented
 * through two databases. The nameDatabase maps databaseName-> an internal databaseId. The
 * idDatabase maps databaseId->DatabaseImpl.
 *
 * <p>For example, suppose we have two databases, foo and bar. We have the following structure:
 *
 * <p>nameDatabase idDatabase IN IN | | BIN BIN +-------------+--------+ +---------------+--------+
 * . | | . | | NameLNs NameLN NameLN MapLNs for MapLN MapLN for internal key=bar key=foo internal
 * dbs key=53 key=79 dbs data= data= data= data= dbId79 dbId53 DatabaseImpl DatabaseImpl | | Tree
 * for foo Tree for bar | | root IN root IN
 *
 * <p>Databases, Cursors, the cleaner, compressor, and other entities have references to
 * DatabaseImpls. It's important that object identity is properly maintained, and that all
 * constituents reference the same DatabaseImpl for the same db, lest they develop disparate views
 * of the in-memory database; corruption would ensue. To ensure that, all entities must obtain their
 * DatabaseImpl by going through the idDatabase.
 *
 * <p>DDL type operations such as create, rename, remove and truncate get their transactional
 * semantics by transactionally locking the NameLN appropriately. A read-lock on the NameLN, called
 * a handle lock, is maintained for all DBs opened via the public API (openDatabase). This prevents
 * them from being renamed or removed while open. See HandleLocker for details.
 *
 * <p>However, for internal database operations, no handle lock on the NameLN is acquired and MapLNs
 * are locked with short-lived non-transactional Lockers. An entity that is trying to get a
 * reference to the DatabaseImpl gets a short lived read lock just for the fetch of the MapLN, and a
 * DatabaseImpl usage count is incremented to prevent eviction; see getDb and releaseDb. A write
 * lock on the MapLN is taken when the database is created, deleted, or when the MapLN is evicted.
 * (see DatabaseImpl.isInUse())
 *
 * <p>The nameDatabase operates pretty much as a regular application database in terms of eviction
 * and recovery. The idDatabase requires special treatment for both eviction and recovery.
 *
 * <p>The issues around eviction of the idDatabase center on the need to ensure that there are no
 * other current references to the DatabaseImpl other than that held by the mapLN. The presence of a
 * current reference would both make the DatabaseImpl not GC'able, and more importantly, would lead
 * to object identity confusion later on. For example, if the MapLN is evicted while there is a
 * current reference to its DatabaseImpl, and then refetched, there will be two in-memory versions
 * of the DatabaseImpl. Since locks on the idDatabase are short lived, DatabaseImpl.useCount acts as
 * a reference count of active current references. DatabaseImpl.useCount must be modified and read
 * in conjunction with appropriate locking on the MapLN. See DatabaseImpl.isInUse() for details.
 *
 * <p>This reference count checking is only needed when the entire MapLN is evicted. It's possible
 * to evict only the root IN of the database in question, since that doesn't interfere with the
 * DatabaseImpl object identity.
 *
 * <p>Another dependency on usage counts was introduced to prevent MapLN deletion during cleaner and
 * checkpointer operations that are processing entries for a DB. (Without usage counts, this problem
 * would have occurred even if DB eviction were never implemented.) When the usage count is non-zero
 * it prohibits deleteMapLN from running. The deleted state of the MapLN must not change during a
 * reader operation (operation by a thread that has called getDb and not yet called releaseDb).
 *
 * <p>Why not just hold a MapLN read lock during a reader operation?
 * -------------------------------------------------------------- Originally this was not done
 * because of cleaner performance. We afraid that * either of the following solutions would not
 * perform well: + If we get (and release) a MapLN lock for every entry in a log file, this adds a
 * lot of per-entry overhead. + If we hold the MapLN read lock for the duration of a log file
 * cleaning (the assumption is that many entries are for the same DB), then we block checkpoints
 * during that period, when they call modifyDbRoot. Therefore, the usage count is incremented once
 * per DB encountered during log cleaning, and the count is decremented at the end. This caching
 * approach is also used by the HA replayer. In both cases, we do not want to lock the MapLN every
 * entry/operation, and we do not want to block checkpoints or other callers of modifyDbRoot. It is
 * acceptable, however, to block DB naming operations.
 *
 * <p>In addition we allow modifyDbRoot to run when even the usage count is non-zero, which would
 * not be possible using a read-write locking strategy. I'm not sure why this was done originally,
 * perhaps to avoid blocking. But currently, it is necessary to prevent a self-deadlock. All callers
 * of modifyDbRoot first call getDb, which increments the usage count. So if modifyDbRoot was to
 * check the usage count and retry if non-zero (like deleteMapLN), then it would loop forever.
 *
 * <p>Why are the retry loops necessary in the DbTree methods?
 * -------------------------------------------------------- Three methods that access the MapLN
 * perform retries (forever) when there is a lock conflict: getDb, modifyDbRoot and deleteMapLN.
 * Initially the retry loops were added to compensate for certain slow operations. To solve that
 * problem, perhaps there are alternative solutions (increasing the lock timeout). However, the
 * deleteMapLN retry loop is necessary to avoid deleting it when the DB is in use by reader
 * operations.
 *
 * <p>Tendency to livelock -------------------- Because MapLN locks are short lived, but a reader
 * operation may hold a MapLN/DatabaseImpl for a longer period by incrementing the usage count,
 * there is the possibility of livelock. One strategy for avoiding livelock is to avoid algorithms
 * where multiple threads continuously call getDb and releaseDb, since this could prevent completion
...
public class DbTree implements Loggable {
   ...
   *
   * <p>Note that we allow dbName to be null, because it may be null when the 'debug database name'
   * is not yet known to DatabaseImpl. This works because the debug name is always known immediately
   * for internal DBs.
   */
  ...
  /**
   * Number of LNs in the naming DB considered to be fairly small, and therefore to result in fairly
   * fast execution of getDbName.
   */
  ...
  public DbTree(EnvironmentImpl env, boolean replicationIntended, boolean preserveVLSN)
    ...
    idDatabase =
        new DatabaseImpl(null, DbType.ID.getInternalName(), new DatabaseId(0), env, idConfig);
    ...
    nameDatabase =
        new DatabaseImpl(null, DbType.NAME.getInternalName(), new DatabaseId(1), env, nameConfig);
    /* Force a reset if enabled globally. */
    nameDatabase.clearKeyPrefixing();
  ...
  /** We get a new database id of the appropriate kind when creating a new database. */
  ...
  /**
   * Initialize the db tree during recovery, after instantiating the tree from the log. a. set up
   * references to the environment impl b. check for replication rules.
   */
   ...
   *
   * <p>Increments the use count of the new DB to prevent it from being evicted. releaseDb should be
   * called when the returned object is no longer used, to allow it to be evicted. See
   * DatabaseImpl.isInUse. [#13415]
   */
  public DatabaseImpl createDb(
      Locker locker, String databaseName, DatabaseConfig dbConfig, HandleLocker handleLocker)
      throws DatabaseException {
  ...
  /**
   * Create a database for internal use. It may or may not be replicated. Since
   * DatabaseConfig.replicated is true by default, be sure to set it to false if this is a internal,
   * not replicated database.
   */
  public DatabaseImpl createInternalDb(Locker locker, String databaseName, DatabaseConfig dbConfig)
      throws DatabaseException {
  ...
  public DatabaseImpl createReplicaDb(
      Locker locker,
      String databaseName,
   ...
   * DatabaseImpl.isInUse. [#13415]
   *
   * <p>Do not evict (do not call CursorImpl.setAllowEviction(true)) during low level DbTree
   * operation. [#15176]
   */
  private DatabaseImpl doCreateDb(
      Locker nameLocker,
      String databaseName,
      DatabaseConfig dbConfig,
    ...
    try {
      newDb = new DatabaseImpl(nameLocker, databaseName, newId, envImpl, dbConfig);
  ...
  /**
   * Opens (or creates if it does not exist) an internal, non-replicated DB. Returns null only if
   * the DB does not exist and the env is read-only.
   */
  public DatabaseImpl openNonRepInternalDB(final DbType dbType) {

    final String name = dbType.getInternalName();

    final Locker autoTxn = Txn.createLocalAutoTxn(envImpl, new TransactionConfig());
  ...
  /**
   * Called after locking a NameLN with nameCursor when opening a database. The NameLN may be locked
   * for read or write, depending on whether the database existed when openDatabase was called. Here
   * we additionally lock the NameLN for read on behalf of the handleLocker, which is kept by the
   * Database handle.
   *
   * <p>The lock must be acquired while the BIN is latched, so the locker will be updated if the LSN
   * changes. There is no lock contention possible because the HandleLocker shares locks with the
   * nameCursor locker, and jumpAheadOfWaiters=true is passed in case another locker is waiting on a
   * write lock.
   *
   * <p>If the lock is denied, checkPreempted is called on the nameCursor locker, in case the lock
   * is denied because the nameCursor's lock was preempted. If so, DatabasePreemptedException will
   * be thrown.
   *
  ...
  private void acquireHandleLock(CursorImpl nameCursor, HandleLocker handleLocker) {
    ...
    try {
      ...
      final LockResult lockResult =
          handleLocker.nonBlockingLock(
              lsn, LockType.READ, true /*jumpAheadOfWaiters*/, nameDatabase);

      if (lockResult.getLockGrant() == LockGrantType.DENIED) {
        nameCursor.getLocker().checkPreempted(null);
        throw EnvironmentFailureException.unexpectedState(
            "No contention is possible with HandleLocker: " + DbLsn.getNoFormatString(lsn));
      }
    } finally {
  ...
  /**
   * Write a MapLN to the log in order to: - propagate a root change - save per-db utilization
   * information - save database config information. Any MapLN writes must be done through this
   * method, in order to ensure that the root latch is taken, and updates to the rootIN are properly
   * safeguarded. See MapN.java for more detail.
   *
   * @param db the database whose root is held by this MapLN
   * @param ifBeforeLsn if argument is not NULL_LSN, only do the write if this MapLN's current LSN
   *     is before isBeforeLSN.
   * @param mustExist if true, throw DatabaseException if the DB does not exist; if false, silently
   *     do nothing.
   */
  public void modifyDbRoot(DatabaseImpl db, long ifBeforeLsn, boolean mustExist)
      throws DatabaseException {
      ...
      while (true) {
        ...
        try {
          ...
          if (ifBeforeLsn == DbLsn.NULL_LSN
              || DbLsn.compareTo(cursor.getCurrentLsn(), ifBeforeLsn) < 0) {

            MapLN mapLN = (MapLN) cursor.getCurrentLN(true, /*isLatched*/ true /*unlatch*/);
  ...
  private static class RewriteMapLN implements WithRootLatched {
    ...
    public IN doWork(@SuppressWarnings("unused") ChildReference root) throws DatabaseException {
  ...
  /**
   * In other places (e.g., when write locking a record in ReadOnlyTxn) we allow writes to the
   * naming DB on a replica, since we allow both replicated and non-replicated DBs and therefore
   * some NameLNs are replicated and some are not. Below is the sole check to prevent a creation,
   * removal, truncation, or configuration update of a replicated DB on a replica. It will throw
   * ReplicaWriteException on a replica if this operation would assign a new VLSN. [#20543]
   */
  ...
  /**
   * Thrown by lockNameLN when an incorrect locker was used via auto-commit. See
   * Environment.DbNameOperation. A checked exception is used to ensure that it is always handled
   * internally and never propagated to the app.
   */
  public static class NeedRepLockerException extends Exception {}

  /**
   * Helper for database operations. This method positions a cursor on the NameLN that represents
   * this database and write locks it.
   *
   * <p>Do not evict (do not call CursorImpl.setAllowEviction(true)) during low level DbTree
   * operation. [#15176]
   *
   * @throws IllegalStateException via Environment.remove/rename/truncateDatabase
   */
  private NameLockResult lockNameLN(
      Locker locker, String databaseName, String action, GetRepContext getRepContext)
    ...
    if (result.dbImpl == null) {
      throw new DatabaseNotFoundException(
          "Attempted to " + action + " non-existent database " + databaseName);
    }
    ...
    try {
      ...
      result.nameLN = (NameLN) result.nameCursor.getCurrentLN(true, /*isLatched*/ true /*unlatch*/);
   ...
   *
   * <p>JE MapLN actually includes the DatabaseImpl information, but it is not transactional, so the
   * DatabaseConfig information is stored in NameLNLogEntry and replicated.
   *
   * <p>So when there is a DatabaseConfig changes, we'll update the NameLN for the database, which
   * will log a new NameLNLogEntry so that the rep stream will transfer it to the replicas and it
   * will be replayed.
   *
   * @param locker the locker used to update the NameLN
   * @param dbName the name of the database whose corresponding NameLN needs to be updated
   * @param repContext information used while replaying a NameLNLogEntry on the replicas, it's null
   *     on master
   */
  public void updateNameLN(Locker locker, String dbName, final DbOpReplicationContext repContext)
   ...
   * @throws DatabaseNotFoundException if the operation fails because the given DB name is not
   *     found.
   */
  private DatabaseImpl doRenameDb(
      Locker locker,
      String databaseName,
      String newName,
      NameLN replicatedLN,
    ...
    final NameLockResult result =
        lockNameLN(
            ...
            new GetRepContext() {
              ...
              public ReplicationContext get(DatabaseImpl dbImpl) {
                return (repContext != null) ? repContext : dbImpl.getOperationRepContext(RENAME);
              }
            });
    ...
    try {
      ...
      final NameLN useLN = (replicatedLN != null) ? replicatedLN : new NameLN(dbImpl.getId());
      /*
      ...
      nameCursor.insertRecord(
          StringUtils.toUTF8(newName), useLN, false /*blindInsertion*/, useRepContext);
   ...
   * @see #doRenameDb
   */
  public DatabaseImpl dbRename(Locker locker, String databaseName, String newName)
      throws DatabaseNotFoundException, NeedRepLockerException {
   ...
   * @see #doRenameDb
   */
  public DatabaseImpl renameReplicaDb(
      Locker locker,
      String databaseName,
      String newName,
      NameLN replicatedLN,
    ...
    try {
      return doRenameDb(locker, databaseName, newName, replicatedLN, repContext);
    } catch (NeedRepLockerException e) {
      /* Should never happen; db is known when locker is created. */
      throw EnvironmentFailureException.unexpectedException(envImpl, e);
    }
   ...
   * @throws DatabaseNotFoundException if the operation fails because the given DB name is not
   *     found, or the non-null checkId argument does not match the database identified by
   *     databaseName.
   */
  private DatabaseImpl doRemoveDb(
      Locker locker,
      String databaseName,
      DatabaseId checkId,
      final DbOpReplicationContext repContext)
    ...
    final NameLockResult result =
        lockNameLN(
            ...
            new GetRepContext() {
              ...
              public ReplicationContext get(DatabaseImpl dbImpl) {
                return (repContext != null) ? repContext : dbImpl.getOperationRepContext(REMOVE);
              }
            });
   ...
   * @see #doRemoveDb
   */
  public DatabaseImpl dbRemove(Locker locker, String databaseName, DatabaseId checkId)
      throws DatabaseNotFoundException, NeedRepLockerException {
   ...
   * @see #doRemoveDb
   */
  public void removeReplicaDb(
      Locker locker, String databaseName, DatabaseId checkId, DbOpReplicationContext repContext)
      throws DatabaseNotFoundException {
   ...
   * operation. [#15176]
   *
   * @param returnCount if true, must return the count of records in the database, which can be an
   *     expensive option.
   * @return the record count, oldDb and newDb packaged in a TruncateDbResult
   * @throws DatabaseNotFoundException if the operation fails because the given DB name is not
   *     found.
   */
  public TruncateDbResult doTruncateDb(
      Locker locker,
      String databaseName,
      boolean returnCount,
      NameLN replicatedLN,
    ...
    try {
      ...
      final DatabaseId newId =
          (replicatedLN != null)
              ? replicatedLN.getId()
              : new DatabaseId(
                  isReplicatedId(oldDb.getId().getId())
  ...
  /** @see #doTruncateDb */
  public TruncateDbResult truncate(Locker locker, String databaseName, boolean returnCount)
      throws DatabaseNotFoundException, NeedRepLockerException {
  ...
  /** @see #doTruncateDb */
  public TruncateDbResult truncateReplicaDb(
      Locker locker,
      String databaseName,
      boolean returnCount,
      NameLN replicatedLN,
    ...
    try {
      return doTruncateDb(locker, databaseName, returnCount, replicatedLN, repContext);
    } catch (NeedRepLockerException e) {
      /* Should never happen; db is known when locker is created. */
      throw EnvironmentFailureException.unexpectedException(envImpl, e);
    }
    ...
    while (!done) {
      ...
      try {
        ...
        if (found) {
           ...
           */
          MapLN mapLN = (MapLN) idCursor.getCurrentLN(true, /*isLatched*/ true /*unlatch*/);
  ...
  /**
   * Get a database object given a database name. Increments the use count of the given DB to
   * prevent it from being evicted. releaseDb should be called when the returned object is no longer
   * used, to allow it to be evicted. See DatabaseImpl.isInUse. [#13415]
   *
   * <p>Do not evict (do not call CursorImpl.setAllowEviction(true)) during low level DbTree
   * operation. [#15176]
   *
   * @param nameLocker is used to access the NameLN. As always, a NullTxn is used to access the
   *     MapLN.
   * @param databaseName target database
   * @return null if database doesn't exist
   */
  public DatabaseImpl getDb(
      Locker nameLocker, String databaseName, HandleLocker handleLocker, boolean writeLock)
      throws DatabaseException {
    ...
    try {
      ...
      if (found) {
        NameLN nameLN = (NameLN) nameCursor.getCurrentLN(true, /*isLatched*/ true /*unlatch*/);
  ...
  /**
   * Get a database object based on an id only. Used by recovery, cleaning and other clients who
   * have an id in hand, and don't have a resident node, to find the matching database for a given
   * log entry.
   */
  ...
  /**
   * Get a database object based on an id only. Specify the lock timeout to use, or -1 to use the
   * default timeout. A timeout should normally only be specified by daemons with their own timeout
   * configuration. public for unit tests.
   */
  ...
  public DatabaseImpl getDb(
      DatabaseId dbId, long lockTimeout, Map<DatabaseId, DatabaseImpl> dbCache)
      throws DatabaseException {
   ...
   * configuration. public for unit tests.
   *
   * <p>Increments the use count of the given DB to prevent it from being evicted. releaseDb should
   * be called when the returned object is no longer used, to allow it to be evicted. See
   * DatabaseImpl.isInUse. [#13415]
   *
   * <p>Do not evict (do not call CursorImpl.setAllowEviction(true)) during low level DbTree
   * operation. [#15176]
   */
  public DatabaseImpl getDb(DatabaseId dbId, long lockTimeout, String dbNameIfAvailable)
      throws DatabaseException {
  ...
  /**
   * Decrements the use count of the given DB, allowing it to be evicted if the use count reaches
   * zero. Must be called to release a DatabaseImpl that was returned by a method in this class. See
   * DatabaseImpl.isInUse. [#13415]
   */
  public void releaseDb(DatabaseImpl db) {
    /* Use count is not incremented for idDatabase and nameDatabase. */
    if (db != null && db != idDatabase && db != nameDatabase) {
      db.decrementUseCount();
    }
  }

  /**
   * Calls releaseDb for all DBs in the given map of DatabaseId to DatabaseImpl. See
   * getDb(DatabaseId, long, Map). [#13415]
   */
  ...
  private void setDebugNameForDatabaseImpl(DatabaseImpl dbImpl, String dbName)
    ...
    if (dbImpl != null) {
      if (dbName != null) {
         ...
         */
        if (envImpl.isValid() && !dbImpl.isDebugNameAvailable() && getFastNameLookup()) {
  ...
  /**
   * Returns true if the naming DB has a fairly small number of names, and therefore execution of
   * getDbName will be fairly fast.
   */
  ...
  /**
   * Return the database name for a given db. Slow, must traverse. Called by Database.getName.
   *
   * <p>Do not evict (do not call CursorImpl.setAllowEviction(true)) during low level DbTree
   * operation. [#15176]
   */
  public String getDbName(final DatabaseId id) throws DatabaseException {
    ...
    class Traversal implements CursorImpl.WithCursor {
      ...
      public boolean withCursor(
          CursorImpl cursor, DatabaseEntry key, @SuppressWarnings("unused") DatabaseEntry data)
          throws DatabaseException {
  ...
  public Map<DatabaseId, String> getDbNamesAndIds() throws DatabaseException {
    ...
    final Map<DatabaseId, String> nameMap = new HashMap<DatabaseId, String>();
  ...
  public List<String> getDbNames() throws DatabaseException {
    ...
    CursorImpl.traverseDbWithCursor(
        ...
        new CursorImpl.WithCursor() {
          public boolean withCursor(
              @SuppressWarnings("unused") CursorImpl cursor,
              DatabaseEntry key,
  ...
  /**
   * Returns the initial log version at the time the env was created, or -1 if the env was created
   * prior to log version 15.
   */
  ...
  /**
   * Release resources and update memory budget. Should only be called when this dbtree is closed
   * and will never be accessed again.
   */
  public void close() {
  ...
  public long getTreeAdminMemory() {
    return idDatabase.getTreeAdminMemory() + nameDatabase.getTreeAdminMemory();
  }
  ...
  public int getLogSize() {
    return LogUtils.getLongLogSize()
        + // lastAllocatedLocalDbId
        LogUtils.getLongLogSize()
        + // lastAllocatedReplicatedDbId
